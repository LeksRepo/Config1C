...
Процедура ОбновитьПанельОтчетов(Знач Событие = "")
	...
	Элементы.ГруппаПоказыватьПодсказки.Видимость = РежимНастройки;
...
Процедура СохранитьПользовательскиеНастройки()
	...
	Колонки = "Ссылка, Подсистема, Видимость, БыстрыйДоступ";
	ИзмененныеНастройки = ДобавленныеВарианты.Выгрузить(Отбор, Колонки);
	Если ИзмененныеНастройки.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИнициализироватьПараметрыЗаполнения(ПараметрыЗаполнения)
	...
	ПараметрыЗаполнения.Вставить("ГруппыИнтерфейса", Новый Массив);
	ПараметрыЗаполнения.ГруппыИнтерфейса.Добавить("БыстрыйДоступ");
	ПараметрыЗаполнения.ГруппыИнтерфейса.Добавить("БезГруппы");
	ПараметрыЗаполнения.ГруппыИнтерфейса.Добавить("СГруппой");
	ПараметрыЗаполнения.ГруппыИнтерфейса.Добавить("СмТакже");
	
	Для Каждого ИмяГруппы Из ПараметрыЗаполнения.ГруппыИнтерфейса Цикл
...
Процедура НайтиВариантыОтчетовДляВывода(ПараметрыЗаполнения)
	...
	|			ИЛИ ВариантыРазмещение.РазделИлиГруппа В (&ПодсистемыНайденныеПоиском))
	...
	|	И ВариантыРазмещение.РазделИлиГруппа В(&МассивПодсистем)
	...
	КонецЕсли;
	Если РежимНастройки И ЗначениеЗаполнено(ОтборАвтор) Тогда
		ПараметрыПоиска.Вставить("Автор", ОтборАвтор);
	КонецЕсли;
	Если ТолькоТекущийРаздел Тогда
...
Процедура ВывестиВариантыРаздела(ПараметрыЗаполнения, РазделСсылка)
	...
	Если ПараметрыЗаполнения.ВариантыКоличество = 0 Тогда
		...
		Если ПараметрыЗаполнения.ВыводятсяВариантыТекущегоРаздела Тогда
			...
			Если ЗначениеЗаполнено(СтрокаПоиска) Или ЗначениеЗаполнено(ОтборАвтор) Тогда
				Если ПараметрыЗаполнения.ТолькоТекущийРаздел Тогда
...
Процедура ОпределитьГруппыИДекорацииДляВыводаВариантов(ПараметрыЗаполнения)
	...
	ЗаголовокРаздела.Шрифт       = Новый Шрифт(ШрифтЖирный, , 10);
	ЗаголовокРаздела.ЦветТекста  = ЦветГруппыВариантовОтчетов;
	...
	Элементы[ПараметрыЗаполнения.Префикс + "БыстрыйДоступШапкаНадпись"].РасширеннаяПодсказка.Заголовок = "";
КонецПроцедуры
...
&НаСервере
Процедура ВывестиВариантыБезГруппы(ПараметрыЗаполнения)
	...
	ГруппаУровня2 = Элементы.Найти(ПараметрыЗаполнения.Префикс + ПараметрыЗаполнения.ИмяГруппы);
	ВБыстрыйДоступ = (ПараметрыЗаполнения.ИмяГруппы = "БыстрыйДоступ");
	ВКореньПодсистемы = (ПараметрыЗаполнения.ИмяГруппы = "БезГруппы");
	
	Если РежимНастройки Тогда
		ПараметрыЗаполнения.КонтекстноеМеню.УбратьИзБыстрогоДоступа.Видимость = ВБыстрыйДоступ;
		ПараметрыЗаполнения.КонтекстноеМеню.ПереместитьВБыстрыйДоступ.Видимость = Не ВБыстрыйДоступ;
	КонецЕсли;
	...
	Для Каждого Вариант Из Варианты Цикл
		...
		ДобавитьЭлементыВариантаОтчета(ПараметрыЗаполнения, Вариант, ГруппаУровня3);
...
Процедура ВывестиВариантыСГруппой(ПараметрыЗаполнения)
	...
	ПараметрыЗаполнения.Вставить("Рекурсия", Рекурсия);
	//ПараметрыЗаполнения.Вставить("ВариантыСГруппой", Варианты);
	
	// Заполнение порядка вывода по данным дерева распределения.
	ЗаполнитьПорядокВывода(ПорядокВывода, КорневаяСтрока, ПараметрыЗаполнения);
...
&НаСервере
Процедура ЗаполнитьПорядокВывода(ПорядокВывода, СтрокаРодителя, ПараметрыЗаполнения)
	
	Рекурсия = ПараметрыЗаполнения.Рекурсия;
	...
	Для Каждого СтрокаДерева Из СтрокаРодителя.Строки Цикл
		...
		Если Не Рекурсия.ЭтоПоследняяКолонка И Рекурсия.СвободноСтрок <= 0 Тогда // текущая колонка исчерпана
			...
			Пока ТекущийРодитель.ПодсистемаСсылка <> ПараметрыЗаполнения.РазделСсылка Цикл
		...
		ЗаполнитьПорядокВывода(ПорядокВывода, СтрокаДерева, ПараметрыЗаполнения);
...
Функция ДобавитьГруппуПодсистемы(ПараметрыЗаполнения, СтрокаПорядкаВывода, ВГруппу)
	...
	ШрифтЗаголовка = Новый Шрифт(ШрифтЖирный, , 8 + 2*УвеличениеРазмераШрифта);
...
Процедура ОпределитьСоставПодсказкиВарианта(ПараметрыЗаполнения, Вариант, СоставПодсказки, Надпись)
	...
	Если ПараметрыЗаполнения.ИспользоватьПодсветку Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ПоказыватьОписание = (ПоказыватьПодсказки <> 0 И Не ПустаяСтрока(Вариант.Описание));
	Если Не ОписаниеВыведено И ПоказыватьОписание Тогда
		Если СоставПодсказки.Количество() > 0 Тогда
			СоставПодсказки.Добавить(Символы.ПС);
		КонецЕсли;
		СоставПодсказки.Добавить(СокрЛП(Вариант.Описание));
	КонецЕсли;
КонецПроцедуры
