...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.КлючНазначенияИспользования = "Раздел_" + Строка(РазделСсылка.УникальныйИдентификатор());
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	Если РежимНастройки Тогда
		ТекстВопроса = ТекстВопросаОСохраненииИзменений();
		Обработчик = Новый ОписаниеОповещения("СохранитьИЗакрыть", ЭтотОбъект);
...
Процедура Настроить(Команда)
	Если РежимНастройки И Модифицированность Тогда
		ТекстВопроса = ТекстВопросаОСохраненииИзменений();
		ЗаголовокВопроса = СтрЗаменить(Заголовок, "*", "");
		Обработчик = Новый ОписаниеОповещения("НастроитьЗавершение", ЭтотОбъект);
		ПоказатьВопрос(Обработчик, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена, 60, КодВозвратаДиалога.Да, ЗаголовокВопроса);
	Иначе
...
&НаКлиенте
Функция ТекстВопросаОСохраненииИзменений()
	Возврат НСтр("ru = 'Настройки расположения отчетов были изменены.
	|Сохранить изменения?'");
КонецФункции
...
&НаКлиенте
Процедура НастроитьЗавершение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда
		СохранитьИзменения(Неопределено);
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
		ОтменитьИзменения(Неопределено);
	КонецЕсли;
КонецПроцедуры
