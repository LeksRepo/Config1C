...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЭтоНазначаемыеОбработки Тогда
		...
		ЭтоФормаОбъекта = ДополнительныеОтчетыИОбработкиПовтИсп.ЭтоФормаОбъекта(ПолноеИмяОбъектаМетаданных, Параметры.ИмяФормы);
...
Процедура ВыполнитьОткрытиеФормыОбработки()
	...
	#Если ТолстыйКлиентОбычноеПриложение Тогда
		ВнешняяОбработка = ДополнительныеОтчетыИОбработкиВызовСервера.ПолучитьОбъектВнешнейОбработки(ВыполняемаяОбработка.Ссылка);
		...
		Если ФормаОбработки = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				...
				Строка(ВыполняемаяОбработка.Ссылка),
