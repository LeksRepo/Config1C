...
Процедура ОбновитьНастройкиСинхронизацииДанных()
	...
	Если КоличествоПроблем > 0 Тогда
		Элементы.ОбщаяКомандаОткрытьРезультатыОбменаДанными.Картинка = БиблиотекаКартинок.Предупреждение;
		Элементы.ОбщаяКомандаОткрытьРезультатыОбменаДанными.Заголовок =
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Проблемы: %1'"), Формат(КоличествоПроблем)
		);
	Иначе
		Элементы.ОбщаяКомандаОткрытьРезультатыОбменаДанными.Картинка = Новый Картинка;
		Элементы.ОбщаяКомандаОткрытьРезультатыОбменаДанными.Заголовок = НСтр("ru = 'Проблем нет'");
	КонецЕсли;
