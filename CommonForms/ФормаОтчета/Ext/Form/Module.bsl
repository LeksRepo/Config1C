...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(НовыеПользовательскиеНастройкиКД)
	// Вызов переопределяемого модуля
	ФормаОтчетаПереопределяемый.ПередЗагрузкойПользовательскихНастроекНаСервере(ЭтотОбъект, НовыеПользовательскиеНастройкиКД);
...
Процедура СброситьНастройки(Команда)
	Если ВариантМодифицирован = Истина Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Перед сбросом пользовательских настроек необходимо сохранить вариант отчета.'"));
		Возврат;
	КонецЕсли;
...
Процедура ВидимостьДоступностьКорректность(Изменения = "")
	...
	Если Изменения = "" ИЛИ Изменения = "СкрытьПоказатьБыстрыеНастройки" Тогда
		...
		Если ЕстьБыстрыеНастройки Тогда
			...
			Если ТипЗнч(ВидимостьБыстрыхНастроек) = Тип("Булево") Тогда
				// Видимость определена пользователем.
				ПоказыватьГруппуБыстрыхНастроек = ВидимостьБыстрыхНастроек;
			Иначе
				// По умолчанию: для отчета - Истина (Показывать), для расшифровки - Ложь (Не показывать).
				ПоказыватьГруппуБыстрыхНастроек = Не РежимРасшифровки;
			КонецЕсли;
		Иначе
			// Нет быстрых настроек - нечего показывать.
			ПоказыватьГруппуБыстрыхНастроек = Ложь;
		КонецЕсли;
		
		// Применение параметров отображения.
		Элементы.НастроитьОтчет.Видимость = ЕстьМедленныеНастройки;
		
		Элементы.КомандыВариантовПользовательскихНастроек.Видимость = ПоказыватьКомандыВариантовОтчетов И ЕстьМедленныеНастройки;
		
		Элементы.СкрытьПоказатьБыстрыеНастройки.Видимость = ЕстьБыстрыеНастройки;
		Элементы.СкрытьПоказатьБыстрыеНастройки.Пометка   = ПоказыватьГруппуБыстрыхНастроек;
		Элементы.БыстрыеНастройки.Видимость               = ПоказыватьГруппуБыстрыхНастроек;
		Если ПоказыватьГруппуБыстрыхНастроек Тогда
...
Процедура БыстрыеНастройкиЗаполнить()
	...
	// Вызов переопределяемого модуля
	ФормаОтчетаПереопределяемый.ПослеЗаполненияПанелиБыстрыхНастроек(ЭтотОбъект);
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправленияИЗагрузитьЗначения()
	...
	// Регистрация старых реквизитов к удалению.
