...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	СуффиксФормыИсточникаИзменений = ИсточникВыбора.ИмяФормы;
	ПозицияТочки = СтрДлина(СуффиксФормыИсточникаИзменений);
	Пока Сред(СуффиксФормыИсточникаИзменений, ПозицияТочки, 1) <> "." Цикл
		ПозицияТочки = ПозицияТочки - 1;
	КонецЦикла;
	СуффиксФормыИсточникаИзменений = ВРег(Сред(СуффиксФормыИсточникаИзменений, ПозицияТочки + 1));
	
	Если СуффиксФормыИсточникаИзменений = ВРег("ВыборПроизвольногоПериода") Тогда
		ЗагрузитьРезультатВыбораПериодаИзОтдельнойФормы(ВыбранноеЗначение);
		Возврат;
	ИначеЕсли СуффиксФормыИсточникаИзменений = ВРег("ФормаНастроекОтчета")
		Или СуффиксФормыИсточникаИзменений = ВРег("ФормаНастроек") Тогда
		ИзмененияИзФормыВариантаОтчета = Ложь;
	ИначеЕсли СуффиксФормыИсточникаИзменений = ВРег("ФормаВариантаОтчета")
		Или СуффиксФормыИсточникаИзменений = ВРег("ФормаВарианта") Тогда
...
Процедура Подключаемый_ЭлементОтбораКомпоновкиДанных_Таблица_Подбор(Команда)
	...
	ПараметрыФормыВыбора.Вставить("ВыборГруппИЭлементов",          ФункцииОтчетовКлиентСервер.ПривестиЗначениеКТипуИспользованиеГруппИЭлементов(ЭлементЗначение.ВыборГруппИЭлементов));
...
Процедура ВыбратьПериодВОтдельнойФорме(ЭлементИлиКоманда)
	...
	ПараметрыФормы.Вставить("ИмяЭлемента",       ЭлементИлиКоманда.Имя);
	ПараметрыФормы.Вставить("РежимВыбора",       Истина);
	
	ОткрытьФорму("ОбщаяФорма.ВыборПроизвольногоПериода", ПараметрыФормы, ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ЗагрузитьРезультатВыбораПериодаИзОтдельнойФормы(РезультатВыбора)
	
	Если ТипЗнч(РезультатВыбора) <> Тип("Структура") Тогда
		Возврат;
	КонецЕсли;
	
	Настройки = СкомпоноватьНастройкиЭлемента(РезультатВыбора.ИмяЭлемента, Ложь, Ложь);
	...
	Если НачалоПериода = Настройки.Пользовательская.Значение.ДатаНачала
		И КонецПериода = Настройки.Пользовательская.Значение.ДатаОкончания Тогда
		Возврат;
	КонецЕсли;
...
Процедура БыстрыеНастройкиСоздатьЭлементыУправления()
	...
	Если ОбщегоНазначенияКлиентСервер.ЭтоПлатформа83БезРежимаСовместимости() Тогда
		ИмяКнопкиСпискаВыбора = "КнопкаВыпадающегоСписка";
	Иначе
		ИмяКнопкиСпискаВыбора = "КнопкаСпискаВыбора";
	КонецЕсли;
	...
	ШаблонГруппыСФлажками.Вставить("Отображение", ОбщегоНазначенияКлиентСервер.ОтображениеОбычнойГруппыЛиния());
	...
	ШаблонПоляВводаПроизвольногоПериода.Вставить(ИмяКнопкиСпискаВыбора, Ложь);
	...
	Для Каждого ПользовательскаяНастройка Из ПользовательскиеНастройкиКД.Элементы Цикл
		...
		Иначе
			...
			ИначеЕсли ТипОбщейНастройки = Тип("НастройкиВложенногоОбъектаКомпоновкиДанных") Тогда
				ДоступныйОбъектНастройкиКД = НастройкиКД.ДоступныеОбъекты.Элементы.Найти(ОбщаяНастройка.ИдентификаторОбъекта);
				Если ДоступныйОбъектНастройкиКД <> Неопределено Тогда
					ЭлементЗаголовок = ДоступныйОбъектНастройкиКД.Заголовок;
				КонецЕсли;
				Если ПустаяСтрока(ЭлементЗаголовок) Тогда
					ВложеннаяСхема = РеквизитФормыВЗначение("Отчет").СхемаКомпоновкиДанных.ВложенныеСхемыКомпоновкиДанных.Найти(ОбщаяНастройка.ИдентификаторОбъекта);
					Если ВложеннаяСхема <> Неопределено Тогда
						ЭлементЗаголовок = ВложеннаяСхема.Заголовок;
					КонецЕсли; 
				КонецЕсли;
				Если ПустаяСтрока(ЭлементЗаголовок) Тогда
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЭлементОтбораКомпоновкиДанных") Тогда
			...
			Если ЭтоГрупповойВидСравнения Тогда
				...
				Если ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии
					Или ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
					КолонкаЗначение.ВыборГруппИЭлементов = ГруппыИЭлементы.Группы;
				Иначе
					КолонкаЗначение.ВыборГруппИЭлементов = ГруппыИЭлементы.ГруппыИЭлементы;
				КонецЕсли;
			...
			Иначе
				...
				Если ПользовательскаяНастройка.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии
					...
					ЗначениеОтбора.ВыборГруппИЭлементов = ГруппыИЭлементы.Группы;
				Иначе
		...
		ИначеЕсли ТипОбщейНастройки = Тип("ЗначениеПараметраНастроекКомпоновкиДанных") Тогда
			...
			Если ТипЗнч(ПользовательскаяНастройка.Значение) = Тип("СтандартныйПериод") Тогда
				...
				Иначе
					ВидПериода[ИмяКнопкиСпискаВыбора] = Истина;
					Период[ИмяКнопкиСпискаВыбора] = Истина;
		...
		ИначеЕсли ТипОбщейНастройки = Тип("СтруктураНастроекКомпоновкиДанных") Тогда
			...
			ГруппаГруппировки.Отображение         = ОбщегоНазначенияКлиентСервер.ОтображениеОбычнойГруппыЛиния();
