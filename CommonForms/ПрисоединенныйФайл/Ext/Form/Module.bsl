...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПерсональныеНастройкиРаботыСЭП = ЭлектроннаяПодписьКлиентСервер.ОбщиеНастройки();
...
Процедура ПроверитьЭП(Команда)
	...
	Если ПерсональныеНастройкиРаботыСЭП.ВыполнятьПроверкуЭПНаСервере Тогда
...
Процедура СоздатьЭлементыФормы()
	...
	Элемент = ДобавитьПолеОбъектаНаФорму(
		"Наименование", Элементы.ГруппаОбщаяИнформация, Истина, НСтр("ru = 'Имя файла'"), Ложь);
	
	Элемент = ДобавитьПолеОбъектаНаФорму(
		"Описание",     Элементы.ГруппаОбщаяИнформация, Истина, , Ложь);
	
	ДобавитьПолеОбъектаНаФорму("Автор",       Элементы.ГруппаСведенияЧасть1, , НСтр("ru = 'Автор'"), , , ,
		НСтр("ru = 'Пользователь, который добавил файл в базу.'"));
		
	ДобавитьПолеОбъектаНаФорму("Изменил",     Элементы.ГруппаСведенияЧасть1, , НСтр("ru = 'Отредактировал'"), , , ,
		НСтр("ru = 'Последний пользователь, который отредактировал файл.'"));
	
	ДобавитьПолеОбъектаНаФорму("Расширение",  Элементы.ГруппаСведенияЧасть1, , НСтр("ru = 'Расширение'"), , , ,
		НСтр("ru = 'Расширение файла.'"));
	
	ДобавитьПолеОбъектаНаФорму("Редактирует", Элементы.ГруппаСведенияЧасть1, , НСтр("ru = 'Редактирует'"), , , ,
		НСтр("ru = 'Пользователь, который занял файл для редактирования.'"));
	
	ДобавитьПолеОбъектаНаФорму("ДатаСоздания", Элементы.ГруппаСведенияЧасть2, , НСтр("ru = 'Создан'"), , , ,
		НСтр("ru = 'Дата добавления файла в базу.'"));
	
	ДобавитьПолеНаФорму("ДатаМодификации",     Элементы.ГруппаСведенияЧасть2, , НСтр("ru = 'Данные от'"), , , ,
		НСтр("ru = 'Дата изменения данных файла.'"));
	
	ДобавитьПолеОбъектаНаФорму("Размер",       Элементы.ГруппаСведенияЧасть2, , НСтр("ru = 'Размер (байт)'"), ,     , 11,
		НСтр("ru = 'Размер данных файла.'"));
...
Процедура ПрочитатьЭП()
	...
	Попытка
		МенеджерКриптографии = Новый МенеджерКриптографии(
			ПерсональныеНастройкиРаботыСЭП.ПровайдерЭП,
			ПерсональныеНастройкиРаботыСЭП.ПутьМодуляКриптографии,
			ПерсональныеНастройкиРаботыСЭП.ТипПровайдераЭП);
	Исключение
		Возврат;
	КонецПопытки;
	
	ЗаполнитьЗначенияСвойств(МенеджерКриптографии, ПерсональныеНастройкиРаботыСЭП);
...
&НаСервере
Процедура ДобавитьПолеНаФорму(Знач ИмяРеквизита,
                              Знач Контейнер = Неопределено,
                              Знач ЗаголовокСверху = Ложь,
                              Знач Заголовок = "",
                              Знач ТолькоПросмотр = Истина,
