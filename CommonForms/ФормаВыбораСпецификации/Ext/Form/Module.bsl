...
Процедура ЗаполнитьТаблицуСпецификаций(Подразделение, Статус, ПереданнаяТаблица = Неопределено)
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	СтатусСпецификацииСрезПоследних.Спецификация.Номер";
	
	Если рУпорядочитьПоДате Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "СтатусСпецификацииСрезПоследних.Спецификация.Номер", "СтатусСпецификацииСрезПоследних.Спецификация.ДатаИзготовления");
	КонецЕсли;
