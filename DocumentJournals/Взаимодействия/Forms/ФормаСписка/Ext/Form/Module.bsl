...
&НаКлиенте
Процедура ТолькоЗначимыеПредметыПриИзменении(Элемент)
...
&НаКлиенте
Процедура ТолькоЗначимыеКонтактыПриИзменении(Элемент)
	...
	Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			ИзменитьОтборСписок("Контакты",Новый Структура("Значение,ОписаниеТипов",
			                    ТекущиеДанные.Контакт, Неопределено), ИмяТекущейСтраницы);
	...
	ИначеЕсли Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаПредмет Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			ИзменитьОтборСписок("Предметы",Новый Структура("Значение,ОписаниеТипов",
			                    ТекущиеДанные.Предмет, Неопределено), ИмяТекущейСтраницы);
...
Процедура УправлениеВидимостьюПриПереключенииПанелиНавигации();
	...
	Элементы.ПредметыПанельНавигацииНастройкаСписка.Видимость = ЭтоПредметы;
	Элементы.ОбновитьПредметыПанелиНавигации.Видимость        = ЭтоПредметы;
	Элементы.НайтиПредметыПанельНавигации.Видимость           = ЭтоПредметы;
	Элементы.ОтменитьПоискПредметыПанельНавигации.Видимость   = ЭтоПредметы;
	
	Элементы.КонтактыПанельНавигацииНастройкаСписка.Видимость = ЭтоКонтакты;
	Элементы.ОбновитьКонтактыПанелиНавигации.Видимость        = ЭтоКонтакты;
	Элементы.НайтиКонтактыПанельНавигации.Видимость           = ЭтоКонтакты;
	Элементы.ОтменитьПоискКонтактыПанельНавигации.Видимость   = ЭтоКонтакты;
...
Процедура ЗаполнитьПанельКонтактов()
	...
	Если ТолькоЗначимыеКонтакты Тогда
		...
		ОтборПриемник = КонтактыПанельНавигации.КомпоновщикНастроек.ФиксированныеНастройки.Отбор;
		ОтборИсточник = Список.КомпоновщикНастроек.ФиксированныеНастройки.Отбор;
	...
	Иначе
		...
		|					ПО
		|						КонтактыВзаимодействий.Взаимодействие = ЖурналДокументовВзаимодействия.Ссылка
		|					ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПредметыПапкиВзаимодействий КАК ПредметыВзаимодействий
		|					ПО
		|						КонтактыВзаимодействий.Взаимодействие = ПредметыВзаимодействий.Взаимодействие)";
...
Процедура ПослеЗаполненияПанелиНавигации()
	...
	Если Элементы.СтраницыПанелиНавигации.ТекущаяСтраница = Элементы.СтраницаКонтакт Тогда
		...
		Элементы.КонтактыПанелиНавигации.ТекущаяСтрока = РегистрыСведений.СостоянияКонтактовВзаимодействий.СоздатьКлючЗаписи(Новый Структура("Контакт", ЗначениеНастроек.ТекущееЗначение));
			...
			ИзменитьОтборСписок("Контакты",Новый Структура("Значение,ОписаниеТипов",
				ЗначениеНастроек.ТекущееЗначение, Неопределено), ИмяТекущейСтраницы);
...
Процедура СпозиционироватьсяНаСтрокеСогласноСохраненномуЗначению(ЗначениеТекущейСтроки,
	...
	Если ЗначениеТекущейСтроки <> Неопределено Тогда
		Если ИмяЭлемента <> "Категории" Тогда
			ИдентификаторНайденнойСтроки = НайтиСтрокуВДанныхФормыДерево(ЭтаФорма[ИмяЭлемента],
				ЗначениеТекущейСтроки,"Значение",Истина);
		Иначе
			ИдентификаторНайденнойСтроки = НайтиСтрокуВДанныхФормыКоллекция(ЭтаФорма[ИмяЭлемента],
...
Процедура ОтложитьРассмотрение(ДатаОтработки, Знач ДанныеДляОбработки)
	...
	Если ДанныеДляОбработки <> Неопределено Тогда
		...
		МассивВзаимодействий = ДанныеДляОбработки;
	...
	Для Каждого Взаимодействие Из МассивВзаимодействий Цикл
		Реквизиты = ВзаимодействияКлиентСервер.СтруктураРеквизитовВзаимодействияДляЗаписи(,,, ДатаОтработки, Ложь);
