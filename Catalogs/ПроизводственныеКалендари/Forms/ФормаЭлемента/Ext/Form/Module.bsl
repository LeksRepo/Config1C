...
Процедура ПеренестиДень(Команда)
	...
	ДатаНазначения = ОткрытьФормуМодально("ОбщаяФорма.ВыборДаты", ПараметрыВыбораДаты);
	
	Если ДатаНазначения <> Неопределено Тогда
		ПеренестиВидДня(ДатаПереноса, ДатаНазначения);
		Элементы.Календарь.Обновить();
	КонецЕсли;
...
Процедура Печать(Команда)
	...
	Если ОбщегоНазначенияКлиентСервер.ПодсистемаСуществует("СтандартныеПодсистемы.Печать") Тогда
		Модуль = ОбщегоНазначенияКлиентСервер.ОбщийМодуль("УправлениеПечатьюКлиент");
		Модуль.ВыполнитьКомандуПечати("Справочник.ПроизводственныеКалендари", "ПроизводственныйКалендарь", 
