...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ВладелецСпецификация = ТипЗнч(Объект.ВладелецФайла) = Тип("ДокументСсылка.Спецификация");
	ВладелецСлужебнаяЗаписка = ТипЗнч(Объект.ВладелецФайла) = Тип("ДокументСсылка.СлужебнаяЗаписка"); 
	
	Если ЭтоДилер И (НЕ ВладелецСпецификация И НЕ ВладелецСлужебнаяЗаписка) Тогда
		Отказ = Истина;
		Возврат;
	КонецЕсли;
...
Процедура ЗаполнитьВидыФайла()
	...
	Если ЗначениеЗаполнено(Объект.ВладелецФайла) Тогда
		...
		ИначеЕсли ТипВладельца = Тип("СправочникСсылка.КаталогИзделий") Тогда
			...
			МассивДоступныхВидовФайла.Добавить(Перечисления.ВидыПрисоединенныхФайлов.ИсходныйФайл);
		...
		ИначеЕсли ТипВладельца = Тип("ДокументСсылка.Договор") Тогда
			
			МассивДоступныхВидовФайла.Добавить(Перечисления.ВидыПрисоединенныхФайлов.СканДокумента);	
				
		ИначеЕсли ТипВладельца = Тип("ДокументСсылка.Обращение") Тогда
