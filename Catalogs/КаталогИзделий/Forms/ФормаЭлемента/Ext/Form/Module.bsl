...
Процедура ОтобразитьКартинку() 
	...
	Если ЗначениеЗаполнено(Объект.Код) Тогда
		...
		Иначе
			ИмяФайла = РабочийКаталог + "Изделие" + ПредопределенноеЗначение("Перечисление.ВидыПрисоединенныхФайлов.КартинкаПравая") + Объект.Код;
			ФайлИзображения = Новый Файл(ИмяФайла);
			Если ФайлИзображения.Существует() Тогда
				ПоместитьФайл(АдресВХранилище, ИмяФайла, , Ложь, ЭтаФорма.УникальныйИдентификатор);
				Изображение  = АдресВХранилище;
			Иначе
				ИмяФайла = РабочийКаталог + "Изделие" + ПредопределенноеЗначение("Перечисление.ВидыПрисоединенныхФайлов.КартинкаЛевая") + Объект.Код;
				ФайлИзображения = Новый Файл(ИмяФайла);
				Если ФайлИзображения.Существует() Тогда
					ПоместитьФайл(АдресВХранилище, ИмяФайла, , Ложь, ЭтаФорма.УникальныйИдентификатор);
					Изображение  = АдресВХранилище;	
...
Процедура ВидимостьНастроек()
	...
	Элементы.НаВсеИзделие.Доступность = КорневойВид = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.Крыша");
	
	//Элементы.УчитыватьПол.Доступность = ;
	
	Элементы.НеВлияетНаОсновной.Доступность = (КорневойВид <> ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.КухняНижний")
	И КорневойВид <> ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.КухняВерхний")
	И КорневойВид <> ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.ОсновнойЭлемент"));
	
	Элементы.Угловой.Доступность = (КорневойВид = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.КухняНижний")
	ИЛИ КорневойВид = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.КухняВерхний"));
