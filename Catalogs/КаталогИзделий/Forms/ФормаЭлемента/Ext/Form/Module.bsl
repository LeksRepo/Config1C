...
Функция ОткрытьФормуПодбора(ИмяПодбора, ТаблицаДляФормы, СтрокаДанных, ВладелецПодобра)
	...
	Если ТипЗнч(ТаблицаДляФормы) = Тип("Структура") Тогда
		...
		ПараметрыПодбора.Вставить("КоординатыПрисадок", АдресаТаблиц.КоординатыПрисадок);
...
Функция ВыгрузитьТаблицыДеталиПрисадкиВХранилище()
	...
	Стр.Вставить("КоординатыПрисадок", ПоместитьВоВременноеХранилище(Объект.СписокКоординатПрисадок.Выгрузить()));
...
Процедура ЗагрузитьТаблицыДеталиПрисадки(Адреса)
	...
	ТЗ = ПолучитьИзВременногоХранилища(Адреса.КоординатыПрисадок);
	Объект.СписокКоординатПрисадок.Загрузить(ТЗ);
...
Процедура ИзменитьСостав(Команда)
	...
	Если ИмяТекущейСтраницы = "СтраницаМатериалы" Тогда
		...
		ТаблицаДляФормы.Вставить("КоординатыПрисадок", Объект.СписокКоординатПрисадок);
