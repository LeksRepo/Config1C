...
Функция ПолучитьАдресХранилища()
	...
	Стр.Вставить("КоординатыПрисадок", ПоместитьВоВременноеХранилище(КоординатыПрисадок.Выгрузить()));
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	Если Данные <> Неопределено Тогда
		...
		УстановитьОтборТаблицаКоординатыПрисадок(Данные.ИдентификаторПрисадок);
...
&НаКлиенте
Функция УстановитьОтборТаблицаКоординатыПрисадок(Инд)
	
	ОтборСтрок = Новый ФиксированнаяСтруктура("ИдентификаторПрисадок", Инд);
	Элементы.КоординатыПрисадок.ОтборСтрок = ОтборСтрок;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КоординатыПрисадок.Загрузить(ПолучитьИзВременногоХранилища(Параметры.КоординатыПрисадок));
...
Процедура УдалитьСтрокиПрисадки(Инд)
	...
	Строки = КоординатыПрисадок.НайтиСтроки(Новый Структура("ИдентификаторПрисадок", Инд));
		
	Для каждого Строка Из Строки Цикл
	 	КоординатыПрисадок.Удалить(Строка);
	КонецЦикла;
...
Процедура ДеталиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование Тогда
		...
		Строки = КоординатыПрисадок.НайтиСтроки(Отбор);
		
		Для Каждого Стр ИЗ Строки Цикл
			
			НоваяПрисадка = КоординатыПрисадок.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяПрисадка,Стр);
			НоваяПрисадка.ИдентификаторПрисадок = НовыйИдентификатор;
...
&НаКлиенте
Процедура КоординатыПрисадокЗначениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	ОткрытьРедакторФормул(Элемент.ТекстРедактирования, Элемент, "Присадки");
КонецПроцедуры

&НаКлиенте
Процедура КоординатыПрисадокПриАктивизацииСтроки(Элемент)
	
	Данные = Элементы.КоординатыПрисадок.ТекущиеДанные;
