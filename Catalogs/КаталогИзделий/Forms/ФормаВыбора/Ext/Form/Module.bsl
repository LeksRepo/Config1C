...
Функция СкопироватьПараметрыФормы(Параметры)
	...
	Если Параметры.Свойство("НазваниеИзделия") И ЗначениеЗаполнено(Параметры.НазваниеИзделия) Тогда
		
		Пар.Вставить("НазваниеИзделия", Параметры.НазваниеИзделия);
...
Процедура ЗаполнитьДеревоИзделий(Параметры, МассивПоиска)
	...
	Если Параметры.Свойство("НазваниеИзделия") И ЗначениеЗаполнено(Параметры.НазваниеИзделия) Тогда
		...
		НазваниеИзделия = Параметры.НазваниеИзделия;
		
		Если НазваниеИзделия.ДоступныеВидыИзделий.Количество() > 0 Тогда
			
			Для каждого Знч Из  НазваниеИзделия.ДоступныеВидыИзделий Цикл
