...
Функция СкопироватьПараметрыФормы(Параметры)
	...
	Если Параметры.Свойство("КухоннаяКоллекция") Тогда
		
		Пар.Вставить("КухоннаяКоллекция", Параметры.КухоннаяКоллекция);
...
Процедура ЗаполнитьДеревоИзделий(Параметры, МассивПоиска)
	...
	КухоннаяКоллекция = ?(Параметры.Свойство("КухоннаяКоллекция"),Параметры.КухоннаяКоллекция, NULL);	
	...
	Запрос.УстановитьПараметр("КухоннаяКоллекция", КухоннаяКоллекция);
	...
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &КухоннаяКоллекция ЕСТЬ NULL
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Каталог.КухоннаяКоллекция = &КухоннаяКоллекция
	|		КОНЕЦ
	|
	|СГРУППИРОВАТЬ ПО
	|	Каталог.Ссылка,
...
&НаКлиенте
Процедура РедактироватьИзделие(Команда)
	
	ТекущиеДанные = Элементы.ДеревоИзделий.ТекущиеДанные;
	ОткрытьЗначение(ТекущиеДанные.Ссылка);
	
КонецПроцедуры

&НаКлиенте
Процедура РедактироватьДопИзделие(Команда)
	
	ТекущиеДанные = Элементы.ДеревоДопИзделий.ТекущиеДанные;
	ОткрытьЗначение(ТекущиеДанные.Ссылка);
