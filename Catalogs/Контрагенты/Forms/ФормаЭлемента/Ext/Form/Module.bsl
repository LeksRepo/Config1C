...
Процедура ПерезаполнитьДанныеИзВебСервиса(Код)
	...
	Если ПолученныеДанные.Свойство("suggestions") И ПолученныеДанные.suggestions.Количество() > 0 Тогда
		...
		ПараметрыФормы.Вставить("Наименование", Данные.name.full + " " + Данные.opf.short);
		ПараметрыФормы.Вставить("ИНН", Данные.inn);
		ПараметрыФормы.Вставить("КПП", Данные.kpp);
		ПараметрыФормы.Вставить("ОГРН", Данные.ogrn);
		ПараметрыФормы.Вставить("ЮридическийАдрес", Данные.address.value);
		ПараметрыФормы.Вставить("ПочтовыйАдрес", Данные.address.value);
		ПараметрыФормы.Вставить("Руководитель", Данные.management.name);
		ПараметрыФормы.Вставить("ПолноеНаименование", Данные.name.full_with_opf);
