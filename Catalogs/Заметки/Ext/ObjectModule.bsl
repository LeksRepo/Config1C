...
Процедура ПередЗаписью(Отказ)
	...
	Если НЕ ЗначениеЗаполнено(Ссылка) И ТекстСодержания = "" Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Запрещено создавать пустые заметки.'"));
		Отказ = Истина;
		Возврат;
	КонецЕсли; 
	...
	Если Не ЭтоГруппа Тогда 
		...
		Если ТипЗнч(Предмет) = Тип("ДокументСсылка.Спецификация") И Предмет.Дилерский Тогда
			 Автор = Предмет.Автор;
		КонецЕсли;
...
Процедура ПриЗаписи(Отказ)
	...
	Справочники.Заметки.ОтправитьЗаметкуНаСайт(Ссылка);
