...
Функция СписокЭлементовГруппы(Группа, Папка = "", Спецификация, Доступность)
	...
	|		   КОГДА &Доступность 
	|		   ТОГДА ИСТИНА
	|	   	   ИНАЧЕ ЕСТЬNULL(НоменклатураПодразделений.Доступность, ЛОЖЬ) = ИСТИНА
	|	  КОНЕЦ	    
	|	И ВЫБОР
...
&НаКлиенте
Процедура ДоступностьНоменклатурыПриИзменении(Элемент)
	
	Элементы.НадписьПредоставитьСамостоятельно.Видимость = ДоступностьНоменклатуры;
