...
Процедура ПечатьФлэш()//Команда = "prtx"
	...
	Если ЭтаФорма.ТолькоПросмотр ИЛИ Записать() Тогда
		...
		НомерДвери = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Объект.Ссылка);
		СтрокаТаблицаНоменклатуры = ПолучитьТаблицуНоменклатурыНаПечать(Объект.Ссылка);
		
		ОбновитьФлэш("prtx☻" + ПримечаниеДляПечати + "☻" + СтруктураНомеров.Договор + Символы.ПС + Объект.Спецификация + Символы.ПС + "Дверь №"+НомерДвери + "☻" + СтрокаТаблицаНоменклатуры);
