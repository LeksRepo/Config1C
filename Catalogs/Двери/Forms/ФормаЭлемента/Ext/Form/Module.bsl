...
Функция СписокЭлементовГруппы(Группа, Папка = "", Спецификация, Профиль)
	...
	НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.НайтиПоНаименованию(Группа, Истина);
...
Функция ПолучитьДоступностьРазмеров(Спецификация)
	...
	Возврат НЕ Спецификация.Изделие.ЗапретИзмененияРазмераДверей;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПримечаниеДляПечати = СтрокиСообщений.ПримечаниеДвери();
...
&НаСервере
Процедура ЗаполнитьДоступныеНомГруппы()
	...
	|	НоменклатурныеГруппы.ТипТекстурыВДверях КАК Тип
	|ИЗ
	|	Справочник.НоменклатурныеГруппы КАК НоменклатурныеГруппы
	|ГДЕ
	|	НоменклатурныеГруппы.ТекстураВДверях
	|
	|УПОРЯДОЧИТЬ ПО
	|	Наименование";
	...
	Пока Выборка.Следующий() Цикл
		...
		ДоступныеНомГруппы = ДоступныеНомГруппы + Выборка.Наименование + "_" + Выборка.Тип + "|";  
		
	КонецЦикла;
	
	ДоступныеНомГруппы = Лев(ДоступныеНомГруппы, СтрДлина(ДоступныеНомГруппы) - 1); 
...
Процедура НаполнитьТекстуруФлэш()//Команда = "fill"
	...
	ОбновитьФлэш("fill☻"+ДоступныеНомГруппы);
