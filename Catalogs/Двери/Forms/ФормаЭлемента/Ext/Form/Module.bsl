...
Процедура ПечатьФлэш()//Команда = "prtx"
	...
	Если ЭтаФорма.ТолькоПросмотр ИЛИ Записать() Тогда
		...
		ОбновитьФлэш("prtx☻" + Примечание +"☻"+ ТекущаяДата()+ Символы.ПС + Символы.ПС);
	КонецЕсли;
...
Процедура НоменклатураКоличествоПриИзменении(Элемент)
	...
	Если (Данные.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ШлегельВенге")
		...
		ИЛИ Данные.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ШлегельХром")) Тогда
		
		КоличествоШлегель = Данные.Количество;
		Данные.Ширина = Данные.Количество * Объект.ВысотаПроема;
