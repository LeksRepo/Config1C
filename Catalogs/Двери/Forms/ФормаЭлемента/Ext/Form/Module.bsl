...
Функция ЗаполнитьПрофили()
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Подразделение", Объект.Спецификация.Подразделение);
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.АлюминПрофиль.Доступность = ЗначениеЗаполнено(Объект.Спецификация);
...
Функция ПроверитьПраваДляРедактированияПрофиля()
	ЕстьДоступ = РольДоступна("РедактированиеПрофиляДвери") ИЛИ РольДоступна("ПолныеПрава");
КонецФункции
...
Процедура ФлэшПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ИначеЕсли Команда = "prof2" Тогда // получена команда "mass"
...
Процедура ГруппыТекстурыФлэш(Группа)//Команда = "matg"
	...
	Если СтрДлина(Группа) > 0 Тогда
		ОбновитьФлэш("matg☻" + СписокЭлементовГруппы(Группа, ,Объект.Спецификация, Объект.АлюминПрофиль));
	КонецЕсли;
...
Процедура НаполнитьГравировкиФлэш()//Команда = "fil2"
	...
	ОбновитьФлэш("fil2☻" + СписокЭлементовГруппы("Гравировка", ,Объект.Спецификация, Объект.АлюминПрофиль));
...
Процедура ГруппыГравировкиФлэш(ПапкаГравировки)//Команда = "mat2"
	...
	ОбновитьФлэш("mat2☻" + СписокЭлементовГруппы("Гравировка", ПапкаГравировки, Объект.Спецификация, Объект.АлюминПрофиль));
