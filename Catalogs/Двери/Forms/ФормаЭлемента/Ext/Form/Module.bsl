...
Процедура ИнициализацияФлэш()//Команда = "init"
	...
	Если Объект.Количество = 1 И Объект.ВидДвери = ПредопределенноеЗначение("Перечисление.ВидыДверей.Раздвижная") Тогда
		Элементы.Трек.Доступность=Ложь;	
	КонецЕсли;
...
&НаКлиенте
Процедура УстановитьРазмерТрека()
	
	Если Объект.ВидДвери = ПредопределенноеЗначение("Перечисление.ВидыДверей.Раздвижная") Тогда
	
		Если  Объект.Количество < 2 Тогда
			
			Объект.Трек = Объект.ШиринаПроема;
			Элементы.Трек.Доступность = Ложь;
			
		Иначе

			Объект.Трек = 100;
			Элементы.Трек.Доступность = Истина;
...
Процедура МассивФлэш(СтрокаОтФлэш)//Команда = "mass"
	...
	Если КоличествоДверейИзменилось Тогда
		УстановитьРазмерТрека();
		ПриИзмененииПараметров();
	КонецЕсли;
