...
&НаСервереБезКонтекста
Функция СписокЭлементовГруппы(Группа, ПроизводственноеПодразделение)
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// { Васильев Александр Леонидович [22.04.2014]
	// это зачем здесь?
	// модуль объекта, обработка заполнения, там надо
...
Процедура ПрофильПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Профиль) Тогда
		СтруктураПрофиля = ПолучитьСтруктуруПрофиля(Объект.Профиль, Объект.ПроизводственноеПодразделение);
...
Процедура ПодразделениеПриИзмененииНаСервере()
	...
	// { Васильев Александр Леонидович [22.04.2014]
	// заменить функцией ЛексСервер.ОтборНоменклатурныхГрупп
	// } Васильев Александр Леонидович [22.04.2014]
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидПрофиля", Перечисления.ВидыПрофилей.Вертикальный);
	Запрос.УстановитьПараметр("ПроизводственноеПодразделение", Объект.ПроизводственноеПодразделение);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Процедура ГруппыТекстурыФлэш(Группа)//Команда = "matg"
	...
	Если СтрДлина(Группа) > 0 Тогда
		ОбновитьФлэш("matg☻" + СписокЭлементовГруппы(Группа, Объект.ПроизводственноеПодразделение));
	КонецЕсли;
...
Процедура НаполнитьГравировкиФлэш()//Команда = "fil2"
	...
	ОбновитьФлэш("fil2☻" + СписокЭлементовГруппы("Гравировка", Объект.ПроизводственноеПодразделение));
...
Процедура ГруппыГравировкиФлэш(ГруппаГравировки)//Команда = "mat2"
	...
	ОбновитьФлэш("mat2☻" + СписокЭлементовГруппы(ГруппаГравировки, Объект.ПроизводственноеПодразделение));
