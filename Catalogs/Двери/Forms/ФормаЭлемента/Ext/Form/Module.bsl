...
&НаСервереБезКонтекста
Функция СписокЭлементовГруппы(Группа, Папка = "", Спецификация, Доступность)
	...
	Запрос.УстановитьПараметр("Доступность", Доступность);
	...
	|	И спрНоменклатура.Базовый
	|	И НоменклатураПодразделений.Подразделение = &ПроизводственноеПодразделение
	|	И ЕСТЬNULL(НоменклатураПодразделений.Доступность, ЛОЖЬ) = &Доступность
	|	И ВЫБОР
	...
	Пока ВыборкаИтог.Следующий() Цикл
		...
		Иначе
			...
			Пока Выборка.Следующий() Цикл
				Строка = Строка + Выборка.Наименование + "♦" + СокрЛП(Выборка.Код) + "♦" + ПолучитьУменьшениеВставки(Выборка.ГлубинаДетали) + "☺";
			КонецЦикла;
		КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя() Тогда
		Элементы.ДоступнаяНоменклатура.Заголовок = "Показать НЕ ДОСТУПНУЮ номенклатуру";
		Элементы.НадписьПредоставитьСамостоятельно.Видимость = Ложь;
	Иначе
		Элементы.ГруппаДоступностьНоменклатуры.Видимость = Ложь;
	КонецЕсли;
...
Процедура ГруппыТекстурыФлэш(Группа)//Команда = "matg"
	...
	Если СтрДлина(Группа) > 0 Тогда
		ОбновитьФлэш("matg☻" + СписокЭлементовГруппы(Группа, ,Объект.Спецификация, ДоступностьНоменклатуры));
	КонецЕсли;
...
Процедура НаполнитьГравировкиФлэш()//Команда = "fil2"
	...
	ОбновитьФлэш("fil2☻" + СписокЭлементовГруппы("Гравировка", ,Объект.Спецификация, ДоступностьНоменклатуры));
...
Процедура ГруппыГравировкиФлэш(ПапкаГравировки)//Команда = "mat2"
	...
	ОбновитьФлэш("mat2☻" + СписокЭлементовГруппы("Гравировка", ПапкаГравировки, Объект.Спецификация, ДоступностьНоменклатуры));
...
&НаКлиенте
Процедура ДоступнаяНоменклатура(Команда)
	...
	Если ДоступностьНоменклатуры Тогда
		Элементы.ДоступнаяНоменклатура.Заголовок = "Показать НЕ ДОСТУПНУЮ номенклатуру";
	Иначе
		Элементы.ДоступнаяНоменклатура.Заголовок = "Показать ДОСТУПНУЮ номенклатуру";
	КонецЕсли;
	
	ОбновитьФлэш("nom☻1"); // Сбросить загруженные массивы, иначе флэшка не будет просить новые данные.
