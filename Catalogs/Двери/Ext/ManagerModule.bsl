...
Функция ПолучитьДанныеДляПечатиДвери(Данные,Примечание) Экспорт
	...
	Если РезультатНаряд.Количество() > 0 Тогда
		...
		Результат.Вставить("Наряд",ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РезультатНаряд.НарядНомер));
		...
		Результат.Вставить("ТаблицаХлысты", Документы.НарядЗадание.ПолучитьХлыстовойМатериалНаДвери(РезультатНаряд.Наряд,Данные.ТекущаяДверь));
