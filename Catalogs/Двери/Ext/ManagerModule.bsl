...
Функция ПолучитьДанныеДляПечатиДвери(Данные,Примечание) Экспорт
	...
	Результат.Вставить("ТаблицаНоменклатуры",Справочники.Двери.ПолучитьТаблицуНоменклатурыНаПечать(Данные.ТекущаяДверь));
	...
	Если РезультатНаряд.Количество() > 0 Тогда
		...
		Результат.Вставить("Наряд",ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(РезультатНаряд.Наряд));
		Результат.Вставить("ДатаИзготовления",Формат(РезультатНаряд.ДатаИзготовления,"ДЛФ=D"));
		Результат.Вставить("Очередь",РезультатНаряд.НомерСтроки);
