...
Процедура ПередЗаписью(Отказ)
	...
	//Не получается получить ссылку на спецуху если дверь не сохранена.
	//ОбщаяНоменклатура = ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуруПолностью(СпецификацияДвери.Подразделение);
	ОбщаяНоменклатура = Справочники.Номенклатура;
...
Процедура ЗаполнитьЗаливками(Заливки, Отказ, ОбщаяНоменклатура)
	...
	Для Каждого Элемент Из МассивЗаливки Цикл
		...
		Если ЗначениеЗаполнено(Параметр.ДополнительныйЭлемент) Тогда
			...
			Если ДопЭлемент = ОбщаяНоменклатура.ЛДСП8Белый Тогда
