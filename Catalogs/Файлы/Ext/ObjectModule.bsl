#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	Процедура ПередЗаписью(Отказ)
		...
		// { Васильев Александр Леонидович [03.02.2016]
		НовоеПолноеИмя = ЛексСервер.ПолучитьИмяПрисоединенногоФайла(ВидФайла, ВладелецФайла);
		Если ЗначениеЗаполнено(НовоеПолноеИмя) Тогда
			ПолноеНаименование = НовоеПолноеИмя;
		КонецЕсли;
		// } Васильев Александр Леонидович [03.02.2016]
		...
		Если СхемаОсновной() И ЕстьСхемаОсновной() Тогда
			ВызватьИсключение "Схема основной уже существует.";
		КонецЕсли;
		
		Если ПроверкаФайлаКрыши() Тогда
			ВызватьИсключение "Файл крыши для этой стороны уже есть";
		КонецЕсли;
	...
	// Проверяет является ли текущий файл основной схемой
	Функция СхемаОсновной()
		
		Осн=Ложь;
		
		Если ЭтотОбъект.ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ВзрывСхемаЛевая 
			ИЛИ ЭтотОбъект.ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ВзрывСхемаОсновная
			ИЛИ ЭтотОбъект.ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ВзрывСхемаПравая
			Тогда
			Осн=Истина;	
		КонецЕсли;
	...
	// Возвращает отказ записи элемента, если схема основной уже существует(основных схем может быть 2 элемента левый И правый) 
	Функция ЕстьСхемаОсновной()
		...
		|	Файлы.ВладелецФайла = &ВладелецФайла
		|	И Файлы.Ссылка <> &Ссылка
		|	И (Файлы.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ВзрывСхемаПравая)
		|			ИЛИ Файлы.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ВзрывСхемаЛевая)
		|			ИЛИ Файлы.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ВзрывСхемаОсновная))
		|	И НЕ Файлы.ПометкаУдаления";
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Если КоличествоОсновных = 2 
				...
				И (ЭтотОбъект.ВидФайла = Выборка.ВидФайла 
				ИЛИ ЭтотОбъект.ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ВзрывСхемаОсновная
				ИЛИ Выборка.ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ВзрывСхемаОсновная)
				)
				Тогда
				ЕстьОсновной = Истина;
			КонецЕсли;
