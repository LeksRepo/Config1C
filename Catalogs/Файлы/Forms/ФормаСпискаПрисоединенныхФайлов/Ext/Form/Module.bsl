...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если Не Копирование Тогда
		...
		Попытка
			...
			//RonEXI: Проверить количество картинок к номенклатуре
			Если ПроверитьКоличествоФайлов(ВладелецФайла.Значение) Тогда
				РаботаСФайламиКлиент.СозданиеНовогоФайла(ВладелецФайла.Значение, ЭтаФорма);
			Иначе
				Предупреждение("К Номенклатуре разрешено прикреплять максимум 2 файла. Редактируйте существующие файлы.");
			КонецЕсли;
...
Процедура СоздатьФайлВыполнить()
	...
	Исключение
		Предупреждение(ФайловыеФункцииСлужебныйКлиентСервер.ОшибкаСозданияНовогоФайла(ИнформацияОбОшибке()));
	КонецПопытки;
...
&НаСервере
Функция ПроверитьКоличествоФайлов(Владелец)
	...
	Если ТипЗнч(Владелец) = Тип("СправочникСсылка.Номенклатура") Тогда
		...
		|	И (Файлы.ВидФайла = Значение(Перечисление.ВидыПрисоединенныхФайлов.ТекстураНоменклатуры)
		|      ИЛИ Файлы.ВидФайла = Значение(Перечисление.ВидыПрисоединенныхФайлов.ВекторныйФайл))";
