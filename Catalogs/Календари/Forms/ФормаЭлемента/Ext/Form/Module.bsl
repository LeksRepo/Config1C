...
Процедура ШаблонЗаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	НомерДня = СтрокаШаблона.НомерСтроки;
	
	РасписаниеЗаполнено = Неопределено;
	РедактироватьРасписаниеДня(НомерДня, СтрокаШаблона.ПредставлениеРасписания, РасписаниеЗаполнено);
	
	Если РасписаниеЗаполнено <> Неопределено Тогда
		// если расписание заполнено - день включен в график
		СтрокаШаблона.ДеньВключенВГрафик = РасписаниеЗаполнено;
	КонецЕсли;
	
	СтрокаШаблона.ПредставлениеРасписания = ПредставлениеРасписанияДня(ЭтаФорма, НомерДня);
...
&НаКлиенте
Процедура РедактироватьРасписаниеДня(НомерДня, ПредставлениеРасписания, РасписаниеЗаполнено = Неопределено)
	
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РасписаниеРаботы", РасписаниеРаботы(НомерДня));
	ПараметрыФормы.Вставить("ТолькоПросмотр", ТолькоПросмотр);
	
	РезультатРедактирования = ОткрытьФормуМодально("Справочник.Календари.Форма.РасписаниеРаботы", ПараметрыФормы, ЭтаФорма);
	
	Если РезультатРедактирования = Неопределено Или ТолькоПросмотр Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОписаниеИнтервала Из РезультатРедактирования.РасписаниеРаботы Цикл
	...
	// если расписание заполнено - день включен в график
	РасписаниеЗаполнено = РезультатРедактирования.РасписаниеРаботы.Количество() > 0;
	ПредставлениеРасписания = ПредставлениеРасписанияДня(ЭтаФорма, НомерДня);
...
Процедура РасписаниеПредпраздничногоДняНажатие(Элемент, СтандартнаяОбработка)
	...
	РедактироватьРасписаниеДня(0, РасписаниеПредпраздничногоДня);
