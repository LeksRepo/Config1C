...
Процедура ФормаДереваАнкетыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если (Поле.Имя = "ДеревоАнкетыЗаметки") И (ТекущиеДанные.ТипСтроки <> "Корень") Тогда
		...
		ДополнительныеПараметры = Новый Структура("СтрокаРедактирования", ВыбраннаяСтрока);
		ОбработчикОповещения = Новый ОписаниеОповещения("РедактированиеЗаметокПриОкончании", ЭтотОбъект, ДополнительныеПараметры); 
...
Процедура ДобавитьСтраницуПревьюТабличногоВопроса(ТипТабличногоВопроса,СоставТабличногоВопроса,ПредопределенныеОтветы,КлючСтроки)
	...
	Анкетирование.ОбновитьПревьюТабличныйВопрос(СоставТабличногоВопроса,ПредопределенныеОтветы,ТипТабличногоВопроса,ЭтотОбъект,ИмяТаблицы,КлючСтроки);
...
Процедура ОбновитьИнформациюТабличныйВопрос(ТекущиеДанные)
	...
	ЭтотОбъект[ИмяРеквизита] = СформироватьСтрокуФормулировкиДляПревью(ТекущиеДанные);
...
&НаКлиенте
Процедура РедактированиеЗаметокПриОкончании(ИзмененныйТекст, ДополнительныеПараметры) Экспорт

	ДанныеСтроки =  Элементы.ФормаДереваАнкеты.ДанныеСтроки(ДополнительныеПараметры.СтрокаРедактирования);
	
	Если ДанныеСтроки.Заметки <> ИзмененныйТекст Тогда
		ДанныеСтроки.Заметки = ИзмененныйТекст;
		Модифицированность = Истина;
	КонецЕсли;
