...
Процедура ГруппыВнешнихПользователейПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если Не ГруппаПомеченаНаУдаление Тогда
		ДействиеСПользователем = ?(Перемещение, НСтр("ru = 'Переместить'"), НСтр("ru = 'Скопировать'"));
	Иначе
		ДействиеСПользователем = ?(Перемещение, НСтр("ru = 'Группа ""%1"" помечена на удаление. Переместить'"), 
			НСтр("ru = 'Группа ""%1"" помечена на удаление. Скопировать'"));
		ДействиеСПользователем = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ДействиеСПользователем, Строка(Строка));
	КонецЕсли;
