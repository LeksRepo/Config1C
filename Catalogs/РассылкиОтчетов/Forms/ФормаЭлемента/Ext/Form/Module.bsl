...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЗначенияИзменились
		...
		И (Объект.ТолькоУведомить ИЛИ НЕ Объект.ИспользоватьЭлектроннуюПочту)
		И Не ПараметрыЗаписи.Свойство("НеОтключатьАрхивацию") Тогда
		...
		Кнопки.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Продолжить'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		
		Обработчик = Новый ОписаниеОповещения("ПередЗаписьюПродолжение", ЭтотОбъект);
		ПоказатьВопрос(Обработчик, ТекстВопроса, Кнопки, 60, КодВозвратаДиалога.Да, ЗаголовокВопроса, КодВозвратаДиалога.Нет);
...
&НаКлиенте
Процедура ПередЗаписьюПродолжение(Ответ, ДополнительныеПараметры) Экспорт
	Если Ответ = КодВозвратаДиалога.Да Тогда // Отключить архивацию и записать.
		Объект.Архивировать = Ложь;
		Записать();
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда // Не отключать архивацию и записать.
		ПараметрыЗаписи = Новый Структура;
		ПараметрыЗаписи.Вставить("НеОтключатьАрхивацию");
