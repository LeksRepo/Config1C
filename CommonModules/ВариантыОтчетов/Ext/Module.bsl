...
Функция ДеревоНастроекВариантовОтчетовКонфигурации() Экспорт
	...
	ДеревоВариантов.Колонки.Добавить("ВыводитьТолькоОтчет",  Новый ОписаниеТипов("Булево"));
	...
	Для Каждого ОтчетМетаданные Из Метаданные.Отчеты Цикл
		...
		СтрокаОтчет.ВыводитьТолькоОтчет  = ВыводитьОтчетыВместоВариантов;
		...
		// Вариант по умолчанию (последний открытый).
		ВариантПоУмолчанию = СтрокаОтчет.Строки.Добавить();
		ВариантПоУмолчанию.Отчет        = СтрокаОтчет.Отчет;
		ВариантПоУмолчанию.КлючВарианта = "";
		ВариантПоУмолчанию.ЭтоВариант   = Истина;
	...
	// Переключение в режим "По вариантам" для отчетов, у которых включен только 1 вариант.
	Для Каждого СтрокаОтчет Из ДеревоВариантов.Строки Цикл
		ВариантПоУмолчанию = Неопределено;
		ВключенныйВариант = Неопределено;
		КоличествоВключенныхВариантов = 0;
		Для Каждого СтрокаВариант Из СтрокаОтчет.Строки Цикл
			ЗаполнитьОписаниеСтрокиВарианта(СтрокаВариант, СтрокаОтчет);
			Если СтрокаВариант.КлючВарианта = "" Тогда
				ВариантПоУмолчанию = СтрокаВариант;
			Иначе
				Если СтрокаВариант.Включен Тогда
					КоличествоВключенныхВариантов = КоличествоВключенныхВариантов + 1;
					Если КоличествоВключенныхВариантов > 1 Тогда
						Прервать;
					КонецЕсли;
					ВключенныйВариант = СтрокаВариант;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если ВариантПоУмолчанию <> Неопределено
			И КоличествоВключенныхВариантов = 1
			И ВключенныйВариант <> Неопределено
			И Не ВключенныйВариант.ВидимостьПоУмолчанию Тогда
			ВариантПоУмолчанию.Включен = Ложь;
			ВключенныйВариант.ВидимостьПоУмолчанию = Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаполнитьОписаниеСтрокиВарианта(СтрокаВариант, СтрокаОтчет = Неопределено) Экспорт
	...
	ЗаполнитьЗначенияСвойств(СтрокаВариант, СтрокаОтчет, "Включен, ВидимостьПоУмолчанию, ВыводитьТолькоОтчет");
	
	Если СтрокаВариант.КлючВарианта = "" Тогда
		// Вариант "по умолчанию".
		СтрокаВариант.Наименование = СтрокаОтчет.Наименование;
		СтрокаВариант.Описание = СтрокаОтчет.Описание;
		Если Не СтрокаВариант.ВыводитьТолькоОтчет Тогда
			СтрокаВариант.Включен = Ложь;
		КонецЕсли;
	Иначе
		// Предопределенный вариант.
		Если СтрокаВариант.ВыводитьТолькоОтчет Тогда
...
Функция СброситьНастройкиВариантаОтчета(ВариантОбъект) Экспорт
	...
	ЗаполнитьЗначенияСвойств(ВариантОбъект, ВариантОбъект.ПредопределенныйВариант, "Наименование, ВидимостьПоУмолчанию");
...
Функция ОбщиеНастройкиПанели() Экспорт
	...
	Если ОбщиеНастройки = Неопределено Тогда
		...
		ОбщиеНастройки.ПоказыватьПодсказки = 1;
...
Процедура ОтложенноеОбновлениеОбщихДанных(Полное)
	...
	|	Таблица.Ссылка
	|ИЗ
	|	Справочник.ПредопределенныеВариантыОтчетов КАК Таблица
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		КонецЕсли;
		ЕстьИзменения = ПроиндексироватьСодержимоеСхемы(ВариантОбъект);
...
Процедура АктуализироватьКлючиПредопределенных(Кэш)
	...
	Пока Выборка.Следующий() Цикл
		...
		ВариантОбъект.ДополнительныеСвойства.Вставить("ДеревоВариантов", Кэш.ДеревоВариантов);
		ВариантОбъект.ДополнительныеСвойства.Вставить("ЗаполнениеПредопределенных", Истина);
		ВариантОбъект.Записать();
	КонецЦикла;
...
Процедура ОбновитьНастройкиПредопределенных(Кэш)
	...
	Пока ВариантВыборка.Следующий() Цикл
		...
		Если ИзменилисьКлючевыеНастройкиПредопределенного(ВариантВыборка, ОписаниеВарианта) Тогда
			Кэш.ОбновитьОбласти = Истина;
		ИначеЕсли ИзменилисьВторостепенныеНастройкиПредопределенного(ВариантВыборка, ОписаниеВарианта) Тогда
			// Обновлять области не требуется.
		Иначе
			Продолжить;
		КонецЕсли;
...
Функция ИзменилисьКлючевыеНастройкиПредопределенного(ВариантВыборка, ОписаниеВарианта)
	Если ВариантВыборка.ПометкаУдаления = Истина // Описание получено => требуется снять пометку удаления.
		Или ВариантВыборка.Наименование <> ОписаниеВарианта.Наименование
...
Функция ИзменилисьВторостепенныеНастройкиПредопределенного(ВариантВыборка, ОписаниеВарианта)
	// Шапка
	Если ВариантВыборка.Включен <> ОписаниеВарианта.Включен
		Или ВариантВыборка.Описание <> ОписаниеВарианта.Описание
		Или ВариантВыборка.ВыводитьТолькоОтчет <> ОписаниеВарианта.ВыводитьТолькоОтчет Тогда
		Возврат Истина;
	КонецЕсли;
...
Функция ЗаписатьНастройкиПредопределенного(ОписаниеВарианта, ВариантСсылка = Неопределено)
	...
	ЗаполнитьЗначенияСвойств(ВариантОбъект, ОписаниеВарианта, "Отчет, КлючВарианта, Наименование, Включен, ВидимостьПоУмолчанию, Описание, ВыводитьТолькоОтчет");
...
Процедура ДеревоПодсистемДобавитьУсловноеОформление(Форма) Экспорт
	...
	Инструкция.Оформление.Очистить();
...
Функция РазрешеноИндексированиеНеразделенныхДанных() Экспорт
	Возврат Не ОбщегоНазначенияПовтИсп.РазделениеВключено() И Не ОбщегоНазначения.ЭтоПодчиненныйУзелРИБ();
КонецФункции
...
Функция НайтиСсылки(ПараметрыПоиска) Экспорт
	...
	Если ПараметрыПоиска.Свойство("Автор") И ЗначениеЗаполнено(ПараметрыПоиска.Автор) Тогда
	...
	Если Не ЕстьОтборПоПодсистемам И Не ЕстьСтрокаПоиска И Не ЕстьОтборПоАвтору И Не ЕстьОтборПоТипамОтчетов Тогда
		Возврат Неопределено;
	КонецЕсли;
	...
	Если ЕстьОтборПоПодсистемам Или ЕстьСтрокаПоиска Тогда
		...
		|		ИНАЧЕ ВариантыОтчетов.ПредопределенныйВариант.ВидимостьПоУмолчанию
		...
		|ГДЕ
		|	ВариантыОтчетов.Автор = &Автор
		|	И ВариантыОтчетов.ТипОтчета В(&ТипыОтчетов)
		...
		|		ВариантыРазмещение.РазделИлиГруппа КАК Подсистема,
		|		ВариантыРазмещение.РазделИлиГруппа.Синоним КАК НаименованиеПодсистемы
		|	ИЗ
		...
		|				И (ВариантыРазмещение.РазделИлиГруппа В (&МассивПодсистем))) КАК РазмещениеВариантов
		...
		|	РазмещениеВсех.Ссылка,
		|	РазмещениеВсех.Подсистема,
		|	РазмещениеВсех.НаименованиеПодсистемы
		...
		|		ПО РазмещениеВсех.Подсистема = ЛичныеНастройки.РазделИлиГруппа
		|			И РазмещениеВсех.Ссылка = ЛичныеНастройки.Вариант
		|			И (ЛичныеНастройки.Пользователь = &ТекущийПользователь)
		|ГДЕ
		|	ЕСТЬNULL(ЛичныеНастройки.Видимость, РазмещениеВсех.Использование)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		Если ЕстьОтборПоВидимости Тогда
			//ИмяИтоговойТаблицы = "РазмещениеВидимых";
		Иначе
			//ИмяИтоговойТаблицы = "РазмещениеВсех";
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РазмещениеВсех", "РазмещениеВидимых");
			// Удаление поля для вычисления видимости.
		...
		Если ЕстьОтборПоАвтору Тогда
			Запрос.УстановитьПараметр("Автор", ПараметрыПоиска.Автор);
		Иначе
			// Удаление отбора по автору.
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"ВариантыОтчетов.Автор = &Автор
				|	И ",
		...
		Иначе
			...
			ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И (ВариантыРазмещение.РазделИлиГруппа В (&МассивПодсистем))", "");
		...
		Если ЕстьСтрокаПоиска Тогда
			...
			Если ЕстьОтборПоАвтору И ПараметрыПоиска.Автор <> ТекущийПользователь Тогда
				// Удаление запроса к таблице пользовательских настроек.
				ТекстЗапроса = СокрП(Лев(ТекстЗапроса, Найти(ТекстЗапроса, "ОБЪЕДИНИТЬ ВСЕ") - 1));
			Иначе
		...
		Если Не ЕстьСтрокаПоиска И Не ПолучатьИтоговуюТаблицу Тогда
			...
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
				"ВариантыРазмещение.РазделИлиГруппа.Синоним КАК",
				"НЕОПРЕДЕЛЕНО КАК");
			ТекстЗапроса = СтрЗаменить(
				ТекстЗапроса,
	...
	Если Не ЕстьСтрокаПоиска Тогда
		ТаблицаВариантов = ТаблицаЗначений.Скопировать(, "Ссылка");
		ТаблицаВариантов.Свернуть("Ссылка");
		Для Каждого СтрокаТаблицы Из ТаблицаВариантов Цикл
