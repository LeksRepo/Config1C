	...
	Попытка
		...
		Иначе
			...
			Иначе
				...
				Служебный = ОбщегоНазначенияПовтИсп.РазделениеВключено()
					И ПользователиИнформационнойБазы.ТекущийПользователь().РазделениеДанных.Количество() = 0;
				
				Если РезультатПользователи.Пустой() Тогда
					Если Служебный ИЛИ Пользователи.ЭтоПолноправныйПользователь( , , Ложь) Тогда
...
Функция ФормированиеСообщенияПользователю(МассивПользователей, ГруппаПриемник,
	...
	Если КоличествоНеПеремещенныхПользователей > 0 Тогда
		...
		Если КоличествоНеПеремещенныхПользователей = 1 Тогда
			Предмет = Строка(МассивНеПеремещенныхПользователей[0]);
			...
			СообщениеПользователю = СообщениеПользователю + Символы.ПС + 
									...
									НСтр("ru = 'т.к. у группы стоит признак ""Все"".'"));
		Иначе
			Предмет = "";
			...
			СообщениеПользователю = НСтр("ru = 'Не все пользователи могут быть включены в группу ""%2"",
									...
									|или у группы стоит признак ""Все"".'");
		КонецЕсли;
