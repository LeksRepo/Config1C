Функция ПеревестиСтрокуВКодыСимволов (ИсходнаяСтрока) Экспорт
	...
	Пока К<=ДлинаСтроки Цикл
		...
		ВозврСтрока = ВозврСтрока + УдалитьСимволыВСтроке(Строка(КодСимвола(Буква)),Символы.НПП)+"*";
...
Функция ПеревестиКодыСимволовВстроку(ИсходнаяСтрока) Экспорт
	...
	Для Каждого сим Из МассивСимволов Цикл
		Попытка
			ВозврСтрока=ВозврСтрока+Символ(Число(сим));	
		Исключение
		КонецПопытки;
	КонецЦикла;
	Возврат ВозврСтрока;
...
Функция УдалитьСимволыВСтроке(ИсходнаяСтрока, Символ) Экспорт
	
	К=0;
	Пока К <= СтрДлина(ИсходнаяСтрока) Цикл
		ИндексПробела = Найти(ИсходнаяСтрока,Символ);
		ИсходнаяСтрока = Лев(ИсходнаяСтрока,ИндексПробела-1)+Сред(ИсходнаяСтрока,ИндексПробела+1);
		К=К+1;
	КонецЦикла;
