////////////////////////////////////////////////////////////////////////////////
// Подсистема "Информационный центр".
//
////////////////////////////////////////////////////////////////////////////////

////////////////////////////////////////////////////////////////////////////////
// СЛУЖЕБНЫЕ ПРОЦЕДУРЫ И ФУНКЦИИ

// Возвращает таблицу с информационными ссылками для форм
//
// Параметры:
//	ПутьКФорме - Строка - полный путь к форме.
//
// Возвращаемое значение:
//	ТаблицаЗначений - таблица информационных ссылок для формы с колонками:
//		Наименование - Строка - наименование информационной ссылки
//		Адрес - Строка - внешний адрес информационной ссылки
//		Вес - Число - вес информационной ссылки
//		ДатаНачалаАктуальности - Дата - дата начала актуальности информационной ссылки
//		ДатаОкончанияАктуальности - Дата - дата окончания актуальности информационной ссылки
//		Подсказка - Строка - подсказка для информационной ссылки
//
Функция ПолучитьТаблицуИнформационныхСсылокДляФормы(ПутьКФорме) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	ТекущаяВерсияКонфигурации = ИнформационныйЦентрСервер.ПолучитьВерсиюЧислом(Метаданные.Версия);
	УстановитьПривилегированныйРежим(Ложь);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПолныйПутьКФорме",          ПутьКФорме);
	Запрос.УстановитьПараметр("НулеваяВерсия",             0);
	Запрос.УстановитьПараметр("ТекущаяВерсияКонфигурации", ТекущаяВерсияКонфигурации);
	Запрос.УстановитьПараметр("ТекущаяДата",               ТекущаяДата()); // Проектное решение БСП
	Запрос.УстановитьПараметр("ПустаяДата",                '00010101');
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПолныеПутиКФормам.Ссылка КАК Ссылка
	               |ПОМЕСТИТЬ ПутиКФормам
	               |ИЗ
	               |	Справочник.ПолныеПутиКФормам КАК ПолныеПутиКФормам
	               |ГДЕ
	               |	ПолныеПутиКФормам.ПолныйПутьКФорме ПОДОБНО &ПолныйПутьКФорме
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	ИнформационныеСсылкиДляФорм.Наименование КАК Наименование,
	               |	ИнформационныеСсылкиДляФорм.Адрес КАК Адрес,
	               |	ИнформационныеСсылкиДляФорм.Вес КАК Вес,
	               |	ИнформационныеСсылкиДляФорм.ДатаНачалаАктуальности КАК ДатаНачалаАктуальности,
	               |	ИнформационныеСсылкиДляФорм.ДатаОкончанияАктуальности КАК ДатаОкончанияАктуальности,
	               |	ИнформационныеСсылкиДляФорм.Подсказка КАК Подсказка
	               |ИЗ
	               |	ПутиКФормам КАК ПутиКФормам
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ИнформационныеСсылкиДляФорм КАК ИнформационныеСсылкиДляФорм
	               |		ПО ПутиКФормам.Ссылка = ИнформационныеСсылкиДляФорм.ПолныйПутьКФорме
	               |ГДЕ
	               |	((ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииОт = &НулеваяВерсия
	               |				И ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииДо = &НулеваяВерсия)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииОт = &НулеваяВерсия
	               |				И ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииДо >= &ТекущаяВерсияКонфигурации)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииОт <= &ТекущаяВерсияКонфигурации
	               |				И ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииДо = &НулеваяВерсия)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииОт <= &ТекущаяВерсияКонфигурации
	               |				И ИнформационныеСсылкиДляФорм.ВерсияКонфигурацииДо >= &ТекущаяВерсияКонфигурации)) И
	               |	((ИнформационныеСсылкиДляФорм.ДатаНачалаАктуальности = &ПустаяДата
	               |				И ИнформационныеСсылкиДляФорм.ДатаОкончанияАктуальности = &ПустаяДата)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ДатаНачалаАктуальности = &ПустаяДата
	               |				И ИнформационныеСсылкиДляФорм.ДатаОкончанияАктуальности >= &ТекущаяДата)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ДатаНачалаАктуальности <= &ТекущаяДата
	               |				И ИнформационныеСсылкиДляФорм.ДатаОкончанияАктуальности = &ПустаяДата)
	               |			ИЛИ (ИнформационныеСсылкиДляФорм.ДатаНачалаАктуальности <= &ТекущаяДата
	               |				И ИнформационныеСсылкиДляФорм.ДатаОкончанияАктуальности >= &ТекущаяДата))";
	Возврат Запрос.Выполнить().Выгрузить();
	
КонецФункции
