...
//
Функция ПредыдущийФорматКонтактнойИнформацииXML(Данные, СтарыйСоставПолей=Ложь) Экспорт
	
	Если КонтактнаяИнформацияКлиентСервер.ЭтоСтрокаXML(Данные) Тогда
		СтарыйФормат = КонтактнаяИнформацияВСтаруюСтруктуру(Данные, СтарыйСоставПолей);
...
//      ВидКонтактнойИнформации - необязательная ссылка на справочник вида, используется, если "Данные" - пустая строка
//
//  Возвращаемое значение:
//      Структура с набором пар ключ-значение
//
Функция ПредыдущаяСтруктураКонтактнойИнформацииXML(Данные, ВидКонтактнойИнформации=Неопределено) Экспорт
...
//
//  Параметры:
//      НаименованиеРегиона  - полное наименование региона с сокращением
//
Функция КодРегиона(ПолноеНаименование) Экспорт
	Вариант = КонтактнаяИнформацияКлиентСервер.ИспользуемыйАдресныйКлассификатор();
...
//
Функция ДесериализацияКонтактнойИнформации(Текст, ОжидаемыйВид=Неопределено) Экспорт
...
Функция ЗданияИПомещенияАдреса(XDTOАдрес, НовоеЗначение=Неопределено) Экспорт
	...
	// Чтение
	Результат.Здания.Колонки.Добавить("Порядок");
	Результат.Помещения.Колонки.Добавить("Порядок");
		...
		Если ДопЭлемент.Номер<>Неопределено Тогда
			...
			Если ТипОбъекта<>Неопределено Тогда
				...
				Если НоваяСтрока<>Неопределено Тогда
					НоваяСтрока.Порядок    = ТипОбъекта.Порядок;
	...
	Результат.Здания.Сортировать("Вид, Порядок, Значение");
	Результат.Здания.Колонки.Удалить("Порядок");
	
	Результат.Помещения.Сортировать("Вид, Порядок, Значение");
	Результат.Помещения.Колонки.Удалить("Порядок");
...
Функция ОшибкиЗаполненияАдреса(XDTOАдрес, ВидИнформации, РезультатГруппами=Ложь) Экспорт
	...
	Если РезультатГруппами Тогда
		Если ОшибкиКлассификатора.Количество()>0 Тогда
			// Добавляем ко всем ошибкам
			ВсеОшибки.Добавить(ГруппаОшибокАдреса("ОшибкиПоКлассификатору",
...
Функция КонтактнаяИнформацияВСтаруюСтруктуру(XDTOКонтактнаяИнформация, СтарыйСоставПолей=Ложь) Экспорт
	...
	Если КонтактнаяИнформацияКлиентСервер.ЭтоСтрокаXML(XDTOКонтактнаяИнформация) Тогда
...
// Внутреннее для создания структур
...
Функция СписокРеквизитовНаселенныйПунктКЛАДР(Код=Неопределено)
	...
	Результат = Новый Структура("Регион, Район, Город, НаселенныйПункт",
		// Второй уровень - одинаково для всех классификаторов
		ЭлементАдреснойСтруктуры("Регион",    "СубъектРФ"),
		ЭлементАдреснойСтруктуры("Район",     "СвРайМО/Район"),
		ЭлементАдреснойСтруктуры("Город",     "Город"),
		ЭлементАдреснойСтруктуры("Нас.пункт", "НаселПункт", ,Истина)
	);
...
Функция ЭлементАдреснойСтруктуры(Заголовок=Неопределено, ПутьXPath=Неопределено, Значение=Неопределено, Предопределенный=Ложь)
