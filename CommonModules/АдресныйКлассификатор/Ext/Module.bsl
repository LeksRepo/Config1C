...
Функция ОбработатьФайлыКЛАДР(Знач ОбъектыЗагрузки, Знач ПереданныеФайлы, Знач ИсточникДанных, Знач ИдентификаторХранилища) Экспорт
	...
	Если ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда 
		// Описание файлов передаем непосредственно
		Параметры.Вставить("ПереданныеФайлы", ПереданныеФайлы);
		
		АдресХранилища = ПоместитьВоВременноеХранилище(Неопределено, ИдентификаторХранилища);
		ФоноваяЗагрузкаФайловКЛАДР(Параметры, АдресХранилища);
		Результат = Новый Структура("ЗаданиеВыполнено", Истина);
		
	Иначе
		// Описание файлов передаем как двоичные данные
...
Процедура ФоноваяЗагрузкаФайловКЛАДР(Знач Параметры, Знач АдресХранилища) Экспорт
	...
	Маска = ?(ОбщегоНазначения.ЭтоLinuxСервер(), "[Kk][Ll][Aa][Dd][Rr].[Dd][Bb][Ff]" , "kladr.dbf");
	ФайлыВерсии = НайтиФайлы(КаталогФайлов, Маска, Ложь);
...
Функция ИзвлечьФайлыКЛАДРСайт(Знач ПереданныеФайлы)
	...
	Для Каждого Описание Из ПереданныеФайлы Цикл
		...
		Архив.ИзвлечьВсе(СерверныйКаталог, РежимВосстановленияПутейФайловZIP.НеВосстанавливать);
...
Функция ИзвлечьФайлыКЛАДРИТС(Знач ПереданныеФайлы)
	...
	// Сейчас они распаковываются на клиенте
	...
	Для Каждого Описание Из ПереданныеФайлы Цикл
		Файл = Новый Файл(Описание.Имя);
		Если Врег(Файл.Расширение) <> ".DBF" Тогда
			Продолжить;
		КонецЕсли;
		ИмяАрхива = СерверныйКаталог + Файл.Имя;
...
Функция ИзвлечьФайлыКЛАДРДиск(Знач ПереданныеФайлы)
	...
	Для Каждого Описание Из ПереданныеФайлы Цикл
		...
		Если Врег(Файл.Расширение) <> ".DBF" Тогда
			Продолжить;
		КонецЕсли;
		ИмяФайла = СерверныйКаталог + Файл.Имя;
...
Функция ЗапросДесериализацияАдресаПоПредставлениюКЛАДР(Знач ТаблицаСтранМира = "( ВЫБРАТЬ &НаименованиеРоссии КАК Наименование )") Экспорт
	...
	Запрос = Новый Запрос("
		...
		|
		|	ВЫБОР 
		|		КОГДА ДанныеАдреса.Значение = &НаименованиеРоссии ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		...
		|	
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	" + ТаблицаСтранМира + " КАК СтраныМира
		|ПО
...
Функция ЗапросАдресаПоИндексуКЛАДР(Знач ВыбиратьСтрок, Знач СкрыватьНеактуальные) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ПредставлениеУлица, ПредставлениеНаселенныйПункт, ПредставлениеГород, ПредставлениеРайон, ПредставлениеРегион
		|;///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		|
		|// Основной результат
		|ВЫБРАТЬ 
		...
		|ВЫБРАТЬ 
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеУлица, """")) ТОГДА
		|		МИНИМУМ(УлицаНаименование) ИНАЧЕ """" КОНЕЦ КАК УлицаНаименование,
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеУлица, """")) ТОГДА
		|		МИНИМУМ(УлицаСокращение) ИНАЧЕ """" КОНЕЦ КАК УлицаСокращение,
		|
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеНаселенныйПункт, """")) ТОГДА
		|		МИНИМУМ(НаселенныйПунктНаименование) ИНАЧЕ """" КОНЕЦ КАК НаселенныйПунктНаименование,
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеНаселенныйПункт, """")) ТОГДА
		|		МИНИМУМ(НаселенныйПунктСокращение) ИНАЧЕ """" КОНЕЦ КАК НаселенныйПунктСокращение,
		|
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеГород, """")) ТОГДА
		|		МИНИМУМ(ГородНаименование) ИНАЧЕ """" КОНЕЦ КАК ГородНаименование,
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеГород, """")) ТОГДА
		|		МИНИМУМ(ГородСокращение) ИНАЧЕ """" КОНЕЦ КАК ГородСокращение,
		|
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеРайон, """")) ТОГДА
		|		МИНИМУМ(РайонНаименование) ИНАЧЕ """" КОНЕЦ КАК РайонНаименование,
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеРайон, """")) ТОГДА
		|		МИНИМУМ(РайонСокращение) ИНАЧЕ """" КОНЕЦ КАК РайонСокращение,
		|
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеРегион, """")) ТОГДА
		|		МИНИМУМ(РегионНаименование) ИНАЧЕ """" КОНЕЦ КАК РегионНаименование,
		|	ВЫБОР КОГДА 1 = КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ЕСТЬNULL(ПредставлениеРегион, """")) ТОГДА
		|		МИНИМУМ(РегионСокращение) ИНАЧЕ """" КОНЕЦ КАК РегионСокращение
		|ИЗ
		|	АдресаПоПочтовымИндексам
		|");
