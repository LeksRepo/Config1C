...
Функция ПолучитьВерсииАдресныхОбъектов() Экспорт
    ...
    Запрос = Новый Запрос("
        ...
        |			1
        |		ИЗ			
        |			РегистрСведений.АдресныйКлассификатор КАК КлассификаторВнутр
        |		ГДЕ
        |			КлассификаторВнутр.ТипАдресногоЭлемента > 1
        |			И КлассификаторВнутр.КодАдресногоОбъектаВКоде = Регион.КодАдресногоОбъектаВКоде
        |			И КлассификаторВнутр.Код > Регион.Код
        |	)
        |
        |УПОРЯДОЧИТЬ ПО
        |   АдресныйОбъект
