...
Функция СтруктураОтветаВебСервера(ТекстОшибки, КодОшибки = 0, ЗначениеРезультата = Ложь,
	...
	СтруктураОтвета.Вставить("ЗначениеРезультата", ЗначениеРезультата);
...
Функция ОписаниеСервисаПроверкиЛегальности()
	...
	ПараметрыСоединения = ПолучениеФайловИзИнтернетаКлиентСервер.СтруктураURI(АдресWSDL);
	...
	Попытка
		...
		ПараметрыКонструктора.Добавить(ПараметрыСоединения.ПутьНаСервере);
	...
	Если УзлыСхемы.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			...
			|%2'"), АдресWSDL, НСтр("ru = 'Отсутствует элемент описания типов данных (<wsdl:types ...>).'"));
		ВызватьИсключение ТекстИсключения;
	...
	Если УзелОписанияСхемы = Неопределено Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, НСтр("ru = 'Отсутствует элемент описания типов данных (<xs:schema ...>)'"));
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(URIСервиса) Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, НСтр("ru = 'Отсутствует URI пространства имен в WSDL-описании.'"));
		ВызватьИсключение ТекстИсключения;
	...
	Если УзлыСервисов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, НСтр("ru = 'Отсутствует описание веб-сервисов в WSDL-описании (<wsdl:service ...>).'"));
		ВызватьИсключение ТекстИсключения;
	...
	Если УзлыПортов.Количество() = 0 Тогда
		...
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, НСтр("ru = 'Отсутствует описание портов в WSDL-описании (<wsdl:port ...>).'"));
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(ИмяПорта) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить имя порта сервиса (%1).'"), ИмяСервиса);
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
			|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, ТекстОшибки);
		ВызватьИсключение ТекстИсключения;
	...
	Если НЕ ЗначениеЗаполнено(АдресПорта) Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Не удалось определить URL заданного порта сервиса (%1).'"), ИмяПорта);
		ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Ошибка при создании описания веб-сервиса (%1).
				|Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:
			|%2'"), АдресWSDL, ТекстОшибки);
		ВызватьИсключение ТекстИсключения;
...
Функция ОтправитьЗапросSOAP(ТекстКонверта, ОписаниеСервисаПроверкиЛегальности)
	...
	ПараметрыКонструктора = Новый Массив;
	ПараметрыКонструктора.Добавить(ОписаниеСервисаПроверкиЛегальности.ПутьПорта);
	HTTPЗапрос = Новый(Тип("HTTPЗапрос"), ПараметрыКонструктора);
