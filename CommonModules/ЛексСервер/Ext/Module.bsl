...
Функция ПечататьТорг12(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ПредставлениеГрузополучателя = "" + Выборка.ПолноеНаименование 
			+ ?(ЗначениеЗаполнено(Выборка.АдресДоставки), ", адрес: " + Выборка.АдресДоставки, "") 
			+ ?(ЗначениеЗаполнено(Выборка.Телефон), ", тел: " + Выборка.Телефон, "") 
			+ ?(ЗначениеЗаполнено(Выборка.БанковскиеРеквизиты), ", " + Выборка.БанковскиеРеквизиты, "");
			ОбластьШапка.Параметры.Заполнить(Выборка);
			НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.НомерДокумента);
			ОбластьШапка.Параметры.НомерДокумента = НомерДокумента;
