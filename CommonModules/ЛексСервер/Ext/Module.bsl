...
Функция ОбработатьСтатусыМассиваСлужебныхЗаписок(МассивЗаписок) Экспорт
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Если НЕ ВыборкаДетальныеЗаписи.Ознакомлен Тогда
...
Функция ДоступностьСпецификации(Спецификация) Экспорт
	...
	ПользовательТехнолог = УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, Справочники.ПрофилиГруппДоступа.СтаршийДизайнер) 
	...
	ИЛИ УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, Справочники.ПрофилиГруппДоступа.Супервайзер);
...
Процедура УстановитьДатуИзготовленияСпецификации(Спецификация) Экспорт
	...
	ДокументПлановыйЛимит = Документы.ПлановыйЛимит.ПолучитьДокументЗаПериод(Спецификация.Подразделение, Спецификация.ДатаОтгрузки, Истина);
	
	Если ЗначениеЗаполнено(Спецификация.ДатаОтгрузки) И ЗначениеЗаполнено(ДокументПлановыйЛимит) Тогда
	...
	//RonEXI: Если не установлен лимит
	Иначе
...
Функция ПолучитьЛоготипДилера(Спецификация) Экспорт
	...
	Если ЗначениеЗаполнено(Спецификация) И ЗначениеЗаполнено(Спецификация.Контрагент) И ЗначениеЗаполнено(Спецификация.Контрагент.Логотип) Тогда
...
Функция ПолучитьЛоготипСервер(Спецификация) Экспорт
	 ...
	 Если (НЕ ЗначениеЗаполнено(СсылкаЛоготип)) И ЗначениеЗаполнено(Спецификация) И ЗначениеЗаполнено(Спецификация.Офис) И ЗначениеЗаполнено(Спецификация.Офис.Логотип) Тогда
			СсылкаЛоготип = Спецификация.Офис.Логотип;
	 Иначе
			Если (НЕ ЗначениеЗаполнено(СсылкаЛоготип)) И ЗначениеЗаполнено(Спецификация) И ЗначениеЗаполнено(Спецификация.Подразделение) И ЗначениеЗаполнено(Спецификация.Подразделение.Логотип) Тогда
...
Процедура ЗаписатьВходПользователя(СерийникДискаС) Экспорт
	...
	НоваяЗапись.Действие = Перечисления.ВидыАктивностиПользователей.ВходВСистему;
	НоваяЗапись.Дата = ТекущаяДата();
	НоваяЗапись.СерийныйНомерДискаС = СерийникДискаС;
	НоваяЗапись.Записать();
	
КонецПроцедуры

Функция ПолучитьНомераПередлокИСрочек(Пользователь) Экспорт
	
	ФизЛицо = Пользователь.ФизическоеЛицо;
	Подразделение = ФизЛицо.Подразделение;
	Управляющий = Подразделение.Управляющий;
	НомераПеределок = "";
	СтруктураДляПеределок = Новый Структура;
	СтруктураДляПеределок.Вставить("МассивСпецификаций", Неопределено);
	СтруктураДляПеределок.Вставить("ЕстьМассив", Ложь);
	...
	Если ФизЛицо = Управляющий Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			СтруктураДляПеределок.Вставить("МассивСпецификаций", РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Спецификация"));
			СтруктураДляПеределок.Вставить("ЕстьМассив", Истина);
			
		КонецЕсли;
		
	КонецЕсли;
	
	Возврат СтруктураДляПеределок;
