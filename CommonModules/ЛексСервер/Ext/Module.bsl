...
Процедура Перепроведение() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			Если МассивОшибки <> Неопределено Тогда
				ТекстОшибки = СокрЛП(ТекстОшибки) + Символы.ПС + Док + " - " + МассивОшибки[0].Текст;
			КонецЕсли;			
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		ПараметрыПисьма.Вставить("Кому", ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Справочники.УчетныеЗаписиЭлектроннойПочты.УчетнаяЗаписьГлБухгалтера, "АдресЭлектроннойПочты"));
