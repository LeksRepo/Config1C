...
Функция ПолучитьВремяНовогоДокумента(Документ) Экспорт
	...
	Если ТипДок = Тип("ДокументОбъект.АвансовыйОтчет")   		   Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 32400; КонецЕсли; 
	Если ТипДок = Тип("ДокументОбъект.ВстречаСКлиентом")           Тогда НоваяДата = НачалоЧаса(ТекущаяДата()); 	   КонецЕсли; 
	Если ТипДок = Тип("ДокументОбъект.ЗакрытиеМесяца")   		   Тогда НоваяДата = КонецМесяца(ТекущаяДата());       КонецЕсли;	
	Если ТипДок = Тип("ДокументОбъект.ИнвентаризацияМатериалов")   Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 57600; КонецЕсли;
	Если ТипДок = Тип("ДокументОбъект.НачислениеАмортизации")      Тогда НоваяДата = КонецМесяца(ТекущаяДата());       КонецЕсли;	
	Если ТипДок = Тип("ДокументОбъект.ПереводНоменклатуры")        Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 36000; КонецЕсли;
	Если ТипДок = Тип("ДокументОбъект.ПеремещениеМатериалов")      Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 34200; КонецЕсли;	
	Если ТипДок = Тип("ДокументОбъект.ПлановыйЛимит")              Тогда НоваяДата = КонецМесяца(ТекущаяДата());       КонецЕсли;	
	Если ТипДок = Тип("ДокументОбъект.ПоступлениеМатериаловУслуг") Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 32400; КонецЕсли;
	Если ТипДок = Тип("ДокументОбъект.СписаниеМатериалов") 		   Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 46800; КонецЕсли;
	Если ТипДок = Тип("ДокументОбъект.ТребованиеНакладная") 	   Тогда НоваяДата = НачалоДня(ТекущаяДата()) + 36000; КонецЕсли;	
