	...
	Для Каждого СтрокаОтверстий Из ТаблицаОтверстий Цикл
		...
		Если СтрокаОтверстий.ВидОтверстий = ПредопределенноеЗначение("Перечисление.ВидыОтверстий.ПазыПетель") 
		 ИЛИ СтрокаОтверстий.ВидОтверстий = ПредопределенноеЗначение("Перечисление.ВидыОтверстий.ТорцеваяПрисадка") Тогда
			...
			//Новый Шаг для петель, отнимаем по 100мм с каждой стороны и равномерно распределяем петли.
...
Процедура ЗаписатьВходПользователя(СерийникДискаС, ИмяКомпьютера) Экспорт
	
	Если НЕ ЗначениеЗаполнено(СерийникДискаС) Тогда
		
		Возврат;
		
	КонецЕсли;
	
	Компьютер = ОпределитьКомпьютер(СерийникДискаС, ИмяКомпьютера);
...
Функция ОпределитьКомпьютер(СерийныйНомер, ИмяКомпьютера)
