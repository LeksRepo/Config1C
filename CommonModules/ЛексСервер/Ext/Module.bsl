...
Функция ОтборНоменклатурныхГрупп(СписокНоменклатурныхГрупп, Подразделение) Экспорт
	...
	Если Подразделение.Регион = Справочники.Регионы.НайтиПоКоду("000000001") Тогда
	
		Подразделение = Справочники.Подразделения.НайтиПоКоду("000000004");
	
	Иначе
	
		Подразделение = Справочники.Подразделения.НайтиПоКоду("000000005");
...
Процедура СформироватьПоказателиСотрудников(Движения, Ссылка) Экспорт
	...
	КоличествоСотруднику = 0.01 * ОставшийсяПроцент;
	
	ДобавитьПоказательСотрудника(СвойстваДокумента.Подразделение, СвойстваДокумента.Автор.ФизическоеЛицо, Движения, Дата, СуммаСотруднику, КоличествоСотруднику, Соглашение);
	...
	Если ЗначениеЗаполнено(СвойстваДокумента.Поделиться) И СвойстваДокумента.ПроцентПоделиться > 0 Тогда
		...
		КоличествоСотруднику = 0.01 * СвойстваДокумента.ПроцентПоделиться;
		
		ДобавитьПоказательСотрудника(СвойстваДокумента.Подразделение, СвойстваДокумента.Поделиться, Движения, Дата, СуммаСотруднику, КоличествоСотруднику, Соглашение);
...
Функция РеализацияИзделийРозничныеПроводки(МассивСпецификаций, Дата, Движения) Экспорт
	...
	Для каждого Спецификация Из МассивСпецификаций Цикл
		...
		Если НЕ СвойстваСпецификации.Подразделение.СвоиМонтажи Тогда
