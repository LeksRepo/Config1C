...
Процедура Перепроведение() Экспорт
	...
	Для Каждого Документ Из Метаданные.Документы Цикл
		...
		Если МассивДатЗапрета.Количество() > 1 Тогда
			ДатаЗапретаРедактирования = МассивДатЗапрета[0].ДатаЗапрета;		
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ТекстОшибки) Тогда
		...
		Путь = "C:\errors\Ошибки перепроведения за " + Формат(ТекущаяДата(), "ДФ=dd-MM-yyyy") + ".txt";
		
		Файл = Новый ЗаписьТекста(Путь);
		Файл.ЗаписатьСтроку(ТекстОшибки);
		Файл.Закрыть();
...
//
Функция ЗакрытьДопСоглашенияДоговора(Движения, ДоговорСсылка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Проводка = Движения.Управленческий.Добавить();
		Проводка.Период = РеквизитыДоговора.Дата;
...
Функция ПолучитьСтруктуруПодставляемойНоменклатурыПоЦветуЛДСП(ЛДСП, Подразделение) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЦветаЛДСП.ПодставляемаяНоменклатура
		|ИЗ
		|	РегистрСведений.НоменклатураПодразделений КАК НоменклатураПодразделений
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦветаЛДСП КАК ЦветаЛДСП
		|		ПО НоменклатураПодразделений.Номенклатура = ЦветаЛДСП.ПодставляемаяНоменклатура
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Имя = Справочники.НоменклатурныеГруппы.ПолучитьИмяПредопределенного(ВыборкаДетальныеЗаписи.ПодставляемаяНоменклатура.НоменклатурнаяГруппа);
		СтруктураПодставляяемойНоменклатуры.Вставить(Имя, ВыборкаДетальныеЗаписи.ПодставляемаяНоменклатура);
...
Функция СформироватьПроводкиПрибыльПроизводства(СпецификацияСсылка) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	СпецификацияСписокНоменклатуры.ВнутренняяСтоимость КАК ВнутренняяСтоимость,
		|	СпецификацияСписокНоменклатуры.Ссылка КАК Ссылка,
		|	СпецификацияСписокНоменклатуры.Ссылка.ВнутренняяСтоимостьСпецификации КАК ВнутренняяСтоимостьСпецификации
		|ИЗ
		|	Документ.Спецификация.СписокНоменклатуры КАК СпецификацияСписокНоменклатуры
		|ГДЕ
		|	СпецификацияСписокНоменклатуры.Ссылка.Дата МЕЖДУ &Дата И &Дата1
		|ИТОГИ
		|	СУММА(ВнутренняяСтоимость),
		|	МАКСИМУМ(ВнутренняяСтоимостьСпецификации)
		|ПО
		|	Ссылка";
