...
Функция ПолучитьСписокНеОзнакомленныхСообщенийДилерам() Экспорт
    ...
    Запрос.Текст = 
	"ВЫБРАТЬ
	|	ОзнакомленСообщенияДилеру.Сообщение КАК Сообщение,
	|	ОзнакомленСообщенияДилеру.Дилер КАК Дилер,
	|	ОзнакомленСообщенияДилеру.Ознакомлен КАК Ознакомлен
	|ПОМЕСТИТЬ ВсеОзнакомленныеДилера
	|ИЗ
	|	РегистрСведений.ОзнакомленСообщенияДилеру КАК ОзнакомленСообщенияДилеру
	|ГДЕ
	|	ОзнакомленСообщенияДилеру.Дилер = &Дилер
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВсеОзнакомленныеДилера.Дилер,
	|	ISNULL(ВсеОзнакомленныеДилера.Ознакомлен, Ложь) КАК Ознакомлен
	|ПОМЕСТИТЬ Ознакомленные
	|ИЗ
	|	Документ.СообщениеДилеру КАК СообщениеДилеру
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВсеОзнакомленныеДилера КАК ВсеОзнакомленныеДилера
	|		ПО СообщениеДилеру.Ссылка = ВсеОзнакомленныеДилера.Сообщение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Ознакомленные.Ссылка
	|ГДЕ
	|	НЕ Ознакомленные.Ознакомлен";
...
Процедура УстановитьДатуИзготовленияСпецификации(Спецификация) Экспорт
	...
	Если ЗначениеЗаполнено(Спецификация.ДатаОтгрузки) Тогда
		...
		Если РазностьВДнях < (3 + ЕстьВоскресеньеОтСоздания + (Спецификация.ЭтоКухня ИЛИ ЕстьФотопечать)) Тогда
			// Это вроде как быстрый заказ
			Спецификация.Быстрый = Истина;
			//Спецификация.ДатаИзготовления = ДатаДокумента;
			//Спецификация.ДатаОтгрузки = ДатаДокумента;
		Иначе
			// А это типа обычный заказ
			
			Спецификация.Быстрый = Ложь;
			
			КоличествоДней = 1 + Число(Спецификация.ЭтоКухня ИЛИ ЕстьФотопечать);
