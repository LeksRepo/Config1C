...
Функция ПолучитьСписокНеОзнакомленныхСообщенийДилерам() Экспорт
	...
	|	И ОзнакомленСообщенияДилеру.Дилер.ОбъектАвторизации.Дилер";
...
//
Функция ПолучитьСкидкуДоговора(Подразделение, Период, Сумма, ВидОплаты, Офис, Контрагент, Договор = Ложь, ПостоянныйКлиент = Ложь) Экспорт
	
	СтруктураСкидка = Новый Структура("РазмерСкидки, РазмерСкидкиПостоянногоКлиента, ЭтоПостоянныйКлиент", 0, 0, Ложь);
	
	Если Договор = Ложь Тогда 
