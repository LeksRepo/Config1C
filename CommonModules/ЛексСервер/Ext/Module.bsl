...
//
Функция НачислитьСуммуУправлению(Регистратор, Движения, Сумма) Экспорт
	
	Если НЕ (ТипЗнч(Регистратор) = Тип("ДокументСсылка.РеализацияГотовойПродукции")
		И ТипЗнч(Регистратор) = Тип("ДокументСсылка.АктВыполненияДоговора")) Тогда
		Возврат Ложь;
	КонецЕсли;
