...
Функция ПолучитьСкидкуДоговора(Подразделение, Период, Сумма, ВидОплаты, Офис, Контрагент, Договор = Ложь, ПроверятьПостоянного = Истина, Постоянный = Ложь) Экспорт
	...
	Если ПроверятьПостоянного Тогда
		...
		Если Контрагент <> Справочники.Контрагенты.ЧастноеЛицо Тогда 
			...
			|ГДЕ
			|	Договор.Проведен
			|	И Договор.Спецификация.Контрагент = &Контрагент
...
Функция ЗакрытьДопСоглашенияДоговора(Движения, ДоговорСсылка, ДатаПроведения) Экспорт
	...
	РеквизитыДоговора = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(ДоговорСсылка, "Дата, Подразделение");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Сумма > 0 Тогда
			...
			Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.Контрагенты] = ДоговорСсылка.Спецификация.Контрагент;
		...
		Иначе
			...
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.Контрагенты] = ДоговорСсылка.Спецификация.Контрагент;
...
Функция УстановитьОрганизацию(ДокОбъект) Экспорт
	...
	Если (ТипЗнч(Основание) = Тип("ДокументСсылка.Договор") И Основание.Контрагент.ЮридическоеЛицо)
...
Функция ДобавитьПоказательСотруднику(ФизЛицо, ВидПоказателя, ДокОбъект, Движения)
	...
	Проводка.Сумма = ДокОбъект.Сумма;
...
Функция ОчисткаНеактуальныхРаскроев() Экспорт
	...
	|	И (СтатусСпец.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСпецификации.Отгружен)
	|			ИЛИ СтатусСпец.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыСпецификации.Изготовлен)
