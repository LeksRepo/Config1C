...
/////Область Функций Для Печати Наряда Задания
Функция ПечатьНарядЗадания(МассивОбъектов, ОбъектыПечати, ТабДок) Экспорт
...
Функция ВывестиИзделияНаСкладеГотовойПродукции(ТабДок, Подразделение)
	...
	|	ВЫБОР
	|		КОГДА докМонтаж.Ссылка ЕСТЬ NULL 
	|			ТОГДА докСпецификация.ДатаОтгрузки
	|		ИНАЧЕ докМонтаж.Дата
	...
	|		ПО (докМонтаж.Спецификация = докСпецификация.Ссылка)
	|ГДЕ
