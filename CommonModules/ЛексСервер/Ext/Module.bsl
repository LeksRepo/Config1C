...
Функция ОтборНоменклатурныхГрупп(СписокНоменклатурныхГрупп, Подразделение) Экспорт
	...
	|ИЗ
	|	РегистрСведений.НоменклатураПодразделений.СрезПоследних(&Период, ) КАК НоменклатураПодразделений
...
Функция ПолучитьСкидкуДоговора(Подразделение, Период, Сумма, ВидОплаты, Офис, Контрагент, Договор = Ложь, ПроверятьПостоянного = Истина, Постоянный = Ложь) Экспорт
	...
	ИначеЕсли ТипЗнч(ВидОплаты) = Тип("ПеречислениеСсылка.ВидыОплатыДоговоров") Тогда
...
Процедура УстановитьДатуИзготовленияСпецификации(Спецификация) Экспорт
	...
	КоличествоДнейНаИзготовлениеПоНоменклатуре = Документы.Спецификация.ПолучитьКоличествоДнейНаИзготовление(Спецификация);
