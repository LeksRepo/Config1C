...
Функция ОтборНоменклатурныхГруппРедактор(СписокНоменклатурныхГрупп, Подразделение) Экспорт
	...
	|	СпрНоменклатура.Ссылка,
	|	СпрНоменклатура.НоменклатурнаяГруппа КАК НоменклатурнаяГруппа,
	|	ЕСТЬNULL(НоменклатураПодразделений.ПодЗаказ,0) КАК ПодЗаказ
	|ИЗ
...
Функция ОпределитьШагОтверстия(ТаблицаОтверстий, ШиринаДетали, ВысотаДетали) 
	...
	Для Каждого СтрокаОтверстий Из ТаблицаОтверстий Цикл
		...
		Если СтрокаОтверстий.ВидОтверстий = ПредопределенноеЗначение("Перечисление.ВидыОтверстий.ПазыПетель") Тогда 
		 //ИЛИ СтрокаОтверстий.ВидОтверстий = ПредопределенноеЗначение("Перечисление.ВидыОтверстий.ТорцеваяПрисадка") 
...
//
Функция ПолучитьСкидкуДоговора(Подразделение, Период, Сумма, ВидОплаты, Офис, Контрагент, Договор = Ложь, ПроверятьПостоянного = Истина, Постоянный = Ложь) Экспорт
	
	СтруктураСкидка = Новый Структура("РазмерСкидки, РазмерСкидкиПостоянногоКлиента, ЭтоПостоянныйКлиент, СуммаДокумента", 0, 0, Ложь, 0);
	
	Если ПроверятьПостоянного Тогда
	...
	Если ТипЗнч(ВидОплаты) = Тип("СправочникСсылка.ВидыОплаты") И НЕ ВидОплаты = Справочники.ВидыОплаты.ПустаяСсылка()  Тогда 
		...
		Если СтруктураСкидка.ЭтоПостоянныйКлиент Тогда
			СтруктураСкидка.РазмерСкидкиПостоянногоКлиента = ВидОплаты.СкидкаПостоянномуКлиенту;	
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ВидОплаты.Формула) Тогда
			...
			СтруктураСкидка.РазмерСкидки = СтруктураСкидка.РазмерСкидкиПостоянногоКлиента;
			
		Иначе
			
			СтруктураСкидка.РазмерСкидки = ВидОплаты.Скидка + СтруктураСкидка.РазмерСкидкиПостоянногоКлиента;
...
Функция НачислитьСуммуУправлению(Подразделение, Движения, Сумма, Дата, Комментарий = "") Экспорт
	...
	Иначе
		...
		ЗапросПодразделение.УстановитьПараметр("ВидПодразделения", Перечисления.ВидыПодразделений.Управление);
...
Функция ДоступностьСпецификации(Спецификация) Экспорт
	...
	МассивСтатусов.Добавить(Перечисления.СтатусыСпецификации.ПроверяетсяМатериал);
	МассивСтатусов.Добавить(Перечисления.СтатусыСпецификации.МатериалПроверен);
...
Функция МатериалПодЗаказПроверен(СтатусСпецификации) Экспорт 
	...
	МатериалПроверен = Ложь;
	
	Для каждого Статус Из МассивСтатусов Цикл
		
		Если СтатусСпецификации = Статус Тогда
			МатериалПроверен = Истина;
			Прервать;
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат МатериалПроверен;
...
Функция ЗакрытьДопСоглашенияДоговора(Движения, ДоговорСсылка, ДатаПроведения) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Сумма > 0 Тогда
			...
			Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.ДокументВзаиморасчетов] = Выборка.Ссылка;
			Проводка.СчетКт = ПланыСчетов.Управленческий.Доходы;
		...
		Иначе
			...
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.ДокументВзаиморасчетов] = Выборка.Ссылка;
			Проводка.СчетДт = ПланыСчетов.Управленческий.Расходы;
...
Процедура УстановитьДатуИзготовленияСпецификации(Спецификация) Экспорт
	...
	КоличествоДнейНаИзготовлениеПоНоменклатуре = Спецификация.ДнейНаИзготовлениеПоНоменклатуре;
	
	Если ЗначениеЗаполнено(ДатаОтгрузки) И ЗначениеЗаполнено(ДокументПлановыйЛимит) Тогда
		
		ДнейНаИзготовление = 2 + КоличествоДнейНаИзготовлениеПоНоменклатуре;
		...
		Иначе // РабочихДнейДоОтгрузки < ДнейНаИзготовление
			...
			ДнейНаИзготовление = 1 + КоличествоДнейНаИзготовлениеПоНоменклатуре;
...
Функция ПолучитьЛоготипДилера(Спецификация) Экспорт
	...
	Если ЗначениеЗаполнено(Спецификация)
		...
		//Логотип = Спецификация.Контрагент.Логотип;
		//ИмяФайла = "Логотип_" + Логотип.Наименование + "." + Логотип.ТекущаяВерсияРасширение;
...
Функция ПолучитьЛоготипОфиса(Спецификация) Экспорт
	...
	Если ЗначениеЗаполнено(Спецификация) И ЗначениеЗаполнено(Спецификация.Офис) И ЗначениеЗаполнено(Спецификация.Офис.Логотип) Тогда
		...
		ИмяФайла = Логотип.Наименование + "." + Логотип.ТекущаяВерсияРасширение;	
	КонецЕсли;
...
Функция ПолучитьЛоготипСервер(Спецификация) Экспорт
	
	Если ЗначениеЗаполнено(Спецификация) И Спецификация.Дилерский Тогда
	...
	Если НЕ ЗначениеЗаполнено(СсылкаЛоготип)
		И ЗначениеЗаполнено(Спецификация)
	...
	Если ЗначениеЗаполнено(СсылкаЛоготип) Тогда
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Версия", СсылкаЛоготип.ТекущаяВерсия);
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ОпределитьКомпьютер(СерийныйНомер, ИмяКомпьютера) Экспорт
	...
	Иначе
		...
		Структура.Одобренный = Выборка.Одобренный;
...
// Кладовщикам особый вид операции для некоторых документов.
//
// Параметры
//  Нет
//
// Возвращаемое значение:
//   Перечисления.ХозяйственнаяОперация   - nuff said
//
Функция ПолучитьВидОперацииПоРоли() Экспорт
	
	Если РольДоступна("ДобавлениеИзменениеАвансовогоОтчетаКладовщиом")
		ИЛИ РольДоступна("ДобавлениеИзменениеПоступлениеМатериаловУслугКладовщиком") Тогда
...
Функция ТекущийПользовательАдминистратор() Экспорт
	...
	Возврат УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, ПрофильАдминистратор);
...
Функция ТекущийПользовательПечатьЭтикеток() Экспорт
	...
	Пользователь = ПользователиКлиентСервер.ТекущийПользователь();
	ПрофильПечатьЭтикеток = Справочники.ПрофилиГруппДоступа.ПечатьЭтикеток;
	Возврат УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, ПрофильПечатьЭтикеток);
...
Функция ПолучитьИмяПрисоединенногоФайла(ВидФайла, ВладелецФайла, Расширение = "") Экспорт
	...
	ИначеЕсли ТипЗнч(ВладелецФайла) = Тип("СправочникСсылка.КаталогИзделий")
		И ВидыФайлов.Найти(ВидФайла) <> Неопределено Тогда
	...
	ИначеЕсли ТипЗнч(ВладелецФайла) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		//Если ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.ОсновнаяКартинка Тогда
		//ИначеЕсли ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.КартинкаЛевая Тогда
		//ИначеЕсли ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.КартинкаПравая Тогда
		//ИначеЕсли ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.КрышаЛевая Тогда
		//ИначеЕсли ВидФайла = Перечисления.ВидыПрисоединенныхФайлов.КрышаПравая Тогда
		//КонецЕсли;
