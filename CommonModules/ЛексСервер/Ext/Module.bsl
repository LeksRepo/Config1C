...
Процедура СформироватьПоказателиСотрудников(Движения, Ссылка) Экспорт
	...
	ФизическоеЛицо = ПолучитьЗначениеНастройкиПользователя(Договор.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
	ДобавитьПоказательСотрудника(СвойстваДокумента.Подразделение, ФизическоеЛицо, Движения, Дата, Сумма, 1, Соглашение);
...
Функция ПолучитьЗначениеНастройкиПользователя(Пользователь, Настройка) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Результат) Тогда
		Результат = Пользователь.ФизическоеЛицо;
	КонецЕсли;
...
Функция ПолучитьНомераПередлокИСрочек(фнПользователь) Экспорт
	...
	ФизЛицо = ПолучитьЗначениеНастройкиПользователя(фнПользователь, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
...
Функция ТекущийПользовательПечатьЭтикеток() Экспорт
	...
	Возврат РольДоступна("ПользовательПечатьЭтикеток");
...
Функция ДобавитьПоказателиСотрудникам(ДокОбъект, Движения) Экспорт
	...
	ФизЛицо = ПолучитьЗначениеНастройкиПользователя(ДокОснование.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
	...
	ИначеЕсли ТипДокОснование = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
		ФизЛицо = ПолучитьЗначениеНастройкиПользователя(ДокОснование.Договор.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
		ДобавитьПоказательСотруднику(ФизЛицо, Выручка, ДокОбъект, Движения);
...
Процедура ИзменитьВремяДокумента(Источник,Приращение)
