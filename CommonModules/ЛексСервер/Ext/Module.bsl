...
//
Функция ПолучитьСкидкуДоговора(Подразделение, Период, Сумма, ВидОплаты, Офис, Контрагент, Договор) Экспорт
	
	СтруктураСкидка = Новый Структура("РазмерСкидки, ЭтоПостоянныйКлиент", 0, Ложь);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
   		СтруктураСкидка.РазмерСкидки = 2;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Если Сумма >= Выборка.ВторойПределСумма Тогда
			СтруктураСкидка.РазмерСкидки = Выборка.ВторойПределСкидка;
		ИначеЕсли Сумма >= Выборка.ПервыйПределСумма Тогда
			СтруктураСкидка.РазмерСкидки = Выборка.ПервыйПределСкидка;
		КонецЕсли;
	КонецЕсли;
