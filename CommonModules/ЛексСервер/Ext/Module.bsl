...
Функция ПолучитьРеквизитыОрганизации(Организация) Экспорт
	...
	Результат.Вставить("ПолноеНаименование", Выборка.ПолноеНаименование);
...
Функция ПолучитьПодразделениеПоРегиону(Регион, ВидПодразделения)
...
Функция ПолучитьСтатьюДоходаУправления(Подразделение) Экспорт
	...
	Если ВидПодразделения = Перечисления.ВидыПодразделений.Логистик Тогда
		Статья = Справочники.СтатьиДоходовРасходов.ВыручкаОтЛогистикиНаУправление;
	ИначеЕсли ВидПодразделения = Перечисления.ВидыПодразделений.Производство Тогда
		Статья = Справочники.СтатьиДоходовРасходов.ВыручкаОтПроизводстваНаУправление;
	ИначеЕсли ВидПодразделения = Перечисления.ВидыПодразделений.Розница Тогда
		Статья = Справочники.СтатьиДоходовРасходов.ВыручкаОтРозницыНаУправление;
	КонецЕсли;
...
Функция ПеремещениеМатериаловСЛогистики(Ссылка, Движения) Экспорт
	...
	|ГДЕ
	|	СписокНоменклатуры.Ссылка В(&Ссылка)
	|	И СписокНоменклатуры.Количество <> 0
	|
	|СГРУППИРОВАТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Комплектация Тогда
			...
			Движение.СчетДт = ПланыСчетов.Управленческий.ГотоваяПродукция;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = Спецификация;
...
Процедура ДобавитьПоказательСотрудника(Подразделение, ФизическоеЛицо, Движения, Дата, Сумма, Количество, Соглашение)
	...
	Проводки.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.ВидыПоказателей] 	= Перечисления.ВидыПоказателейСотрудников.СтоимостьЗаключенныхДоговоров;
	...
	Если НЕ Соглашение Тогда
		...
		Проводки.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.ВидыПоказателей] 	= Перечисления.ВидыПоказателейСотрудников.КоличествоЗаключенныхДоговоров;
		Проводки.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.ФизическиеЛица] 	= ФизическоеЛицо;
...
Функция РеализацияИзделийРозничныеПроводки(МассивСпецификаций, Дата, Движения) Экспорт
	...
	Для каждого Спецификация Из МассивСпецификаций Цикл
		...
		СвойстваСпецификации = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Спецификация, "Контрагент, Подразделение, СуммаДокумента, ВнутренняяСтоимостьСпецификации, Производство");
		
		// Оприходование изделия
		...
		Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = Документы.Спецификация.ПолучитьДоговор(Спецификация);
		...
		Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = Документы.Спецификация.ПолучитьДоговор(Спецификация);
		
		// Доходы
		Проводка = Движения.Управленческий.Добавить();
		Проводка.Период = Дата;
		Проводка.Подразделение = СвойстваСпецификации.Подразделение;
		Проводка.Сумма = СвойстваСпецификации.СуммаДокумента;
		
		Проводка.СчетДт = ПланыСчетов.Управленческий.ВзаиморасчетыСПокупателями;
		Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = Спецификация;
		
		Если СвойстваСпецификации.Контрагент = Справочники.Контрагенты.ЧастноеЛицо Тогда
		...
		Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = СтатьяДохода;	КонецЦикла;
