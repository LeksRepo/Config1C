...
Функция ПеремещениеМатериаловСЛогистики(Ссылка, Движения) Экспорт
	...
	|			ТОГДА 1
	|		ИНАЧЕ СписокНоменклатуры.Номенклатура.КоэффициентБазовых
	|	КОНЕЦ * ЕСТЬNULL(ЦеныНоменклатурыПоПодразделениямСрезПоследних.Внутренняя, 0) КАК СуммаРеализации
	|ИЗ
	|	СписокНоменклатуры КАК СписокНоменклатуры
	...
	|							ИЗ
	|								СписокНоменклатуры))) КАК ЦеныНоменклатурыПоПодразделениямСрезПоследних
	|		ПО (СписокНоменклатуры.Номенклатура.БазоваяНоменклатура = ЦеныНоменклатурыПоПодразделениямСрезПоследних.Номенклатура
	|				ИЛИ СписокНоменклатуры.Номенклатура = ЦеныНоменклатурыПоПодразделениямСрезПоследних.Номенклатура)";
