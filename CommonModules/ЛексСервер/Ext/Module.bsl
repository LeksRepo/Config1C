...
Функция ПрисвоитьФискальныйНомер(Документ) Экспорт
	...
	Договор = ?(НЕ РасходДенежныхСредств, Документ.Субконто2Кт, Документ.Субконто2Дт);
	Запрос = Новый Запрос;
	...
	|	И ПриходДенежныхСредств.Субконто2Кт = &Договор";
	...
	Если НЕ РасходДенежныхСредств Тогда
		
		Если ТипЗнч(Документ.Субконто2Кт) = Тип("ДокументСсылка.Договор") Тогда
			
			НомерДоговора = Документ.Субконто2Кт.Номер;
			
		ИначеЕсли ТипЗнч(Документ.Субконто2Кт) = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
			
			НомерДоговора = Документ.Субконто2Кт.Договор.Номер;
			
		КонецЕсли;
		
	Иначе
		
		Если ТипЗнч(Документ.Субконто2Дт) = Тип("ДокументСсылка.Договор") Тогда
			
			НомерДоговора = Документ.Субконто2Дт.Номер;
			
		ИначеЕсли ТипЗнч(Документ.Субконто2Дт) = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
			
			НомерДоговора = Документ.Субконто2Дт.Договор.Номер;
