...
// Создает проводки по передаче материалов в производство.
// Вызывается из Требования накладной и Комплектации
// Параметры
//  ТаблицаМатериалов  - ТаблицаЗначений - <описание параметра>
//  Подразделение  - СправочникСсылка.Подразделения - <описание параметра>
//  Склад  - СправочникСсылка.Склады - <описание параметра>
//  Движения  - КоллекцияДвижений - <описание параметра>
//  МоментВремени  - МоментВремени - <описание параметра>
//
// Возвращаемое значение:
//   ТаблицаЗначений   - нехватка материалов
//
Функция ПеремещениеМатериаловВПроизводство(ТаблицаМатериалов, Подразделение, Склад, Движения, МоментВремени) Экспорт
	...
	НехваткаМатериалов.Колонки.Добавить("КоличествоТребуется");
	НехваткаМатериалов.Колонки.Добавить("КоличествоОстаток");
	НехваткаМатериалов.Колонки.Добавить("Нехватка");
	...
	ЭлементБлокировки.ИсточникДанных = ТаблицаМатериалов;
	ЭлементБлокировки.ИспользоватьИзИсточникаДанных(ПланыВидовХарактеристик.ВидыСубконто.Номенклатура, "Номенклатура");
	Блокировка.Заблокировать();
	...
	|							тз.Номенклатура
	|						ИЗ
	|							тзМатериалы КАК тз)) КАК УправленческийОстатки
	|		ПО тзМатериалы.Номенклатура = УправленческийОстатки.Субконто1";
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Нехватка > 0 Тогда
			...
			НоваяСтрокаНехватка = НехваткаМатериалов.Добавить();
			НоваяСтрокаНехватка.Номенклатура = Выборка.Номенклатура;
			НоваяСтрокаНехватка.НомерСтроки = Выборка.НомерСтроки;
			НоваяСтрокаНехватка.КоличествоТребуется = Выборка.Количество;
			НоваяСтрокаНехватка.КоличествоОстаток = Выборка.КоличествоОстаток;
			НоваяСтрокаНехватка.Нехватка = Нехватка;
		...
		БазоваяНоменклатура = ?(Выборка.Базовый, Выборка.Номенклатура, Выборка.БазоваяНоменклатура);
		...
		Движение.Сумма = Себестоимость;
		Движение.СчетДт = ПланыСчетов.Управленческий.ОсновноеПроизводство;
		Движение.СубконтоДт.Номенклатура = БазоваяНоменклатура;
		Движение.КоличествоДт = Выборка.Количество * Выборка.КоэффициентБазовых;
		Движение.СчетКт = ПланыСчетов.Управленческий.МатериалыНаСкладе;
		...
		Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.РасходыНаПроизводственныйМатериал;
		Движение.СчетКт = ПланыСчетов.Управленческий.ОсновноеПроизводство;
		Движение.СубконтоКт.Номенклатура = БазоваяНоменклатура;
...
Процедура СформироватьПоказателиСотрудников(Движения, Ссылка) Экспорт
	...
	Если ТипЗнч(Замер) = Тип("ДокументСсылка.Замер") и не Замер = Документы.Замер.ПустаяСсылка() Тогда
...
Функция РеализацияИзделийРозничныеПроводки(МассивСпецификаций, Дата, Движения, ВызовИзАкта = Ложь) Экспорт
	...
	// переделай!!! будь мужиком блеать, признай что говно стало!!!!!1111один
...
//
Функция НачислитьСуммуУправлению(Подразделение, Движения, Сумма, Дата) Экспорт
	...
	Подразделение,
	ПодразделениеУправление, 
	Справочники.СтатьиДоходовРасходов.РасходПеречислениеПроцентаНаУправление, 
...
Функция СоздатьПроводкуВзаиморасчетыСПодразделением(Движения, ИзмерениеПодразделение, СубконтоПодразделение, Статья, ДебетКредит, Дата, Сумма)
