...
Функция ДоговорНаЮрЛицо(Договор) Экспорт
	
	Возврат Договор.Спецификация.Контрагент.ЮридическоеЛицо;
...
Функция ПолучитьПредставлениеДокумента(Данные, Представление, СтандартнаяОбработка) Экспорт
	...
	Если ТипЗнч(Данные.Ссылка) = Тип("ДокументСсылка.НарядЗадание") Тогда
		
		Дата = Формат(Данные.Ссылка.ДатаИзготовления, "ДФ=dd.MM.yy");
		Представление = ИмяДокумента + " № " + Номер + " на " + Дата;
		
	ИначеЕсли ТипЗнч(Данные.Ссылка) = Тип("СправочникСсылка.Заметки") Тогда
		
		Представление = Данные.Ссылка.Метаданные().ПредставлениеОбъекта + " к " + Данные.Ссылка.Предмет;
...
Процедура Перепроведение() Экспорт
	...
	НоваяДатаЗапрета = НачалоДня(ТекущаяДата()) - 7 * 86400;
...
Функция ПолучитьСписокАктивныхОбращений() Экспорт
	...
	|ГДЕ
	|	Обращение.Проведен
	|	И НЕ Обращение.Архив
	|	И Обращение.Ответственный = &Пользователь
	|	И НАЧАЛОПЕРИОДА(Обращение.ДатаКонтроля, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Период, ДЕНЬ)";
	
	МассивОбращений = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Обращение");
...
Функция ПолучитьЭтажИзАдреса(Адрес) Экспорт
...
Функция ПечататьТорг12(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Если Спецификация Тогда
		...
		"ВЫБРАТЬ
		|	Спец.Автор,
		|	Спец.Дата КАК ДатаДокумента,
		|	Спец.Контрагент,
		|	Спец.Номер КАК НомерДокумента,
		|	Спец.Подразделение,
		|	ВЫБОР
		|		КОГДА Спец.КоэффициентТОРГ12 <> 0
		|			ТОГДА Спец.КоэффициентТОРГ12
		|		ИНАЧЕ Спец.Подразделение.КоэффициентТОРГ12
		|	КОНЕЦ КАК КоэффициентТОРГ12,
		|	Спец.Подразделение.ОсновнойСклад,
		|	ВЫБОР
		|		КОГДА НЕ Договор.Ссылка ЕСТЬ NULL
		|			ТОГДА Договор.СуммаДокумента
		|		ИНАЧЕ Спец.СуммаДокумента
		|	КОНЕЦ КАК СуммаДокумента,
		|	1 КАК Номер,
		|	Спец.Ссылка КАК Номенклатура,
		|	Спец.Контрагент.ПолноеНаименование КАК ПолноеНаименование,
		|	"""" КАК АдресДоставки,
		|	Спец.Контрагент.Телефон КАК Телефон,
		|	Спец.Контрагент.БанковскиеРеквизиты КАК БанковскиеРеквизиты,
		|	1 КАК Количество,
		|	""Шт."" КАК ЕдиницаИзмерения,
		|	""Шт."" КАК ЕдиницаНаименование,
		|	""Без НДС"" КАК СтавкаНДС,
		|	796 КАК ЕдиницаКодПоОКЕИ,
		|	0 КАК СуммаНДС,
		|	Спец.Подразделение.ФактическийАдрес КАК ФактическийАдрес,
		|	Спец.Подразделение.ОрганизацияДляФизЛиц КАК ОрганизацияДляФизЛиц,
		|	Спец.Подразделение.ОрганизацияДляЮрЛиц КАК ОрганизацияДляЮрЛиц,
		|	ЕСТЬNULL(Договор.Ссылка, ""Без договора"") КАК Договор
		|ИЗ
		|	Документ.Спецификация КАК Спец
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Договор КАК Договор
		|		ПО Спец.Ссылка = Договор.Спецификация
		|ГДЕ
		|	Спец.Ссылка В(&МассивДокументов)"
	...
	Иначе
		...
		|	),
		|	РеализацияМатериалов.ОбрезкиХлыстовогоМатериала.(
		|		НомерСтроки КАК Номер,
		|		Номенклатура,
		|		Номенклатура.Код КАК ТоварКод,
		|		РазмерПродаваемый КАК Количество,
		|		Цена,
		|		Сумма КАК СуммаБезНДС,
		|		Сумма КАК СуммаСНДС,
		|		""Без НДС"" КАК СтавкаНДС,
		|		0 КАК СуммаНДС,
		|		""пог. м."" КАК ЕдиницаИзмерения,
		|		""пог. м."" КАК ЕдиницаНаименование,
		|		""018"" КАК ЕдиницаКодПоОКЕИ
		|	),
		|	РеализацияМатериалов.Подразделение.ФактическийАдрес КАК ФактическийАдрес,
		|	РеализацияМатериалов.Подразделение.ОрганизацияДляФизЛиц КАК ОрганизацияДляФизЛиц,
		|	РеализацияМатериалов.Подразделение.ОрганизацияДляЮрЛиц КАК ОрганизацияДляЮрЛиц,
		|	ВЫБОР
		|		КОГДА РеализацияМатериалов.КоэффициентТОРГ12 <> 0
		|			ТОГДА РеализацияМатериалов.КоэффициентТОРГ12
		|		ИНАЧЕ РеализацияМатериалов.Подразделение.КоэффициентТОРГ12
		|	КОНЕЦ КАК КоэффициентТОРГ12
		|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗаказчикЮрЛицо = Заказчик.ЮридическоеЛицо;
		КоличествоЭкземпляров = ?(ЗаказчикЮрЛицо,2,1);
		Коэф = 1;
		...
		Для Сч = 1 По КоличествоЭкземпляров Цикл 
			...
			РеквизитыФиз = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Выборка.ОрганизацияДляФизЛиц, "ПолноеНаименование, ИНН, РасчетныйСчет, БИК, КорреспондирующийСчет, Банк");
			РеквизитыЮр = ОбщегоНазначения.ПолучитьЗначенияРеквизитов(Выборка.ОрганизацияДляЮрЛиц, "ПолноеНаименование, ИНН, РасчетныйСчет, БИК, КорреспондирующийСчет, Банк");
			
			РеквизитыФиз = РеквизитыФиз.ПолноеНаименование + ", ИНН " + РеквизитыФиз.ИНН + ", " + Выборка.ФактическийАдрес + ", р/с " + РеквизитыФиз.РасчетныйСчет + ", Банк/БИК " + РеквизитыФиз.Банк + "/"+ РеквизитыФиз.БИК + ", корр/с " + РеквизитыФиз.КорреспондирующийСчет; 
			РеквизитыЮр = РеквизитыЮр.ПолноеНаименование + ", ИНН " + РеквизитыЮр.ИНН + ", " + Выборка.ФактическийАдрес + ", р/с " + РеквизитыЮр.РасчетныйСчет + ", Банк/БИК " + РеквизитыЮр.Банк + "/"+ РеквизитыЮр.БИК + ", корр/с " + РеквизитыЮр.КорреспондирующийСчет; 
			
			ПредставлениеОрганизации = РеквизитыЮр;
			ПредставлениеПоставщика = РеквизитыЮр;;
			...
			Если Спецификация Тогда
				ОбластьШапка.Параметры.Основание = Выборка.Договор;
			КонецЕсли;
			
			ПредставлениеПлательщика = "";
			ПредставлениеГрузополучателя = "";
			
			Если ЗаказчикЮрЛицо Тогда
				
				РекивизыЗаказчик = Заказчик.ПолноеНаименование + ", ИНН " + Заказчик.ИНН + ", " + Заказчик.ЮридическийАдрес + ", " + Заказчик.БанковскиеРеквизиты; 
				
				ПредставлениеПлательщика = РекивизыЗаказчик;
				ПредставлениеГрузополучателя = РекивизыЗаказчик;

			Иначе
				
				Коэф = Выборка.КоэффициентТОРГ12;
				ПредставлениеПлательщика = РеквизитыФиз;
				ПредставлениеГрузополучателя = РеквизитыФиз;

			КонецЕсли;
			
			ОбластьШапка.Параметры.ПредставлениеПлательщика = ПредставлениеПлательщика;
			...
			Если Спецификация Тогда
				...
				ВсегоКоличество = 1;
				ВсегоСумма = Выборка.СуммаДокумента * Коэф;
				
				ОбластьСтрока.Параметры.Заполнить(Выборка);
				ОбластьСтрока.Параметры.Цена = ВсегоСумма; 
				ОбластьСтрока.Параметры.СуммаБезНДС = ВсегоСумма;
				ОбластьСтрока.Параметры.СуммаСНДС = ВсегоСумма;
			...
			Иначе
				...
				Пока ВыборкаСписокНоменклатуры.Следующий() Цикл
					...
					Цена = ВыборкаСписокНоменклатуры.Цена * Коэф;
					Сумма = ВыборкаСписокНоменклатуры.СуммаБезНДС * Коэф; 
					
					ВсегоСумма = Сумма + ВсегоСумма;
					
					ОбластьСтрока.Параметры.Заполнить(ВыборкаСписокНоменклатуры);
					
					ОбластьСтрока.Параметры.Цена = Цена; 
					ОбластьСтрока.Параметры.СуммаБезНДС = Сумма;
				...
				ВыборкаСписокНоменклатуры = Выборка.ОбрезкиХлыстовогоМатериала.Выбрать();
				
				Пока ВыборкаСписокНоменклатуры.Следующий() Цикл
					
					ВсегоКоличество = (ВыборкаСписокНоменклатуры.Количество / 1000) + ВсегоКоличество;
					Цена = ВыборкаСписокНоменклатуры.Цена * Коэф;
					Сумма = ВыборкаСписокНоменклатуры.СуммаБезНДС * Коэф; 
					...
					ОбластьСтрока.Параметры.Количество = (ВыборкаСписокНоменклатуры.Количество / 1000);
					ОбластьСтрока.Параметры.Цена = Цена; 
					ОбластьСтрока.Параметры.СуммаБезНДС = Сумма;
					ОбластьСтрока.Параметры.СуммаСНДС = Сумма;
					ОбластьСтрока.Параметры.Номер = НомерСтроки;
			...
			СуммаПрописью = ЧислоПрописью(ВсегоСумма, СтрокиСообщений.ФормСтрока(), СтрокиСообщений.ПарПредмета());
			
			ОбластьПодвал.Параметры.Заполнить(Выборка);
			ОбластьПодвал.Параметры.СуммаПрописью = СуммаПрописью;
			
			ОбластьПодвал.Параметры.ДатаДокумента = Формат(Выборка.ДатаДокумента, "ДЛФ=ДД");
...
Функция ГрупповаяСменаСтатуса(МассивСпецификаций, НовыйСтатус, ПроверяемыйСтатус = Неопределено) Экспорт
	...
	Если Массив.Количество() > 0 Тогда
		...
		Документы.Спецификация.ОтправитьСтатусыНаСайт(Массив);
...
Функция ВыборМакетов(Договоры, ДоговорДилера) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВТ_ДоговорДвери.Спецификация КАК Спецификация,
	...
	Пока ВыборкаГруппа.Следующий() Цикл
		...
		Если ВыборкаГруппа.Спецификация.ПакетУслуг = Перечисления.ПакетыУслуг.СамовывозОтПроизводителя Тогда
			СтруктураДоговора.Вставить("УсловияДоставки", "");
		Иначе
			СтруктураДоговора.Вставить("УсловияДоставки", "УсловияДоставкиВыписка");
		КонецЕсли;
		
		Если ВыборкаГруппа.Спецификация.ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж Тогда
			СтруктураДоговора.Вставить("Инструкция", "");
		Иначе
...
Функция ПолучитьИзделиеПоНастройке(Настройка) Экспорт
...
Функция ПолучитьНомераПередлокИСрочек(фнПользователь) Экспорт
	...
	|				И (Спецификация.Изделие.ЭтоПеределка
...
Функция ПолучитьТаблицуТиповИВремени()
	...
	ДобавитьСтрокуТипВремя(тз, "ИнвентаризацияМатериалов", Полночь + 8 * Часов);
...
Процедура СписатьНеноменклатурныйМатериал(Движение, Подразделение, Дата, Сумма) Экспорт
	...
	Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.РасходыСебестоимостьМатериалаПроизводство;
	
КонецПроцедуры

Функция ПроверитьИзменениеСтатьи(Ошибки, Объект) Экспорт
	
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		
		ТекстОшибки = "Запрещено добавлять новые элементы";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
		
	КонецЕсли;
	
	Если Объект.Ссылка.Родитель <> Объект.Родитель Тогда
		
		ТекстОшибки = "Запрещено изменять группу статьи";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
	...
	Если Объект.Ссылка.Наименование <> Объект.Наименование Тогда
		...
		ТекстОшибки = "Запрещено изменять наименование статьи";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
		
	КонецЕсли;
	
	Если НЕ Объект.ЭтоГруппа
		И Объект.Родитель.ВидСтатьи <> Объект.ВидСтатьи Тогда
		
		ТекстОшибки = "Вид статьи у группы и элемента должен совпадать";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
...
Функция ПолучитьПараметрыФункциональныхОпций() Экспорт
	
	Стр = Новый Структура();
	
	Если ЗначениеЗаполнено(ПараметрыСеанса.ТекущееПодразделение) Тогда
	
		Стр.Вставить("ТекущееПодразделение", ПараметрыСеанса.ТекущееПодразделение);
		Стр.Вставить("ПолныйУчет", Перечисления.ВидыНастроекПодразделений.ПолныйУчет);
...
Функция ПолучитьПараметрДилера(Контрагент, Параметр) Экспорт
...
Функция ПолучитьПроцентБонусаЗаОплатуНаличными(Подразделение, Дата) Экспорт
	
	Возврат ЛексСервер.ПолучитьНастройкуПодразделения(Подразделение, Перечисления.ВидыНастроекПодразделений.ПроцентБонусаЗаОплатуНаличными, Дата);
	
КонецФункции

Функция РассчитатьБонусЗаОплатуНаличными(Подразделение, Дата, Сумма) Экспорт
	
	Результат = 0;
	
	ПроцентБонуса = ПолучитьПроцентБонусаЗаОплатуНаличными(Подразделение, Дата);
	Если ЗначениеЗаполнено(ПроцентБонуса) Тогда
		Результат = Сумма * ПроцентБонуса / 100;
	КонецЕсли;
	
	Результат = Окр(Результат, 1);
	Результат = Окр(Результат, 0, РежимОкругления.Окр15как10);
