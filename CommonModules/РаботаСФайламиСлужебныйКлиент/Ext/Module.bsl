...
Функция ЗакончитьРедактирование(ОбъектСсылка,
	...
	Если РасширениеПодключено Тогда
		...
		Если ПолныйПутьКФайлу = "" Тогда
			ПолныйПутьКФайлу = ФайловыеФункцииСлужебныйКлиент.ПолучитьПолныйПутьКФайлуВРабочемКаталоге(ДанныеФайла);
		КонецЕсли;
...
Процедура ОбновитьИзФайлаНаДискеРеализация(ДанныеФайла, ИдентификаторФормы, ДиалогПолноеИмяФайла, 
	...
	ВРабочемКаталогеВладельца = Ложь;
	ПолноеИмяФайла = "";
	ФайлВРабочемКаталоге = Ложь; //ФайлНаходитсяВЛокальномКэшеФайлов(Неопределено, ПредыдущаяВерсия, ПолноеИмяФайла, ВРабочемКаталогеНаЧтение, ВРабочемКаталогеВладельца);
	...
	ПолучитьФайлВерсииВРабочийКаталог(ДанныеФайла, ДиалогПолноеИмяФайла, ИдентификаторФормы);
...
#Если Не ВебКлиент Тогда
	...
	МаксРазмер = ЛексКлиентСервер.ПолучитьМаксимальныйРазмерКэшаФайлов();
...
Функция ПолучитьПутьФайлаВРабочемКаталоге(ДанныеФайла)
	...
	Если Не ПустаяСтрока(ИмяФайла) Тогда
		...
		Иначе
			ИмяФайлаСПутем = ЛексКлиентСервер.ПолучитьПапкуДляКэшФайлов(ИмяКаталога) + ИмяФайла;
...
Функция ПолучитьССервераИЗарегистрироватьВЛокальномКэшеФайлов(
	...
	Если ИмяФайлаСПутем = "" Тогда
		...
		Если НЕ ПустаяСтрока(ИмяФайла) Тогда
			ИмяФайлаСПутем = ЛексКлиентСервер.ПолучитьПапкуДляКэшФайлов(ИмяКаталога) + ИмяФайла;
			//ИмяФайлаСПутем = ИмяКаталога + ФайловыеФункцииСлужебныйКлиентСервер.ПолучитьУникальноеИмяСПутем(ИмяКаталога, ИмяФайла, ОбщегоНазначенияКлиентПовтИсп.ТипПлатформыКлиента());
		КонецЕсли;
...
#КонецЕсли
	...
	Попытка
		...
		Если ДанныеФайла.Свойство("ПутьОбновленияИзФайлаНаДиске") Тогда
			...
			Если ДанныеФайла.ПутьОбновленияИзФайлаНаДиске <> ИмяФайлаСПутем Тогда
		...
		Иначе
			...
			ТФайл = Новый Файл(ИмяФайлаСПутем);	
			Если ТФайл.Существует() Тогда
				ТФайл.УстановитьТолькоЧтение(Ложь);
			КонецЕсли;
...
Процедура ОбходФайловДляПроверкиПредельногоРазмера(
	...
	МаксРазмерФайла = ЛексКлиентСервер.ПолучитьМаксимальныйРазмерФайла();			
