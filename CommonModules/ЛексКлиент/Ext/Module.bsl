...
Функция ОбщегоНазначенияКлиентПереопределяемыйПриНачалеРаботыСистемы() Экспорт
	...
	#Если НЕ ВебКлиент Тогда
		...
		НЕОбновлятьФайлы = ЛексСервер.НастройкаПользователя(Пользователь,"Не обновлять файлы");
		
		Если НЕ ЗначениеЗаполнено(НЕОбновлятьФайлы) ИЛИ НЕ НЕОбновлятьФайлы Тогда
