...
Функция ПараметрыПриЗавершенииРаботы()
	...
	ВыполнятьПриЗавершенииРаботы = ?(НастройкиРезервногоКопирования = Неопределено,
		Ложь, НастройкиРезервногоКопирования.ВыборПунктаНастройки = 2);
...
Функция НастройкиРезервногоКопирования(НачалоРаботы = Ложь) Экспорт
	...
	Если Не Результат.Свойство("ПервыйЗапуск") Тогда
		Результат.Вставить("ПервыйЗапуск", Истина);
	КонецЕсли;
...
Функция ВариантОповещения() Экспорт
	...
	Если ПараметрыОповещенияОКопировании.ВыборПунктаНастройки = 3 Тогда
		
		Результат = "НеОповещать";
		
	ИначеЕсли ПараметрыОповещенияОКопировании.ВыборПунктаНастройки = 0 Тогда
...
Функция НачальноеЗаполнениеНастроекРезервногоКопирования(СохранятьПараметры = Истина) Экспорт
	...
	Параметры.Вставить("ВыборПунктаНастройки", 0);
...
Функция ПараметрыУдаленияРезервныхКопийПоУмолчанию()
	...
	ПараметрыУдаления.Вставить("ПроизводитьУдаление", Истина);
