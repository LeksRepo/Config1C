...
//  ОбработчикРезультата    - ОписаниеОповещения - процедура, в которую необходимо передать управление при завершении.
//                            Параметры вызываемой процедуры:
//                             ПодписьСформирована - Булево - Истина, если подпись сформирована успешно, иначе Ложь;
//                             ДополнительныеПараметры - Произвольный - значение, которое было указано при создании объекта оповещения.
//  ПрисоединенныйФайл - Ссылка присоединенный файл.
//  ДанныеФайла        - Структура с данными файла.
//
Процедура СформироватьПодписьФайла(ОбработчикРезультата, Знач ПрисоединенныйФайл, Знач ДанныеФайла) Экспорт
	Параметры = Новый Структура;
	Параметры.Вставить("ОбработчикРезультата", ОбработчикРезультата);
	Параметры.Вставить("ПрисоединенныйФайл", ПрисоединенныйФайл);
	ОписаниеОповещения = Новый ОписаниеОповещения("СформироватьПодписьФайлаПодписьПолучена", ЭтотОбъект, Параметры);
	ВыбратьСертификатыЭПИСформироватьДанныеПодписи(ОписаниеОповещения, ПрисоединенныйФайл, ДанныеФайла);
КонецПроцедуры

// Продолжение процедуры СформироватьПодписьФайла.
Процедура СформироватьПодписьФайлаПодписьПолучена(ДанныеПодписи, ДополнительныеПараметры) Экспорт
	ПодписьСформирована = ДанныеПодписи <> Неопределено;
	Если ПодписьСформирована Тогда
		ПрисоединенныеФайлыСлужебныйВызовСервера.ЗанестиИнформациюОднойПодписи(ДополнительныеПараметры.ПрисоединенныйФайл, ДанныеПодписи);
	КонецЕсли;
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОбработчикРезультата, ПодписьСформирована);
КонецПроцедуры
