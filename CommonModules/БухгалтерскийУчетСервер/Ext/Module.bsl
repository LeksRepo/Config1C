...
Процедура ПроведениеДенежныхСредств(Ссылка, Движения) Экспорт
	...
	Для Каждого Элемент Из Ссылка.СчетКт.ВидыСубконто Цикл
		Движение.СубконтоКт[Элемент.ВидСубконто] = ?(Ссылка["Субконто" +Элемент.НомерСтроки+ "Кт"] = Документы.Спецификация.ПустаяСсылка(), Неопределено, Ссылка["Субконто" +Элемент.НомерСтроки+ "Кт"]);
	КонецЦикла;
