	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		Если Найти(Реквизит, ".") > 0 Тогда
			...
			Если Колонки.Количество() > 0 Тогда
				...
				КолонкиСтрокой = СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Колонки);
...
Функция ПолучитьРеквизитыФормы(Форма)
	...
	Для Каждого Реквизит Из Реквизиты Цикл
		...
		Если ТипЗнч(Форма[Реквизит.Имя]) = Тип("ДанныеФормыКоллекция") Тогда
			...
			РеквизитыСтрокой.Добавить(СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(Колонки, "."));
	...
	Возврат СтроковыеФункцииКлиентСервер.СтрокаИзМассиваПодстрок(РеквизитыСтрокой);
КонецФункции
...
Функция ВыгрузкаСсылкиРазрешена(УзелОбмена, Ссылка, ДополнительныеСвойства=Неопределено) Экспорт
	...
	Если ОбъектСсылки = Неопределено Тогда
		// Объект удален удаление объекта посылаем всегда.
		Возврат Истина;
	КонецЕсли;
