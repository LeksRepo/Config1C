...
Процедура ЗаполнитьСвойстваВидаДоступа(Знач Свойства) Экспорт
	...
	Если Свойства.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.Подразделения Тогда
		...
		//Свойства.ВидДоступаИспользуется = Истина;
		//Свойства.ВидДоступаИспользуетсяВсегда = Истина;
		
	ИначеЕсли Свойства.ВидДоступа = ПланыВидовХарактеристик.ВидыДоступа.Счета Тогда
														  
		Свойства.Таблицы.Добавить("ПланСчетов.Управленческий");
		Свойства.ВидДоступаЕдинственныйДляТипаЗначенияДоступа = Истина;
		Свойства.ВидДоступаБезГруппЗначенияДоступа = Истина;
		//Свойства.ВидДоступаИспользуется = Истина;
		//Свойства.ВидДоступаИспользуетсяВсегда = Истина;	
...
Функция УпрощенныйИнтерфейсНастройкиПравДоступа() Экспорт
	...
	// { Васильев Александр Леонидович [23.01.2013]
...
Функция ПолучитьПользователяКонтрагента(Контрагент) Экспорт
	
	Пользотватель = Неопределено;
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Выборка.Следующий();
		Пользотватель= Выборка.Ссылка;
	КонецЕсли;
	
	Возврат Пользотватель;
