...
Процедура КомандаОбновитьИндексППДВыполнить(ИнформацияОбАктуальностиИндексаППД,ППДВключен,ИндексАктуален) Экспорт
	...
	ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросОНеобходимостиОбновленияПолнотекстовогоПоиска", ЭтотОбъект);
...
// Обработчик окончания выбора типа документа при создании документа взаимодействий
//
// Параметры:
//  РезультатВыборка  - ЭлементСпискаЗначений - содержит в значение строковое представление типа создаваемого документа.
//  ДополнительныеПараметры  - Структура - содержит одно поле - "ПараметрыСоздания".
//
Процедура ВыборТипаДокументаДляСозданияПриЗавершении(РезультатВыбора, ДополнительныеПараметры) Экспорт

	Если НЕ РезультатВыбора = Неопределено Тогда
		
		СоздатьНовоеВзаимодействие(РезультатВыбора.Значение,ДополнительныеПараметры.ПараметрыСоздания);
...
Процедура СписокПередНачаломДобавления(Элемент, Отказ, Копирование,ТолькоПочта,ДокументыДоступныеДляСоздания,ПараметрыСоздания = Неопределено) Экспорт
	...
	Иначе
		...
		Иначе
			...
			ДополнительныеПараметры = Новый Структура("ПараметрыСоздания", ПараметрыСоздания);
			ОбработчикОповещения = Новый ОписаниеОповещения("ВыборТипаДокументаДляСозданияПриЗавершении", ЭтотОбъект, ДополнительныеПараметры);
			ДокументыДоступныеДляСоздания.ПоказатьВыборЭлемента(ОбработчикОповещения, НСтр("ru = 'Выберите тип документа'"));
