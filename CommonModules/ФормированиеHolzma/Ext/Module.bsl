...
Функция УдалитьСпецСимволы(Строка)
	...
	Для Каждого С Из Мас Цикл
		...
		Строка = СформироватьСтрокуИзМассива(Строка, "");
	КонецЦикла;
			...
			Для каждого СтрокаЛисты Из СтрокаНоменклатура.Строки Цикл
				...
				Если (СтрокаЛисты.ВысотаЛиста = СтрокаЛисты.Номенклатура.ДлинаДетали
					...
					Если СтрокаЛисты.ПоперечныйПил Тогда
						...
						Иначе
							НоваяСтрока.ШиринаЛиста = СтрокаЛисты.Номенклатура.ШиринаДетали + 20; 
						КонецЕсли;
					...
					Иначе
						...
						Иначе
							НоваяСтрока.ВысотаЛиста = СтрокаЛисты.Номенклатура.ДлинаДетали + 20; 
						КонецЕсли;
				...
				Иначе
					...
					Если СтрокаЛисты.ПоперечныйПил Тогда
						 НоваяСтрока.ШиринаЛиста = СтрокаЛисты.ШиринаЛиста + 20;
					Иначе
						 НоваяСтрока.ВысотаЛиста = СтрокаЛисты.ВысотаЛиста + 20;
					КонецЕсли;
