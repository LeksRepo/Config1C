...
Функция СформироватьПрибыльПоСпецификациям(МассивСпецификаций, Движения, Дата, ВызовИзАкта = Ложь) Экспорт
	...
	Пока ВыборкаПоСпецификациям.Следующий() Цикл
		...
		Если СуммаОфиснойНаценки > 0 Тогда
			...
			Проводка.СчетКт = ПланыСчетов.Управленческий.Доходы;
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.ДоходыРозничнаяНаценка;
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = ВыборкаПоСпецификациям.Спецификация;
			
		ИначеЕсли СуммаОфиснойНаценки < 0 Тогда
			
			ТекстСоообщения = "Ошибка 673 :" + ВыборкаПоСпецификациям.Спецификация + " отрицательная наценка, сообщите программистам.";
		...
		Пока ВыборкаПоСтатьям.Следующий() Цикл
			...
			Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СпецификацияДоговор] = ВыборкаПоСтатьям.Спецификация;
