...
Процедура СписатьСоСкладаГотовойПродукции(МассивСпецификаций, МоментВремени, Подразделение, Движения, Ошибки) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если (Дата < Дата("20170701000000")
			...
			И НЕ Выборка.Изделие.ЭтоДопСоглашение)
...
Функция СформироватьПрибыльПоСпецификациям(МассивСпецификаций, Движения, Дата, ВызовИзАкта = Ложь) Экспорт
	...
	|	И НЕ СпецификацияСписокНоменклатуры.Ссылка.Изделие.ЭтоПеределка
	|	И НЕ СпецификацияСписокНоменклатуры.Ссылка.Изделие.ЭтоДопСоглашение
	|	И НЕ СпецификацияСписокНоменклатуры.Ссылка.Изделие.ЭтоГарантийноеОбслуживание
	|	И (&ВызовИзАкта
	|			ИЛИ ВЫБОР
