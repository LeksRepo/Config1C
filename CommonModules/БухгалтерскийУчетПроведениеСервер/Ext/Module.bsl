...
Процедура СформироватьПроводкиАктВыполнения(МассивСпецификаций, МоментВремени, Договор, Подразделение, Движения, Ошибки) Экспорт
	...
	Если НЕ Выборка.СвоиМонтажи Тогда
		...
		Управление = Константы.Управление.Получить();
		ПроцентСЗаказаУправлениюПроизводство = ЛексСервер.ПолучитьНастройкуПодразделения(Выборка.Производство, Перечисления.ВидыНастроекПодразделений.ПроцентСЗаказаУправлению, Дата);
		
		Если ЗначениеЗаполнено(Управление) И ЗначениеЗаполнено(ПроцентСЗаказаУправлениюПроизводство) Тогда
			СуммаУправлениюОтПроизводства = Выборка.ВнутренняяСтоимостьСпецификации * 0.01 * ПроцентСЗаказаУправлениюПроизводство;
		КонецЕсли;
		
	КонецЕсли;
	
	Если СуммаУправлениюОтПроизводства <> 0 Тогда
		ЛексСервер.НачислитьСуммуУправлению(Выборка.Производство, Движения, СуммаУправлениюОтПроизводства, Дата);
	КонецЕсли;
...
Функция СформироватьПроводкиПрибыльПроизводства(МассивСпецификаций, Движения, Дата, ВызовИзАкта = Ложь) Экспорт
	...
	|	ЕСТЬNULL(докДоговор.СуммаДокументаБезСкидки - СпецификацияСписокНоменклатуры.Ссылка.СуммаДокумента, 0) КАК Отклонение,
	...
	|	СпецификацияСписокНоменклатуры.Ссылка.Подразделение,
	|	докДоговор.Ссылка,
	|	ЕСТЬNULL(докДоговор.СуммаДокументаБезСкидки - СпецификацияСписокНоменклатуры.Ссылка.СуммаДокумента, 0),
	...
	Пока ВыборкаПоСпецификациям.Следующий() Цикл
		...
		Если ОдноПодразделение Тогда
			...
			Если ВыборкаПоСпецификациям.Отклонение > 0 Тогда
				...
				Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.ДоходыОтРозницы;//(???)
			...
			ИначеЕсли ВыборкаПоСпецификациям.Отклонение < 0 Тогда
				...
				Проводка.СчетДт = ЛексСервер.ПолучитьЗатратныйСчетПодразделения(ВыборкаПоСпецификациям.Подразделение);
				Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.ПрочиеХозяйственныеРасходы;//(???)
		...
		Пока Выборка.Следующий() Цикл
			...
			Проводка = СоздатьПроводку(Движения, Дата, Выборка.Производство, Выборка.Сумма);		
			
			Если ОдноПодразделение Тогда
				
				Проводка.СчетДт = ПланыСчетов.Управленческий.ВзаиморасчетыСПокупателями;
				Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.Контрагенты] = Выборка.КонтрагентСпецификация;
...
Функция ЗакрытьДопСоглашенияДоговора(МассивСпецификаций, Движения, ДатаПроведения, ВызовИзАкта = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВызовИзАкта Тогда
			СуммаДоговораСДопСолгашениями = ОбщаяСумма + СуммаДоговора;
			Управление = Константы.Управление.Получить();
			ПроцентСЗаказаУправлениюРозница = ЛексСервер.ПолучитьНастройкуПодразделения(ПодразделениеРозницы, Перечисления.ВидыНастроекПодразделений.ПроцентСЗаказаУправлению, ДатаПроведения);
			Если ЗначениеЗаполнено(Управление) И ЗначениеЗаполнено(ПроцентСЗаказаУправлениюРозница) Тогда
				СуммаУправлениюОтРозница = СуммаДоговораСДопСолгашениями * 0.01 * ПроцентСЗаказаУправлениюРозница;
				ЛексСервер.НачислитьСуммуУправлению(ПодразделениеРозницы, Движения, СуммаУправлениюОтРозница, ДатаПроведения);
			КонецЕсли;
		КонецЕсли;
...
Функция РеализацияИзделийРозничныеПроводки(МассивСпецификаций, Движения, Дата, ВызовИзАкта = Ложь) Экспорт
	...
	|	докСпецификация.Подразделение.СвоиМонтажи КАК СвоиМонтажи,
	|	ЕСТЬNULL(докДоговор.СуммаДокументаБезСкидки - докСпецификация.СуммаДокумента, 0) КАК Отклонение,
...
Функция СформироватьПоказателиМонтажникаИлиВодителя(МассивСпецификаций, Движения, Дата) Экспорт 
	//littox Литус Антон - 26.08.2014
	//Понижающий коэффициент зарплаты монтажников
	//Добавил параметр с коэффициентом и условие в запрос,
	//если оценка неудовлетворительно, то умножаем на коэффициент
	//26.08.2014 - littox Литус Антон 
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивСпецификаций", МассивСпецификаций);
	Запрос.УстановитьПараметр("ПонижающийКоэффициент", 0.8);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СпецификацияСписокНоменклатуры.Ссылка,
	|	СпецификацияСписокНоменклатуры.Ссылка.Производство,
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ВыездМастера) И АктВыполненияДоговора.ОценкаСупервайзера = ЗНАЧЕНИЕ(Перечисление.ОценкаСупервайзера.Неудовлетворительно)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество * &ПонижающийКоэффициент
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ВыездМастераНаСборкуКухни) И АктВыполненияДоговора.ОценкаСупервайзера = ЗНАЧЕНИЕ(Перечисление.ОценкаСупервайзера.Неудовлетворительно)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество * &ПонижающийКоэффициент
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПроездМонтажникаЗаГородом) И АктВыполненияДоговора.ОценкаСупервайзера = ЗНАЧЕНИЕ(Перечисление.ОценкаСупервайзера.Неудовлетворительно)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество * &ПонижающийКоэффициент
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.СборкаИзделия) И АктВыполненияДоговора.ОценкаСупервайзера = ЗНАЧЕНИЕ(Перечисление.ОценкаСупервайзера.Неудовлетворительно)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество * &ПонижающийКоэффициент
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ДоставкаЗаГородом) И АктВыполненияДоговора.ОценкаСупервайзера = ЗНАЧЕНИЕ(Перечисление.ОценкаСупервайзера.Неудовлетворительно)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество * &ПонижающийКоэффициент
	...
	Производство = Выборка.Производство;
	...
	Если ЗначениеЗаполнено(Монтажник) И (Выборка.ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж) Тогда
		...
		Если Выборка.ВыездМастера <> 0 Тогда
			ДобавитьПоказательСотрудника(Движения, Дата, Монтажник, Перечисления.ВидыПоказателейСотрудников.КоличествоУстановленныхИзделий, Выборка.ВыездМастера, Производство)
		КонецЕсли;
		
		Если Выборка.ВыездМастераНаСборкуКухни <> 0 Тогда
			ДобавитьПоказательСотрудника(Движения, Дата, Монтажник, Перечисления.ВидыПоказателейСотрудников.КоличествоУстановленныхКухонь, Выборка.ВыездМастераНаСборкуКухни, Производство)
		КонецЕсли;
		
		Если Выборка.СборкаИзделия <> 0 Тогда
			ДобавитьПоказательСотрудника(Движения, Дата, Монтажник, Перечисления.ВидыПоказателейСотрудников.КоличествоМетровУстановленныхИзделий, Выборка.СборкаИзделия, Производство)
		КонецЕсли;
		
		Если Выборка.ПроездМонтажникаЗаГородом <> 0 Тогда
			ДобавитьПоказательСотрудника(Движения, Дата, Монтажник, Перечисления.ВидыПоказателейСотрудников.КилометровУдаленныхМонтажей, Выборка.ПроездМонтажникаЗаГородом, Производство)
		КонецЕсли;	
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Экспедитор) И Выборка.ПакетУслуг <> Перечисления.ПакетыУслуг.СамовывозОтПроизводителя Тогда
		
		ДобавитьПоказательСотрудника(Движения, Дата, Экспедитор, Перечисления.ВидыПоказателейСотрудников.КоличествоДоставленныхИзделий, 1, Производство);
		
		Если Выборка.ДоставкаЗаГородом <> 0 Тогда
			ДобавитьПоказательСотрудника(Движения, Дата, Экспедитор, Перечисления.ВидыПоказателейСотрудников.КилометровУдаленныхДоставок, Выборка.ДоставкаЗаГородом, Производство)
		КонецЕсли;
...
Функция ДобавитьПоказательСотрудника(фДвижения, фДата, Физлицо, Показатель, Значение, Производство)
