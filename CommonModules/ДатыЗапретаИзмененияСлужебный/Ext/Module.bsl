...
Процедура ПересчитатьТекущиеЗначенияОтносительныхДатЗапрета(ЗаписатьОписаниеРезультатаВЖурналРегистрации = Истина, ОписаниеРезультата = "") Экспорт
	...
	Попытка
		...
		Если День(ТекущаяДатаНаСервере) > 10 Тогда
			РассчитаннаяДатаЗапрета = НачалоМесяца(ТекущаяДатаНаСервере)-60*60*24;
		Иначе
			РассчитаннаяДатаЗапрета = ДобавитьМесяц(НачалоМесяца(ТекущаяДатаНаСервере),-1)-60*60*24;
		КонецЕсли;
...
Функция РассчитатьДатуЗапретаПоОписанию(ТекущаяДатаНаСервере, ОписаниеДатыЗапрета)
	...
	ИначеЕсли ВариантДатыЗапрета = "ТриДняДоТекущей" Тогда
		ТекущаяДатаЗапрета    = НачалоДня(ТекущаяДатаНаСервере) - 3 * Сутки;
		ПредыдущаяДатаЗапрета = НачалоДня(ТекущаяДатаЗапрета) - 3 * Сутки;
