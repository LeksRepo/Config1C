...
Процедура УстановкаПериодаРассчитанныхИтогов() Экспорт
	...
	Для Каждого РегистрМетаданные Из Метаданные.РегистрыНакопления Цикл
		Если РегистрМетаданные.ВидРегистра = ВидОстатки Тогда
			...
			Если РегистрНакопленияМенеджер.ПолучитьПериодРассчитанныхИтогов() < РегистрНакопленияПериод Тогда
				РегистрНакопленияМенеджер.УстановитьПериодРассчитанныхИтогов(РегистрНакопленияПериод);
	...
	Для Каждого РегистрМетаданные Из Метаданные.РегистрыБухгалтерии Цикл
		...
		Если РегистрБухгалтерииМенеджер.ПолучитьПериодРассчитанныхИтогов() < РегистрБухгалтерииПериод Тогда
			РегистрБухгалтерииМенеджер.УстановитьПериодРассчитанныхИтогов(РегистрБухгалтерииПериод);
...
Функция СформироватьПараметрыИтоговИАгрегатов()
	...
	Для Каждого РегистрМетаданные Из Метаданные.РегистрыНакопления Цикл
		Если РегистрМетаданные.ВидРегистра = ВидОстатки Тогда
			...
			Параметры.ДатаРасчетаИтогов  = Мин(Параметры.ДатаРасчетаИтогов, РегистрыНакопления[РегистрМетаданные.Имя].ПолучитьПериодРассчитанныхИтогов());
		КонецЕсли;
	КонецЦикла;
