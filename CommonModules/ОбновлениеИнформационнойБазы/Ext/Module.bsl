...
Функция НеобходимоОбновлениеИнформационнойБазы() Экспорт
	...
	Если НЕ ОбщегоНазначенияПовтИсп.РазделениеВключено()
	   И Константы.ТребуетсяОбновлениеИнформационнойБазы.Получить()
	   И НЕ ТребуетсяОбновлениеКонфигурацииУзлаРИБ() Тогда
...
//
Функция ВыполнитьОбновлениеИнформационнойБазы(Знач Принудительно = Ложь, Знач ПриЗапускеКлиентскогоПриложения = Ложь) Экспорт
	
	Если НЕ ПриЗапускеКлиентскогоПриложения Тогда
		СтандартныеПодсистемыСервер.ЗагрузитьИлиОбновитьПараметрыРаботыПрограммы(Принудительно);
	КонецЕсли;
...
Функция ЕстьПраваНаОбновлениеИнформационнойБазы(УчитыватьПривилегированныйРежим = Истина, РазделенныеДанные = Неопределено)
