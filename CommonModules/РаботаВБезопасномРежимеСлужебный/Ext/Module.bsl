...
Функция ЗапросыНаОбновлениеРазрешенийКонфигурации(Знач ВключаяЗапросСозданияПрофиляИБ = Истина) Экспорт
	...
	Попытка
		...
		Если ВключаяЗапросСозданияПрофиляИБ Тогда
			Результат.Добавить(ЗапросНаСозданиеНабораРазрешений(РаботаВБезопасномРежимеСлужебныйПовтИсп.СлужебныйИОМ()));
		КонецЕсли;
...
Процедура ЗаполнитьВнешнийМодульДляПредоставленныхРазрешений() Экспорт
	...
	Обработки.НастройкаРазрешенийНаИспользованиеВнешнихРесурсов.ЗаполнитьВнешнийМодульДляПредоставленныхРазрешений();
