...
Функция  ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура)
	...
	Пока ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ТаблицаВсехДеталей[0].ВысотаЛиста Цикл
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
			...
			Если НЕ ПакетСобран Тогда
				...
				//ТаблицаОстатков = СписокДеталейНоменклатуры.СкопироватьКолонки();
				...
				Если Элемент.ВысотаДетали <= Элемент.ВысотаЛиста Тогда
					
					НоваяСтрока = ПоследнееРешение.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Элемент);
					ТекущаяВысота = Элемент.ВысотаДетали;
					
				КонецЕсли;
				
				Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
					
					Если ТекущаяВысота + Деталь.ВысотаДетали + ТолщинаПила <= Элемент.ВысотаЛиста Тогда
						
						НоваяСтрока = ПоследнееРешение.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Деталь);
						
						ТекущаяВысота = ТекущаяВысота + Деталь.ВысотаДетали + ТолщинаПила;
						
					Иначе
						
						ПлощадьПолосы = ШиринаПакета * Элемент.ВысотаЛиста;
						ПлощадьОстаткаСверху = ШиринаПакета * (Элемент.ВысотаЛиста - ТекущаяВысота);
			...
			Иначе
				...
				Для каждого Строка Из ЛучшееРешение Цикл
					...
					Строка = СписокДеталейНоменклатуры.Найти(Строка.ИД);
		...
		//Элемент.ДетальВПакете = Истина;
			//Элемент.КоординатаУ = КоординатаУ; 
					...
					//Элемент.НомерПакета = НомерПакета;
		...
		//ДеталиНаЛисте = ТаблицаВсехДеталей.НайтиСтроки(Новый Структура("НомерПакета", НомерПакета));
		//
		//НоваяСтрока = СписокДеталейНоменклатуры.Добавить();
		//
		//ЗаполнитьЗначенияСвойств(НоваяСтрока, ПоследняяДетальПакета);
		//
		//НоваяСтрока.ВысотаДетали = КоординатаУ - ТолщинаПила;
		//НоваяСтрока.ШиринаДетали = ПоследняяДетальПакета.ШиринаДетали;
		//НоваяСтрока.ВысотаЛиста = ВысотаЛиста;
		//НоваяСтрока.ШиринаЛиста = ШиринаЛиста;
		//НоваяСтрока.Количество = 1;
		//НоваяСтрока.КоординатаУ = 0;
		...
		//НоваяСтрока.Порядок = 100;
		//НоваяСтрока.СоблюдениеТекстуры = Номенклатура.НаличиеТекстуры;
		//НоваяСтрока.ОсобыйПорядок = "ПакетДеталей";
		//НоваяСтрока.НомерЛиста = 0;
		//НоваяСтрока.Остаток = Ложь;
		//НоваяСтрока.ДетальВПакете = Ложь;
		//
	...
	Для каждого Пакет Из ТаблицаПакетов Цикл
		...
		Для Каждого Элемент Из Пакет.Пакет Цикл
			...
			Если КоординатаХ + Пакет.ШиринаПакета + ТолщинаПила <= Элемент.ШиринаЛиста Тогда
			
				КоординатаХ = КоординатаХ + Пакет.ШиринаПакета + ТолщинаПила;
		...
		ДеталиНаЛисте = Пакет.Пакет.НайтиСтроки(Новый Структура("НомерПакета", Пакет.НомерПакета));
		...
		НоваяСтрока.ВысотаДетали = КоординатаУ - ТолщинаПила;
		НоваяСтрока.ШиринаДетали = Пакет.ШиринаПакета;
		НоваяСтрока.ВысотаЛиста = ВысотаЛиста;
		НоваяСтрока.ШиринаЛиста = ШиринаЛиста;
		НоваяСтрока.Количество = 1;
		НоваяСтрока.КоординатаУ = 0;
	...
	Если ЭтоСпецификация Тогда
		...
		Для Каждого Элемент Из СписокЯщиков Цикл
			...
			Если НовыеЯщики Тогда
				...
				Схема = Элемент.СхемаЯщика;
				
				Для Каждого Деталь Из Схема.Детали Цикл
