...
Функция ПолучитьСтрокуРаскроя(Спецификация, СписокДеталей = Неопределено) Экспорт
	...
	НомерСпецификации = Спецификация.Номер;
	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		//Проверка если есть деталь, одна из сторон которой превышает размер листа и она безтекстурная, тогда разварачиваем ее и текстурируем(т.е. повернуть ее нельзя)
		//Если повернулась и попала в размеры листа - Хорошо, иначе возврат всей процедуры раскроя(по идее не должно быть)
		Если НЕ Элемент.СоблюдениеТекстуры И (Элемент.ВысотаДетали > Элемент.Номенклатура.ДлинаДетали ИЛИ Элемент.ШиринаДетали > Элемент.Номенклатура.ШиринаДетали) Тогда
			
			Если Элемент.ВысотаДетали <= Элемент.Номенклатура.ШиринаДетали ИЛИ Элемент.ШиринаДетали <= Элемент.Номенклатура.ДлинаДетали Тогда
				
				ВысотаДеталиБезТек = Элемент.ВысотаДетали;
				Элемент.ВысотаДетали = Элемент.ШиринаДетали;
				Элемент.ШиринаДетали = ВысотаДеталиБезТек;
				Элемент.СоблюдениеТекстуры = 1;
				Элемент.ПоворотДетали = 1;
				
			Иначе
				
				Возврат "Ошибка раскроя № 1";
		...
		Если НЕ СоблюдатьТекстуруНоменклатуры И НЕ Элемент.ПоворотДетали Тогда
		...
		Если НЕ Перераскрой И ЗначениеЗаполнено(Элемент.СтрокаДляФлэш) Тогда
			СтрокаКривогоПила = ФормированиеСтрокиКривогоПила(Элемент, СтрокаКривогоПила, НомерСпецификации);
		КонецЕсли;
				...
				Для Каждого Элемент Из СписокДеталейНоменклатуры Цикл
					...
					Если НЕ Элемент.Остаток Тогда
						...
						Для Каждого ЭлементМассива Из МассивОстатков Цикл
							...
							ИначеЕсли ((МассивОстатков.Найти(ЭлементМассива) + 1) = МассивОстатков.Количество())
								И (Элемент.ВысотаЛиста >= Элемент.ВысотаДетали И Элемент.ШиринаЛиста >= Элемент.ШиринаДетали) Тогда
...
Функция ФормированиеСтрокиКривогоПила(Элемент, СтрокаКривогоПила, НомерСпецификации)
	...
	+ "Спецификация " + НомерСпецификации + "☺"
...
Функция ФормированиеСтрокиРаскроя(СтрокаРаскроя, ТаблицаДеталей, Спецификация)
	...
	Запрос.УстановитьПараметр("Подразделение", Спецификация.Производство);
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Спецификация.Номер);
	
	Пока Выборка.Следующий() Цикл
		
		НоменклатураПодЗаказ = ?(Выборка.ПодЗаказ, " (Вернуть остатки) ","");
		...
		+ "Спец. № " + НомерНаПечать + НоменклатураПодЗаказ + "☺"
		+ "<b></b>" + "☻";
...
Функция ПолучитьСтрокуРаскрояЗеркал(СписокЗеркал) Экспорт
