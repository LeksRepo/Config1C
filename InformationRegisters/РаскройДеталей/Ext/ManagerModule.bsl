...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено, ПоказатьВесьСписокДеталей = Ложь, Наряды = Ложь) Экспорт
	...
	Если НарядЗадание Тогда
		...
		ПараметрыФормирования.Вставить("Наряд", ДокОбъект);
		ПараметрыФормирования.Вставить("СписокДеталей", СписокДеталей);
		ПараметрыФормирования.Вставить("Подразделение", МассивСпецификаций[0].Подразделение);
		
		СтрокаPTX = ФормированиеHolzma.Сформировать(ПараметрыФормирования);
	...
	Структура.Вставить("СтрокаPTX", СтрокаPTX);
...
Функция СформироватьНаименованиеДетали(Деталь, Буква = "") Экспорт
	...
	СтруктураНадпись.ЯщикДверьИдентификатор = ?(Деталь.ЭтоЯщик, " ящ" + ?(Деталь.Комментарий <> "",Символы.ПС + Деталь.Комментарий,""), ?(Деталь.ЭтоДверь, " двр", ""));
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Дилерский = Ложь, Буква = "", НарядЗадание = Ложь)
	...
	Спец = фнДокОбъект.Ссылка;
	СборкаВЦеху = Спец.Изделие.СборкаВЦеху;
	
	Для Каждого Деталь ИЗ тзДетали Цикл
		
		Деталь.Спецификация = Спец;
		
		Если Деталь.НомерИзделия > 0 И тзКороба.Количество() > (Деталь.НомерИзделия-1) Тогда
			Деталь.Сборка = тзКороба[Деталь.НомерИзделия-1].Сборка;
		КонецЕсли;
		
		Если НЕ Деталь.Сборка Тогда
			Деталь.Сборка = СборкаВЦеху;	
		КонецЕсли;
	...
	Если фнДокОбъект.СписокЯщики.Количество() > 0 Тогда 
		...
		Для Каждого ДетальЯщика Из ТаблицаДеталиЯщиков Цикл
			...
			НоваяДеталь.Комментарий = ДетальЯщика.Наименование;
		...
		Если СборныйРамочныйФасад и НЕ Деталь.СборныйРамочныйФасад Тогда
			...
			Для ы = 1 По Деталь.Количество Цикл
				...
				Для з=1 По 2 Цикл
					...
					НоваяСтрока.СоблюдениеТекстуры = Истина;
					НоваяСтрока.МожноПовернуть = Ложь;
					НоваяСтрока.ПоворотДетали = Ложь;
		...
		Пока (ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ВысотаЛиста) ИЛИ ВстретилсяПостформинг = Истина Цикл
			...
			Если ТаблицаПервыхДеталей.Количество() > 0 И ?(МожетВстретитьсяПостформинг И ТаблицаПервыхДеталей.Получить(0).Постформинг, НЕ ЭтоНеПолныйЛист, Истина) Тогда
				...
				Если ПерваяДеталь.Остаток
					...
					ИЛИ ПерваяДеталь.ВысотаДетали > ВысотаЛиста 
					Тогда
	...
	Для каждого Элемент Из СписокДеталей Цикл
		...
		Если ЗначениеЗаполнено(Элемент.СтрокаДляФлэш) Тогда
			...
			Для Каждого Эл Из МассивСтрок Цикл
				...
				Если Эл.Строка = Элемент.СтрокаДляФлэш И Эл.НомерСтроки = Элемент.НомерСтроки Тогда
					УжеБыла = Истина;
					Прервать;
				КонецЕсли;
			...
			Если НЕ УжеБыла Тогда
				
				НСтр = Новый Структура();
				НСтр.Вставить("Строка", Элемент.СтрокаДляФлэш);
				НСтр.Вставить("НомерСтроки", Элемент.НомерСтроки);
				
				МассивСтрок.Добавить(НСтр);
