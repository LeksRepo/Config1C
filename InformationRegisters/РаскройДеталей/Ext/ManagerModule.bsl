...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, Знач МассивКусков)
	...
	Если ПоперечныйПил Тогда
		...
		ТаблицаПакетов = АлгоритмОРанце(СписокДеталейДляПоперечногоПила, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, МассивКусков);
		
	Иначе
		
		ТаблицаПакетов = АлгоритмОРанце(СписокДеталейНоменклатуры, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, МассивКусков);
	...
	Если ПоперечныйПил Тогда
		...
		ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, 0, 0, МассивКусков, , , , Истина);
		
	Иначе
		
		ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, 0, 0, МассивКусков, , , , Истина);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей)
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			...
			|	ВЫБОР
			|		КОГДА ВсеКромкиВОдномМесте.Кромка В
			|				(ВЫБРАТЬ
			...
			|		ПО ВсеКромкиВОдномМесте.Кромка = ВТ_МатериалЗаказчика.МатериалЗаказчика
			|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_СписокМатериаловПодЗаказ КАК ВТ_СписокМатериаловПодЗаказ
			|		ПО ВсеКромкиВОдномМесте.Кромка = ВТ_СписокМатериаловПодЗаказ.МатериалПодЗаказ";
	...
	ЗаменитьКромкиЗаказчика(СтрокаРаскроя,МассивВсехКромок,СписокМатериаловЗаказчика);
