...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено) Экспорт
	...
	НарядЗадание = Ложь;
	Если ТипЗнч(ДокОбъект) = Тип("ДокументСсылка.НарядЗадание") Тогда
		НарядЗадание = Истина;
		ПервыйОбход = Истина;
		...
		Если ДокОбъект.СписокСпецификаций.Количество() > 0 Тогда
			...
			Для каждого Строка Из ДокОбъект.СписокСпецификаций Цикл
				...
				ТаблицаДеталей =  ФормированиеСпискаДеталей(Строка.Спецификация, Буква);
		...
		КонецЕсли;
	КонецЕсли;
		//Структура = РасположитьДеталиПростымАлгоритмом(ДокОбъект, СписокДеталей);
	...
	//Иначе
		...
		СтрокаРаскрой = ПолучитьСтрокуДляФлэшИзДерева(Структура.Дерево, ДокОбъект, НарядЗадание, МассивБуквеннойНумерации);
...
Функция СформироватьНаименованиеДетали(Деталь, Буква = "") Экспорт
	...
	НаименованиеДетали = ?(Буква <> "", НаименованиеДетали + " " + Буква + " ", НаименованиеДетали);
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Буква = "")
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание)
	
	СтруктураДляСтрокиРаскроя = Новый Структура;
	МассивДеталейДляПоворота = СписокДеталейНоменклатуры.НайтиСтроки(Новый Структура("МожноПовернуть", Истина));
				...
				Если ПробуемРаспихатьДеталиВОстатки Тогда
					...
					Для Каждого Ы ИЗ ПоследнийЛист Цикл
						...
						Для Каждого Строка Из Пакет Цикл
							...
							ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
							Если НЕ Строка.МожноПовернуть Тогда
								НоваяСтрока.ВысотаДетали = Строка.ШиринаДетали;
								НоваяСтрока.ШиринаДетали = Строка.ВысотаДетали;
							КонецЕсли;
					...
					Для Каждого Деталь Из ТаблицаРазвернутыхДеталей Цикл
						...
						Для каждого ЭлементМассива Из МассивОстатков Цикл
							...
							Если ПоперечныйПил Тогда
								...
								Иначе
									ОстатокПоВысоте = ЭлементМассива.ШиринаДетали - Деталь.ШиринаДетали;
									ОстатокПоШирине = ЭлементМассива.ВысотаДетали - Деталь.ВысотаДетали;
								КонецЕсли;
							Иначе
							...
							ИначеЕсли ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
								...
								Если ПоперечныйПил Тогда
									//Если НЕ РазмещаемаяДеталь.МожноПовернуть Тогда
									//	РазмещаемаяДеталь.ВысотаДетали = Деталь.ВысотаДетали;
									//	РазмещаемаяДеталь.ШиринаДетали = Деталь.ШиринаДетали;
									//КонецЕсли;
							...
							ИначеЕсли МассивОстатков.Найти(ЭлементМассива) + 1 = МассивОстатков.Количество() Тогда
								...
								Если (ПоперечныйПил и (Деталь.ВысотаЛиста >= Деталь.ШиринаДетали И Деталь.ШиринаЛиста >= Деталь.ВысотаДетали)) или 
									(НЕ ПоперечныйПил и (Деталь.ВысотаЛиста >= Деталь.ВысотаДетали И Деталь.ШиринаЛиста >= Деталь.ШиринаДетали)) Или
									(ПоперечныйПил и НЕ Деталь.МожноПовернуть и (Деталь.ВысотаЛиста >= Деталь.ВысотаДетали И Деталь.ШиринаЛиста >= Деталь.ШиринаДетали)) Тогда
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= МаксимальныйПроверяемыйПроцент Цикл
				...
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ,ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание);
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание);
...
//Функция РасположитьДеталиПростымАлгоритмом(ДокОбъект, СписокДеталей)
//	
//	МассивИтоговогоРасположенияДеталей = Новый Массив;
//	АлгоритмРаскроя = "";
//	Подразделение = СсылкаДокумента.Подразделение;
...
Функция ПолучитьСтрокуДляФлэшИзДерева(Дерево, СсылкаДокумента, НарядЗадание, МассивБуквеннойНумерации)
	...
	Если Дерево.Строки.Количество() > 0 Тогда
		...
		ДанныеДляРаскроя = ФормированиеСтрокиРаскрояПоРанцу(ДанныеДляРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации)
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			Если НарядЗадание Тогда
				ТаблицаДеталей = СсылкаДокумента.СписокСпецификаций[0].Спецификация.СписокМатериалы.ВыгрузитьКолонки("НомерСтроки, Номенклатура, ВысотаДетали, ШиринаДетали, Количество,
				...
				|Обтачивать, Постформинг, РадиусФасада");
			Иначе
				ТаблицаДеталей = СсылкаДокумента.СписокМатериалы.ВыгрузитьКолонки("НомерСтроки, Номенклатура, ВысотаДетали, ШиринаДетали, Количество,
				|Идентификатор, НоменклатураДляСклеивания, НомерИзделия, НеТорцевать, СтруктураОтверстий, Комментарий, РасположениеПазов,
				|РадиусЛевоВерх, РадиусЛевоНиз, РадиусПравоВерх, РадиусПравоНиз, КривойПилСверху, КривойПилСлева, КривойПилСнизу, КривойПилСправа, 
				|ВыборМебельнойКромкиСверху, ВыборМебельнойКромкиСлева, ВыборМебельнойКромкиСнизу, ВыборМебельнойКромкиСправа, Срез, КоличествоПетель,
				|ПереключательКромкаСверху, ПереключательКромкаСлева, ПереключательКромкаСнизу, ПереключательКромкаСправа, РасположениеПазовИРучкиНаФасадах, 
			...
			КомментарийДокумента = "Полный пиздец";//СсылкаДокумента.Комментарий;
			ВидДокумента = "Спец.";
			...
			Запрос.УстановитьПараметр("Подразделение", Подразделение);
			Если НарядЗадание Тогда
				СписокМатериаловЗаказчика = СсылкаДокумента.СписокСпецификаций[0].Спецификация.СписокМатериалы.ВыгрузитьКолонки();
			...
			Если НарядЗадание И МассивБуквеннойНумерации.Количество() > 0 Тогда
				НомерНаПечать  = "";
