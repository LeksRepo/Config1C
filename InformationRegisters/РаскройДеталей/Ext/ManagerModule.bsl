...
Функция СформироватьНаименованиеДетали(Деталь) Экспорт
	...
	СтруктураНадпись.Постформинг = ?(Деталь.Обтачивать И ДанныеДетали.ЭтоСтолешница, "Пформ", "");
...
Функция ФормированиеСпискаДеталей(фнДокОбъект)
	...
	Для каждого Деталь Из тзДетали Цикл
		...
		Если Деталь.Количество > 1 Тогда
			...
			Деталь.КоличествоКривыхДеталей = ?(ЗначениеЗаполнено(Деталь.СтрокаДляФлэш), Деталь.Количество, 0);
			Деталь.Количество = 1;
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПроверкаОстатков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
	...
	ТаблицаВсехДеталейДляТестов = СписокДеталейНоменклатуры.Скопировать(МассивВсехДеталей);
	ТаблицаВсехДеталейОкончательная = СписокДеталейНоменклатуры.Скопировать(МассивВсехДеталей);
	...
	Если ШиринаСамойУзкойДетали <= ШиринаЛиста и ШиринаСамойУзкойДетали > 0 Тогда
		...
		Пока ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ВысотаЛиста Цикл
				...
				Если НЕ ПакетСобран Тогда
					...
					Если СтруктураДляПроверкиЗаполненности <> Неопределено Тогда
						...
						Иначе
							...
							Для каждого Строка Из ТаблицаВсехДеталейОкончательная Цикл
							
								ИсправляемаяСтрока = ТаблицаВсехДеталейДляТестов.Найти(Строка.GUID, "GUID");
								ИсправляемаяСтрока.ДетальВПакете = Строка.ДетальВПакете;
								ИсправляемаяСтрока.ИспользуетсяВОстаткахПакета = Строка.ИспользуетсяВОстаткахПакета;
								Строка.КоординатаХ = ИсправляемаяСтрока.КоординатаХ;
								Строка.КоординатаУ = ИсправляемаяСтрока.КоординатаУ;
			...
			МассивВсехДеталей = ТаблицаВсехДеталейДляТестов.НайтиСтроки(Отбор);
			ТаблицаВсехДеталей = ТаблицаВсехДеталейДляТестов.Скопировать(МассивВсехДеталей);
			ИтогВысотыПоДеталям = ТаблицаВсехДеталей.Итог("ВысотаДетали") + ТолщинаПила * (ТаблицаВсехДеталей.Количество() - 1);
			ТаблицаВсехДеталей.Сортировать("ШиринаДетали Убыв, ВысотаДетали Убыв");
			ТаблицаОставшихсяДеталей = ТаблицаВсехДеталей.Скопировать();
			ТаблицаОставшихсяДеталей.Сортировать("ШиринаДетали Убыв, ВысотаДетали Убыв");
