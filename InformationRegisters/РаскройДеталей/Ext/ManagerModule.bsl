...
Функция ФормированиеСпискаДеталей(фнДокОбъект)
	...
	// 1. Идентификатор
	// 2. КоличествоПетель
	// 3.переключатели кромки
	// 4. РасположениеПазовИРучкиНаФасадах
	// 5. СоблюдениеТекстуры
	// 6 ОсобыйПорядок
	// 7. ЭтоЯщик
	// 8. ЭтоДверь
	// 9. ДетальДляСклеивания
	// 10. ИД ИЛИ GUID
	// 11. Порядок или ПорядокПечати
	// } Васильев Александр Леонидович [09.08.2015]
	
	тзДетали = фнДокОбъект.СписокМатериалы.Выгрузить(,"НомерСтроки, Материал, Номенклатура, ВысотаДетали, ШиринаДетали, Количество,
	|Идентификатор, НоменклатураДляСклеивания, НомерИзделия, НеТорцевать, СтруктураОтверстий, Комментарий, РасположениеПазов,
	|РадиусЛевоВерх, РадиусЛевоНиз, РадиусПравоВерх, РадиусПравоНиз, КривойПилСверху, КривойПилСлева, КривойПилСнизу, КривойПилСправа, СтрокаДляФлэш,
	|ВыборМебельнойКромкиСверху, ВыборМебельнойКромкиСлева, ВыборМебельнойКромкиСнизу, ВыборМебельнойКромкиСправа, Срез, КоличествоПетель,
	|ПереключательКромкаСверху, ПереключательКромкаСлева, ПереключательКромкаСнизу, ПереключательКромкаСправа, РасположениеПазовИРучкиНаФасадах, 
	...
	Для каждого Деталь Из тзДетали Цикл
		...
		ПорядокПечати = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(НоменклатурнаяГруппа, "ПорядокПечати");
		...
		// { Васильев Александр Леонидович [09.08.2015]
		// Сделать.
		// Сформировать наименование детали. (Надпись в центре)
		// } Васильев Александр Леонидович [09.08.2015]
		
		// { Васильев Александр Леонидович [09.08.2015]
		// Сделать.
		// Проверку на размер детали, помещается ли на лист.
		// Оставить её в одном месте, только здесь.
		// } Васильев Александр Леонидович [09.08.2015]
				...
				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					Для Каждого ЭлементМассива Из ТаблицаОстатков Цикл
						...
						Если ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
							...
							РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ТолщинаПила);
							Прервать;
						...
						ИначеЕсли ТаблицаОстатков.Индекс(ЭлементМассива) + 1 = ТаблицаОстатков.Количество() Тогда
							...
							Иначе
								...
								СформироватьОшибкуПревышениеРазмера(СтруктураДляСтрокиРаскроя, Деталь);
		...
		Для Инд = 0 По ОбходовПоТекстуре Цикл
			...
			Если Инд = 1 Тогда //Есть детали без текстуры
				...
				Для Каждого ЭлементБезТекстуры Из СписокДеталейНоменклатуры Цикл
					...
					МожноПовернуть = ЭлементБезТекстуры.ВысотаДетали <= ЭлементБезТекстуры.ШиринаЛиста
					И ЭлементБезТекстуры.ШиринаДетали <= ЭлементБезТекстуры.ВысотаЛиста;
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ТолщинаПила)
