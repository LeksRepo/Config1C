...
Функция ФормированиеПакетаДляБольшогоКоличестваДеталей(СписокДеталейНоменклатуры,
	ВысотаЛиста,
	ШиринаЛиста,
	ТолщинаПила,
	Номенклатура,
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры,
	Знач ВысотаЛиста,
	Знач ШиринаЛиста,
	ТолщинаПила,
	Номенклатура,
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 или МассивКусков.Количество() > 0 Тогда
		...
		Для Каждого Строка Из ТаблицаРазмещенияСтолбцов Цикл
			...
			ПробуемРаспихатьДеталиВОстатки = МожноВращать и ТаблицаРазмещенияСтолбцов.Количество() > 1
			и Строка = ТаблицаРазмещенияСтолбцов[ТаблицаРазмещенияСтолбцов.Количество() - 1] и НЕ ПоперечныйПил;
				...
				Для каждого Столбец Из Строка.Пакет Цикл
					...
					Для Каждого Деталь Из РазмещаемыйПакет Цикл
						...
						Если НЕ Деталь.ДетальРазмещена
							И (НЕ ПроверятьНомерЛиста или
							(ПроверятьНомерЛиста и ПроверочныйНомерЛиста <> Число(СписокНомеров[0].Представление)
							И ПроверочныйНомерЛиста <> Число(СписокНомеров[1].Представление))) Тогда
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, ПолныйСписокДеталей, НарядЗадание, МассивБуквеннойНумерации)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Если КоличествоДеталейНоменклатуры > 999 Тогда
			//Доработать.
			СтруктураДляСтрокиРаскроя = ФормированиеПакетаДляБольшогоКоличестваДеталей(СписокДеталейДляФормированияПакетов, 
			ВысотаЛиста,
			ШиринаЛиста,
			ТолщинаПила,
			ТекущаяНоменклатура,
		...
		Иначе
			...
			СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов,
			ВысотаЛиста,
			ШиринаЛиста,
			ТолщинаПила,
			ТекущаяНоменклатура, ,
			...
			Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
				...
				СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(
				СписокПовернутыхДеталейДляФормированияПакетов,
				ВысотаЛиста,
				ШиринаЛиста,
				ТолщинаПила,
				ТекущаяНоменклатура,
				...
				БезОкругленияДоЛистов,
				НарядЗадание,
				МассивОстатковДляРазвернутыхДеталей,
				МассивБуквеннойНумерации);
		...
		Для каждого СтрокаТаб Из Таб Цикл
			...
			//СтрокаДеталь.Спецификация = // Как-то родить сюда спецуху надо
