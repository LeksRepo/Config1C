...
Функция ПолучитьСтрокуРаскрояПоВсемНоменклатурам(МассивИтоговогоРасположенияДеталей, СсылкаДокумента)
	...
	Для каждого ЭлементМассива Из МассивИтоговогоРасположенияДеталей Цикл
		...
		ДанныеДляРаскроя = ФормированиеСтрокиРаскроя(ДанныеДляРаскроя, ЭлементМассива, СсылкаДокумента, Производство, ЭтоСпецификация, ЭтоНаряд);
...
Процедура ФормированиеПакета(СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура)
	
	ТаблицаОсобых = СписокДеталейНоменклатуры.Скопировать();
	
	ТаблицаОсобых = ТаблицаОсобых.НайтиСтроки(Новый Структура("ДетальВПакете", Ложь));
	ИтогВысотыПоДеталям = ТаблицаОсобых.Итог("ВысотаДетали");
	
	Если ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ТаблицаОсобых[0].ВысотаЛиста Тогда
		
		ТаблицаОсобых.Сортировать("ШиринаДетали Убыв, ВысотаДетали Убыв");
		НомерПакета = 1;
		КоординатаХ = 0;
		КоординатаУ = 0;
		ВысотаПакета = 0;
		ШиринаПакета = 0;
		ПерваяДетальПакета = Истина;
		...
		Для Каждого Элемент Из ТаблицаОсобых Цикл
			...
			//Для Каждого ЭлементМассива Из МассивВысоких Цикл
			
			Если КоординатаУ + Элемент.ВысотаДетали <= Элемент.ВысотаЛиста Тогда
			...
			Иначе
				...
				//ВысотаПакета = Элемент.ВысотаЛиста - КоординатаУ;
