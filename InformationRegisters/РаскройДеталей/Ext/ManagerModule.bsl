...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено) Экспорт
	...
	Если ИспользоватьСтарыйАлгоритм Тогда
		...
		Структура.Вставить("ЛучшийПроцентОтхода", 0);
	...
	ИначеЕсли ИспользоватьДваАлгоритма Тогда
		
		Структура = РасположитьНаЛистахДвумяАлгоритмами(ДокОбъект, СписокДеталей);
...
Функция ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, КонечныйНомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, ЕстьШаг = Ложь)
	...
	Для ТекущийНомерЛиста = КонечныйНомерЛиста + 1 По НомерЛиста Цикл
		...
		Для Каждого ЭлементМассиваОстатков Из МассивОстатков Цикл
			...
			Если НЕ ПолезныйОстаток(ПределыОстатков, ЭлементМассиваОстатков.ВысотаДетали, ЭлементМассиваОстатков.ШиринаДетали) Тогда
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков)
	...
	//Если МожноВращать И ТаблицаРазмещенияСтолбцов.Количество() > 1 Тогда
	//	
	//	ПоследнийЛист = ТаблицаРазмещенияСтолбцов[ТаблицаРазмещенияСтолбцов.Количество() - 1].Пакет;
	//	
	//	Для Каждого Строка ИЗ ПоследнийЛист Цикл
	//		
	//		СтрокаПоиска = ТаблицаПакетов.Найти(Строка.GUID, "НомерПакета");
	//		Пакет = СтрокаПоиска.Пакет;
	//		ПакетДляТеста = Пакет.СкопироватьКолонки();
	//		
	//		Для каждого Строка Из Пакет Цикл
	//			
	//			НоваяСтрока = ПакетДляТеста.Добавить();
	//			ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
	//			НоваяСтрока.ВысотаДетали = Строка.ШиринаДетали;
	//			НоваяСтрока.ШиринаДетали = Строка.ВысотаДетали;
	//			
	//		КонецЦикла;
	//		
	//		Для каждого Решение Из ТаблицаПакетов Цикл
	//			
	//			ПакетДляПроверки = Решение.Пакет;
	//			
	//			Если ПакетДляПроверки <> Пакет Тогда
	//				
	//				Таблица = ПроверитьВозможностьИспользованияОстатков(ПакетДляПроверки, ПакетДляТеста, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина);
	//				
	//			КонецЕсли;
	//			
	//		КонецЦикла;
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 3 и НЕ ПоперечныйПил Тогда
		...
		Для Каждого Строка Из ТаблицаРазмещенияСтолбцов Цикл
			...
			Если ПробуемРаспихатьДеталиВОстатки Тогда
				...
				Для Каждого Ы ИЗ ПоследнийЛист Цикл
					...
					Для Каждого Деталь Из Пакет Цикл
						...
						Для каждого ЭлементМассива Из МассивОстатков Цикл
							...
							//Если Деталь.МожноПовернуть Тогда
								...
								Если ПоперечныйПил Тогда
									ОстатокПоВысотеПоворот = ЭлементМассива.ВысотаДетали - Деталь.ВысотаДетали - ТолщинаПила;
									ОстатокПоШиринеПоворот = ЭлементМассива.ШиринаДетали - Деталь.ШиринаДетали - ТолщинаПила;
									//СтрокаПоиска = СписокДеталейНоменклатуры.Найти(Деталь.GUID, "GUID");
									
									//РазмещаемаяДеталь.КоординатаУ = ЭлементМассива.КоординатаУ;
									//РазмещаемаяДеталь.КоординатаХ = ЭлементМассива.КоординатаХ;
									//РазмещаемаяДеталь.Порядок = 100;
									
								Иначе
									ОстатокПоВысотеПоворот = ЭлементМассива.ВысотаДетали - Деталь.ШиринаДетали - ТолщинаПила;
									ОстатокПоШиринеПоворот = ЭлементМассива.ШиринаДетали - Деталь.ВысотаДетали - ТолщинаПила;
								КонецЕсли;
							...
							//	
							//	Если ПоперечныйПил Тогда
							//		РазместитьДеталь(СписокДеталейНоменклатуры, Деталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ТолщинаПила);
							//	Иначе
							//		РазместитьДеталь(СписокДеталейНоменклатуры, Деталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ТолщинаПила);
							//	КонецЕсли;
							//	Прервать;
							...
							ИначеЕсли МассивОстатков.Найти(ЭлементМассива) + 1 = МассивОстатков.Количество() Тогда
								...
								Если (Деталь.ВысотаЛиста >= Деталь.ВысотаДетали И Деталь.ШиринаЛиста >= Деталь.ШиринаДетали) Тогда
									...
									ИскомыйНомер = ПолучитьНомерЛиста(СчетчикЛистов, МассивНомеровЛистов);
							...
							//	
							//	Если ПоперечныйПил Тогда
							//		ОстатокСверху = ОстатокПоВысотеПоворот;
							//		КоординатаХ = ЭлементМассива.КоординатаХ + Деталь.КоординатаУ;
							//		КоординатаУ = ЭлементМассива.КоординатаУ + Деталь.КоординатаХ;
							//		СтрокаПоиска = СписокДеталейНоменклатуры.Найти(Деталь.GUID, "GUID");
							//		СтрокаПоиска.ВысотаДетали = Деталь.ВысотаДетали;
							//		СтрокаПоиска.ШиринаДетали = Деталь.ШиринаДетали;
							//		СтрокаПоиска.НомерЛиста = ЭлементМассива.НомерЛиста;
							//		Деталь.НомерЛиста = ЭлементМассива.НомерЛиста;
							//		//Деталь.ВысотаДетали = СтрокаПоиска.ВысотаДетали;
							//		//Деталь.ШиринаДетали = СтрокаПоиска.ШиринаДетали;
							//		РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Деталь, ТолщинаПила, КоординатаУ, КоординатаХ, ЭлементМассива.НомерЛиста, ВысотаЛиста, ШиринаЛиста, КоординатаХ = 0, ОстатокСверху, ЭлементМассива.ИспользуетсяВОстаткахПакета, ПоперечныйПил);
							//	Иначе
							//		ОстатокСлева = ОстатокПоШиринеПоворот;
							//		КоординатаХ = ЭлементМассива.КоординатаУ + Деталь.КоординатаХ;
							//		РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Деталь, ТолщинаПила, Деталь.КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, КоординатаУ = 0, ОстатокСлева, ЭлементМассива.ИспользуетсяВОстаткахПакета); //ОстатокПоВысоте, ОстатокПоШирине,
							//	КонецЕсли;
							//	
							//КонецЕсли;
		...
		НомерСтолбца = 0;
			Для каждого Столбец Из Строка.Пакет Цикл
				...
				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					Если НЕ Деталь.ДетальРазмещена И (НЕ ПроверятьНомерЛиста или (ПроверятьНомерЛиста и НомерЛиста <> Число(СписокНомеров[0].Представление) И НомерЛиста <> Число(СписокНомеров[1].Представление))) Тогда
						//Если ПробуемРаспихатьДеталиВОстатки и Не ЛистДобавлен Тогда
						//	ЛистДобавлен = Истина;
						//	НомерЛиста = НомерЛиста + 1;
						//	МассивНомеровЛистов.Добавить(НомерЛиста);
						//КонецЕсли;
		...
		Если СписокНомеров.Количество() > 0 Тогда
			...
			Для Каждого Деталь Из ТаблицаДеталейОтобранныхЛистов Цикл
				...
				Для Каждого ЭлементМассива Из МассивОстатков Цикл
					...
					ОстатокПоВысоте = ЭлементМассива.ВысотаДетали - Деталь.ВысотаДетали - ТолщинаПила;
					ОстатокПоШирине = ЭлементМассива.ШиринаДетали - Деталь.ШиринаДетали - ТолщинаПила;
...
Функция ПолучитьНомерЛиста(СчетчикЛистов, МассивНомеров)
		...
		Для Инд = 0 По ОбходовПоТекстуре Цикл
			...
			Для Каждого ОстатокЛинии Из МассивОстатковЛиний Цикл
				...
				Для ИндВысокие = 1 По 2 Цикл
					...
					ПределыОстатков = НоменклатурнаяГруппа.СеткаОстатков.СеткаОстатков;
...
Функция РасположитьНаЛистахДвумяАлгоритмами(СсылкаДокумента, СписокДеталей)
	...
	МассивОстатковЛиний.Добавить(300);
	МассивОстатковЛиний.Добавить(500);
	СписокДеталейДляСтарогоАлгоритма = СписокДеталей.Скопировать();
	СписокДеталейДляНовогоАлгоритма = СписокДеталей.Скопировать();
	ЛучшиеАлгоритмы = "";
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		//Основные таблицы по умолчанию
		СписокДеталейНоменклатурыДляСтарогоАлгоритма = СписокДеталей.Скопировать(Новый Структура("Номенклатура", ТекущаяНоменклатура));
		
		Если ТекущаяНоменклатура.СеткаОстатков = Справочники.СеткаОстатков.ПустаяСсылка() Тогда
			
			ПределыОстатков = ?(НоменклатурнаяГруппа.СеткаОстатков.СеткаОстатков = Справочники.СеткаОстатков.ПустаяСсылка(), Новый ТаблицаЗначений, НоменклатурнаяГруппа.СеткаОстатков.СеткаОстатков);
			
		Иначе
			
			ПределыОстатков = ТекущаяНоменклатура.СеткаОстатков.СеткаОстатков;
			
		КонецЕсли;
		
		СтруктураСтарогоАлгоритма = СтарыйАлгоритм(ЭлементСписка, ТолщинаПила, ПределыОстатков, МассивОстатковЛиний, СписокДеталейНоменклатурыДляСтарогоАлгоритма);
		////////////////////////////////////////////////////
		ТекущийНомерЛиста = -1;
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= 90 Цикл
				
				СписокДеталейДляФормированияПакетов = СписокДеталейНоменклатуры.Скопировать();
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине,, ПределыОстатков);
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СписокПовернутыхДеталейДляФормированияПакетов = СписокДеталейНоменклатуры.Скопировать();
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПределыОстатков);
		...
		Если СтруктураДляСтрокиРаскроя.КоличествоЛистов < СтруктураСтарогоАлгоритма.КоличествоЛистов Тогда
			
			ЛучшийАлгоритм = "Новый";
			
		ИначеЕсли СтруктураДляСтрокиРаскроя.КоличествоЛистов > СтруктураСтарогоАлгоритма.КоличествоЛистов Тогда
			
			ЛучшийАлгоритм = "Старый";
			ВзятьСНового = Ложь;
			
		Иначе
			
			Если СтруктураДляСтрокиРаскроя.МощностьРаскроя > СтруктураСтарогоАлгоритма.МощностьРаскроя Тогда
				
				ЛучшийАлгоритм = "Новый";
				
			ИначеЕсли СтруктураДляСтрокиРаскроя.МощностьРаскроя < СтруктураСтарогоАлгоритма.МощностьРаскроя Тогда
		...
		Иначе
			...
			Таб = СтруктураСтарогоАлгоритма.ТаблицаДеталейИтог;
			АлгоритмРаскроя = АлгоритмРаскроя + СтруктураСтарогоАлгоритма.АлгоритмРаскроя;
			СтрокаЛистовНоменклатуры.Количество = СтруктураСтарогоАлгоритма.КоличествоЛистов;
		...
		СамыйЛучшийПроцентОтхода = СамыйЛучшийПроцентОтхода + "|" + ЛучшийАлгоритм;
...
Функция СтарыйАлгоритм(ЭлементСписка, ТолщинаПила, ПределыОстатков, МассивОстатковЛиний, СписокДеталейНоменклатуры)
	...
	СтруктураИтогАлгоритма = Новый Структура;
	СтруктураИтогАлгоритма.Вставить("ТаблицаДеталейИтог", ТаблицаДеталейИтог);
	//СтруктураИтогАлгоритма.Вставить("ТаблицаЛистовНоменклатуры", ТаблицаЛистовНоменклатуры);
	СтруктураИтогАлгоритма.Вставить("АлгоритмРаскроя", ОптимальныйАлгоритмРаскроя);
	СтруктураИтогАлгоритма.Вставить("МощностьРаскроя", МощностьРаскроя);
	СтруктураИтогАлгоритма.Вставить("КоличествоЛистов", КоличествоЛистов);
	СтруктураИтогАлгоритма.Вставить("КоличествоОстатков", МассивОстатков.Количество());
	СтруктураИтогАлгоритма.Вставить("СредняяПлощадьОстатков", СредняяПлощадьОстатков);
	
	Возврат СтруктураИтогАлгоритма;
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ТолщинаПила, КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, МаленькийОстатокПоШирине, ИспользуетсяВОстатках, ПоперечныйПил = Ложь)
	...
	Если ДобавитьБольшойОстаток И БольшойОстатокПоЛисту > 0 Тогда
		...
		НоваяСтрока.Количество = 1;
		НоваяСтрока.НомерЛиста = НомерЛиста;
		НоваяСтрока.ВысотаДетали = ?(ПоперечныйПил, БольшойОстатокПоЛисту ,Элемент.ВысотаЛиста - КоординатаУ);
		НоваяСтрока.ШиринаДетали = ?(ПоперечныйПил, Элемент.ШиринаЛиста - КоординатаХ, БольшойОстатокПоЛисту);
		НоваяСтрока.ВысотаЛиста = Элемент.ВысотаЛиста;
		НоваяСтрока.ШиринаЛиста = Элемент.ШиринаЛиста;
	...
	Если МаленькийОстатокПоШирине > 0 И НЕ ИспользуетсяВОстатках Тогда
		...
		НоваяСтрока.Количество = 1;
		НоваяСтрока.НомерЛиста = НомерЛиста;
		НоваяСтрока.ВысотаДетали = ?(ПоперечныйПил, МаленькийОстатокПоШирине, Элемент.ВысотаЛиста - КоординатаУ);
		НоваяСтрока.ШиринаДетали = ?(ПоперечныйПил, Элемент.ШиринаЛиста - КоординатаХ, МаленькийОстатокПоШирине);
		НоваяСтрока.ВысотаЛиста = Элемент.ВысотаЛиста;
		НоваяСтрока.ШиринаЛиста = Элемент.ШиринаЛиста;
		НоваяСтрока.КоординатаУ = ?(ПоперечныйПил, КоординатаУ + Элемент.ВысотаДетали + ТолщинаПила, КоординатаУ);
		НоваяСтрока.КоординатаХ = ?(ПоперечныйПил, КоординатаХ, Элемент.КоординатаХ + Элемент.ШиринаДетали + ТолщинаПила);
	...
	Если ИспользуетсяВОстатках Тогда
		...
		НоваяСтрока.Количество = 1;
		НоваяСтрока.НомерЛиста = НомерЛиста;
		НоваяСтрока.ВысотаДетали = ?(ПоперечныйПил, ЗаменяемыйОстаток.ВысотаДетали - Элемент.ВысотаДетали - ТолщинаПила, Элемент.ВысотаЛиста - КоординатаУ);
		НоваяСтрока.ШиринаДетали = ?(ПоперечныйПил, ЗаменяемыйОстаток.ШиринаДетали, ЗаменяемыйОстаток.ШиринаДетали - Элемент.ШиринаДетали - ТолщинаПила);
		НоваяСтрока.ВысотаЛиста = Элемент.ВысотаЛиста;
		НоваяСтрока.ШиринаЛиста = Элемент.ШиринаЛиста;
		НоваяСтрока.КоординатаУ = ?(ПоперечныйПил, КоординатаУ + Элемент.ВысотаДетали + ТолщинаПила, КоординатаУ);
		НоваяСтрока.КоординатаХ = ?(ПоперечныйПил, КоординатаХ, Элемент.КоординатаХ + Элемент.ШиринаДетали + ТолщинаПила);
