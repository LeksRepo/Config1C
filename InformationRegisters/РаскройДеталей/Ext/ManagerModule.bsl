...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПределыОстатков, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь)
	...
	Если ПоперечныйПил Тогда
		...
		Для каждого Пакет Из ТаблицаПакетов Цикл
			...
			НоваяСтрока.ВысотаДетали = Пакет.ВысотаПакета;
			НоваяСтрока.ШиринаДетали = ШиринаЛиста;
				...
				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					Если НЕ ПроверятьНомерЛиста или (ПроверятьНомерЛиста и НомерЛиста <> Число(СписокНомеров[0].Представление) И НомерЛиста <> Число(СписокНомеров[1].Представление)) Тогда
						...
						Если РазмерПредыдущейДетали = 0 Тогда
							...
							РазмерПредыдущейДетали = ?(ПоперечныйПил, Деталь.ВысотаДетали, Деталь.ШиринаДетали);
						...
						Иначе
							...
							ОстатокСверху = РазмерПредыдущейДетали - Деталь.ВысотаДетали -ТолщинаПила;
							КоординатаХ = ?(Деталь.КоординатаУ > 0, Столбец.КоординатаУ + Деталь.КоординатаУ, Столбец.КоординатаУ);
							РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, Деталь.КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСверху, ИспользуетсяВОстатках, ПоперечныйПил);
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		ПоперечныйПил = НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.МДФ18;
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= МаксимальныйПроверяемыйПроцент Цикл
				...
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ,ПоперечныйПил);
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПоперечныйПил);
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ТолщинаПила, КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, МаленькийОстатокПоШирине, ИспользуетсяВОстатках, ПоперечныйПил = Ложь)
	...
	Если ОстатокПоПолосе > 0 Тогда
		...
		НоваяСтрока.Количество = 1;
		НоваяСтрока.НомерЛиста = НомерЛиста;
		НоваяСтрока.ВысотаДетали = ОстатокПоПолосе;
		НоваяСтрока.ШиринаДетали = Элемент.ШиринаДетали;
		НоваяСтрока.ВысотаЛиста = Элемент.ВысотаЛиста;
		НоваяСтрока.ШиринаЛиста = Элемент.ШиринаЛиста;
