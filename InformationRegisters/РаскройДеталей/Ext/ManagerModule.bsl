...
Функция СформироватьПараметрыЭтикеток(Деталь, Буква = "")
	...
	//Если Деталь.ДетальДляСклеивания Тогда
	//	Данные.Тип = "СКЛЕЙКА";
	//ИначеЕсли  	
	//	
	//КонецЕсли;
	
	Если ЗначениеЗаполнено(Деталь.СтрокаДляФлэш) ИЛИ Деталь.КривойПилСверху ИЛИ Деталь.КривойПилСнизу ИЛИ Деталь.КривойПилСлева ИЛИ Деталь.КривойПилСправа Тогда
		Мас.Добавить("КРИВ");
	КонецЕсли;
	
	Если Деталь.ЭтоЯщик Тогда
		Мас.Добавить("ЯЩ");
	КонецЕсли;
	
	Если Деталь.ЭтоДверь Тогда
		Мас.Добавить("ДВР");
	КонецЕсли;
	
	Данные.Тип = СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(Мас, " / ");
	...
	Для каждого Деталь Из тзДетали Цикл
		...
		Если ДанныеДетали.Клееная Тогда
			...
			Если Деталь.НоменклатураДляСклеивания.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Пластик Тогда
				...
				НоваяСтрока.ПереключательКромкаСверху = "Нет";
				НоваяСтрока.ПереключательКромкаСнизу = "Нет";
				НоваяСтрока.ПереключательКромкаСлева = "Нет";
				НоваяСтрока.ПереключательКромкаСправа = "Нет";
			КонецЕсли;
...
Функция ДобавитьЭлементКСтрокеКривогоПила(Строка, Элемент, Документ, НомерНаряда)
	...
	Если ЗначениеЗаполнено(НомерНаряда) Тогда
		 ...
		 Для каждого Спец Из НомерНаряда.Наряд.СписокСпецификаций Цикл
			 ...
			 Если Спец.Спецификация = Документ Тогда
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей, СписокДеталей)
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			...
			ТаблицаДеталей.Колонки.Добавить("ПараметрыДляЭтикеток", Новый ОписаниеТипов("Строка"));
			ТаблицаДеталей.Колонки.Добавить("КоличествоОдинаковых", Новый ОписаниеТипов("Число"));
