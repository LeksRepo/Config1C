...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПределыОстатков, ПоворотДеталей = Ложь)
	...
	ТаблицаРазмещенияСтолбцов = АлгоритмРанца(ТаблицаСтолбцов, ШиринаЛиста, Номенклатура.ДлинаДетали, ТолщинаПила, Номенклатура, , , , Истина);
...
Функция АлгоритмРанца(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПроверкаОбрезков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
			...
			Иначе
				...
				Если НЕ РазмещениеСтолбцов Тогда
					...
					ЛучшееРешение = ПроверитьВозможностьИспользованияОбрезков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура, ВысотаЛиста);
		...
		Если ТаблицаОставшихсяДеталей.Количество() > 0 И СписокДеталейНоменклатуры.Количество() <> ТаблицаОставшихсяДеталей.Количество() Тогда
			...
			Пока ВысотаПакета > ВысотаЛиста Цикл
				...
				Если ВысотаПакета <> СтараяВысотаПакета Или СтараяВысотаПакета = 0 Тогда
					...
					НоваяТаблицаПакетов = АлгоритмРанца(ТаблицаОставшихсяДеталей, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, Ложь, ВысотаПакета, Истина);
...
Функция ПроверитьВозможностьИспользованияОбрезков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура, ВысотаЛиста)
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей, Перераскрой)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков);
		...
		Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
			...
			СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, Истина);
			
			Если ТипЗнч(СтруктураДляСтрокиРаскроя) = Тип("Структура") Тогда
				
				Если СтруктураДляСтрокиРаскрояПовернутыхДеталей.КоличествоЛистов < СтруктураДляСтрокиРаскроя.КоличествоЛистов ИЛИ 
					СтруктураДляСтрокиРаскрояПовернутыхДеталей.ПлощадьОстатковПоНоменклатуре > СтруктураДляСтрокиРаскроя.ПлощадьОстатковПоНоменклатуре И СтруктураДляСтрокиРаскрояПовернутыхДеталей.КоличествоЛистов = СтруктураДляСтрокиРаскроя.КоличествоЛистов Тогда
