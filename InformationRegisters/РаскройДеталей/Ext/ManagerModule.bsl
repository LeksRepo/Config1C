...
Процедура ДобавитьНовыйЛист(Знач СписокДеталейНоменклатуры, Знач НомерЛиста, Знач Номенклатура, Знач ВысотаЛиста, Знач ШиринаЛиста, Знач ТаблицаОстатков)
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПределыОстатков, ИдеальныйПроцентОтхода, ПроцентОтходаДляСтолбцов, ПоворотДеталей = Ложь)
	...
	ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, ПроцентОтходаДляСтолбцов, , , , Истина);
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
		...
		//Решил попробовать размещать детали в остатках. Не по порядку, а в самый мелкий из подходящих
		ТаблицаОстатков = СписокДеталейНоменклатуры.Скопировать(МассивОстатков);
		ТаблицаОстатков.Сортировать("ШиринаДетали Возр, ВысотаДетали Возр");
		ДобавитьНовыйЛист(СписокДеталейНоменклатуры, НомерЛиста, Номенклатура, ВысотаЛиста, ШиринаЛиста, ТаблицаОстатков);
		...
		Для Каждого Строка Из ТаблицаРазмещенияСтолбцов Цикл
			...
			Для каждого Столбец Из Строка.Пакет Цикл
				...
				КоординатаУ = 0;
				КоординатаХ = 0;
				РазмещаемыйПакет.Сортировать("ИспользуетсяВОстаткахПакета Возр, ШиринаДетали Убыв");
				...
				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					//Для Каждого ЭлементМассива Из МассивОстатков Цикл
						Для Каждого ЭлементМассива Из ТаблицаОстатков Цикл
						...
						Если ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
							...
							РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, Ложь, 0, ТолщинаПила);
							Прервать;
							
						ИначеЕсли ТаблицаОстатков.Индекс(ЭлементМассива) + 1 = ТаблицаОстатков.Количество() Тогда
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, ПроверкаОстатков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
			...
			Иначе // НЕ ПакетСобран
				...
				Если НЕ РазмещениеСтолбцов Тогда
					...
					ЛучшееРешение = ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода);
...
Функция ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода)
	...
	Для каждого Деталь Из ЛучшееРешение Цикл
		...
		Если Деталь.ШиринаДетали < ШиринаПакета и ШиринаПредыдущегоОстатка <> Деталь.ШиринаДетали Тогда
			
			ШиринаОстатка = ШиринаПакета - Деталь.ШиринаДетали - ТолщинаПила;
	...
	Если ТаблицаОстатков.Количество() > 0 Тогда
		...
		Для каждого Остаток Из ТаблицаОстатков Цикл
			...
			ТаблицаДеталей = АлгоритмОРанце(ТаблицаВсехДеталей, Остаток.ВысотаОстатка, Остаток.ШиринаОстатка, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, Истина);
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Для ИдеальныйПроцентОтхода = 0 По 0 Цикл
			
			Для ПроцентОтходаДляСтолбцов = 0 По 0 Цикл
				
				СписокДеталейДляФормированияПакетов = СписокДеталейНоменклатуры.Скопировать();
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, ПроцентОтходаДляСтолбцов);
				...
				Если СтруктураДляСтрокиРаскроя.КоличествоЛистов < НаименьшееКоличествоЛистов
					...
					ЛучшийПроцентОтходаДляСтолбцов = ПроцентОтходаДляСтолбцов;
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков, ИдеальныйПроцентОтхода, ПроцентОтходаДляСтолбцов, Истина);
		...
		СамыйЛучшийПроцентОтхода = СамыйЛучшийПроцентОтхода + "/" + ЛучшийПроцентОтхода + "-" + ЛучшийПроцентОтходаДляСтолбцов;
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, СмешанныйПил, ТолщинаПила)
