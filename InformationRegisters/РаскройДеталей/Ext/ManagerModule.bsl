...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь)
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 3 Тогда
...
Функция ПолезныйОстаток(ПределыОстатков, ВысотаОстатка, ШиринаОстатка)
	...
	Возврат ?(ЕстьСтроки, ПределыОстатков.НайтиСтроки(Новый Структура("Высота, Ширина", ПределДлина, ПределШирина))[0].Значение, Ложь);
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= МаксимальныйПроверяемыйПроцент Цикл
				...
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ,ПоперечныйПил);
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПоперечныйПил);
