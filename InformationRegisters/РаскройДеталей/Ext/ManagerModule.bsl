...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение)
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
		...
		Если НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту Тогда
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	НоменклатураПодразделений.ОсновнаяПоСкладу.КоэффициентБазовых Как ПлощадьЛиста
			|ИЗ
			|	РегистрСведений.НоменклатураПодразделений КАК НоменклатураПодразделений
			|ГДЕ
			|	НоменклатураПодразделений.Подразделение = &Подразделение
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если НЕ КоличествоВПогонныхМетрах Тогда
					ПлощадьЛиста = ВыборкаДетальныеЗаписи.ПлощадьЛиста * 1000000;
				Иначе
					ПлощадьЛиста = ВыборкаДетальныеЗаписи.ПлощадьЛиста * 1000;
				КонецЕсли;
			КонецЦикла;
