			...
			Для каждого Столбец Из Строка.Пакет Цикл
				...
				ШиринаПредыдущейДетали = 0;
 				...
 				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, Деталь.КоординатаУ, Столбец.КоординатаУ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстатокСлева, ОстатокСлева); //ОстатокПоВысоте, ОстатокПоШирине,
					СтарыйНомерСтолбца = НомерСтолбца;
					ШиринаПредыдущейДетали = Деталь.ШиринаДетали;
	...
	Пока ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ВысотаЛиста Цикл
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
				...
				Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
					...
					Если ВысотаПакета + Деталь.ВысотаДетали + ТолщинаПила <= ВысотаЛиста
						И Деталь.ШиринаДетали <= ШиринаЛиста 
						 И Деталь.ШиринаДетали >= ШиринаПоследнейДетали / 100 * 80
						Тогда
			...
			Иначе // НЕ ПакетСобран
				...
				ЛучшееРешение.Сортировать("ШиринаДетали Убыв");
				ШиринаПакета = ?(ЛучшееРешение.Количество() > 0, ЛучшееРешение[0].ШиринаДетали, 0);
				ТаблицаПакетов = ДобавитьЛучшееРешение(ЛучшееРешение, СписокДеталейНоменклатуры, ТаблицаПакетов, ШиринаПакета, ВысотаПакета, НомерПакета, ТолщинаПила);
...
Функция ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода)
	...
	Для каждого Деталь Из ЛучшееРешение Цикл
		...
		Если Деталь.ШиринаДетали < ШиринаПакета и ШиринаПредыдущегоОстатка <> Деталь.ШиринаДетали Тогда
			...
			ВысотаОстатка = ВысотаЛиста - КоординатаВерхаДетали;
			...
			Если ТаблицаДеталей.Количество() > 0 Тогда
				...
				Для Каждого Таблица Из ТаблицаДеталей Цикл
					...
					Для каждого Деталь Из Таблица.Пакет Цикл
						...
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Деталь);
						НоваяСтрока.КоординатаУ = Остаток.КоординатаУ;
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Для ИдеальныйПроцентОтхода = 0 По 0 Цикл
			...
			Для ПроцентОтходаДляСтолбцов = 0 По 0 Цикл
				...
				Если НЕ ТипЗнч(СтруктураДляСтрокиРаскроя) = Тип("Строка") Тогда
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ТолщинаПила, КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстатокСлева, МаленькийОстатокПоШирине)  //ОстатокПоВысоте, ОстатокПоШирине,
	...
	Если МаленькийОстатокПоШирине > 0 Тогда
		НоваяСтрока = СписокДеталейНоменклатуры.Вставить(ИндексОстатка);
		НоваяСтрока.Номенклатура = Элемент.Номенклатура;
		НоваяСтрока.Количество = 1;
		НоваяСтрока.НомерЛиста = НомерЛиста;
