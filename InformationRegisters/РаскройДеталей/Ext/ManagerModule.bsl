...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	Если ЭтоСтруктура Тогда
		...
		//ДанныеДляРаскроя = ПолучитьСтрокуРаскрояПоВсемНоменклатурам(Структура.Дерево, СсылкаДокумента);
			...
			Для Каждого Деталь Из РазмещаемыйПакет Цикл
				...
				МассивОстатков = СписокДеталейНоменклатуры.НайтиСтроки(Новый Структура("Остаток", Истина));
					...
					Для Каждого ЭлементМассива Из МассивОстатков Цикл
						...
						РазмещаемаяДеталь = СписокДеталейНоменклатуры.Найти(Деталь.ИД, "ИД");
...
Функция ПолучитьСтрокуРаскрояПоВсемНоменклатурам(Дерево, СсылкаДокумента)
	...
	Для каждого ЭлементМассива Из Дерево Цикл
	...
	//	
	//	ДанныеДляРаскроя = ФормированиеСтрокиРаскроя(ДанныеДляРаскроя, Дерево, СсылкаДокумента, Подразделение, ЭтоСпецификация, ЭтоНаряд);
	//		
	//КонецЕсли;
							...
							Для Каждого Элемент Из ТекущийСписокДеталей Цикл
								...
								Для Каждого ЭлементМассива Из МассивОстатков Цикл
									...
									ИначеЕсли МассивОстатков.Найти(ЭлементМассива) + 1 = МассивОстатков.Количество() Тогда
										...
										Если (Элемент.ВысотаЛиста >= Элемент.ВысотаДетали И Элемент.ШиринаЛиста >= Элемент.ШиринаДетали)
											ИЛИ (Элемент.ШиринаЛиста >= Элемент.ВысотаДетали И Элемент.ВысотаЛиста >= Элемент.ШиринаДетали И НЕ Элемент.СоблюдениеТекстуры) Тогда
...
//////Nrn////
//Функция ФормированиеСтрокиРаскроя(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, ЭтоСпецификация, ЭтоНаряд)
///////
Функция ФормированиеСтрокиРаскроя(СтрокаРаскроя, ТаблицаДеталей, СсылкаДокумента, Подразделение, ЭтоСпецификация, ЭтоНаряд)
	...
	//			
	//			ТаблицаДеталей.Колонки.Добавить(Колонка.Имя, Колонка.ТипЗначения);
	//			
	//		КонецЦикла;
	//		
	//		Для Каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
	//			
	//			ЗаполнитьЗначенияСвойств(ТаблицаДеталей.Добавить(), СтрокаДерева);
	//			
	//		КонецЦикла;
	//		
