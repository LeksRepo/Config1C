...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		Если Элемент.Материал = "ФасадЯщика" И ЗначениеЗаполнено(Элемент.НомерИзделия) И Элемент.СоблюдениеТекстуры Тогда
			...
			//Элемент.ОсобыйПорядок = "ФасадЯщика";	
		...
		Если Элемент.ШиринаЛиста < Элемент.ВысотаДетали И Элемент.ШиринаДетали <= 201 
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл
		...
		//ЭтоМДФ = НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.МДФ18;
		...
		Если ЭтоМДФ Тогда
			ФормированиеПакетаОсобых(СписокДеталейНоменклатуры, "УзкоДлинные", ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура);
		КонецЕсли;
		
		ФормированиеПакетаОсобыхЯщиков(СписокДеталейНоменклатуры, "ФасадЯщика", ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура);
			...
			Для Инд = 0 По ОбходовПоТекстуре Цикл
				...
				Для Каждого ОстатокЛинии Из МассивОстатковЛиний Цикл
					...
					Для Каждого Элемент Из ТекущийСписокДеталей Цикл
						...
						Если Элемент.Остаток ИЛИ Элемент.ДетальВПакете Тогда
							Продолжить;
						КонецЕсли;
						...
						Для Каждого ЭлементМассива Из МассивОстатков Цикл
							...
							Если ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
								...
								РазместитьДеталь(ТекущийСписокДеталей, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, ТолщинаПила);
								Прервать;
								
							ИначеЕсли ОстатокПоВысотеПоворот >= 0  И ОстатокПоШиринеПоворот >= 0 Тогда
								
								РазместитьДеталь(ТекущийСписокДеталей, Элемент, ЭлементМассива, ОстатокПоВысотеПоворот, ОстатокПоШиринеПоворот, ГоризонтальныйПил, ТолщинаПила, Истина);
								Прервать;
					...
					Если (КоличествоЛистов = 0) ИЛИ (КоличествоЛистов >= КоличествоЛистовАлгоритма) Тогда
						...
						Если (ПлощадьОстатковИтог = -1) ИЛИ (СтруктураОптимальногоОстатка.ПлощадьОстатковПоНоменклатуре > ПлощадьОстатковИтог) ИЛИ (КоличествоЛистов >= КоличествоЛистовАлгоритма) Тогда
							...
							ОптимальныйАлгоритмРаскроя = "Текстурный - " + Инд + ", Остаток - " + ОстатокЛинии + "; ";
...
Функция ФормированиеСпискаДеталей(СсылкаДокумента, ЭтоСпецификация, ЭтоНаряд)
	...
	Если ЭтоСпецификация Тогда
		...
		|ПереключательКромкаСверху, ПереключательКромкаСлева, ПереключательКромкаСнизу, ПереключательКромкаСправа, РасположениеПазовИРучкиНаФасадах, Обтачивать, РадиусФасада");
		...
		Для Каждого Элемент Из СписокЯщиков Цикл
			...
			БелаяКромкаДляДна = Справочники.Номенклатура.НайтиПоКоду("000001634");
			//ДНО ЯЩИКА
			НоваяСтрока = СписокДеталей.Добавить();
			...
			НоваяСтрока.ВыборМебельнойКромкиСверху = БелаяКромкаДляДна;
			НоваяСтрока.ВыборМебельнойКромкиСнизу = БелаяКромкаДляДна;
			НоваяСтрока.ВыборМебельнойКромкиСлева = БелаяКромкаДляДна;
			НоваяСтрока.ВыборМебельнойКромкиСправа = БелаяКромкаДляДна;
...
Процедура РазместитьДеталь(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, ТолщинаПила, ПоворотДетали = Ложь)
	...
	Если ГоризонтальныйПил И НЕ ПакетУзких Тогда
	...
	Если ЕстьПакет Тогда
		...
		Для Каждого ЭлементПакета Из Элемент.ПакетДеталей Цикл
			...
			ИначеЕсли ЭлементПакета.ОсобыйПорядок = "ФасадЯщика" Тогда
		...
		Пока ВыборкаПоДеталям.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(ВыборкаПоДеталям.СтруктураОтверстий) Тогда
				...
				Если НЕ ЗначениеЗаполнено(ВыборкаПоДеталям.Идентификатор) Тогда
					СтрокаОтверстий = ЛексСервер.ЗаполнитьРеквизитОтверстиями(СтрокаОтверстий, ВыборкаПоДеталям.СтруктураОтверстий, ВыборкаПоДеталям.ШиринаДетали, ВыборкаПоДеталям.ВысотаДетали, ВыборкаПоДеталям.ПоворотДетали);
				КонецЕсли;
...
Процедура ФормированиеПакетаОсобых(Знач СписокДеталейНоменклатуры, Знач ОсобыйПорядок, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура)
	...
	Если ТаблицаОсобых.Количество() > 0 Тогда
		...
		ШиринаМакс = 0;
		ВысотаМакс = ВысотаЛиста;//ТаблицаОсобых[0].ВысотаДетали;
...
Процедура ФормированиеПакетаОсобыхЯщиков(Знач СписокДеталейНоменклатуры, Знач ОсобыйПорядок, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура)
	...
	Если ТаблицаОсобых.Количество() > 0 Тогда
		...
		ТаблицаОсобых.Сортировать("НомерСтроки Убыв");
		НомерИзделия = 0;
		...
		Для Каждого Элемент Из ТаблицаОсобых Цикл
			...
			Если НомерИзделия <> Элемент.НомерИзделия Тогда
				...
				Для Каждого ЭлементМассива Из МассивЭлементов Цикл
					...
					Если ЭлементМассива.ШиринаДетали >= ЭлементМассива.ВысотаДетали Тогда
						КоординатаУ = ЭлементМассива.ВысотаДетали + КоординатаУ + ТолщинаПила;
						ШиринаПакета = ЭлементМассива.ШиринаДетали;
						ВысотаПакета = КоординатаУ - ТолщинаПила;
					Иначе
						КоординатаХ = ЭлементМассива.ШиринаДетали + КоординатаХ + ТолщинаПила;
						ШиринаПакета = КоординатаХ - ТолщинаПила;
						ВысотаПакета = ЭлементМассива.ВысотаДетали;
					КонецЕсли;
