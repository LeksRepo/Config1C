...
Функция СформироватьПараметрыЭтикеток(Деталь, Буква = "")
	...
	НаКриволинейку = ЗначениеЗаполнено(Деталь.СтрокаДляФлэш) 
				 ИЛИ Деталь.КривойПилСверху 
				 ИЛИ Деталь.КривойПилСнизу 
				 ИЛИ Деталь.КривойПилСлева 
				 ИЛИ Деталь.КривойПилСправа
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Дилерский = Ложь, Буква = "", НарядЗадание = Ложь)
	...
	|Срез,
	|КоличествоПетель,
	|Обтачивать, Постформинг, РадиусФасада, КантыИзРедактора");
		...
		Если СписокДеталейРазмещенныхНаОстатках.Количество() > 0 Тогда
			...
			Для каждого ДетальВОстатках Из СписокДеталейРазмещенныхНаОстатках Цикл
				...
				Если НЕ ПоперечныйПил Тогда
					...
					РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, ДетальВОстатках.КоординатаY, КоординатаX, НомерЛиста, ДетальВОстатках.ВысотаЛиста, ДетальВОстатках.ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСлева, ИспользуетсяВОстатках); //ОстатокПоВысоте, ОстатокПоШирине,
					
				Иначе
					
					ОстатокСверху = РазмерПредыдущейДетали - ДетальВОстатках.ВысотаДетали -ТолщинаПила;
					КоординатаY = ДетальВОстатках.КоординатаY;
					РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, КоординатаY, ДетальВОстатках.КоординатаX, НомерЛиста, ДетальВОстатках.ВысотаЛиста, ДетальВОстатках.ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСверху, ИспользуетсяВОстатках, ПоперечныйПил);
...
Функция ФормированиеПакетаДляБольшогоКоличестваДеталей(СписокДеталейНоменклатуры, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивКусков, МассивБуквеннойНумерации)
	...
	СтруктураДляСтрокиРаскроя = СформироватьСтруктуруДляСтрокиРаскроя(Номенклатура, СписокДеталейНоменклатуры, ТолщинаПила, ПределыОстатков, НадписьНаОстатке, , Подразделение, БезОкругленияДоЛистов, ВысотаЛиста, МассивНомеровОстатков, МассивБуквеннойНумерации);
		...
		Если СписокДеталейРазмещенныхНаОстатках.Количество() > 0 Тогда
			...
			Для каждого ДетальВОстатках Из СписокДеталейРазмещенныхНаОстатках Цикл
				...
				Если НЕ ПоперечныйПил Тогда
					...
					ОстатокСлева = РазмерПредыдущейДетали - ДетальВОстатках.ШиринаДетали -ТолщинаПила;
					КоординатаX = ДетальВОстатках.КоординатаX;
					РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, НоваяСтрока, ТолщинаПила, ДетальВОстатках.КоординатаY, КоординатаX, НомерЛиста, ДетальВОстатках.ВысотаЛиста, ДетальВОстатках.ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСлева, ИспользуетсяВОстатках); //ОстатокПоВысоте, ОстатокПоШирине,
					
				Иначе
					
					ОстатокСверху = РазмерПредыдущейДетали - ДетальВОстатках.ВысотаДетали -ТолщинаПила;
					КоординатаY = ДетальВОстатках.КоординатаY;
					РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, НоваяСтрока, ТолщинаПила, КоординатаY, ДетальВОстатках.КоординатаX, НомерЛиста, ДетальВОстатках.ВысотаЛиста, ДетальВОстатках.ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСверху, ИспользуетсяВОстатках, ПоперечныйПил);
					...
					Для Каждого Деталь Из РазмещаемыйПакет Цикл
						...
						Если НЕ Деталь.ДетальРазмещена И (НЕ ПроверятьНомерЛиста или (ПроверятьНомерЛиста и ПроверочныйНомерЛиста <> Число(СписокНомеров[0].Представление) И ПроверочныйНомерЛиста <> Число(СписокНомеров[1].Представление))) Тогда
							...
							Если НЕ ПоперечныйПил Тогда
								...
								РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, Деталь.КоординатаY, КоординатаX, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСлева, ИспользуетсяВОстатках); //ОстатокПоВысоте, ОстатокПоШирине,
							...
							Иначе
								...
								РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, КоординатаY, КоординатаX, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСверху, ИспользуетсяВОстатках, ПоперечныйПил);
...
Функция СформироватьСтруктуруДляСтрокиРаскроя(Номенклатура, СписокДеталейНоменклатуры, ТолщинаПила, ПределыОстатков, НадписьНаОстатке, ПоворотДеталей = Ложь, Подразделение, БезОкругленияДоЛистов, ВысотаЛиста, МассивНомеровОстатков, МассивБуквеннойНумерации = Неопределено)
	...
	Для Каждого Строка Из ТаблицаПодсчетаЛистов Цикл
		...
		Если СтруктураРазмеровЛиста.ВысотаЛиста <> Строка.ВысотаЛиста ИЛИ СтруктураРазмеровЛиста.ШиринаЛиста <> Строка.ШиринаЛиста Тогда			
	...
	|			ТОГДА ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.ДлинаДетали, НоменклатураПодразделений.Номенклатура.ДлинаДетали)
	|		ИНАЧЕ ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.ДлинаДетали, НоменклатураПодразделений.Номенклатура.ДлинаДетали) * ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.ШиринаДетали, НоменклатураПодразделений.Номенклатура.ШиринаДетали)
	...
	Фотопечать = Номенклатура = ОбщаяНоменклатура.СтеклоСФотопечатью4мм Или Номенклатура = ОбщаяНоменклатура.СтеклоСФотопечатью6мм;
	...
	Иначе
		...
		Иначе
			...
			Если НЕ КоличествоВПогонныхМетрах Тогда
				 ...
				 ПлощадьПила = (ПростоПлощадьВсехЛистов + ПлощадьВсехКусков) - (ПлощадьДеталей + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре + СтруктураОптимальногоОстатка.ПлощадьОстатковПоНоменклатуре);   
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, ПолныйСписокДеталей, НарядЗадание, МассивБуквеннойНумерации)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Если КоличествоДеталейНоменклатуры > 999 Тогда
			//Доработать.
			СтруктураДляСтрокиРаскроя = ФормированиеПакетаДляБольшогоКоличестваДеталей(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОстатков,МассивБуквеннойНумерации);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей, СписокДеталей)
	...
	Для Каждого СтрокаДерева1 ИЗ Дерево.Строки[0].Строки Цикл
		
		Неноменклатурный = СтрокаДерева1.Номенклатура.Неноменклатурный;
		...
		Для каждого СтрокаДерева2 Из СтрокаДерева1.Строки Цикл
			...
			Для Каждого СтрокаДерева3 Из СтрокаДерева2.Строки Цикл
				...
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаДерева3);
			...
			ЗапросНаКромки.УстановитьПараметр("Номенклатура", СтрокаДерева1.Номенклатура);
			...
			|ГДЕ
			|	ТабДетали.Номенклатура = &Номенклатура
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			ТаблицаКромок.Свернуть("Кромка,КраткоеНаименование,Предоставление,Комментарий");
			
			СтрокаКромокДляШапки = "";
			
			Для Каждого Стр ИЗ ТаблицаКромок Цикл
			...
			Запрос = Новый Запрос;
			Запрос.УстановитьПараметр("Номенклатура", СтрокаДерева1.Номенклатура);
			Запрос.УстановитьПараметр("Подразделение", Подразделение);
			Запрос.Текст = 
			"ВЫБРАТЬ
			...
			|ВЫБРАТЬ
			|	%ВСЕПОЛЯТАБЛИЦЫДЕТАЛИ%,
			|	ТаблицаДеталей.Номенклатура.Неноменклатурный КАК Неноменклатурный,
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НарядЗадание Тогда
					ШапкаЛиста.Добавить(Выборка.Парковка); //16
				КонецЕсли;
				...
				Пока ВыборкаПоДеталям.Следующий() Цикл
					...
					ПроверкаКромкиСверху = ПодставляемаяНоменклатура.Найти(ВыборкаПоДеталям.ВыборМебельнойКромкиСверху, "ПодставляемаяНоменклатура");
					ПроверкаКромкиСнизу = ПодставляемаяНоменклатура.Найти(ВыборкаПоДеталям.ВыборМебельнойКромкиСнизу, "ПодставляемаяНоменклатура");
					ПроверкаКромкиСлева = ПодставляемаяНоменклатура.Найти(ВыборкаПоДеталям.ВыборМебельнойКромкиСлева, "ПодставляемаяНоменклатура");
					ПроверкаКромкиСправа = ПодставляемаяНоменклатура.Найти(ВыборкаПоДеталям.ВыборМебельнойКромкиСправа, "ПодставляемаяНоменклатура");					
