			...
			Для каждого Столбец Из Строка.Пакет Цикл
				...
				РазмещаемыйПакет.Сортировать("ИспользуетсяВОстаткахПакета Возр, ШиринаДетали Убыв, КоординатаХ Возр, КоординатаУ Возр");
 				...
 				Для Каждого Деталь Из РазмещаемыйПакет Цикл
					...
					ОстатокСлева = ШиринаПредыдущейДетали - Деталь.ШиринаДетали -ТолщинаПила;
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПроверкаОстатков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
	...
	//ТаблицаВсехДеталейОкончательная = ТаблицаВсехДеталей.Скопировать();
		...
		Пока ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ВысотаЛиста Цикл
			...
			Если ТаблицаПервыхДеталей.Количество() > 0 Тогда
				...
				Если ПерваяДеталь.Остаток
					...
					//ИсправляемаяСтрока = ТаблицаВсехДеталейОкончательная.Найти(ПерваяДеталь.GUID, "GUID");
					//ИсправляемаяСтрока.ДетальВПакете = Истина;
				...
				Если ПерваяДеталь.ВысотаДетали <= ВысотаЛиста Тогда
					...
					ИтогВысотыПоДеталям = ТаблицаПервыхДеталей.Итог("ВысотаДетали") + ТолщинаПила * (ТаблицаПервыхДеталей.Количество() - 1);
				Иначе
					//ТаблицаПервыхДеталей.Найти(ПерваяДеталь.GUID, "GUID").ДетальПроверена = Истина;
					//ТаблицаВсехДеталейДляТестов.Найти(ПерваяДеталь.GUID, "GUID").ДетальПроверена = Истина;
					ТаблицаПервыхДеталей.Удалить(ПерваяДеталь);
					ИтогВысотыПоДеталям = ТаблицаПервыхДеталей.Итог("ВысотаДетали") + ТолщинаПила * (ТаблицаПервыхДеталей.Количество() - 1);
					Продолжить;
				КонецЕсли;
			...
			Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
				...
				//ПоследнееРешение.Очистить();
				//ТаблицаОставшихсяНепроверенныхДеталей = ТаблицаОставшихсяДеталей.Скопировать();
				...
				Если НЕ ПакетСобран Тогда
					...
					ТаблицаОставшихсяДеталей.Удалить(СтрокаДляПоиска);
					//СтрокаДляПоиска = ТаблицаОставшихсяНепроверенныхДеталей.Найти(Элемент.GUID, "GUID");
					//ТаблицаОставшихсяНепроверенныхДеталей.Удалить(СтрокаДляПоиска);
					...
					Иначе
						...
						//ТаблицаВсехДеталейДляТестов.Найти(Элемент.GUID, "GUID").ДетальПроверена = Истина;
					...
					КонецЕсли;
					//ТаблицаОставшихсяДеталей.Колонки.Добавить("ПроверенаВКачествеВторойДетали", Новый ОписаниеТипов("Булево"));
					//Счетчик = 0;
					//
					//Пока ТаблицаОставшихсяНепроверенныхДеталей.Количество() > 0 Цикл
					//	
					//	ТаблицаОставшихсяДеталей[Счетчик].ПроверенаВКачествеВторойДетали = Истина;
						
						//Для каждого Деталь Из ТаблицаОставшихсяНепроверенныхДеталей Цикл
						Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
							...
							Если ВысотаПакета + Деталь.ВысотаДетали + ТолщинаПила <= ВысотаЛиста
								...
								//ИначеЕсли Деталь.МожноПовернуть И ВысотаПакета + Деталь.ШиринаДетали + ТолщинаПила <= ВысотаЛиста 
								//	И Деталь.ВысотаДетали <= ШиринаЛиста 
								//	И Деталь.ВысотаДетали >= ШиринаПоследнейДетали / 100 * РазрешенныйПроцентПоШирине
								//	Тогда
								//	
								//	ВысотаДетальки = Деталь.ВысотаДетали;
								//	Деталь.ВысотаДетали = Деталь.ШиринаДетали;
								//	Деталь.ШиринаДетали = ВысотаДетальки;
								//	Деталь.ПоворотДетали = НЕ Деталь.ПоворотДетали;
								//	
								//	НоваяСтрока = ПоследнееРешение.Добавить();
								//	ЗаполнитьЗначенияСвойств(НоваяСтрока, Деталь);
								//	
							...
							//		ЛучшаяПолоса = ПроверитьВозможностьИспользованияОстатков(ПоследнееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине);
							//	Иначе
							//		ЛучшаяПолоса = ПоследнееРешение.Скопировать();
							//	КонецЕсли;
							//	
							...
							//Если ПроцентСвободногоПространстваСтолбца < ПоследнийПроцентОстаткаСвободногоПространстваСтолбца Тогда
							//	
							//	ПоследнийПроцентОстаткаСвободногоПространстваСтолбца = ПроцентСвободногоПространстваСтолбца;
							//	ЛучшийСтолбец = ПоследнееРешение.Скопировать();
							//	
							//КонецЕсли;
					...
					//	Отбор.Вставить("ПроверенаВКачествеВторойДетали", Ложь);
					//	МассивВсехДеталей = ТаблицаОставшихсяДеталей.НайтиСтроки(Отбор);
					//	ТаблицаОставшихсяНепроверенныхДеталей = ТаблицаОставшихсяДеталей.Скопировать(МассивВсехДеталей);
					//	Счетчик = Счетчик + 1;
					//	
					//КонецЦикла;
					
					//ТаблицаОставшихсяДеталей.Колонки.Удалить("ПроверенаВКачествеВторойДетали");
					...
					Если СтруктураДляПроверкиЗаполненности <> Неопределено Тогда
						...
						Если ПроцентЗаполненностиПолосы > ПоследнийПроцентЗаполненностиПолосы Тогда
							...
							//ТаблицаВсехДеталейОкончательная = ТаблицаВсехДеталейДляТестов.Скопировать();
				...
				Иначе // НЕ ПакетСобран
					//Пока отключено, пакет не будет собран никогда
					...
					//	//Если НЕ ПроверкаОстатков Тогда
					//	//	ЛучшийСтолбец = ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине);
					//	//КонецЕсли;
					//КонецЕсли;
					//
					//ВысотаПакета = ЛучшаяПолоса.Итог("ВысотаДетали") + ТолщинаПила * (ЛучшаяПолоса.Количество() - 1);
				...
				////////Для каждого Строка из ПоследнееРешение Цикл 
				////////	ИсправляемаяСтрока = ТаблицаВсехДеталейДляТестов.Найти(Строка.GUID, "GUID");
				////////	ИсправляемаяСтрока.ДетальВПакете =ИсправляемаяСтрока.GUID = ПерваяДеталь.GUID;
				////////КонецЦикла;
			...
			Если ЛучшаяПолоса.Количество() > 0 Тогда
				...
				//	ЛучшийСтолбец = ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине);
				//КонецЕсли;
				//СписокДеталейНоменклатуры = ТаблицаВсехДеталейОкончательная.Скопировать();
				РешениеДляДобавления = ЛучшаяПолоса.Скопировать();
...
Функция ДобавитьЛучшееРешение(ЛучшееРешение, СписокДеталейНоменклатуры, ТаблицаПакетов, ШиринаПакета, ВысотаПакета, НомерПакета, ТолщинаПила, ПроставлятьКоординаты = Ложь)
	...
	//	Для каждого Строка Из СписокДеталейНоменклатуры Цикл
	//		
	//		Если НЕ Строка.ДетальВПакете Тогда
	//			Строка.КоординатаХ = 0;
...
Функция ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине)
	...
	КонецЕсли;
		...
		//
		//	ИсправляемаяСтрока = СписокДеталейНоменклатуры.Найти(Строка.GUID, "GUID");
		//	ИсправляемаяСтрока.ДетальВПакете = Ложь;
		//	
		//КонецЦикла;
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		МаксимальныйПроверяемыйПроцент = ?(КоличествоДеталейНоменклатуры <=50, 90, 0);
