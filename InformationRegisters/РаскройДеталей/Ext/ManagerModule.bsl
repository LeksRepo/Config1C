...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	ТестДЛЛ = РаскройСервер.СформироватьКартинку(Структура.МассивИтоговогоРасположенияДеталей, СсылкаДокумента);
	// } Васильев Александр Леонидович [26.05.2015]
...
Функция ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, КонечныйНомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, ГоризонтальныйПил)
	...
	Для ТекущийНомерЛиста = КонечныйНомерЛиста + 1 По НомерЛиста Цикл
		...
		Для Каждого ЭлементМассиваОстатков Из МассивОстатков Цикл
			...
			// Nrn - а почему удалить онельзя было мелкие наверху, когда уже каждый на эту херь проверяли????
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПределыОстатков)
	
	СписокДеталейНоменклатуры.Колонки.Добавить("ИспользуетсяВОбрезкахПакета", Новый ОписаниеТипов("Булево"));
	ТаблицаПакетов = АлгоритмРанца(СписокДеталейНоменклатуры, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура);
	
	ТаблицаСтолбцов = Новый ТаблицаЗначений; //Для передачи пакетов как деталей в АлгоритмРанца
	...
	ТаблицаСтолбцов.Колонки.Добавить("ИД", Новый ОписаниеТипов("Число"));
	НомерЛиста = 1;
	
	Для каждого Пакет Из ТаблицаПакетов Цикл
		
		НоваяСтрока = ТаблицаСтолбцов.Добавить();
		НоваяСтрока.ВысотаДетали = Пакет.ШиринаПакета;//Чтобы адекватно работал алгоритм, придется менять высоту и ширину деталей и листа
		НоваяСтрока.ШиринаДетали = Пакет.ВысотаПакета;
		НоваяСтрока.ИД = Пакет.НомерПакета;
		
	КонецЦикла;
	
	ТаблицаРазмещенияСтолбцов = АлгоритмРанца(ТаблицаСтолбцов, ШиринаЛиста, Номенклатура.ДлинаДетали, ТолщинаПила, Номенклатура);
	...
	Для Каждого Строка Из ТаблицаРазмещенияСтолбцов Цикл
		...
		Для каждого Столбец Из Строка.Пакет Цикл
			...
			СтрокаПоиска = ТаблицаПакетов.Найти(Столбец.ИД, "НомерПакета");
			...
			Для Каждого Деталь Из РазмещаемыйПакет Цикл
				...
				Если Деталь.ИспользуетсяВОбрезкахПакета Тогда
					
					
					
				Иначе
					
					//РазместитьДеталь(СписокДеталейНоменклатуры, Деталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, ЭлементСмешанногоПила, ТолщинаПила);
					...
					Для Каждого ЭлементМассива Из МассивОстатков Цикл
						...
						Если ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
							...
							РазместитьДеталь(СписокДеталейНоменклатуры, Деталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, Ложь, 0, ТолщинаПила);
							Прервать;
							
						ИначеЕсли ОстатокПоВысотеПоворот >= 0  И ОстатокПоШиринеПоворот >= 0 Тогда
							
							РазместитьДеталь(СписокДеталейНоменклатуры, Деталь, ЭлементМассива, ОстатокПоВысотеПоворот, ОстатокПоШиринеПоворот, Ложь, 0, ТолщинаПила, Истина);
							Прервать;
					...
					//Деталь.ДетальВПакете = Истина;
					//Деталь.КоординатаУ = КоординатаУ; 
					//Деталь.КоординатаХ = КоординатаХ;
					//Деталь.НомерПакета = Пакет.НомерПакета;
					//
					//КоординатаУ = КоординатаУ + Деталь.ВысотаДетали + ТолщинаПила;
					//
					//ПоследняяДетальПакета = Деталь;
					//
					//СписокДеталейНоменклатуры.Найти(Деталь.ИД, "ИД").ДетальВПакете = Истина;
					//СписокДеталейНоменклатуры.Найти(Деталь.ИД, "ИД").НомерПакета = Пакет.НомерПакета;
					
					//ОстатокПоШирине = ШиринаПакета - Элемент.ШиринаДетали;
					...
					//	НоваяСтрока.Количество = 1;
					//	НоваяСтрока.НомерЛиста = 1;  ///////////////////////////////Указать позже номер листа
					...
					//	НоваяСтрока.КоординатаХ = Элемент.КоординатаХ + Элемент.ШиринаДетали;
					//	НоваяСтрока.Остаток = Истина;
					//	
					//КонецЕсли;
					//
					//Если ОстатокПоВысоте > 0 Тогда
					//	НоваяСтрока = ТаблицаОстатков.Вставить(ИндексОстатка);
					//	НоваяСтрока.Номенклатура = Элемент.Номенклатура;
					//	НоваяСтрока.Количество = 1;
	...
	СтруктураОптимальногоОстатка = ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, НомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, Ложь);
	
	ТаблицаДеталейИтог = СтруктураОптимальногоОстатка.СписокДеталейНоменклатуры;
	ОптимальныйАлгоритмРаскроя = Строка(НоменклатурнаяГруппа) + "Текстурный - " + 0 + 
	...
	СтруктураДляСтрокиРаскроя.Вставить("КоличествоЛистов", НомерЛиста - 1);
	//Для Каждого Элемент Из МассивПакетов Цикл
	//	
	//	ОставшиесяПакеты.Добавить(Элемент);
	//	ОбщаяШирина = ОбщаяШирина + Элемент.Итог("ШиринаДетали");
	//	
	//КонецЦикла;
	//
	//ИдеальныйОстатокСКраю = 10;
	
	//Пока ОбщаяШирина >0 и ОбщаяШирина > ШиринаЛиста Цикл
	//	
	//	ПакетРазмещен = Ложь;
	//	
	//	Для каждого Элемент Из МассивПакетов Цикл
	//		
	//		Если НЕ ПакетРазмещен Тогда
	//			
	//			Для каждого Пакет Из ОставшиесяПакеты Цикл
	//			
	//				
	//			
	//			КонецЦикла;
...
Функция АлгоритмРанца(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПроверкаОбрезков = Ложь)
	...
	ТаблицаПакетов.Колонки.Добавить("НомерПакета", Новый ОписаниеТипов("Число"));
	...
	Пока ИтогВысотыПоДеталям > 0 И ИтогВысотыПоДеталям > ВысотаЛиста Цикл
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
			...
			Если НЕ ПакетСобран Тогда
				...
				ШиринаПакета = ?(ШиринаПакета = 0, Элемент.ШиринаДетали, ШиринаПакета);
				СтрокаДляПоиска = ТаблицаОставшихсяДеталей.Найти(Элемент.ИД, "ИД");
				...
				Иначе
					...
					СписокДеталейНоменклатуры.Найти(Элемент.ИД, "ИД").ДетальПроверена = Истина;
					Продолжить;
				...
				Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
					...
					Если ВысотаПакета + Деталь.ВысотаДетали + ТолщинаПила <= ВысотаЛиста И Деталь.ШиринаДетали <= ШиринаЛиста Тогда
					...
					ПлощадьОстаткаСверху = ШиринаПакета * (ВысотаЛиста - ВысотаПакета);
			...
			Иначе
				...
				ЛучшееРешение = ПроверитьВозможностьИспользованияОбрезков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура);
	...
	Если НЕ ПроверкаОбрезков Тогда
		
		МассивВсехДеталей = СписокДеталейНоменклатуры.НайтиСтроки(Новый Структура("ДетальВПакете", Ложь));
		ТаблицаОставшихсяДеталей = СписокДеталейНоменклатуры.Скопировать(МассивВсехДеталей);
		ТаблицаВсехДеталей.Сортировать("ШиринаДетали Убыв, ВысотаДетали Убыв");
		
		Если ТаблицаВсехДеталей.Количество() > 0 Тогда
			
			ШиринаПакета = ТаблицаВсехДеталей[0].ШиринаДетали;
			ВысотаПакета = ТаблицаВсехДеталей.Итог("ВысотаДетали") + ТолщинаПила * (ТаблицаВсехДеталей.Количество() - 1);
			ТаблицаПакетов = ДобавитьЛучшееРешение(ТаблицаОставшихсяДеталей, СписокДеталейНоменклатуры, ТаблицаПакетов, ШиринаПакета, ВысотаПакета, НомерПакета);
...
Функция ДобавитьЛучшееРешение(ЛучшееРешение, СписокДеталейНоменклатуры, ТаблицаПакетов, ШиринаПакета, ВысотаПакета, НомерПакета)
	...
	Для каждого Строка Из ЛучшееРешение Цикл
		...
		ИсправляемаяСтрока = СписокДеталейНоменклатуры.Найти(Строка.ИД, "ИД");
...
Функция ПроверитьВозможностьИспользованияОбрезков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, ШиринаПакета, Номенклатура)
	...
	Для каждого Деталь Из ЛучшееРешение Цикл
		...
		Если Деталь.ШиринаДетали < ШиринаПакета Тогда
			
			ШиринаОбрезка = ШиринаПакета - Деталь.ШиринаДетали;
			ВысотаОбрезка = Деталь.ВысотаЛиста - КоординатаУ;
	...
	Если ТаблицаОбрезков.Количество() > 0 Тогда
		...
		Для каждого Обрезок Из ТаблицаОбрезков Цикл
			...
			ТаблицаДеталей = АлгоритмРанца(ТаблицаВсехДеталей, Обрезок.ВысотаОбрезка, Обрезок.ШиринаОбрезка, ТолщинаПила, Номенклатура, Истина);
...
//Функция РазместитьПакетДеталей(ТекущийСписокДеталей, Столбец, НомерЛиста, ТекущаяНоменклатура, ВысотаЛиста, ШиринаЛиста, ТолщинаПила)
//	
...
//	КонецЕсли;
//	
//	Для каждого Деталь Из Столбец Цикл
//		
//		Для Каждого ЭлементМассива Из МассивОстатков Цикл
//			
//			//Возможность размещения детали на выбранном остатке
...
//			Если ОстатокПоВысоте >= 0 И ОстатокПоШирине >= 0 Тогда
//				
//				РазместитьДеталь(ТекущийСписокДеталей, Деталь, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, Ложь, 0, ТолщинаПила);
//				Прервать;
//				
//			КонецЕсли;
//			
...
Функция ФормированиеСпискаДеталей(СсылкаДокумента)
	...
	Если ЭтоСпецификация Тогда
		...
		Для Каждого Элемент Из СписокЯщиков Цикл
			...
			//RonEXI: Новый код для ящиков. Старый потом удалить. Когда готово будет.
...
Функция РасположитьНаЛистеМетодомВетвейИГраниц(СписокНоменклатуры, СписокДеталей, ТолщинаПила)
	
	//Для каждого ТекущаяНоменклатура Из СписокНоменклатуры Цикл
	//	
	//	ОтсортированныйСписокДеталей = СписокДеталей.Скопировать();
	//	ОтсортированныйСписокДеталей.Сортировать("ШиринаДетали Убыв, ВысотаДетали Убыв");
	//	
	//	НомерЛиста = 1;
	//	//Новый лист
	//	НоваяСтрока = ОтсортированныйСписокДеталей.Добавить();
	//	НоваяСтрока.Номенклатура = ТекущаяНоменклатура;
	//	НоваяСтрока.Количество = 1;
	//	НоваяСтрока.НомерЛиста = НомерЛиста;
	//	НоваяСтрока.ВысотаДетали = ТекущаяНоменклатура.Номенклатура.ДлинаДетали;
	//	НоваяСтрока.ШиринаДетали = ТекущаяНоменклатура.Номенклатура.ШиринаДетали;
	//	НоваяСтрока.ВысотаЛиста = ТекущаяНоменклатура.Номенклатура.ДлинаДетали;
	//	НоваяСтрока.ШиринаЛиста = ТекущаяНоменклатура.Номенклатура.ШиринаДетали;
	//	НоваяСтрока.КоординатаУ = 0;
	//	НоваяСтрока.КоординатаХ = 0;
	//	НоваяСтрока.Остаток = Истина;
	//	ОстатокПоШирине = ТекущаяНоменклатура.Номенклатура.ДлинаДетали;;
	//	ОстатокПоВысоте = ТекущаяНоменклатура.Номенклатура.ШиринаДетали;
	//			
	//	Для каждого Деталь Из ОтсортированныйСписокДеталей Цикл
	//		
	//		Если ОстатокПоШирине > 0 Тогда
	//			
	//			НоваяСтрока = ОтсортированныйСписокДеталей.Вставить(ИндексОстатка);
	//			НоваяСтрока.Номенклатура = Деталь.Номенклатура;
	//			НоваяСтрока.Количество = 1;
	//			//НоваяСтрока.НомерЛиста = ЭлементМассива.НомерЛиста;
	//			НоваяСтрока.ВысотаДетали = Деталь.ВысотаДетали;
	//			НоваяСтрока.ШиринаДетали = ОстатокПоШирине;
	//			НоваяСтрока.ВысотаЛиста = Деталь.ВысотаЛиста;
	//			НоваяСтрока.ШиринаЛиста = Деталь.ШиринаЛиста;
	//			НоваяСтрока.КоординатаУ = Деталь.КоординатаУ;
	//			НоваяСтрока.КоординатаХ = Деталь.КоординатаХ + Деталь.ШиринаДетали + ТолщинаПила;
	//			НоваяСтрока.Остаток = Истина;
	//			
	//		КонецЕсли;
	//		
	...
	//			//НоваяСтрока.НомерЛиста = ЭлементМассива.НомерЛиста;
	//			НоваяСтрока.ВысотаДетали = ОстатокПоВысоте;
	//			НоваяСтрока.ШиринаДетали = Деталь.ШиринаДетали;
	//			НоваяСтрока.ВысотаЛиста = Деталь.ВысотаЛиста;
	//			НоваяСтрока.ШиринаЛиста = Деталь.ШиринаЛиста;
	//			НоваяСтрока.КоординатаУ = Деталь.КоординатаУ + Деталь.ВысотаДетали + ТолщинаПила;
	//			НоваяСтрока.КоординатаХ = Деталь.КоординатаХ;
	//			НоваяСтрока.Остаток = Истина;
	//			
	//		КонецЕсли;
	//		
...
Функция РасположитьНаЛистах(СсылкаДокумента, СписокДеталей, Перераскрой)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		//////nrn
		//СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейНоменклатуры, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ПределыОстатков);
		//////
				...
				Для Каждого ОстатокЛинии Из МассивОстатковЛиний Цикл
					...
					Для ИндВысокие = 1 По 2 Цикл
						...
						Для Каждого ЭлементСмешанногоПила Из МассивСмешанногоПила Цикл
							...
							//РазместитьПакетыДеталей(МассивПакетов,ТекущийСписокДеталей, ТолщинаПила);
		...
		//МассивИтоговогоРасположенияДеталей.Добавить(СтруктураДляСтрокиРаскроя.ТаблицаДеталейИтог);
...
Процедура РазместитьПакетыДеталей(МассивПакетов,ТекущийСписокДеталей, ТолщинаПила);
