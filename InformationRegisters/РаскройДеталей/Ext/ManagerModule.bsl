...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено, ПоказатьВесьСписокДеталей = Ложь, Наряды = Ложь) Экспорт
	...
	Если ТипЗнч(ДокОбъект) = Тип("ДокументСсылка.НарядЗадание") И СписокДеталей = Неопределено Тогда
		...
		Если ДокОбъект.СписокСпецификаций.Количество() > 0 Тогда
			//Если все спецификации одного дилера
			Дилерский = ПроверитьКонтрагента(ДокОбъект.СписокСпецификаций.Выгрузить(,"Спецификация"));
			НомерСимвола = 64;
	...
	Если СписокДеталей = Неопределено или НарядЗадание Тогда
		...
		Дилерский = ?(НарядЗадание, Дилерский, ДокОбъект.Контрагент.Дилер);
...
Функция ПроверитьКонтрагента(СписокСпецификаций)

		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("ТЗ", СписокСпецификаций);
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ВЫРАЗИТЬ(ТЗ.Спецификация КАК Документ.Спецификация) КАК Спецификация
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СписокСпецификаций.Спецификация.Контрагент КАК Контрагент,
		|	СписокСпецификаций.Спецификация.Контрагент.Дилер КАК Дилер
		|ИЗ
		|	СписокСпецификаций КАК СписокСпецификаций
		|
		|СГРУППИРОВАТЬ ПО
		|	СписокСпецификаций.Спецификация.Контрагент,
		|	СписокСпецификаций.Спецификация.Контрагент.Дилер
		|ИТОГИ
	...
	ВыборкаДилер = ВыборкаКонтрагент.Выбрать();
	ВыборкаДилер.Следующий();	
	
	Дилерский = ВыборкаКонтрагент.Контрагент = 1 И ВыборкаДилер.Дилер;
	
	Возврат Дилерский;
	
КонецФункции // ПроверитьКонтрагента()
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Дилерский = Ложь, Буква = "", НарядЗадание = Ложь)
	...
	Если Дилерский Тогда
		Для каждого ИзменяемаяСтрока Из ТЗДетали Цикл
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, Знач МассивКусков)
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 или МассивКусков.Количество() > 0 Тогда
		...
		СтруктураДляСтрокиРаскроя = СформироватьСтруктуруДляСтрокиРаскроя(Номенклатура, СписокДеталейНоменклатуры, ТолщинаПила, ПределыОстатков, НадписьНаОстатке, ПоворотДеталей, Подразделение, БезОкругленияДоЛистов, ВысотаЛиста, МассивНомеровОстатков);
...
Функция СформироватьСтруктуруДляСтрокиРаскроя(Номенклатура, СписокДеталейНоменклатуры, ТолщинаПила, ПределыОстатков, НадписьНаОстатке, ПоворотДеталей = Ложь, Подразделение, БезОкругленияДоЛистов, ВысотаЛиста, МассивНомеровОстатков)
	...
	Если КоличествоВПогонныхМетрах Тогда
		...
		ОтборДеталейПоЛистам = Новый Структура("Остаток", Ложь);
		ОтборДеталейПоЛистам.Вставить("НомерЛиста",0);
		
		//Цикл по листам
		//Для Листочек = 1 По ВсегоЛистовДаннойНоменклатуры Цикл
		Для Каждого Листочек Из ТаблицаПодсчетаЛистов Цикл
			
			ПроверочноеЧисло = 0;
			ПлощадьДеталейОдногоЛиста = 0;
			
			ОтборДеталейПоЛистам.НомерЛиста = Листочек.НомерЛиста;
			МассивДеталейДляПодсчета = СписокДеталейНоменклатуры.НайтиСтроки(ОтборДеталейПоЛистам);
	...
	Иначе
		...
		Иначе
			ПлощадьОбрезковПоНоменклатуре = ?(НЕ КоличествоВПогонныхМетрах, ПлощадьДеталей, ПлощадьДеталейВПогонныхМетрах)
			                                                                      +СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей, НарядЗадание)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Иначе
			...
			СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ,ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОстатков);
			...
			Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
				...
				СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, Истина, ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОстатковДляРазвернутыхДеталей);
