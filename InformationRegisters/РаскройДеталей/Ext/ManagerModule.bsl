	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда  //    НарядЗадание или 
		...
		Иначе
			Если Фотопечать Тогда
				ПлощадьОбрезковПоНоменклатуре = СтруктураОптимальногоОстатка.ПлощадьОстатковПоНоменклатуре;
			Иначе
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НЕ МатериалПодЗаказ и ПроверятьНеобходимостьУдаленияМатериала и НЕ ПоказатьВесьСписокДеталей Тогда
					Продолжить;
				КонецЕсли;
