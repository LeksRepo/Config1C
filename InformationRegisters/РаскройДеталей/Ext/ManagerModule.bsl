...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	Если ТипЗнч(СсылкаДокумента.Ссылка) = Тип("ДокументСсылка.Спецификация") Тогда
		...
		Если СсылкаДокумента.Дилерский Тогда
	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		Если Элемент.Материал = "ФасадЯщика" И ЗначениеЗаполнено(Элемент.НомерИзделия) Тогда
	...
	//______________________________________________________	
	СписокНоменклатуры = СписокДеталей.Скопировать(,"Номенклатура, СоблюдениеТекстуры");
	СписокНоменклатуры.Свернуть("Номенклатура", "СоблюдениеТекстуры");
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл
		...
		НоменклатурнаяГруппа = ЭлементСписка.Номенклатура.НоменклатурнаяГруппа;
		
		//ЭтоМДФ = НоменклатурнаяГруппа.ПринадлежитЭлементу(Справочники.НоменклатурныеГруппы.МДФ);
		...
		СписокДеталейНоменклатуры = СписокДеталей.Скопировать(Новый Структура("Номенклатура", ЭлементСписка.Номенклатура));
		...
		Для Каждого НаправлениеПила Из МассивНаправленийПила Цикл
			...
			Если ГоризонтальныйПил Тогда //И НЕ ЭтоМДФ Тогда
				Продолжить;
			КонецЕсли;
				...
				Для Каждого ОстатокЛинии Из МассивОстатковЛиний Цикл
					...
					Для Каждого Элемент Из СписокДеталейНоменклатуры Цикл
						...
						Если МассивОстатков.Количество() = 0 Тогда
							ДобавитьНовыйЛист(МассивОстатков, НомерЛиста, СписокДеталейНоменклатуры, Элемент);
						КонецЕсли;
						...
						Для Каждого ЭлементМассива Из МассивОстатков Цикл
							...
							Если ОстатокПоВысоте >= 0  И ОстатокПоШирине >= 0 Тогда
								...
								РазместитьДеталь(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, ТолщинаПила);
								Прервать;
								
							ИначеЕсли ОстатокПоВысотеПоворот >= 0  И ОстатокПоШиринеПоворот >= 0 Тогда
								
								РазместитьДеталь(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысотеПоворот, ОстатокПоШиринеПоворот, ГоризонтальныйПил, ТолщинаПила, Истина);
								Прервать;
					...
					Если (КоличествоЛистов = 0) ИЛИ (КоличествоЛистов >= КоличествоЛистовАлгоритма) Тогда
						СтруктураОптимальногоОстатка = ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, КонечныйНомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, ГоризонтальныйПил);
	...
	ПустойРаскрой = (ДанныеДляРаскроя = "%РОСПИЛ%☻") ИЛИ (ДанныеДляРаскроя = "%ЛЕКС%☻");		
	ПустойКривойПил = (СтрокаКривогоПила = "%РОСПИЛ%☻") ИЛИ (СтрокаКривогоПила = "%ЛЕКС%☻");
...
Процедура ДобавитьНовыйЛист(Знач МассивОстатков, Знач НомерЛиста, Знач СписокДеталейНоменклатуры, Знач Элемент)
...
Процедура РазместитьДеталь(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ГоризонтальныйПил, ТолщинаПила, ПоворотДетали = Ложь)
	...
	//	ИндексВысоты = ?(ГоризонтальныйПил, ИндексОстатка, ИндексОстатка + 1);
	//	НоваяСтрока = СписокДеталейНоменклатуры.Вставить(ИндексВысоты);
	//	НоваяСтрока.Номенклатура = Элемент.Номенклатура;
	//	НоваяСтрока.Количество = 1;
	//	НоваяСтрока.НомерЛиста = ЭлементМассива.НомерЛиста;
	//	НоваяСтрока.ВысотаДетали = ОстатокПоВысоте;
	//	НоваяСтрока.ШиринаДетали = ?(ГоризонтальныйПил, ЭлементМассива.ШиринаДетали, Элемент.ШиринаДетали);
	//	НоваяСтрока.ВысотаЛиста = Элемент.ВысотаЛиста;
	//	НоваяСтрока.ШиринаЛиста = Элемент.ШиринаЛиста;
	...
	//	ИндексШирины = ?(ГоризонтальныйПил, ИндексОстатка + 1, ИндексОстатка);
	//	НоваяСтрока = СписокДеталейНоменклатуры.Вставить(ИндексШирины);
	//	НоваяСтрока.Номенклатура = Элемент.Номенклатура;
	//	НоваяСтрока.Количество = 1;
	//	НоваяСтрока.НомерЛиста = ЭлементМассива.НомерЛиста;
	//	НоваяСтрока.ВысотаДетали = ?(ГоризонтальныйПил, Элемент.ВысотаДетали, ЭлементМассива.ВысотаДетали);
