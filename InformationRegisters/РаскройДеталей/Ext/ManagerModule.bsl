...
// Предназначена для получения элементов сумма которых приближена к 
// МаксимальнаяСумма. Метод динамического программирования (табличный алгоритм).
// Источник:
...
// 
// Параметры
//  МассивЭлементов  - Массив - Массив распологаемых элементов (Отрезки)
//  МаксимальнаяСумма  - Число - Максимальное значение суммы (Длина трубы)
//  УвеличениеЭлемента  - Число - Добавляем к размеру каждого элемента (Толщина пила)
//
// Возвращаемое значение:
//   Структура   - Массив, Сумма, КоличествоЭлементов, МинимальныйОстаток, ОбщаяДлинаПила
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
		...
		Для Каждого Строка Из ТаблицаРазмещенияСтолбцов Цикл
			...
			Для каждого Столбец Из Строка.Пакет Цикл
				...
				//РазмещаемыйПакет.Сортировать("ИспользуетсяВОстаткахПакета Возр");
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, ПроверкаОстатков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
		...
		Для Каждого Элемент Из ТаблицаВсехДеталей Цикл
				...
				Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
					...
					Если ВысотаПакета + Деталь.ВысотаДетали + ТолщинаПила <= ВысотаЛиста
						И Деталь.ШиринаДетали <= ШиринаЛиста 
						 И Деталь.ШиринаДетали >= ШиринаПоследнейДетали / 100 * 40
						Тогда
			...
			Иначе // НЕ ПакетСобран
				...
				Если НЕ РазмещениеСтолбцов Тогда
					...
					ЛучшееРешение = ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода);
	...
	Если НЕ ОбходОставшихся Тогда  // НЕ ПроверкаОстатков И
...
Функция ПроверитьВозможностьИспользованияОстатков(ЛучшееРешение, СписокДеталейНоменклатуры, ТолщинаПила, Номенклатура, ВысотаЛиста, ИдеальныйПроцентОтхода)
