...
Функция УдалитьКолонкиНеНужныеДляАлгоритма(СписокДеталей)
	...
	НовыйСписокДеталей = СписокДеталей.Скопировать(,СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(МассивКолонок,","));
...
Функция СкопироватьДеталиСОтбором(СписокДеталей, Поле, Значение)

	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("тзДетали", СписокДеталей);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	*
	|ПОМЕСТИТЬ втДетали
	...
	Запрос.Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(Запрос.Текст, Поле, Значение);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей, СписокДеталей)
	...
	Для Каждого СтрокаДерева1 ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева2 Из СтрокаДерева1.Строки Цикл
			...
			|	ТаблицаДеталей.ПараметрыДляЭтикеток,
			|	ТаблицаДеталей.Сборка,
			|	ТаблицаДеталей.СхемаЯщика,
			|	ТаблицаДеталей.НомерДеталиИзделия,
			|	ТаблицаДеталей.КоличествоОдинаковых,
			...
			Пока Выборка.Следующий() Цикл
				...
				ТаблицаДеталейТекущейНоменклатуры = СписокДеталей.Скопировать(Новый Структура("Номенклатура", Выборка.Номенклатура));
