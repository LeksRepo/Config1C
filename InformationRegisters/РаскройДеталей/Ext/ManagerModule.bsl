...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение)
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
		...
		Если ОкруглятьДоЛистов Тогда
			СтруктураДляСтрокиРаскроя.Вставить("ПлощадьОбрезковПоНоменклатуре", ПлощадьЛиста * ВсегоЛистовДаннойНоменклатуры);
		Иначе
			СтруктураДляСтрокиРаскроя.Вставить("ПлощадьОбрезковПоНоменклатуре", ?((ПлощадьДеталейПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре) > ПлощадьЛиста, ПлощадьЛиста * ВсегоЛистовДаннойНоменклатуры, ПлощадьСДеталямиПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре));
		КонецЕсли;
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.ДлинаДетали, НоменклатураПодразделений.Номенклатура.ДлинаДетали) КАК ВысотаЛиста,
		|	ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.ШиринаДетали, НоменклатураПодразделений.Номенклатура.ШиринаДетали) КАК ШиринаЛиста
		|ИЗ
		|	РегистрСведений.НоменклатураПодразделений.СрезПоследних(, Подразделение = &Подразделение) КАК НоменклатураПодразделений
		|ГДЕ
		|	НоменклатураПодразделений.Номенклатура = &Номенклатура";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			ВысотаЛиста = ВыборкаДетальныеЗаписи.ВысотаЛиста;
			ШиринаЛиста = ВыборкаДетальныеЗаписи.ШиринаЛиста;
