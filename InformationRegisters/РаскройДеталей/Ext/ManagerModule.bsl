...
Функция ПолучитьСтрокуРаскроя(Спецификация) Экспорт
	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		Порядок = 1;
		МассивЛиний = СписокДеталей.НайтиСтроки(Новый Структура("Номенклатура, ШиринаДетали, Порядок", Элемент.Номенклатура, Элемент.ШиринаДетали, 100));
	...
	СписокДеталей.Сортировать("Номенклатура, Порядок, ШиринаДетали Убыв, ВысотаДетали Убыв");
	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		Если НЕ Элемент.Остаток Тогда
			...
			Если НоменклатураДетали <> Элемент.Номенклатура Тогда
				...
				МассивОстатков = СписокДеталей.НайтиСтроки(Новый Структура("Номенклатура, Остаток", НоменклатураДетали, Истина));
				Если МассивОстатков.Количество() = 0 Тогда
					НоваяСтрока = СписокДеталей.Добавить();
					НоваяСтрока.Номенклатура = НоменклатураДетали;
