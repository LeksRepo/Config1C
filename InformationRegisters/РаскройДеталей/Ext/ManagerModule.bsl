	...
	Для Каждого Элемент Из СписокДеталей Цикл
		...
		Если НЕ Элемент.СоблюдениеТекстуры И (Элемент.ВысотаДетали > Элемент.Номенклатура.ДлинаДетали ИЛИ Элемент.ШиринаДетали > Элемент.Номенклатура.ШиринаДетали) Тогда
			...
			Иначе
				...
				Возврат "Ошибка раскроя № 1. Обратитесь к программистам";
						...
						Для Каждого ЭлементМассива Из МассивОстатков Цикл
							...
							ИначеЕсли ((МассивОстатков.Найти(ЭлементМассива) + 1) = МассивОстатков.Количество()) Тогда
								...
								Иначе
									...
									Возврат "Ошибка раскроя № 2. Обратитесь к программистам";
...
Функция ФормированиеСпискаДеталей(СсылкаДокумента, ЭтоСпецификация, ЭтоНаряд)
	...
	Если ЭтоСпецификация Тогда
		...
		// { Васильев Александр Леонидович [29.08.2014]
		// Лёш, сделай красиво
		Для каждого Стр Из СписокДеталей Цикл
			Стр.СоблюдениеТекстуры = Стр.Номенклатура.НаличиеТекстуры;
		КонецЦикла;
		// } Васильев Александр Леонидович [29.08.2014]
