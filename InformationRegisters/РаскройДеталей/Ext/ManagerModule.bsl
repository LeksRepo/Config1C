...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение)
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
		...
		Если НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту Тогда
			...
			Запрос.Текст = 
			"ВЫБРАТЬ
			|	ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.КоэффициентБазовых, НоменклатураПодразделений.Номенклатура.ДлинаДетали * НоменклатураПодразделений.Номенклатура.ШиринаДетали) КАК ПлощадьЛиста,
			|	ВЫБОР
			...
			ПлощадьСДеталямиПоследнегоЛиста = ПлощадьДеталейПоследнегоЛиста + ?(СписокНомеров.Количество() > 0, (СписокНомеров.Количество() - 1) * ПлощадьЛиста, 0);
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= МаксимальныйПроверяемыйПроцент Цикл
				...
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ,ПоперечныйПил, ПределыОстатков, Подразделение);
				...
				//	СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПоперечныйПил, ПределыОстатков, Подразделение);
				//	
				//	Если СтруктураДляСтрокиРаскроя.Свойство("КоличествоЛистов") Тогда
				//		
