...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено) Экспорт
	...
	Если ТипЗнч(ДокОбъект) = Тип("ДокументСсылка.НарядЗадание") Тогда
		...
		Если ДокОбъект.СписокСпецификаций.Количество() > 0 Тогда
			Для каждого Строка Из ДокОбъект.СписокСпецификаций Цикл
				ТаблицаДеталей =  ФормированиеСпискаДеталей(Строка.Спецификация);
	...
	//Иначе
		...
		СтрокаРаскрой = ПолучитьСтрокуДляФлэшИзДерева(Структура.Дерево, ДокОбъект, НарядЗадание);
...
Функция ПолучитьСтрокуДляФлэшИзДерева(Дерево, СсылкаДокумента, НарядЗадание)
	...
	Если Дерево.Строки.Количество() > 0 Тогда
		...
		ДанныеДляРаскроя = ФормированиеСтрокиРаскрояПоРанцу(ДанныеДляРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание)
