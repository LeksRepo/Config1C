...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	Если ИспользоватьСтарыйАлгоритм Тогда
		Структура = РасположитьНаЛистах(СсылкаДокумента, СписокДеталей);
		ДанныеДляРаскроя = ПолучитьСтрокуРаскрояПоВсемНоменклатурам(Структура.МассивИтоговогоРасположенияДеталей, СсылкаДокумента);
	Иначе
		Структура = РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей);
	...
	// Я чего-то не понимаю, разве может вернуться не структура, или снова старый рудимент? :)
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ПределыОстатков, ИдеальныйПроцентОтхода, ПоворотДеталей = Ложь)
	...
	Для каждого Пакет Из ТаблицаПакетов Цикл
		...
		НоваяСтрока.ШиринаДетали = ШиринаЛиста; //Для алгоритма размещения столбцов на пофигу на их ширину
		НоваяСтрока.GUID = Строка(Пакет.НомерПакета);
		
	КонецЦикла;
	
	ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, 0, , , , Истина);
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, ПроверкаОбрезков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
	...
	// { Васильев Александр Леонидович [22.07.2015]
	// Зачем эта строка? Ниже ведь сразу очищаем.
			...
			Если НЕ ПакетСобран Тогда
				...
				Для каждого Деталь Из ТаблицаОставшихсяДеталей Цикл
					...
					Если ВысотаПакета + Деталь.ВысотаДетали + ТолщинаПила <= ВысотаЛиста
						...
						// И Деталь.ШиринаДетали >= ШиринаПоследнейДетали / 2
						Тогда
		...
		Если ТаблицаОставшихсяДеталей.Количество() > 0
			...
			Пока ВысотаПакета > ВысотаЛиста Цикл
				...
				Если ВысотаПакета <> СтараяВысотаПакета Или СтараяВысотаПакета = 0 Тогда
					...
					// { Васильев Александр Леонидович [22.07.2015]
					// Интересно посчитать глубину рекурсии... сколько их бывает.
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Для ИдеальныйПроцентОтхода = 18 По 18 Цикл
			...
			Если СтруктураДляСтрокиРаскроя.КоличествоЛистов < НаименьшееКоличествоЛистов
				ИЛИ СтруктураДляСтрокиРаскроя.ПлощадьОстатковПоНоменклатуре > НаибольшаяПлощадьОстатков
			...
			Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
				...
				Если ТипЗнч(СтруктураДляСтрокиРаскроя) = Тип("Структура") Тогда
					...
					Если СтруктураДляСтрокиРаскрояПовернутыхДеталей.КоличествоЛистов < НаименьшееКоличествоЛистов
						ИЛИ СтруктураДляСтрокиРаскрояПовернутыхДеталей.ПлощадьОстатковПоНоменклатуре > НаибольшаяПлощадьОстатков
