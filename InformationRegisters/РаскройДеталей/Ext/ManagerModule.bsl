...
Функция СформироватьСтруктуруДляСтрокиРаскроя(Номенклатура, СписокДеталейНоменклатуры, ТолщинаПила, ПределыОстатков, НадписьНаОстатке, ПоворотДеталей = Ложь, Подразделение, БезОкругленияДоЛистов, ВысотаЛиста, МассивНомеровОстатков)
	...
	Иначе
		...
		Иначе
			МассивОстатковОтбор = СписокДеталейНоменклатуры.НайтиСтроки(Новый Структура("Остаток", Истина));
			ПлощадьОстатковДеталей = 0;
			Для каждого Деталь Из МассивОстатковОтбор Цикл
				ПлощадьОстатковДеталей = ПлощадьОстатковДеталей	+ Деталь.ВысотаДетали * Деталь.ШиринаДетали;
			КонецЦикла;
			ПлощадьОбрезковПоНоменклатуре = ПростоПлощадьВсехЛистов - ПлощадьОстатковДеталей*(100-Номенклатура.ПроцентОтхода)/100;         //т.е.то за что он платит, детали +обрезки + Процент отходов от остатков
		КонецЕсли;
