...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено, ПоказатьВесьСписокДеталей = Ложь) Экспорт
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Буква = "", НарядЗадание = Ложь)
	...
	Если НарядЗадание и фнДокОбъект.Дилерский Тогда
		Для каждого Строка Из фнДокОбъект.СписокМатериаловПодЗаказ Цикл
			Если Строка.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = Перечисления.ВидыМатериалов.Листовой Тогда
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не НарядЗадание и СсылкаДокумента.Дилерский и ИспользуетсяРаскройНаНаряд Тогда
				...
				Если МатериалПодЗаказ и ПроверятьНеобходимостьУдаленияМатериала и НЕ ПоказатьВесьСписокДеталей Тогда
					Продолжить;
				КонецЕсли;
