...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено) Экспорт
	...
	Если СписокДеталей.Колонки.Найти("НомерПакетаРамок") = Неопределено Тогда
		
		СписокДеталей.Колонки.Добавить("НомерПакетаРамок", Новый ОписаниеТипов("Число"));
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков)
	...
	Если ЕстьРамки Тогда
		...
		Для каждого Строка Из МассивРамок Цикл
			...
			Если НомерПакетаРамок = Строка.НомерПакетаРамок Тогда
				
				ВысотаДетали = ВысотаДетали + Строка.ВысотаДетали;
				СтрокаДляПоиска = СписокДеталейНоменклатуры.Найти(Строка.GUID, "GUID");
				СписокДеталейНоменклатуры.Удалить(СтрокаДляПоиска);
				
			Иначе
				
				НоваяСтрока.ВысотаДетали = ВысотаДетали + ТолщинаПила * 3;
	...
	//	Отбор = Новый Структура("СборныйРамочныйФасад", Истина);
	//	Отбор.Вставить("КромкаДляСборнойРамки", Справочники.Номенклатура.ПустаяСсылка());
	//	МассивТестированнойТаблицы = СписокДеталейНоменклатуры.НайтиСтроки(Отбор);
	//	
	//	Для каждого Строка Из МассивТестированнойТаблицы Цикл
	//		
	//		//Абсцисса = Строка.КоординатаХ;
	//		//Ордината = Строка.КоординатаУ;
	//		//Счетчик = 0;
	//		//Отбор = Новый Структура("СборныйРамочныйФасад", Истина);
	//		//Отбор.Вставить("КромкаДляСборнойРамки", Справочники.Номенклатура.ПустаяСсылка());
	//		//Отбор.Вставить("НомерПакетаРамок", Строка.НомерПакетаРамок);
	//		//МассивРамок = ОригинальныйСписокДеталей.НайтиСтроки(Отбор);
	//		
	//		//Для Каждого Рамка Из МассивРамок Цикл
	//		//	Рамка.КоординатаХ = ?(ПоперечныйПил, Строка.КоординатаУ, Строка.КоординатаХ);
	//		//	Рамка.КоординатаУ = ?(ПоперечныйПил, Строка.КоординатаХ, Строка.КоординатаУ);
	//		//	Если Счетчик > 0 Тогда
	//		//		Рамка.КоординатаХ = ?(ПоперечныйПил, Ордината, Абсцисса);
	//		//		Рамка.КоординатаУ = ?(ПоперечныйПил, Абсцисса, Ордината);
	//		//	КонецЕсли;
	//		//	Абсцисса = Строка.КоординатаХ;
	//		//	Ордината = Ордината + Рамка.ВысотаДетали + ТолщинаПила;
	//		//	Счетчик = Счетчик+ 1;
	//		//КонецЦикла;
	//		
	//		Для каждого Пакет Из ТаблицаПакетов Цикл
	//		
	//			Таблица = Пакет.Пакет;
	//			ЗаменяемаяДеталь = Таблица.Найти(Строка.GUID, "GUID");
	//			
	//			Если ЗаменяемаяДеталь <> Неопределено Тогда
	//				Индекс = Таблица.Индекс(ЗаменяемаяДеталь);
	//				Абсцисса = ?(ПоперечныйПил, ЗаменяемаяДеталь.КоординатаУ, ЗаменяемаяДеталь.КоординатаХ);
	//				Ордината = ?(ПоперечныйПил, ЗаменяемаяДеталь.КоординатаХ, ЗаменяемаяДеталь.КоординатаУ);
	//				Отбор = Новый Структура("СборныйРамочныйФасад", Истина);
	//				Отбор.Вставить("КромкаДляСборнойРамки", Справочники.Номенклатура.ПустаяСсылка());
	//				Отбор.Вставить("НомерПакетаРамок", Строка.НомерПакетаРамок);
	//				МассивРамок = ОригинальныйСписокДеталей.НайтиСтроки(Отбор);
	//				
	//				//МассивРамок =ОригинальныйСписокДеталей.НайтиСтроки(Новый Структура("НомерПакетаРамок", Строка.НомерПакетаРамок));
	//				Для Каждого Деталь Из МассивРамок Цикл
	//					
	//					НоваяСтрока = Таблица.Вставить(Индекс);
	//					
	//					НоваяСтрока.КоординатаХ = ?(ПоперечныйПил, Ордината, Абсцисса);
	//					НоваяСтрока.КоординатаУ = ?(ПоперечныйПил, Абсцисса, Ордината);
	//					НоваяСтрока.GUID = Деталь.GUID;
	//					НоваяСтрока.ДетальРазмещена = ЗаменяемаяДеталь.ДетальРазмещена;
	//					НоваяСтрока.ИспользуетсяВОстаткахПакета = ЗаменяемаяДеталь.ИспользуетсяВОстаткахПакета;
	//					Абсцисса = ?(ПоперечныйПил, ЗаменяемаяДеталь.КоординатаУ, ЗаменяемаяДеталь.КоординатаХ);
	//					Ордината =?(ПоперечныйПил, Ордината + Деталь.ВысотаДетали + ТолщинаПила, Ордината + Деталь.ВысотаДетали + ТолщинаПила);
	//					Индекс = Индекс+ 1;
	//					
	//				КонецЦикла;
	//				
	//				Таблица.Удалить(ЗаменяемаяДеталь);
	//				
	//			КонецЕсли;
	//			
	//		КонецЦикла;
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда
					...
					Для Каждого Деталь Из РазмещаемыйПакет Цикл
						...
						Если НЕ Деталь.ДетальРазмещена И (НЕ ПроверятьНомерЛиста или (ПроверятьНомерЛиста и ПроверочныйНомерЛиста <> Число(СписокНомеров[0].Представление) И ПроверочныйНомерЛиста <> Число(СписокНомеров[1].Представление))) Тогда
							...
							Если РазмерПредыдущейДетали = 0 Тогда
								...
								РазмерПредыдущейДетали = РазмещаемаяДеталь.ШиринаДетали;
		...
		//Для вычисления площади, которую потом надо добавить к деталям, если считаем по последнему листу детали.
		Если НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту Тогда
			...
			ПлощадьДеталейПоследнегоЛиста = ПлощадьДеталейПоследнегоЛиста + ПлощадьДеталейПоследнегоЛиста / 100 * Номенклатура.ПроцентОтхода;
			ПлощадьДеталейПоследнегоЛиста = ?(ПлощадьДеталейПоследнегоЛиста > ПлощадьЛиста, ПлощадьЛиста, ПлощадьДеталейПоследнегоЛиста);
			ПлощадьСДеталямиПоследнегоЛиста = ПлощадьДеталейПоследнегоЛиста + ?(СписокНомеров.Количество() > 0, (СписокНомеров.Количество() - 1) * Номенклатура.ДлинаДетали * Номенклатура.ШиринаДетали, 0);
			
		КонецЕсли;
		
		СтруктураДляСтрокиРаскроя.Вставить("ПлощадьОбрезковПоНоменклатуре", ?(НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту, ПлощадьСДеталямиПоследнегоЛиста - ПлощадьВсехДеталей, СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре));
...
Процедура РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, Элемент, ТолщинаПила, КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, МаленькийОстатокПоШирине, ИспользуетсяВОстатках, ПоперечныйПил = Ложь)
	...
	Если ИспользуетсяВОстатках Тогда
		ВысотаДетальки = ?(ПоперечныйПил, ЗаменяемыйОстаток.ШиринаДетали - Элемент.ШиринаДетали - ТолщинаПила, ЗаменяемыйОстаток.ВысотаДетали);
