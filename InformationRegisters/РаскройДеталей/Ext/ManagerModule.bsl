...
Функция СформироватьРаскрой(ДокОбъект, СписокДеталей = Неопределено, ПоказатьВесьСписокДеталей = Ложь, Наряды = Ложь) Экспорт
	...
	Если ТипЗнч(ДокОбъект) = Тип("ДокументСсылка.НарядЗадание") И СписокДеталей = Неопределено Тогда
		...
		Если ДокОбъект.СписокСпецификаций.Количество() > 0 Тогда
			...
			Для каждого Строка Из ДокОбъект.СписокСпецификаций Цикл
				...
				ТаблицаДеталей =  ФормированиеСпискаДеталей(Строка.Спецификация, Буква, НарядЗадание);
	...
	КонецЕсли;
		...
		Если СписокДеталей = Неопределено или НарядЗадание Тогда
			...
			СписокДеталей = ?(НарядЗадание, СписокДеталей, ФормированиеСпискаДеталей(ДокОбъект));
		...
		Структура = РасположитьНаЛистахМетодомРанца(ДокОбъект, СписокДеталей, НарядЗадание);
		...
		СтрокаРаскрой = ПолучитьСтрокуДляФлэшИзДерева(Структура.Дерево, ДокОбъект, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей);
...
Функция ФормированиеСпискаДеталей(фнДокОбъект, Буква = "", НарядЗадание = Ложь)
	...
	Если НарядЗадание Тогда//  и фнДокОбъект.Дилерский
...
Функция ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, КонечныйНомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, НадписьНаОстатке, ЕстьШаг = Ложь)
	...
	Для ТекущийНомерЛиста = КонечныйНомерЛиста + 1 По НомерЛиста Цикл
		...
		Для Каждого ЭлементМассиваОстатков Из МассивОстатков Цикл
			...
			Если ПолезныйОстаток(ПределыОстатков, ЭлементМассиваОстатков.ВысотаДетали, ЭлементМассиваОстатков.ШиринаДетали) Тогда
				...
				ЭлементМассиваОстатков.НаименованиеДетали = НадписьНаОстатке;
				
			ИначеЕсли ТекущийНомерЛиста = НомерЛиста Тогда
...
Функция ФормированиеПакета(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ПоворотДеталей = Ложь, ПоперечныйПил = Ложь, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОбрезков)
	...
	Если ЕстьРамки Тогда
		...
		Для каждого Строка Из МассивРамок Цикл
			...
			//НоваяСтрока.НомерПакетаРамок = НоваяСтрока.НомерПакетаРамок - 1;
			...
			Иначе
				...
				НоваяСтрока.ВысотаДетали = ВысотаЛиста;  //ВысотаДетали
				ВысотаДетали = Строка.ВысотаДетали;
				//НомерПакетаРамок = Строка.НомерПакетаРамок;
				НоваяСтрока = СписокДеталейНоменклатуры.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, Строка);
				НоваяСтрока.МожноПовернуть = Истина;
				//НоваяСтрока.НомерПакетаРамок = НоваяСтрока.НомерПакетаРамок - 1;
		...
		НоваяСтрока.ВысотаДетали = ВысотаЛиста;  //ВысотаДетали
		//НоваяСтрока.НомерПакетаРамок = НомерПакетаРамок;
	Иначе
	...
	КонецЕсли;
	//////////////////Раскрой на куски////////////
	Для каждого Остаток Из МассивОбрезков Цикл
	...
	Если ПоперечныйПил Тогда
		...
		ТаблицаПакетов = АлгоритмОРанце(СписокДеталейДляПоперечногоПила, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, МассивОбрезков);
		
	Иначе
		
		//СписокДеталейНоменклатуры.Колонки.Добавить("ИспользуетсяВОстаткахПакета", Новый ОписаниеТипов("Булево"));
		ТаблицаПакетов = АлгоритмОРанце(СписокДеталейНоменклатуры, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, МассивОбрезков);
	...
	Если ПоперечныйПил Тогда
		...
		ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, Номенклатура, 0, 0, МассивОбрезков, , , , Истина);
		
	Иначе
		
		ТаблицаРазмещенияСтолбцов = АлгоритмОРанце(ТаблицаСтолбцов, ШиринаЛиста, ВысотаЛиста, ТолщинаПила, Номенклатура, 0, 0, МассивОбрезков, , , , Истина);
	...
	Если ТаблицаРазмещенияСтолбцов.Количество() > 0 Тогда  //    НарядЗадание или 
				...
				Для Каждого Деталь Из ТаблицаРазвернутыхДеталей Цикл
						...
						РазмещаемаяДеталь = СписокДеталейНоменклатуры.Найти(Деталь.GUID, "GUID");
							...
							Для каждого ЭлементМассива Из МассивОстатков Цикл
								...
								Если ПоперечныйПил Тогда
									ОстатокПоВысоте = ЭлементМассива.ШиринаДетали - Деталь.ВысотаДетали;
									ОстатокПоШирине = ЭлементМассива.ВысотаДетали - Деталь.ШиринаДетали;
								Иначе
								...
								Если ОстатокПоВысотеПоворот >= 0 И ОстатокПоШиринеПоворот >= 0 Тогда
									...
									Если ПоперечныйПил Тогда
										РазмещаемаяДеталь.ПоворотДетали = НЕ РазмещаемаяДеталь.ПоворотДетали;
				...
				Если НЕ ПробуемРаспихатьДеталиВОстатки Тогда
					...
					Если СписокДеталейРазмещенныхНаОстатках.Количество() > 0 Тогда
						...
						Для каждого ДетальВОстатках Из СписокДеталейРазмещенныхНаОстатках Цикл
							...
							//СтарыйНомерСтолбца = ДетальВОстатках.КоординатаХ;
							...
							Если РазмерПредыдущейДетали = 0 Тогда
								...
								ДобавитьБольшойОстаток = СтараяКоордината <> ДетальВОстатках.КоординатаХ;
								...
								Если НЕ ПоперечныйПил Тогда
									...
									//КоординатаХ = ?(ДетальВОстатках.КоординатаХ > 0, ДетальВОстатках.КоординатаУ + ДетальВОстатках.КоординатаХ, ДетальВОстатках.КоординатаУ);
									КоординатаХ = ДетальВОстатках.КоординатаХ;
									РазместитьДетальАлгоритмОРанце(СписокДеталейНоменклатуры, РазмещаемаяДеталь, ТолщинаПила, ДетальВОстатках.КоординатаУ, КоординатаХ, НомерЛиста, ВысотаЛиста, ШиринаЛиста, ДобавитьБольшойОстаток, ОстатокСлева, ИспользуетсяВОстатках); //ОстатокПоВысоте, ОстатокПоШирине,
									
								Иначе
									
									ОстатокСверху = РазмерПредыдущейДетали - ДетальВОстатках.ВысотаДетали -ТолщинаПила;
									КоординатаХ = ?(ДетальВОстатках.КоординатаУ > 0, ДетальВОстатках.КоординатаХ + ДетальВОстатках.КоординатаУ, ДетальВОстатках.КоординатаХ);
									КоординатаУ = ?(ДетальВОстатках.КоординатаХ > 0, ДетальВОстатках.КоординатаУ + ДетальВОстатках.КоординатаХ, ДетальВОстатках.КоординатаУ);
								...
								СтараяКоордината = ДетальВОстатках.КоординатаХ;
		...
		Если СписокНомеров.Количество() > 0 Тогда   //  НарядЗадание или
		...
		СтруктураОптимальногоОстатка = ОптимизацияОстатков(СписокДеталейНоменклатуры, МаксимальныйНомерЛиста , 0, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, НадписьНаОстатке, Истина);
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ЕСТЬNULL(НоменклатураПодразделений.ОсновнаяПоСкладу.КоэффициентБазовых, ВЫБОР
		...
		|	ВЫБОР
		|		КОГДА НоменклатураПодразделений.ОсновнаяПоСкладу.КоэффициентБазовых ЕСТЬ NULL 
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ИСТИНА
		|	КОНЕЦ КАК Умножать,
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ОкруглятьДоЛистов = ?(БезОкругленияДоЛистов, Ложь, ВыборкаДетальныеЗаписи.ОкруглятьДоЛистов);
			Если ВыборкаДетальныеЗаписи.Умножать Тогда
				Если НЕ КоличествоВПогонныхМетрах Тогда
					ПлощадьЛиста = ?(ВыборкаДетальныеЗаписи.ПлощадьЛиста > 0, ВыборкаДетальныеЗаписи.ПлощадьЛиста * 1000000, Номенклатура.ДлинаДетали * Номенклатура.ШиринаДетали);
				Иначе
					ПлощадьЛиста = ?(ВыборкаДетальныеЗаписи.ПлощадьЛиста > 0, ВыборкаДетальныеЗаписи.ПлощадьЛиста * 1000, Номенклатура.ДлинаДетали);
				КонецЕсли;
			Иначе
				ПлощадьЛиста = ВыборкаДетальныеЗаписи.ПлощадьЛиста;
			КонецЕсли;
		КонецЦикла;
		...
		Если ОкруглятьДоЛистов Тогда
			...
			ПлощадьОбрезковПоНоменклатуре = ПлощадьЛиста * ВсегоЛистовДаннойНоменклатуры;
		...
		Иначе
			...
			Иначе
				ПлощадьВсегоНаПоследнемЛисте = ?((ПлощадьДеталейПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре + 
				+ (ПлощадьДеталейПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре) / 100 * Номенклатура.ПроцентОтхода) < ПлощадьЛиста,
				ПлощадьДеталейПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре + 
				+ (ПлощадьДеталейПоследнегоЛиста + СтруктураОптимальногоОстатка.ПлощадьОбрезковПоНоменклатуре) / 100 * Номенклатура.ПроцентОтхода, ПлощадьЛиста);
				
				ПлощадьОбрезковПоНоменклатуре = ПлощадьВсегоНаПоследнемЛисте + ?(СписокНомеров.Количество() > 0, (СписокНомеров.Количество() - 1) * ПлощадьЛиста, 0);
			КонецЕсли;
		КонецЕсли;
...
Функция АлгоритмОРанце(Знач СписокДеталейНоменклатуры, Знач ВысотаЛиста, Знач ШиринаЛиста, ТолщинаПила, Номенклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, МассивОстатков, ПроверкаОстатков = Ложь, СтараяВысотаПакета = 0, ОбходОставшихся = Ложь, РазмещениеСтолбцов = Ложь)
...
Функция РасположитьНаЛистахМетодомРанца(СсылкаДокумента, СписокДеталей, НарядЗадание)
	...
	Для Каждого ЭлементСписка Из СписокНоменклатуры Цикл // цикл по материалам
		...
		Пока ИдеальныйПроцентОтхода <= 0 Цикл
			...
			Пока РазрешенныйПроцентПоШирине <= МаксимальныйПроверяемыйПроцент Цикл
				...
				СтруктураДляСтрокиРаскроя = ФормированиеПакета(СписокДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, ,ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОстатков);
				...
				Если НЕ ТекущаяНоменклатура.НаличиеТекстуры Тогда
					...
					СтруктураДляСтрокиРаскрояПовернутыхДеталей = ФормированиеПакета(СписокПовернутыхДеталейДляФормированияПакетов, ВысотаЛиста, ШиринаЛиста, ТолщинаПила, ТекущаяНоменклатура, ИдеальныйПроцентОтхода, РазрешенныйПроцентПоШирине, Истина, ПоперечныйПил, ПределыОстатков, Подразделение, БезОкругленияДоЛистов, НарядЗадание, МассивОстатков);
...
Процедура РазместитьДеталь(СписокДеталейНоменклатуры, Элемент, ЭлементМассива, ОстатокПоВысоте, ОстатокПоШирине, ТолщинаПила, ПоворотДетали = Ложь, ПоперечныйПил = Ложь)
	...
	Если ПоперечныйПил Тогда
		...
		Если ОстатокПоВысоте > 0 Тогда
			...
			НоваяСтрока.ШиринаДетали = ?(ПоперечныйПил, ЭлементМассива.ШиринаДетали, Элемент.ШиринаДетали);
		...
		Если ОстатокПоШирине > 0 Тогда
			...
			НоваяСтрока.Количество = 1;
			НоваяСтрока.НомерЛиста = ЭлементМассива.НомерЛиста;
			НоваяСтрока.ВысотаДетали = ?(ПоперечныйПил, Элемент.ВысотаДетали, ЭлементМассива.ВысотаДетали);
...
Функция ПолучитьСтрокуДляФлэшИзДерева(Дерево, СсылкаДокумента, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей)
	...
	Если Дерево.Строки.Количество() > 0 Тогда
		...
		ДанныеДляРаскроя = ФормированиеСтрокиРаскрояПоРанцу(ДанныеДляРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей);
...
Функция ФормированиеСтрокиРаскрояПоРанцу(СтрокаРаскроя, Дерево, СсылкаДокумента, Подразделение, НарядЗадание, МассивБуквеннойНумерации, ПоказатьВесьСписокДеталей)
	...
	Для Каждого СтрокаДерева ИЗ Дерево.Строки[0].Строки Цикл
		...
		Для каждого СтрокаДерева Из СтрокаДерева.Строки Цикл
			...
			//ТаблицаДеталей.Колонки.Добавить("НомерПакетаРамок", Новый ОписаниеТипов("Число"));
			...
			//МатериалЗаказчика = 
			Если НЕ НарядЗадание Тогда
				ЕстьПодЗаказ = СсылкаДокумента.СписокМатериаловПодЗаказ.Количество() > 0;
			КонецЕсли;
			...
			МатериалыЗаказчика = СписокМатериаловЗаказчика;
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Не НарядЗадание и ИспользуетсяРаскройНаНаряд Тогда//  и СсылкаДокумента.Дилерский 
				...
				Если НЕ Неноменклатурный и ПроверятьНеобходимостьУдаленияМатериала и НЕ ПоказатьВесьСписокДеталей Тогда   // НЕ МатериалПодЗаказ и 
					Продолжить;
				КонецЕсли;
				
				ШапкаЛиста = Новый Массив;
				
				ШапкаЛиста.Добавить(?(МатериалПодЗаказ, Строка(Выборка.Номенклатура) + " - <b>ПОД ЗАКАЗ " + Комментарий + "</b>" , Выборка.Номенклатура)); //0
