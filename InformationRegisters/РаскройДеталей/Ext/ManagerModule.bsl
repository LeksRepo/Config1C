...
Функция ПолучитьСтрокуРаскроя(СсылкаДокумента, СписокДеталей = Неопределено) Экспорт
	...
	Если СписокДеталей = Неопределено Тогда
		...
		СтрокаКривогоПила = ФормированиеСтрокиКривогоПила(СписокДеталей, СсылкаДокумента.Номер);
		
	КонецЕсли;
	
	Если ИспользоватьСтарыйАлгоритм Тогда
		Структура = РасположитьНаЛистах(СсылкаДокумента, СписокДеталей);
		//СтрокаРаскрой = ПолучитьСтрокуРаскрояПоВсемНоменклатурам(Структура.МассивИтоговогоРасположенияДеталей, СсылкаДокумента);
		СтрокаРаскрой = ПолучитьСтрокуДляФлэшИзТаблицы(Структура.МассивИтоговогоРасположенияДеталей, СсылкаДокумента);
	Иначе
		...
		СтрокаРаскрой = ПолучитьСтрокуДляФлэшИзДерева(Структура.Дерево, СсылкаДокумента);
	КонецЕсли;
