...
Функция ОптимизацияОстатков(СписокДеталейНоменклатуры, НомерЛиста, КонечныйНомерЛиста, ТолщинаПила, ПределыОстатков, НоменклатурнаяГруппа, НадписьНаОстатке, ЕстьШаг = Ложь)
	...
	Для ТекущийНомерЛиста = КонечныйНомерЛиста + 1 По НомерЛиста Цикл
		...
		СтруктураПроверкиДеталей =  ПроверкаДеталейКонтрагентов(ТекущийНомерЛиста,  ТаблицаДеталейКонтрагентов);
...
Функция ПроверкаДеталейКонтрагентов(ТекущийНомерЛиста, ТаблицаНаименованийДеталей)
	...
	ТаблицаБуквы.Колонки.Добавить("БукваДетали", Новый ОписаниеТипов("Строка"));
	
	Для каждого Стр  Из ТаблицаБуквы Цикл
		Стр.БукваДетали = Лев(Стр.НаименованиеДетали,1);
	КонецЦикла;
	ТаблицаБуквы.Свернуть("БукваДетали");
	Если ТаблицаБуквы.Количество() = 1 Тогда 
		СтруктураПроверкиДеталей.Вставить("ДополнениеНазванияОстатка","/Дилер");
	КонецЕсли;
	//
