...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если НоваяЗапись.Рассмотрено <> СтараяЗапись.Рассмотрено И (НЕ (НоваяЗапись.Рассмотрено И СтараяЗапись.Рассмотрено = Неопределено)) Тогда
	...
	Если НоваяЗапись.Папка <> СтараяЗапись.Папка И (НЕ НоваяЗапись.Рассмотрено) Тогда
		
		Взаимодействия.РассчитатьРассмотреноПоПапкам(Взаимодействия.ТаблицаДанныхДляРасчетаРассмотрено(ДанныеДляРасчета, "Папка"));
		
	КонецЕсли;
	
	Если НоваяЗапись.Предмет <> СтараяЗапись.Предмет И (НЕ НоваяЗапись.Рассмотрено) Тогда
