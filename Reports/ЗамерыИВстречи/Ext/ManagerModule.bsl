Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	|		КОНЕЦ
	|	И Замер.ДатаЗамера МЕЖДУ &ДатаНачала И &ДатаОкончания
	...
	|ВЫБРАТЬ
	|	Спецификация.Ссылка,
	|	Спецификация.ДатаМонтажа,
	|	Спецификация.ДокументОснование.Замерщик,
	|	Спецификация.ДокументОснование.АдресЗамера,
	|	""М""
	|ИЗ
	...
	|			КОГДА &Подразделение = ЗНАЧЕНИЕ(Справочник.Подразделения.ПустаяСсылка)
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Спецификация.ДокументОснование.Подразделение = &Подразделение
	|		КОНЕЦ
	|	И Спецификация.ДатаМонтажа МЕЖДУ &ДатаНачала И &ДатаОкончания
