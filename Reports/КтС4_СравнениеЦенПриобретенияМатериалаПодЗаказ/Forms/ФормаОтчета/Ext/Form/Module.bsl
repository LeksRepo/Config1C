...
Функция СформироватьРасшифровку(ИмяПоля, Номенклатура, Период, Регион)
	...
	ИначеЕсли ИмяПоля = "КОЛИЧЕСТВОФАКТ" ИЛИ ИмяПоля = "СУММАФАКТ" Тогда
		...
		Если Номенклатура.Неноменклатурный Тогда
			...
			|ПОМЕСТИТЬ втНоменклатураНеном
			|ИЗ
			|	Документ.ПоступлениеМатериаловУслуг.Материалы КАК Поступление
			|ГДЕ
			|	Поступление.Ссылка.Проведен
			|	И Поступление.Ссылка.Подразделение = &Подразделение
			|	И Поступление.Номенклатура.Неноменклатурный
			|	И Поступление.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
			|	И (Поступление.Номенклатура = &Номенклатура
			|			ИЛИ Поступление.Номенклатура.БазоваяНоменклатура = &Номенклатура)
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
			...
			|	И Авансовый.Ссылка.Подразделение = &Подразделение
			|	И Авансовый.Номенклатура.Неноменклатурный
			|	И Авансовый.Ссылка.Дата МЕЖДУ &НачалоПериода И &КонецПериода
			|	И (Авансовый.Номенклатура = &Номенклатура
			|			ИЛИ Авансовый.Номенклатура.БазоваяНоменклатура = &Номенклатура)
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			|	Ном.Документ КАК Документ,
			|	СУММА(ВЫБОР
			|			КОГДА Ном.Номенклатура.Базовый
			|				ТОГДА Ном.Количество
			|			ИНАЧЕ Ном.Количество * Ном.Номенклатура.КоэффициентБазовых
			|		КОНЕЦ) КАК Количество,
			|	СУММА(Ном.Сумма) КАК Стоимость
			|ИЗ
			|	втНоменклатураНеном КАК Ном
			|
			|СГРУППИРОВАТЬ ПО
			|	Ном.Документ
			|
