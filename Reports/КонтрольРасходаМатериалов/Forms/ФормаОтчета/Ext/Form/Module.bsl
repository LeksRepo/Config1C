...
Функция СформироватьРасшифровку(ИмяПоля, Номенклатура, Период, Регион)
	...
	Если ЭтаФорма.КлючТекущегоВарианта = "ВПлановыхЦенах" Тогда			
		Запрос.УстановитьПараметр("ПлановаяЗакупочная", Истина);	
	Иначе
		Запрос.УстановитьПараметр("ПлановаяЗакупочная", Ложь);	
	КонецЕсли;
