...
Функция СформироватьОтчетЭкономика(НачалоПериода, КонецПериода, СписокПодразделений) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		СуммаПродажи = СуммаПродажи + ВСумма;
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		ОбластьСтрокаКолонкаСиняя.Параметры.Расшифровка = Новый Структура("Раздел, ВидРасшифровки, Период", "Продажи", "ДР", ВыборкаПериод.Период);
		ТабДок.Присоединить(ОбластьСтрокаКолонкаСиняя);
...
Функция ЗаполнитьТаблицу(НачалоПериода, КонецПериода, СписокПодразделений, ВидОтчета, ТЗСтатьи)
	...
	Если ВидОтчета = "Касса" Тогда
		...
		ИтогПоТаблице = СтрокаИтоги.Итог;
		ОстаткиПоКассе = ПолучитьОстаткиПоКассе(НачалоПериода, КонецПериода, СписокПодразделений, Счет);
		Отклонение = 0;
		
		Если Окр(ОстаткиПоКассе.ОстатокНаНачалоПериода + ИтогПоТаблице) <> Окр(ОстаткиПоКассе.ОстатокНаКонецПериода) Тогда
			Отклонение = Окр(ОстаткиПоКассе.ОстатокНаНачалоПериода + ИтогПоТаблице) - Окр(ОстаткиПоКассе.ОстатокНаКонецПериода);
			Отклонение = Макс(Отклонение,-Отклонение);
		КонецЕсли;
		
		СтрокаОтклонение = ТЗ.Добавить();
		СтрокаОтклонение.Период = "Отклонение";
		СтрокаОтклонение.Итог = Формат(Отклонение, "ЧДЦ="); 
...
Функция ПолучитьОстаткиПоКассе(НачалоПериода, КонецПериода, СписокПодразделений, Счет)
