...
Функция СформироватьОтчет(НачалоПериода, КонецПериода, СписокПодразделений, ВидОтчета) Экспорт
	...
	Таблица = ЗаполнитьТаблицуДанных(НачалоПериода, КонецПериода, СписокПодразделений, ВидОтчета, ТЗСтатьи);
...
Функция ЗаполнитьТаблицуДанных(НачалоПериода, КонецПериода, СписокПодразделений, ВидОтчета, тзГруппыСтатей)
	...
	ЗаполнитьПродажи(ПараметрыОтчета);
...
Функция ЗаполнитьПродажи(ПараметрыОтчета)
	...
	"ВЫБРАТЬ
	|	УправленческийОбороты.Период КАК Период,
	|	УправленческийОбороты.ЗначениеОборот КАК Сумма
	|ИЗ
	|	РегистрНакопления.Продажи.Обороты(&НачалоПериода, КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ), День, Подразделение В (&СписокПодразделений)) КАК УправленческийОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период
	|ИТОГИ
