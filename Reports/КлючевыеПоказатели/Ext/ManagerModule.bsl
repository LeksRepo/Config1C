...
Функция СформироватьОтчетЭкономика(НачалоПериода, КонецПериода, СписокПодразделений) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ВЫРАЗИТЬ(УправленческийОбороты.Субконто1 КАК Справочник.СтатьиДоходовРасходов).НомерКолонки КАК НомерКолонки,
	|	ВЫРАЗИТЬ(УправленческийОбороты.Субконто1 КАК Справочник.СтатьиДоходовРасходов).РазделАналитики КАК Раздел,
	...
	|ИЗ
	|	РегистрБухгалтерии.Управленческий.Обороты(&НачалоПериода, КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ), , Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПрибыльУбытки)), , Подразделение В (&СписокПодразделений), , ) КАК УправленческийОбороты
	|
	|СГРУППИРОВАТЬ ПО
	|	ВЫРАЗИТЬ(УправленческийОбороты.Субконто1 КАК Справочник.СтатьиДоходовРасходов).РазделАналитики,
	|	ВЫРАЗИТЬ(УправленческийОбороты.Субконто1 КАК Справочник.СтатьиДоходовРасходов).НомерКолонки
	|
	|УПОРЯДОЧИТЬ ПО
	|	Раздел
	|;
	...
	|ИЗ
	|	РегистрБухгалтерии.Управленческий.Обороты(&НачалоПериода, КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ), День, Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ПрибыльУбытки)), , Подразделение В (&СписокПодразделений), , ) КАК УправленческийОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период,
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		ВыборкаРаздел = ВыборкаПериод.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "Раздел" ,"ВСЕ");
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьСтрокаКолонкаСиняя.Параметры.Сумма = Окр(Прибыль / Выборка.Доход * 100, 2);
		ТабДок.Присоединить(ОбластьСтрокаКолонкаСиняя);
