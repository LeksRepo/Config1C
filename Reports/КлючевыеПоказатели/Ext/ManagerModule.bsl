...
Функция ЗаполнитьТаблицу(НачалоПериода, КонецПериода, СписокПодразделений, ВидОтчета, ТЗСтатьи)
	...
	|	И Статьи.Ссылка <> ЗНАЧЕНИЕ(Справочник.СтатьиДоходовРасходов.ЗакрытиеДоходовНаФинРезультат)
	|	И Статьи.Ссылка <> ЗНАЧЕНИЕ(Справочник.СтатьиДоходовРасходов.ЗакрытиеРасходовНаФинРезультат)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Статьи.НомерКолонки";
