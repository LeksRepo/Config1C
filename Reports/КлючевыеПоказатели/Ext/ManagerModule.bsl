...
Функция ЗаполнитьСтроки(ПараметрыОтчета, Счет, СпрСтатьи)
	...
	"ВЫБРАТЬ
	|	УправленческийОбороты.Период КАК Период,
	|	УправленческийОбороты.Субконто1.Родитель КАК Статья,
	...
	|			,
	|			Подразделение В (&СписокПодразделений)
	|				И &УсловиеПеремещениеДС
	|				И &УсловиеФинРезультат,
	|			,
	|			) КАК УправленческийОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Если ПараметрыОтчета.ВидОтчета = "Начисления" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПеремещениеДС", "Истина");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеФинРезультат", "Субконто1 <> ЗНАЧЕНИЕ(Справочник.СтатьиДоходовРасходов.ЗакрытиеДоходовНаФинРезультат) И Субконто1 <> ЗНАЧЕНИЕ(Справочник.СтатьиДоходовРасходов.ЗакрытиеРасходовНаФинРезультат)");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "УправленческийОбороты.СуммаОборот", "-УправленческийОбороты.СуммаОборот");
	ИначеЕсли ПараметрыОтчета.ВидОтчета = "Касса" Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеПеремещениеДС", "Субконто1 <> ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПеремещениеДенежныхСредств)");
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеФинРезультат", "Истина");
	КонецЕсли;
	...
	Пока ВыборкаПериод.Следующий() Цикл
		...
		Пока ВыборкаСтатья.Следующий() Цикл
			...
			Если НЕ ЗначениеЗаполнено(ВыборкаСтатья.Статья)
				И ЗначениеЗаполнено(ВыборкаСтатья.Сумма) Тогда
				
				ТекстСообщения = "%1. За %2 есть движения на сумму %3 без указания статьи";
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения,
				ПараметрыОтчета.ВидОтчета,
				ВыборкаСтатья.Период,
				ВыборкаСтатья.Сумма);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
				
			ИначеЕсли ЗначениеЗаполнено(ВыборкаСтатья.Статья) Тогда
...
Функция ЗаполнитьПродажи(НачалоПериода, КонецПериода, СписокПодразделений) Экспорт
	...
	|	Данные.СуммаОборот КАК Мин,
	|	НАЧАЛОПЕРИОДА(Данные.Период, ДЕНЬ) КАК Период
	|ИЗ
	|	РегистрНакопления.ПродажиФакт.Обороты(&НачалоПериода, КОНЕЦПЕРИОДА(&КонецПериода, ДЕНЬ), День, Подразделение В (&СписокПодразделений)) КАК Данные
	|
	|УПОРЯДОЧИТЬ ПО
	|	Период
	|ИТОГИ
...
Функция ПолучитьВыборкуГрупп(ВидОтчета)
	...
	Если ВидОтчета = "Начисления" Тогда
		...
		ЗапросГруппы.Текст = СтрЗаменить(ЗапросГруппы.Текст, "&УсловиеПеремещениеДС", "Истина");
	Иначе
		ЗапросГруппы.Текст = СтрЗаменить(ЗапросГруппы.Текст, "&УсловиеПеремещениеДС", "Статьи.Ссылка <> ЗНАЧЕНИЕ(Справочник.СтатьиДвиженияДенежныхСредств.ПеремещениеМеждуСчетами)");
	КонецЕсли;
