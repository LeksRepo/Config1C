...
Функция СформироватьОтчет(Период, Раздел, СписокПодразделений) Экспорт
	...
	Если Раздел = "Продажи" Тогда 
		...
		ОбластьЗаголовок.Параметры.Статья = "Продажи";
	...
	Иначе 
		...
		|				И ВЫБОР
		|					КОГДА &Раздел = ""Доход""
		|						ТОГДА Субконто1.ВидСтатьи = ЗНАЧЕНИЕ(Перечисление.ВидыСтатейДР.Доход)
		|					КОГДА &Раздел = ""Итог""
		|						ТОГДА ИСТИНА
		|					ИНАЧЕ Субконто1.РазделАналитики = &Раздел
		|				КОНЕЦ,
		|			,
		|			) КАК УправленческийОбороты
		|
