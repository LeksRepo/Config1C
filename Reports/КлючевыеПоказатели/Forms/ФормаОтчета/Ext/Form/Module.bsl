...
Процедура ЗаполнитьЛимит()
	...
	Запрос.Текст="ВЫБРАТЬ
	             ...
	             |	СУММА(УправленческийОбороты.СуммаОборот) КАК СуммаВсего
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	СРЕДНЕЕ(ВТ.Норматив) КАК Норматив,
	             |	СРЕДНЕЕ(ВТ.СуммаВсего) КАК СуммаВсего
	             |ИЗ
	             |	ВТ_Показатели КАК ВТ";
				 
	Результат=Запрос.Выполнить().Выбрать();
	Результат.Следующий();
	СреднийЛимит = Окр(Результат.Норматив);
	СреднийНаряд = Окр(Результат.СуммаВсего);
...
Процедура СформироватьНаСервере()
	...
	ТабДокКасса = Отчеты.КлючевыеПоказатели.СформироватьОтчет(Отчет.НачалоПериода, КонецПериода, СписокПодразделений, "Касса");
	ТабДокНачисления = Отчеты.КлючевыеПоказатели.СформироватьОтчет(Отчет.НачалоПериода, КонецПериода, СписокПодразделений, "Начисления");
	ТабДокЭкономика = Отчеты.КлючевыеПоказатели.СформироватьОтчетЭкономика(Отчет.НачалоПериода, КонецПериода, СписокПодразделений);
...
Процедура СреднийЛимитНажатие(Элемент, СтандартнаяОбработка)
	...
	ОткрытьФорму("Отчет.КлючевыеПоказатели.Форма.ФормаНормативИНаряды", ПараметрыФормы);
