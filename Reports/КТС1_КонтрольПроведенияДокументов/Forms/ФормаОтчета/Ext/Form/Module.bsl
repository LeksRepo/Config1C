...
Функция СформироватьТабличныйДокумент()
	...
	Если НЕ РезультатОсновной.Пустой() Тогда
		...
		Пока ВыборкаНаряд.Следующий() Цикл
			...
			ОблНаряд.Параметры.Наряд = "" + ВыборкаНаряд.Наряд + " ( изготовление "+Формат(ВыборкаНаряд.НарядДатаИзготовления,"ДФ=dd.MM.yy")+" )";
			...
			Пока ВыборкаСпец.Следующий() Цикл
				 ...
				 Если НЕ (ВыборкаСпец.Комплектация = "НеТребуется") Тогда
					...
					Если ЗначениеЗаполнено(ВыборкаСпец.КомплектацияДатаДокумента) Тогда
						ОблСтрока.Параметры.ДатаДокумента = ВыборкаСпец.КомплектацияДатаДокумента;
					Иначе
						ОблСтрока.Параметры.ДатаДокумента = "";
					КонецЕсли;
