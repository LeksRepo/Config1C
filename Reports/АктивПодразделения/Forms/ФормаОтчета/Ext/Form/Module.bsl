...
Функция СформироватьТабличныйДокумент()
	...
	ОбходИерархии(Выборка, Ложь);
	
	ТабДок.ЗакончитьАвтогруппировкуСтрок();
	
	Если СуммаНач > 0 Тогда
		ОблПодвал.Параметры.ОстатокДебетНач = Формат(СуммаНач, "ЧДЦ=");
		ОблПодвал.Параметры.ОстатокКредитНач = 0;	
	Иначе
		ОблПодвал.Параметры.ОстатокДебетНач = 0;
		ОблПодвал.Параметры.ОстатокКредитНач = Формат(СуммаНач, "ЧДЦ=");
	КонецЕсли;
	...
	Если СуммаКон > 0 Тогда
		ОблПодвал.Параметры.ОстатокДебетКон = Формат(СуммаКон, "ЧДЦ=");
		ОблПодвал.Параметры.ОстатокКредитКон = 0;	
	Иначе
		ОблПодвал.Параметры.ОстатокДебетКон = 0;
		ОблПодвал.Параметры.ОстатокКредитКон = Формат(СуммаКон, "ЧДЦ=");
	КонецЕсли;	
...
Процедура ОбходИерархии(Выборка, Забалансовый)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Ур = 0 Тогда 
			...
			СуммаДт = СуммаДт + ?(ЗначениеЗаполнено(Выборка.СуммаОборотДт),Выборка.СуммаОборотДт,0);
			СуммаКт = СуммаКт + ?(ЗначениеЗаполнено(Выборка.СуммаОборотКт),Выборка.СуммаОборотКт,0);
		...
		Если Выборка.СуммаНачальныйОстаток > 0 Тогда
			Область.Параметры.ОстатокДебетНач = Формат(Выборка.СуммаНачальныйОстаток, "ЧДЦ=");	
			Область.Параметры.ОстатокКредитНач = 0;	
		Иначе
			Область.Параметры.ОстатокДебетНач = 0;
			Область.Параметры.ОстатокКредитНач = Формат(Выборка.СуммаНачальныйОстаток, "ЧДЦ=");
		КонецЕсли;
		...
		Если Выборка.СуммаКонечныйОстаток > 0 Тогда
			Область.Параметры.ОстатокДебетКон = Формат(Выборка.СуммаКонечныйОстаток, "ЧДЦ=");	
			Область.Параметры.ОстатокКредитКон = 0;	
		Иначе
			Область.Параметры.ОстатокДебетКон = 0;
			Область.Параметры.ОстатокКредитКон = Формат(Выборка.СуммаКонечныйОстаток, "ЧДЦ=");
		КонецЕсли;
		...
		Если Выборка.ТипЗаписи() <> ТипЗаписиЗапроса.ДетальнаяЗапись Тогда
			...
			Если ЗначениеЗаполнено(Выборка.Счет) Тогда
				...
				Если Выборка.Счет = ПредопределенноеЗначение("ПланСчетов.Управленческий.ФинансовыйРезультат") Тогда
					ФинПрибыль = Выборка.СуммаОборотКт - ?(ЗначениеЗаполнено(Выборка.СуммаОборотДт),Выборка.СуммаОборотДт,0);
				КонецЕсли;
