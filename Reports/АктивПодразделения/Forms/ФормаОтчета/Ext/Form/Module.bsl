...
Функция СформироватьТДСвернутый()
	...
	|				ИЛИ ОстаткиИОбороты.Счет = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ИзделияВЦеху)))
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок
	|ИТОГИ
	...
	|			,
	|			Подразделение В (&Подразделение)) КАК ОстаткиИОбороты
	|
	|УПОРЯДОЧИТЬ ПО
	|	Порядок
	|ИТОГИ
...
Процедура ОбходИерархииРазвернутый(Выборка, Забалансовый)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ Забалансовый 
			 И (Выборка.Счет = ПланыСчетов.Управленческий.СкладГотовойПродукции ИЛИ Выборка.Счет = ПланыСчетов.Управленческий.ИзделияУКлиента ИЛИ Выборка.Счет = ПланыСчетов.Управленческий.ИзделияВЦеху) Тогда
		 	Продолжить;	
		КонецЕсли;
		
		Если Забалансовый 
			 И НЕ (Выборка.Счет = ПланыСчетов.Управленческий.СкладГотовойПродукции ИЛИ Выборка.Счет = ПланыСчетов.Управленческий.ИзделияУКлиента ИЛИ Выборка.Счет = ПланыСчетов.Управленческий.ИзделияВЦеху) Тогда
		 	Продолжить;	
		КонецЕсли;
