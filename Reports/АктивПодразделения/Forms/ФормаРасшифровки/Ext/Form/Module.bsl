...
Функция СформироватьТабличныйДокумент()
	...
	|	ЕСТЬNULL(УправленческийОстаткиИОбороты.СуммаКонечныйОстаток, 0) КАК СуммаКонечныйОстаток,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипЗаписи() = ТипЗаписиЗапроса.ИтогПоГруппировке Тогда
		   ...
		   Если Выборка.СуммаНачальныйОстаток > 0 Тогда
				ОблСтрока.Параметры.ОстатокДебетНач = Формат(Выборка.СуммаНачальныйОстаток, "ЧДЦ=");	
				ОблСтрока.Параметры.ОстатокКредитНач = 0;	
			Иначе
				ОблСтрока.Параметры.ОстатокДебетНач = 0;
				ОблСтрока.Параметры.ОстатокКредитНач = Формат(Выборка.СуммаНачальныйОстаток, "ЧДЦ=");
			КонецЕсли;
		   
		   ОблСтрока.Параметры.Приход = Формат(Выборка.СуммаОборотДт, "ЧДЦ=");
    	   ОблСтрока.Параметры.Расход = Формат(Выборка.СуммаОборотКт, "ЧДЦ=");
    	   
		   Если Выборка.СуммаКонечныйОстаток > 0 Тогда
				ОблСтрока.Параметры.ОстатокДебетКон = Формат(Выборка.СуммаКонечныйОстаток, "ЧДЦ=");	
				ОблСтрока.Параметры.ОстатокКредитКон = 0;	
			Иначе
				ОблСтрока.Параметры.ОстатокДебетКон = 0;
				ОблСтрока.Параметры.ОстатокКредитКон = Формат(Выборка.СуммаКонечныйОстаток, "ЧДЦ=");
			КонецЕсли;
