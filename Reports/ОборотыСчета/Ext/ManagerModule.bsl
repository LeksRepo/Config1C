...
Процедура УстановитьУсловиеВЗапросеНаПоказатели(ПараметрыОтчета, Схема)
	...
	Если ПараметрыОтчета.ПоказательКонтроль Тогда         
		//ТекстУсловиеОтборКонтрольОбороты = "
		//|ВЫБОР
		//|	КОГДА УправленческийОбороты.Счет.НалоговыйУчет
		//|	ТОГДА УправленческийОбороты.СуммаОборотДт - УправленческийОбороты.СуммаНУОборотДт - УправленческийОбороты.СуммаПРОборотДт - УправленческийОбороты.СуммаВРОборотДт
		//|ИНАЧЕ 0
		//|КОНЕЦ <> 0
		//|ИЛИ ВЫБОР
		//|	КОГДА УправленческийОбороты.Счет.НалоговыйУчет
		//|	ТОГДА УправленческийОбороты.СуммаОборотКт - УправленческийОбороты.СуммаНУОборотКт - УправленческийОбороты.СуммаПРОборотКт - УправленческийОбороты.СуммаВРОборотКт
		//|ИНАЧЕ 0
		//|КОНЕЦ <> 0";
	Иначе
	...
	Для Каждого Показатель Из Показатели Цикл
		Если Показатель = "Контроль" ИЛИ Показатель = "НУ" ИЛИ Показатель = "ПР" ИЛИ Показатель = "ВР" ИЛИ Показатель = "ВалютнаяСумма" Тогда
			Продолжить;
		КонецЕсли;
	...
	Для Каждого Поле Из Поля Цикл
		...
		|ИЛИ УправленческийОбороты." + Поле + "ОборотДт <> 0
		|ИЛИ УправленческийОбороты." + Поле + "ОборотКт <> 0";
	КонецЦикла;
