Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	ЗаполнитьКонтрагента = ПараметрыОтчета.ЗаполнитьКонтрагента;
	...
	Если ЗаполнитьКонтрагента Тогда
		
		МНачОстатки.Параметры.СуммаНачальныйОстатокДтКонтр = Выборка.ОстатокДт;
		МНачОстатки.Параметры.СуммаНачальныйОстатокКтКонтр = Выборка.ОстатокКт;
	...
	Если НЕ Результат.Пустой() Тогда
		  ...
		  Если ЗаполнитьКонтрагента Тогда
				...
				МОборотыИтог.Параметры.СуммаОборотДтКонтр = Выборка.СуммаОборотДт;
		  		МОборотыИтог.Параметры.СуммаОборотКтКонтр = Выборка.СуммаОборотКт;
		  ...
		  Пока ВыборкаРегистратор.Следующий() Цикл
			   ...
			   Если ЗаполнитьКонтрагента Тогда
					...
					МОбороты.Параметры.СуммаОборотДтКонтр = ВыборкаРегистратор.СуммаОборотДт;
			   		МОбороты.Параметры.СуммаОборотКтКонтр = ВыборкаРегистратор.СуммаОборотКт;
			   		МОбороты.Параметры.ДатаДокументаКонтр = ВыборкаРегистратор.Период;
			  		МОбороты.Параметры.РегистраторКонтр = ВыборкаРегистратор.Регистратор;
	...
	Если ЗаполнитьКонтрагента Тогда
		...
		МКонОстатки.Параметры.СуммаКонечныйОстатокДтКонтр = Выборка.ОстатокДт;
		МКонОстатки.Параметры.СуммаКонечныйОстатокКтКонтр = Выборка.ОстатокКт;
	...
	ФИОПредставителя = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(ПараметрыСеанса.ТекущийПользователь, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
