...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	Запрос.УстановитьПараметр("Автор", ПараметрыОтчета.СтаршийДизайнер);
	Запрос.Текст =
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|ГДЕ
	|	Договор.Автор.ФизическоеЛицо = &Автор
...
Функция СформироватьОтчетОбщий(ПараметрыОтчета, ОтчетСтДиз = Истина) Экспорт
	...
	Пока Выборка.Следующий() Цикл	
		...
		ИначеЕсли НЕ ЭтоЗамеры И ОтчетСтДиз Тогда
