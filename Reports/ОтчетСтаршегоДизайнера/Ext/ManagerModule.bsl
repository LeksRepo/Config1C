...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	|	NULL КАК Изделие,
	|	NULL КАК Монтажник,
	|	НаличиеЗаметокПоПредмету.ЕстьЗаметка КАК Заметка
	|ИЗ
		...
		Пока ВыборкаПоСотрудникам.Следующий() Цикл
			...
			Пока ВыборкаПоДокумента.Следующий() Цикл
				...
				Иначе
					...
					ОбластьСтрокаМонтаж.Параметры.Заметка = Новый Структура("Заметка, Предмет", ВыборкаПоДокумента.Заметка, ВыборкаПоДокумента.Предмет);
...
Функция СформироватьОтчетОбщий(ПараметрыОтчета) Экспорт
	...
	|			Замер.Ссылка) КАК Встречи
	|		ПО докЗамер.Ссылка = Встречи.Ссылка
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.НаличиеЗаметокПоПредмету КАК НаличиеЗаметокПоПредмету
	|		ПО (докЗамер.Ссылка = (ВЫРАЗИТЬ(НаличиеЗаметокПоПредмету.Предмет КАК Документ.Замер)))
	|ГДЕ
	|	АктивностьЗамеров.Статус
	|	И докЗамер.Подразделение.Регион = &Регион
