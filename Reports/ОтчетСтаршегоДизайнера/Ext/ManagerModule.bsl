	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаПоСотрудникам.Следующий() Цикл
			...
			Если ЭтоЗамеры Тогда
				...
				Если ТипЗнч(ВыборкаПоСотрудникам.Автор) = Тип("СправочникСсылка.Пользователи") Тогда
					ОбластьСтрокаЗамеры.Параметры.Дизайнер = ФизическиеЛицаКлиентСервер.ФамилияИнициалыФизЛица(ВыборкаПоСотрудникам.Автор.ФизическоеЛицо.Наименование);
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ВыборкаПоСотрудникам.ДатаВстречи) И ВыборкаПоСотрудникам.ДатаВстречи < НачалоДня(ТекущаяДата()) Тогда
					 ОбластьСтрокаЗамеры.Параметры.ДатаВстречи = "";
				КонецЕсли;
				...
				Если ЗначениеЗаполнено(ВыборкаПоСотрудникам.ДатаВстречи) Тогда
					Если НачалоДня(ВыборкаПоСотрудникам.ДатаВстречи) = НачалоДня(ТекущаяДата()) Тогда
						...
						ОбластьСтрокаЗамеры.Области.СтрокаЗамерыОбщ.ЦветТекста = Новый Цвет(0,0,0);
						//RonEXI: Пуховой попросил убрать
			...
			ИначеЕсли ЭтоДоговорыБезМонтажа Тогда
				...
				ОбластьСтрокаДоговорыБезМонтажа.Параметры.Предмет =
				ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ВыборкаПоСотрудникам.Предмет.Номер)
				+ " от " + Формат(ВыборкаПоСотрудникам.Предмет.Дата,"ДФ=dd.MM.yy");
...
Функция ПолучитьЗапрос(ПараметрыОтчета)
	...
	|		ГДЕ
	|			ВстречаСКлиентом.Дата > &НачалоДня
	|			И ВстречаСКлиентом.Проведен
	|		
	|		СГРУППИРОВАТЬ ПО
