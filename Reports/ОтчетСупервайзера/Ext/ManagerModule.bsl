...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	|ВЫБРАТЬ
	|	ВТ_Сотрудники.ФизЛицо,
	|	СУММА(ПродажиОбороты.ЗначениеОборот),
	|	0,
	|	NULL,
	|	NULL,
	|	NULL,
	...
	|ИЗ
	|	ВТ_Сотрудники КАК ВТ_Сотрудники
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.Продажи.Обороты(&ДатаНачала, &ДатаОкончания, Регистратор, Показатель = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСотрудников.Выручка)) КАК ПродажиОбороты
	|		ПО ВТ_Сотрудники.ФизЛицо = ПродажиОбороты.Дизайнер
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТ_Сотрудники.ФизЛицо,
	|	ВТ_Сотрудники.Работает,
	...
	|	NULL,
	|	NULL,
	|	АктивностьЗамеров.Замер.ДатаЗамера КАК Дата,
