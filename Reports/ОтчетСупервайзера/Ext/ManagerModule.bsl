...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	|					ИЛИ Должность = ЗНАЧЕНИЕ(Справочник.Должности.Супервайзер))) КАК ДолжностиСотрудниковСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|	МАКСИМУМ(ВстречаСКлиентом.Основание),
	|	МАКСИМУМ(ВстречаСКлиентом.Основание.Агент),
	|	МАКСИМУМ(НаличиеЗаметокПоПредмету.ЕстьЗаметка),
	|	ВТ_Сотрудники.Работает
	|ИЗ
	|	ВТ_Сотрудники КАК ВТ_Сотрудники
			...
			Если ЗначениеЗаполнено(Выборка.Дата) ИЛИ ЗначениеЗаполнено(Выборка.ДоговорВРаботе) Тогда
				...
				Пока ВыборкаВстречиДоговора.Следующий() Цикл
					...
					ИначеЕсли ЗначениеЗаполнено(ВыборкаВстречиДоговора.ДоговорВРаботе) Тогда
						...
						ОбластьСтрокаДокРаб.Параметры.Расшифровка = ВыборкаВстречиДоговора.ДоговорВРаботе;
