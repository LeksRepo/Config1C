...
Функция СформироватьОтчет(ТабДок, Подразделение, КлиентскаяДата) Экспорт
	...
	ОбластьСтрокаСерая = Макет.ПолучитьОбласть("СтрокаСерая");
	...
	ОбластьСтрокаСераяБезКартинки = Макет.ПолучитьОбласть("СтрокаСерая");
	...
	Для Каждого Рис Из ОбластьСтрокаСераяБезКартинки.Рисунки Цикл
		ОбластьСтрокаСераяБезКартинки.Рисунки.Очистить();
	КонецЦикла;
		...
		Пока ВыборкаНаряд.Следующий() Цикл
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
				...
				Если ВыборкаДетальныеЗаписи.Срочный Тогда
					ОбластьВывод = ОбластьСтрокаСерая;
				...
				Если ВыборкаДетальныеЗаписи.Спецификация.ПакетУслуг = Перечисления.ПакетыУслуг.СамовывозОтПроизводителя Тогда
					...
					Если ВыборкаДетальныеЗаписи.Срочный Тогда
						ОбластьВывод = ОбластьСтрокаСераяБезКартинки;
					ИначеЕсли ВыборкаДетальныеЗаписи.ДатаОтгрузки < КлиентскаяДата Тогда
