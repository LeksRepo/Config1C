...
Функция СформироватьОтчет(ПараметрыОтчета) Экспорт
	...
	|	ДокументЗамер.ДатаЗамера КАК Дата,
	|	ЧАС(ДокументЗамер.ДатаЗамера) КАК ВремяЗамера,
	|	ЧАС(ДокументЗамер.ДатаЗамера) + 1 КАК ВремяЗамераПлюсОдин,
	...
	|	И ДокументЗамер.ДатаЗамера МЕЖДУ &НачалоПериода И &КонецПериода
