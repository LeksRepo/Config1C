...
Функция ВывестиПроизводственныйОтчет(ТабДок, Подразделение, МассивСсылок, Состав = Неопределено) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока ВыборкаСтатус.Следующий() Цикл
			...
			Если ВыборкаСтатус.Статус = Перечисления.СтатусыСпецификации.Проверяется Тогда
				Шапка.Параметры.НаименованиеТаблицы = "Спецификации на проверке.";				
			Иначе
	...
	СтрокаКрасная = Макет.ПолучитьОбласть("ИзделияНаСкладеСтрокаКрасная");
	СтрокаЗаметка = Макет.ПолучитьОбласть("СтрокаЗаметка");
	...
	|	докСпецификация.Срочный КАК Срочный,
