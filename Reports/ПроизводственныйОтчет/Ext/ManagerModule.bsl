...
Функция ВывестиМонтажи(ТабДок, Макет, Подразделение)
	...
	|	МИНИМУМ(Монтажи.День) КАК ДатаМонтажа,
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РабочиеДниМонтажников КАК Монтажи
	|		ПО Статусы.Спецификация = Монтажи.Спецификация
	...
	|
	|СГРУППИРОВАТЬ ПО
	|	Статусы.Спецификация,
	|	Статусы.Спецификация.Изделие,
	|	Статусы.Спецификация.АдресМонтажа,
	|	Статусы.Спецификация.Монтажник,
	|	ВЫРАЗИТЬ(Статусы.Спецификация.ДокументОснование КАК Документ.Замер).Замерщик,
	|	Заметки.Ссылка
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаМонтажа";
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ДатаМонтажа < ТекущаяДата() Тогда
				ТекСтрока = МонтажиСтрокаКрасная;
			Иначе
				ТекСтрока = МонтажиСтрока;
			КонецЕсли;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЗначениеЗаполнено(Выборка.Заметка) Тогда
				...
				ЗаметкаСтрока.Параметры.Заметка = Выборка.Заметка;
				ТабДок.Вывести(ЗаметкаСтрока);
