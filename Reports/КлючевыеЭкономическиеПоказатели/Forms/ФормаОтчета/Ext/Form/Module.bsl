...
Процедура СформироватьНаСервере()
	...
	ЗаполнитьПоказатели();
	ЗаполнитьТабДок(ТабДокКасса, ПланыСчетов.Управленческий.ДенежныеСредства);
	
КонецПроцедуры

&НаСервере
Функция ЗаполнитьТабДок(ТабДок, Счет)
	...
	////ОбластьШапкаСтатьяПлюс = Макет.ПолучитьОбласть("Шапка|СтатьяПлюс");
	////ОбластьШапкаСтатьяМинус = Макет.ПолучитьОбласть("Шапка|СтатьяМинус");
	//
	//ОбластьСтрокаПериод = Макет.ПолучитьОбласть("Период|Статья");
	//ОбластьСтрокаСтатьяМинус = Макет.ПолучитьОбласть("Период|СтатьяМинус");
	//ОбластьСтрокаСтатьяПлюс = Макет.ПолучитьОбласть("Период|СтатьяПлюс");
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВЫРАЗИТЬ(УправленческийОбороты.Субконто1 КАК Справочник.СтатьиДвиженияДенежныхСредств).ВидСтатьи УБЫВ,
	|	Период,
	|	Статья
	|ИТОГИ
	|	СУММА(Сумма)
	...
	Пока ВыборкаСтатья.Следующий() Цикл
		ОбластьШапкаКолонка.Параметры.Заполнить(ВыборкаСтатья);
