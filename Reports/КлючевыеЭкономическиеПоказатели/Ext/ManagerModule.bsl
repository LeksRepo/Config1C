...
Функция СформироватьОтчет(ПериодОтчета, СписокПодразделений, ВидОтчета) Экспорт
	...
	Таблица = ЗаполнитьТаблицу(Запрос);
	...
	Для каждого Колонка Из Таблица.Колонки Цикл
		...
		//Если Справочники.СтатьиДвиженияДенежныхСредств[Колонка.Имя].ВидСтатьи = Перечисления.ВидыСтатейДДС.Поступление Тогда
		//	Область = ОбластьШапкаКолонкаЗеленая;
		//Иначе
		//	Область = ОбластьШапкаКолонкаКрасная;
		//КонецЕсли;
	...
	Для каждого Строка Из Таблица Цикл
		...
		Для каждого Колонка Из Таблица.Колонки Цикл
			...
			//Если Справочники.СтатьиДвиженияДенежныхСредств[Колонка.Имя].ВидСтатьи = Перечисления.ВидыСтатейДДС.Поступление Тогда
			//	Область = ОбластьСтрокаКолонкаЗеленая;
			//Иначе
			//	Область = ОбластьСтрокаКолонкаКрасная;
			//КонецЕсли;
...
Функция ЗаполнитьТаблицу(Запрос)
	...
	//ТаблицаКолонок.Колонки.Добавить("ЗаголовокКолонки");
	//ТаблицаКолонок.Колонки.Добавить("ПолеСортировки");
	//
	//ВыборкаПериод = РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, "ПЕРИОД" ,"ВСЕ");
	//ВыборкаПериод.Следующий();
	//ВыборкаСтатья = ВыборкаПериод.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкамСИерархией, "СТАТЬЯ" ,"ВСЕ");
	//Пока ВыборкаСтатья.Следующий() Цикл
	//	НоваяСтрока = ТаблицаКолонок.Добавить();
	//	НоваяСтрока.ИмяКолонки = СпрСтатьи.ПолучитьИмяПредопределенного(ВыборкаСтатья.Статья);
	//	НоваяСтрока.ЗаголовокКолонки = ВыборкаСтатья.Статья;
	//	НоваяСтрока.ПолеСортировки = ВыборкаСтатья.Статья.НомерКолонки;
	//КонецЦикла;
	//
	//ТаблицаКолонок.Сортировать("ПолеСортировки");
	//
	//Для каждого НоваяКолонка Из ТаблицаКолонок Цикл
	//	ТЗ.Колонки.Добавить(НоваяКолонка.ИмяКолонки,,НоваяКолонка.ЗаголовокКолонки);
	//КонецЦикла;
	//
