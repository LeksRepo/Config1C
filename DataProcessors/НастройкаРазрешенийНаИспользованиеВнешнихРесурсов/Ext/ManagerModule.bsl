...
Функция ЗапросНаАдминистрированиеРазрешенийИспользованияВнешнихРесурсов(Знач ВнешнийМодуль, Знач Операция) Экспорт
	...
	Если Операция = Перечисления.ОперацииСНаборамиРазрешений.Создание Тогда
		Менеджер.Имя = НовоеИмяПрофиляБезопасности(ВнешнийМодуль);
	Иначе
		Менеджер.Имя = ИмяПрофиляБезопасности(ВнешнийМодуль);
	КонецЕсли;
...
Функция ЗапросыНаОтключениеПрофилейБезопасности(Знач ВключаяПрофильИБ = Истина)
	...
	Попытка
		...
		Если ВключаяПрофильИБ Тогда
			Результат.Добавить(РаботаВБезопасномРежимеСлужебный.ЗапросНаУдалениеНабораРазрешений(СлужебныйИОМ()));
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			Результат.Добавить(РаботаВБезопасномРежимеСлужебный.ЗапросНаУдалениеНабораРазрешений(Выборка.ВнешнийМодуль));
		КонецЦикла;
...
Функция ЗапросыЗамещенияДляВсехПредоставленныхРазрешений()
	...
	ШаблонЗапросаКТаблицеРегистра = "ВЫБРАТЬ РАЗЛИЧНЫЕ ТаблицаРазрешений.ВнешнийМодуль КАК ВнешнийМодуль, ТаблицаРазрешений.Владелец КАК Владелец ИЗ %1 КАК ТаблицаРазрешений";
	...
	ТекстЗапроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		"ВЫБРАТЬ РАЗЛИЧНЫЕ ВложенныйЗапрос.ВнешнийМодуль КАК ВнешнийМодуль, ВложенныйЗапрос.Владелец КАК Владелец ИЗ (%1) КАК ВложенныйЗапрос",
		ТекстЗапросаКТаблицамРегистров);
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗапросЗамещения = РаботаВБезопасномРежимеСлужебный.ЗапросНаИзменениеРазрешенийИспользованияВнешнихРесурсов(
			Выборка.Владелец, Истина, Новый Массив(), , Выборка.ВнешнийМодуль);
...
//  ПсевдонимТаблицы - Строка - пвседоним для таблицы (для текста запроса),
...
Функция ПланПримененияЗапросовНаИспользованиеВнешнихРесурсов(Знач ИдентификаторыЗапросов)
	...
	ЗамещаемыеПоТипам.Колонки.Добавить("ВнешнийМодуль", ОписаниеТипаЛюбаяСсылка());
	ЗамещаемыеПоТипам.Колонки.Добавить("Владелец", ОписаниеТипаЛюбаяСсылка());
...
Процедура ПрименитьЗапросы(Знач ИдентификаторыЗапросов) Экспорт
	...
	Иначе
		...
		Попытка
			...
			ФоновыеЗадания.Выполнить("РаботаВБезопасномРежиме.ВыполнитьМетодКонфигурации", ,"f023cc5d-1846-4c49-b0ba-27aa65924879");
	...
	Попытка
		...
		Для Каждого ЭлементСреза Из ТекущийСрез Цикл
			...
			ИмяТаблицы = ЭлементСреза.Ключ;
			Таблица = ЭлементСреза.Значение;
			МенеджерТекущегоРегистра = Неопределено;
			
			Для Каждого МенеджерРегистра Из МенеджерыРегистров Цикл
				
				Если МенеджерРегистра.ТипXDTOПредставленияРазрешений().Имя = ИмяТаблицы Тогда
					
					МенеджерТекущегоРегистра = МенеджерРегистра;
					Прервать;
			...
			Если МенеджерТекущегоРегистра = Неопределено Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Ошибка при разборе запроса разрешений на использование внешних ресурсов: неизвестное имя типа %1!'"),
					ИмяТаблицы);
			КонецЕсли;
			
			НаборЗаписей = МенеджерТекущегоРегистра.СоздатьНаборЗаписей();
...
Процедура ПослеОбработкиЗапросов(Знач ИдентификаторыЗапросов) Экспорт
	...
	Попытка
		...
		ТекстЗапроса =
			...
			|	ЗапросыАдминистрированияРазрешенийИспользованияВнешнихРесурсов.Имя
			|ИЗ
		...
		Пока Выборка.Следующий() Цикл
			...
			ЭтоПрофильКонфигурации = (Выборка.Владелец = СлужебныйИОМ());
			...
			Иначе
				...
				Иначе
					...
					ОчиститьПредоставленныеРазрешения(Выборка.Владелец, Истина);
...
Функция СценарийПримененияЗапросовНаИзменениеДоступаКВнешнимРесурсам(Знач ИдентификаторыЗапросов)
	...
	Для Каждого Описание Из ОперацииАдминистрирования Цикл
		...
		ЭтоПрофильКонфигурации = (Описание.ВнешнийМодуль = СлужебныйИОМ());
	...
	Для Каждого ФрагментИзменений Из Дельта Цикл
		...
		Если ИмяПрофиля = Неопределено Тогда
			...
			Если Модуль <> СлужебныйИОМ() Тогда
		...
		Если Модуль <> СлужебныйИОМ() И ЭлементРезультата.Операция = Перечисления.ОперацииСНаборамиРазрешений.Обновление Тогда
...
// Возвращает описание типов для набора ссылок вида ЛюбаяСсылка.
//
...
//
Функция НовоеИмяПрофиляБезопасности(Знач ВнешнийМодуль) Экспорт
	
	Если ВнешнийМодуль = СлужебныйИОМ() Тогда
...
Процедура ЗаполнитьВнешнийМодульДляПредоставленныхРазрешений() Экспорт
	...
	Попытка
		...
		СтрокаКонфигурации.БезопасныйРежим = ИмяПрофиляБезопасности(СлужебныйИОМ());
