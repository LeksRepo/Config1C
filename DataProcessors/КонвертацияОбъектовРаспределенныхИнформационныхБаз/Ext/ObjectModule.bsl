	...
	Если ЗагрузитьТолькоПараметры Тогда
		...
		Если ЗагрузитьМетаданные Тогда
			...
			Попытка
				...
				// Получаем параметры работы программы
				ПрочитатьПараметрыРаботыПрограммы(ЧтениеСообщения);
...
Процедура ЗаписатьИзмененияВФайлСообщенияОбмена(Отказ, ЗаписьXML)
	...
	Попытка
		...
		// Записываем параметры работы программы в конец сообщения обмена
		ЗаписатьПараметрыРаботыПрограммы(ЗаписьСообщения);
...
Процедура ЗаписатьПараметрыРаботыПрограммы(Знач ЗаписьСообщения)
	...
	Если ЗаписьСообщения.Получатель <> ПланыОбмена.ГлавныйУзел() Тогда
		...
		Попытка
			...
			СтандартныеПодсистемыСервер.ПолучитьПараметрыРаботыПрограммы(Параметры);
			
			ВыборкаИзменений = ПланыОбмена.ВыбратьИзменения(ЗаписьСообщения.Получатель, ЗаписьСообщения.НомерСообщения, Параметры);
...
Процедура ПрочитатьПараметрыРаботыПрограммы(Знач ЧтениеСообщения)
