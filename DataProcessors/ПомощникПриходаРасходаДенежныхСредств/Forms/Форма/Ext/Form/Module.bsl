...
Функция ПолучитьСуммуОплатыДоговора(Договор)
	...
	Если ЗначениеЗаполнено(Договор.ВидОплаты) Тогда
		ИспользуемыйВидОплаты = Договор.ВидОплаты;	
	Иначе
	...
	Если ТипЗнч(ИспользуемыйВидОплаты) = Тип("ПеречислениеСсылка.ВидыОплатыДоговоров") Тогда
	...
	Если ТипЗнч(ИспользуемыйВидОплаты) = Тип("СправочникСсылка.ВидыОплаты") Тогда
	
			Если Оплата > 0 Тогда
				Если ИспользуемыйВидОплаты.КоличествоМесяцевРассрочки = 4 Тогда
					ТекМесяц = Месяц(ТекущаяДата());
					Для каждого Строка Из Договор.Рассрочка Цикл
						Если Месяц(Строка.Дата) = ТекМесяц Тогда
							Сумма = Строка.Сумма;
			...
			ИначеЕсли НЕ ИспользуемыйВидОплаты.ПолнаяПредоплата Тогда
				Сумма = Окр((Договор.СуммаДокумента / 2) + 0.49, 0);
			КонецЕсли;
