...
Процедура Назад(Команда)
	...
	Если ТекущаяОсновнаяСтраница = ПодчиненныеСтраницы.ЭтапВыбораКонтрагентаДоговора Тогда
		ПеречитатьДанныеДокумента();
	КонецЕсли;
...
Функция СформироватьДокумент()
	...
	НовыйДокумент.Дата = НачалоДня(ТекущаяДата()); // Должно помочь для платежей по спецификациям.
...
Процедура СформироватьПриход(НовыйДокумент)
	...
	ЭтоЗамер = ТипОснования = Тип("ДокументСсылка.Замер");
	
	НовыйДокумент.СуммаДокумента = Сумма;
	НовыйДокумент.НачислятьБонусЗаОплатуНаличными = НачислятьБонусЗаОплатуНаличными;
	НовыйДокумент.БонусЗаОплатуНаличными = БонусЗаОплатуНаличными;
	...
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.РеализацияМатериалов") Тогда
		СтатьяПоступления = Справочники.СтатьиДвиженияДенежныхСредств.ПоступленияОтРеализацииМатериалов;
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.Договор") ИЛИ ТипОснования = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
		СтатьяПоступления = Справочники.СтатьиДвиженияДенежныхСредств.ПоступленияПоДоговорам;
	ИначеЕсли ТипОснования = Тип("ДокументСсылка.Спецификация") И НЕ Контрагент.Дилер Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ДокументОснование") Тогда
		...
		ВидПлатежа = "Приход";
		СпособПлатежа = "Нал";
		ПоказыватьСуществующиеПлатежи = Ложь;
		
		ПеречитатьДанныеДокумента();
		
	КонецЕсли;
	
	Если ПоказыватьСуществующиеПлатежи Тогда
		ЗаполнитьСуществующиеПлатежи(ДокументОснование);
	Иначе
		Элементы.СуществующиеПлатежи.Видимость = Ложь;
	КонецЕсли;
	
	Печать = Истина;
	
	МассивОтбора = Новый Массив;
	МассивОтбора.Добавить(Новый ПараметрВыбора("Отбор.Подразделение", Подразделение));
...
&НаСервере
Процедура ЗаполнитьСуществующиеПлатежи(Документ)
	...
	|	ПриходДенежныхСредств.Ссылка КАК Документ
	|ИЗ
	|	Документ.ПриходДенежныхСредств КАК ПриходДенежныхСредств
	|ГДЕ
	|	ПриходДенежныхСредств.Субконто2Кт = &Документ
	...
	Если Результат.Количество() > 0 Тогда
		...
		СуществующиеПлатежи.Загрузить(Результат);
...
Функция ПолучитьСуммуОплатыДоговора(Договор)
	...
	Запрос.УстановитьПараметр("Подразделение", Договор.Подразделение);
	...
	Если ТипЗнч(ВидОплаты) = Тип("СправочникСсылка.ВидыОплаты") Тогда
		
		Если Оплата > 0 Тогда
			Если ВидОплаты.КоличествоМесяцевРассрочки > 1 Тогда
		...
		ИначеЕсли НЕ ВидОплаты.ПолнаяПредоплата Тогда
...
Процедура РасчетныйСчетНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ЗначениеОтбора  = Новый Структура("Активность", Истина);
	ПараметрыВыбора = Новый Структура("Отбор", ЗначениеОтбора);
	ОткрытьФорму("Справочник.Счета.ФормаВыбора", ПараметрыВыбора, Элемент);
...
Процедура ВидПлатежаПриИзменении(Элемент)
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Спецификация")
		ИЛИ ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияМатериалов") Тогда
		ВидПлатежа = "Приход";
	КонецЕсли;
...
&НаСервере
Функция ПеречитатьДанныеДокумента()
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Договор") Тогда 
		...
		Контрагент = ДокументОснование.Спецификация.Контрагент;
		...
		ПоказыватьСуществующиеПлатежи = Истина;
		
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
		
		Контрагент = ДокументОснование.Договор.Спецификация.Контрагент;
		Подразделение = ДокументОснование.Договор.Подразделение;
		ВидПлатежа = ?(ДокументОснование.СуммаДокумента > 0, "Приход", "Расход");
		Офис = ДокументОснование.Офис;
		Сумма = ?(ДокументОснование.СуммаДокумента > 0, ДокументОснование.СуммаДокумента, -ДокументОснование.СуммаДокумента);
		ПоказыватьСуществующиеПлатежи = Истина;
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Спецификация") Тогда
		...
		Если Контрагент = Справочники.Контрагенты.ЧастноеЛицо Тогда
			...
			Отказ = Истина;
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Заполните поле ""Заказчик""", ДокументОснование, "Контрагент", , Отказ);
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РеализацияМатериалов") Тогда
	...
	ИначеЕсли ДокументОснование = Неопределено
		И Параметры.Свойство("ПлатежОтДилера")
		И Параметры.ПлатежОтДилера Тогда
		
		Элементы.ВидПлатежа.СписокВыбора.Удалить(1);
		НастройкаПодразделение = ПланыВидовХарактеристик.НастройкиПользователей.Подразделение;
		Подразделение = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(Пользователь, НастройкаПодразделение);
...
Процедура НачислятьБонусЗаОплатуНаличнымиПриИзменении(Элемент)
	...
	Если НачислятьБонусЗаОплатуНаличными Тогда
		Сумма = ЛексКлиентСервер.ПолучитьСуммуБезБонуса(Сумма, ЛексСервер.ПолучитьПроцентБонусаЗаОплатуНаличными(Подразделение, ТекущаяДата()));
		БонусЗаОплатуНаличными = ЛексСервер.РассчитатьБонусЗаОплатуНаличными(Подразделение, ТекущаяДата(), Сумма);
	Иначе
		// Может быть не очень красиво перечитывать все данные из документа
		// когда нам нужна только сумма, да и пользователь мог изменить
		// какие-нибудь реквизиты платежа, но думаю такого не бывает.
		ПеречитатьДанныеДокумента();
		БонусЗаОплатуНаличными = 0;
	КонецЕсли;
