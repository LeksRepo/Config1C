...
Процедура HTMLТекстПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если (ЭлементHTML.id = "FullTextSearchListItem") Тогда
		...
		Если Позиция <> 0 Тогда
			ЧастьURL = Сред(ЧастьURL, Позиция+9, 1);
		КонецЕсли;
...
Процедура ОткрытьЗначениеПоиска(Значение)
	...
	// При открытии результата из списка найденного в полнотекстовом поиске.
	...
	Если СтандартнаяОбработка = Истина Тогда // Стандартное поведение.
