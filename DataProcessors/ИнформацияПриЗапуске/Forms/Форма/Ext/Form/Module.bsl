...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонфигурацияБазовая       = СтандартныеПодсистемыПереопределяемый.ЭтоБазоваяВерсияКонфигурации();
	...
	Если ЭтоВебКлиент Тогда
		...
		КодЛокализации = СтрЗаменить(КодЛокализации, "ru_RU", "ru");
		КодЛокализации = СтрЗаменить(КодЛокализации, "en_GB", "en");
		КодЛокализации = СтрЗаменить(КодЛокализации, "en_US", "en");
	...
	Иначе
		ПоказыватьПриНачалеРаботы = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить("ИнформацияПриЗапуске", "Показывать", Истина);
	КонецЕсли;
...
Функция ЗаполнитьКоманды()
	...
	ТабличныйДокумент = ЭтотОбъект.ПолучитьМакет("Описатель");
...
Функция ПросмотрСтраницы(ТипДействия, Параметр = Неопределено)
	Перем СтрокаКоманды, АдресСтраницы, НоваяСтрокаИстории, НоваяСтрокаИсторииИндекс;
