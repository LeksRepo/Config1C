...
Процедура НадписьПереченьРелизовНаДискахИТСНажатие(Элемент)
	...
	Если ПустаяСтрока(ПереченьРелизовНаДискахИТС) Тогда
		ОписаниеОповещения = Новый ОписаниеОповещения("НадписьПереченьРелизовНаДискахИТСНажатиеЗавершение", ЭтотОбъект);
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'Информация о перечне релизов не обнаружена.'"));
	Иначе
		НадписьПереченьРелизовНаДискахИТСНажатиеЗавершение(Неопределено);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура НадписьПереченьРелизовНаДискахИТСНажатиеЗавершение(ДополнительныеПараметры) Экспорт
...
Функция ПолучитьПараметрыАдминистрированияИБ()
	...
	Возврат СоединенияИБВызовСервераПовтИсп.ПолучитьПараметрыАдминистрированияИБ();
...
Процедура ВосстановитьНастройкиОбновленияКонфигурации()
	...
	Если ПараметрыРаботыКлиента.НастройкиОбновления.ЕстьДоступДляОбновления Тогда
		...
		Настройка = СоединенияИБВызовСервераПовтИсп.ПолучитьПараметрыАдминистрированияИБ();
...
Процедура ПередОткрытиемСтраницы(НоваяТекущаяСтраница = Неопределено) 
	...
	Если НоваяТекущаяСтраница = Страницы.ДоброПожаловать Тогда
		...
		Если Не ИспользоватьОбновлениеСДискаИТС И Элементы.ИсточникОбновления.СписокВыбора.Количество() = 3 Тогда
			
			Элементы.ИсточникОбновления.СписокВыбора.Удалить(1);
...
Функция ОбработатьСтраницуПараметрыАвторизации(ВыдаватьСообщения = Истина)
	...
	Если Объект.КластерТребуетАутентификации Тогда
		...
		Если НЕ ЗначениеЗаполнено(Объект.ИмяАдминистратораКластера) Тогда
			Если ВыдаватьСообщения Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Укажите имя администратора кластера серверов.'"));
			КонецЕсли;
		...
		Если НЕ ЗначениеЗаполнено(Объект.ПарольАдминистратораКластера) Тогда
			Если ВыдаватьСообщения Тогда
				ПоказатьПредупреждение(, НСтр("ru = 'Укажите пароль администратора кластера серверов.'"));
			КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ТекстОшибки) И ВыдаватьСообщения Тогда
		Если ВыдаватьСообщения Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Не удалось установить пароль администратора:'") + " " + КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецЕсли;
		Возврат Страницы.ПараметрыАвторизации.Имя;
	КонецЕсли; 
...
Функция ПроверитьОбновлениеИТС(ВыдаватьСообщения = Истина)
	...
	Иначе
		Если ПустаяСтрока(ДискИТС) Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Укажите местоположение диска 1С:ИТС.'"));
		...
		Если НЕ ПроверкаСуществования.Существует() ИЛИ НЕ ПроверкаСуществования.ЭтоКаталог() Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Указанный каталог не существует.'"));
			Возврат Страницы.МестоположениеДискаИТС.Имя;
		КонецЕсли;
