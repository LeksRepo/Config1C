...
Процедура СоздатьЭлементыПодвала(МассивИменСсылок)
	...
	Элементы.ОбщийПодвал.Отображение = ОбщегоНазначенияКлиентСервер.ОтображениеОбычнойГруппыЛиния();
...
Функция СформироватьГруппуДляПожелания(Пожелание, ОсновнаяГруппа, Итерация, ДеревоЭлементов)
	...
	ГруппаПожелания.Отображение              = ОбщегоНазначенияКлиентСервер.ОтображениеОбычнойГруппыЛиния();
...
Процедура СформироватьГруппуРейтингаПожелания(Пожелание, Итерация, РодительскаяГруппа, РодительскаяСтрокаДерева)
	...
	Если Пожелание.ClosingDate <> '00010101' Тогда 
		...
		ИмяЭлементаВыполнено            = "Выполнено" + Строка(Итерация);
		ИмяЭлементаВыполнено            = Элементы.Добавить(ИмяЭлементаВыполнено, Тип("ДекорацияФормы"), ГруппаРейтинга);
		ИмяЭлементаВыполнено.Заголовок  = НСтр("ru = 'сделано
		                                          |'") + Строка(Формат(Пожелание.ClosingDate, "ДЛФ=D"));
		ИмяЭлементаВыполнено.Высота     = 2;
		ИмяЭлементаВыполнено.Ширина     = ШиринаЭлементовГруппы;
		ИмяЭлементаВыполнено.ГоризонтальноеПоложение = ГоризонтальноеПоложениеЭлемента.Центр;
		
		СтрокаДерева             = СтрокаГруппы.Строки.Добавить();
		СтрокаДерева.ИмяЭлемента = ИмяЭлементаВыполнено;
