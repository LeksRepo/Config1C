...
Процедура ДобавитьКомментарий(Команда)
	...
	Если ПустаяСтрока(НовыйКомментарий) Тогда 
		Предупреждение(НСтр("ru = 'Пожалуйста введите комментарий.'"));
		Возврат;
	КонецЕсли;
	
	Результат = ДобавитьКомментарийСервер();
	Если Результат Тогда
		НовыйКомментарий = "";
		ПоказатьОповещениеПользователя(НСтр("ru = 'Комментарий добавлен.'"));
	Иначе
		Предупреждение(НСтр("ru = 'Комментарий не добавлен. Повторите попытку позднее.'"));
		Возврат;
	КонецЕсли;
...
Функция СформироватьЭлементыСтраницы(НомерСтраницы)
	...
	ОбъектыПожелания = ПолучитьОбъектыПожелания();
...
&НаСервере
Функция ПолучитьОбъектыПожелания()
...
Процедура СформироватьЭлементыПожелания(ОбъектыПожелания)
	...
	Если ПустаяСтрока(КомментарийРазработчика) Тогда
		Элементы.ГруппаОтветаСлужбыПоддержки.Видимость = Ложь;
	КонецЕсли;
...
&НаСервере
Функция ДобавитьКомментарийСервер()
