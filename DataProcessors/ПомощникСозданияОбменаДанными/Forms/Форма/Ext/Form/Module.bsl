&НаКлиенте
Перем ЗакрытьФормуБезусловно, ПользовательОтветилДаНаВопросОСопоставлении, ПропускатьКонтрольОтказаТекущейСтраницы;
...
Процедура УзнатьПрефиксЭтойИнформационнойБазы(Команда)
	...
	ОткрытьФорму("Обработка.ПомощникСозданияОбменаДанными.Форма.УзнатьПрефиксЭтойИнформационнойБазы", , ЭтотОбъект);
...
Функция ПроверитьПодключениеCOMНаСервере()
	...
	Если Результат.Соединение = Неопределено Тогда
		Если Результат.ОшибкаПодключенияКомпоненты И ИнформационнаяБазаФайловая Тогда
			Элементы.НадписьОбновитьВерсиюКомпоненты.Видимость = Истина;
		КонецЕсли;
