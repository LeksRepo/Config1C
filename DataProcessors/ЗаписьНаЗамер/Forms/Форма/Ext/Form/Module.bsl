...
Функция ЗаполнитьПлан()
	...
	|	ГрафикЗамеров.Сотрудник КАК Сотрудник,
	|	ГрафикЗамеров.Регистратор.АдресЗамера КАК АдресЗамера
	|ИЗ
	|	РегистрСведений.ГрафикЗамеров КАК ГрафикЗамеров
	|ГДЕ
	|	ГрафикЗамеров.Период МЕЖДУ &НачалоПериода И &КонецПериода
	|	И ГрафикЗамеров.Подразделение = &Подразделение
	|	И ГрафикЗамеров.Сотрудник В(&СписокЗамерщиков)";
...
Функция ПолучитьЗамер(Период, Подразделение, Замерщик)
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ГрафикЗамеровСрезПоследних.Регистратор
	|ИЗ
	|	РегистрСведений.ГрафикЗамеров.СрезПоследних(
	|			&Период,
	|			Подразделение = &Подразделение
	|				И Сотрудник = &Замерщик) КАК ГрафикЗамеровСрезПоследних";
	Результат = Запрос.Выполнить();
...
Функция ПолучитьЗамерщиков(Подразделение, Период)
	...
	|	ГДЕ
	|		ФизическиеЛица.ОсновнаяДолжность = ЗНАЧЕНИЕ(Справочник.Должности.Замерщик)
