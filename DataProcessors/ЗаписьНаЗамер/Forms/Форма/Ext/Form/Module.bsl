...
Функция ПолучитьЗамерщиков(Подразделение, Период)
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		ДолжностиСотрудниковСрезПоследних.ФизЛицо.Ссылка КАК Ссылка
	|	ИЗ
	|		РегистрСведений.ДолжностиСотрудников.СрезПоследних(
	|				,
	|				Подразделение = &Подразделение
	|					И Работает
	|					И Должность = ЗНАЧЕНИЕ(Справочник.Должности.ВедущийДизайнер)) КАК ДолжностиСотрудниковСрезПоследних
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
