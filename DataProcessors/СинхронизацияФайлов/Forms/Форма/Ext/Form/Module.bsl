...
Функция ЗагрузкаДвоичныхДанныхСервер(МассивСсылок)
	...
	Для Каждого ВерсияФайла ИЗ МассивСсылок Цикл
		Отбор = Новый Структура("ВерсияФайла", ВерсияФайла);
		Файл = РегистрыСведений.ХранимыеФайлыВерсий.Получить(Отбор);
		МассивДанных.Добавить(Файл.ХранимыйФайл.Получить());
	КонецЦикла;
...
Процедура ЗагрузкаДвоичныхДанных(ИмяТаблицы, ОбработатьСтрок)
	...
	Для Индекс = 0 По ОбработатьСтрок - 1 Цикл
		...
		МассивСсылок.Добавить(Строка.ВерсияФайла);
	КонецЦикла;
...
Функция ОбновитьФайлы(ИмяФайла)
	...
	Если КоличествоОбновить > 0 Тогда
		...
		ОбщийРазмерФайлов = Окр(ТаблицаЗначений.Итог("Размер") / 1024, 2);
...
Функция ЗаполнитьТаблицуИзБазы(фнТаблица) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ХэшБазы) ИЛИ ХэшБазы = 0 Тогда
			ХэшБазы = ОбновитьХэшФайла(Выборка.ВерсияФайла);
		КонецЕсли;
...
&НаСервере
Функция ОбновитьХэшФайла(ВерсияФайла)
	...
	ХэшСумма = ЛексСервер.ПолучитьХэшСумму(Запись.ХранимыйФайл.Получить());
