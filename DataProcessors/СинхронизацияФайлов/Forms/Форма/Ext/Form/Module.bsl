...
Процедура ПолноеОбновление(Команда)
	...
	ФайлДБФ = Новый XBase;
	СоздатьФайлДБФ(ФайлДБФ, РабочийКаталог, "hash");
	//ФайлДБФ.Записать();
	ФайлДБФ.ЗакрытьФайл();
...
Функция СинхронизацияОбязательныхФайлов(ПолноеОбновление)
	...
	Если КоличествоФайлов > 0 Тогда
		...
		ФайлДБФ = Новый XBase(РабочийКаталог + "required.dbf");
		
		// Костыль.
		
		Если НЕ ФайлДБФ.Открыта() Тогда
			СоздатьФайлДБФ(ФайлДБФ, РабочийКаталог, "required");
		КонецЕсли;
...
Процедура СохранитьНаКлиенте(ФайлДБФ, ИмяТаблицы, ОбработатьСтрок)
	...
	Для Индекс = 0 По ОбработатьСтрок - 1 Цикл
		...
		// { Васильев Александр Леонидович [25.11.2016]
		// Сделать. Что это за строки такие...
		...
		Если ФайлДБФ.Открыта() Тогда
			...
			Пока НЕ ФайлДБФ.ВКонце() Цикл
				...
				// Не понимаю, почему не ребоатет ВКонце();
				Если ФайлДБФ.НомерЗаписи() = ФайлДБФ.КоличествоЗаписей() Тогда
					Прервать;
				КонецЕсли;
...
Процедура ДополнитьТаблицу()
	...
	ЗаполнитьТаблицуИзБазы(тз, ВидыФайлов);
...
Функция СоздатьФайлДБФ(ФайлДБФ, фнКаталог, ИмяФайла) Экспорт
	...
	ФайлДБФ.СоздатьФайл(фнКаталог + ИмяФайла +".dbf", РабочийКаталог + ИмяФайла + ".cdx");
