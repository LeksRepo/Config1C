...
Процедура ОтменитьВсе(Команда = Неопределено)
	...
	Для каждого Элемент Из ТаблицаЗначений Цикл
		Элемент.Обновить = Найти(Элемент.Имя, "HTML") ИЛИ Найти(Элемент.Имя, "Flash");
	КонецЦикла;
...
Процедура ФайлыНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.ЭтоИзделие Тогда
			Имя = Строка(Выборка.ВидФайла) + СокрЛП(Выборка.Код); //СокрЛП(Выборка.Владелец.ВладелецФайла);//+"."+Выборка.Расширение;
		Иначе
...
&НаКлиенте
Процедура ТаблицаЗначенийОбновитьПриИзменении(Элемент)
	
	Данные = Элементы.ТаблицаЗначений.ТекущиеДанные;
	Если Найти(Данные.Имя, "HTML") ИЛИ Найти(Данные.Имя, "Flash") Тогда
