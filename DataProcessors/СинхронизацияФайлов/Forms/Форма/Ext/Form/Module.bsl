...
Процедура Обновить(Команда = Неопределено)
	...
	Для Каждого ВидимаяСтрока Из ВидимаяТаблица Цикл
		...
		Если ВидимаяСтрока.Тип = 1 И (НЕ ВидимаяСтрока.Обновить) Тогда
			 ...
			 СтрокиДляУдаления = ТаблицаЗначений.НайтиСтроки(Новый Структура("Группа", ВидимаяСтрока.Группа));
...
Процедура ФайлыНаСервере()
	...
	|	ВЫБОР
	|		КОГДА ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВладелецФайла ССЫЛКА Справочник.Номенклатура
	|			ТОГДА ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВладелецФайла.Родитель
	|		ИНАЧЕ NULL
	|	КОНЕЦ КАК Группа
	|ИЗ
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА Т.Группа = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА ""Файлы изделий""
	|		ИНАЧЕ Т.Группа
	|	КОНЕЦ КАК Группа,
