...
Процедура ПриходныйОрдер(ТабДок, Ссылка) Экспорт
	...
	Для каждого Документ Из Ссылка Цикл
		...
		|	АвансовыйОтчетСписокПлатежей.Субконто1 КАК Номенклатура,
		|	АвансовыйОтчетСписокПлатежей.Субконто1.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
		...
		|   СпНоменклатуры.ЭтоМатериал Как ЭтоМатериал,
		...
		Пока Выборка.Следующий() Цикл
			...
			СписокПлатежи = Новый Массив;
	
			Для Каждого СтрокаНом Из СписокНоменклатуры Цикл
				
				Если СтрокаНом.ЭтоМатериал Тогда
				...
				Иначе
					...
					СписокПлатежи.Добавить(СтрокаНом);
			...
			Если СписокПлатежи.Количество() > 0 Тогда
				
				ТабДок.Вывести(ОбластьРазделитель);
				ТабДок.Вывести(ОбластьСписокПлатежейШапка);
								
				Для Каждого Строка Из СписокПлатежи Цикл
...
Функция ЗаполнитьМассивНоменклатуры(Выборка)
	...
	Пока Выборка.Следующий() Цикл
		...
		Стр.Вставить("ЭтоМатериал", Выборка.ЭтоМатериал);
