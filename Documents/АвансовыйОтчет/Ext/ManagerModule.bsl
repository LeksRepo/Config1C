	...
	Пока Выборка.Следующий() Цикл
		...
		Если СписокНаОсновании.Количество() > 0 Тогда
			...
			Для Каждого Строка Из СписокНаОсновании Цикл
				...
				ОбластьСписокПлатежей.Параметры.Цена = Строка.Цена;
				ОбластьСписокПлатежей.Параметры.ПлановаяЦена = Строка.ПлановаяЦена;
				ОбластьСписокПлатежей.Параметры.Сумма = Окр(Строка.Количество*Строка.Цена, 2);

				Отклонение = Строка.Количество * Строка.Цена - Строка.Количество*Строка.ПлановаяЦена; 
				ОбластьСписокПлатежей.Параметры.Отклонение = Окр(Отклонение, 2);
		...
		ИтогПлатежей.Параметры.Отклонение = Окр(СуммаОтклонение, 2);
		...
		Если СписокНеКупленных.Количество() > 0 Тогда
			...
			Для Каждого Строка Из СписокНеКупленных Цикл
				...
				ОбластьСписокПлатежей.Параметры.Заполнить(Строка);
				ОбластьСписокПлатежей.Параметры.Цена = Строка.ПлановаяЦена;
				ОбластьСписокПлатежей.Параметры.ПлановаяЦена = Строка.ПлановаяЦена;
				ОбластьСписокПлатежей.Параметры.Сумма = Окр(Строка.Количество * Строка.ПлановаяЦена, 2);
...
Процедура ДобавитьВСписок(Элемент, Массив, Количество)
	...
	Если Количество > 0 Тогда 
		...
		Если НЕ Флаг Тогда
			...
			Стр.НомерСтроки = Массив.Количество() + 1;
			Массив.Добавить(Стр);
