...
Функция ПолучитьОстаткиКомплектацииСпецификаций(СсылкаСпецификация)
	...
	|	СписоПодЗаказ.Комментарий КАК Комментарий
	|ИЗ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Спецификация.СписокМатериаловПодЗаказ КАК СписоПодЗаказ
	|		ПО ((ВЫРАЗИТЬ(УправленческийОстатки.Субконто2 КАК Справочник.Номенклатура)) = СписоПодЗаказ.Номенклатура)
	|			И (СписоПодЗаказ.Ссылка = &Ссылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	МестоОбработки УБЫВ,
	|	Наименование";
