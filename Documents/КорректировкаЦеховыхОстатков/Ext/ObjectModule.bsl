...
Процедура ПересчитатьСуммуДокумента()
	...
	|ВЫБРАТЬ
	|	СУММА(ВЫБОР
	|			КОГДА ЦеныНоменклатурыСрезПоследних.Номенклатура.Базовый
	|				ТОГДА ЦеныНоменклатурыСрезПоследних.ПлановаяЗакупочная * (ВТ_Список.Оприходовать - ВТ_Список.Списать)
	|			ИНАЧЕ ЦеныНоменклатурыСрезПоследних.ПлановаяЗакупочная * ЦеныНоменклатурыСрезПоследних.Номенклатура.КоэффициентБазовых * (ВТ_Список.Оприходовать - ВТ_Список.Списать)
	|		КОНЕЦ) КАК Сумма
	|ИЗ
	...
	|				ИЛИ ВТ_Список.Номенклатура = ЦеныНоменклатурыСрезПоследних.Номенклатура.БазоваяНоменклатура)";
	
	Если Запрос.Выполнить().Выгрузить().Количество() > 0 Тогда
		
		СуммаДокумента = Запрос.Выполнить().Выгрузить()[0].Сумма;
