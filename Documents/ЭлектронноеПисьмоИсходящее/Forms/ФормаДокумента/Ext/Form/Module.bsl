...
Процедура ДобавитьВложениеВыполнить()
	...
	#Иначе
		...
		Адрес = "";
		ВыбранныйФайл = "";
		ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ПомещениеФайлаПриОкончании", ЭтаФорма);
...
Процедура СвойстваВложения(Команда)
	...
	Если ТекущиеДанные.Ссылка = Неопределено Тогда
		...
		ОбработчикОповещенияОЗакрытии = Новый ОписаниеОповещения("ВопросОЗаписиФайлаПослеЗакрытия", ЭтаФорма, ДополнительныеПараметры);
...
Процедура ОпределитьСпособРедактированияПисьма()
	...
	Если ФорматСообщения = Перечисления.СпособыРедактированияЭлектронныхПисем.HTML Тогда
		...
		ИначеЕсли ПараметрыРаботыПользователяПользователя.Подпись <> Неопределено Тогда
			...
			ЭлементыПодписи = ПараметрыРаботыПользователяПользователя.Подпись.ПолучитьЭлементы();
			
			Для Каждого ЭлементПодписи Из ЭлементыПодписи Цикл
				Если ТипЗнч(ЭлементПодписи) = Тип("ТекстФорматированногоДокумента") Тогда
					ДобавленныйЭлемент = ТекстПисьмаФорматированныйДокумент.Добавить(ЭлементПодписи.Текст, ТипЗнч(ЭлементПодписи));
					ЗаполнитьЗначенияСвойств(ДобавленныйЭлемент, ЭлементПодписи,"ЦветТекста, ЦветФона, Шрифт");
				ИначеЕсли ТипЗнч(ЭлементПодписи) = Тип("КартинкаФорматированногоДокумента") Тогда
					ТекстПисьмаФорматированныйДокумент.Добавить(ЭлементПодписи.Картинка, ТипЗнч(ЭлементПодписи));
				Иначе
					ТекстПисьмаФорматированныйДокумент.Добавить(Неопределено, ТипЗнч(ЭлементПодписи));
				КонецЕсли;
			КонецЦикла;
...
Процедура ОтправитьВыполнить()
	...
	Если НайденныеСтроки[0].УдалятьПослеОтправки Тогда
			...
			ОбработчикОповещенияЗакрытия = Новый ОписаниеОповещения("ВопросОНеСохраненииОтправленногоПисьма", ЭтаФорма);
