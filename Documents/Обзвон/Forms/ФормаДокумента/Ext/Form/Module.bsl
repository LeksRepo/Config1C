...
Процедура ЗаполнитьДилеровНаСервере()
	...
	Объект.СписокОбзвонаДилеров.Очистить();
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока = Объект.СписокОбзвонаДилеров.Добавить();
...
&НаКлиенте
Процедура СписокОбзвонаДилеровПередУдалением(Элемент, Отказ)
	
	Режим = РежимДиалогаВопрос.ДаНет;
	Текст = "Удалить обзвон по данному дилеру?";
	
	Если Объект.СписокОбзвонаДилеров.Количество() > 0 
		И Вопрос(Текст, Режим, 0) = КодВозвратаДиалога.Нет Тогда
		Отказ = Истина;
	КонецЕсли;
