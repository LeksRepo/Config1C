...
Процедура ЗаполнитьНаСервере()
	...
	|	ОстаткиВЦеху.КоличествоОстатокДт КАК ОстатокВЦехуСУчетомЛимита
	...
	|	ВТТребуетсяПоСпецификациям.КоличествоВсегоТребуется - ЕСТЬNULL(ВТЦеховыеОстатки.ОстатокВЦехуСУчетомЛимита, 0) - ЕСТЬNULL(ВТОстаткиНаСкладе.Количество, 0) КАК Количество
	...
	|ГДЕ
	|	ВТТребуетсяПоСпецификациям.КоличествоВсегоТребуется - ЕСТЬNULL(ВТЦеховыеОстатки.ОстатокВЦехуСУчетомЛимита, 0) - ЕСТЬNULL(ВТОстаткиНаСкладе.Количество, 0) > ЕСТЬNULL(ВТОстаткиНаСкладе.Количество, 0)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|						ТОГДА ВЫРАЗИТЬ(втТребуетсяПоСпецификациям.КоличествоСрочно / ЕСТЬNULL(спрНоменклатура.КоэффициентБазовых, 1) + 0.49999 КАК ЧИСЛО(15, 0))
	|					ИНАЧЕ ВЫРАЗИТЬ(втМатериалыДляЗакупа.Количество / ЕСТЬNULL(спрНоменклатура.КоэффициентБазовых, 1) + 0.49999 КАК ЧИСЛО(15, 0))
	|				КОНЕЦ
	|		ИНАЧЕ 0
	|	КОНЕЦ КАК КоличествоСрочно,
...
Процедура РассчитатьСуммы()
	...
	Для Каждого Строка Из Объект.СписокНоменклатуры Цикл
		...
		Строка.Стоимость = Строка.РучнойВвод * Строка.Цена;
		
		СуммаЗаявленная = СуммаЗаявленная + Строка.РучнойВвод * Строка.Цена;
		СуммаЗакупа = СуммаЗакупа + Строка.Количество * Строка.Цена;
		СуммаСрочно = СуммаСрочно + Строка.КоличествоСрочно * Строка.Цена;
...
Процедура ПриОткрытии(Отказ)
	...
	// { Васильев Александр Леонидович [12.02.2015]
	РассчитатьСуммы();
	// Рома, это пиздец полный. Какого хуя?
	// Документ открывается только для чтения,
	// а ты пытаешься его реквизиты менять.
	// } Васильев Александр Леонидович [12.02.2015]
	
	
	// { Васильев Александр Леонидович [12.02.2015]
	// Нахера это нужно при пустом документе?
	// И вообще почему не в ПриСозданииНаСеревере?
	// } Васильев Александр Леонидович [12.02.2015]
	ПолучитьРасшифровку();
