...
Функция ПересчитатьСуммыДокумента(ПодЗаказ)
	...
	Если ПодЗаказ Тогда
		Для каждого Строка Из Объект.НоменклатураПодЗаказ Цикл
			Объект.СуммаПодотчет = Объект.СуммаПодотчет + Строка.КоличествоАвансовый * Строка.Цена;
...
Процедура ЗаполнитьНаСервере()
	...
	|	ЕСТЬNULL(НоменклатураПодразделений.ЗакупОптом, ИСТИНА) КАК ЗакупОптом,
