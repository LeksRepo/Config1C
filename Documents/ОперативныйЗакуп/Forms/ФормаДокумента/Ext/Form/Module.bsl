...
Процедура ЗаполнитьНаСервере()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	втОстаткиРегистр.Номенклатура КАК Номенклатура,
	|	СУММА(втОстаткиРегистр.Количество) КАК Количество
	|ПОМЕСТИТЬ втОстаткиНаСкладе
	|ИЗ
	|	втОстаткиРегистр КАК втОстаткиРегистр
	|
	|СГРУППИРОВАТЬ ПО
	|	втОстаткиРегистр.Номенклатура
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ВЫБРАТЬ
	|	ВЫБОР
	|		КОГДА НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу ЕСТЬ NULL 
	|				ИЛИ НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА НарядЗаданиеСписокЛистовНоменклатуры.Номенклатура
	|		ИНАЧЕ НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу
	|	КОНЕЦ КАК Номенклатура,
	|	СУММА(НарядЗаданиеСписокЛистовНоменклатуры.Количество) КАК Количество,
	...
	|		КОГДА НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу ЕСТЬ NULL 
	|				ИЛИ НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|			ТОГДА СпецификацияСписокНоменклатуры.Номенклатура
	|		ИНАЧЕ НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу
	|	КОНЕЦ КАК Номенклатура,
	|	СУММА(СпецификацияСписокНоменклатуры.Количество / ЕСТЬNULL(НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу.КоэффициентБазовых, 1)) КАК КоличествоВсегоТребуется
	...
	|	ВЫРАЗИТЬ(ВТТребуетсяПоСпецификациям.КоличествоВсегоТребуется - ЕСТЬNULL(ВТОстаткиНаСкладе.Количество, 0) + 0.49999 КАК ЧИСЛО(15, 0)) КАК Количество
	...
	|	втМатериалыДляЗакупа.Номенклатура,
	...
	|	ВТМатериалыСоСтоимостью.Номенклатура,
