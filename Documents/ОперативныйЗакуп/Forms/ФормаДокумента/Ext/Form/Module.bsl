...
Процедура ЗаполнитьНаСервере()
	...
	Запрос.УстановитьПараметр("НарядЗадания", Объект.НарядЗадания.Выгрузить(Неопределено, "Наряд"));
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Список.Спецификация КАК Спецификация
	|ПОМЕСТИТЬ списокСпецНарядЗадания
	|ИЗ
	|	Документ.НарядЗадание.СписокСпецификаций КАК Список
	|ГДЕ
	|	Список.Ссылка В (&НарядЗадания)	
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ПОМЕСТИТЬ номНарядЗадания
	|ИЗ
	|	Документ.Спецификация.СписокНоменклатуры КАК Список
	|ГДЕ
	|	Список.Ссылка В (ВЫБРАТЬ
	|						т.Спецификация
	|					 ИЗ
	|						списокСпецНарядЗадания КАК т)
	|   И Список.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)
	|	И НЕ Список.ПредоставитЗаказчик
	|
	|СГРУППИРОВАТЬ ПО
	|	Список.Номенклатура
	|;
	...
	|			ТОГДА НоменклатураНаряды.Номенклатура
	|		ИНАЧЕ УправленческийОстатки.Субконто2
	|	КОНЕЦ КАК Номенклатура,
	|	ЕСТЬNULL(УправленческийОстатки.КоличествоРазвернутыйОстатокДт, 0) + ЕСТЬNULL(НоменклатураНаряды.Количество, 0) КАК Количество
	...
	|		ПОЛНОЕ СОЕДИНЕНИЕ номНарядЗадания КАК НоменклатураНаряды
	|		ПО ((ВЫРАЗИТЬ(УправленческийОстатки.Субконто2 КАК Справочник.Номенклатура)) = НоменклатураНаряды.Номенклатура)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
&НаКлиенте
Процедура НарядЗаданиеПриИзменении(Элемент)
	ПередЗаполнитьНаСервере();
КонецПроцедуры
