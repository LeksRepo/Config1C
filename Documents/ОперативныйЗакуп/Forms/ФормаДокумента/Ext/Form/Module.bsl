...
Процедура РассчитатьСуммы()
	...
	Для Каждого Строка Из Объект.СписокНоменклатурыПодЗаказ Цикл
		...
		Строка.Стоимость = Окр(Строка.РучнойВвод * Строка.Цена);
		
		СуммаЗаявленная = СуммаЗаявленная + Окр(Строка.РучнойВвод * Строка.Цена);
		СуммаЗакупа = СуммаЗакупа + Окр(Строка.Количество * Строка.Цена);
		СуммаСрочно = СуммаСрочно + Окр(Строка.КоличествоСрочно * Строка.Цена);
	...
	Объект.СуммаЗаявленная = СуммаЗаявленная;
	Объект.СуммаДокумента = СуммаЗакупа;
	Объект.СуммаДокументаСрочно = СуммаСрочно;
