...
Процедура ЗаполнитьНаСервере()
	...
	|	РезервныйЗапас.Количество / ЕСТЬNULL(НоменклатураПодразделенийСрезПоследних.ОсновнаяПоСкладу.КоэффициентБазовых, 1) КАК Количество
	...
	|		ПО РезервныйЗапас.Номенклатура = НоменклатураПодразделенийСрезПоследних.Номенклатура
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура
	|;
	...
	|	ВЫРАЗИТЬ(ВТРезервныйЗапас.Количество + 0.49999 КАК ЧИСЛО(15, 0)) КАК Количество
