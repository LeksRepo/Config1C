...
Процедура ЗаполнитьНаСервере()
	...
	|	СУММА(ВТТребуетсяПоСпецификациям.КоличествоВсегоТребуется - ЕСТЬNULL(ВТЦеховыеОстатки.ОстатокВЦехуСУчетомЛимита, 0)) КАК Количество,
	...
	|	КОНЕЦ КАК Номенклатура,
	|	СУММА(ЕСТЬNULL(ВТДокупитьНаСклад.Количество, 0) + ЕСТЬNULL(ВТДокупитьНаЦех.Количество, 0) - ЕСТЬNULL(ВТДокупитьНаСклад.ОстатокНаСкладе, 0)) КАК Количество,
	|	ВТТребуетсяПоСпецификациям.СсылкаСпец КАК СсылкаСпец,
	|	ВТТребуетсяПоСпецификациям.Поставщик КАК Поставщик
