...
Процедура ЗаполнитьНаСервере()
	...
	Для Каждого Стр ИЗ ТЗНом Цикл
		...
		НоваяСтрока.КоличествоСтрокНом = 1;
	...
	Для Каждого Стр ИЗ ТЗНомПодЗаказ Цикл
		...
		НоваяСтрока.КоличествоСтрокНомПодЗаказ = 1;
...
Процедура НоменклатураПодЗаказКоличествоКупитьПриИзменении(Элемент)
	...
	ПересчитатьСуммыДокумента(Истина);
	
КонецПроцедуры

&НаКлиенте
Процедура НоменклатураПодЗаказКоличествоПоставщикПриИзменении(Элемент)
...
Процедура УстановитьОтборРасшифровки()
	...
	Если Элементы.ГруппаТаблицы.ТекущаяСтраница.Имя = "СтраницаПодЗаказ" Тогда
		Данные = Элементы.НоменклатураПодЗаказ.ТекущиеДанные;
		ПодЗаказ = Истина;
	ИначеЕсли Элементы.ГруппаТаблицы.ТекущаяСтраница.Имя = "СтраницаСписокНоменклатуры" Тогда
...
Процедура УстановитьОтборНоменклатуры()
	...
	ОтборСтрок = Новый ФиксированнаяСтруктура("ПодЗаказ", Ложь);
	Элементы.СписокНоменклатуры.ОтборСтрок = ОтборСтрок;
	
	ОтборСтрок = Новый ФиксированнаяСтруктура("ПодЗаказ", Истина);
	Элементы.НоменклатураПодЗаказ.ОтборСтрок = ОтборСтрок;
