...
Функция ПолучитьСтрокуКантов(ВидМебельнойКромки)
	...
	ИначеЕсли ВидМебельнойКромки = "Кромка2*35мм" Тогда
		
		СписокНоменклатурныхГрупп.Добавить(Справочник.Кромка2_35);
...
Процедура ФлэшПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	ПолученоОтФлэш = ОтФлэшСтруктура.ПолученоОтФлэш;
	ЛДСП = (Материал = "10 ЛДСП" ИЛИ Материал = "16 ЛДСП" ИЛИ Материал = "22 ЛДСП" ИЛИ Материал = "26 ЛДСП");
	...
	Если Команда = "kant" Тогда
		...
		Если ЛДСП или Материал = "ФасадЛДСП" Тогда
			...
			Если Материал = "10 ЛДСП" Тогда
				...
				ОтправитьВФлэш("kant☻Кромка045мм");
				
			ИначеЕсли (Материал = "22 ЛДСП" ИЛИ Материал = "26 ЛДСП") Тогда
				
				ОтправитьВФлэш("kant☻Кромка2*35мм");
	...
	ИначеЕсли Команда = "knam" Тогда
		...
		ИначеЕсли ПолученоОтФлэш = "Кромка2*35мм" Тогда
			
			ОтправитьВФлэш("knam☻"+ПолучитьСтрокуКантов("Кромка2*35мм"));
	...
	ИначеЕсли Команда = "prin" Тогда
		...
		ПечатьИзРедактора(ПолученоОтФлэш);
...
&НаКлиенте
Процедура ПечатьИзРедактора(Данные)
	
	//ПараметрыФормы = Новый Структура;
	//ПараметрыФормы.Вставить("Спецификация", ПредопределенноеЗначение("Документ.Спецификация.ПустаяСсылка"));
	//ПараметрыФормы.Вставить("ХранимыйФайл", "КривойПил");
	//ПараметрыФормы.Вставить("СтрокаКривогоПила", Данные);
	//
	...
	//УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.Спецификация", "КриволинейныеДетали", ПараметрКоманды, ПараметрыВыполненияКоманды, ПараметрыПечати);
