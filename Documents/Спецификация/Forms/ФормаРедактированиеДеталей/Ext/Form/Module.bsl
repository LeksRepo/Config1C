...
Функция ПолучитьСтрокуКантов(ВидМебельнойКромки)
	...
	МассивыНоменклатурныхГрупп = ЛексСервер.ОтборНоменклатурныхГруппРедактор(СписокНоменклатурныхГрупп, Подразделение);
		...
		Для каждого Массив Из МассивыНоменклатурныхГрупп Цикл
			Для каждого Элемент Из Массив.Значение Цикл
				Если Элемент.ПодЗаказ = Заказ Тогда
					Строка = Строка + Элемент.Ссылка.КраткоеНаименование + "@" + Элемент.Ссылка.Наименование + "_" + Элемент.Ссылка.Код + "_" + ?(Элемент.ПодЗаказ,1,0) + "|";
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
