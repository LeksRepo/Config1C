...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.ТолькоПросмотр = Неопределено Тогда
		...
		Элементы.ФормаПечатьДетали.Видимость = ТолькоПросмотр; 
	КонецЕсли;	
	...
	ИмяSWF = Справочники.Файлы.Редактор2DFlash.Наименование + "." + Справочники.Файлы.Редактор2DFlash.ТекущаяВерсия.Расширение;
...
Процедура ПередЗакрытием(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ТолькоПросмотр Тогда
		...
		СтрокаДляСравнения1 = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ПолученоОтФлэш, "☻"); 
		Если СтрокаДляСравнения1.Количество()>8 Тогда
			СтрокаДляСравнения1 = СтрокаДляСравнения1[8];
		Иначе
			СтрокаДляСравнения1 = "";
		КонецЕсли;
		
		СтрокаДляСравнения2 = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаДляФлэш, "☻");
		
		Если СтрокаДляСравнения2.Количество()>1 Тогда
			СтрокаДляСравнения2 = СтрокаДляСравнения2[1];
		Иначе
			СтрокаДляСравнения2 = "";
		КонецЕсли;
		
		Если СтрокаДляСравнения1 <> СтрокаДляСравнения2 Тогда
...
Процедура ФлэшДокументСформирован(Элемент)
	...
	ЭтаФорма.Модифицированность = НЕ ТолькоПросмотр;
...
&НаКлиенте
Процедура ПечатьДетали(Команда)
	
	Мас = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаДляФлэш, "☻");
	Стр = ?(Мас.Количество()>1, Мас[1], Мас[0]);
	ОповеститьОВыборе("prin☻" + Стр);
