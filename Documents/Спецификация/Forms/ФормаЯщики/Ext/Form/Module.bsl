...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка Из Детали Цикл
		...
		СтруктураПараметровЯщика = Документы.Спецификация.ПолучитьПараметрыЯщика(Строка, ЕстьРебро);
...
Процедура РучкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//
	//НомГруппа = ПредопределенноеЗначение("Справочник.НоменклатурныеГруппы.Ручка");
	//
	//Пар = Новый Структура();
	//Пар.Вставить("Режим", 1);
	//Пар.Вставить("НомГруппа", НомГруппа);
	//Пар.Вставить("Производство", Подразделение);	
	//ОткрытьФорму("Справочник.Номенклатура.Форма.ФормаПодборКомплектации", Пар, Элементы.Ручка);
