...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаполнитьДанныеСхемыДляВсехСтрок();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьДанныеСхемыДляВсехСтрок()
	
	Для Каждого Строка Из Детали Цикл
		
		Если ЗначениеЗаполнено(Строка.СхемаЯщика) Тогда
			
			Строка.ЕстьДно = Ложь;
			
			Для Каждого Деталь Из Строка.СхемаЯщика.Детали Цикл
				     Если Деталь.Тип = Перечисления.ВидыДеталейЯщиков.Дно Тогда		
						Строка.ЕстьДно = Истина;
						Прервать;	
					КонецЕсли;		
			КонецЦикла;
				
			Если НЕ Строка.ЕстьДно Тогда
				Строка.ДноНоменклатура = Неопределено;	
			КонецЕсли;
...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка Из Детали Цикл
		...
		Если Строка.ЕстьДно И НЕ ЗначениеЗаполнено(Строка.ДноНоменклатура) Тогда
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	Если (Данные <> Неопределено) Тогда
		...
		Если ЗначениеЗаполнено(Данные.СхемаЯщика) Тогда 
			...
			Данные.ЕстьДно = ДанныеСхемы.ЕстьДно;
...
Процедура ДоступностьНастроек()
	...
	Если Данные <> Неопределено Тогда
		...
		Если ЗначениеЗаполнено(ВидЯщика) Тогда
			...
			Элементы.ЕстьРебро.Доступность = Данные.ЕстьДно;
			Элементы.Дно.Доступность = Данные.ЕстьДно;
