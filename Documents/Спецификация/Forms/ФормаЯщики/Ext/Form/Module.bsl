	...
	Если НовыеЯщики Тогда		
		...
		Если РассчитатьФасад Тогда
			Если ЗначениеЗаполнено(Данные.ВысотаЯщика) И ЗначениеЗаполнено(Данные.ПроемЯщика) И Данные.ВидФасада <> "Нет" Тогда
				...
				Выполнить("Данные.ВысотаФасад = " + ПодставитьОбъектВФормулу("Данные", ДанныеСхемы.ВысотаФасада)); 
				Выполнить("Данные.ШиринаФасад = " + ПодставитьОбъектВФормулу("Данные", ДанныеСхемы.ШиринаФасада)); 
...
&НаКлиенте
Функция ПодставитьОбъектВФормулу(Знач Ссылка, Знач Формула)
	...
	Для Каждого Эл Из МассивПеременныхФормулы Цикл
		Формула = СтрЗаменить(Формула,Эл,Ссылка+"."+Эл);	
	КонецЦикла;
...
Процедура УстановитьГлубинуЯщика()
	...
	Если Данные.ПроемЯщика > 0 И Данные.ВысотаЯщика > 0 Тогда
		...
		Выполнить("Данные.ГлубинаЯщика = " + ПодставитьОбъектВФормулу("Данные", ДанныеСхемы.ГлубинаЯщика));	
