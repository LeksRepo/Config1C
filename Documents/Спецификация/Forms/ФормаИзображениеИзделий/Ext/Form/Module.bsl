...
Процедура ПриСозданииНовая(Параметры) 
	...
	Если КоличествоИзделий > 0 Тогда
		...
		Для Каждого Строка Из СписокИзделий Цикл
			Если Строка.Изделие.ВидИзделия = Перечисления.ВидыИзделийПоКаталогу.КухняВерхний Тогда
				Группа = Элементы.ГруппаВерх;
				Элементы.Декорация1.Видимость = Истина;
				РазмерВерх = РазмерВерх + Строка.ШиринаИзделия;
			ИначеЕсли Строка.Изделие.ВидИзделия = Перечисления.ВидыИзделийПоКаталогу.КухняНижний Тогда
			...
			НовыйЭлементКартинки.Вид = ВидПоляФормы.ПолеКартинки;
			НовыйЭлементКартинки.РазмерКартинки = РазмерКартинки.Пропорционально;
			//НовыйЭлементКартинки.Заголовок = "(" + Строка.ШиринаИзделия + "; " + Строка.ВысотаИзделия + "; " + Строка.ГлубинаИзделия + ")";
...
Процедура ПриОткрытииНовая()
	...
	Для Каждого Строка Из СписокИзделий Цикл
		...
		КонецЕсли;
		Этаформа[НашРазмер]  = "Размеры: " + Строка.ГлубинаИзделия + ";" + Строка.ШиринаИзделия + ";" + Строка.ВысотаИзделия;
