...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если КоличествоИзделий > 0 Тогда                         
		...
		Для Каждого Строка Из СписокИзделий Цикл
			Если Строка.Изделие.ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.КухняВерхний Тогда
				Группа = Элементы.ГруппаВерх;
				Элементы.Декорация1.Видимость = Истина;
				РазмерВерх = РазмерВерх + Строка.ШиринаИзделия;
			ИначеЕсли Строка.Изделие.ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.КухняНижний Тогда
