...
&НаСервереБезКонтекста
Функция НайтиНовуюСтрокуФлэш(Документ, Команда, Данные = Неопределено)
	...
	ИначеЕсли Команда = "recr" Тогда
		...
		СтруктураРаскроя = РегистрыСведений.РаскройДеталей.СформироватьРаскрой(Документ, ТЗ);
...
Процедура КнопкаПоказатьНовыйРаскройПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если КомандаФлэш = "init" Тогда
		...
		Если Наряды Тогда
			...
			Если Элементы.СписокНарядов.ТекущиеДанные <> Неопределено Тогда
				СсылкаНаряд = Элементы.СписокНарядов.ТекущиеДанные.Ссылка;
				НоваяСтрока = НайтиНовуюСтрокуФлэш(СсылкаНаряд, КомандаФлэш);
	...
	ИначеЕсли КомандаФлэш = "print" Тогда
		...
		Если ЗначениеЗаполнено(СтрокаРедактированнаяФлэш) Тогда
			СсылкаДокумента = ?(Наряды, Элементы.СписокНарядов.ТекущиеДанные.Ссылка, Элементы.Список.ТекущиеДанные.Ссылка);
...
Процедура ВыполнитьПерераскрой()
	...
	СсылкаДокумента = ?(Наряды, Элементы.СписокНарядов.ТекущиеДанные.Ссылка, Элементы.Список.ТекущиеДанные.Ссылка);
	ДанныеОтФлэш = НайтиНовуюСтрокуФлэш(СсылкаДокумента, "recr", ДанныеОтФлэш);
