...
&НаСервере
Функция УстановитьПараметрыЗапроса(НачалоПериода, КонецПериода, НомерДокумента = "", НайденныеДокументы = "")
	
	Если НЕ Наряды Тогда
		
		Список.Параметры.УстановитьЗначениеПараметра("ОтборДокументов", НЕ ЗначениеЗаполнено(НайденныеДокументы));
	...
	Иначе
		...
		СписокНарядов.Параметры.УстановитьЗначениеПараметра("ОтборДокументов", НЕ ЗначениеЗаполнено(НайденныеДокументы));
...
Функция УстановитьОтборПоПериоду(Текущий, НомерДокумента = "", Наряды = Ложь)
	...
	Если ЗначениеЗаполнено(НомерДокумента) Тогда
		...
		ВведенныйНомерСНулями = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(НомерДокумента, 10, "0", "Слева");
		НайденныеДокументы = ПолучитьДокументПоНомеру(ВведенныйНомерСНулями, Наряды);
		
		Если ЗначениеЗаполнено(НайденныеДокументы) Тогда
			//УстановитьПараметрыЗапроса(НачалоПериода, КонецПериода,"_"+ВведенныйНомерСНулями);
			УстановитьПараметрыЗапроса(НачалоПериода, КонецПериода,,НайденныеДокументы);  //!!!!
		Иначе
...
Функция ПолучитьДокументПоНомеру(НомерДокумента, Наряды)
	...
	Если Наряды Тогда
		...
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("НомерДокумента", "_"+НомерДокумента);
	...
	Иначе
		...
		|	И ДокументСпецификация.Номер ПОДОБНО &НомерДокумента";
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда
			...
			КонецЦикла;
		//Иначе
		//	  МассивДокументов.Добавить(СпецификацияСсылка);
		КонецЕсли;
