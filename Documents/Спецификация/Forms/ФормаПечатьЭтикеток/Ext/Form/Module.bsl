...
Функция НайтиНовуюСтрокуФлэш(Документ, Команда, Данные = Неопределено, Наряды = Ложь)
	...
	Отбор = Новый Структура("Объект", Документы);
	...
	Если Команда = "init" Тогда
		...
		Если НЕ ЗначениеЗаполнено(НашаСтрокаРаскроя) Тогда
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Раскрой на Наряд № %1 не сформирован", ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Документ.Номер));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
			Возврат "";
		КонецЕсли;
