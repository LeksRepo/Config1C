...
Функция СохранитьСтрокуЭтикеток(СсылкаДокумента, СтрокаРедактированнаяФлэш)
	...
	Если ЗначениеЗаполнено(СсылкаДокумента) Тогда
		...
		Если НаборЗаписей.Количество() = 1 Тогда
			...
			ТекущаяСтрока = НаборЗаписей[0].СтрокаЭтикеток;
			НоваяСтрока = ДополнитьСтрокуЭтикеток(ТекущаяСтрока,СтрокаРедактированнаяФлэш); 
			НаборЗаписей[0].СтрокаЭтикеток = НоваяСтрока;
...
&НаСервереБезКонтекста
Функция ДополнитьСтрокуЭтикеток(ТекущаяСтрока, СтрокаРедактированнаяФлэш)

	ТекущийМассив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ТекущаяСтрока, "^");
	НовыйМассив = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрокаРедактированнаяФлэш, "^");
	
	Для Каждого Эл ИЗ НовыйМассив Цикл
		
		Если НЕ (ТекущийМассив.Найти(Эл) <> Неопределено) Тогда
			ТекущийМассив.Добавить(Эл);			
		КонецЕсли;
		
	КонецЦикла;
	
	Возврат СтроковыеФункцииКлиентСервер.ПолучитьСтрокуИзМассиваПодстрок(ТекущийМассив, "^");
