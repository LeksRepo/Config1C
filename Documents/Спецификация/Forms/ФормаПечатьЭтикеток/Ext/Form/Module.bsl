...
&НаСервереБезКонтекста
Функция НайтиНовуюСтрокуФлэш(Спецификация)
	...
	Возврат ЛексКлиентСервер.ПеревестиСтрокуВКодыСимволов(Выборка.СтрокаРаскрой);
...
Процедура КнопкаПоказатьРаскройПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если КомандаФлэш = "init" Тогда
		...
		Если Элементы.Список.ТекущиеДанные <> Неопределено Тогда
			...
			НоваяСтрока = НайтиНовуюСтрокуФлэш(СсылкаСпецификация);
			...
			//НоваяСтрока = НоваяСтрока + СтрокаПечатьЭтикеток;
			...
			//Элементы.КнопкаПоказатьРаскрой.Документ.getElementById("input").tag = НоваяСтрока;
