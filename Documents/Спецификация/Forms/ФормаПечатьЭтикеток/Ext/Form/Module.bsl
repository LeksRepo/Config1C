...
Функция УстановитьОтборПоПериоду(Текущий, НомерДокумента = Неопределено)
	...
	// позиционируемся на документе с указанным номеро
	Если ЗначениеЗаполнено(НомерДокумента) Тогда
		ВведенныйНомерСНулями = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(НомерДокумента, 9, "0", "Слева");
