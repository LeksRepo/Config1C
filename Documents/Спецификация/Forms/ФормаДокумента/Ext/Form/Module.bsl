...
Функция ЗаписатьСтрокуРаскроя(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("РисунокРаскроя") Тогда
		Запись.РисунокРаскроя = ПараметрыЗаписи.РисунокРаскроя;
	КонецЕсли;
	
	Если ПараметрыЗаписи.Свойство("РисунокКривогоПила") Тогда
		Запись.РисунокКривогоПила = ПараметрыЗаписи.РисунокКривогоПила;
	КонецЕсли;
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если ФасадСПрофилем Тогда
			...
			ИначеЕсли Материал = "ФасадАлюминиевый" Тогда
				...
				ДобавитьСтроку(ТекущийОбъект, Номенклатура.ВтулкаДистанционнаяАлюминиевая, КоличествоВтулок, Строка.НомерИзделия);
				ДопЭлемент = Номенклатура.ВтулкаДистанционнаяАлюминиевая.ДополнительныйЭлемент;
				Если ЗначениеЗаполнено(ДопЭлемент) Тогда
					ДобавитьСтроку(ТекущийОбъект, ДопЭлемент, КоличествоВтулок, Строка.НомерИзделия);
				КонецЕсли;
			...
			Если НоваяСтрока.ВидФасада <> "Нет" Тогда
				...
				Если ИзделиеШкафКупе Тогда
					...
					Если ЗначениеЗаполнено(СтруктураПодбираемойПоЦветуНоменклатуры)
						И СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("Кромка045_19")
						И СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("Кромка2_19") Тогда
