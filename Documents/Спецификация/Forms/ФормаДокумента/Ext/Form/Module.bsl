...
&НаКлиенте 
Процедура ЗаписатьРисункиРаскроя(ПараметрыЗаписи)
		...
		Для Каждого Строка Из СписокНоменклатурыИзделия Цикл
			...
			Попытка
				...
				Если НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.МонтажныйУголок и ЗначениеЗаполнено(СтруктураПодбираемойПоЦветуНоменклатуры) Тогда
					...
					Номенклатура = СтруктураПодбираемойПоЦветуНоменклатуры.МонтажныйУголок;
				...
				ИначеЕсли НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Заглушка и ЗначениеЗаполнено(СтруктураПодбираемойПоЦветуНоменклатуры) Тогда
					...
					Номенклатура = СтруктураПодбираемойПоЦветуНоменклатуры.Заглушка;
...
// Возвращает рабочий телефон Заведующего Складами Материалов из указанного подразделения
//
// Параметры:
//  Подразделение  - СправочникСсылка.Подразделение, подразделение, в котором ищем ЗаведующегоСкладамиМатериалов
//
// Возвращаемое значение:
//   ТелЗавСкладамиМатериалов   - Строка, телефон ЗаведующегоСкладамиМатериалов
&НаСервереБезКонтекста
Функция ПолучитьТелЗавСкладамиМатериалов(Подразделение)
	Запрос = Новый Запрос;
	...
	Если РезультатЗапроса.Пустой() Тогда
		ТелЗавСкладамиМатериалов = "В подразделении нет заведующего складами материалов";
	Иначе
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ТелЗавСкладамиМатериалов = Выборка.ТелефонРабочий;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ТелЗавСкладамиМатериалов;
	
КонецФункции // ПолучитьТелЗавСкладами()
...
Функция ОбновитьЭлементыФормы(лФорма, лПараметры)
	...
	Если лПараметры.Свойство("ЕстьМатериалПодЗаказ") Тогда
		Элементы.ТелЗавСкладами.Видимость = лПараметры.ЕстьМатериалПодЗаказ;
	КонецЕсли;
...
Функция ОбновитьДоступностьКоманд()
	...
	Логист = УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, Справочники.ПрофилиГруппДоступа.ЗаведующийСкладом);
	...
	Ком.ФормаПроверенаЛогистом.Доступность = Администратор ИЛИ Логист;
...
Процедура ПередатьВПроизводство(Команда)
	...
	Если ПараметрыЗаписиДокумента <> Неопределено Тогда
		ЗаписатьРисункиРаскроя(ПараметрыЗаписиДокумента);
		Закрыть();
	КонецЕсли;
...
Функция ПередатьВПроизводствоНаСервере()
	...
	Если Объект.Дилерский И Объект.ЕстьМатериалПодЗаказ И ЛексСервер.СпецификацияПроверенаЛогистом(СтатусСпецификации) Тогда
...
&НаКлиенте
Процедура ТелЗавСкладамиНажатие(Элемент, СтандартнаяОбработка)	
	СтандартнаяОбработка = Ложь;
...
#КонецОбласти
ТелЗавСкладами = "Материал под заказ. Согласуйте " + ПолучитьТелЗавСкладамиМатериалов(Объект.Подразделение);