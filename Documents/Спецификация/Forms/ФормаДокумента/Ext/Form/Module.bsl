...
&НаСервере
Функция ЗаполненыПоставщики(ТаблицаСписокМатериаловПодЗаказ)
	
	
	
	//ТаблицаСписокМатериаловПодЗаказ
...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	ТаблицаПодЗаказ.Свернуть("Номенклатура, ШиринаДетали, ДлинаДетали, Поставщик, Комментарий", "Количество");
...
Функция ОбновитьЭлементыФормы(лФорма, лПараметры)
	...
	Если лПараметры.Свойство("СоздатьЗамер") Тогда
		...
		Элементы.СоздатьЗамер.Видимость = НЕ лПараметры.СоздатьЗамер;
...
Функция ОбновитьДоступностьКоманд()
	...
	ПроверенЛогистом = НЕ Объект.ЕстьМатериалПодЗаказ ИЛИ ЛексСервер.СпецификацияПроверенаЛогистом(СтатусСпецификации) ИЛИ ЗаполненыПоставщики(Объект.СписокМатериаловПодЗаказ);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("СоздатьЗамер", ЗначениеЗаполнено(Объект.ДокументОснование) И НЕ Объект.Дилерский);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	КонецЕсли;
	Строка = ТекущийОбъект.СписокНоменклатуры.Найти(Справочники.Номенклатура.НаклеиваниеПленкиФотопечати, "Номенклатура");
	...
	Для каждого ИзделиеПоКаталогу Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
		Если ИзделиеПоКаталогу.Изделие.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.КухняВерхний ИЛИ ИзделиеПоКаталогу.Изделие.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.КухняНижний Тогда
	...
	Если ЗначениеЗаполнено(ТекущийОбъект.ДатаОтгрузки) Тогда
		...
		Если РазностьВДнях < (3 + ЕстьВоскресеньеОтСоздания + (ТекущийОбъект.ЭтоКухня ИЛИ ЕстьФотопечать)) Тогда
		...
		Иначе
			...
			КоличествоДней = ?(ТекущийОбъект.ЭтоКухня ИЛИ ЕстьФотопечать, 1 + Число(ТекущийОбъект.ЭтоКухня ИЛИ ЕстьФотопечать), 1);
