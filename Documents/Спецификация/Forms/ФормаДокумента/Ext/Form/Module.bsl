...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Иначе
		...
		Для каждого Строка Из ТекущийОбъект.СписокЯщики Цикл
			...
			//Накручивание ручки
			Если Строка.КоличествоЯщиков > 0 И Строка.КоличествоРучек > 0 И (НЕ Строка.КриволинейныйВыпилПодРучку И НЕ Строка.РучкаЩель) Тогда
...
Процедура ОбработкаИзделийПоКаталогу()
	...
	Если ИзделиеШкафКупе Тогда
		...
		СтрокаАнтПолка = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.АнтресольнаяПолка,"ВидИзделия");
		
		СтрокиОсновной = ТаблицаИзделийПоКаталогу.НайтиСтроки(Новый Структура("ВидИзделия",Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент));
		СтрокаОсновной = СтрокиОсновной[0]; 
		
		Для Каждого Осн Из СтрокиОсновной Цикл
			
			Если Осн.НомерОтсека = 1 Тогда ШиринаОтсека1 = Осн.ШиринаОтсека КонецЕсли;
			Если Осн.НомерОтсека = 2 Тогда ШиринаОтсека2 = Осн.ШиринаОтсека КонецЕсли;
			Если Осн.НомерОтсека = 3 Тогда ШиринаОтсека3 = Осн.ШиринаОтсека КонецЕсли;
			Если Осн.НомерОтсека = 4 Тогда ШиринаОтсека4 = Осн.ШиринаОтсека КонецЕсли;
			
		КонецЦикла;
		
		Если ЗначениеЗаполнено(СтрокаАнтПолка) Тогда
			ВысотаАнтПолки = СтрокаАнтПолка.ВысотаИзделия;
		КонецЕсли;
		...
		Иначе
			...
			ШиринаОсн = ШиринаОтсека1 + ШиринаОтсека2 + ШиринаОтсека3 + ШиринаОтсека4; 
			...
			ШиринаПроема = ШиринаОсн - ?(ЛевыйНеВлияет, СтрокаЛевыйЭлемент.ШиринаИзделия, 0) - ?(ПравыйНеВлияет, СтрокаПравыйЭлемент.ШиринаИзделия, 0);
		...
		КоличествоОтделов = Объект.ВариантШкафаПоКаталогу.КоличествоОтсеков; 
	...
	Для Каждого Элемент Из ТаблицаИзделийПоКаталогу Цикл
		...
		НомерОтдела = Элемент.НомерОтсека; 
		...
		Если ВсеФормулы.Количество() > 0 Тогда
			...
			Для Каждого ЭлементФормулы Из ВсеФормулы Цикл
				...
				Попытка
					Выполнить("ЗначениеФормулы = " + ЭлементФормулы.Значение);
				Исключение
				...
				Фор.Вставить(ЭлементФормулы.Ключ, ЗначениеФормулы);
		...
		Для Каждого Строка Из СписокЯщикиИзделия Цикл
			...
			Если ИзделиеШкафКупе Тогда
				...
				ИначеЕсли ЗначениеЗаполнено(СтруктураПодбираемойПоЦветуНоменклатуры)
					И ЗначениеЗаполнено(СтруктураПодбираемойПоЦветуНоменклатуры.Кромка2_19) Тогда
					
					НоваяСтрока.КромкаНоменклатура = СтруктураПодбираемойПоЦветуНоменклатуры.Кромка2_19;
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
					...
					НоваяСтрока.РучкаЩель = СтрокаПоиска.РучкаЩель И (НЕ СтрокаПоиска.БезФасада);
					...
					Попытка
						...
						РучкаЩель = СтрокаПоиска.РучкаЩель;
			...
			Если НоваяСтрока.ВидФасада <> "Нет" Тогда
				...
				Иначе
					...
					Если НоваяСтрока.КриволинейныйВыпилПодРучку ИЛИ НоваяСтрока.РучкаЩель Тогда
						НоваяСтрока.КоличествоРучек = 0;
					КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.КухоннаяКоллекция) Тогда
		...
		ФормулыКухонныеКоллекции = НайтиФормулыКухонныеКоллекции();
		...
		Для Каждого Стр ИЗ Коллекция.СписокДетали Цикл
			...
			Если Стр.Форма = Настройки.Форма Тогда
				...
				//Расчет формул справочника
				Если ФормулыКухонныеКоллекции.Количество() > 0 Тогда
					
					Фор = Новый Структура;
					
					Для Каждого ЭлементФормулы Из ФормулыКухонныеКоллекции Цикл
						...
						Попытка
							Выполнить("ЗначениеФормулы = " + ЭлементФормулы.Значение);
						Исключение
							Текст = "Ошибка при формировании формул кухонные коллекции. Наименование формулы - " + ЭлементФормулы.Ключ;
							ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);	
						КонецПопытки;
...
&НаСервере
Функция НайтиФормулыКухонныеКоллекции()
	...
	|	НЕ ФормулыКаталога.ПометкаУдаления
	|	И ФормулыКаталога.Активность
	|	И ФормулыКаталога.Тип = ""КухонныеКоллекции""";
...
Функция ОпределитьКромку(ВидКромкиПоУмолчанию, Кромка)
	...
	Если ВидКромки <> НоменклатурнаяГруппаКромки Тогда
		...
		ИначеЕсли ЭтоУниверсальнаяКромка Тогда
			Структура.Вставить("Кромка", СтруктураПодбираемойПоЦветуКромки.КромкаМДФ);
...
&НаСервере
Процедура ЗаполнитьРедактированые(ТекущийОбъект) 

	Для Каждого Деталь Из ТекущийОбъект.СписокДеталей Цикл
		
		ВидДетали = Деталь.ВидДетали;
		Криволинейная =  Деталь.КривойПилСверху > 0 
		...
		Если Криволинейная Тогда
			 Деталь.МестоОбработки = Перечисления.ЦеховыеЗоны.Криволинейная;			  
		КонецЕсли;				  
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаполнитьРедактированые(ТекущийОбъект);
