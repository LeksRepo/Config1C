			 ...
			 ИЛИ ИзделиеК.Изделие.ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.КухняНижний) Тогда
				...
				Если ДетальРедактированная Тогда
					...
					Если ТекущийОбъект.Дилерский Тогда
						 ДобавитьСтроку(ТекущийОбъект, Справочники.Номенклатура.ИндивидуальнаяСборка, 1, ИзделиеК.НомерСтроки);
						 ДобавитьСтроку(ТекущийОбъект, ИзделиеК.Изделие.УслугаСборки, 1, ИзделиеК.НомерСтроки);
					Иначе
						 ДобавитьСтроку(ТекущийОбъект, ИзделиеК.Изделие.УслугаСборки, 2, ИзделиеК.НомерСтроки);
					КонецЕсли;
...
Процедура СписокЯщикиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Сообщить("СписокЯщикиОбработкаВыбора");
