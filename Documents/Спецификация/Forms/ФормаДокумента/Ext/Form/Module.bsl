		...
		Иначе // НЕ ФасадСПрофилем
			...
			Иначе
				...
				Если Пуфик Тогда
					...
					ПлощадьКожи = ((ВысотаДетали + 200) * (ШиринаДетали +200) / 1000000) * КоличествоДеталей;
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|	ТаблицаСпискаНоменклатуры.Номенклатура,
	|	ТаблицаСпискаНоменклатуры.НомерИзделия,
	|	СУММА(ТаблицаСпискаНоменклатуры.Количество + ТаблицаСпискаНоменклатуры.Количество * НоменклатураПодразделений.ПроцентОтхода / 100) КАК Количество,
	|	ТаблицаСпискаНоменклатуры.ЧерезСклад КАК ЧерезСклад,
	|	МИНИМУМ(ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &Коэффициент) КАК Цена,
	|	СУММА(ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &Коэффициент * (ТаблицаСпискаНоменклатуры.Количество + ТаблицаСпискаНоменклатуры.Количество * НоменклатураПодразделений.ПроцентОтхода / 100)) КАК РозничнаяСтоимость,
...
Процедура ОбработкаИзделийПоКаталогу()
	...
	//Если рассчитываем шкаф купе, определим завалы
	Если ИзделиеШкафКупе Тогда
		
		Пол = 0;
		ЗавалПол = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.Пол,"ВидИзделия").Завал;
		ЗавалЛевый = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.ЛевыйБоковойЭлемент,"ВидИзделия").Завал;
		ЗавалПравый = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.ПравыйБоковойЭлемент,"ВидИзделия").Завал;
		ЗавалПотолок = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.Потолок,"ВидИзделия").Завал;
		ЗавалСтенка = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.ЗадняяСтенка,"ВидИзделия").Завал;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ТекущийОбъект.Срочный = (ТекущийОбъект.Изделие = Справочники.Изделия.Детали ИЛИ ТекущийОбъект.Дилерский)
	И ТекущийОбъект.ДатаИзготовления = ТекущийОбъект.ДатаОтгрузки;
