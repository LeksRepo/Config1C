...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Сообщение") Тогда
		...
		СообщениеДляДизайнера = Параметры.Сообщение;
...
Процедура ПриОткрытии(Отказ)
	...
	Если ПовторнаяПроверка Тогда
		ОткрытьФормуПодбора("ФормаДетали", Объект.СписокМатериалы, Элементы.СписокМатериалы.ТекущиеДанные, Элементы.СписокМатериалы);
	КонецЕсли;
	
	Если СтрДлина(СообщениеДляДизайнера) > 0 Тогда
		
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = СообщениеДляДизайнера;
		Сообщение.Сообщить();
