...
Процедура ЗаполнитьКромки(НоваяСтрока, СтруктураКромки)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураКромки.Кромка) Тогда
		Сообщить("Деталь: "+НоваяСтрока.НомерСтроки+". Не найдена подходящая по цвету кромка. Заполните кромку вручную.");	
	КонецЕсли;
...
Функция ДобавитьНоменклатуруИзКомплектации(фнОбъект)
	...
	Для каждого Строка Из фнОбъект.Комплектация Цикл
		...
		Иначе
			...
			Если Строка.Номенклатура.МестоОбработки = Перечисления.МестоОбработки.Отгрузка Тогда
				Место = "Склад";
			КонецЕсли;
