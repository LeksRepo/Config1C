...
Процедура ПересчитатьСуммуСоСкидкой()
	...
	ПроцентСкидки = ЛексСервер.ПолучитьСкидкуДоговора(Объект.Подразделение, Объект.Дата, Объект.СуммаДокумента, ВидОплаты);
...
Функция ЗаполнитьМатериалы(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.Комплектация Цикл
		...
		Иначе
			...
			Сообщение.Текст = "Позиция "+Строка.Номенклатура+" поставляется только размером, кратным "+СтруктураКратности.Кратность+". Количество материала было изменено.";
			Сообщение.Поле = "Элементы.Комплектация.Количество";
			Сообщение.Сообщить();
