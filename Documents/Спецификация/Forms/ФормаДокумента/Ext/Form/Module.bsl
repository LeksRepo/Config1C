...
Функция УвеличитьКоличествоМатериаловНаПроцентОтхода(фнОбъект)
	...
	|				ИЛИ втНоменклатура.Номенклатура.НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту
	|				ИЛИ втНоменклатура.Номенклатура.МестоОбработки = ЗНАЧЕНИЕ(Перечисление.МестоОбработки.Цех)
	...
	|				И НЕ НоменклатураПодразделений.ОкруглятьДоЛистов
	|				И НЕ(втНоменклатура.Номенклатура.МестоОбработки = ЗНАЧЕНИЕ(Перечисление.МестоОбработки.Цех)
	|						И втНоменклатура.ПоКаталогуМестоОбработки = ""Склад"")
	|				И НЕ втНоменклатура.Номенклатура.НоменклатурнаяГруппа.КоличествоПоПоследнемуЛисту
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если ЗначениеЗаполнено(НоменклатураДляСклеивания) Тогда
			Попытка
				...
				Если ТекущийОбъект.ЭтоКухня И НоменклатураДляСклеивания.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Ручка Тогда
					ДобавитьСтроку(ТекущийОбъект, НоменклатураДляСклеивания, Площадь, НомерИзделия, Ошибки, "детали № " + НомерСтроки, "СписокМатериалы", НомерСтроки, Ложь, Ложь, -1, "Склад");	
				Иначе
		...
		Если ДанныеДетали.СпецФасад Тогда
			...
			Если СборныйРамочныйФасад Тогда
				...
				ДобавитьСтроку(ТекущийОбъект, Строка.КромкаДляСборнойРамки, Количество, Строка.НомерИзделия, Ошибки, "кромки для сборной рамки у детали № " + Строка.НомерСтроки, "СписокМатериалы", Строка.НомерСтроки);
				...
				ДобавитьСтроку(ТекущийОбъект, Строка.УниверсальнаяКромка, Площадь, Строка.НомерИзделия, Ошибки, "обрамления фасада у детали № " + Строка.НомерСтроки, "СписокМатериалы", Строка.НомерСтроки);
		...
		Иначе
			...
			Если ЗначениеЗаполнено(Строка.ВыборМебельнойКромкиСлева) Тогда
				...
				Иначе
					...
					Стр.Вставить("Длина",ВысотаДетали - Строка.РадиусЛевоНиз - Строка.РадиусЛевоВерх + ?(Строка.РадиусЛевоНиз>0, 3.14*Строка.РадиусЛевоНиз/2, 0) + ?(Строка.РадиусЛевоВерх>0, 3.14*Строка.РадиусЛевоВерх/2, 0));
			...
			Если ЗначениеЗаполнено(Строка.ВыборМебельнойКромкиСправа) Тогда
				...
				Иначе
					...
					Иначе
						ДлинаРадиусаПравоНиз = ?(Строка.РадиусПравоНиз>0, 3.14*Строка.РадиусПравоНиз/2, 0);
					КонецЕсли;
					
					Стр.Вставить("Длина",ВысотаДетали - Строка.РадиусПравоНиз - Строка.РадиусПравоВерх + ДлинаРадиусаПравоНиз + ?(Строка.РадиусПравоВерх>0, 3.14*Строка.РадиусПравоВерх/2, 0));
			...
			Для Каждого Строчка Из КромкиСверхуСнизу Цикл
				...
				Иначе
					Площадь = Строчка.Длина * КоличествоДеталей / Метр;
				КонецЕсли;
				
				ДобавитьСтроку(ТекущийОбъект, Строчка.Кромка, Площадь, НомерИзделия, Ошибки, "кромки у детали № " + НомерСтроки, "СписокМатериалы", НомерСтроки);
...
Процедура ПересчитатьСуммы()
	...
	СуммаДокументаСоСкидкой = Объект.СуммаДокумента * ((100 - ПроцентСкидки) / 100);
...
Процедура КомплектацияПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.Номенклатура) Тогда
		Отказ = КомплектацияПередОкончаниемРедактированияНаСервере(ТекущиеДанные.Номенклатура);
		Если Отказ Тогда
			Предупреждение("В комплектации запрещен выбор листового материала",, "Ошибка выбора номенклатуры");
...
&НаСервереБезКонтекста
Функция  КомплектацияПередОкончаниемРедактированияНаСервере(Элемент)
	Возврат Элемент.НоменклатурнаяГруппа.ВидМатериала = Перечисления.ВидыМатериалов.Листовой;
КонецФункции
