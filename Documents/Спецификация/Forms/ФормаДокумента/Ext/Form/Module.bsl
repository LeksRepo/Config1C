...
Процедура ОбработкаИзделийПоКаталогу()
	...
	Для Каждого Элемент Из ТаблицаИзделийПоКаталогу Цикл
		...
		Для Каждого Строка Из СписокДеталейИзделия Цикл
			...
			Иначе
				...
				//RonEXI: Сначала заполняем из структуры отверстий, потом из нового реквизита. Потому что структура от верстий будет удалена позже.
	...
	//						
	//						Если СтрокаОтверстий.РасположениеОтверстия = "Сверху" Тогда
	//							НоваяСтрока.ЕвропазВерх = Элемент.Европаз; 
	//						ИначеЕсли СтрокаОтверстий.РасположениеОтверстия = "Снизу" Тогда
	//							НоваяСтрока.ЕвропазНиз = Элемент.Европаз;
	//						ИначеЕсли СтрокаОтверстий.РасположениеОтверстия = "Слева" Тогда
	//							НоваяСтрока.ЕвропазЛево = Элемент.Европаз;
	//						ИначеЕсли СтрокаОтверстий.РасположениеОтверстия = "Справа" Тогда
	//							НоваяСтрока.ЕвропазПраво = Элемент.Европаз;
	//						КонецЕсли;
	//
	//					КонецЕсли;
	//					
	//				КонецЦикла;
	//				
	//				СтрокиОтверстийУдалить = СписокОтверстий.НайтиСтроки(Новый Структура("ВидОтверстий", Перечисления.ВидыОтверстий.Европаз));
	//				Для Каждого Стр ИЗ СтрокиОтверстийУдалить Цикл
	//					СписокОтверстий.Удалить(Стр);
	//				КонецЦикла;
	//				
	//				Если СписокОтверстий.Количество() > 0 Тогда
	//				     НоваяСтрока.СтруктураОтверстий = ЗначениеВСтрокуВнутр(СписокОтверстий);
	//				Иначе
	//					 НоваяСтрока.СтруктураОтверстий = "";
	//				КонецЕсли;
	//
	//			КонецЕсли;
...
Функция ПроверитьЗаполненностьДанныхКухКоллекции()
	...
	Если ЗначениеЗаполнено(Объект.КухоннаяКоллекция) И Объект.НастройкиКоллекции.Количество() > 0 Тогда
		...
		Если ЗначениеЗаполнено(Данные.Форма) Тогда
			...
			Если Данные.Форма = ПредопределенноеЗначение("Перечисление.ФормыКухонь.Прямая") Тогда
				
				Если ЗначениеЗаполнено(Данные.ШиринаКухниЦентр) Тогда
					 Заполнено = Истина;	
				КонецЕсли;
				  
			ИначеЕсли Данные.Форма = ПредопределенноеЗначение("Перечисление.ФормыКухонь.Угловая") Тогда
				
				Если ЗначениеЗаполнено(Данные.ШиринаКухниЛево) И ЗначениеЗаполнено(Данные.ШиринаКухниЦентр) Тогда
					 Заполнено = Истина;	
				КонецЕсли;
				
			ИначеЕсли Данные.Форма = ПредопределенноеЗначение("Перечисление.ФормыКухонь.ПОбразная") Тогда
