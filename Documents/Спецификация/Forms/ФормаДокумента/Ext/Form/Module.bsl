...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И НЕ ЗначениеЗаполнено(Объект.Контрагент) Тогда
		Объект.Контрагент = Справочники.Контрагенты.ЧастноеЛицо;	
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ Отказ Тогда
		...
		// { Васильев Александр Леонидович [06.10.2016]
		// Добавил сюда, чтобы при перепроведении старых документов суммы не изменились.
		// Где ровно платили, при округлении в большую стороно будет не хватает оплаты.
		
		//RonEXI: Закоментил, не учитывается срочность.
		//ТекущийОбъект.СуммаДокумента = Окр(ТекущийОбъект.СписокНоменклатуры.Итог("РозничнаяСтоимость") + 0.499);
