...
Функция ЗаполнитьУслуги(ТекущийОбъект, СтруктураРаскроя)
	...
	Для Каждого СтрокаТабЛистов Из СтруктураРаскроя.ТаблицаЛистовНоменклатуры Цикл
		...
		Если СтрокаТабЛистов.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ЛДСП10 
			 ИЛИ СтрокаТабЛистов.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ЛДСП16 Тогда
	...
	Если СуммаЛистовНоменклатуры > 0 Тогда
		...
		ДобавитьСтроку(ТекущийОбъект, Номенклатура.РаспилЗаЛист, СуммаЛистовНоменклатуры, 0);
		
	КонецЕсли;
	
	
	//Рез металического поганажа
	
	//Для каждого Строка Из Объект.СписокНоменклатуры Цикл
	//	
	//	СтрокаНоменклатура = Строка.Номенклатура;
	//	
	//	Если СтрокаНоменклатура.РезМаталлическогоПогонажа Тогда
	//		
	//		МаксимальныйРазмерДетали = СтрокаНоменклатура.ДлинаДетали;
	//		
	//		Если Строка.Количество < МаксимальныйРазмерДетали Тогда
	//			
	//			ДобавитьСтроку(ТекущийОбъект, Номенклатура.РезМаталлическогоПогонажа, 1, 0);
	//			
	//		КонецЕсли;
	//		
	//	КонецЕсли;
		...
		Если СписокОтсеков.Количество() > 0 Тогда
			...
			Для Каждого Отсек Из СписокОтсеков Цикл
				...
				Если Отсек.НомерСтроки = 1 Тогда
					Отсек1 = Число(Отсек.КоэффициентОтсека); 
				ИначеЕсли Отсек.НомерСтроки = 2 Тогда
					Отсек2 = Число(Отсек.КоэффициентОтсека);
				ИначеЕсли Отсек.НомерСтроки = 3 Тогда
					Отсек3 = Число(Отсек.КоэффициентОтсека);
				ИначеЕсли Отсек.НомерСтроки = 4 Тогда
					Отсек4 = Число(Отсек.КоэффициентОтсека);
				ИначеЕсли Отсек.НомерСтроки = 5 Тогда
					Отсек5 = Число(Отсек.КоэффициентОтсека);
				ИначеЕсли Отсек.НомерСтроки = 6 Тогда
					Отсек6 = Число(Отсек.КоэффициентОтсека);
				КонецЕсли;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//СписокЗагружаемойНоменклатурыЗаказчика = ПолучитьСписокНоменклатурыЗаказчика();
	//Элементы.СписокМатериаловЗаказчикаНоменклатура.СписокВыбора.ЗагрузитьЗначения(СписокЗагружаемойНоменклатурыЗаказчика.МассивНоменклатуры);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СписокНоменклатурыЗаказчика = ПолучитьСписокНоменклатурыЗаказчика();
	
КонецПроцедуры

&НаСервере
Функция ПолучитьСписокНоменклатурыЗаказчика()
	...
	Для каждого Строка Из Объект.СписокНоменклатуры Цикл
		...
		Если Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Материал Тогда
			...
			ПроверитьНаличие = МассивНоменклатуры.Найти(Номенклатура);
			
			Если ПроверитьНаличие = Неопределено И ЗначениеЗаполнено(Номенклатура) Тогда
