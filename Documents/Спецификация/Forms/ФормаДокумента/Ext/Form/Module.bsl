...
Функция ОбновитьЭлементыФормы(фнФорма, лПараметры)
	...
	Если лПараметры.Свойство("СерийноеИзделие") Тогда
		...
		Элементы.Телефон.Видимость = НЕ лПараметры.СерийноеИзделие;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ПользовательАдминистратор Тогда
		...
		Если ЗначениеЗаполнено(ЭтаФорма.Параметры.ЗначениеКопирования) Тогда
			    ДатаУстаревания = ДобавитьМесяц(ЭтаФорма.Параметры.ЗначениеКопирования.Дата,1);
			    Копирование = Истина;
		КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ДатаУстаревания = ДобавитьМесяц(ТекущийОбъект.Дата,1);
	
	Если ДатаУстаревания < ТекущаяДата() Тогда
		Сообщить("Спецификация устарела: " + ДатаУстаревания + ". Запись не возможна.");
		Отказ = Истина;
		Возврат;
	КонецЕсли;
