	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Иначе
			...
			Если Материал = "Столешница" ИЛИ Материал = "Пристенок" Тогда // по хорошему бы от единицы измерения отталкиваться
				...
				СтруктураКратности = ЛексСервер.ПосчитатьКратность(Строка.Номенклатура, ВысотаДетали/1000);
				
				Если СтруктураКратности.КоличествоМатериала * 1000 = ВысотаДетали Тогда
					
					Площадь = (ВысотаДетали / 1000) * КоличествоДеталей;
					
				Иначе
					
					Площадь = СтруктураКратности.КоличествоМатериала;
