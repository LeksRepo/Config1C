...
Функция ОбновитьСписокНоменклатурыЗаказчика(фнОбъект)
	...
	// Сделать.
	// Возможно запросом, если воое оставим текущую схему.
	// } Васильев Александр Леонидович [09.06.2015]
...
Функция ЗаполнитьТаблицуСкладГотовойПродукции(фнОбъект)
	...
	Для каждого Строка Из фнОбъект.СписокНоменклатуры Цикл
		...
		КоличествоБезОтхода = Строка.КоличествоТребуется;// * 100 / (100 + СтруктураРеквизитов.ПроцентОтхода);
	...
	Для каждого Ящик Из фнОбъект.СписокЯщики Цикл
		...
		Если Ящик.ФасадНоменклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ЛДСП16 
			...
			СтрокаРучка = фнОбъект.СкладГотовойПродукции.НайтиСтроки(Отбор)[0];
...
Функция УвеличитьКоличествоМатериаловНаПроцентОтхода(фнОбъект)
	...
	"ВЫБРАТЬ
	|	втНоменклатура.Номенклатура,
	|	втНоменклатура.Количество + втНоменклатура.Количество * ВЫРАЗИТЬ(втНоменклатура.Номенклатура КАК Справочник.Номенклатура).ПроцентОтхода / 100 КАК Количество,
	|	ВЫБОР
...
Функция ДобавитьНоменклатуруИзКомплектации(фнОбъект)
	...
	Для каждого Строка Из фнОбъект.Комплектация Цикл
		...
		ДобавитьСтроку(фнОбъект, Строка.Номенклатура, Строка.Количество, Строка.НомерИзделия,, "строки комплектации № " + Строка.НомерСтроки, "Комплектация", Строка.НомерСтроки, Ложь, Ложь, -1, Строка.ПоКаталогуМестоОбработки);
