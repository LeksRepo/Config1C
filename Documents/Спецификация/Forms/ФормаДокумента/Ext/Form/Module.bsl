...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если КосойПил > 0 Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Номенклатура.КосойПил, КосойПил * Строка.Количество, Строка.НомерИзделия);
		...
		Для Каждого Эл Из ДанныеДетали.Услуги Цикл
			...
			Попытка
				...
				Если КоличествоУслуги > 0 Тогда
					...
					ДобавитьСтроку(ТекущийОбъект, Эл.Услуга, КоличествоУслуги, Строка.НомерИзделия);
...
Функция УвеличитьКоличествоМатериаловНаПроцентОтхода(ТекущийОбъект)
	...
	|	ТаблицаСпискаНоменклатуры.ИспользуетсяВКоробе,
	|	ТаблицаСпискаНоменклатуры.УслугаСМатериаломЗаказчика,
	|	ТаблицаСпискаНоменклатуры.ПредоставитЗаказчик,
...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки)
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если ЗначениеЗаполнено(КантыИзРедактора) Тогда
			...
			Для Каждого Кромка Из МассивКантов Цикл
				...
				ДлинаКромления = ДлинаКромления * (1 + Клееная);
	...
	Для каждого Строка Из ТекущийОбъект.СписокДверей Цикл
		...
		Для каждого СтрокаНоменклатуры Из ТаблицаНоменклатурыДверей Цикл
			...
			//Если СтрокаНоменклатуры.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = Перечисления.ВидыМатериалов.Хлыстовой Тогда
			//	
			//	Если СтрокаНоменклатуры.Длина <> 0 Тогда
			//		ВызватьИсключение "Ошибка 225: " + СтрокаНоменклатуры.Номенклатура + " является хлыстовым материалом";
			//	КонецЕсли;
			//	
			//	КоличествоМатериала = СтрокаНоменклатуры.Ширина / 1000;
			//	КоличествоСтрок = СтрокаНоменклатуры.Количество;
			//	
			//Иначе
			//	
			//	КоличествоМатериала = ЛексСервер.ПосчитатьКратность(СтрокаНоменклатуры.Номенклатура, СтрокаНоменклатуры.КоличествоСОтходом).КоличествоМатериала;
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	Запрос.УстановитьПараметр("тзМатериалЗаказчика", ТекущийОбъект.СписокМатериаловЗаказчика);
	Запрос.УстановитьПараметр("Подразделение", ТекущийОбъект.Подразделение);
	...
	|	СУММА(втДанные.ПлановаяЗакупочная * втДанные.Количество) КАК ЗарплатаЦеха,
	...
	|	СУММА(втДанные.Цена * втДанные.Количество) КАК РозничнаяСтоимость,
	|	СУММА(втДанные.Цена * втДанные.Количество) КАК СуммаБезНаценкиОфиса,
	|	СРЕДНЕЕ(втДанные.Цена) КАК Цена,
	|	втДанные.ЧерезСклад,
	...
	ТекущийОбъект.СписокНоменклатуры.Загрузить(Запрос.Выполнить().Выгрузить());
...
&НаСервере
Функция ПрименитьКоэффициентыОфиса(фОбъект)
	
	Если фОбъект.Дилерский Тогда
		Возврат Истина;
	КонецЕсли;
	
	СвойстваОфиса = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(фОбъект.Офис, "КоэффициентУслуги, КоэффициентМатериалы");
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("тзНоменклатура", фОбъект.СписокНоменклатуры);
	...
	фОбъект.СписокНоменклатуры.Загрузить(Результат.Выгрузить());
...
&НаСервере
Функция ЗаполнитьСтрокуСписокНоменклатуры(ТекущийОбъект, Номенклатура, Количество, НомерИзделия)
	
	ЭтоМатериал = Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Материал;
	
	// { Васильев Александр Леонидович [25.09.2015]
	// Очень не нравится эта строка, хотелось бы обойтись без такого поиска. :(
	Если ЭтоМатериал Тогда
		ПредоставляетЗаказчик = ТекущийОбъект.СписокМатериаловЗаказчика.Найти(Номенклатура, "Номенклатура") <> Неопределено;
	Иначе
		ПредоставляетЗаказчик = Ложь;
	КонецЕсли;
	// } Васильев Александр Леонидович [25.09.2015]
	...
	НоваяСтрока.УслугаСМатериаломЗаказчика = Ложь;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.Номенклатура.СписокВыбора.ЗагрузитьЗначения(СписокНоменклатурыЗаказчика.ВыгрузитьЗначения());
