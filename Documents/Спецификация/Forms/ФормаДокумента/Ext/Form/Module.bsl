...
Функция ЗаполнитьУслуги(ТекущийОбъект, СтруктураРаскроя)
	...
	Если ТекущийОбъект.СборкаКоробов Тогда
		
		Для каждого Строка Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
			Если НЕ (Строка.Услуги = Справочники.Номенклатура.ПустаяСсылка()) Тогда
				 ДобавитьСтроку(ТекущийОбъект, Строка.Услуги, 1, Строка.НомерСтроки);
			КонецЕсли;
		КонецЦикла;
...
Функция ОткрытьФормуПодбора(ИмяПодбора, ТаблицаДляФормы, СтрокаДанных, ВладелецПодобра, ПараметрыПодбора = Неопределено)
	...
	Если ИмяПодбора = "ФормаИзделийПоКаталогу" ИЛИ ИмяПодбора = "ФормаШкафПоКаталогу" Тогда
		 ПараметрыПодбора.Вставить("Сборка", Объект.СборкаКоробов);	
	КонецЕсли;
...
Функция ЗаполнитьСтрокуСписокНоменклатуры(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия)
	...
	//ТЗНоменклатуры = СписокНоменклатурыДляЗаполненияРеквизита;
...
Функция ПолучитьСписокНоменклатурыЗаказчика(ТекущийОбъект)
	...
	//МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ВертикальныйПрофиль);
	//МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.РамкаВерхняя);
	//МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.РамкаНижняя);
	//МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.РамкаСредняяБезКрепления);
	//МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.РамкаСредняяСКреплением);
	//
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//А = ТекущийОбъект.СписокМатериаловЗаказчика.Выгрузить();
	//СписокНоменклатурыДляЗаполненияРеквизита = А.Скопировать();
...
Процедура ИзделияПоКаталогуОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("Структура") И ВыбранноеЗначение.Свойство("ТаблицаДетали") Тогда
		Объект.СборкаКоробов = ВыбранноеЗначение.Сборка;
		ЗагрузитьТабличнуюЧасть(ВыбранноеЗначение.ТаблицаДетали, Элемент.Имя);
	Иначе
		ЗагрузитьТабличнуюЧасть(ВыбранноеЗначение, Элемент.Имя);
	КонецЕсли;
