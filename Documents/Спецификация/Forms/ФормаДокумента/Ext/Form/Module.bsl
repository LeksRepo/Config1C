...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	//ОбНоменклатура = ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуруПолностью(ТекущийОбъект.Подразделение);
				...
				Если ДетальЯщика.НомерСтроки = НомерСтрокиЯщика Тогда
					...
					Если ДетальЯщика.Тип = Перечисления.ВидыДеталейЯщиков.Фасад Тогда
						...
						Если ЗначениеЗаполнено(ДетальЯщика.КромкаНоменклатура)
							...
							ДобавитьСтроку(ТекущийОбъект, Номенклатура.ОбгонкаУглов, 4*Строка.КоличествоЯщиков, ДетальЯщика.НомерИзделия);
		...
		Для Каждого ИзделиеК ИЗ СписокИзделий Цикл
			...
			Если ИзделиеК.Сборка
				...
				Если ДетальРедактированная Тогда
					...
					ДобавитьСтроку(ТекущийОбъект, Справочники.Номенклатура.НадбавкаЗаИзменениеКонструкции, 1, ИзделиеК.НомерСтроки);
...
Процедура ЗаполнитьОбгонкуУгловКромки(ТекущийОбъект, Деталь)
	...
	ДобавитьСтроку(ТекущийОбъект, Номенклатура.ОбгонкаУглов, КоличествоУслуги, Деталь.НомерИзделия);
