...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	|ГДЕ
	|	втНоменклатура.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = ЗНАЧЕНИЕ(Перечисление.ВидыМатериалов.Листовой)
...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки, СоответствиеОбрезков)
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		//Листовой = НоменклатурнаяГруппа.ЛистовойМатериал;
...
Процедура КомплектацияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Массивчик =СписокНоменклатурныхГрупп.Массив;
	
	ФиксМассивчик = Новый ФиксированныйМассив (Массивчик);
	Элементы.КомплектацияНоменклатура.ПараметрыВыбора = ФиксМассивчик;
КонецПроцедуры
