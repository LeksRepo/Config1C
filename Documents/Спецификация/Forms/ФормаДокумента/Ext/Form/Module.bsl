...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если ТекущийОбъект.ПакетУслуг <> Перечисления.ПакетыУслуг.СамовывозОтПроизводителя Тогда
		...
		Иначе
			Попытка
				Этаж = Число(ЛексСервер.ПолучитьСтруктуруИзАдреса(ТекущийОбъект.АдресМонтажа).Этаж);
			Исключение
				Этаж = 1;
			КонецПопытки;
		КонецЕсли;
	...
	Если ТекущийОбъект.НеноменклатурныйМатериал.Количество() > 0 ИЛИ ТекущийОбъект.ЕстьМатериалПодЗаказ Тогда
...
Функция ЗаполнитьМатериалы(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.СписокДверей Цикл
		...
		Для каждого СтрокаНоменклатуры Из ТаблицаНоменклатурыДверей Цикл
			...
			ДобавитьСтроку(ТекущийОбъект, СтрокаНоменклатуры.Номенклатура, СтруктураКратности.КоличествоМатериала, 0);
...
Функция ДобавитьУслугуДляМебельнойКромки(ТекущийОбъект, Строка, ИмяПоля)
	...
	Если Строка[Сторона] < 80 Тогда
		
		ДобавитьСтроку(ТекущийОбъект, Справочники.Номенклатура.КромлениеМалоразмернойДетали, 1, Строка.НомерИзделия);
