...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	|		ПО ТаблицаСпискаНоменклатуры.Номенклатура = спрНоменклатура.БазоваяНоменклатура
	|ГДЕ
	...
	ТаблицаПодЗаказ.Свернуть("Номенклатура, КоэффициентБазовых", "Количество");
	...
	Для Каждого Элемент Из ТаблицаПодЗаказ Цикл
		...
		Если СтрокаМатериала <> Неопределено Тогда
			...
			ОстатокМатериала = СтрокаМатериала.Количество * Элемент.КоэффициентБазовых - Элемент.Количество;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Модифицированность И ТекущийСтатус = Перечисления.СтатусыСпецификации.ПроверенИнженером И ТекущийОбъект.Изделие <> Справочники.Изделия.ШкафКупеПоКаталогу Тогда
...
Процедура Проверена(Команда)
	...
	СохранитьИИзменитьСтатус(ПредопределенноеЗначение("Перечисление.СтатусыСпецификации.ПроверенИнженером"));
