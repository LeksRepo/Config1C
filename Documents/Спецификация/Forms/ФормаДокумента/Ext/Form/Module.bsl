...
Функция ПроверитьЗаменитьКратность()
	...
	Для каждого Строка Из Объект.Комплектация Цикл
		...
		Если Строка.Номенклатура.НоменклатурнаяГруппа.ЛистовойМатериал Тогда
			...
			"Объект.Комплектация["+Строка(Строка.НомерСтроки-1)+"].Номенклатура","Для заказа данного материала используйте вкладку ""Детали""" );
...
Функция ЗаполнитьУслуги(ТекущийОбъект, СтруктураРаскроя)
	...
	Для каждого Строка Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
		Если Строка.Сборка И (НЕ (Строка.УслугаСборки = Справочники.Номенклатура.ПустаяСсылка())) Тогда
			 ДобавитьСтроку(ТекущийОбъект, Строка.УслугаСборки, 1, Строка.НомерСтроки);
		КонецЕсли;
	КонецЦикла;
