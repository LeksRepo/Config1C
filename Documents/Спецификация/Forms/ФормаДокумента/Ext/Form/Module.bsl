...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	Для Каждого Строка Из ТаблицаОкруглятьДоЛистов Цикл
		...
		ПлощадьЛиста = ОпределитьПлощадьЛиста(Строка.Номенклатура);
		...
		// } Васильев Александр Леонидович [07.05.2015]
		ОстатокМатериала = СтрокаМатериала.Количество * (ПлощадьЛиста / КоличествоММ) - Строка.Количество;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	СтруктураПроверки = ПроверитьЗаменитьКратность(Отказ); // Изменяем материал в комплектации по "Кратности"
