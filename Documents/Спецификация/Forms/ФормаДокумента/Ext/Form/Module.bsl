...
Функция ЗаполнитьТаблицуСкладГотовойПродукции(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.СписокНоменклатуры Цикл
		...
		Если Строка.ЧерезСклад ИЛИ Номенклатура.ВидНоменклатуры <> Перечисления.ВидыНоменклатуры.Материал Тогда
			Продолжить;
		КонецЕсли;
		
		Если Номенклатура.МестоОбработки = Перечисления.МестоОбработки.Склад Тогда
			КоличествоЦех = 0;
			КоличествоСклад = Строка.Количество;
		ИначеЕсли Номенклатура.МестоОбработки = Перечисления.МестоОбработки.ЧерезСкладВЦех Тогда
			КоличествоСклад = 0;
...
Процедура ДобавитьСтроку(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия)
	...
	Если НЕ Номенклатура = Справочники.Номенклатура.ПустаяСсылка() Тогда
		...
		Если Площадь > 0 Тогда
			...
			НоваяСтрока.ЧерезСклад = (Номенклатура.МестоОбработки = Перечисления.МестоОбработки.Цех ИЛИ Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Услуга);
