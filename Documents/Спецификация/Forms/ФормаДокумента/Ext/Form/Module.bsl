...
Функция ЗаполнитьМатериалы(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.Комплектация Цикл
		...
		Если Строка.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Ручка Тогда
			
			НоваяСтрока = ТекущийОбъект.СписокНоменклатуры.Добавить();
			ДобавитьСтрокуПоОсобымПравилам(НоваяСтрока, Строка.Номенклатура, Строка.Количество);
...
&НаСервере
Процедура ДобавитьСтрокуПоОсобымПравилам(НоваяСтрока, Номенклатура, Количество)
...
Процедура ДобавитьСтроку(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия)
	...
	Если НЕ Номенклатура = Справочники.Номенклатура.ПустаяСсылка() Тогда
		...
		Если Площадь > 0 И НЕ Номенклатура.Неноменклатурный Тогда
