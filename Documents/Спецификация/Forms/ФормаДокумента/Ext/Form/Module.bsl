...
&НаСервере
Функция ПроверитьЗаменитьКратность()
	
	СтрокаСообщенияОКратности = Неопределено;
	...
	Возврат СтрокаСообщенияОКратности;
				...
				Если Подмассив.Количество() > 0 Тогда
					...
					Попытка
						...
						ИначеЕсли КоличествоОтверстиВРяду > 0 Тогда
							...
							ВсегоОтверстий = ВсегоОтверстий + КоличествоОтверстиВРяду;
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если НЕ ФасадСПрофилем Тогда
			...
			Для Каждого Строчка Из КромкиСлеваСправа Цикл
				...
				Если Строчка.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ТорцеваяРучка16мм Тогда
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	//Пока так. Вообще бралось из реквизита СписокМатериаловЗаказчика, который сейчас переименован. 
	//При исправлении убедится, что работает при пустой таблице и без наличия недоступного материала
	...
	|				ТОГДА 0
	|			ИНАЧЕ ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &Коэффициент
	|		КОНЕЦ) КАК Цена,
	|	СУММА(ВЫБОР
	|			КОГДА ТаблицаСпискаНоменклатуры.Номенклатура = ТаблицаСпискаМатериаловЗаказчика.Номенклатура
	|				ТОГДА 0
	|			ИНАЧЕ ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &Коэффициент * (ТаблицаСпискаНоменклатуры.Количество + ТаблицаСпискаНоменклатуры.Количество * ЕСТЬNULL(НоменклатураПодразделений.ПроцентОтхода, 0) / 100)
	...
	Если ТекущийОбъект.Срочный Тогда
		...
		НоваяСтрока.РозничнаяСтоимость = Структура.РозничнаяСтоимость; // Розничную стоимость умножать
		НоваяСтрока.Цена = Структура.РозничнаяСтоимость;
		НоваяСтрока.ЧерезСклад = Ложь;
...
Функция ПолучитьУведомлениеМатериалПодЗаказ(Подразделение)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ВидНастройки", Перечисления.ВидыНастроекПодразделений.ФизЛицоДляСогласованияМатериала);
	Запрос.УстановитьПараметр("Подразделение", Подразделение);
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НастройкиПодразделений.Значение.ТелефонРабочий КАК ТелефонРабочий,
	|	НастройкиПодразделений.Значение.Телефон
	|ИЗ
	|	РегистрСведений.НастройкиПодразделений КАК НастройкиПодразделений
	|ГДЕ
	|	НастройкиПодразделений.Подразделение = &Подразделение
	|	И НастройкиПодразделений.ВидНастройки = &ВидНастройки";
	...
	Иначе
		...
		Если ЗначениеЗаполнено(Выборка.ТелефонРабочий) Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	ТекстСообщения = ПроверитьЗаменитьКратность();
...
Функция ПолучитьСписокНоменклатурыЗаказчика(ТекущийОбъект)
	...
	СправочникНоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы;
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.АГТПанель);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.АГТПрофиль);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.АлюминиевыйПрофиль);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ДВП);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Зеркало);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кант);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.КантАлюминиевый);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.КантТ);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кромка045_19);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кромка2_19);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кромка2_35);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кромка2_42);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Кромка2_45);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.КромкаМДФ);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.КромкаСтолешница);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ЛДСП10);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ЛДСП16);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.МДФ10);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.МДФ18);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.МДФ8);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Пристенок);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Профиль_П_образный);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Стекло);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.СтеклоДляСкругляемогоФасада);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.Столешница);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ТорцеваяРучка16мм);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ТорцеваяРучка18мм);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ФасадныйАлюминий);
	МассивНоменклатурныхГрупп.Добавить(СправочникНоменклатурнаяГруппа.ЩитМебельный);
	...
	Для каждого Строка Из ТекущийОбъект.СписокНоменклатуры Цикл
		...
		Если Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Материал Тогда
			...
			НужнаяГруппа = МассивНоменклатурныхГрупп.Найти(НоменклатурнаяГруппа);
			
			Если НужнаяГруппа <> Неопределено и ПроверитьНаличиеВСпискеНоменклатуры = Неопределено И ЗначениеЗаполнено(Номенклатура) Тогда
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ТекущийОбъект.СписокМатериаловЗаказчика.Количество() > 1 Тогда
...
Функция ПроверитьДоступностьПодразделениямНаСервере(СписокНоменклатуры, фПодразделение)
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		СписокМатериаловЗаказчика = Объект.СписокМатериаловЗаказчика.Выгрузить();
		Объект.СписокМатериаловЗаказчика.Очистить();
	...
	Если СписокМатериаловЗаказчика.Количество() > 0 Тогда
		...
		СписокМатериаловЗаказчика.Свернуть("Номенклатура", "Количество, Комментарий");
	
	КонецЕсли;
	
	Для каждого Строка Из СписокМатериаловЗаказчика Цикл
	
		НоваяСтрока = Объект.СписокМатериаловЗаказчика.Добавить();
	
	КонецЦикла;
	
	Объект.СписокМатериаловЗаказчика.Загрузить(СписокМатериаловЗаказчика);
