...
Функция ПолучитьУведомлениеМатериалПодЗаказ(Подразделение)
	...
	Если РезультатЗапроса.Пустой() Тогда
		ТекстУведомления = "Материал под заказ. В подразделении нет заведующего складами материалов";
	Иначе
		Выборка = РезультатЗапроса.Выбрать();
		Пока Выборка.Следующий() Цикл
			ТелЗавСкладамиМатериалов = "Материал под заказ. Согласуйте " + Выборка.ТелефонРабочий;
		КонецЦикла;
	КонецЕсли;
