...
Процедура НаПроверкуИнженеру(Команда)
	...
	Если Объект.ЕстьМатериалПодЗаказ И НЕ ЗаполненыПоставщикиНаКлиенте() И НЕ ЛексСервер.МатериалПодЗаказПроверен(СтатусСпецификации) Тогда
		НовыйСтатус = ПредопределенноеЗначение("Перечисление.СтатусыСпецификации.ПроверяетсяМатериал");
	КонецЕсли;
...
Функция ПередатьВПроизводствоНаСервере()
	...
	Если Объект.Дилерский
		...
		И НЕ ЛексСервер.МатериалПодЗаказПроверен(СтатусСпецификации) Тогда
