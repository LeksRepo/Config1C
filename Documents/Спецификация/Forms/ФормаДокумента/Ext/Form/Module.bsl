...
Функция ЗаполнитьУслуги(ТекущийОбъект, СтруктураРаскроя)
	...
	Для каждого Строка Из Объект.СписокМатериалы Цикл
		...
		Если ЛДСП ИЛИ Материал = "ДВП" ИЛИ Материал = "МДФ" ИЛИ Столешница ИЛИ Пуфик Или Материал = "АГТПанель" Тогда
...
&НаСервере
Функция ПолучитьКромкуДляФлэшПоКаталогу()
	
	Стр = Новый Структура;
	Стр.Вставить("Наименование", "W-сонома");
	Стр.Вставить("Код", "000003879");
...
Процедура МасштабироватьИЗаполнитьПараметрыРедактированнойДетали(СтрокаФлэш, СтрокаТаблицы)
	...
	СтруктураКромка = ПолучитьКромкуДляФлэшПоКаталогу();
