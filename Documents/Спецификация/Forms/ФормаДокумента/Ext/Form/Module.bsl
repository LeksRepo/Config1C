...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Для каждого Строка Из Объект.СписокМатериалы Цикл
		...
		//Фасад = Материал = "ФасадАГТ" ИЛИ Материал = "ФасадЛДСП" ИЛИ Материал = "ФасадМДФ" ИЛИ Материал = "ФасадАлюминиевый" ИЛИ Материал = "ФасадСтеклянный" ИЛИ Материал = "ФасадСтеклянныйЗакругленный";
		...
		Если ЗначениеЗаполнено(Строка.СтрокаДляФлэш) Тогда
			...
			Если КоличествоПрисадок > 0 Тогда
				...
				Иначе
					Услуга = Номенклатура.ПрисадкаПодПетли;
				КонецЕсли;
				
				ДобавитьСтроку(ТекущийОбъект, Услуга, КоличествоПрисадок * Строка.Количество, Строка.НомерИзделия);
		...
		Если ЛДСП ИЛИ Материал = "ДВП" ИЛИ Материал = "МДФ" ИЛИ Столешница Тогда
			...
			Площадь = Строка.Количество * (Клееная + 1);
			...
			Если ЗначениеЗаполнено(КантыИзРедактора) Тогда 
				...
				Для Каждого Строчка Из МассивКантов Цикл
					...
					Если НЕ Кромка = "Окантовка" Тогда
						
						ДобавитьСтроку(ТекущийОбъект, Номенклатура.КриволинейноеКромление, ДлинаКанта * Строка.Количество / 1000, Строка.НомерИзделия);
						
					Иначе
						
						ДобавитьСтроку(ТекущийОбъект, Номенклатура.Окантовка, ДлинаКанта * Строка.Количество / 1000, Строка.НомерИзделия);
			...
			ИначеЕсли Материал = "ФасадМДФ" Тогда
				...
				Если Кромка = "Обычная" Тогда
					...
					Если Строка.УниверсальнаяКромка.ГлубинаДетали < 1 Тогда
						Услуга = Номенклатура.КромлениеТонкойКромки;
					Иначе
						Услуга = Номенклатура.КромлениеТолстойКромки;
					КонецЕсли;
					ДобавитьСтроку(ТекущийОбъект, Услуга, Строка.Количество * 2 * (Строка.ВысотаДетали + Строка.ШиринаДетали) / 1000, Строка.НомерИзделия);
	...
	КонецЦикла; // цикл по всем строкам деталей
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.НеноменклатурныйМатериал Цикл
		...
		НоваяСтрока.Количество = 0;//Строка.Количество; Иначе - задваивает количество в "заказе"
	...
	Если ТекущийОбъект.Срочный Тогда
		...
		//// 500 руб. потом брать из регистра
		//НоваяСтрока = Таблица.Добавить();
		//НоваяСтрока.Номенклатура = Справочники.Номенклатура.СрочностьЗаказа;
		//НоваяСтрока.ЕдиницаИзмерения = НоваяСтрока.Номенклатура.ЕдиницаИзмерения;
		//НоваяСтрока.Количество = 1;
		//НоваяСтрока.ЗарплатаЦеха = 0;
		//НоваяСтрока.РозничнаяСтоимость = 500;
...
Функция ОбновитьДоступностьДатыМонтажа()
	...
	Элементы.ДатаМонтажа.Доступность = Объект.ПакетУслуг = ПредопределенноеЗначение("Перечисление.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж");
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	//ОповеститьОВыборе(СуммаДокументаСоСкидкой);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Иначе
		...
		Если ДеньНедели(ДатаОтгрузки) = 1 Тогда
			КоличествоДней = 2;
		ИначеЕсли ДеньНедели(ДатаОтгрузки) = 2 И ТекущийОбъект.ЭтоКухня Тогда
			КоличествоДней = 3;
		КонецЕсли;
		
		ТекущийОбъект.ДатаИзготовления = ДатаОтгрузки - КоличествоДней * 86400;
...
Процедура ПриЗакрытии()
	...
	Если ЭтаФорма.ВладелецФормы <> Неопределено Тогда
		...
		Если ЭтаФорма.ВладелецФормы.Заголовок = "Помощник ввода доп. соглашений" Тогда
			
			СтруктураПараметров = Новый Структура;
			СтруктураПараметров.Вставить("Спецификация", Объект.Ссылка);
