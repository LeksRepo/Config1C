...
Функция УвеличитьКоличествоМатериаловНаПроцентОтхода(фнОбъект)
	...
	|	втНоменклатура.КоличествоТребуется КАК КоличествоТребуется,
	...
	фнОбъект.СписокНоменклатуры.Загрузить(Таблица);
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	СформироватьВТНоменклатура(Запрос, ТекущийОбъект);
...
Функция ПрименитьКоэффициентыОфиса(фнОбъект)
	...
	|	ВЫБОР
	|		КОГДА втНоменклатура.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = ЗНАЧЕНИЕ(Перечисление.ВидыМатериалов.Хлыстовой)
	|			ТОГДА втНоменклатура.КоличествоТребуется
	|		ИНАЧЕ втНоменклатура.КоличествоТребуется * &КоэффициентМатериалы
	|	КОНЕЦ КАК КоличествоТребуется,
	|	втНоменклатура.ЕдиницаИзмерения,
	|	ВЫБОР
