...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	ТекущийОбъект.ЕстьМатериалПодЗаказ = ТекущийОбъект.СписокМатериаловПодЗаказ.Количество() > 0;
...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	// { Васильев Александр Леонидович [17.06.2015]
	// Чет хуёвый тут запрос.
	// Всяко можно бодрей и понятней сделать.
	// } Васильев Александр Леонидович [17.06.2015]
	...
	Для Каждого Строка Из ТаблицаОкруглятьДоЛистов Цикл
		...
		СтрокаЛистыНоменклатуры = ТекущийОбъект.СписокЛистовНоменклатуры.Найти(Строка.Номенклатура);
		
		Если СтрокаЛистыНоменклатуры = Неопределено Тогда
			Сообщить("Ошибка определения количества листов номенклатуры " + Строка.Номенклатура);
			Продолжить; // не должны сюда попадать
		КонецЕсли;
		
		// Столешницы и пристенки считаем в погонных метрах.
		Если СтрокаЛистыНоменклатуры.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Столешница 
			ИЛИ СтрокаЛистыНоменклатуры.Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Пристенок Тогда
		...
		// } Васильев Александр Леонидович [07.05.2015]
		ОстатокМатериала = СтрокаЛистыНоменклатуры.Количество * КоличествоМатериала - Строка.Количество;
		...
		Если ОстатокМатериала > 0 Тогда
			...
			НоваяСтрока.ПредоставитЗаказчик = ТекущийОбъект.СписокМатериаловЗаказчика.Найти(Строка.Номенклатура) <> Неопределено;
	...
	// { Васильев Александр Леонидович [17.06.2015]
	// о_О
	// Это нахрена из-за пары новых строк ВСЮ таблицу гонять по запросу и целиком её загружать.
	// Сделать норм.
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// { Васильев Александр Леонидович [09.06.2015]
	// Формировать картинки будем только при размещении заказа.
	// Теперь это происходит в команде.
	// } Васильев Александр Леонидович [09.06.2015]
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Элементы.СписокМатериаловЗаказчикаНоменклатура.СписокВыбора.ЗагрузитьЗначения(СписокНоменклатурыЗаказчика.ВыгрузитьЗначения());
...
&НаСервере
Функция ПроверитьДоступностьПодразделениямНаСервере(ТекущийОбъект, Ошибки)
	...
	Запрос.УстановитьПараметр("СписокМатериаловЗаказчика", ТекущийОбъект.СписокМатериаловЗаказчика.Выгрузить());
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	РезультатЗапроса = Запрос.Выполнить();
	ТекущийОбъект.СписокМатериаловЗаказчика.Загрузить(РезультатЗапроса.Выгрузить());
