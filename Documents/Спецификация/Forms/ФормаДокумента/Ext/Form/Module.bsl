...
Функция ЗаполнитьХлыстовойМатериал(фнОбъект)
	...
	|	ЕСТЬNULL(рсНомПодразделений.ОкруглятьДоЛистов, ЛОЖЬ) КАК ОкруглятьДоЛистов
	|ИЗ
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			МассивОтрезков.Добавить(Выборка.КоличествоТребуется * 1000);
		...
		Для каждого СтрокаНоменклатуры Из ТаблицаНоменклатурыДверей Цикл
			...
			Если СтрокаНоменклатуры.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = Перечисления.ВидыМатериалов.Хлыстовой Тогда
				...
				Для ъ = 1 по СтрокаНоменклатуры.Количество Цикл
					...
					ДобавитьСтроку(ТекущийОбъект, СтрокаНоменклатуры.Номенклатура, СтрокаНоменклатуры.Ширина / 1000, 0, Ошибки, ТекстСообщения, "СписокДверей", Строка.НомерСтроки);
				КонецЦикла;
...
Функция НоваяСтрокаСписокНоменклатуры(фнПараметры, фнНоменклатура, фнКоличество)
	...
	НоваяСтрока.Количество = фнПараметры.Количество;
	...
	НоваяСтрока.ЧерезСклад = СвойстваНоменклатуры.МестоОбработки <> Перечисления.МестоОбработки.Цех;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Элементы.КолонкаПроизводство.Доступность = Ложь;
		Элементы.КолонкаДаты.Доступность = Ложь;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Изделие) Тогда
		Сообщить("Поле ""Изделие"" не заполнено");
		Отказ = Истина;
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.Телефон) И НЕ Объект.Дилерский И Объект.Изделие = ПредопределенноеЗначение("Справочник.Изделия.Детали") Тогда
		Сообщить("Поле ""Телефон"" не заполнено");
		Отказ = Истина;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ Отказ Тогда //Nrn переделал, было если отказ - возврат. Но спецификация сохранялась.
		...
		Если ТекущийОбъект.ОкруглятьДоЛистов Тогда
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.Изделие) Тогда
		...
		Элементы.КолонкаПроизводство.Доступность = Истина;
		Элементы.КолонкаДаты.Доступность = Истина;
...
Процедура СписокМатериалыОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Объект.ОкруглятьДоЛистов = Истина;
...
Процедура КомплектацияНоменклатураНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НомГруппыНеЛистовой = ПолучитьНомГруппыНеЛистовойМатериал();
    ...
    Отбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("НоменклатурнаяГруппа");
    Отбор.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке;
    Отбор.ПравоеЗначение = НомГруппыНеЛистовой;
