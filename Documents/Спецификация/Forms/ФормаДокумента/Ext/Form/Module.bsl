...
Функция ЗаполнитьХлыстовойМатериал(фнОбъект)
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(ОсновнаяПоСкладу)
			ИЛИ НЕ ОсновнаяПоСкладу.Базовый Тогда
...
Функция ЗаполнитьТаблицуСкладГотовойПродукции(ТекущийОбъект)
	...
	Для каждого Строка Из ТекущийОбъект.СписокНоменклатуры Цикл
		...
		КоличествоБезОтхода = Строка.КоличествоТребуется * 100 / (100 + СтруктураРеквизитов.ПроцентОтхода); // стыдно за такую хуйню :(
...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если НЕ ТекущийОбъект.Изделие.БезЦеховыхУслуг Тогда
				...
				ИначеЕсли ТипЗнч(Строка.Номенклатура) = Тип("СправочникСсылка.НоменклатурныеГруппы") Тогда
					...
					Если Строка.Номенклатура = Справочники.НоменклатурныеГруппы.КухонныеНожки Тогда
						...
						Если ЗначениеЗаполнено(Номенклатура.ДополнительныйЭлемент) Тогда
							...
							//Если ножки идут с доп элементом делим их на 2
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) И НЕ ЗначениеЗаполнено(Объект.Изделие) Тогда
