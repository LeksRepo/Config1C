...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки)
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Материал = Строка.Материал;
		Строка.ВидДетали = Справочники.ВидыДеталей.НайтиПоНаименованию(Материал);
...
Функция ПрименитьКоэффициентыОфиса(фОбъект)
	...
	|	тзНоменклатура.РозничнаяСтоимость,
	|	ВЫБОР
	|		КОГДА ВЫРАЗИТЬ(тзНоменклатура.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга)
	|			ТОГДА &КоэффициентУслуги
	|		ИНАЧЕ &КоэффициентМатериалы
	...
	|	втНоменклатура.Количество,
	|	втНоменклатура.ЕдиницаИзмерения,
	|	втНоменклатура.Цена * втНоменклатура.Коэффициент КАК Цена,
	|	втНоменклатура.ЗарплатаЦеха,
	|	втНоменклатура.НомерИзделия,
	|	втНоменклатура.НомерСтроки,
	|	втНоменклатура.ЧерезСклад,
	|	втНоменклатура.ПредоставитЗаказчик,
	|	втНоменклатура.Цена * втНоменклатура.Коэффициент * втНоменклатура.Количество КАК РозничнаяСтоимость
	|ИЗ
	|	втНоменклатура КАК втНоменклатура";
		...
		Для Каждого Строка Из СписокЯщикиИзделия Цикл
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
					...
					НоваяСтрока.ВидЯщика = НоваяСтрока.СхемаЯщика.Вид;							
...
Функция ОбновитьЭлементыФормы(лФорма, лПараметры)
	...
	Если лПараметры.Свойство("ТолькоПросмотр") Тогда
		...
		лФорма.ТолькоПросмотр = лПараметры.ТолькоПросмотр;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	// { Васильев Александр Леонидович [26.06.2015]
	// Не понятно почему иногда слетает при записи доступность с элементов.
	// Будем втыкать куда придётся...
	// } Васильев АЯлександр Леонидович [26.06.2015]
	
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ЕстьМонтаж", Объект.ПакетУслуг = ПредопределенноеЗначение("Перечисление.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж"));
