...
Процедура ОбработкаИзделийПоКаталогу()
	...
	Если ИзделиеШкафКупе Тогда 
		...
		Если СписокДверей.Количество() = 1 Тогда
			...
			ВысотаПроема = СписокДверей[0].Двери.ВысотаПроема;
			ШиринаПроема = СписокДверей[0].Двери.ШиринаПроема;
		...
		СтрокаСтенка = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.ЗадняяСтенка,"ВидИзделия");
				
		ВысотаПол = ?(СтрокаПол.Изделие.НеВлияетНаОсновной, СтрокаПол.ВысотаИзделия, 0);
		ШиринаЛевый = ?(СтрокаЛевыйЭлемент.Изделие.НеВлияетНаОсновной, СтрокаЛевыйЭлемент.ШиринаИзделия, 0);
		ШиринаПравый = ?(СтрокаПравыйЭлемент.Изделие.НеВлияетНаОсновной, СтрокаПравыйЭлемент.ШиринаИзделия, 0);
		ВысотаПотолок = ?(СтрокаПотолок.Изделие.НеВлияетНаОсновной, СтрокаПотолок.ВысотаИзделия, 0);                        
		...
		Если НЕ КорпусныйШкафКупе Тогда
			...
			СтруктураЗавалов.Вставить("ЗавалСтенка", Число(СтрокаСтенка.Завал));
	...
	Для Каждого Элемент Из ТаблицаИзделийПоКаталогу Цикл
		...
		Если ВсеФормулы.Количество() > 0 Тогда
			...
			Для Каждого ЭлементФормулы Из ВсеФормулы Цикл
				...
				Формула.Вставить(ЭлементФормулы.Ключ, ЗначениФормулы);
		...
		Для Каждого Строка Из СписокМатериалыИзделия Цикл
			...
			Если ЗначениеЗаполнено(Элемент.СтрокаТаблицаФасадов) Тогда
				...
				Если ЗначениеЗаполнено(СтрокаФасада) Тогда
					Если СтрокаФасада.ВидЭлемента = "Фасад" Тогда
						...
						ПоложениеРучки = СтрокаФасада.ДеталиРасположениеПазовИРучкиНаФасадах.ПоложениеРучки;
						Петли = СтрокаФасада.Петли;
						КоличествоПетель = СтрокаФасада.КоличествоПетель;
						РасположениеПазов = СтрокаФасада.ДеталиРасположениеПазовИРучкиНаФасадах.ПоложениеПазов;
						ЭтоФасад = Истина;
			...
			ИначеЕсли ЭтоФасад Тогда
				...
				Если ЗначениеЗаполнено(Строка.ДеталиРасположениеПазовИРучкиНаФасадах.ПоложениеРучки) Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СтруктураПараметров.Вставить("ШкафКупеПоКаталогу", Объект.Изделие = Справочники.Изделия.ШкафКупеПоКаталогу ИЛИ Объект.Изделие = Справочники.Изделия.КорпусныйШкафКупеПоКаталогу);
	СтруктураПараметров.Вставить("ПоказатьТелефон", Объект.Изделие = Справочники.Изделия.Детали И Объект.Контрагент = Справочники.Контрагенты.ЧастноеЛицо);
	ОбновитьЭлементыФормы(ЭтаФорма, СтруктураПараметров);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Модифицированность
			...
			И ТекущийОбъект.Изделие <> Справочники.Изделия.ШкафКупеПоКаталогу
			И ТекущийОбъект.Изделие <> Справочники.Изделия.КорпусныйШкафКупеПоКаталогу Тогда
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	// Интересно, внеконтекстным вызовом будет быстрее?
	ШкафКупеПоКаталогу = ПредопределенноеЗначение("Справочник.Изделия.ШкафКупеПоКаталогу");
	КорпусныйШкафКупеПоКаталогу = ПредопределенноеЗначение("Справочник.Изделия.КорпусныйШкафКупеПоКаталогу");
	Переделка = ПредопределенноеЗначение("Справочник.Изделия.Переделка");
	Детали = ПредопределенноеЗначение("Справочник.Изделия.Детали");
	ЧастноеЛицо = ПредопределенноеЗначение("Справочник.Контрагенты.ЧастноеЛицо");
	// } Васильев Александр Леонидович [07.08.2014]
	
	ШкафПоКаталогу = (Объект.Изделие = ШкафКупеПоКаталогу ИЛИ Объект.Изделие = КорпусныйШкафКупеПоКаталогу);
	
	Если ШкафПоКаталогу И (Объект.СписокМатериалы.Количество() > 0 ИЛИ Объект.СписокЯщики.Количество() > 0
	...
	СтруктураПараметров = Новый Структура;
	СтруктураПараметров.Вставить("ШкафКупеПоКаталогу", ШкафПоКаталогу);
	СтруктураПараметров.Вставить("Переделка", Объект.Изделие = Переделка);
	СтруктураПараметров.Вставить("ПоказатьТелефон", Объект.Изделие = Детали И Объект.Контрагент = ЧастноеЛицо);
