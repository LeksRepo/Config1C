...
Процедура Подбор(Команда)
	ОткрытьФорму("Документ.Спецификация.Форма.ФормаПодборКомплектации",Неопределено,ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВКомплектациюИзПодбора(Выбранные)

	Для каждого КомЭлемент Из Объект.Комплектация Цикл
		
		Инд=0;
		Для каждого ВыбЭлемент Из Выбранные Цикл
		
			Если КомЭлемент.Номенклатура=ВыбЭлемент.Номенклатура Тогда
			
				 КомЭлемент.Количество=КомЭлемент.Количество+ВыбЭлемент.Количество;
				 Выбранные.Удалить(Инд);
				 Прервать;
