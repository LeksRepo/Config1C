...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки)
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		ВместимостьНаЛисте(Номенклатура, ВысотаДетали, ШиринаДетали, Ошибки, "фасада у детали № " + НомерСтроки, "СписокМатериалы", НомерСтроки);
...
Функция ЗаполнитьСтрокуСписокНоменклатуры(ТекущийОбъект, Номенклатура, Количество, НомерИзделия)
	...
	НоваяСтрока.УслугаСМатериаломЗаказчика = ПредоставляетЗаказчик И Номенклатура.ВидНоменклатуры = Перечисления.ВидыНоменклатуры.Услуга;
