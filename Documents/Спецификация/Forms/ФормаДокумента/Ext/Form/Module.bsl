...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если ТекущийОбъект.Изделие.Серийное Тогда
		...
		Для каждого Строка Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
			КоличествоЧасов = КоличествоЧасов + Строка.Изделие.КоличествоУслугиИзготовления;
		КонецЦикла;
		
		ДобавитьСтроку(ТекущийОбъект, Справочники.Номенклатура.НормоЧасСерийногоИзделия, КоличествоЧасов, Строка.НомерСтроки);
...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки, СоответствиеОбрезков)
	...
	Для каждого Строка Из ТекущийОбъект.Комплектация Цикл
		...
		ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Строка.Количество, Строка.НомерИзделия, Ошибки, "строки комплектации № " + Строка.НомерСтроки, "Комплектация", Строка.НомерСтроки, Ложь, Ложь, -1, Строка.ПоКаталогуМестоОбработки);
			...
			Попытка
				...
				Если Номенклатура.МестоОбработки = Перечисления.МестоОбработки.ПоКаталогуВЦех Тогда
					...
					Если Строка.ВСборе И Элемент.Сборка Тогда
						МестоОбработки = "Цех";
					Иначе
						МестоОбработки = "Склад";
					КонецЕсли;
					
				КонецЕсли;
				
				ДобавитьКомплектацию(Номенклатура, Количество, Элемент.НомерСтроки, МестоОбработки);
				
				Если ДобавитьДопЭлементДляНожек Тогда
					
					ДобавитьДопЭлементДляНожек = Ложь;
					КоличествоДоп = Цел(Количество / 2);
					ДобавитьКомплектацию(Номенклатура.ДополнительныйЭлемент, КоличествоДоп, Элемент.НомерСтроки, МестоОбработки);			
