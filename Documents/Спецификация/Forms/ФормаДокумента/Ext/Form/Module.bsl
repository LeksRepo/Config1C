	...
	Для Каждого Элемент Из Объект.СписокИзделийПоКаталогу Цикл
		...
		Для Каждого Строка Из СписокНоменклатурыИзделия Цикл
			...
			Попытка
				...
				Если НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.МонтажныйУголок и СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("ШкафПоКаталогу") Тогда
					
					Номенклатура = ?(СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Свойство("МонтажныйУголок"), СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.МонтажныйУголок, Номенклатура);
					
				ИначеЕсли НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Заглушка и СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("ШкафПоКаталогу") Тогда
					
					Номенклатура = ?(СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Свойство("Заглушка"), СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Заглушка, номенклатура);
		...
		Для Каждого Строка Из СписокЯщикиИзделия Цикл
			...
			Если НоваяСтрока.ВидФасада <> "Нет" Тогда
				...
				Если ИзделиеШкафКупе Тогда
					...
					Если СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("ШкафПоКаталогу") и СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Свойство("Кромка045_19") и
						СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Свойство("Кромка2_19") Тогда
						
						НоваяСтрока.КромкаНоменклатура = СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Кромка045_19;
						НоваяСтрока.КромкаФасадНоменклатура = СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Кромка2_19;
				...
				Иначе
					...
					Если СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("КухняПоКаталогу") и СтруктураПодбираемойПоЦветуНоменклатуры.КухняПоКаталогу.Свойство("Кромка045_19") и
						СтруктураПодбираемойПоЦветуНоменклатуры.КухняПоКаталогу.Свойство("Кромка2_19") Тогда
						
						НоваяСтрока.КромкаНоменклатура = СтруктураПодбираемойПоЦветуНоменклатуры.КухняПоКаталогу.Кромка045_19;
						НоваяСтрока.КромкаФасадНоменклатура = СтруктураПодбираемойПоЦветуНоменклатуры.КухняПоКаталогу.Кромка2_19;
...
Функция ОбновитьДоступностьКоманд()
	...
	ПроверенЛогистом = НЕ Объект.ЕстьМатериалПодЗаказ ИЛИ СтатусСпецификации = Перечисления.СтатусыСпецификации.ПроверяетсяТехнологом;
...
Процедура АдресМонтажаНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		ПараметрыАдреса.Вставить("ТолькоПросмотр", ТолькоПросмотр);
