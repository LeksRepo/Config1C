...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если Объект.Изделие.ДобавлятьНаценки Тогда
		...
		Если НЕ Объект.ЭтоКухня Тогда
			
			ДобавитьСтроку(ТекущийОбъект, Номенклатура.ВыездМастера, 1, 0);
			
		Иначе
			
			ДобавитьСтроку(ТекущийОбъект, Номенклатура.ВыездМастера, 2, 0);
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|				ТОГДА (ЕСТЬNULL(РозничныеЦены.Розничная, 0) - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * 0.01 * &СкидкаНаУслуги) * &Коэффициент
	...
	ПараметрыОтбора.Вставить("Номенклатура", СпрНоменклатура.РозничнаяНаценка);
	...
	Для каждого Строка Из ТекущийОбъект.НеноменклатурныйМатериал Цикл
		...
		//НоваяСтрока.Цена = Строка.Цена; // спецификация 7050, каким блять хуем смогли оставить цену пустой?
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.НадписьДилерам.Видимость = Объект.Дилерский;
	НадписьДилерам = "Вы можете размещать заказ на производстве без проверки.";
