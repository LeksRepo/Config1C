...
Процедура ОбработкаИзделийПоКаталогу()
	...
	Если ИзделиеШкафКупе Тогда 
		...
		ЕстьДверь = СписокДверей.Количество() = 1;
		ВысотаПроема = ?(ЕстьДверь, СписокДверей[0].Двери.ВысотаПроема, 0);
		ШиринаПроема = ?(ЕстьДверь, СписокДверей[0].Двери.ШиринаПроема, 0);
		...
		СтрокаСтенка = ТаблицаИзделийПоКаталогу.Найти(Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент,"ВидИзделия");
