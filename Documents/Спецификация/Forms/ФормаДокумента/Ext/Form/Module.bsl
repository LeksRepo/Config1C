...
Функция ПроверитьЗаменитьКратность()
	...
	СтруктураПроверки.Вставить("Ошибки", Ошибки);
	СтруктураПроверки.СтрокаСообщенияОКратности = СтрокаСообщенияОКратности;
...
Функция УвеличитьМатериалыДоЦелыхЛистов(ТекущийОбъект)
	...
	Для Каждого Элемент Из ТаблицаОкруглятьДоЛистов Цикл
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	Если НЕ ТекущийОбъект.Дилерский Тогда
		...
		КоэффициентУслуги  = Макс(ТекущийОбъект.Офис.КоэффициентУслуги, 1);
		КоэффициентМатериалы = Макс(ТекущийОбъект.Офис.КоэффициентМатериалы, 1);
	Иначе
		//КоэфициентОфиса = 1;
		КоэффициентУслуги  = 1;
	...
	|					КОГДА ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)
	|							ИЛИ ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Комплект)
	|						ТОГДА ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &КоэффициентМатериалы * (ТаблицаСпискаНоменклатуры.Количество + ТаблицаСпискаНоменклатуры.Количество * ЕСТЬNULL(НоменклатураПодразделений.ПроцентОтхода, 0) / 100)
	|					ИНАЧЕ ЕСТЬNULL(РозничныеЦеныСрез.Розничная, 0) * &КоэффициентУслуги * (ТаблицаСпискаНоменклатуры.Количество + ТаблицаСпискаНоменклатуры.Количество * ЕСТЬNULL(НоменклатураПодразделений.ПроцентОтхода, 0) / 100)
	|				КОНЕЦ
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	СтруктураПроверки = ПроверитьЗаменитьКратность();
	...
	Отказ = НЕ СтруктураПроверки.Ошибки = Неопределено;
...
Процедура Подбор(Команда)
	...
	Пар = Новый Структура();
	Пар.Вставить("Производство", Объект.Подразделение);
