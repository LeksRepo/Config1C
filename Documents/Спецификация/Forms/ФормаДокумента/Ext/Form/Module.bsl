...
&НаСервере
Функция ЗаписатьСтрокуРаскроя(ДанныеДляРаскроя)
	...
	//СтруктураРаскроя = РегистрыСведений.РаскройДеталей.ПолучитьСтрокуРаскроя(Объект.Ссылка);
	
	Запись.СтрокаРаскрой = ДанныеДляРаскроя;
	Запись.Объект = Объект.Ссылка;
	
	НаборЗаписей.Записать();
	
	//Объект.СписокЛистовНоменклатуры.Загрузить(СтруктураРаскроя.ТаблицаЛистовНоменклатуры);
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	Для каждого Строка Из ТаблицаСпискаНоменклатуры Цикл
		...
		Если ПроцентОтхода > 0 И НЕ Строка.ПодЗаказ Тогда //под заказ без процентов отхода?
...
Функция ОбновитьЭлементыФормы(лФорма, лПараметры)
	...
	Если лПараметры.Свойство("ЕстьМатериалПодЗаказ") Тогда
		Элементы.ЕстьМатериалПодЗаказ.Видимость = лПараметры.ЕстьМатериалПодЗаказ;
	КонецЕсли;
...
Функция ОбновитьДоступностьКоманд()
	...
	КомандыИзменитьСтатус.ФормаПроверенаЛогистом.Доступность = ПользовательАдминистратор;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	ЗаписатьСтрокуРаскроя(ПараметрыЗаписи.ДанныеДляРаскроя);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	// { Васильев Александр Леонидович [29.05.2014]
	// УБРАТЬ НАХУЙ ОТСЮДА
	// } Васильев Александр Леонидович [29.05.2014]
	
	// дата монтажа
	
	Элементы.ЕстьМатериалПодЗаказ.Видимость = ТекущийОбъект.ЕстьМатериалПодЗаказ;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Модифицированность И ТекущийСтатус = Перечисления.СтатусыСпецификации.ПроверенТехнологом И ТекущийОбъект.Изделие <> Справочники.Изделия.ШкафКупеПоКаталогу Тогда
...
Процедура ПередатьВПроизводство(Команда)
	...
	Если ПередатьВПроизводствоНаСервере() Тогда
		Закрыть();
	КонецЕсли;
...
Функция ПередатьВПроизводствоНаСервере()
	...
	Если Объект.Дилерский И Объект.ЕстьМатериалПодЗаказ И СтатусСпецификации <> Перечисления.СтатусыСпецификации.ПроверенЛогистом Тогда
...
Процедура ПроверенаЛогистом(Команда)
	...
	Иначе
		НовыйСтаутс = ПредопределенноеЗначение("Перечисление.СтатусыСпецификации.ПроверяетсяТехнологом");
	КонецЕсли;
...
Процедура Двери(Команда)
	...
	ПараметрыФормы.Вставить("ЗначенияЗаполнения", Новый Структура("Спецификация", Объект.Ссылка));
