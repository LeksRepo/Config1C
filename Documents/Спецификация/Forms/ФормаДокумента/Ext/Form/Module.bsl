...
Функция ОбновитьДоступностьКоманд()
	...
	ПроверенЛогистом = НЕ Объект.ЕстьМатериалПодЗаказ ИЛИ СтатусСпецификации = Перечисления.СтатусыСпецификации.ПроверенЛогистом;
	...
	ДизайнерВПроизводство = Объект.Изделие = Справочники.Изделия.Детали И Дизайнер;
	ТехнологВПроизводство = СтатусСпецификации = Перечисления.СтатусыСпецификации.Рассчитывается И Технолог;
	
	Ком = КоманднаяПанель.ПодчиненныеЭлементы.Статусы.ПодчиненныеЭлементы;
	
	Ком.ФормаНаПроверкуИнженеру.Доступность = Администратор ИЛИ НЕ Дилер;
	Ком.ФормаЕстьЗамечания.Доступность = Администратор ИЛИ Технолог;
	Ком.ФормаПроверенаЛогистом.Доступность = Администратор;
...
// { Васильев Александр Леонидович [05.06.2014]
// непонятные команды Двери и Добавить двери
// считаю должны быть ВыбратьДверь и ДобавитьДверь
// } Васильев Александр Леонидович [05.06.2014]
...
#Область События_таблицы_НеноменклатурныйМатериал
//////////////////////////////////////////////////////////////////////////////////
