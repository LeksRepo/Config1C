...
Функция ЗаполнитьМатериалы(ТекущийОбъект)
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если ФасадСПрофилем Тогда
			...
			// Миха, твой косяк. Очень дохуя АГТ профиля не досчитались
			...
			// Вот правильная:
			// ПлощадьПрофиля= (((2 * ВысотаДетали + 4 * Строка.УниверсальнаяКромка.ГлубинаДетали) + (2 * ШиринаДетали + 4 * Строка.УниверсальнаяКромка.ГлубинаДетали)) / 1000) * КоличествоДеталей;
			// Найди отличия ёпт
			// } Васильев Александр Леонидович [28.12.2013]
...
Процедура ОфисПриИзменении(Элемент)
	...
	Объект.АдресМонтажа = "ВведитеАдрес";
	ОбновитьСкидки();
