...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Иначе
		...
		Для каждого Строка Из ТекущийОбъект.СписокДеталей Цикл
			...
			Если ЭтоФасад И ЗначениеЗаполнено(Строка.НоменклатураДляСклеивания) И Строка.НоменклатураДляСклеивания.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Ручка Тогда  
				
				ДопУслуга = Строка.НоменклатураДляСклеивания.ДополнительнаяУслуга;
