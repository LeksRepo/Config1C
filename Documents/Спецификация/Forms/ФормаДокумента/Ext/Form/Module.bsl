...
Функция ЗаполнитьМатериалы(ТекущийОбъект, Ошибки)
	...
	Для каждого Строка Из ТекущийОбъект.СписокЯщики Цикл
		...
		Если НЕ Строка.БезНаправляющих Тогда
			ДобавитьСтроку(ТекущийОбъект, Строка.НаправляющиеНоменклатура, КоличествоЯщиков, Строка.НомерИзделия, Ошибки, "направляющей у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
		КонецЕсли;
		...
		Если Строка.ВидЯщика = Перечисления.ВидыЯщика.Обычный Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, ((Строка.ШиринаБоковойСтороны * Строка.ВысотаБоковойСтороны * 2) / 1000000) * КоличествоЯщиков, Строка.НомерИзделия, Ошибки, "боковой стороны у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, ((Строка.ДлинаБоковойСтороны * Строка.ВысотаБоковойСтороны * 2) / 1000000) * КоличествоЯщиков, Строка.НомерИзделия, Ошибки, "боковой стороны у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
			...
			Если ЗначениеЗаполнено(Строка.ДлинаРебро) Тогда
				...
				ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, ((Строка.ДлинаРебро * Строка.ВысотаЯщика) / 1000000) * КоличествоЯщиков, Строка.НомерИзделия, Ошибки, "ребра у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
		...
		Иначе
			...
			ВместимостьНаЛисте(Строка.Номенклатура, Строка.ШиринаБоковойСтороны, Строка.ВысотаБоковойСтороны, Ошибки, "боковой стороны у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
			ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Площадь, Строка.НомерИзделия, Ошибки, "боковой стороны у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
		...
		ДобавитьСтроку(ТекущийОбъект, Строка.КромкаНоменклатура, ПлощадьКромки, Строка.НомерИзделия, Ошибки, "кромки у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
		...
		ДобавитьСтроку(ТекущийОбъект, Строка.ДноНоменклатура, Площадь, Строка.НомерИзделия, Ошибки, "дно у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
		
		//Добавляем фасады: материал, кромку и ручки
		Если Строка.ВидФасада <> "Нет" Тогда
			
			Если Строка.ШиринаФасад > 0 И Строка.ВысотаФасад > 0 Тогда
			...
			ВместимостьНаЛисте(Строка.ФасадНоменклатура, Строка.ВысотаФасад, Строка.ШиринаФасад, Ошибки, "фасада у ящика № " + Строка.НомерСтроки, "СписокЯщики", Строка.НомерСтроки);
...
Функция ЗаполнитьНеноменклатурныйМатериал()
	...
	Если Объект.СписокНоменклатуры.Количество() > 0 Тогда
		...
		Для Каждого Строка Из Объект.СписокНоменклатуры Цикл
			...
			Если Номенклатура.МатериалЗаказчика 
...
&НаСервере
Функция ДобавитьСтроку(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия, Ошибки = Неопределено, ВидНоменклатуры = "", Таблица = Неопределено, НомерСтроки = 0)
...
Функция ПолучитьУведомлениеМатериалПодЗаказ(Подразделение)
	...
	ТекстУведомления = "Материал под заказ. ";
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПараметрыЗаписи.Вставить("ПередатьВПроизводство", Ложь);
	...
	ТекущийОбъект.ДополнительныеСвойства.Вставить("ПередатьВПроизводство", ПараметрыЗаписи.ПередатьВПроизводство);
