...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если Объект.Упаковка Тогда
		...
		// пробовал хоть включать? пиздец
		//
		//Если Объект.СписокДверей.Количество() > 0 Тогда
		//	
		//	КоличествоДверей = ПосчитатьДвери(); // как бы NULL бывает
		//	
		//КонецЕсли;
		...
		Если КоличествоДверей > 0 Тогда
			...
			// 1.5 упаковки это нормально?
			//?(КоличествоДверей = Цел(КоличествоДверей), КоличествоДверей/2, Окр(КоличествоДверей/2+0.5)), 0)
...
&НаСервере
Функция ПосчитатьДвери()
	
	// нихуя что запрос идёт к базе, а там ещё может не быть этого объекта?
	...
	|ГДЕ
	|	СпецификацияСписокДверей.Ссылка = &Ссылка";
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	Возврат Выборка.ДвериКоличество;
	
КонецФункции // ПосчитатьДвери()
