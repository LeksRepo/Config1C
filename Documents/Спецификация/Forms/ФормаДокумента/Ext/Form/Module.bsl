...
Функция ЗаполнитьНеноменклатурныйМатериал()
	...
	Элементы.НеноменклатурныйМатериалНоменклатура.СписокВыбора.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
...
Функция ЗаполнитьМатериалы(ТекущийОбъект)
	...
	ЕстьНеноменклатурный = ТекущийОбъект.НеноменклатурныйМатериал.Количество() > 0;
	...
	Для каждого Строка Из Изделие.СписокНоменклатуры Цикл
		ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Строка.Количество, 0, ЕстьНеноменклатурный);
	КонецЦикла;
	...
	Для Каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Если НЕ Клееная И ЗначениеЗаполнено(Строка.НоменклатураДляСклеивания) Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.НоменклатураДляСклеивания, Строка.Количество, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Если НЕ ФасадСПрофилем Тогда
			...
			Если Материал = "Столешница" или Материал = "Пристенок" Тогда
				...
				ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Иначе
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
			ПлощадьПрофиля= (((2 * ВысотаДетали + 2 * Строка.УниверсальнаяКромка.ШиринаДетали) + (2 * ШиринаДетали - 2 * Строка.УниверсальнаяКромка.ШиринаДетали)) / 1000) * КоличествоДеталей;
			ДобавитьСтроку(ТекущийОбъект, Строка.УниверсальнаяКромка, ПлощадьПрофиля, Строка.НомерИзделия, ЕстьНеноменклатурный);
			ШиринаВставки 	= Строка.Номенклатура.ГлубинаДетали;
			ШиринаПаза		= Строка.УниверсальнаяКромка.ШиринаПаза;
			
			Если Материал = "ФасадАГТ" Тогда
				
				ДобавитьСтроку(ТекущийОбъект, Номенклатура.Шпонка, КоличествоДеталей * 8, Строка.НомерИзделия, ЕстьНеноменклатурный);
				...
				Если ШиринаПаза > ШиринаВставки Тогда
					...
					2 / 1000) * КоличествоДеталей, Строка.НомерИзделия, ЕстьНеноменклатурный);
					
				КонецЕсли;
				
			Иначе 
				
				ДобавитьСтроку(ТекущийОбъект, Номенклатура.УголСоединительныйКАлюминевомуПрофилю, КоличествоДеталей * 4, Строка.НомерИзделия, ЕстьНеноменклатурный);
				ДобавитьСтроку(ТекущийОбъект, Номенклатура.ВтулкаДистанционнаяАлюминиевая, КоличествоДеталей * 2, Строка.НомерИзделия, ЕстьНеноменклатурный);
				...
				Если ШиринаПаза > ШиринаВставки Тогда
					...
					(ШиринаДетали + 2 * Строка.УниверсальнаяКромка.ШиринаПаза)) * 2 / 1000) * КоличествоДеталей, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Если ЗначениеЗаполнено(Строка.Петли) Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.Петли, Строка.КоличествоПетель * КоличествоДеталей, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Если ФасадМДФ Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.УниверсальнаяКромка, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
			
		КонецЕсли;
		
		Если Клееная Тогда
			
			ДобавитьСтроку(ТекущийОбъект, Строка.НоменклатураДляСклеивания, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Если НЕ ФасадСПрофилем Тогда
			...
			Если ТаблицаКантовПоместу.Количество() > 0 Тогда
				...
				ТаблицаКантовПоместу.Свернуть("Номенклатура, ЕдиницаИзмерения, НомерИзделия", "Количество");
				...
				Для каждого Строчка Из ТаблицаКантовПоместу Цикл
					...
					НоваяСтрока.Количество 			= Строчка.Количество + Строчка.Количество * 0.01 * 15; //Дима сказал добавить к канту по месту 15 процентов
			...
			Для Каждого Строчка Из КромкиСверхуСнизу Цикл
				...
				Площадь = ШиринаДетали * КоличествоДеталей / 1000;
				ДобавитьСтроку(ТекущийОбъект, Строчка, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
			...
			Если ЗначениеЗаполнено(КантыИзРедактора) Тогда 
				...
				Для Каждого Кромка Из МассивКантов Цикл
					...
					ДобавитьСтроку(ТекущийОбъект, НоменклатураКанта, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
	...
	Для каждого Строка Из ТекущийОбъект.СписокЯщики Цикл
		...
		Если НЕ Строка.БезНаправляющих Тогда
			ДобавитьСтроку(ТекущийОбъект, Строка.НаправляющиеНоменклатура, КоличествоЯщиков, Строка.НомерИзделия, ЕстьНеноменклатурный);
		КонецЕсли;
		...
		Иначе
			...
			Если Строка.ВысотаЯщика > 150 Тогда
				...
				Если ЗначениеЗаполнено(РейлингКНаправляющим) Тогда
					ДобавитьСтроку(ТекущийОбъект, РейлингКНаправляющим, ?(Строка.ВысотаЯщика < 201, КоличествоЯщиков, КоличествоЯщиков * 2), Строка.НомерИзделия, ЕстьНеноменклатурный);
				КонецЕсли;
				...
				Если Строка.ВидЯщика = Перечисления.ВидыЯщика.Тандембокс Тогда
					...
					ДобавитьСтроку(ТекущийОбъект, Соединитель, КоличествоЯщиков, Строка.НомерИзделия, ЕстьНеноменклатурный);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ДобавитьСтроку(ТекущийОбъект, Строка.КромкаНоменклатура, ПлощадьКромки, Строка.НомерИзделия, ЕстьНеноменклатурный);
		ДобавитьСтроку(ТекущийОбъект, Строка.Номенклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		ДобавитьСтроку(ТекущийОбъект, Строка.ДноНоменклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
		...
		Если Строка.ВидФасада <> "Нет" Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.ФасадНоменклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
			
			//Ручки для ящиков
			Если Строка.КоличествоРучек > 0 Тогда
				
				Площадь = Строка.КоличествоРучек*КоличествоЯщиков;
				ДобавитьСтроку(ТекущийОбъект, Строка.РучкаНоменклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
			...
			ДобавитьСтроку(ТекущийОбъект, Строка.КромкаФасадНоменклатура, Площадь, Строка.НомерИзделия, ЕстьНеноменклатурный);
	...
	Для каждого Строка Из ТекущийОбъект.Комплектация Цикл
		...
		ДобавитьСтрокуПоОсобымПравилам(ТекущийОбъект, Строка.Номенклатура, Строка.Количество);
	...
	Для каждого Строка Из ТекущийОбъект.СписокДверей Цикл
		...
		ДобавитьСтроку(ТекущийОбъект, Справочники.Номенклатура.КлючШестигранный, 1, 0, ЕстьНеноменклатурный);
		
		Для каждого СтрокаНоменклатуры Из ТаблицаНоменклатурыДверей Цикл
			
			ДобавитьСтроку(ТекущийОбъект, СтрокаНоменклатуры.Номенклатура, СтрокаНоменклатуры.КоличествоСОтходом, 0, ЕстьНеноменклатурный);
...
&НаСервере
Процедура ДобавитьСтрокуПоОсобымПравилам(ТекущийОбъект, Номенклатура, Количество)
	
	НоваяСтрока 								= ТекущийОбъект.СписокНоменклатуры.Добавить();
	НоваяСтрока.Номенклатура 			= Номенклатура;
	НоваяСтрока.ЕдиницаИзмерения 	= НоваяСтрока.Номенклатура.ЕдиницаИзмерения;
	ПроцентОтхода 							= Номенклатура.ПроцентОтхода;
	НоваяСтрока.Количество 			= Количество;
	НоваяСтрока.НомерИзделия 		= 0;
	МестоОбработки 						= Номенклатура.МестоОбработки;
	
	НоваяСтрока.ЧерезСклад 			=Номенклатура.ВидНоменклатуры <> Перечисления.ВидыНоменклатуры.Услуга;
	
КонецПроцедуры

&НаСервере
Процедура ДобавитьСтроку(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия, ЕстьНеноменклатурный = Ложь)
	...
	Если НЕ Номенклатура = Справочники.Номенклатура.ПустаяСсылка() Тогда
		...
		Если Площадь > 0 Тогда
			...
			Если Номенклатура.Неноменклатурный Тогда
				...
				Если НЕ ЕстьНеноменклатурный Тогда
					
					ДобавитьНеноменклатурныйМатериал(ТекущийОбъект, Номенклатура, Площадь);
			...
			Иначе
				...
				НоваяСтрока.ЧерезСклад = МестоОбработки = Перечисления.МестоОбработки.Склад ИЛИ МестоОбработки = Перечисления.МестоОбработки.ПоКаталогуВЦех и НомерИзделия = 0;
...
&НаСервере
Процедура ДобавитьНеноменклатурныйМатериал(ТекущийОбъект, Номенклатура, Площадь)
	
	//НоваяСтрока = ТекущийОбъект.НеноменклатурныйМатериал.Добавить();
	//НоваяСтрока.Номенклатура = Номенклатура;
	//НоваяСтрока.ЕдиницаИзмерения = Номенклатура.ЕдиницаИзмерения;
	//
	//ПроцентОтхода = Номенклатура.ПроцентОтхода;
	//НоваяСтрока.Количество = Площадь + Площадь * 0.01 * ПроцентОтхода;
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|	МИНИМУМ(ВЫБОР
	|		КОГДА ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал) и ((ЕСТЬNULL(РозничныеЦены.Розничная, 0) - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * 0.01 * &СкидкаНаМатериал) * &Коэффициент) >= ЕСТЬNULL(РозничныеЦены.Внутренняя, 0)
	|			ТОГДА (ЕСТЬNULL(РозничныеЦены.Розничная, 0) - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * 0.01 * &СкидкаНаМатериал) * &Коэффициент 
	|		КОГДА ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга) и ((ЕСТЬNULL(РозничныеЦены.Розничная, 0) - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * 0.01 * &СкидкаНаУслуги) * &Коэффициент) >= ЕСТЬNULL(РозничныеЦены.Внутренняя, 0)
	|			ТОГДА (ЕСТЬNULL(РозничныеЦены.Розничная, 0) - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * 0.01 * &СкидкаНаМатериал) * &Коэффициент 
	|		ИНАЧЕ ЕСТЬNULL(РозничныеЦены.Внутренняя, 0)
	|	КОНЕЦ) КАК Цена,
	|	СУММА(ВЫБОР
	|		КОГДА ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал) и (ЕСТЬNULL(РозничныеЦены.Розничная, 0) * ТаблицаСпискаНоменклатуры.Количество - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * ТаблицаСпискаНоменклатуры.Количество * 0.01 * &СкидкаНаМатериал) * &Коэффициент >= ЕСТЬNULL(РозничныеЦены.Внутренняя, 0) * ТаблицаСпискаНоменклатуры.Количество
	...
	|		КОГДА ТаблицаСпискаНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга) и (ЕСТЬNULL(РозничныеЦены.Розничная, 0) * ТаблицаСпискаНоменклатуры.Количество - ЕСТЬNULL(РозничныеЦены.Розничная, 0) * ТаблицаСпискаНоменклатуры.Количество * 0.01 * &СкидкаНаУслуги) * &Коэффициент >= ЕСТЬNULL(РозничныеЦены.Внутренняя, 0) * ТаблицаСпискаНоменклатуры.Количество
...
Процедура Двери(Команда)
	...
	ПараметрыФормы = Новый Структура("Подразделение", Объект.Производство);
