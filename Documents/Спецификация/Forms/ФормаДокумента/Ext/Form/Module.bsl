...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Для Каждого ИзделиеК Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
		...
		Стр.Вставить("УпаковкаВСборе",ИзделиеК.Изделие.ВидИзделияПоКаталогу.УпаковкаВСборе);
	...
	Для каждого Строка Из ТекущийОбъект.СписокМатериалы Цикл
		...
		Иначе
			...
			Если НЕ ДанныеИзделия.Сборка И ДанныеИзделия.УпаковкаВСборе Тогда
	...
	Если ТекущийОбъект.Упаковка Тогда
		...
		Для каждого Строка Из ТекущийОбъект.СписокИзделийПоКаталогу Цикл
			...
			Если Строка.Сборка Тогда
				КоличествоКоробовДляУпаковки = КоличествоКоробовДляУпаковки + Число(Строка.Изделие.ВидИзделияПоКаталогу.УпаковкаВСборе);
			КонецЕсли;
