	...
	Для Каждого Элемент Из Объект.СписокИзделийПоКаталогу Цикл
		...
		Для Каждого Строка Из СписокНоменклатурыИзделия Цикл
			...
			Попытка
				...
				Если Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ФурнитураКрепежная и СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("ШкафПоКаталогу") Тогда
					
					Номенклатура = СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.ФурнитураКрепежная;
					
				ИначеЕсли Номенклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ФурнитураКрепежная и СтруктураПодбираемойПоЦветуНоменклатуры.Свойство("ШкафПоКаталогу") Тогда
					
					Номенклатура = СтруктураПодбираемойПоЦветуНоменклатуры.ШкафПоКаталогу.Фурнитура;
