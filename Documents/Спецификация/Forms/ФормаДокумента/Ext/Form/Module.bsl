...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если Объект.НеноменклатурныйМатериал.Количество() > 0 ИЛИ Объект.ЕстьМатериалПодЗаказ Тогда
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|	МАКСИМУМ(ЕСТЬNULL(НоменклатураПодразделений.ПодЗаказ, ЛОЖЬ)) КАК ПодЗаказ,
	|	МАКСИМУМ(ЕСТЬNULL(НоменклатураПодразделений.Доступность, ЛОЖЬ)) КАК Доступность
	|ИЗ
	|	ТаблицаСпискаНоменклатуры КАК ТаблицаСпискаНоменклатуры
