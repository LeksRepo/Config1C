...
Функция ЗаполнитьУслуги(ТекущийОбъект, СтруктураРаскроя)
	...
	Для Каждого СтрокаТабЛистов Из СтруктураРаскроя.ТаблицаЛистовНоменклатуры Цикл
		...
		//RonEXI edit: решили считать по всем листам
...
Процедура МасштабироватьИЗаполнитьПараметрыРедактированнойДетали(СтрокаФлэш, СтрокаТаблицы, СтрКромка)
	...
	СтрокаТаблицы.ДлинаКривогоПила = Строка(Число(МассивСимволов[1])*Число(СтрокаТаблицы.Количество));
	ДлинаКривогоПила = Число(МассивСимволов[1])*Число(СтрокаТаблицы.Количество);
	
	Если ДлинаКривогоПила > 0 Тогда
			//ДобавитьСтроку(Объект.Ссылка, Справочники.Номенклатура.КриволинейныйПилЗонаКромки, ДлинаКривогоПила / 1000 * Число(СтрокаТаблицы.Количество), Число(СтрокаТаблицы.НомерИзделия));
	КонецЕсли;
