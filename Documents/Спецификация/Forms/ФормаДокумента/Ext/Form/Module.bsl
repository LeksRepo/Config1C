...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|	СУММА(втДанные.Цена * втДанные.КоэффициентНаценки * втДанные.Количество) КАК РозничнаяСтоимость,
	|	СУММА(втДанные.Цена * втДанные.Количество) КАК СуммаБезНаценкиОфиса,
	|	СРЕДНЕЕ(втДанные.Цена * втДанные.КоэффициентНаценки) КАК Цена,
	|	втДанные.ЧерезСклад,
		...
		Для Каждого Строка Из СписокЯщикиИзделия Цикл
			...
			Иначе
				...
				Если ЗначениеЗаполнено(СтрокаПоиска) Тогда
					...
					НоваяСтрока.СхемаЯщика = Справочники.СхемыЯщиков.НайтиПоНаименованию(НоваяСтрока.ВидЯщика, Ложь);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	ПроверитьДоступностьПодразделениямНаСервере(ТекущийОбъект, Ошибки);
	...
	ЗаписатьСтрокуРаскроя(ТекущийОбъект.Ссылка, СтруктураРаскроя);
...
&НаСервере
Функция ПроверитьДоступностьПодразделениямНаСервере(фнОбъект, Ошибки)
	
	СписокМатериаловЗаказчика = Новый ТаблицаЗначений;
	СписокМатериаловЗаказчика.Колонки.Добавить("Номенклатура");
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Подразделение", фнОбъект.Подразделение);
	Запрос.УстановитьПараметр("СписокНоменклатуры", фнОбъект.СписокНоменклатуры);
	Запрос.УстановитьПараметр("СписокМатериаловЗаказчика", фнОбъект.СписокМатериаловЗаказчика);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВЫРАЗИТЬ(СписокМатериаловЗаказчика.Номенклатура КАК Справочник.Номенклатура) КАК Номенклатура,
	...
	|		ПО СписокНоменклатуры.Номенклатура = СписокМатериаловЗаказчика.Номенклатура
	|ГДЕ
	|	(НЕ ЕСТЬNULL(НоменклатураПодразделений.Доступность, ЛОЖЬ)
	|			ИЛИ СписокМатериаловЗаказчика.ПредоставитЗаказчик)
	|	И СписокНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)";
	
	РезультатЗапроса = Запрос.Выполнить();
	фнОбъект.СписокМатериаловЗаказчика.Загрузить(РезультатЗапроса.Выгрузить());
...
Процедура УведомлениеМатериалПодЗаказНажатие(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Сообщить("Смотрите на вкладке 'Под заказ'");
