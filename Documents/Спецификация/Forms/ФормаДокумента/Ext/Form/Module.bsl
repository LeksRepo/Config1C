...
Функция Собрать()
	...
	Если ИмяТекущейСтраницы = "СтраницаМатериалы"  Тогда
		...
		//ИначеЕсли ИмяТекущейСтраницы = "СтраницаСтекла" Тогда
		//	
		//	ИмяПодбора = "ФормаСтекла";
		//	ТаблицаДляФормы = Объект.СписокСтекла;
		//	СтрокаДанных = Элементы.СписокСтекла.ТекущиеДанные;
		//	ВладелецПодобра = Элементы.СписокСтекла;
	...
	ИначеЕсли ИмяТекущейСтраницы = "СтраницаЯщики" Тогда
		...
		//	
		//	ИмяПодбора 		= "ФормаФурнитура";
		//	ТаблицаДляФормы = Объект.Комплектация;
		//	СтрокаДанных	= Элементы.Комплектация.ТекущиеДанные;
		//	ВладелецПодобра = Элементы.Комплектация;
		
		//ИначеЕсли ИмяТекущейСтраницы = "СтраницаФасады" Тогда
		//	
		//	ИмяПодбора = "ФормаФасады";
		//	ТаблицаДляФормы = Объект.СписокФасады;
		//	СтрокаДанных = Элементы.СписокФасады.ТекущиеДанные;
		//	ВладелецПодобра = Элементы.СписокФасады;
		//	
		//ИначеЕсли ИмяТекущейСтраницы = "СтраницаСтолешницы" Тогда
		//	
		//	ИмяПодбора = "ФормаСтолешницы";
		//	ТаблицаДляФормы = Объект.СписокСтолешницы;
		//	СтрокаДанных = Элементы.СписокСтолешницы.ТекущиеДанные;
		//	ВладелецПодобра = Элементы.СписокСтолешницы;
...
Функция ЗаполнитьУслуги(ТекущийОбъект)
	...
	Если НЕ ДилерскийЗаказ И НЕ Объект.Изделие = Справочники.Изделия.Детали И НЕ Объект.Изделие = Справочники.Изделия.Переделка Тогда
		...
		Если ЛДСП ИЛИ Материал = "ДВП" ИЛИ Материал = "МДФ" ИЛИ Столешница Тогда
			...
			Если ЗначениеЗаполнено(КантыИзРедактора) Тогда 
				...
				Если Кромка = "Обычная" ИЛИ Кромка = "ДляСтолешниц" Тогда
					...
					ДобавитьУслугуКромления(ТекущийОбъект, НоменклатураКанта, ДлинаКанта, Кромка = "ДляСтолешниц", Строка.НомерИзделия);
		...
		ИначеЕсли Фасад Тогда
			...
			Если Материал = "ФасадАГТ" Тогда
				...
				Если ШиринаПаза < ШиринаВставки Тогда 
					...
					ДобавитьСтроку(ТекущийОбъект, Номенклатура.ФрезеровкаЛДСПЗонаКухонь, Строка.Количество, Строка.НомерИзделия);
...
#ОБЛАСТЬ ЯЩИКИ
	Для каждого Строка Из Объект.СписокЯщики Цикл
		...
		ИначеЕсли НоменклатурнаяГруппаФасада = СправочникНоменклатурнаяГруппа.ЛДСП16 ИЛИ НоменклатурнаяГруппаФасада = СправочникНоменклатурнаяГруппа.МДФ Тогда	
...
#КОНЕЦОБЛАСТИ
	...
	Если НЕ Объект.Изделие = Справочники.Изделия.Детали Тогда
	...
	Если Объект.Упаковка Тогда
		...
		ДобавитьСтроку(ТекущийОбъект, Номенклатура.УпаковкаДеталей, ПлощадьИзготавливаемыхМатериалов, 0);
	...
	Если Объект.Доставка Тогда
		...
		Если Объект.Километраж > 0 Тогда
			...
			ДобавитьСтроку(ТекущийОбъект, Номенклатура.ДоставкаЗаГородом, Объект.Километраж, 0);
	...
	Для каждого Строка Из ТекущийОбъект.СписокЯщики Цикл
		...
		Иначе
			...
			Если Строка.ВысотаЯщика > 150 Тогда
				...
				ДобавитьСтроку(ТекущийОбъект, НайтиНаправляющиеНоменклатура(ГруппаРейлингов, Строка.НаправляющиеНоменклатура.ДлинаДетали), ?(Строка.ВысотаЯщика < 201, КоличествоЯщиков, КоличествоЯщиков * 2), Строка.НомерИзделия);
		...
		Если Строка.ВидФасада <> "Нет" Тогда
			...
			Если Строка.ФасадНоменклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ЛДСП16 ИЛИ
				Строка.ФасадНоменклатура.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.МДФ Тогда
...
Процедура ДобавитьСтроку(ТекущийОбъект, Номенклатура, Площадь, НомерИзделия)
	...
	НоваяСтрока.ЧерезСклад 			= ?(МестоОбработки = Перечисления.МестоОбработки.Склад 
		ИЛИ МестоОбработки = Перечисления.МестоОбработки.ПоКаталогуВЦех и НомерИзделия <> НЕОПРЕДЕЛЕНО, Истина, Ложь);
...
Процедура ЗаполнитьСтоимость(ТекущийОбъект)
	...
	|	ТаблицаСпискаНоменклатуры.ЕдиницаИзмерения,
	|	ЕСТЬNULL(РозничныеЦены.Внутренняя, 0) * ТаблицаСпискаНоменклатуры.Количество КАК ВнутренняяСтоимость,
	|	ВЫБОР
	...
	ТекущийОбъект.СписокНоменклатуры.Загрузить(Выборка);
	
	/////////////////////////////////////////////СРОЧНОСТЬ//////////////////////////////////////////////////////
	//Если ТекущийОбъект.Срочный И ЗначениеЗаполнено(ТекущийОбъект.ДатаИзготовления) И Объект.Изделие = Справочники.Изделия.Детали Тогда
	//	СтруктураПоиска = Новый Структура;
	//	СтруктураПоиска.Вставить("Номенклатура", Справочники.Номенклатура.СрочностьЗаказа);
	//	СтрокаСрочности = Объект.СписокНоменклатуры.НайтиСтроки(СтруктураПоиска);
	//	Если ЗначениеЗаполнено(СтрокаСрочности) Тогда
	//		ТекущаяСтрокаСрочности = СтрокаСрочности[0];
	//		НоваяСтрока = ТекущийОбъект.СписокНоменклатуры.Добавить();
	//		НоваяСтрока.Номенклатура = ТекущаяСтрокаСрочности.Номенклатура;
	//		НоваяСтрока.ЕдиницаИзмерения = ТекущаяСтрокаСрочности.ЕдиницаИзмерения;
	//		НоваяСтрока.Количество = ТекущаяСтрокаСрочности.Количество;
	//		НоваяСтрока.НомерИзделия = ТекущаяСтрокаСрочности.НомерИзделия;
	//		НоваяСтрока.ЗарплатаЦеха = ТекущаяСтрокаСрочности.ЗарплатаЦеха;
	//		НоваяСтрока.РозничнаяСтоимость = ТекущаяСтрокаСрочности.РозничнаяСтоимость;
	//	КонецЕсли;
	//КонецЕсли;
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	//ТекущийОбъект.ВнутренняяСтоимостьСпецификации 	= Выборка.Итог("ВнутренняяСтоимость");
	//ТекущийОбъект.СуммаНарядаСпецификации 				= ТекущийОбъект.СписокНоменклатуры.Итог("ЗарплатаЦеха");
	//ТекущийОбъект.СуммаДокумента 								= ТекущийОбъект.СписокНоменклатуры.Итог("РозничнаяСтоимость");
	// перенес в модуль объекта
	// } Васильев Александр Леонидович [07.10.2013]
...
Функция СравнитьНоменклатурнуюГруппу(НоменклатурнаяГруппа)
	...
	Если  НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Кромка045_19 ИЛИ 
		НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Кромка2_19 ИЛИ 
		НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Кромка2_35 или
		НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.КромкаМДФ Тогда
		
		Кромка = "Обычная";
		
	ИначеЕсли НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.КромкаСтолешница Тогда
...
Процедура ОбработкаИзделийПоКаталогу()
	...
	Для Каждого Элемент Из Объект.СписокИзделийПоКаталогу Цикл
		...
		Для Каждого Строка Из СписокМатериалыИзделия Цикл
			...
			Если ЗначениеЗаполнено(КромкаНоменклатуры) Тогда
				...
				ИначеЕсли КромкаНоменклатуры.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.АГТПрофиль
					ИЛИ КромкаНоменклатуры.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.КромкаМДФ Тогда	
		...
		Для Каждого Строка Из СписокЯщикиИзделия Цикл
			...
			СтрокаПоиска = ПараметрыЯщиков.Найти(Строка.ИмяЯщика);
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ПользовательИнженер = УправлениеДоступомПереопределяемый.ЕстьДоступКПрофилюГруппДоступа(Пользователь, Справочники.ПрофилиГруппДоступа.Инженер);
...
Процедура АдресМонтажаНажатие(Элемент, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ТипЗнч(СтруктураАдреса) = Тип("Структура") Тогда
			...
			Километраж = СтруктураАдреса.Километраж;
...
Процедура ДатаМонтажаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ПараметрыФормы.Вставить("Подразделение", ПолучитьОсновноеПроизводство(Объект.Подразделение));
...
Процедура СписокДверейВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	Двери = Элемент.ТекущиеДанные.Двери;
	ОткрытьЗначение(Двери);
