...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СписокНоменклатурыКомплектации", ТекущийОбъект.СкладГотовойПродукции);
	Запрос.Текст =
	"ВЫБРАТЬ
	...
	//|		ИЛИ СписокНоменклатурыКомплектации.Номенклатура.МестоОбработки = Значение(Перечисление.МестоОбработки.ПоКаталогуВЦех) и СписокНоменклатурыКомплектации.НомерИзделия <> НЕОПРЕДЕЛЕНО
	//|
	//|СГРУППИРОВАТЬ ПО
	//|	ВЫБОР 
	//|		КОГДА ЕдиницаИзмерения.Дробный ТОГДА СписокНоменклатурыКомплектации.НомерСтроки
