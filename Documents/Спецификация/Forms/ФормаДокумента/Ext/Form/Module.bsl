...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы="Документ.Спецификация.Форма.ФормаВыбораМонтажника" Тогда
	...
	Если ИсточникВыбора.ИмяФормы="Документ.Спецификация.Форма.ФормаПодборКомплектации" Тогда
		ДобавитьВКомплектациюИзПодбора(ВыбранноеЗначение);
	КонецЕсли;
...
Процедура Открыть3дРедактор(Команда)
	...
	ОткрытьФорму("Документ.Спецификация.Форма.ФормаФлэш", Параметры3д)
КонецПроцедуры
...
Процедура МонтажникНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("НачалоПериода", Объект.Дата);
	ПараметрыФормы.Вставить("КонецПериода", КонецПериода);
	ПараметрыФормы.Вставить("Офис", Объект.Офис);
...
Процедура Подбор(Команда)
	...
	ОткрытьФорму("Документ.Спецификация.Форма.ФормаПодборКомплектации",Пар,ЭтаФорма);
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВКомплектациюИзПодбора(Выбранные)

	Объект.Комплектация.Очистить();
	
	Для каждого ВыбЭлемент Из Выбранные Цикл
		Если ВыбЭлемент.Количество > 0 Тогда
			КомЭлемент=Объект.Комплектация.Добавить();
			КомЭлемент.Номенклатура=ВыбЭлемент.Номенклатура;
			КомЭлемент.Количество=ВыбЭлемент.Количество;
			Если ВыбЭлемент.НомерИзделия<>0 Тогда
				КомЭлемент.НомерИзделия=ВыбЭлемент.НомерИзделия;
			КонецЕсли;
			КомЭлемент.ЕдиницаИзмерения=ВыбЭлемент.ЕдиницаИзмерения;
		КонецЕсли;
	КонецЦикла;
