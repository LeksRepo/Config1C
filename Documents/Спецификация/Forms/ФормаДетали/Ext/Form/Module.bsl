...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка Из Детали Цикл
		...
		КоличествоНеЗаполненныхКромок = ПроверитьЗаполненностьКромки(Строка.ПереключательКромкаСверху, Строка.ВыборМебельнойКромкиСверху, Идентификатор, КоличествоНеЗаполненныхКромок);
		КоличествоНеЗаполненныхКромок = ПроверитьЗаполненностьКромки(Строка.ПереключательКромкаСнизу, Строка.ВыборМебельнойКромкиСнизу, Идентификатор, КоличествоНеЗаполненныхКромок);
		КоличествоНеЗаполненныхКромок = ПроверитьЗаполненностьКромки(Строка.ПереключательКромкаСлева, Строка.ВыборМебельнойКромкиСлева, Идентификатор, КоличествоНеЗаполненныхКромок);
		КоличествоНеЗаполненныхКромок = ПроверитьЗаполненностьКромки(Строка.ПереключательКромкаСправа, Строка.ВыборМебельнойКромкиСправа, Идентификатор, КоличествоНеЗаполненныхКромок);
		
		Результат = ?(КоличествоНеЗаполненныхКромок > 0, Ложь, Результат);
...
&НаКлиенте
Функция ПроверитьЗаполненностьКромки(Переключатель, Кромка, Идентификатор, КоличествоНеЗаполненныхКромок)
	
	Если НЕ Переключатель = "Нет" и НЕ Переключатель = "" и НЕ ЗначениеЗаполнено(Кромка) Тогда
		
		Элементы.Детали.ТекущаяСтрока = Идентификатор;
		Текст = "Выберете кромку либо нажминет ""нет"" на переключателе";
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст, , "Элементы.Детали.ТекущиеДанные.ВыборМебельнойКромкиСверху");
		КоличествоНеЗаполненныхКромок = КоличествоНеЗаполненныхКромок + 1;
		
	КонецЕсли;
	
	Возврат КоличествоНеЗаполненныхКромок;
