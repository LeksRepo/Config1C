	...
	Для каждого Строка ИЗ Детали Цикл
		...
		Если Фасад Тогда
			...
			Если Материал = "ФасадМДФ" ИЛИ Материал = "ФасадЛДСП" Тогда
				...
				РезультатПоКромке = ПроверитьКромку(СтруктураКромок, Строка.УниверсальнаяКромка, КоличествоТорцевыхРучек);
...
&НаСервере
Функция ПроверитьКромку(СтруктураКромок, КромкаДляСравнения, КоличествоТорцевыхРучек)
	...
	Для каждого Строка Из СтруктураКромок Цикл
		...
		Если ТипЗнч(Кромка) = Тип("СправочникСсылка.Номенклатура") Тогда
			...
			Если НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ТорцеваяРучка16мм или НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ТорцеваяРучка18мм Тогда
				...
				Если (Строка.Ключ = "Сверху" или Строка.Ключ = "Сверху") и Кромка.ДлинаДетали < СтруктураКромок.ШиринаДетали Тогда
