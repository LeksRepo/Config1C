...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка ИЗ Детали Цикл
		...
		Если Фасад И (НЕ ЗначениеЗаполнено(Строка.НоменклатураДляСклеивания) И ЗначениеЗаполнено(Строка.РасположениеПазовИРучкиНаФасадах)) Тогда 
		...
		Если Фасад И (ЗначениеЗаполнено(Строка.НоменклатураДляСклеивания) И НЕ ЗначениеЗаполнено(Строка.РасположениеПазовИРучкиНаФасадах) 
			или ((ЗначениеЗаполнено(Строка.Петли) ИЛИ Строка.КоличествоПетель > 0)) И НЕ ЗначениеЗаполнено(Строка.РасположениеПазовИРучкиНаФасадах)) Тогда
			
			Если НЕ Материал = "АГТ+АГТ" Тогда
				Элементы.Детали.ТекущаяСтрока = Идентификатор;
				Текст = "Не выбрано расположение пазов и ручки";
...
Функция ОбновитьДоступность()
	...
	ИначеЕсли ОбычныйФасад Тогда
		...
		Если Материал = "АГТ+АГТ" ИЛИ Материал = "АГТ+10 ЛДСП" Тогда
