	...
	Для каждого Строка ИЗ Детали Цикл
		...
		Если Фасад Тогда
			...
			Если Материал = "ФасадМДФ" ИЛИ Материал = "ФасадЛДСП" Тогда
				...
				МассивКромок.Добавить(Строка.ВыборМебельнойКромкиСверху);
				МассивКромок.Добавить(Строка.ВыборМебельнойКромкиСнизу);
				МассивКромок.Добавить(Строка.ВыборМебельнойКромкиСлева);
				МассивКромок.Добавить(Строка.ВыборМебельнойКромкиСправа);
				РезультатПоКромке = ПроверитьКромку(МассивКромок, Строка.УниверсальнаяКромка, КоличествоТорцевыхРучек);
				
				Если РезультатПоКромке.КоличествоТорцевыхРучек > 1 или РезультатПоКромке.НесовпадениеКромки Тогда
					
					Текст = ?(РезультатПоКромке.НесовпадениеКромки, "Обрамление фасада должно совпадать с выбором ""Ок""", "Нельзя использовать больше одной торцевой ручки");
					Элементы.Детали.ТекущаяСтрока = Идентификатор;
...
&НаСервере
Функция ПроверитьКромку(МассивКромок, КромкаДляСравнения, КоличествоТорцевыхРучек)
	...
	Для каждого Кромка Из МассивКромок Цикл
		...
		ИначеЕсли Кромка <> КромкаДляСравнения И Кромка <> Справочники.Номенклатура.ПустаяСсылка() Тогда
