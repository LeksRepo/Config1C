...
Функция ПроверитьНаМинимальныйРазмер(СтарыйРазмер, НовыйРазмер)
	...
	Если МеньшеДопустимого Тогда
		...
		Ответ = Вопрос("Деталь со стороной менее 80мм не кромится. Будут очищены поля с кромкой. Продолжить?",Режим);
		...
		Иначе
			...
			//СтруктураПодсчета.Вставить("НовыйРазмер", Ложь);
			//СтруктураПодсчета.Вставить("Размер", СтарыйРазмер);
			Размер = СтарыйРазмер;
