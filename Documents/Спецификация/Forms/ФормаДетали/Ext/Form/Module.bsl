...
&НаКлиенте
Функция СформироватьСтрокуРедактор3Д()
...
Функция ОбновитьФлэш()
	...
	Если Элементы.Детали.ТекущиеДанные <> Неопределено Тогда
		...
		ПередатьВоФлэш(ЭлементФлэш, СформироватьСтрокуФлэш(), Ложь);
		
		ЭлементФлэш3D = Элементы.Флэш3D.Документ.getElementById("input");
		ПередатьВоФлэш(ЭлементФлэш3D, СформироватьСтрокуРедактор3Д(), Истина);
