...
Функция СформироватьСтрокуРедактор3Д()
	...
	РеквизитыНоменклатуры = ЛексСервер.ЗначенияРеквизитовОбъекта(ТекущиеДанные.Номенклатура, "ГлубинаДетали, Цвет");
	ЗаполнитьЗначенияСвойств(Структура, РеквизитыНоменклатуры);
	
	Структура.Команда = "add";
	
	Структура.Цвет ="FFFFFF";
	
	Структура.ЦветКромкиСверху = 0;
	Структура.ЦветКромкиСнизу = 0;
	Структура.ЦветКромкиСлева = 0;
	Структура.ЦветКромкиСправа = 0;
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	Если Найти(Номенклатура, "ЛДСП") <> 0 Тогда
...
Процедура Флэш3DПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Если Команда = "init" Тогда
		ПередатьВоФлэш(ЭлементФлэш3Dinput, "init☺new", Истина);
	КонецЕсли;
