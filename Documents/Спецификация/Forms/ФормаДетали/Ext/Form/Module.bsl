...
&НаКлиенте
Перем МассивКраткихНаименований;

&НаКлиенте
Перем МассивРазмеровНоменклатуры;
...
Функция ПроверитьОбточку()
	...
	Если ДанныеДетали.ЭтоСтолешница Тогда
		...
		Если (ТекущиеДанные.РадиусПравоНиз > 0 ИЛИ ТекущиеДанные.РадиусПравоВерх > 0) И ЕстьКромка(ТекущиеДанные) И ТекущиеДанные.Обтачивать Тогда
			
			СтруктураКромок = ПолучитьДанныеКромок(СтруктураКромок);
			
			Если СтруктураКромок.Верх.ГлубинаДетали > 0 ИЛИ СтруктураКромок.Низ.ГлубинаДетали > 0 Тогда
...
&НаКлиенте
Функция ПроверитьРадиусы(РадиусЛевоВерх, РадиусПравоВерх, РадиусПравоНиз, РадиусЛевоНиз, Идентификатор, КантСлева, КантСправа, КантСверху, КантСнизу, Срез, Ошибки, ПолеОшибки, НомерДеталиСтрока)
		...
		Если ДанныеДеталиСтроки.ЭтоФасад Тогда
			...
			Если ДанныеДеталиСтроки.СпецФасад Тогда
				...
				Если РезультатПоКромке.КоличествоТорцевыхРучек > 1 или РезультатПоКромке.НесовпадениеКромки Тогда
					...
					Текст = ?(РезультатПоКромке.НесовпадениеКромки, НомерДеталиСтрока + "Обрамление фасада различается.", НомерДеталиСтрока + "Нельзя использовать больше одной торцевой ручки.");
					ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, Текст);
					Результат = Ложь;
