...
&НаКлиенте
Функция ПроверитьРадиусы(РадиусЛевоВерх, РадиусПравоВерх, РадиусПравоНиз, РадиусЛевоНиз, Идентификатор, КантСлева, КантСправа, КантСверху, КантСнизу, Срез)
	
	// { Васильев Александр Леонидович [15.06.2014]
	// Тут не очень хорошо получается.
	// До первой ошибки доходит и возвращается.
	// Переделать на "СообщитьОшибкиПользователю" и не возвращаться после обнаружения ошибки.
	// } Васильев Александр Леонидович [15.06.2014]
...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка Из Детали Цикл
		...
		Если НЕ Материал = "Столешница" Тогда
			...
			РезультатПроверкиРадиусов = ПроверитьРадиусы(РадиусЛевоВерх,РадиусПравоВерх,РадиусПравоНиз,РадиусЛевоНиз, Идентификатор, КантСлева, КантСправа, КантСверху, КантСнизу, Срез);
...
Функция ПроверкаКоличестваИТолщины()
	...
	// { Васильев Александр Леонидович [16.06.2014]
	// Это зачем здесь? Не понимаю логики.
	// } Васильев Александр Леонидович [16.06.2014]
...
Процедура НоменклатураПриИзменении(Элемент)
	...
	Если Материал = "16 ЛДСП" Тогда
		...
		ТекущиеДанные.Кант = ?(СтруктураПодставляяемойНоменклатуры.Свойство("Кант"), СтруктураПодставляяемойНоменклатуры.Кант, Неопределено);
		ТекущиеДанные.Кромка2мм = ?(СтруктураПодставляяемойНоменклатуры.Свойство("Кромка2_19"), СтруктураПодставляяемойНоменклатуры.Кромка2_19, Неопределено);
		ТекущиеДанные.Кромка045мм = ?(СтруктураПодставляяемойНоменклатуры.Свойство("Кромка045_19"), СтруктураПодставляяемойНоменклатуры.Кромка045_19, Неопределено);
...
&НаКлиенте
Функция СформироватьСтрокиРедактор3Д()
	...
	Если ТекущиеДанные.ВысотаДетали = 0
		...
		//ИЛИ НЕ ЗначениеЗаполнено(ТекущиеДанные.ИД)
		Тогда
		Возврат Строки;
	КонецЕсли;
	
	ФорматнаяСтрока = "Команда☺ИД☺ВысотаДетали☺ШиринаДетали☺ГлубинаДетали☺Цвет☺
	|ЦветКромкиСверху☺ЦветКромкиСнизу☺ЦветКромкиСлева☺ЦветКромкиСправа☺
	|РадиусЛевоВерх☺РадиусПравоВерх☺РадиусПравоНиз☺РадиусЛевоНиз";
	
	МассивПолей = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ФорматнаяСтрока, "☺");
	...
	Деталь.Вставить("Номенклатура");
	Деталь.Вставить("ВыборМебельнойКромкиСверху");
	Деталь.Вставить("ВыборМебельнойКромкиСнизу");
	Деталь.Вставить("ВыборМебельнойКромкиСлева");
	Деталь.Вставить("ВыборМебельнойКромкиСправа");
	
	ЗаполнитьЗначенияСвойств(Деталь, ТекущиеДанные);
	ЦветаДетали = ПолучитьЦвета(Деталь);
	ЗаполнитьЗначенияСвойств(Структура, ТекущиеДанные);
	
	// хранить бы где-нибудь цвет И глубину материала
	Если ТекущиеДанные.Материал = "10 ЛДСП+10 ЛДСП" Тогда
		Глубина = 20;
	ИначеЕсли ТекущиеДанные.Материал = "16 ЛДСП+10 ЛДСП" Тогда
		Глубина = 26;
	ИначеЕсли ТекущиеДанные.Материал = "16 ЛДСП+16 ЛДСП" Тогда
		Глубина = 32;
	Иначе
		Глубина = ЛексСервер.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Номенклатура, "ГлубинаДетали");
	КонецЕсли;
	
	Структура.ГлубинаДетали = Глубина;
	
	ЗаполнитьЗначенияСвойств(Структура, ЦветаДетали);
	Структура.Команда = "add";
	
	Для ы = 1 по ТекущиеДанные.Количество Цикл
		
		Структура.ИД = "" + ТекущиеДанные.ИД + "_" + ы;
		
		Для каждого Поле Из Структура Цикл
			
			Если ТипЗнч(Поле.Значение) <> Тип("Число") Тогда
				Стр = Строка(Поле.Значение);
			Иначе 
				Стр = Формат(Поле.Значение, "ЧРГ=");
			КонецЕсли;
			
			СтрокаРезультат = СтрокаРезультат + Строка(Поле.Значение) + "☺";
			
		КонецЦикла;
		
		СтрокаРезультат = СтрЗаменить(СтрокаРезультат, Символы.НПП, "");
...
Функция ОбновитьФлэш()
	...
	// { Васильев Александр Леонидович [16.06.2014]
	// Нужно разделить на две функции. Флэш и Флэш3D
	// } Васильев Александр Леонидович [16.06.2014]
	...
	Если Элементы.Детали.ТекущиеДанные <> Неопределено Тогда
		...
		//ЭлементФлэш3D = Элементы.Флэш3D.Документ.getElementById("input");
		//СтрокиДляФлэш = СформироватьСтрокиРедактор3Д();
		//
		//Для каждого Строка Из СтрокиДляФлэш Цикл
...
Процедура ДеталиПриИзменении(Элемент)
	...
	// { Васильев Александр Леонидович [15.06.2014]
	// Разве в таблице у нас что-то меняется?
	// Когда, ты считаешь, это должно срабатывать?
	// } Васильев Александр Леонидович [15.06.2014]
...
Процедура ДеталиПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Копирование ИЛИ НЕ ЗначениеЗаполнено(ТекущиеДанные.ИД) Тогда
		ТекущиеДанные.ИД = ТекущиеДанные.ПолучитьИдентификатор();
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Функция НайтиСтрокуПоИД(ИД)
	
	ПараметрыОтбора = Новый Структура;
	ПараметрыОтбора.Вставить("ИД", ПолучитьИДБезСуффикса(ИД));
	
	НайденныеСтроки = Детали.НайтиСтроки(ПараметрыОтбора);
	
	Если НайденныеСтроки.Количество() <> 1 Тогда
		ВызватьИсключение "Ошибка 379: рассинхронизация данных";
	КонецЕсли;
...
&НаКлиенте
Функция ВыделитьСтрокуПоИДДетали(ИД)
	
	Строки = НайтиСтрокуПоИД(ИД);
	Элементы.Детали.ТекущаяСтрока = Строки[0].ПолучитьИдентификатор();
	
КонецФункции

&НаКлиенте
Функция УдалитьСтрокуПоИДДетали(ИД)
	
	Строки = НайтиСтрокуПоИД(ИД);
	Строка = Строки[0];
	
	Если Строка.Количество = 1 Тогда
		Детали.Удалить(Детали.Индекс(Строка));
	Иначе
