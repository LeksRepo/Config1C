...
Процедура ЗаполнитьОтверстия(Команда)
	...
	Если ЗапретРедактирования И Данные.НомерИзделия <> 0 Тогда
		Возврат;	
	КонецЕсли;
...
Процедура ОбновитьДоступность()
	...
	Элементы.ЗаполнитьОтверстия.Доступность = ?(ЗначениеЗаполнено(Спецификация),?(Дилерский, Ложь, Дос.ДоступностьПоляЗаполнитьОтверстия), Ложь);
...
Процедура ДеталиРасположениеПазовИРучкиНаФасадахОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ТекущиеДанные.РасположениеПазовИРучкиНаФасадах) Тогда
		...
		ТекущиеДанные.СтруктураОтверстий = ЗаполнитьПетлиФасадов(ТекущиеДанные.КоличествоПетель, ТекущиеДанные.ВысотаДетали, ТекущиеДанные.ШиринаДетали, ТекущиеДанные.РасположениеПазовИРучкиНаФасадах);
...
&НаСервере
Функция ЗаполнитьПетлиФасадов(КоличПетель, ШиринаДетали, ВысотаДетали, РасположениеПазовИРучкиНаФасадах)
	
	ТаблицаОтверстийФасады.Очистить();
	
	Если КоличПетель > 0 И РасположениеПазовИРучкиНаФасадах.ПоложениеПазов <> "Отсутствует" Тогда
		...
		Строка.РасположениеОтверстия = РасположениеПазовИРучкиНаФасадах.ПоложениеПазов;
		
	 КонецЕсли;
	 
	 СтруктураПетель = РеквизитФормыВЗначение("ТаблицаОтверстийФасады");
	 
	 Если СтруктураПетель.Количество() > 0 Тогда
