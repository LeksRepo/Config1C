...
Функция ПроверитьПередСохранением()
	...
	Для каждого Строка Из Детали Цикл
		...
		Если (Материал = "ФасадАГТ" или Материал = "ФасадМДФ" или Материал = "ФасадАлюминиевый" или Материал = "ФасадСтеклянныйЗакругленный" или Материал = "ФасадСтеклянный") 
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ТекущиеДанные.ВысотаДетали = 0 Тогда
			...
			ТекущиеДанные.СоблюдениеТекстуры 	= Истина;
			ВысотаСКромкой 								= 0;
			ШиринаСКромкой 								= 0;
...
Процедура ПроверитьРазмерыСтолешницы()
	...
	Столешница 		= Элементы.Детали.ТекущиеДанные.Материал = "Столешница";
	НеОбтачивать 	= НЕ Элементы.Детали.ТекущиеДанные.Обтачивать;
	НужнаяШирина 	= Элементы.Детали.ТекущиеДанные.ШиринаДетали > 585;
	КромкаВыбрана 	= НЕ Элементы.Детали.ТекущиеДанные.ПереключательКромкаСправа = "Нет";
	
	Если Столешница и НеОбтачивать и НужнаяШирина или Столешница и КромкаВыбрана и НужнаяШирина Тогда
		
		Элементы.Детали.ТекущиеДанные.ШиринаДетали = 585;
		ОбновитьФлэш();
		ПосчитатьЧистыйРазмер();
