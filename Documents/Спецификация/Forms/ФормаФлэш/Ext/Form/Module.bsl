...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//	СтрокаДляФлэш = Параметры.СтрокаДвериФлэш;
	КонецЕсли;
...
Процедура ХТМЛПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	//	Если ПолученнаяСтрока = "init" Тогда
	//		ЭлементДляВвода = Элементы.ХТМЛ.Документ.getElementById("back");
	//		ЭлементДляВвода.tag = "ppic☻"+СтрокаДляФлэш;
	//		ЭлементДляВвода.click();			
	//	ИначеЕсли Найти(ПолученнаяСтрока, "ppic") > 0 Тогда
	//		Бэйс64 = Прав(ПолученнаяСтрока, СтрДлина(ПолученнаяСтрока)-5);
	//		РаскройГотов = Истина;
	//		Закрыть(Бэйс64);
	//		Возврат;			
	//	КонецЕсли;
