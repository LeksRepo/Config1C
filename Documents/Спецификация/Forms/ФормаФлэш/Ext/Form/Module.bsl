...
Процедура ХТМЛПриНажатии(Элемент, ДанныеСобытия, СтандартнаяОбработка)
	...
	Иначе
		...
		Элементы.ХТМЛ.Документ.getElementById("output").tag = "";
	...
	Если ОпределительФлеш = "ЧертежДвери" Тогда
		...
		ИначеЕсли Команда = "prtx" Тогда
			...
			НомерДвери = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(СтруктураМассивовДверей.ТекущаяДверь);
			СтрокаТаблицаНоменклатуры = ПолучитьТаблицуНоменклатурыНаПечать(СтруктураМассивовДверей.ТекущаяДверь);
			
			ЭлементДляВвода.tag = "prtx☻" + Примечание + "☻" + СтруктураМассивовДверей.Договор + Символы.ПС + СтруктураМассивовДверей.Спецификация + Символы.ПС + "Дверь № "+НомерДвери + "☻" + СтрокаТаблицаНоменклатуры;
			ЭлементДляВвода.click();
...
Функция ПолучитьТаблицуНоменклатурыНаПечать(Ссылка)
	
	Возврат Справочники.Двери.ПолучитьТаблицуНоменклатурыНаПечать(Ссылка);
