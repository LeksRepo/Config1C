...
Процедура УстановитьДоступность(ВидИзделия)
	...
	СтрокаДетали = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидИзделия))[0];
	
	ЭтоНеПредопределенный = НЕ СтрокаДетали.Предопределенный;
	ЭтоБоковаяПанель = СтрокаДетали.ВидИзделияПоКаталогу = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.БоковаяПанельСлева") ИЛИ
	СтрокаДетали.ВидИзделияПоКаталогу = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.БоковаяПанельСправа");
	
	Элементы.ДеталиНоменклатураЛДСП.Доступность = ЭтоНеПредопределенный;
	Элементы.ДеталиКромкаЛДСП.Доступность = ЭтоНеПредопределенный;
	Элементы.РучкиЯщиков.Доступность = ЭтоНеПредопределенный И СтрокаДетали.ЕстьЯщики;
	Элементы.ДеталиНоменклатураЛДСПДоп.Доступность = ЭтоНеПредопределенный И СтрокаДетали.ЕстьЛДСПДоп;
	Элементы.ДеталиКромкаЛДСПДоп.Доступность = ЭтоНеПредопределенный И СтрокаДетали.ЕстьЛДСПДоп;
	Элементы.ДеталиНоменклатураДВП.Доступность = ЭтоНеПредопределенный И СтрокаДетали.ЕстьДВП;
	Элементы.ДеталиЗавал.ТолькоПросмотр = ЭтоБоковаяПанель;
	Элементы.ДеталиВидСтены.ТолькоПросмотр = СтрокаДетали.ВидИзделия = ПредопределенноеЗначение("Справочник.ВидыИзделийПоКаталогу.Пол") ИЛИ ЭтоБоковаяПанель;
