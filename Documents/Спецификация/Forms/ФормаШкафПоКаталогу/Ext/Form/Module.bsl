...
Процедура ВыборИзделия(Команда)
	...
	Если Дилерский Тогда
		Изделие = ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора", Новый Структура("Отбор, ВидИзделияПоКаталогу, МассивИзделий, Изделие", Новый Структура("АктивныйДляДилера, Корпусный", Истина, Корпусный), ТекущийЭлементИзделия, МассивИзделий, ИзделиеСпец), ЭтаФорма);
	Иначе
		Изделие = ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора", Новый Структура("Отбор, ВидИзделияПоКаталогу, МассивИзделий, Изделие", Новый Структура("Активный, Корпусный", Истина, Корпусный), ТекущийЭлементИзделия, МассивИзделий, ИзделиеСпец), ЭтаФорма);	
	КонецЕсли;
