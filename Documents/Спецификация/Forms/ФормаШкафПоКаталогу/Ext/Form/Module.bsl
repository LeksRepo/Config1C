...
Функция ПроверитьИУстановитьРазмерыПередСохранением(РазмерыОсновного)
	...
	Для Каждого Строка Из Детали Цикл
		Если НЕ Строка.Предопределенный Тогда
...
Процедура УстановитьДоступность(ВидИзделия)
	...
	Если (СтрокаДетали.Количество() = 1) И (ВидИзделия <> ПредопределенноеЗначение("Перечисление.ВидыИзделийПоКаталогу.ЗадняяСтенка")) Тогда
		Флаг = ?(СтрокаДетали[0].Предопределенный, Ложь, Истина);
...
Процедура ОтобразитьКартинку(АдресЭлемента, ЭлементИзделия)
	...
	Иначе
		Предупреждение("Критическая ошибка каталога (нет изображения изделия)!!!");
	КонецЕсли;

КонецПроцедуры	

&НаКлиенте
Процедура ЗаполнитьСтроку(НашаСтрока, Изделие = Неопределено, ВидИзделия, СтруктураИзделия = Неопределено, ЭлементИзделия = Неопределено)
	...
	Иначе
		...
		Если Изделие <> Неопределено Тогда
			...
			НашаСтрока.ВысотаИзделия = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ВысотаИзделия, 0);
			НашаСтрока.ШиринаИзделия = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ШиринаИзделия, 0);
			НашаСтрока.ГлубинаИзделия = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ГлубинаИзделия, 0);
			НашаСтрока.ВысотаИзделияМин = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ВысотаИзделияМин, 0);
			НашаСтрока.ШиринаИзделияМин = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ШиринаИзделияМин, 0);
			НашаСтрока.ГлубинаИзделияМин = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ГлубинаИзделияМин, 0);
			НашаСтрока.ВысотаИзделияМакс = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ВысотаИзделияМакс, 0);
			НашаСтрока.ШиринаИзделияМакс = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ШиринаИзделияМакс, 0);
			НашаСтрока.ГлубинаИзделияМакс = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ГлубинаИзделияМакс, 0);
			НашаСтрока.ЕстьЯщики = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.ЕстьЯщики, Ложь);
			НашаСтрока.НаВсеИзделие = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.НаВсеИзделие, Ложь);
			НашаСтрока.НеВлияетНаОсновной = ?(СтруктураИзделия <> Неопределено, СтруктураИзделия.НеВлияетНаОсновной, Ложь);
...
Процедура ВыборИзделия(Команда)
	...
	Если ЗначениеЗаполнено(Изделие) Тогда	
		...
		ЗаполнитьСтроку(НоваяСтрока, Изделие, ТекущийЭлементИзделия, СтруктураИзделия, ЭлементИзделия);		
	КонецЕсли;	
КонецПроцедуры
