...
Функция ПолучитьСтруктуруИзделия(Изделие, ВидИзделия)
	...
	СпрДВП = Справочники.ВидыДеталей.НайтиПоНаименованию("ДВП");
	СпрЛДСП = Справочники.ВидыДеталей.НайтиПоНаименованию("ФасадЛДСП");
	
	ЕстьДВП = ЗначениеЗаполнено(Изделие.СписокМатериалы.Найти(СпрДВП , "ВидДетали"));
	
	МассивФасадов = Изделие.СписокМатериалы.НайтиСтроки(Новый Структура("ВидДетали", СпрЛДСП));
...
Процедура УстановитьДоступность(ВидИзделия)
	...
	Если СтрокаДетали.Количество() = 1 Тогда
		...
		ЭтоБоковаяПанель = НашаДеталь.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСлева ИЛИ
		НашаДеталь.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСправа;
...
Процедура ЗаполнитьСтроку(НашаСтрока, ВидИзделия, Изделие = Неопределено, СтруктураИзделия = Неопределено, ЭлементИзделия = Неопределено)
	...
	Если Изделие <> Неопределено Тогда
		...
		Если ВидИзделия = ВидыИзделий.ОсновнойЭлемент Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Корпусный = ИзделиеСпец = Справочники.Изделия.КорпусныйШкафКупеПоКаталогу;
...
Процедура ЗаполнитьВидыИзделий()
	...
	ВидыИзделий.Вставить("ОсновнойЭлемент", Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент);
...
Процедура ВыборИзделия(Команда)
	...
	Если Дилерский Тогда
		ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора", Новый Структура("Отбор, ВидИзделияПоКаталогу, МассивИзделий, Изделие", Новый Структура("АктивныйДляДилера, Корпусный", Истина, Корпусный), ТекущийЭлементИзделия, МассивИзделий, ИзделиеСпец), ЭтаФорма);
	Иначе
		ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора", Новый Структура("Отбор, ВидИзделияПоКаталогу, МассивИзделий, Изделие", Новый Структура("Активный, Корпусный", Истина, Корпусный), ТекущийЭлементИзделия, МассивИзделий, ИзделиеСпец), ЭтаФорма);	
	КонецЕсли;
