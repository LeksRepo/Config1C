...
Процедура УстановитьДоступность(ВидИзделия)
	...
	ЭтоНеПредопределенный = (СтрокаДетали.Количество() = 1) И (НЕ СтрокаДетали[0].Предопределенный);
	...
	Элементы.РучкиЯщиков.Доступность = ЭтоНеПредопределенный И СтрокаДетали[0].ЕстьЯщики;
	Элементы.ДеталиНоменклатураЛДСПДоп.Доступность = ЭтоНеПредопределенный И СтрокаДетали[0].ЕстьЛДСПДоп;
	Элементы.ДеталиКромкаЛДСПДоп.Доступность = ЭтоНеПредопределенный И СтрокаДетали[0].ЕстьЛДСПДоп;
	Элементы.ДеталиНоменклатураДВП.Доступность = ЭтоНеПредопределенный И СтрокаДетали[0].ЕстьДВП;
