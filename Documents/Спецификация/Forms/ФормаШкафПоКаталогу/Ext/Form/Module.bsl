...
&НаКлиенте
Процедура ШиринаОтсека1ПриИзменении(Элемент)
	ЗаполнитьШиринуОтсека();
КонецПроцедуры

&НаКлиенте
Процедура ШиринаОтсека2ПриИзменении(Элемент)
	ЗаполнитьШиринуОтсека();
КонецПроцедуры

&НаКлиенте
Процедура ШиринаОтсека3ПриИзменении(Элемент)
	ЗаполнитьШиринуОтсека();
КонецПроцедуры

&НаКлиенте
Процедура ШиринаОтсека4ПриИзменении(Элемент)
	ЗаполнитьШиринуОтсека();
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьШиринуОтсека()
	
	РазмерыОсновного = ПолучитьРазмерыОсновногоЭлемента();
	
	Если ЗначениеЗаполнено(РазмерыОсновного) Тогда
	
		Если КоличествоОтсеков = 2 Тогда
			
			 ШиринаОтсека2 = РазмерыОсновного.Ширина - ШиринаОтсека1 - 16; 
			
		ИначеЕсли КоличествоОтсеков = 3 Тогда
			
			Если ПроверитьВыдвижнойВСреднем() Тогда
				
				ШиринаОтсека1 = ОКР((РазмерыОсновного.Ширина - ШиринаОтсека2 - 32)/2);
				ШиринаОтсека3 = ОКР((РазмерыОсновного.Ширина - ШиринаОтсека2 - 32)/2);
				
			Иначе
			
			 	ШиринаОтсека3 = РазмерыОсновного.Ширина - ШиринаОтсека1 - ШиринаОтсека2 - 32;
			КонецЕсли;
			
		ИначеЕсли КоличествоОтсеков = 4 Тогда
			
			 СреднийРазмер = ОКР(РазмерыОсновного.Ширина/2);
			 ШиринаОтсека2 = СреднийРазмер - ШиринаОтсека1 - 24;
			 ШиринаОтсека4 = СреднийРазмер - ШиринаОтсека3 - 24;
			
		КонецЕсли;
		 
	КонецЕсли;
		
	Строки = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.ОсновнойЭлемент1));
			
	Для Каждого Стр ИЗ Строки Цикл
		 Стр.ШиринаОтсека = ЭтаФорма["ШиринаОтсека"+Стр.НомерОтсека];
	КонецЦикла;
	
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьВысотуОтсека()
		
	Строки = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.АнтресольнаяПолка));
			
	Для Каждого Стр ИЗ Строки Цикл
		 Стр.ВысотаОтсека = ЭтаФорма.ВысотаАнтПолки;
	КонецЦикла;
...
Процедура ПолучитьИтоговыеРазмеры(ВысотаОсновного, ШиринаОсновного)
	...
	Для Каждого Строка Из Детали Цикл
		Если (Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.Потолок 
			ИЛИ Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.Пол
			ИЛИ Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.АнтресольнаяПолка) И НЕ Строка.НеВлияетНаОсновной Тогда
...
Функция ПроверитьИУстановитьРазмерыПередСохранением(РазмерыОсновного)
	...
	//RonEXI: Сначала проверяем размеры отсеков и расположение отсеков с выдвижными элементами.
	
	КоличествоОтсеков = ВариантШкафа.КоличествоОтсеков;
	ОтнятьОтВыдвижного = 100;
	Отсек1 = Неопределено;
	Отсек2 = Неопределено;
	Отсек3 = Неопределено;
	Отсек4 = Неопределено;
	ОснЭлементы = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.ОсновнойЭлемент1));
	МаксРазмерВыдвижного = 10000; 
	
	Для Каждого Стр ИЗ ОснЭлементы Цикл
		
		Если Стр.НомерОтсека = 1 Тогда Отсек1 = Стр КонецЕсли;
		Если Стр.НомерОтсека = 2 Тогда Отсек2 = Стр КонецЕсли;
		Если Стр.НомерОтсека = 3 Тогда Отсек3 = Стр КонецЕсли;
		Если Стр.НомерОтсека = 4 Тогда Отсек4 = Стр КонецЕсли;
		
	КонецЦикла;
	
	Если КоличествоОтсеков = 2 Тогда
		
		Если Отсек1.Изделие.ЕстьВыдвижнойЭлемент И Отсек2.Изделие.ЕстьВыдвижнойЭлемент Тогда
			 Текст = "Недопустимое расположение отсеков с выдвижными элементами";
			 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент2", Текст);
		КонецЕсли;
		 
		МаксРазмерВыдвижного = (РазмерыОсновного.Ширина / КоличествоОтсеков) - ОтнятьОтВыдвижного;

	ИначеЕсли КоличествоОтсеков = 3 Тогда 
		
		Если (Отсек1.Изделие.ЕстьВыдвижнойЭлемент И Отсек2.Изделие.ЕстьВыдвижнойЭлемент) 
		 ИЛИ (Отсек2.Изделие.ЕстьВыдвижнойЭлемент И Отсек3.Изделие.ЕстьВыдвижнойЭлемент) Тогда
			 Текст = "Недопустимое расположение отсеков с выдвижными элементами";
			 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент2", Текст);
		КонецЕсли;
	...
	ИначеЕсли КоличествоОтсеков = 4 Тогда
		...
		Если (Отсек1.Изделие.ЕстьВыдвижнойЭлемент И Отсек2.Изделие.ЕстьВыдвижнойЭлемент) 
		 ИЛИ (Отсек3.Изделие.ЕстьВыдвижнойЭлемент И Отсек4.Изделие.ЕстьВыдвижнойЭлемент)
		 ИЛИ (Отсек2.Изделие.ЕстьВыдвижнойЭлемент И Отсек3.Изделие.ЕстьВыдвижнойЭлемент) Тогда
			 Текст = "Недопустимое расположение отсеков с выдвижными элементами";
			 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент2", Текст);
		КонецЕсли;
	...
	СуммаШириныОтсеков = 0;
	
	Для Каждого Стр ИЗ Мас Цикл
		
		Если Стр <> Неопределено Тогда
			
			СуммаШириныОтсеков = СуммаШириныОтсеков + Стр.ШиринаОтсека; 
			
			Если Стр.Изделие.ЕстьВыдвижнойЭлемент И Стр.ШиринаОтсека > МаксРазмерВыдвижного Тогда
				 Текст = "Ширина отсека №" + Стр.НомерОтсека + " (" + Стр.ШиринаОтсека + ") превышает максимальную (" + МаксРазмерВыдвижного + ").";
				 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент1", Текст);
			КонецЕсли;
			 
			Если Стр.ШиринаОтсека < Стр.ШиринаИзделияМин ИЛИ Стр.ШиринаОтсека > Стр.ШиринаИзделияМакс Тогда
				 Текст = "Ширина отсека №" + Стр.НомерОтсека + " (" + Стр.ШиринаОтсека + ") должна быть в интервале между " + Стр.ШиринаИзделияМин + " и " + Стр.ШиринаИзделияМакс;  
				 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент1", Текст);
			КонецЕсли;
	...
	ОтнятьОтОсновногоДляПроверки = 0;
	
	Если КоличествоОтсеков = 2 Тогда
		 ОтнятьОтОсновногоДляПроверки = 16;
	ИначеЕсли КоличествоОтсеков = 3 Тогда
		 ОтнятьОтОсновногоДляПроверки = 32;
		 
		 Если ПроверитьВыдвижнойВСреднем() Тогда
			 
			 Если (Отсек2.ШиринаОтсека + 100) > Отсек1.ШиринаОтсека
				 ИЛИ (Отсек2.ШиринаОтсека + 100) > Отсек3.ШиринаОтсека Тогда
				     Текст = "Ширина среднего отсека с выдвижным элементов должна быть меньше боковых на 100";
					 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент2", Текст);
			 КонецЕсли;
	...
	ИначеЕсли КоличествоОтсеков = 4 Тогда
		 ОтнятьОтОсновногоДляПроверки = 48;
	КонецЕсли;
	
	Если СуммаШириныОтсеков <> (РазмерыОсновного.Ширина - ОтнятьОтОсновногоДляПроверки) Тогда
		Текст = "Сумма ширин всех отсеков должна равняться: " + (РазмерыОсновного.Ширина - ОтнятьОтОсновногоДляПроверки) + ". Текущее значение: " + СуммаШириныОтсеков;
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ОсновнойЭлемент1", Текст);
	КонецЕсли;
	...
	Для Каждого Строка Из Детали Цикл
		...
		Если ЗначениеЗаполнено(Строка.Завал) И Число(Строка.Завал) >= 10 
			...
			ИЛИ ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.ФальшПанельСправа
			ИЛИ ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.ПодложкаПодТрекСверху
			ИЛИ ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.ПодложкаПодТрекСнизу) Тогда
		...
		ИначеЕсли Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент Тогда
			...
			Если Строка.НомерОтсека = 1 Тогда
				СтруктураДиапазонов.ВысотаМинЦентр = СтруктураДиапазонов.ВысотаМинЦентр + Строка.ВысотаИзделияМин;
				СтруктураДиапазонов.ВысотаМаксЦентр = СтруктураДиапазонов.ВысотаМаксЦентр + Строка.ВысотаИзделияМакс;
			КонецЕсли;
			...
			Строка.ШиринаИзделия = Строка.ШиринаОтсека;
			Строка.ВысотаИзделия = РазмерыОсновного.Высота;
			
		ИначеЕсли Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.АнтресольнаяПолка Тогда
			
			Строка.ШиринаИзделия = РазмерыОсновного.Ширина;
			Строка.ВысотаИзделия = Строка.ВысотаОтсека;
	...
	Структура.Вставить("ВариантШкафа", ВариантШкафа);
...
Функция ПолучитьСтруктуруИзделия(Изделие, ВидИзделия)
	...
	ЕстьЛДСПДоп = ЗначениеЗаполнено(Изделие.СписокДеталей.Найти(Истина, "ВторойЦвет"));
	ЕстьКрючки = ЗначениеЗаполнено(Изделие.СписокНоменклатуры.Найти(Справочники.НоменклатурныеГруппы.Крючок, "Номенклатура"));
	
	СпрДВП = Справочники.ВидыДеталей.НайтиПоКоду("000000007");
	СпрЛДСП = Справочники.ВидыДеталей.НайтиПоКоду("000000022");
	
	ЕстьДВП = ЗначениеЗаполнено(Изделие.СписокДеталей.Найти(СпрДВП , "ВидДетали"));
	
	МассивФасадов = Изделие.СписокДеталей.НайтиСтроки(Новый Структура("ВидДетали", СпрЛДСП));
...
Функция ПолучитьРазмерыОсновногоЭлемента()
	...
	НашОсновнойЭлемент = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.ОсновнойЭлемент1));
	...
	Если ЕстьОсновной Тогда
		...
		Для Каждого Строка Из Детали Цикл
			Если Строка.ВидИзделия = ВидыИзделий.Потолок Тогда
				...
				КонецЕсли;
			ИначеЕсли Строка.ВидИзделия = ВидыИзделий.Пол И НЕ Строка.НеВлияетНаОсновной Тогда
				ВысотаОсновногоЭлемента = ВысотаОсновногоЭлемента - Строка.ВысотаИзделия;
			ИначеЕсли (Строка.ВидИзделия = ВидыИзделий.ЛевыйБоковойЭлемент
				ИЛИ Строка.ВидИзделия = ВидыИзделий.ПравыйБоковойЭлемент)
				И НЕ Строка.НеВлияетНаОсновной Тогда
				ШиринаОсновногоЭлемента = ШиринаОсновногоЭлемента - Строка.ШиринаИзделия;
			ИначеЕсли Строка.ВидИзделия = ВидыИзделий.АнтресольнаяПолка Тогда
				ВысотаАнтПолка = Строка.ВысотаОтсека;
			КонецЕсли;	
		КонецЦикла;                                                      
		...
		Размеры.Вставить("ВысотаАнтПолка", ВысотаАнтПолка);
		
		Возврат Размеры;
	Иначе
		
		Если ЗначениеЗаполнено(ВариантШкафа) 
           И ЗначениеЗаполнено(ВысотаИтог) 
		   И ЗначениеЗаполнено(ШиринаИтог)  
		   И ЗначениеЗаполнено(ГлубинаИтог) Тогда
...
Процедура УстановитьДоступность(ВидИзделия)
	...
	НомерОтсека = Элементы.Детали.ТекущиеДанные.НомерОтсека;
	СтрокаДетали = Детали.НайтиСтроки(Новый Структура("ВидИзделия, НомерОтсека", ВидИзделия, НомерОтсека));
	ЕстьИзделие = Истина;
	...
	Если СтрокаДетали.Количество() = 1 Тогда
		...
		ЭтоБоковаяПанель = (НашаДеталь.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСлева ИЛИ НашаДеталь.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСправа);
		
		Элементы.ДеталиЗавал.ТолькоПросмотр = ЭтоБоковаяПанель;
		
		МассивВидСтены = Новый Массив;
		МассивВидСтены.Добавить("Бетон");
		Если ВидИзделия = ВидыИзделий.Пол Тогда
			МассивВидСтены.Добавить("Ламинат");
		Иначе
			МассивВидСтены.Добавить("Гипс");
		КонецЕсли;
		Если ВидИзделия = ВидыИзделий.Потолок Тогда
			МассивВидСтены.Добавить("Натяжной");
		КонецЕсли;
		
		Элементы.ДеталиВидСтены.СписокВыбора.Очистить();
...
Процедура ОтобразитьКартинку(АдресЭлемента, ЭлементИзделия, АдресКрыши, ТолькоКрыша = Ложь)
	...
	Если ЗначениеЗаполнено(АдресЭлемента) Тогда
		...
		ЕстьКрыша = Детали.НайтиСтроки(Новый Структура("ВидИзделияПоКаталогу", ВидыИзделий.Крыша)).Количество()>0;
		
		Если ЗначениеЗаполнено(АдресКрыши) И ЕстьКрыша Тогда
		...
		Если СтенаСверху И (ЗначениеЗаполнено(ПотолокЛевый) ИЛИ ЗначениеЗаполнено(ПотолокПравый)) ИЛИ НЕ ЕстьКрыша Тогда
			ПотолокЛевый = "";
			ПотолокПравый = "";
		ИначеЕсли НЕ СтенаСверху И ЭлементИзделия = "Потолок" Тогда
			
			Для Каждого Элемент Из Детали Цикл
				Если Элемент.ВидИзделия = ВидыИзделий.ПравыйБоковойЭлемент
					ИЛИ Элемент.ВидИзделия = ВидыИзделий.ЛевыйБоковойЭлемент Тогда
...
Процедура ЗаполнитьСтроку(НашаСтрока, ВидИзделия, Изделие = Неопределено, СтруктураИзделия = Неопределено, ЭлементИзделия = Неопределено)
	...
	Если Изделие <> Неопределено Тогда
		...
		Если СтруктураИзделия <> Неопределено Тогда
			...
			Й = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЭлементИзделия, "ОсновнойЭлемент");
			
			Если Й.Количество() > 1 Тогда
				НашаСтрока.НомерОтсека = Число(Й[1]);
			КонецЕсли;
		...
		Если НЕ Корпусный Тогда
			...
			ЭтоБоковаяПанель = НашаСтрока.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСлева ИЛИ
			НашаСтрока.ВидИзделияПоКаталогу = ВидыИзделий.БоковаяПанельСправа;
		...
		Если ВидИзделия = ВидыИзделий.Потолок Тогда
			СтенаСверху = Изделие = ВидыИзделий.ПотолокЗаказчика;
		КонецЕсли;
		...
		Если ВидИзделия = ВидыИзделий.ОсновнойЭлемент1
		 ИЛИ ВидИзделия = ВидыИзделий.ОсновнойЭлемент2
		 ИЛИ ВидИзделия = ВидыИзделий.ОсновнойЭлемент3
		 ИЛИ ВидИзделия = ВидыИзделий.ОсновнойЭлемент4 Тогда
			 НашаСтрока.ПредставлениеВидаИзделия = "Основной";
		КонецЕсли;
		
		Если ВидИзделия = ВидыИзделий.АнтресольнаяПолка Тогда
			НашаСтрока.ПредставлениеВидаИзделия = "Ант.Полка";
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(НашаСтрока.ВысотаИзделияСтрока) ИЛИ ЗначениеЗаполнено(НашаСтрока.ШиринаИзделияСтрока) ИЛИ ЗначениеЗаполнено(НашаСтрока.ГлубинаИзделияСтрока) Тогда
			...
			ПолучитьЗавалы(ЗавалПол, ЗавалЛевый, ЗавалПравый, ЗавалПотолок, ЗавалСтенка, НашаСтрока);
...
Процедура ПриОткрытии(Отказ)
	...
	Для Каждого Элемент Из Детали Цикл
		...
		ОтобразитьКартинку(Элемент.АдресЭлемента, СтрЗаменить(Строка(Элемент.ВидИзделия), " ", "") + ?(Элемент.НомерОтсека > 0,Элемент.НомерОтсека,""), Элемент.АдресКрыши);
		
	КонецЦикла;
	
	Основные = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.ОсновнойЭлемент1));
	
	Для Каждого О Из Основные Цикл
		ЭтаФорма["ШиринаОтсека"+О.НомерОтсека] = О.ШиринаОтсека;
	КонецЦикла;
	        
	АнтПолка = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.АнтресольнаяПолка));
	
	Для Каждого А Из АнтПолка Цикл
		ЭтаФорма.ВысотаАнтПолки = А.ВысотаОтсека;
	КонецЦикла;
...
Процедура ОбновитьВидимостьЭлементов()
	...
	Элементы.ГруппаЭлементов.Доступность = ЗначениеЗаполнено(ВариантШкафа) 
	                                     И ЗначениеЗаполнено(ВысотаИтог) 
										 И ЗначениеЗаполнено(ШиринаИтог)  
										 И ЗначениеЗаполнено(ГлубинаИтог); 
	
	Для К = 1 По 4 Цикл
		Элементы["ОсновнойЭлемент" + К].Видимость = ?(К<=КоличествоОтсеков,Истина,Ложь);
		Элементы["ШиринаОтсека" + К].Видимость = ?(К<=КоличествоОтсеков,Истина,Ложь);
	КонецЦикла;
	
	Элементы.ШиринаОтсека2.Доступность = Истина; 
	Элементы.ШиринаОтсека3.Доступность = Истина;
	Элементы.ШиринаОтсека4.Доступность = Истина;
	
	Если КоличествоОтсеков = 2 Тогда
		 Элементы.ШиринаОтсека2.Доступность = Ложь;
	ИначеЕсли КоличествоОтсеков = 3 Тогда
 
		  Если ПроверитьВыдвижнойВСреднем() Тогда
			Элементы.ШиринаОтсека1.Доступность = Ложь;
			Элементы.ШиринаОтсека3.Доступность = Ложь;
		  Иначе
		 	Элементы.ШиринаОтсека3.Доступность = Ложь;
		  КонецЕсли;
	...
	ИначеЕсли КоличествоОтсеков = 4 Тогда
		 Элементы.ШиринаОтсека2.Доступность = Ложь;
		 Элементы.ШиринаОтсека4.Доступность = Ложь;
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПроверитьВыдвижнойВСреднем()
	
	Есть = Ложь;
	
	Для Каждого Стр Из Детали Цикл
		
		Если Стр.НомерОтсека = 2 И ЗначениеЗаполнено(Стр.Изделие) Тогда
			
			Если Стр.Изделие.ЕстьВыдвижнойЭлемент Тогда
				Есть = Истина;
				Прервать;
			КонецЕсли;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Корпусный = ИзделиеСпец.КорпусныйШкаф;
	ВариантШкафа = Параметры.ВариантШкафа;
	
	Если ЗначениеЗаполнено(ВариантШкафа) Тогда
		КоличествоОтсеков = ВариантШкафа.КоличествоОтсеков; 
	КонецЕсли;
	...
	ЗаполнитьВидыИзделий();
	ЗаполнитьНомГруппы();
	ЗаполнитьШапкуОсновныхНастроек();
	
	ПерезаполнитьСтруктуруИзделий();	
	
КонецПроцедуры

&НаСервере
Процедура ПерезаполнитьСтруктуруИзделий()
	
	СтруктураФормы = ПолучитьИзВременногоХранилища(Параметры.АдресТаблицы);
	Двери = СтруктураФормы.Двери;
	
	Если ЗначениеЗаполнено(СтруктураФормы.Детали) Тогда
	...
	Для Каждого Строка Из Детали Цикл
		...
		Если НашВидИзделия = Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент Тогда
			...
			ШиринаОсновного = ?(ЗначениеЗаполнено(ШиринаОсновного), ШиринаОсновного, 0) + Строка.ШиринаИзделия;
		...
		ИначеЕсли НашВидИзделия = Справочники.ВидыИзделийПоКаталогу.АнтресольнаяПолка Тогда
			
			Строка.ПредставлениеВидаИзделия = "Ант.Полка";
	...
	Если ЗначениеЗаполнено(ШиринаОсновного) И ШиринаОсновного > 0 Тогда
		...
		ШиринаОсновного = ШиринаОсновного + ОтнятьОтОсновногоДляПроверки;
...
Процедура ЗаполнитьНомГруппы()
	...
	МассивНомГрупп.Вставить("Крючок", Справочники.НоменклатурныеГруппы.Крючок);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьВидыИзделий()
	
	ВидыИзделий = Новый Структура;
	
	ВидыИзделий.Вставить("АнтресольнаяПолка", Справочники.ВидыИзделийПоКаталогу.АнтресольнаяПолка);
	
	ВидыИзделий.Вставить("ОсновнойЭлемент1", Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент);
	ВидыИзделий.Вставить("ОсновнойЭлемент2", Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент);
	ВидыИзделий.Вставить("ОсновнойЭлемент3", Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент);
	ВидыИзделий.Вставить("ОсновнойЭлемент4", Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент);
	
	ВидыИзделий.Вставить("ЛевыйБоковойЭлемент", Справочники.ВидыИзделийПоКаталогу.ЛевыйБоковойЭлемент);
	ВидыИзделий.Вставить("ПравыйБоковойЭлемент", Справочники.ВидыИзделийПоКаталогу.ПравыйБоковойЭлемент);
	
	ВидыИзделий.Вставить("БоковаяПанельСлева", Справочники.ВидыИзделийПоКаталогу.БоковаяПанельСлева);
	ВидыИзделий.Вставить("БоковаяПанельСправа", Справочники.ВидыИзделийПоКаталогу.БоковаяПанельСправа);
	
	ВидыИзделий.Вставить("БоковаяПанельЧерезОтбойнуюПланкуСлева", Справочники.ВидыИзделийПоКаталогу.БоковаяПанельЧерезОтбойнуюПланкуСлева);
	ВидыИзделий.Вставить("БоковаяПанельЧерезОтбойнуюПланкуСправа", Справочники.ВидыИзделийПоКаталогу.БоковаяПанельЧерезОтбойнуюПланкуСправа);
	
	ВидыИзделий.Вставить("КоробБоковойСлева", Справочники.ВидыИзделийПоКаталогу.КоробБоковойСлева);
	ВидыИзделий.Вставить("КоробБоковойСправа", Справочники.ВидыИзделийПоКаталогу.КоробБоковойСправа);
	
	ВидыИзделий.Вставить("ОтбойнаяПланкаСлева", Справочники.ВидыИзделийПоКаталогу.ОтбойнаяПланкаСлева);
	ВидыИзделий.Вставить("ОтбойнаяПланкаСправа", Справочники.ВидыИзделийПоКаталогу.ОтбойнаяПланкаСправа);
	
	ВидыИзделий.Вставить("СтенаСлева", Справочники.ВидыИзделийПоКаталогу.СтенаСлева);
	ВидыИзделий.Вставить("СтенаСправа", Справочники.ВидыИзделийПоКаталогу.СтенаСправа);
	
	ВидыИзделий.Вставить("ФальшПанельСлева", Справочники.ВидыИзделийПоКаталогу.ФальшПанельСлева);
	ВидыИзделий.Вставить("ФальшПанельСправа", Справочники.ВидыИзделийПоКаталогу.ФальшПанельСправа);
	
	ВидыИзделий.Вставить("Пол", Справочники.ВидыИзделийПоКаталогу.Пол);
	ВидыИзделий.Вставить("Потолок", Справочники.ВидыИзделийПоКаталогу.Потолок);
	
	ВидыИзделий.Вставить("ПотолокЗаказчика", Справочники.КаталогИзделий.ПотолокЗаказчика);
	
	ВидыИзделий.Вставить("Крыша", Справочники.ВидыИзделийПоКаталогу.Крыша);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ВыбранноеЗначение) = Тип("СправочникСсылка.КаталогИзделий") И ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		ИзделиеПриИзменении(ВыбранноеЗначение);
	КонецЕсли;
...
Процедура ВыборИзделия(Команда)
	...
	ТекущийЭлементИзделия = ВидыИзделий[ЭлементИзделия];
	...
	Для Каждого Строка Из Детали Цикл
		...
		Если Строка.ВидИзделия = ВидыИзделий.ПравыйБоковойЭлемент Тогда
			
			Если ВидИзделия = ВидыИзделий.БоковаяПанельСлева Тогда
				ВидИзделия = ВидыИзделий.БоковаяПанельСправа;
			ИначеЕсли ВидИзделия = ВидыИзделий.БоковаяПанельЧерезОтбойнуюПланкуСлева Тогда
				ВидИзделия = ВидыИзделий.БоковаяПанельЧерезОтбойнуюПланкуСправа;
			ИначеЕсли ВидИзделия = ВидыИзделий.КоробБоковойСлева Тогда
				ВидИзделия = ВидыИзделий.КоробБоковойСправа;
			ИначеЕсли ВидИзделия = ВидыИзделий.ОтбойнаяПланкаСлева Тогда
				ВидИзделия = ВидыИзделий.ОтбойнаяПланкаСправа;
			ИначеЕсли ВидИзделия = ВидыИзделий.СтенаСлева Тогда
				ВидИзделия = ВидыИзделий.СтенаСправа;
			ИначеЕсли ВидИзделия = ВидыИзделий.ФальшПанельСлева Тогда
				ВидИзделия = ВидыИзделий.ФальшПанельСправа;
			КонецЕсли;
	...
	Пар.Вставить("ВидИзделияПоКаталогу", ТекущийЭлементИзделия);
	Пар.Вставить("МассивИзделий", МассивИзделий);
	...
	Пар.Вставить("Корпусный", Корпусный);
	
	ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора",Пар,ЭтаФорма);
	
КонецПроцедуры

&НаКлиенте
Процедура ИзделиеПриИзменении(Изделие)
	...
	Если ЗначениеЗаполнено(Изделие) Тогда	
		...
		Если Й.Количество() > 1 Тогда
			НашаСтрока = Детали.НайтиСтроки(Новый Структура("ВидИзделия, НомерОтсека", ТекущийЭлементИзделия, Число(Й[1])));
		Иначе
...
Процедура Очистить(Команда)
	...
	Если Й.Количество() > 1 Тогда
		НашаСтрока = Детали.НайтиСтроки(Новый Структура("ВидИзделия, НомерОтсека", ТекущийЭлементИзделия, Число(Й[1])));
	Иначе
		НашаСтрока = Детали.НайтиСтроки(Новый Структура("ВидИзделия, НомерОтсека", ТекущийЭлементИзделия, 0));
	КонецЕсли;
...
Процедура ИзделиеНажатие(Элемент, СтандартнаяОбработка)
	...
	ТекущийЭлементИзделия = ВидыИзделий[ЭтаФорма.ТекущийЭлемент.Имя];	
	
	Й = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(ЭтаФорма.ТекущийЭлемент.Имя, "ОсновнойЭлемент");
	...
	Если НашаСтрока.Количество() = 1 Тогда
		...
		ИндексСтроки = Детали.Индекс(НашаСтрока[0]);
		Элементы.Детали.ТекущаяСтрока = ИндексСтроки;
...
Процедура ДеталиНоменклатураЛДСППриИзменении(Элемент)
	...
	Если Найти(ТекущиеДанные.ВысотаИзделияСтрока, "НоменклатураМатериала") > 0 
		...
		ПолучитьЗавалы(ЗавалПол, ЗавалЛевый, ЗавалПравый, ЗавалПотолок, ЗавалСтенка, ТекущиеДанные);
...
&НаСервере
Функция ПолучитьКоличествоДверей(Изделие)
	
	Характеристики = Изделие.СписокХарактеристик.НайтиСтроки(Новый Структура("Характеристика", ПланыВидовХарактеристик.ХарактеристикиШкафовКупе.КоличествоДверей));
	
	Возврат Характеристики[0].ЗначениеХарактеристики;	
...
Процедура ДвериНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если РазмерыДвери.ВысотаПроема > 0 И РазмерыДвери.ШиринаПроема > 0  Тогда
		...
		Если Ответ = КодВозвратаДиалога.Да Тогда
			...
			ПараметрыФормы.Вставить("КоличествоДверей", КоличествоОтсеков);
	...
	Если РазмерыОсновногоЭлемента <> Неопределено Тогда
		...
		Если ВысотаПроема > 0 И ШиринаПроема > 0  Тогда
			Для Каждого Элемент Из Детали Цикл
				...
				Если Элемент.ВидИзделия = ВидыИзделий.ЛевыйБоковойЭлемент ИЛИ
					
					Элемент.ВидИзделия = ВидыИзделий.ПравыйБоковойЭлемент Тогда
					
					Если Элемент.НеВлияетНаОсновной Тогда
						ШиринаПроема = ШиринаПроема - Элемент.ШиринаИзделия;
					КонецЕсли;

				ИначеЕсли Элемент.ВидИзделия = ВидыИзделий.Потолок ИЛИ
					
					Элемент.ВидИзделия = ВидыИзделий.Пол Тогда
...
Процедура ДеталиЗавалПриИзменении(Элемент)
	...
	Для Каждого Стр ИЗ Детали Цикл 
		...
		Если Стр.ВидИзделия = Данные.ВидИзделия Тогда
			Стр.Завал = Данные.Завал;
			Стр.ВидСтены = Данные.ВидСтены;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(ТекущиеДанные.ВысотаИзделияСтрока) ИЛИ ЗначениеЗаполнено(ТекущиеДанные.ШиринаИзделияСтрока) ИЛИ ЗначениеЗаполнено(ТекущиеДанные.ГлубинаИзделияСтрока) Тогда
			...
			//RonEXI: Костыль, в формуле используется переменная НоменклатураМатериала, тут она называется НоменклатураЛДСП
			НоменклатураМатериала = ЛексСервер.ЗначенияРеквизитовОбъекта(ТекущиеДанные.НоменклатураЛДСП,"ДлинаДетали,ШиринаДетали,ГлубинаДетали");
...
&НаКлиенте
Процедура ПолучитьЗавалы(ЗавалПол, ЗавалЛевый, ЗавалПравый, ЗавалПотолок, ЗавалСтенка, ТекущиеДанные)
...
&НаКлиенте
Процедура ДеталиКрючкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	НомГруппы = Новый СписокЗначений;
	НомГруппы.Добавить(МассивНомГрупп.Крючок);
...
&НаКлиенте
Процедура ДеталиЗавалНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	Ошибки = Неопределено;
	Изделие = Элементы.Детали.ТекущиеДанные;
	
	Если НЕ Изделие.Предопределенный И НЕ ЗначениеЗаполнено(Изделие.НоменклатураЛДСП) Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Текст = "Сначала заполните номенклатуру ЛДСП.";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.НоменклатураЛДСП", Текст);
...
&НаКлиенте
Процедура ДвериОткрытие(Элемент, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Двери) Тогда
		...
		Если РазмерыДвери.ВысотаПроема > 0 И РазмерыДвери.ШиринаПроема > 0  Тогда
			...
			НашОсновнойЭлемент = Детали.НайтиСтроки(Новый Структура("ВидИзделия", ВидыИзделий.ОсновнойЭлемент));
			КоличествоДверей = ПолучитьКоличествоДверей(НашОсновнойЭлемент[0].Изделие);	
			
			ПараметрыФормы = Новый Структура;
			ПараметрыФормы.Вставить("ВысотаПроема", РазмерыДвери.ВысотаПроема);
			ПараметрыФормы.Вставить("ШиринаПроема", РазмерыДвери.ШиринаПроема);
			ПараметрыФормы.Вставить("ЗначенияЗаполнения", Новый Структура("Спецификация", Спецификация));
			ПараметрыФормы.Вставить("КоличествоДверей", КоличествоДверей);
			ПараметрыФормы.Вставить("Ключ", Двери);
...
&НаКлиенте
Процедура ВариантШкафаПриИзменении(Элемент)
	
	КоличествоОтсеков = ЛексСервер.ЗначениеРеквизитаОбъекта(ВариантШкафа, "КоличествоОтсеков");
	
	Для К = 1 ПО 4 Цикл
		Если К > КоличествоОтсеков Тогда
			
			Строки = Детали.НайтиСтроки(Новый Структура("НомерОтсека", К));
			
			Если Строки.Количество() > 0 Тогда
				 Детали.Удалить(Строки[0]);
				 ЭтаФорма["ОсновнойЭлемент" + К] = "";
				 ЭтаФорма["ШиринаОтсека" + К] = 0;
			КонецЕсли;
...
&НаКлиенте
Процедура ДеталиВидСтеныПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВысотаИтогПриИзменении(Элемент)	
	ОбновитьВидимостьЭлементов();
КонецПроцедуры

&НаКлиенте
Процедура ШиринаИтогПриИзменении(Элемент)
	ЗаполнитьШиринуОтсека();
	ОбновитьВидимостьЭлементов();
КонецПроцедуры

&НаКлиенте
Процедура ГлубинаИтогПриИзменении(Элемент)
	ОбновитьВидимостьЭлементов();
КонецПроцедуры

&НаКлиенте
Процедура ВысотаАнтПолкиПриИзменении(Элемент)
	ЗаполнитьВысотуОтсека();
КонецПроцедуры
