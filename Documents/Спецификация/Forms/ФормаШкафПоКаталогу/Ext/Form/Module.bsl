...
Процедура ПриОткрытии(Отказ)
	...
	Если Детали.Количество() > 1 Тогда
		...
		Если ЗначениеЗаполнено(Детали[1].НоменклатураЛДСП) Тогда
			...
			СтруктураПодставляяемойНоменклатуры = ЛексСервер.ПолучитьСтруктураПодставляяемойНоменклатурыПоЦветуЛДСП(Детали[1].НоменклатураЛДСП, Подразделение);
...
Процедура ДеталиНоменклатураЛДСППриИзменении(Элемент)
	...
	СтруктураПодставляяемойНоменклатуры = ЛексСервер.ПолучитьСтруктураПодставляяемойНоменклатурыПоЦветуЛДСП(ЛДСП, Подразделение);
