...
Функция ПроверитьИУстановитьРазмерыПередСохранением(РазмерыОсновного)
	...
	Для Каждого Строка Из Детали Цикл
		...
		ИначеЕсли Строка.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент Тогда
			...
			Строка.ШиринаИзделия = Окр(РазмерыОсновного.Ширина/КоличествоОтсеков);
...
&НаКлиенте
Процедура УстановитьДоступность(ВидИзделия, НомерОтсека = 0)
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	НомерОтсека = Элемент.ТекущиеДанные.НомерОтсека; 
	УстановитьДоступность(ТекущийЭлементИзделия, НомерОтсека);
