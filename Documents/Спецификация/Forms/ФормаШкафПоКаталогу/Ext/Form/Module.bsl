...
Процедура ВыборИзделия(Команда)
	...
	Если Дилерский Тогда
		Изделие = ОткрытьФормуМодально("Справочник.КаталогИзделий.ФормаВыбора", Новый Структура("Отбор, ВидИзделияПоКаталогу, МассивИзделий", Новый Структура("АктивныйДляДилера, Корпусный", Истина, Корпусный), ТекущийЭлементИзделия, МассивИзделий), ЭтаФорма);
	Иначе
