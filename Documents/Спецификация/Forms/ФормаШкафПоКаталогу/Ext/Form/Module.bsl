...
Функция ПолучитьСтруктуруИзделия(Изделие)
	...
	Структура.Вставить("ВлияетНаПроем", Изделие.ВлияетНаПроем);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого Строка Из Детали Цикл
		...
		Строка.ВлияетНаПроем = НашеИзделие.ВлияетНаПроем;
...
Процедура ВыборИзделия(Команда)
	...
	Если ЗначениеЗаполнено(Изделие) Тогда	
		...
		НоваяСтрока.ВлияетНаПроем = СтруктураИзделия.ВлияетНаПроем;
	...
	Если РазмерыОсновногоЭлемента <> Неопределено Тогда
		...
		Если ВысотаПроема > 0 И ШиринаПроема > 0  Тогда
			Для Каждого Элемент Из Детали Цикл
				...
				ИначеЕсли Элемент.ВлияетНаПроем И 
