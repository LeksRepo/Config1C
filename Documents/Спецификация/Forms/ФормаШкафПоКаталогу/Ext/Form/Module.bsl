...
Процедура ДеталиНоменклатураЛДСППриИзменении(Элемент)
	...
	Кромка = ПолучитьКромкуПоЦвету(ЛДСП, Подразделение);
...
Функция ПолучитьКромкуПоЦвету(ЛДСП, Подразделение)
	...
	////Пока возмется первая подходящая. С номенклатурными группами надо решать будет
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Возврат ВыборкаДетальныеЗаписи.ПодставляемаяНоменклатура;
