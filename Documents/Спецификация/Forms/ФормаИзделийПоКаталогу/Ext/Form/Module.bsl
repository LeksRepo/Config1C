...
Функция ПолучитьДопЭлемент(ДопЭлемент, ШиринаИзделия, Подразделение)
	...
	Иначе
		...
		// { Васильев Александр Леонидович [03.04.2015]
		// Жульдиков нашёл. :)
		
		//Если ДопЭлемент = Справочники.НоменклатурныеГруппы.СушкаДляКоробаСДном Тогда
		//	НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.СушкаДляКоробаБезДна;
		//КонецЕсли;
...
Процедура УстановитьПараметр(Таблица, Параметр, ТипПараметра)
	...
	Для Каждого Строка Из Таблица Цикл
		...
		ИначеЕсли Строка.ВидЭлемента = "Фасад" И ОбщийФасад <> Строка.ВидФасада И ТипПараметра <> "ВидФасада" Тогда
	...
	Для каждого Элемент Из Детали Цикл
		...
		Иначе
			...
			Если ШиринаИзделия > Элемент.ШиринаИзделияМакс ИЛИ ШиринаИзделия < Элемент.ШиринаИзделияМин Тогда
				ТекстСообщения = "Для " + ОписаниеОшибки + " укажите ширину в диапазоне " + Элемент.ШиринаИзделияМин + "-" + Элемент.ШиринаИзделияМакс;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Изделие", ТекстСообщения);	
			КонецЕсли;
			Если Элемент.ВысотаИзделия > Элемент.ВысотаИзделияМакс ИЛИ Элемент.ВысотаИзделия < Элемент.ВысотаИзделияМин Тогда
				ТекстСообщения = "Для " + ОписаниеОшибки + " укажите высоту в диапазоне " + Элемент.ВысотаИзделияМин + "-" + Элемент.ВысотаИзделияМакс;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Изделие", ТекстСообщения);	
			КонецЕсли;
			Если Элемент.ГлубинаИзделия > Элемент.ГлубинаИзделияМакс ИЛИ Элемент.ГлубинаИзделия < Элемент.ГлубинаИзделияМин Тогда	
				ТекстСообщения = "Для " + ОписаниеОшибки + " укажите глубину в диапазоне " + Элемент.ГлубинаИзделияМин + "-" + Элемент.ГлубинаИзделияМакс;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Изделие", ТекстСообщения);		
			КонецЕсли;
			...
			Для Каждого Строка Из Элемент.ТаблицаФасадов Цикл
				...
				Иначе
					...
					Если НЕ ЗначениеЗаполнено(Строка.СхемаЯщика) Тогда
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.ДеталиТаблицаФасадов", ТекстСообщения);
					КонецЕсли;
			...
			Если Элемент.ЕстьСушка И НЕ ЗначениеЗаполнено(Элемент.Сушка) Тогда
				ТекстСообщения = "Выберите сушку у " + ОписаниеОшибки;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Сушка", ТекстСообщения);
			КонецЕсли;
			
			Если Элемент.ЕстьЛДСП Тогда
				Если НЕ ЗначениеЗаполнено(Элемент.НоменклатураЛДСП) Тогда
					ТекстСообщения = "Не выбрана номенклатура ЛДСП у " + ОписаниеОшибки;
					...
					//Иначе	
					//	ПроверитьКромкиЛДСП(Ошибки, Элемент.НоменклатураЛДСП);
				КонецЕсли;
				Если НЕ ЗначениеЗаполнено(Элемент.КромкаЛДСП) Тогда
					ТекстСообщения = "Не выбрана кромка ЛДСП у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьЛДСПДоп Тогда
				Если НЕ ЗначениеЗаполнено(Элемент.НоменклатураЛДСПДоп) Тогда
					ТекстСообщения = "Не выбрана номенклатура ЛДСП доп у " + ОписаниеОшибки;
				...
				Если НЕ ЗначениеЗаполнено(Элемент.КромкаЛДСПДоп) Тогда
					ТекстСообщения = "Не выбрана кромка ЛДСП доп у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьКлееная И НЕ ЗначениеЗаполнено(Элемент.НоменклатураДляСклеивания) Тогда
				ТекстСообщения = "Не выбрана доп. текстура у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьНожки И НЕ ЗначениеЗаполнено(Элемент.Ножки) Тогда
				ТекстСообщения = "Не выбраны ножки у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьСтекло И НЕ ЗначениеЗаполнено(Элемент.НоменклатураСтекло) Тогда
				ТекстСообщения = "Не выбрано стекло у " + ОписаниеОшибки;
			...
			Если (Элемент.ЕстьДВП ИЛИ Элемент.Изделие.СписокЯщики.Количество() > 0) И НЕ ЗначениеЗаполнено(Элемент.НоменклатураДВП) Тогда
				ТекстСообщения = "Не выбрана ДВП у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьМДФ И НЕ ЗначениеЗаполнено(Элемент.НоменклатураМДФ) Тогда
				ТекстСообщения = "Не выбрана МДФ у " + ОписаниеОшибки;
			...
			Если Элемент.ЕстьМДФ И НЕ ЗначениеЗаполнено(Элемент.КромкаМДФ) Тогда
				ТекстСообщения = "Не заполнена кромка для МДФ у " + ОписаниеОшибки;
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтруктураИзделия) Тогда
		...
		ДанныеИзделия.Сборка = Истина;
		
		ДанныеИзделия.Европаз = 8;
...
Процедура ДеталиТаблицаФасадовВыдвижнойЭлементПриИзменении(Элемент)
	...
	Если ДанныеТаблицаФасадов <> Неопределено Тогда
		...
		Элемент = ДанныеТаблицаФасадов.СхемаЯщика;
		
		НомГруппы = ПолучитьНомГруппыНаправляющие(Элемент);
		ДанныеТаблицаФасадов.МеханизмЭлемента = Неопределено;
		Элементы.ДеталиТаблицаФасадовМеханизмЭлемента.СписокВыбора.Очистить();
		
		Если НомГруппы.Количество() > 0 Тогда
			Элементы.ДеталиТаблицаФасадовМеханизмЭлемента.СписокВыбора.ЗагрузитьЗначения(НомГруппы);
		КонецЕсли;
