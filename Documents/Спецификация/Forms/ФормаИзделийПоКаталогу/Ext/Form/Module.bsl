...
Функция ПолучитьСтруктуруИзделия(Изделие, Подразделение, Серийное, Изменение = Ложь, ОбщаяНоменклатура) 
	...
	Структура.Вставить("ВысотаИзделияВерх", Изделие.ВысотаИзделияВерх);
	Структура.Вставить("ВысотаИзделияНиз", Изделие.ВысотаИзделияНиз);
	Структура.Вставить("ЕвропазДоступен", Изделие.ЕвропазДоступен);
	Структура.Вставить("ЭтоПенал", Изделие.ЭтоПенал);
	Структура.Вставить("БезРучек", Изделие.БезРучек);
	...
	Структура.Вставить("ВысотаИзделияМинВерх", Изделие.ВысотаИзделияМинВерх);
	Структура.Вставить("ВысотаИзделияМинНиз", Изделие.ВысотаИзделияМинНиз);
	...
	Структура.Вставить("ВысотаИзделияМаксВерх", Изделие.ВысотаИзделияМаксВерх);
	Структура.Вставить("ВысотаИзделияМаксНиз", Изделие.ВысотаИзделияМаксНиз);
	...
	Структура.Вставить("ЕстьВолшебныйУголок", Ложь);
	Структура.Вставить("НоменклатураСушка", Неопределено);
	Структура.Вставить("НоменклатураВолшебныйУголок", Неопределено);
	Структура.Вставить("НоменклатурнаяГруппаСушка", Неопределено);
	Структура.Вставить("НоменклатурнаяГруппаВолшебныйУголок", Неопределено);
	...
	Структура.Вставить("Угловой", Изделие.Угловой);
	...
	Для Каждого Элемент Из СписокДопЭлементов Цикл	
		...
		Если Элемент.ДопЭлемент = Справочники.НоменклатурныеГруппы.Сушка 
			...
			ИЛИ Элемент.ДопЭлемент = Справочники.НоменклатурныеГруппы.СушкаДляКоробаСДном
			ИЛИ Элемент.ДопЭлемент = Справочники.НоменклатурныеГруппы.СушкаНижняя Тогда
		...
		ИначеЕсли Элемент.ДопЭлемент = Справочники.НоменклатурныеГруппы.ВолшебныйУголок Тогда
			
			Структура.Вставить("ЕстьВолшебныйУголок", Истина);
			Структура.Вставить("НоменклатураВолшебныйУголок", НоменклатураДопЭлемента);
			Структура.Вставить("НоменклатурнаяГруппаВолшебныйУголок", Элемент.ДопЭлемент);
...
Функция ВыгрузитьНужнуюТаблицуВХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(Детали.Выгрузить(, "Изделие, ГлубинаИзделия, ШиринаИзделия, ВысотаИзделия, ВысотаИзделияВерх, ВысотаИзделияНиз, КодИзделия, Расположение"));
...
&НаСервереБезКонтекста
Функция ПолучитьДопЭлемент(ДопЭлемент, ШиринаИзделия, Подразделение, Расположение = Неопределено)
	...
	Иначе
		...
		ИначеЕсли ДопЭлемент = Справочники.НоменклатурныеГруппы.СушкаДляКоробаБезДна
			...
			ИЛИ ДопЭлемент = Справочники.НоменклатурныеГруппы.Сушка
			ИЛИ ДопЭлемент = Справочники.НоменклатурныеГруппы.СушкаНижняя Тогда
			Знак = "";
		Иначе
			Знак = "<";
		КонецЕсли;
		...
		Если ДопЭлемент = Справочники.НоменклатурныеГруппы.ВолшебныйУголок Тогда
			
			Если ЗначениеЗаполнено(Расположение) И Расположение = Перечисления.ВидыПрисоединенныхФайлов.КартинкаПравая Тогда
				Правый = Истина;	
			КонецЕсли;
		...
		Запрос.УстановитьПараметр("Правый", Правый);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("АдресТаблицаНастройкиКоллекции") Тогда
		
		НастройкиКоллекции.Загрузить(ПолучитьИзВременногоХранилища(Параметры.АдресТаблицаНастройкиКоллекции));
		
		Если НастройкиКоллекции.Количество() > 0 Тогда 
			КухоннаяКоллекция = НастройкиКоллекции[0].Коллекция; 
		КонецЕсли;
	...
	Элементы.ДеталиВолшебныйУголок.Видимость = Ложь;
...
Функция УстановитьПараметрыВыбораИзделия()
	...
	Если ЗначениеЗаполнено(КухоннаяКоллекция) Тогда
		ПараметрыВыбора.Добавить(Новый ПараметрВыбора("Отбор.КухоннаяКоллекция", КухоннаяКоллекция));
	КонецЕсли;
...
Процедура ПерезаполнитьСтруктуруИзделийКаталога(Детали)
	...
	Если ЗначениеЗаполнено(Детали) Тогда
		...
		Для каждого Элемент Из Детали Цикл
			...
			Элемент.ВысотаИзделияМинВерх = Структура.ВысотаИзделияМинВерх;
			Элемент.ВысотаИзделияМинНиз = Структура.ВысотаИзделияМинНиз;
			...
			Элемент.ВысотаИзделияМаксВерх = Структура.ВысотаИзделияМаксВерх;
			Элемент.ВысотаИзделияМаксНиз = Структура.ВысотаИзделияМаксНиз;
			...
			Элемент.ЕстьВолшебныйУголок = Структура.ЕстьВолшебныйУголок;
			...
			Элемент.НоменклатурнаяГруппаВолшебныйУголок = Структура.НоменклатурнаяГруппаВолшебныйУголок;
			...
			Элемент.Угловой = Структура.Угловой;
...
Процедура ЗаполнитьМассивНомГрупп()
	...
	МассивНомГрупп.Вставить("СушкаНижняя", Справочники.НоменклатурныеГруппы.СушкаНижняя);
	...
	МассивНомГрупп.Вставить("ВолшебныйУголок", Справочники.НоменклатурныеГруппы.ВолшебныйУголок);
...
Функция ПроверитьПередСохранением()
	...
	//RonEXI: Сначала проверяем на соответствие настройкам коллекции.
	
	Если Детали.Количество() > 0 Тогда
	
		Если ЗначениеЗаполнено(КухоннаяКоллекция) Тогда
		
			ДанныеКоллекции = НастройкиКоллекции[0]; 
			
			//Проверяем количество угловых нижних коробов.
			
			КоличествоУгловых = 0;
			УгловыхДостаточно = Истина;
			ПроверятьУгловые = (ДанныеКоллекции.КоличествоСторон > 1);
						   
			Если ПроверятьУгловые Тогда
				
				Для Каждого Элемент Из Детали Цикл
					
					Если Элемент.Изделие.ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.КухняНижний
					   И Элемент.Изделие.Угловой Тогда
					   
					   КоличествоУгловых = КоличествоУгловых + 1;
						
					КонецЕсли;
					
				КонецЦикла;
				
				Если ДанныеКоллекции.КоличествоСторон = 2 И КоличествоУгловых <> 1 Тогда
					
					 Текст = "Для кухни типа: " + ДанныеКоллекции.Форма + ", необходимо наличие 1 углового короба. У вас " + КоличествоУгловых + " угловых коробов.";
					 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , Текст);
					 УгловыхДостаточно = Ложь;
					 
				ИначеЕсли ДанныеКоллекции.КоличествоСторон = 3 И КоличествоУгловых <> 2 Тогда
					
					 Текст = "Для кухни типа: " + ДанныеКоллекции.Форма + ", необходимо наличие 2 угловых коробов. У вас " + КоличествоУгловых + " угловых коробов.";
					 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , Текст);
					 УгловыхДостаточно = Ложь;
			...
			//Проверяем размеры кухни для каждого типа кухни
			
			Если УгловыхДостаточно Тогда
				...
				Для Каждого Элемент Из Детали Цикл
					...
					Левый = (Элемент.Расположение = Перечисления.ВидыПрисоединенныхФайлов.КартинкаЛевая);
					
					Если Элемент.Изделие.ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.КухняНижний Тогда
						
						Если ДанныеКоллекции.КоличествоСторон = 1 Тогда
							
							СуммаЦентр = СуммаЦентр + Элемент.ШиринаИзделия; 
							
						КонецЕсли;
						
						Если ДанныеКоллекции.КоличествоСторон = 2 Тогда

							Если Элемент.Изделие.Угловой Тогда
								
								СуммаЛево = СуммаЛево + ?(Левый, Элемент.ШиринаИзделия, Элемент.ГлубинаИзделия);
								СуммаЦентр = СуммаЦентр + ?(Левый, Элемент.ГлубинаИзделия, Элемент.ШиринаИзделия);
							...
							Иначе
								...
								Если ТекущаяСторона = 0 Тогда
								     СуммаЛево = СуммаЛево + Элемент.ШиринаИзделия;
								КонецЕсли;
						...
						Если ДанныеКоллекции.КоличествоСторон = 3 Тогда
							...
							Если Элемент.Изделие.Угловой Тогда
								...
								ИначеЕсли ТекущаяСторона = 1 Тогда
									
									СуммаЦентр = СуммаЦентр + ?(Левый, Элемент.ШиринаИзделия, Элемент.ГлубинаИзделия);
									СуммаПраво = СуммаПраво + ?(Левый, Элемент.ГлубинаИзделия, Элемент.ШиринаИзделия);
							...
							Иначе
								...
								Если ТекущаяСторона = 2 Тогда
								     СуммаПраво = СуммаПраво + Элемент.ШиринаИзделия;
								КонецЕсли; 
				...
				Если ДанныеКоллекции.КоличествоСторон = 1 Тогда
					...
					Если СуммаЦентр <> ДанныеКоллекции.ШиринаКухниЦентр Тогда
						 Текст = "Ширина кухни, указанная в настройках коллекции (" + ДанныеКоллекции.ШиринаКухниЦентр + "), не совпадает с шириной выбранных коробов (" + СуммаЦентр + ")";
					КонецЕсли;
					
				ИначеЕсли ДанныеКоллекции.КоличествоСторон = 2 Тогда
					
					Если СуммаЦентр <> ДанныеКоллекции.ШиринаКухниЦентр ИЛИ СуммаЛево <> ДанныеКоллекции.ШиринаКухниЛево Тогда
						 Текст = "Ширина кухни, указанная в настройках коллекции (" + ДанныеКоллекции.ШиринаКухниЛево + "х" + ДанныеКоллекции.ШиринаКухниЦентр + "), не совпадает с шириной выбранных коробов (" + СуммаЛево + "х" + СуммаЦентр + ")";
					КонецЕсли;
					
				ИначеЕсли ДанныеКоллекции.КоличествоСторон = 3 Тогда
					
					Если СуммаЦентр <> ДанныеКоллекции.ШиринаКухниЦентр ИЛИ СуммаЛево <> ДанныеКоллекции.ШиринаКухниЛево ИЛИ СуммаПраво <> ДанныеКоллекции.ШиринаКухниПраво Тогда
						 Текст = "Ширина кухни, указанная в настройках коллекции (" + ДанныеКоллекции.ШиринаКухниЛево + "х" + ДанныеКоллекции.ШиринаКухниЦентр + "х" + ДанныеКоллекции.ШиринаКухниПраво + "), не совпадает с шириной выбранных коробов (" + СуммаЛево + "х" + СуммаЦентр + "х" + СуммаПраво + ")";
					КонецЕсли;
	...
	//RonEXI: Проверяем заполнение коробов.	
	...
	Для Каждого Элемент Из Детали Цикл
		...
		Иначе
			...
			Если ВысотаИзделия > Элемент.ВысотаИзделияМакс ИЛИ ВысотаИзделия < Элемент.ВысотаИзделияМин Тогда
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстСообщения);
			КонецЕсли;
			Если ВысотаИзделияВерх > Элемент.ВысотаИзделияМаксВерх ИЛИ ВысотаИзделияВерх < Элемент.ВысотаИзделияМинВерх Тогда
				ТекстСообщения = НомерИзделияСтрока + "Укажите высоту верх в диапазоне " + Элемент.ВысотаИзделияМинВерх + "-" + Элемент.ВысотаИзделияМаксВерх;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстСообщения);
			КонецЕсли;
			Если ВысотаИзделияНиз > Элемент.ВысотаИзделияМаксНиз ИЛИ ВысотаИзделияНиз < Элемент.ВысотаИзделияМинНиз Тогда
				ТекстСообщения = НомерИзделияСтрока + "Укажите высоту низ в диапазоне " + Элемент.ВысотаИзделияМинНиз + "-" + Элемент.ВысотаИзделияМаксНиз;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстСообщения);
			КонецЕсли;
			...
			Для Каждого Строка Из СтрокиФасадов Цикл
				...
				Если Элемент.БезРучек Тогда
					 Строка.БезРучки = Истина;	
				КонецЕсли;
				...
				Если НЕ БезФасада И НЕ ЗначениеЗаполнено(Ручка) И Строка.ЕстьРучка И НЕ БезРучки И НЕ Строка.КриволинейныйВыпилПодРучку И НЕ Строка.РучкаЩель Тогда
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ИндексТекущейСтроки <> ТекущиеДанные.НовыйИндекс Тогда
			ИндексТекущейСтроки = ТекущиеДанные.НовыйИндекс;
		Иначе
			Возврат;
		КонецЕсли;
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтруктураИзделия) Тогда
		...
		Если ЗначениеЗаполнено(КухоннаяКоллекция) 
		   И НастройкиКоллекции[0].ВысотаКоробовВерх > 0
		   И НастройкиКоллекции[0].ВысотаКоробовНиз > 0 
		   И СтруктураИзделия.ВысотаИзделияМаксВерх > 0
		   И СтруктураИзделия.ВысотаИзделияМаксНиз > 0 Тогда
			ТекущиеДанные.ВысотаИзделияВерх = НастройкиКоллекции[0].ВысотаКоробовВерх;
			ТекущиеДанные.ВысотаИзделияНиз = НастройкиКоллекции[0].ВысотаКоробовНиз;
		Иначе
			ТекущиеДанные.ВысотаИзделияВерх = СтруктураИзделия.ВысотаИзделияВерх;
			ТекущиеДанные.ВысотаИзделияНиз = СтруктураИзделия.ВысотаИзделияНиз;
		КонецЕсли;
		...
		ТекущиеДанные.ВысотаИзделияМинВерх = СтруктураИзделия.ВысотаИзделияМинВерх;
		ТекущиеДанные.ВысотаИзделияМинНиз = СтруктураИзделия.ВысотаИзделияМинНиз;
		...
		ТекущиеДанные.ВысотаИзделияМаксВерх = СтруктураИзделия.ВысотаИзделияМаксВерх;
		ТекущиеДанные.ВысотаИзделияМаксНиз = СтруктураИзделия.ВысотаИзделияМаксНиз;
		...
		ТекущиеДанные.БезРучек = СтруктураИзделия.БезРучек;
		...
		ТекущиеДанные.ЕстьВолшебныйУголок = СтруктураИзделия.ЕстьВолшебныйУголок;
		...
		ТекущиеДанные.ВолшебныйУголок = СтруктураИзделия.НоменклатураВолшебныйУголок;
		...
		ТекущиеДанные.НоменклатурнаяГруппаВолшебныйУголок = СтруктураИзделия.НоменклатурнаяГруппаВолшебныйУголок;
		...
		ТекущиеДанные.Угловой = СтруктураИзделия.Угловой;
	...
	Если ДанныеИзделия <> Неопределено Тогда
		Если ЗначениеЗаполнено(ДанныеИзделия.Изделие) Тогда
			...
			Если ДанныеИзделия.ЭтоПенал Тогда
				...
				Элементы.ВысотаИзделияВерх.Подсказка = Строка(ДанныеИзделия.ВысотаИзделияМинВерх) + " - " + Строка(ДанныеИзделия.ВысотаИзделияМаксВерх);
				Элементы.ВысотаИзделияВерх.МаксимальноеЗначение = ДанныеИзделия.ВысотаИзделияМаксВерх;
				Элементы.ВысотаИзделияВерх.МинимальноеЗначение = ДанныеИзделия.ВысотаИзделияМинВерх;
				
				Элементы.ВысотаИзделияНиз.Подсказка = Строка(ДанныеИзделия.ВысотаИзделияМинНиз) + " - " + Строка(ДанныеИзделия.ВысотаИзделияМаксНиз);
				Элементы.ВысотаИзделияНиз.МаксимальноеЗначение = ДанныеИзделия.ВысотаИзделияМаксНиз;
				Элементы.ВысотаИзделияНиз.МинимальноеЗначение = ДанныеИзделия.ВысотаИзделияМинНиз;
...
Процедура УстановитьВидимостьЭлементовФормы()
	...
	Если Элементы.ДеталиРучка.Видимость И ДанныеИзделия.БезРучек Тогда
		Элементы.ДеталиРучка.Видимость = Ложь;
		Элементы.ТаблицаФасадовНоменклатураРучки.Видимость = Ложь;
	Иначе
		Элементы.ТаблицаФасадовНоменклатураРучки.Видимость = Истина;
	КонецЕсли;
	...
	Элементы.ДеталиВолшебныйУголок.Видимость = ДанныеИзделия.ЕстьВолшебныйУголок;
...
Процедура ДеталиНоменклатураЛДСППриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(СтруктураПодставляяемойНоменклатуры.Кромка045_19) Тогда
		ТекущиеДанные.КромкаЛДСП = СтруктураПодставляяемойНоменклатуры.Кромка2_19;
	Иначе
...
Процедура ДеталиРасположениеПриИзменении(Элемент)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ТекущиеДанные.ЕстьВолшебныйУголок Тогда
			ТекущиеДанные.ВолшебныйУголок = ПолучитьДопЭлемент(ТекущиеДанные.НоменклатурнаяГруппаВолшебныйУголок, ТекущиеДанные.ШиринаИзделия, Подразделение, ТекущиеДанные.Расположение);
		КонецЕсли;
	КонецЕсли;
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ДанныеТаблицаФасадов <> Неопределено Тогда
			...
			Если ЭтоЯщик Тогда
				...
				Элементы.ДекорацияКриволинейныйВыпилПодРучку.Доступность = Элементы.ТаблицаФасадовКриволинейныйВыпилПодРучку.Доступность;
				
				Элементы.ТаблицаФасадовРучкаЩель.Доступность = ДанныеСхемы.РучкаЩель И (НЕ ДанныеТаблицаФасадов.БезФасада);
				Элементы.ДекорацияРучкаЩель.Доступность = Элементы.ТаблицаФасадовРучкаЩель.Доступность;
			...
			Иначе
				...
				Элементы.ТаблицаФасадовРучкаЩель.Доступность = Ложь;
			КонецЕсли;
			...
			Элементы.ТаблицаФасадовНоменклатураРучки.Доступность = (НЕ ДанныеТаблицаФасадов.БезФасада И НЕ ДанныеТаблицаФасадов.БезРучки) И ?(ЭтоЯщик, НЕ ДанныеТаблицаФасадов.КриволинейныйВыпилПодРучку И НЕ ДанныеТаблицаФасадов.РучкаЩель, Истина);	
...
Процедура ТаблицаФасадовВыдвижнойЭлементПриИзменении(Элемент)
	...
	Если ДанныеТаблицаФасадов <> Неопределено Тогда
		...
		ДанныеТаблицаФасадов.РучкаЩель = Ложь;
...
Процедура ОткрытьФормуПодбора(НомГруппы, ЭлементФормы)
	...
	Если НомГруппы.Получить(0).Значение = МассивНомГрупп.ВолшебныйУголок Тогда
		
		 ПраваяНоменклатура = (Элементы.Детали.ТекущиеДанные.Расположение = ПредопределенноеЗначение("Перечисление.ВидыПрисоединенныхФайлов.КартинкаПравая"));
		 Пар.Вставить("ПраваяНоменклатура", ПраваяНоменклатура);
...
Процедура ДеталиСушкаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НомГруппаСушка = Элементы.Детали.ТекущиеДанные.НоменклатурнаяГруппаСушка;
	
	НомГруппы = ПолучитьНомГруппыДеталиСушка(НомГруппаСушка);
...
&НаКлиенте
Функция ПолучитьНомГруппыДеталиСушка(НомГруппа)
	
	НомГруппы = Новый СписокЗначений;
	
	Если НомГруппа = МассивНомГрупп.СушкаНижняя Тогда
		НомГруппы.Добавить(МассивНомГрупп.СушкаНижняя);
	Иначе
...
Процедура ДеталиИзделиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(КухоннаяКоллекция) Тогда
		Пар.Вставить("КухоннаяКоллекция", КухоннаяКоллекция);
	КонецЕсли;
...
&НаКлиенте
Процедура ДеталиВолшебныйУголокНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	
	НомГруппы = Новый СписокЗначений;	
	НомГруппы.Добавить(Элементы.Детали.ТекущиеДанные.НоменклатурнаяГруппаВолшебныйУголок);
...
&НаКлиенте
Процедура ТаблицаФасадовРучкаЩельПриИзменении(Элемент)
...
&НаКлиенте
Процедура ДеталиОкончаниеПеретаскивания(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка)
