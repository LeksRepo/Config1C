...
Функция ПолучитьСтруктуруИзделия(Изделие, Подразделение, Изменение = Ложь) 
	...
	Структура.Вставить("РасположениеИзделия", Перечисления.РасположениеКартинки.Левая);
...
Процедура ОтобразитьКартинку()
	...
	Если Данные <> Неопределено Тогда
		Если ЗначениеЗаполнено(Данные.КодИзделия) Тогда
			АдресИзделия = РабочийКаталог + "Изделие" + Данные.РасположениеИзделия + Данные.КодИзделия;
		КонецЕсли;
		АдресВХранилище = "";
...
Функция ПроверитьПередСохранением()
	...
	Для каждого Элемент Из Детали Цикл
		...
		Если НЕ ЗначениеЗаполнено(Элемент.Изделие) Тогда                    
			...
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Детали[" + Элемент.ПолучитьИдентификатор() + "].Изделие", ТекстСообщения);
		Иначе
...
Процедура ДеталиНоменклатураЛДСППриИзменении(Элемент)
	...
	ТекущиеДанные.КромкаЛДСП = ?(СтруктураПодставляяемойНоменклатуры.Свойство("Кромка2_19"), СтруктураПодставляяемойНоменклатуры.Кромка2_19, Неопределено);
