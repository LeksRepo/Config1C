		...
		Иначе
			...
			Для Каждого Строка Из СтрокиФасадов Цикл
				...
				Если Строка.ВидЭлемента = "Фасад" Тогда
					...
					Если НЕ ЗначениеЗаполнено(Строка.Петли) И ЗначениеЗаполнено(Строка.РасположениеПазовИРучкиНаФасадах) И НЕ ПазыОтсутствуют И НЕ Строка.БезПетель Тогда
						ТекстСообщения = НомерФасадаСтрока + "Укажите петли.";
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстСообщения);
					КонецЕсли;
					
					Если ЗначениеЗаполнено(Строка.Петли) И НЕ ЗначениеЗаполнено(Строка.КоличествоПетель) И НЕ Строка.БезПетель Тогда
						ТекстСообщения = НомерФасадаСтрока + "Укажите количество петель.";
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстСообщения);
					КонецЕсли;
					
					Если ЗначениеЗаполнено(Строка.Петли) И НЕ ЗначениеЗаполнено(Строка.РасположениеПазовИРучкиНаФасадах) И НЕ Строка.БезПетель Тогда
...
Процедура ТаблицаФасадовПриАктивизацииСтроки(Элемент = Неопределено)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ДанныеТаблицаФасадов <> Неопределено Тогда
			...
			Элементы.ТаблицаФасадовБезФасада.Видимость = ЭтоЯщик;
