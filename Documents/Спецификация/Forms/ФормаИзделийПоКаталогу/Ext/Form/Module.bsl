...
Процедура ОтобразитьКартинку()
	...
	Если ЗначениеЗаполнено(ИмяФайла) Тогда	
		АдресКартинки = "<html><head><META HTTP-EQUIV=""Pragma"" CONTENT=""no-cache""></head><body bottommargin=""0"" topmargin=""0"" leftmargin=""0"" rightmargin=""0""><img src=""" + ИмяФайла + """ /></body></html>";
	Иначе		
		АдресКартинки = "";
	КонецЕсли;	
...
Процедура ДеталиПриАктивизацииСтроки(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(IDИзделия) ИЛИ НЕ (ТекущиеДанные.UID = IDИзделия) Тогда
		
		IDИзделия = ТекущиеДанные.UID; 
...
Процедура ТаблицаФасадовПриАктивизацииСтроки(Элемент = Неопределено)
	...
	Если ТекущиеДанные <> Неопределено Тогда
		...
		Если ДанныеТаблицаФасадов <> Неопределено Тогда
			...
			Элементы.ГруппаФасадыЯщик.Доступность = ЭтоЯщик;
			Элементы.ГруппаФасадыДоп.Доступность = НЕ ЭтоЯщик;
