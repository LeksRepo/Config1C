...
Функция ПолучитьСтруктуруИзделия(Изделие, Подразделение, Изменение = Ложь) 
	...
	Для Каждого Элемент Из СписокДопЭлементов Цикл	
		...
		ИначеЕсли Элемент.ДопЭлемент = Справочники.НоменклатурныеГруппы.Бутылочница Тогда
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Детали) Тогда
		...
		Для каждого Элемент Из Детали Цикл
			...
			Элемент.АдресКартинки = Структура.Адрес;
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтруктураИзделия) Тогда
		...
		ДанныеИзделия.АдресКартинки = РабочийКаталог + СтруктураИзделия.Адрес;
...
Функция УстановитьДопЭлемент(НоменклатураЭлемента, Ширина, Подразделение) 
	...
	Если НоменклатураЭлемента.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.Бутылочница Тогда
		НовыйЭлемент = ПолучитьДопЭлемент(Справочники.НоменклатурныеГруппы.Бутылочница, Ширина, Подразделение);
