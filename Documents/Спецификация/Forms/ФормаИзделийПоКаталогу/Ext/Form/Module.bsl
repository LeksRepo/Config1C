...
Функция ПолучитьСтруктуруИзделия(Изделие, Подразделение, Изменение = Ложь) 
	...
	Структура.Вставить("СтандартныеНаправляющие", ?(ВидИзделияПоКаталогу = Справочники.ВидыИзделийПоКаталогу.Камод, Справочники.НоменклатурныеГруппы.НаправляющиеШариковые45, Справочники.НоменклатурныеГруппы.НаправляющиеШариковыеСДоводчиком));	
...
Функция ПроверитьПередСохранением()
	...
	Для каждого Элемент Из Детали Цикл
		...
		Иначе
			...
			НоменклатураЛДСПДоп = ЗначениеЗаполнено(Элемент.Изделие.СписокМатериалы.Найти(Истина, "ВторойЦвет"));
			...
			Если НоменклатураЛДСПДоп Тогда
