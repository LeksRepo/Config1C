...
Функция ПолучитьСтруктуруИзделия(Изделие, Подразделение, Изменение = Ложь) 
	...
	Структура.Вставить("РасположениеИзделия", Перечисления.ВидыПрисоединенныхФайлов.КартинкаЛевая);
	...
	Структура.Вставить("СтандартныеНаправляющие", Справочники.НоменклатурныеГруппы.НаправляющиеШариковыеСДоводчиком);	
	...
	Если Изменение Тогда
		...
		Для Каждого Элемент Из Массив Цикл
			...
			Для Каждого Строка Из СтрокиФасада Цикл
				...
				Структура.Вставить("ТаблицаФасадов" + НомерФасада + "РасположениеПазов", Строка.РасположениеПазов);
			КонецЦикла;	
		КонецЦикла;
...
Функция ВыгрузитьНужнуюТаблицуВХранилище()
	...
	Возврат ПоместитьВоВременноеХранилище(Детали.Выгрузить(, "Изделие, ГлубинаИзделия, ШиринаИзделия, ВысотаИзделия, КодИзделия"));
...
&НаСервере
Функция СравнитьВид(Вид1, Т)

	Знч=Ложь;	
	
	Если Т=0 Тогда
		 Если Вид1 = Перечисления.ВидыПрисоединенныхФайлов.КартинкаЛевая Тогда	
			Знч=Истина;	
		КонецЕсли;
	КонецЕсли;
	
	Если Т=1 Тогда
		 Если Вид1 = Перечисления.ВидыПрисоединенныхФайлов.КартинкаПравая Тогда	
			Знч=Истина;	
		КонецЕсли;
	КонецЕсли;
...
Процедура ОтобразитьКартинку()
	...
	Если Данные <> Неопределено Тогда
		Если ЗначениеЗаполнено(Данные.КодИзделия) Тогда
			...
			Расп="";
			Если СравнитьВид(Данные.РасположениеИзделия, 0) Тогда
				 Расп="Левая";	
			КонецЕсли;
			 
			Если СравнитьВид(Данные.РасположениеИзделия, 1) Тогда
				 Расп="Правая";	
			КонецЕсли;
			
			АдресИзделия = РабочийКаталог + "Изделие" + Расп + Данные.КодИзделия;
		КонецЕсли;
		...
		Если ЗначениеЗаполнено(АдресИзделия) Тогда
			...
			Иначе
				ИмяФайла = РабочийКаталог + "Изделие" + Данные.КодИзделия;
				ФайлИзображения = Новый Файл(ИмяФайла);
				Если ФайлИзображения.Существует() Тогда
					ПоместитьФайл(АдресВХранилище, ИмяФайла, , Ложь);
				КонецЕсли;	
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Детали) Тогда
		...
		Для каждого Элемент Из Детали Цикл
			...
			Элемент.РасположениеИзделия = ?(ЗначениеЗаполнено(Элемент.РасположениеИзделия), Элемент.РасположениеИзделия, Структура.РасположениеИзделия);
	...
	Иначе
		Стр = Детали.Добавить();
		Стр.РасположениеИзделия = Перечисления.ВидыПрисоединенныхФайлов.КартинкаЛевая;
		...
		Иначе
			...
			Для Каждого Строка Из Элемент.ТаблицаФасадов Цикл
				...
				Если Строка.ВидЭлемента = "Фасад" Тогда
					...
					Если ЗначениеЗаполнено(Строка.РасположениеПазов) И НЕ ЗначениеЗаполнено(Строка.Петли) Тогда
					...
					Если ЗначениеЗаполнено(Строка.Петли) И НЕ ЗначениеЗаполнено(Строка.РасположениеПазов) Тогда
			...
			Если НЕ ЗначениеЗаполнено(Элемент.Столешница) Тогда
				ТекстСообщения = "Не выбрана столешница";
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Столешница", ТекстСообщения);
			КонецЕсли;
			
			Если ЗначениеЗаполнено(Элемент.Изделие.СписокМатериалы.Найти("Пристенок")) И НЕ ЗначениеЗаполнено(Элемент.Пристенок) Тогда
				ТекстСообщения = "Не выбран пристенок";
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Элементы.Детали.ТекущиеДанные.Пристенок", ТекстСообщения);
			КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(Элемент.НоменклатураДВП) И (ЗначениеЗаполнено(Элемент.Изделие.СписокМатериалы.Найти("ДВП")) ИЛИ Элемент.Изделие.СписокЯщики.Количество() > 0) Тогда
...
Процедура ИзделиеПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СтруктураИзделия) Тогда
		...
		ДанныеИзделия.РасположениеИзделия = СтруктураИзделия.РасположениеИзделия;
		...
		Для Индекс = 1 По СтруктураИзделия.КоличествоТаблицаФасадов Цикл
			...
			Если Строка.ВидЭлемента = "Ящик" Тогда
				...
				Строка.МеханизмЭлемента = СтруктураИзделия.СтандартныеНаправляющие;
			КонецЕсли;	
		КонецЦикла;
...
&НаКлиенте
Процедура ДеталиРасположениеИзделияПриИзменении(Элемент)
...
Процедура ДеталиТаблицаФасадовПовернутьФасадПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ДанныеТаблицаФасадов.РасположениеРучки) Тогда
		
		ДанныеТаблицаФасадов.РасположениеРучки = ПовернутьФасад(ДанныеТаблицаФасадов.РасположениеРучки, ДанныеТаблицаФасадов.ПовернутьФасад);
