...
Функция ОпределитьМакеты (Документ)
	...
	Если Документ.СписокМатериалы.Количество() > 0 или Документ.СписокЯщики.Количество() > 0 Тогда
		
		СписокМакетов = СписокМакетов + "Спецификация";
		
		Для Каждого Элемент Из Документ.СписокМатериалы Цикл
			
			Если Элемент.Материал = "ФасадСтеклянный" И ЗначениеЗаполнено(Элемент.РадиусФасада) Тогда
