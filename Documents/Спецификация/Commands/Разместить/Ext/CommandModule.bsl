	...
	Если Результат.СформироватьРаскрой Тогда
		...
		Попытка
			...
			Если НЕ ЗначениеЗаполнено(ДокОбъект.Технолог) 
				...
				ДокОбъект.Технолог = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(ПараметрыСеанса.ТекущийПользователь, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
			КонецЕсли;
