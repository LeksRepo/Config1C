...
Функция Проверить(СпецификацияСсылка, Ошибки)
	...
	Если ТекущийСтатус <> Перечисления.СтатусыСпецификации.Проверяется
		...
		И ТекущийСтатус <> Перечисления.СтатусыСпецификации.Рассчитывается 
		И ТекущийСтатус <> Перечисления.СтатусыСпецификации.ОжиданиеМатериала Тогда
		
		Результат.Разместить = Ложь;
		
		ТекстСообщения = "Разместить спецификацию можно со статусами 'Сохранен', 'Рассчитывается', 'Ожидание материала' или 'Проверяется' . У %1 статус '%2'";
...
Процедура Разместить(СпецификацияСсылка, Проверка)
	...
	Попытка
		...
		Если Проверка Тогда
			Документы.Спецификация.УстановитьСтатусСпецификации(СпецификацияСсылка, Перечисления.СтатусыСпецификации.Проверяется);
		Иначе
			Если ДокОбъект.Дилерский И ДокОбъект.СписокМатериаловПодЗаказ.Количество() > 0 Тогда 
