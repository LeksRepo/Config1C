...
Функция ОпределитьМакеты(Документ)
	...
	МожноПечататьРаскрой = Ложь;
	
	Если Дилер Тогда
		
		МожноПечататьРаскрой = ПроверитьДоходОтДилера(Документ.Автор, Документ.Подразделение);
		
	КонецЕсли;
	
	Если (НЕ Дилер ИЛИ (Дилер И Документ.Проведен) или (Дилер и МожноПечататьРаскрой)) И (Документ.СписокМатериалы.Количество() > 0 или Документ.СписокЯщики.Количество() > 0 или Документ.СписокДверей.Количество() > 0) Тогда
