	...
	Для Каждого Документ Из ПараметрКоманды Цикл
		...
		Для каждого Элемент Из МассивМакетов Цикл
		 ...
		 Если ЕстьЧертежДвери Тогда
			...
			ПараметрыФормыФлэш.Вставить("Договор", "");
		...
		Если ЗначениеЗаполнено(МассивМакетов) Тогда
			УправлениеПечатьюКлиент.ВыполнитьКомандуПечати("Документ.Спецификация", МассивМакетов, Массив, ПараметрыВыполненияКоманды.Источник, ПараметрыПечати);
		Иначе
