	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Пока ВыборкаПоМатериалам.Следующий() Цикл
				...
				Если НЕ (МатериалЛДСП И ЗначениеЗаполнено(ВыборкаПоМатериалам.НомерИзделия)) Тогда
		...
		|			ТОГДА СпецификацияСписокЯщики.ШиринаБоковойСтороны - 2
		|		ИНАЧЕ 0
...
Функция ПечатьДверь(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	ДвериНоменклатура.Номенклатура КАК Номенклатура,
	|	ДвериНоменклатура.Длина,
	|	ДвериНоменклатура.Количество КАК Количество,
	|	ДвериНоменклатура.Ширина,
	|	ВЫБОР
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Двери.Номенклатура КАК ДвериНоменклатура
	|		ПО СпецификацияСписокДверей.Двери = ДвериНоменклатура.Ссылка
	|ГДЕ
	|	СпецификацияСписокДверей.Ссылка В(&МассивДокументов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДвериНоменклатура.Номенклатура.Наименование
	|ИТОГИ
	|	МАКСИМУМ(Контрагент),
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаДверь.Следующий() Цикл
			...
			ОбластьСтрока.Параметры.Заполнить(ВыборкаДверь);
			ТабДок.Вывести(ОбластьСтрока);
...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	ОбластьДвериПодпись = Макет.ПолучитьОбласть("ОбластьДвериПодпись");
	ОбластьДеталиПодписи = Макет.ПолучитьОбласть("ОбластьДеталиПодписи");
	ОбластьКомплектацииПодписи = Макет.ПолучитьОбласть("ОбластьКомплектацииПодписи");
	...
	|			КОГДА СпецификацияСписокНоменклатуры.ЧерезСклад
	|					И СпецификацияСписокНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)
	|					И (СпецификацияСписокНоменклатуры.Номенклатура.МестоОбработки = ЗНАЧЕНИЕ(Перечисление.МестоОбработки.Отгрузка)
	|						ИЛИ СпецификацияСписокНоменклатуры.НомерИзделия = 0)
	...
	|			КОГДА СпецификацияСписокНоменклатуры.ЧерезСклад
	|					И СпецификацияСписокНоменклатуры.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Материал)
	|					И (СпецификацияСписокНоменклатуры.Номенклатура.МестоОбработки = ЗНАЧЕНИЕ(Перечисление.МестоОбработки.ПоКаталогуВЦех)
	|						ИЛИ СпецификацияСписокНоменклатуры.НомерИзделия <> 0)
	|				ТОГДА СпецификацияСписокНоменклатуры.Количество
	|			ИНАЧЕ 0
	...
	|	КОНЕЦ КАК Заказчик,
	|	Спецификация.АдресМонтажа КАК АдресЗаказчика,
	|	Спецификация.Упаковка,
	|	Спецификация.Комментарий,
	|	Спецификация.СуммаНарядаСпецификации КАК СуммаНаряда,
	|	Спецификация.Срочный,
	|	НАЧАЛОПЕРИОДА(Спецификация.ДатаИзготовления, ДЕНЬ) КАК ДатаАдрес,
	|	Спецификация.Номер КАК НомерСпецификации,
	...
	|		ПО Спецификация.Ссылка = Двери.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоДеталей КАК Детали
	|		ПО Спецификация.Ссылка = Детали.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоКомплектация КАК КоличествоКомплектация
	|		ПО (КоличествоКомплектация.Ссылка = Спецификация.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоКоробов КАК КоличествоКухонныхКоробов
	|		ПО (КоличествоКухонныхКоробов.Ссылка = Спецификация.Ссылка)
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		НаименованиеАктДоставки = "Акт доставки мебельного комплекта";
		НаименованиеАктОтгрузки = "Акт отгрузки мебельного комплекта";
		...
		ТабДок.Вывести(ОбластьШапка);
		ТабДок.Вывести(ОбластьЛинияОтрыва);
		ОбластьДеталиПодписи.Параметры.Заполнить(Выборка);
		ОбластьДвериПодпись.Параметры.Заполнить(Выборка);
		ОбластьКомплектацииПодписи.Параметры.Заполнить(Выборка);
		
		Если ЗначениеЗаполнено(Выборка.КоличествоДеталей) или ЗначениеЗаполнено(Выборка.КоличествоКоробов) Тогда
			
			ТабДок.Вывести(ОбластьДеталиПодписи);
			
			Если ЗначениеЗаполнено(Выборка.КоличествоДверей) Тогда
				
				ТабДок.Присоединить(ОбластьДвериПодпись);
				
				Если ЗначениеЗаполнено(Выборка.СкладКоличество) или ЗначениеЗаполнено(Выборка.ЦехКоличество) Тогда
					
					ТабДок.Присоединить(ОбластьКомплектацииПодписи);
					
				КонецЕсли;
				
			КонецЕсли;
			
		ИначеЕсли ЗначениеЗаполнено(Выборка.КоличествоДверей) Тогда
			
			ТабДок.Вывести(ОбластьДвериПодпись);
		...
		ИначеЕсли ЗначениеЗаполнено(Выборка.СкладКоличество) или ЗначениеЗаполнено(Выборка.ЦехКоличество) Тогда
			
			ТабДок.Вывести(ОбластьКомплектацииПодписи);
...
Функция ПечатьИзделиеПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СпецификацияСписокИзделийПоКаталогу.ШиринаИзделия КАК ШиринаКороба,
	...
	|	СпецификацияСписокИзделийПоКаталогу.СтрокаФасады,
	|	СпецификацияСписокИзделийПоКаталогу.СтрокаЯщики,
	...
	Пока Выборка.Следующий() Цикл 
		...
		Пока ВыборкаПоИзделиям.Следующий() Цикл
			...
			ОбластьТело.Параметры.ФурнитураОтдельно = ФурнитураОтдельно;
			ОбластьТело.Параметры.ФурнитураВСборе = ФурнитураВСборе;
			
			Если ВыборкаПоИзделиям.ВидИзделия = Перечисления.ВидыИзделий.ШкафКупе Тогда
				
				ЧислоФасадов = 0;
				
			Иначе
				
				// { Васильев Александр Леонидович [07.02.2014]
				// Лёш, вот тут беда какая-то.
				// Бывае, что СтрокаФасады и СтрокаЯщики не заполнены
				// } Васильев Александр Леонидович [07.02.2014]
				
				Если ЗначениеЗаполнено(ВыборкаПоИзделиям.СтрокаФасады) Тогда
					ТаблицаФасадов = ЗначениеИзСтрокиВнутр(ВыборкаПоИзделиям.СтрокаФасады);
				Иначе
					ТаблицаФасадов = Новый ТаблицаЗначений;
				КонецЕсли;
				
				Если ЗначениеЗаполнено(ВыборкаПоИзделиям.СтрокаЯщики) Тогда
					ТаблицаЯщиков = ЗначениеИзСтрокиВнутр(ВыборкаПоИзделиям.СтрокаЯщики);
				Иначе
					ТаблицаЯщиков = Новый ТаблицаЗначений;
				КонецЕсли;
				
				ЧислоФасадов = ТаблицаФасадов.Количество() + ТаблицаЯщиков.Количество();
			...
			Если ЧислоФасадов > 0 Тогда
				
				ТабДок.Вывести(ОбластьШапкаФасады);
				
				Если ТаблицаФасадов.Количество() > 0 Тогда
					СписокМатериалы = ВыборкаПоИзделиям.Изделие.СписокМатериалы;
					Для Каждого Строка Из ТаблицаФасадов Цикл
						ОбластьСтрокаФасады.Параметры.ИмяФасада = Строка.Представление;
						ОбластьСтрокаФасады.Параметры.ВидФасада = СтрЗаменить(Строка.ВидФасада, "Фасад", "");
						ОбластьСтрокаФасады.Параметры.Петля = СписокМатериалы.Найти(Строка.Представление,"ИмяДетали").Петли.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.ЦветФасада = Строка.Номенклатура.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.Обрамление = Строка.Кромка.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.Ручка = Строка.Ручка.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.РасположениеРучки = СписокМатериалы.Найти(Строка.Представление,"ИмяДетали").ПоложениеРучки;
						ОбластьСтрокаФасады.Параметры.Механизм1 = "";
						ОбластьСтрокаФасады.Параметры.Демпфер = СписокМатериалы.Найти(Строка.Представление,"ИмяДетали").Демпфер;
						ОбластьСтрокаФасады.Параметры.ВыдвижнойЭлемент = "";
						ОбластьСтрокаФасады.Параметры.Механизм = "";
						ОбластьСтрокаФасады.Параметры.ЛДСП = "";
						ОбластьСтрокаФасады.Параметры.Торцовка = "";
						ТабДок.Вывести(ОбластьСтрокаФасады);
					КонецЦикла;
				КонецЕсли;
				
				Если ТаблицаЯщиков.Количество() > 0 Тогда
					СписокЯщики = ВыборкаПоИзделиям.Изделие.СписокЯщики;
					Для Каждого СтрокаЯщики Из ТаблицаЯщиков Цикл
						ОбластьСтрокаФасады.Параметры.ИмяФасада = СтрокаЯщики.Представление;
						ОбластьСтрокаФасады.Параметры.ВидФасада = "";
						ОбластьСтрокаФасады.Параметры.Петля = "";
						ОбластьСтрокаФасады.Параметры.ЦветФасада = СтрокаЯщики.Фасад.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.Обрамление = СтрокаЯщики.КромкаФасада.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.Ручка = СтрокаЯщики.Ручка.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.РасположениеРучки = "";
						ОбластьСтрокаФасады.Параметры.Механизм1 = "";
						ОбластьСтрокаФасады.Параметры.Демпфер = "";
						ОбластьСтрокаФасады.Параметры.ВыдвижнойЭлемент = СтрокаЯщики.ВидЯщика;
						Глубина = 0;
						Попытка
							Выполнить("Глубина = " + СписокЯщики.Найти(СтрокаЯщики.Представление, "ИмяЯщика").ГлубинаЯщика);
						Исключение
							Глубина = ВыборкаПоИзделиям.ГлубинаКороба;
						КонецПопытки;
						Направляющие = НайтиНаправляющиеНоменклатура(ВыборкаПоИзделиям.Подразделение, СтрокаЯщики.Направляющие, Глубина);
						ОбластьСтрокаФасады.Параметры.Механизм = ?(ЗначениеЗаполнено(Направляющие), Направляющие.КраткоеНаименование, "");
						ОбластьСтрокаФасады.Параметры.ЛДСП = ВыборкаПоИзделиям.НоменклатураЛДСП.КраткоеНаименование;
						ОбластьСтрокаФасады.Параметры.Торцовка = ВыборкаПоИзделиям.КромкаЛДСП.КраткоеНаименование;
						ТабДок.Вывести(ОбластьСтрокаФасады);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
