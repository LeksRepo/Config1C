...
Функция РазложитьЯщикиНаДетали(Документ) Экспорт
	...
	Для Каждого Ящик Из Документ.СписокЯщики Цикл
		...
		Для Каждого ДетальСхема Из Схема.Детали Цикл
			...
			НоваяДеталь.Наименование = ДетальСхема.Наименование; 
			...
			Если ДетальСхема.Тип = Перечисления.ВидыДеталейЯщиков.Фасад Тогда
				...
				НоваяДеталь.РучкаЩель = Ящик.РучкаЩель;
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка в формуле Схема ящика: Высота");
			КонецПопытки;
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка в формуле Схема ящика: Ширина");
			КонецПопытки;
			...
			Исключение
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Ошибка в формуле Схема ящика: Количество");
			КонецПопытки;
...
Функция ОтправитьСтатусыНаСайт(МассивСпецификаций) Экспорт

	ТекДата = ТекущаяДата();
	
	Для Каждого СпецификацияСсылка Из МассивСпецификаций Цикл 
		...
		Контрагент = СпецификацияСсылка.Контрагент;
		
		Если ЗначениеЗаполнено(Контрагент.Фамилия)
			И ЗначениеЗаполнено(Контрагент.Имя)
			И ЗначениеЗаполнено(Контрагент.Отчество) Тогда
			
			Контрагент = Контрагент.Фамилия + " " + Лев(Контрагент.Имя, 1) + ". " + Лев(Контрагент.Отчество, 1) + ".";  
			
		КонецЕсли;
		
		ДатаОтгрузки = Формат(СпецификацияСсылка.ДатаОтгрузки, "ДФ=dd.MM.yyyy"); 
		ДатаМонтажа = Формат(СпецификацияСсылка.ДатаМонтажа, "ДФ=dd.MM.yyyy");
		ПервичнаяДатаМонтажа = Формат(СпецификацияСсылка.ПервичнаяДатаМонтажа, "ДФ=dd.MM.yyyy");
		НомерОчередиМонтажа = ПолучитьНомерОчередиМонтажа(СпецификацияСсылка);
		
		СтатусСтр = Строка(ТекущийСтатус);
		
		Если ТекущийСтатус = Перечисления.СтатусыСпецификации.Сохранен Тогда
			 СтатусСтр = "Заказ в стадии оформления.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Рассчитывается Тогда
			 СтатусСтр = "Заказ обрабатывается инженером.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Проверяется Тогда
			 СтатусСтр = "Заказ обрабатывается инженером.";	 
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.ОжиданиеМатериала Тогда
			 СтатусСтр = "Заказ комплектуется материалом.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Размещен Тогда
			 СтатусСтр = "Заказ в очереди на изготовление.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.ПереданВЦех Тогда
			 СтатусСтр = "Заказ изготавливается.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Изготовлен Тогда
			 СтатусСтр = "Заказ изготовлен, ожидает отгрузки.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Отгружен Тогда
			 СтатусСтр = "Заказ отгружен со склада.";
		ИначеЕсли ТекущийСтатус = Перечисления.СтатусыСпецификации.Установлен Тогда
			 СтатусСтр = "Заказ выполнен.";
		КонецЕсли;
	    ...
	    |<date>" + ТекДата + "</date>
	    |<status>" + СтатусСтр + "</status>
	    |<customer>" + Контрагент + "</customer>
	    |<output_date>" + ДатаОтгрузки + "</output_date>
	    |<installation_date>" + ДатаМонтажа + "</installation_date>
	    |<first_installation_date>" + ПервичнаяДатаМонтажа + "</first_installation_date>
		|<queue_position>" + НомерОчередиМонтажа + "</queue_position>
		|</order_status>";
		
		Соединение = Новый HTTPСоединение("rospil.me",443,,,,,Новый ЗащищенноеСоединениеOpenSSL());
 
	  	ЗапросPOST = Новый HTTPЗапрос("/api/order-status"); 
...
Функция ПолучитьНомерОчередиМонтажа(Спец)
	
	НомерОчереди = 0;
	Счетчик = 0;
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Подразделение", Спец.Подразделение);
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка = Спец Тогда
			 НомерОчереди = Счетчик;
...
Функция УстановитьСтатусСпецификации(СпецификацияСсылка, НовыйСтатус) Экспорт
	...
	МасСпец = Новый Массив();
	МасСпец.Добавить(СпецификацияСсылка);
	ОтправитьСтатусыНаСайт(МасСпец);
...
Процедура ПечатьСпецификации_ВывестиШапку(Спецификация, ТабДок, НазваниеЗоны)
	...
	ОбластьИзделиеЦеховаяСборка = Макет.ПолучитьОбласть("ИзделиеЦеховаяСборка");	
	...
	Если НазваниеЗоны = "Зона ОТК" И Спецификация.Изделие.СборкаВЦеху Тогда
		 ТабДок.Вывести(ОбластьИзделиеЦеховаяСборка);
	КонецЕсли;
		...
		Пока ВыборкаПоМатериалам.Следующий() Цикл
			...
			Иначе
				...
				Если НЕ ((МатериалЛДСП ИЛИ МатериалДВП) 
					...
					Если ВыборкаПоМатериалам.РадиусЛевоВерх > 0 Тогда
						ОбластьСтрока.Параметры.РадиусЛевоВерх = "R" + Строка(ВыборкаПоМатериалам.РадиусЛевоВерх);
					КонецЕсли;
					
					Если ВыборкаПоМатериалам.РадиусЛевоНиз > 0 Тогда
						ОбластьСтрока.Параметры.РадиусЛевоНиз = "R" + Строка(ВыборкаПоМатериалам.РадиусЛевоНиз);
					КонецЕсли;	
					
					Если ВыборкаПоМатериалам.РадиусПравоВерх > 0 Тогда
						ОбластьСтрока.Параметры.РадиусПравоВерх = "R" + Строка(ВыборкаПоМатериалам.РадиусПравоВерх);
					КонецЕсли; 
					
					Если ВыборкаПоМатериалам.РадиусПравоНиз > 0 Тогда
						ОбластьСтрока.Параметры.РадиусПравоНиз = ?(ВыборкаПоМатериалам.Срез, "S", "R") + Строка(ВыборкаПоМатериалам.РадиусПравоНиз);
					КонецЕсли;
...
Процедура ПечатьСпецификации_Фасады(Спецификация, ТабДок, НазваниеЗоны)
	...
	Если НазваниеЗоны = "Зона ОТК" Тогда
		...
		Запрос.УстановитьПараметр("ПоВсемЗонам", Истина);
	Иначе
		Запрос.УстановитьПараметр("ПоказыватьВСборе", Истина);
		Запрос.УстановитьПараметр("ПоВсемЗонам", Ложь);
	КонецЕсли;
	
	Если НазваниеЗоны = "Зона сборки" Тогда
		 Запрос.УстановитьПараметр("ПроверятьСтеколку", Истина);
	Иначе
		 Запрос.УстановитьПараметр("ПроверятьСтеколку", Ложь);
	КонецЕсли;
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ПоКаталогу.Сборка, ЛОЖЬ)
	|			ТОГДА ""В СОСТАВЕ КОРОБА""
	|		ИНАЧЕ """"
	|	КОНЕЦ КАК ВКоробе
	|ИЗ
	...
	|			КОГДА &ПоВсемЗонам
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ Список.ВидДетали.ЦеховаяЗона В (&ЦеховыеЗоны)
	|		КОНЕЦ
	...
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА &ПроверятьСтеколку
	|				ТОГДА ВЫБОР
	|						КОГДА Список.ВидДетали.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Стеколка)
	|							ТОГДА ВЫБОР
	|									КОГДА Список.Ссылка.Изделие.СборкаВЦеху
	|											ИЛИ Список.НомерИзделия > 0
	|										ТОГДА ИСТИНА
	|									ИНАЧЕ ЛОЖЬ
	|								КОНЕЦ
	|						ИНАЧЕ ИСТИНА
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьСтрокаЯчейка.Параметры.ВКоробе = Выборка.ВКоробе;
...
Функция ПечатьШкафПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если НЕ ЗначениеЗаполнено(КоличествоОтсеков) ИЛИ КоличествоОтсеков = 0 Тогда
			КоличествоОтсеков = 1;	
		КонецЕсли;
		
		Макет = Документы.Спецификация.ПолучитьМакет("ШкафПоКаталогу" + КоличествоОтсеков);
		...
		Пока ВыборкаПоКоробам.Следующий() Цикл
			...
			ИначеЕсли ВыборкаПоКоробам.ВидИзделия = Справочники.ВидыИзделийПоКаталогу.ОсновнойЭлемент Тогда
				...
				Если НЕ ЗначениеЗаполнено(Отсек) ИЛИ Отсек = 0 Тогда
					 Отсек = 1;
				КонецЕсли;
