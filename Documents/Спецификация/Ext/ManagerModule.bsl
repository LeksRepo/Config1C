...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СУММА(Список.КоличествоСклад) КАК Количество,
	|	Список.Номенклатура.Наименование КАК НоменклатураНаименование
	|ИЗ
	|	Документ.Спецификация.СкладГотовойПродукции КАК Список
	|ГДЕ
	|	Список.Номенклатура.НоменклатурнаяГруппа.ВидМатериала = ЗНАЧЕНИЕ(Перечисление.ВидыМатериалов.Хлыстовой)
	|	И Список.Ссылка = &Спецификация
	|	И НЕ Список.Номенклатура.МатериалЗаказчика
	|	И Список.КоличествоСклад > 0
	|
	|СГРУППИРОВАТЬ ПО
	|	Список.Номенклатура,
	...
	ТЗХлыстовойМатериал = Запрос.Выполнить().Выгрузить();
	...
	|	Документ.Спецификация.СкладГотовойПродукции КАК СпецификацияСкладГотовойПродукции
	|ГДЕ
	|	СпецификацияСкладГотовойПродукции.Ссылка = &Спецификация
	|
	|СГРУППИРОВАТЬ ПО
	|	СпецификацияСкладГотовойПродукции.Ссылка
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	СписокЛистов.Ссылка КАК Ссылка,
	|	СУММА(ВЫБОР
	|			КОГДА НоменклатураПодразделений.ОкруглятьДоЛистов
	|					И &Дилерский
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК Количество
	|ПОМЕСТИТЬ КоличествоВернутьОстатки
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА Специфик.Дилерский
	|			ТОГДА Специфик.Контрагент.Телефон
	...
	|	Остатки.Количество КАК ВернутьОстаткиКоличество
	|ИЗ
	|	Документ.Спецификация КАК Специфик
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоВернутьОстатки КАК Остатки
	|		ПО Специфик.Ссылка = Остатки.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоДверей КАК Двери
	|		ПО Специфик.Ссылка = Двери.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоДетали КАК Детали
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ КоличествоКороба КАК КоличествоКухонныхКоробов
	|		ПО (КоличествоКухонныхКоробов.Ссылка = Специфик.Ссылка)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Договор КАК Договор
	|		ПО (Договор.Спецификация = Специфик.Ссылка)
	|ГДЕ
	|	Специфик.Ссылка = &Спецификация" ;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТЗХлыстовойМатериал.Количество() > 0 Тогда
			
			ОбластьШапкаХлыстовойМатериал = Макет.ПолучитьОбласть("ШапкаХлыстовойМатериал");
			ОбластьСтрокаХлыстовойМатериал = Макет.ПолучитьОбласть("СтрокаХлыстовойМатериал");
			
			//ТабДок.Вывести(ОбластьПустаяСтрока);
			ТабДок.Вывести(ОбластьШапкаХлыстовойМатериал);
			
			НомерСтроки = 0;
			
			Для Каждого Стр Из ТЗХлыстовойМатериал Цикл
				
				НомерСтроки = НомерСтроки + 1;
				
				ОбластьСтрокаХлыстовойМатериал.Параметры.Заполнить(Стр);
				ОбластьСтрокаХлыстовойМатериал.Параметры.НомерСтроки = НомерСтроки;
				ТабДок.Вывести(ОбластьСтрокаХлыстовойМатериал);
		...
		Если ЗначениеЗаполнено(СтруктураНаряда) Тогда
			...
			ОбластьШапкаАктаВыпуска.Параметры.ДатаНаряда = Формат(СтруктураНаряда.ДатаНаряда, "ДЛФ=DD");
			ОбластьШапкаАктаВыпуска.Параметры.ДатаИзготовления = Формат(СтруктураНаряда.ДатаИзготовления, "ДЛФ=DD");
	...
	|	Изделия.Ссылка,
	|	СУММА(ВЫБОР
	|			КОГДА Изделия.Сборка
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК КоробовВСборе,
	|	СУММА(ВЫБОР
	|			КОГДА Изделия.Сборка
	|				ТОГДА 0
	|			ИНАЧЕ 1
	|		КОНЕЦ) КАК КоробовБезСборки
	|ИЗ
	|	Документ.Спецификация.СписокИзделийПоКаталогу КАК Изделия
	|ГДЕ
	|	Изделия.Ссылка = &Ссылка
	...
	Если ВыборкаКоробов.Количество() > 0 Тогда
		...
		Пока ВыборкаКоробов.Следующий() Цикл
			...
			ОбластьШапкаКороба.Параметры.Заполнить(ВыборкаКоробов);
			ТабДок.Вывести(ОбластьШапкаКороба);
