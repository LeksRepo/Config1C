...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьЗаголовок.Параметры.ДоговорАдрес 		= "" + Договор +"/Адрес: " + Выборка.АдресМонтажа;
		ОбластьЗаголовок.Параметры.Комментарий 			= Выборка.ОбщийКомментарий;
		...
		//СтруктураКомплектации 	= Документы.Спецификация.ПерезаполнитьКомплектацию(Спецификация);
		ВыборкаКомплектации 	= Документы.Спецификация.ПерезаполнитьКомплектацию(Спецификация);//СтруктураКомплектации.Выборка;
		//ТЗКомплектации 			= СтруктураКомплектации.ТЗ;
		...
		Пока ВыборкаПоМатериалам.Следующий() Цикл
			...
			Пока ДетальныеЗаписи.Следующий() Цикл
				...
				// { Васильев Александр Леонидович [16.12.2013]
				// отработать "Кант по месту"
				
				МассивПереключатель = Новый Массив;
				МассивНоменклатура = Новый Массив;
				МассивПереключатель.Добавить(ДетальныеЗаписи.ПереключательКромкаСверху);
				МассивПереключатель.Добавить(ДетальныеЗаписи.ПереключательКромкаСнизу);
				МассивПереключатель.Добавить(ДетальныеЗаписи.ПереключательКромкаСлева);
				МассивПереключатель.Добавить(ДетальныеЗаписи.ПереключательКромкаСправа);
				МассивНоменклатура.Добавить("ВыборМебельнойКромкиСверху");
				МассивНоменклатура.Добавить("ВыборМебельнойКромкиСнизу");
				МассивНоменклатура.Добавить("ВыборМебельнойКромкиСлева");
				МассивНоменклатура.Добавить("ВыборМебельнойКромкиСправа");
				
				Для ы = 0 по 3 Цикл
					Если МассивПереключатель[ы] = "По месту" Тогда
						ОбластьСтрока.Параметры[МассивНоменклатура[ы]] = "Кант по месту";
					КонецЕсли;
				КонецЦикла;
		...
		Если ВыборкаЯщиков.Количество() > 0 Тогда
			...
			ОбластьКоличествоДеталейЯщиков 				= Макет.ПолучитьОбласть("КоличествоДеталейЯщики");
			...
			КоличествоДеталейЯщиков 	= 0;
			КоличествоФасадов 			= 0;
			
			Пока ВыборкаЯщиков.Следующий() Цикл
				
				НомерСтроки 						= НомерСтроки +1;
				КоличествоДеталейЯщиков 	= КоличествоДеталейЯщиков +ВыборкаЯщиков.КоличествоЯщиков;
			...
			ОбластьКоличествоДеталейЯщиков.Параметры.КоличествоДеталей = КоличествоДеталейЯщиков;
			ТабДок.Вывести(ОбластьКоличествоДеталейЯщиков);
			...
			Если ТаблицаФасадовЯщиков.Количество() > 0 Тогда
				...
				ОбластьКоличествоДеталейЯщиков.Параметры.КоличествоДеталей = КоличествоФасадов;
				ТабДок.Вывести(ОбластьКоличествоДеталейЯщиков);
		...
		Если ВыборкаКомплектации.Количество() > 0 Тогда
			...
			//	
			//	НомерСтроки = НомерСтроки + 1;
			//	ОбластьСтрокаКомплектации.Параметры.Заполнить(Строка);
...
Функция ПечатьДверь(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	И ДвериНоменклатура.Номенклатура.МестоОбработки <> ЗНАЧЕНИЕ(Перечисление.МестоОбработки.ЦехМерный)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьЗаголовок.Параметры.Комментарий = "Договор: " + Договор + " /  Адрес: " + Выборка.АдресМонтажа;
		...
		//УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабДок, НомерСтрокиНачало, ОбъектыПечати, Выборка.Двери);
