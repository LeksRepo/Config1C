...
Функция ПечатьЗаказ(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	МАКСИМУМ(СпецификацияСписокНоменклатуры.ВнутренняяСтоимость) КАК ВнутренняяСтоимость,
	|	МАКСИМУМ(СпецификацияСписокНоменклатуры.ВнутренняяСтоимость / СпецификацияСписокНоменклатуры.Количество) КАК ВнутренняяЦена
	|ИЗ
	|	Документ.Спецификация.СписокНоменклатуры КАК СпецификацияСписокНоменклатуры
	|ГДЕ
	...
	Пока ВыборкаПоДокументу.Следующий() Цикл
		...
		НомерСтрокиНачало = ТабДок.ВысотаТаблицы + 1;
		БрендДилер = ВыборкаПоДокументу.Подразделение = Справочники.Подразделения.НайтиПоНаименованию("Бренд дилер ИП Щербакова Н.А.")
			или ВыборкаПоДокументу.Подразделение = Справочники.Подразделения.НайтиПоНаименованию("Бренд дилер ИП Захаров Е.Ю.");
		...
		Пока ВыборкаПоВидуНоменклатуры.Следующий() Цикл
			...
			Если ВидНомеклатуры = Перечисления.ВидыНоменклатуры.Материал Тогда
				...
				Если БрендДилер Тогда
					...
					ОбластьШапкаНоменклатуры.Параметры.ВнутренниеЦены = "Список выводится по внутренним ценам";
				...
				Пока ВыборкаМатериалов.Следующий() Цикл
					...
					Если БрендДилер Тогда
						...
						РозничнаяСтоимость = ВыборкаМатериалов.ВнутренняяСтоимость;
						ОбластьСтрока.Параметры.Цена = ВыборкаМатериалов.ВнутренняяЦена;
