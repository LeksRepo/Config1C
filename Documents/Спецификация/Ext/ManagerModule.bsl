...
Функция ПечатьСчетНаОплатуПокупателю(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = 
		...
		|	ПРЕДСТАВЛЕНИЕ(Спецификация.Организация) КАК ПредставлениеПолучателяПлатежа,
		|	Спецификация.Организация.ПолноеНаименование КАК ПредставлениеПоставщика,
		...
		|	Спецификация.Номер,
		|	Спецификация.Дата,
		|	ПРЕДСТАВЛЕНИЕ(Спецификация.Контрагент) КАК ПредставлениеПокупателя,
	...
	Товар = РезультатЗапроса[1].Выгрузить();
	...
	ТабДок.Очистить();
	 ...
	 ОбластьЗаголовок.Параметры.ТекстЗаголовка =  "Счет на оплату №"; 
	//ТабДок.Вывести(ОбластьЗаголовок);
