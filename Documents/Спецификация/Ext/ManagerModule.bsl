...
Функция ПолучитьПараметрыЯщика(СтрокаЯщика, ЕстьРебро) Экспорт
	...
	Если ВидЯщика = Перечисления.ВидыЯщика.Обычный И Проем >= 150 Тогда
		...
		Если ЗначениеЗаполнено(СтрокаЯщика.НаправляющиеНоменклатура) Тогда
			
			ТолщинаНаправляющей = СтрокаЯщика.НаправляющиеНоменклатура.ГлубинаДетали;
			
		Иначе
			
			ТолщинаНаправляющей = 28;
			
		КонецЕсли;
		
		СтрокаЯщика.ШиринаБоковойСтороны = Проем - ТолщинаНаправляющей - 30;
		СтрокаЯщика.ШиринаДно = Проем - ТолщинаНаправляющей - 30;
