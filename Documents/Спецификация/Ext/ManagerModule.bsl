...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Если НЕ ВыборкаПодЗаказ.Пустой() ИЛИ НЕ ВыборкаПредоставленный.Пустой() Тогда
		...
		Пока ВыборкаПодЗаказ.Следующий() Цикл
			...
			ОбластьПодЗаказСтрока.Параметры.Данные = "     "+ВыборкаПодЗаказ.Номенклатура + ?(ЗначениеЗаполнено(ВыборкаПодЗаказ.Комментарий), " ("+ВыборкаПодЗаказ.Комментарий+")","") + ?(ЗначениеЗаполнено(ВыборкаПодЗаказ.Поставщик), " - "+ВыборкаПодЗаказ.Поставщик,"");  
		...
		Пока ВыборкаПредоставленный.Следующий() Цикл
			...
			ОбластьПредоставленныйСтрока.Параметры.Данные = "     "+ВыборкаПредоставленный.Номенклатура + ?(ЗначениеЗаполнено(ВыборкаПредоставленный.Комментарий), " ("+ВыборкаПредоставленный.Комментарий+")","");  
