...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	УслугиРасчета.Добавить(ОбщаяНоменклатура.РасчетКухни);
	УслугиРасчета.Добавить(ОбщаяНоменклатура.РасчетШкафаКупе);
	УслугиРасчета.Добавить(ОбщаяНоменклатура.РасчетКорпуснойМебели);
	...
	Запрос.УстановитьПараметр("Подразделение", Спецификация.Подразделение);
	Запрос.УстановитьПараметр("ЦеховыеЗоны", ЦеховыеЗоны);
	Запрос.УстановитьПараметр("УслугиРасчета", УслугиРасчета);
	...
	|ГДЕ
	|	СписокНом.Ссылка = &Спецификация
	|	И СписокНом.Номенклатура В(&УслугиРасчета)";
	...
	ВыборкаРасчет = МассивРезультатовЗапроса[8];
	...
	ДилерскийСРасчетом = Выборка.Дилерский И (НЕ ВыборкаРасчет.Пустой());
	
	Если НЕ (ДоговорБезМонтажа ИЛИ ДилерскийСРасчетом) Тогда
...
Функция ПечатьПрисадкиПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СписокИзделий.ВысотаИзделия2,
	|	СписокИзделий.ВысотаИзделия3,
	...
	|	ДеталиСпец.ВысотаИзделия2,
	|	ДеталиСпец.ВысотаИзделия3,
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для Каждого Строка ИЗ ТЗ Цикл
			...
			ВысотаИзделия2 = Строка.ВысотаИзделия2;
			ВысотаИзделия3 = Строка.ВысотаИзделия3;
