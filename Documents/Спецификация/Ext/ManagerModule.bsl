...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		|	Документ.Спецификация.СписокДверей КАК СпецификацияСписокДверей
		|ГДЕ
		|	СпецификацияСписокДверей.Ссылка = &Спецификация
		|
		|СГРУППИРОВАТЬ ПО
		|	СпецификацияСписокДверей.Ссылка
		|;
	...
	//Если НЕ Объект.РедактированиеЗавершено Тогда
	//		ОбластьШапка.Параметры.РедактированиеЗавершено = "Подписи не действительны. Только для просмотра!";
	//КонецЕсли;
	
	//Если ЗначениеЗаполнено(НарядЗадание) Тогда
	//		
	//	ПодписьНаряд = "Нар: " + СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(НарядЗадание.Номер, "0", "Слева");
	//	ОбластьШапка.Параметры.ПодписьНаряд = ПодписьНаряд;
	//		
	//КонецЕсли;
	
	Возврат ТабДок;
	
	Договор = ПолучитьДоговор(МассивДокументов);
	
	Если НЕ Договор = Документы.Договор.ПустаяСсылка() Тогда
		
		//ОбластьШапка.Параметры.ВидДоставки = "Доставка: " + ВидДоставки;
	...
	//		
	//		ОбластьШапка.Параметры.Доставка="V";
	//		
	//	Иначе
	//		
	//		ОбластьШапка.Параметры.Доставка="";
	//		
	//	КонецЕсли;
	//	
	//	ТабДок.Вывести(ОбластьШапка);
			...
			Если ЧислоФасадов > 0 Тогда
				...
				Если ТаблицаФасадов.Количество() > 0 Тогда
					...
					Для Каждого Строка Из ТаблицаФасадов Цикл
						...
						ОбластьСтрокаФасады.Параметры.ВидФасада = Строка.ВидФасада;
						ОбластьСтрокаФасады.Параметры.Петля = СписокМатериалы.Найти(Строка.Представление,"ИмяДетали").Петли;
						ОбластьСтрокаФасады.Параметры.ЦветФасада = Строка.Номенклатура;
						ОбластьСтрокаФасады.Параметры.Обрамление = Строка.Кромка;
						ОбластьСтрокаФасады.Параметры.Ручка = Строка.Ручка;
						...
						ОбластьСтрокаФасады.Параметры.Демпф = "";
				...
				Если ТаблицаЯщиков.Количество() > 0 Тогда
					...
					Для Каждого СтрокаЯщики Из ТаблицаЯщиков Цикл
						...
						ОбластьСтрокаФасады.Параметры.ЦветФасада = "";
						ОбластьСтрокаФасады.Параметры.Обрамление = "";
						ОбластьСтрокаФасады.Параметры.Ручка = СтрокаЯщики.Ручка;
						...
						КонецПопытки;
						ОбластьСтрокаФасады.Параметры.Механизм = НайтиНаправляющиеНоменклатура(ВыборкаПоИзделиям.Подразделение, СтрокаЯщики.Направляющие, Глубина);
						ОбластьСтрокаФасады.Параметры.ЛДСП = СтрокаЯщики.Фасад;
						ОбластьСтрокаФасады.Параметры.Торцовка = СтрокаЯщики.КромкаФасада;
						ТабДок.Вывести(ОбластьСтрокаФасады);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
