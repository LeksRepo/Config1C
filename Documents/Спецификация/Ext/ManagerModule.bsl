...
Функция РазложитьЯщикиНаДетали(Документ) Экспорт
	...
	Для Каждого Ящик Из Документ.СписокЯщики Цикл
		...
		Для Каждого ДетальСхема Из Схема.Детали Цикл
			...
			Если ЭтоПластик Тогда
				ПодложкаПодПластик = ТЗ.Добавить();
				ЗаполнитьЗначенияСвойств(ПодложкаПодПластик,НоваяДеталь);
				ПодложкаПодПластик.Номенклатура = Ящик.Номенклатура;
			КонецЕсли;
...
Функция ПечатьПрисадкиПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	Присадки.ЛинейкаВерхНиз,
	|	Присадки.ЛинейкаВерхНизФормула,
	|	Присадки.ЛинейкаЛевоПраво,
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для Каждого Строка ИЗ ТЗ Цикл
			...
			ОбластьСтрокаТаблицы.Параметры.Деталь = НомерДетали + СтрокаОпераций;
			...
			Попытка
				...
				ОбластьСтрокаТаблицы.Параметры.ЛинейкаВерхНизЗначение = ЛинейкаВерхНизЗначение;
				ОбластьСтрокаТаблицы.Параметры.ЛинейкаЛевоПравоЗначение = ЛинейкаЛевоПравоЗначение;
