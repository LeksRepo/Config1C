...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СпецификацияСписокМатериалы.Скругление,
	...
	|	СпецификацияСписокМатериалы.НоменклатураДляСклеивания.КраткоеНаименование КАК Ручка
	|ИЗ
	...
	Пока Выборка.Следующий() Цикл
		...
		|	СпецификацияСписокЯщики.ШиринаБоковойСтороны - 2 КАК ШиринаРебро,
		|	ВЫБОР
		...
		Пока ВыборкаПоМатериалам.Следующий() Цикл
			...
			ИначеЕсли Материал = "10 ЛДСП" или Материал = "16 ЛДСП" или Материал = "10 ЛДСП+10 ЛДСП" или Материал = "10 ЛДСП+16 ЛДСП" или Материал = "16 ЛДСП+16 ЛДСП" 
				или Материал = "ДВП" или Материал = "МДФ" Тогда
			...
			Пока ДетальныеЗаписи.Следующий() Цикл
				...
				Если ВыборкаПоМатериалам.Материал = "ФасадЛДСП" Тогда
					...
					ОбластьСтрока.Параметры.УниверсальнаяКромка = ДетальныеЗаписи.Кромка2мм;
				...
				Если ЗначениеЗаполнено(ДетальныеЗаписи.НоменклатураДляСклеивания) И Фасад Тогда
		...
		Если ВыборкаЯщиков.Количество() > 0 Тогда
			...
			Пока ВыборкаЯщиков.Следующий() Цикл
				...
				Если НЕ ВыборкаЯщиков.ВидФасада = "Нет" Тогда
					...
					НомерСтрокиФасадов 	= НомерСтрокиФасадов + 1;
					ТорцеваяЛента 			= Новый Структура;
					...
					НоваяСтрока.НомерСтроки 				= НомерСтрокиФасадов;
	...
	Для каждого д Из МассивДокументов Цикл
		
		Если д.СписокМатериалы.Количество() = 0 Тогда
...
Функция ПечатьДверь(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СпецификацияСписокДверей.Ссылка.Контрагент,
	|	СпецификацияСписокДверей.Ссылка.Подразделение,
	|	СпецификацияСписокДверей.Ссылка.Автор,
	|	СпецификацияСписокДверей.Ссылка.Номер,
	|	СпецификацияСписокДверей.Ссылка.ДатаИзготовления,
	...
	Пока Выборка.Следующий() Цикл 
		...
		ОбластьЗаголовок.Параметры.Комментарий 			= "" + Договор + " /  Адрес: " + Выборка.Ссылка.АдресМонтажа;
		ОбластьЗаголовок.Параметры.Автор 					= Выборка.Автор;
		ОбластьЗаголовок.Параметры.Подразделение 		= Выборка.Подразделение;
		...
		ТабДок.ПовторятьПриПечатиСтроки 								= ТабДок.Область(1,,4,);
		Дверь																		= Выборка.Двери;
		ОбластьШапкаТаблицы.Параметры.ВысотаПроема 			= Дверь.ВысотаПроема;
		ОбластьШапкаТаблицы.Параметры.ШиринаПроема 		= Дверь.ШиринаПроема;
		ОбластьШапкаТаблицы.Параметры.ВысотаДвери 			= Дверь.ВысотаДвери;
		ОбластьШапкаТаблицы.Параметры.ШиринаДвери 			= Дверь.ШиринаДвери;
		ОбластьШапкаТаблицы.Параметры.КоличествоДверей 	= Дверь.Количество;
		ОбластьШапкаТаблицы.Параметры.ВысотаПолотна 		= Дверь.ВысотаПолотна;
		ОбластьШапкаТаблицы.Параметры.ШиринаПолотна 		= Дверь.ШиринаПолотна;
		
		ТабДок.Вывести(ОбластьШапкаТаблицы);
		
		НоменклатураДверей = Дверь.Номенклатура;
		
		Для каждого Элемент Из НоменклатураДверей Цикл
			
			Если НЕ Строка(Элемент.Номенклатура.ВидНоменклатуры) = "Услуга" Тогда
				
				ОбластьСтрока.Параметры.НомерДетали 	= Элемент.НомерСтроки;
				ОбластьСтрока.Параметры.Номенклатура 	= Строка(Элемент.Номенклатура);
				ОбластьСтрока.Параметры.Длина 				= Элемент.Длина;
				ОбластьСтрока.Параметры.Ширина 			= Элемент.Ширина;
				ОбластьСтрока.Параметры.Количество 		= Элемент.КоличествоСОтходом;
...
Функция ПечатьЗаказ(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	СпецификацияСписокНоменклатуры.Ссылка.Контрагент.ПолноеНаименование КАК ПолноеНаименование,
...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	"ВЫБРАТЬ
	|	Спецификация.Ссылка,
	|	Спецификация.Подразделение.Организация.ПолноеНаименование КАК ПолноеНаименование,
	|	Спецификация.Подразделение.Организация.ЮридическийАдрес КАК ЮридическийАдрес,
	|	ВЫБОР
...
Функция ПечатьИзделиеПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СпецификацияСписокИзделийПоКаталогу.ШиринаИзделия,
	...
	|		ПО СпецификацияСписокИзделийПоКаталогу.Изделие.Ссылка = ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВладелецФайла.Ссылка
	|ГДЕ
	...
	Пока Выборка.Следующий() Цикл 
		...
		Пока ВыборкаПоИзделиям.Следующий() Цикл
			...
			ОбластьТело.Параметры.Ширина = ВыборкаПоИзделиям.ШиринаИзделия;
			ОбластьТело.Параметры.Высота = ВыборкаПоИзделиям.ВысотаИзделия;
			ОбластьТело.Параметры.Глубина = ВыборкаПоИзделиям.ГлубинаИзделия;
			ОбластьТело.Параметры.НомерДетали 	= ВыборкаПоИзделиям.НомерСтроки;
			ОбластьТело.Параметры.Наименование = ВыборкаПоИзделиям.Наименование;
			ОбластьТело.Параметры.Комментарий = ВыборкаПоИзделиям.Комментарий;
			...
			Если НЕ ТабДок.ПроверитьВывод(ОбластьТело) Тогда
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИзделиеПоКаталогу") Тогда
		...
		ПодготовитьПечатнуюФорму("ИзделиеПоКаталогу", "ИзделиеПоКаталогу", "Документ.Спецификация.ИзделиеПоКаталогу",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СтеклянныеФасады") Тогда
		...
		ПодготовитьПечатнуюФорму("СтеклянныеФасады", "СтеклянныеФасады", "Документ.Спецификация.СтеклянныеФасады",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктГотовности") Тогда
		...
		ПодготовитьПечатнуюФорму("АктГотовности", "АктГотовности", "Документ.Спецификация.АктГотовности",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
...
// Получить коэффициент норматива 
//
// Параметры:
...
//   Коэффициент 
//
Функция ПолучитьСтруктуруНорматива(Объект, Подразделение, ДатаРазмещения, ДатаИзготовления, ЗарплатаЦеха, СуммаСрочности, РежимЗаписи) Экспорт
	
	Структура = Новый Структура;
	РазницаДней = 2;
	ЗарплатаЦеха = ЗарплатаЦеха - СуммаСрочности;
	
	Если РежимЗаписи = РежимЗаписиДокумента.Запись Тогда
		Регистратор = Объект.Ссылка;
	Иначе
		Регистратор = Документы.Спецификация.ПустаяСсылка();
	КонецЕсли;
	
	Если ДеньНедели(ДатаРазмещения) >= 5 Тогда
		РазницаДней = 3;
	КонецЕсли;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ЕСТЬNULL(Оборот.СуммаОборот, 0) КАК СуммаОборот,
	|	ПлановыйЛимит.НормативныйМаксимум,
	|	ПлановыйЛимит.Норматив,
	|	ПлановыйЛимит.КоэффициентСверхнорматива КАК Коэффициент,
	|	ВЫБОР
	|		КОГДА РАЗНОСТЬДАТ(&Дата, &Период, ДЕНЬ) <= &РазницаДней
	|				ИЛИ ЕСТЬNULL(Оборот.СуммаОборот, 0) + &ЗарплатаЦеха >= ПлановыйЛимит.Норматив
	|					И ЕСТЬNULL(Оборот.СуммаОборот, 0) + &ЗарплатаЦеха <= ПлановыйЛимит.НормативныйМаксимум
	|			ТОГДА ИСТИНА
	|		КОГДА ЕСТЬNULL(Оборот.СуммаОборот, 0) + &ЗарплатаЦеха > ПлановыйЛимит.НормативныйМаксимум
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Срочность
	|ИЗ
	...
	Запрос.УстановитьПараметр("ЗарплатаЦеха", ЗарплатаЦеха);
	Запрос.УстановитьПараметр("Дата", ДатаРазмещения);
	Запрос.УстановитьПараметр("РазницаДней", РазницаДней);
	Запрос.УстановитьПараметр("Регистратор", Регистратор);
	Запрос.УстановитьПараметр("Период", ДатаИзготовления);
	Запрос.УстановитьПараметр("Подразделение", Подразделение);
	
	Выборка = Запрос.Выполнить().Выбрать();
	Выборка.Следующий();
	
	Структура.Вставить("ОборотЗаДень", Выборка.СуммаОборот);
	...
	Структура.Вставить("Срочность", Выборка.Срочность);
	
	Возврат Структура;	
	
КонецФункции // ПроверитьНорматив()
...
Функция ПолучитьСтоимостьУслуг(СписокНоменклатуры) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(СписокНоменклатуры.ЗарплатаЦеха) КАК ЗарплатаЦеха,
	|	СУММА(СписокНоменклатуры.РозничнаяСтоимость) КАК РозничнаяСтоимость
	|ИЗ
	|	СписокНоменклатуры КАК СписокНоменклатуры
	|ГДЕ
	...
	Выборка.Следующий();
	Структура.Вставить("ЗарплатаЦеха", ?(ЗначениеЗаполнено(Выборка.ЗарплатаЦеха), Выборка.ЗарплатаЦеха, 0));
