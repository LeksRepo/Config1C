...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Пуфик = ДанныеДетали.МатериалСтрока = "Пуфик";
			...
			Если ДанныеДетали.ЭтоФасад Тогда
				...
				Если ДанныеДетали.ЭтоСтекло Тогда
			...
			ИначеЕсли (МатериалЛДСП 
				  ИЛИ ЕстьДВП 
				  ИЛИ ЕстьМДФ 
				  ИЛИ ЕстьАГТПанель 
				  ИЛИ ЕстьСтолешница 
				  ИЛИ ЕстьПристенок) И (НЕ ДанныеДетали.ЭтоФасад) И НЕ Пуфик Тогда
			...
			Если СписокМатериалыКороба.Количество() > 0 Тогда
				...
				Для Каждого ЭлементКороба Из СписокМатериалыКороба Цикл
					...
					Если МассивДеталиКороба.Количество() = 1 Тогда
						...
						СтруктураНадписи.Вставить("Материал", ТекущаяДеталь.ВидДетали.МатериалСтрока);
						СтруктураНадписи.Вставить("РадиусФасада", ТекущаяДеталь.РадиусФасада);
						СтруктураНадписи.Вставить("ДетальДляСклеивания", ТекущаяДеталь.ВидДетали.Клееная);
						СтруктураНадписи.Вставить("НеТорцевать", ТекущаяДеталь.НеТорцевать);
						СтруктураНадписи.Вставить("ЭтоПуфик", ТекущаяДеталь.ВидДетали.МатериалСтрока="Пуфик");
...
Функция ПолучитьДанныеДетали(ТипДетали)
	...
	Стр.Вставить("ВысотаТекстура", ТипДетали.ШиринаТекстура);
