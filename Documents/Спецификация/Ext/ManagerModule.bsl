...
Функция ПолучитьЗарплатуЦеха(ТаблицаЗначений) Экспорт
	...
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ТЗ", ТаблицаЗначений.Выгрузить());
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	втТаб КАК Таб2
	|ГДЕ
	|	Таб2.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга)
...
Процедура ПечатьАктДоставкаСборка(ТабДок, Ссылка) Экспорт
	...
	|	докСпецификация.НазваниеИзделия КАК Изделие,
