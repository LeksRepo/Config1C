...
Функция ПечатьЗаказБрендДилера(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	"""" + СпецификацияСписокНоменклатуры.Ссылка.Контрагент.Фамилия + "" "" + СпецификацияСписокНоменклатуры.Ссылка.Контрагент.Имя + "" "" + СпецификацияСписокНоменклатуры.Ссылка.Контрагент.Отчество КАК ПолноеНаименование,
	|	СпецификацияСписокНоменклатуры.Ссылка.ДатаОтгрузки КАК ДатаОтгрузки,
	|	СпецификацияСписокНоменклатуры.ВнутренняяСтоимость
	|ИЗ
	|	Документ.Спецификация.СписокНоменклатуры КАК СпецификацияСписокНоменклатуры
	|ГДЕ
	|	СпецификацияСписокНоменклатуры.Ссылка В(&МассивДокументов)
...
Функция ПечатьРаскройСпецификации(СтруктураОбъектов, ОбъектыПечати) Экспорт
	...
	МассивПараметров = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(СтрЗаменить(СтруктураОбъектов.СтрокаРаскроя,"save☻", ""), "☺");
	Спецификация = СтруктураОбъектов.МассивДокументов[0];
...
Процедура Печать(СтруктураОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	 ...
	 Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "Спецификация") Тогда
		...
		СтруктураОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
...
Процедура ПодготовитьПечатнуюФорму(Знач ИмяМакета, ПредставлениеМакета, ПолныйПутьКМакету = "", СтруктураОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	
	Если ТипЗнч(СтруктураОбъектов) = Тип("Структура") Тогда	
		МассивОбъектов =  СтруктураОбъектов.МассивДокументов;
	Иначе	
		МассивОбъектов = СтруктураОбъектов;
	КонецЕсли;
	...
	ИначеЕсли ИмяМакета = "ЗаказБрендДилера" Тогда
		...
		ТабДок = ПечатьЗаказБрендДилера(МассивОбъектов, ОбъектыПечати);
	...
	ИначеЕсли ИмяМакета = "Раскрой" Тогда
		...
		ТабДок = ПечатьРаскройСпецификации(СтруктураОбъектов, ОбъектыПечати);
