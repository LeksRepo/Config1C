...
Процедура ПодготовитьПечатнуюФорму(Знач ИмяМакета, ПредставлениеМакета, ПолныйПутьКМакету = "", МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	ИначеЕсли ИмяМакета = "ЭскизнаяЗаявка" Тогда
		...
		ТабДок = ПечатьЭскизнаяЗаявка(МассивОбъектов, ОбъектыПечати);
...
Функция ПечатьЭскизнаяЗаявка(Выборка, ОбъектыПечати)
	...
	Макет = Документы.Спецификация.ПолучитьМакет("ЭскизнаяЗаявка");
...
Функция ПечатьЗамерныйЛист(Выборка, ОбъектыПечати)
	...
	Макет = Документы.Спецификация.ПолучитьМакет("ПечатьКартинки");
	ОбластьТело = Макет.ПолучитьОбласть("Тело");
	...
	Пока ВыборкаФайлов.Следующий() Цикл
		...
		Если Расширение = "jpg" 
        	...
        	Рисунок.Картинка = Новый Картинка(Картинка);
       	    Рисунок.РазмерКартинки = РазмерКартинки.Пропорционально;
            Рисунок.Расположить(ТабДок.Область(ОбластьТело));
