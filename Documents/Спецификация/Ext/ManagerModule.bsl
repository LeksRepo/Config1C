...
Функция ПолучитьТаблицуМатериаловЯщики(ТаблицаДеталей, Документ, ОбщаяНоменклатура) Экспорт
	...
	ИзделиеДетали = (Документ.Изделие = Справочники.Изделия.Детали);
...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|		ИНАЧЕ ВЫБОР
	|				КОГДА Спец.Изделие = ЗНАЧЕНИЕ(Справочник.Изделия.Переделка)
	|					ТОГДА ""ПЕРЕДЕЛКА""
	|				ИНАЧЕ ВЫБОР
	...
	Если ТипЗнч(Договор) = Тип("ДокументСсылка.Договор") И Договор <> Документы.Договор.ПустаяСсылка() Тогда
		...
		ОбластьШапка.Параметры.НомерДоговор = Выборка.Договор.Номер;
	КонецЕсли;
	...
	Если НЕ Результат.Пустой() Тогда
		...
		Для Каждого Строка ИЗ ТЗ Цикл
			...
			Попытка
				...
				Выполнить("ЛинейкаВерхНизЗначение = " + Строка.ЛинейкаВерхНизФормула);
				Выполнить("ЛинейкаЛевоПравоЗначение = " + Строка.ЛинейкаЛевоПравоФормула);
	
			Исключение
				Сообщить("Ошибка при формировании формулы для присадок");
			КонецПопытки;
			...
			Если НЕ ПерваяСтрока
				 ...
				 ИЛИ ПрошлыеДанные.СверлоЛево <> Строка.СверлоЛево
				 ИЛИ ПрошлыеДанные.СверлоПраво <> Строка.СверлоПраво
			...
			Если ПерваяСтрока Тогда
				...
				ОбластьСтрока.Параметры.ЛинейкаВерхНизЗначение = ЛинейкаВерхНизЗначение;
				ОбластьСтрока.Параметры.ЛинейкаЛевоПравоЗначение = ЛинейкаЛевоПравоЗначение;
			...
			Иначе
				...
				Если ПрошлыеДанные.СверлоЛево <> Строка.СверлоЛево Тогда
					ОбластьСтрока.Параметры.СверлоЛево = Строка.СверлоЛево; 	
				КонецЕсли;
				
				Если ПрошлыеДанные.СверлоПраво <> Строка.СверлоПраво Тогда
					ОбластьСтрока.Параметры.СверлоПраво = Строка.СверлоПраво; 	
				КонецЕсли;
			...
			ОбластьСтрока.Параметры.Развернуть = Строка.Развернуть;
			...
			ПрошлыеДанные.ЛинейкаВерхНиз = Строка.ЛинейкаВерхНиз;
		 	...
		 	ПрошлыеДанные.СверлоЛево = Строка.СверлоЛево;
		    ПрошлыеДанные.СверлоПраво = Строка.СверлоПраво;
...
Функция ПроверитьРедактированностьКаталожнойДетали(Данные)
	...
	Попытка
		Выполнить("НоваяКоличество = " + Данные.КаталогКоличество);
	Исключение
		НоваяКоличество = Данные.Количество;
	КонецПопытки;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПрисадкиПоКаталогу") Тогда
		...
		ПодготовитьПечатнуюФорму("ПрисадкиПоКаталогу", "                       Присадки", "Документ.Спецификация.ПрисадкиПоКаталогу",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
