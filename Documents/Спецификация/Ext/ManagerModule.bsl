...
Функция ПечатьИзделиеПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл 
		...
		Пока ВыборкаПоИзделиям.Следующий() Цикл
			...
			МассивДеталиКороба = СписокДеталей.НайтиСтроки(Новый Структура("НомерИзделия", ВыборкаПоИзделиям.НомерКороба)); 
			...
			Если МассивДеталиКороба.Количество() > 0 Тогда
				...
				Для Каждого ДетальКороба Из МассивДеталиКороба Цикл
					...
					ОбластьСтрокаДетали.Параметры.ВысотаДетали = ДетальКороба.ВысотаДетали; 
					ОбластьСтрокаДетали.Параметры.ШиринаДетали = ДетальКороба.ШиринаДетали;
			...
			//ТабДокФурнитураИДеталей.Присоединить(ТабДокДеталей);
			
			//ТабДок.Вывести(ТабДокДеталей);
