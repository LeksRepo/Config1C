...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		Если Спецификация.СписокМатериаловЗаказчика.Количество() > 0 Тогда
			...
			Для Каждого МатЗаказчика Из Спецификация.СписокМатериаловЗаказчика Цикл
...
Функция ПечатьМатериалЗаказчика(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Спецификация.ДатаИзготовления КАК ДатаИзготовления,
	|	Спецификация.Номер КАК СпецификацияНомер,
	|	Спецификация.Контрагент КАК Контрагент,
	|	Спецификация.СписокМатериаловЗаказчика.(
	|		Ссылка,
	|		НомерСтроки,
	|		Количество,
	|		Комментарий,
	...
	Пока Выборка.Следующий() Цикл
		...
		ВыборкаСписокНоменклатуры = Выборка.СписокМатериаловЗаказчика.Выбрать();
		
		Если ВыборкаСписокНоменклатуры.Количество() > 0 Тогда
...
Функция ПечатьОбточка(Выборка, ОбъектыПечати)
	...
	ОбластьМакет = Макет.ПолучитьОбласть("Макет");
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Маты.Ссылка Как Спецификация,
	|	Маты.Номенклатура Как Номенклатура,
	|	Маты.Количество Как Количество,
	|	Маты.ВысотаДетали Как Высота,
	|	Маты.ШиринаДетали Как Ширина,
	|	Маты.НомерСтроки Как Номер,
	|	Маты.РадиусЛевоВерх Как R1,
	|	Маты.РадиусПравоВерх Как R2,
	|	Маты.РадиусПравоНиз Как R3,
	|	Маты.РадиусЛевоНиз Как R4,
	|	Маты.Срез Как Срез
	|ИЗ
	...
	Пока ВыборкаДеталей.Следующий() Цикл
		...
		ОбластьМакет.Параметры.Заполнить(ВыборкаДеталей);
		
		Если ВыборкаДеталей.R1 > 0 Тогда
			 ОбластьМакет.Параметры.R1 = "R1: "+ВыборкаДеталей.R1;	
		КонецЕсли;
		 
		Если ВыборкаДеталей.R2 > 0 Тогда
			 ОбластьМакет.Параметры.R2 = "R2: "+ВыборкаДеталей.R2;	
		КонецЕсли; 
		 
		Если ВыборкаДеталей.R3 > 0 Тогда
			
			Если ВыборкаДеталей.Срез Тогда
				ОбластьМакет.Параметры.R3 = "S3: "+ВыборкаДеталей.R3;
			Иначе
				ОбластьМакет.Параметры.R3 = "R3: "+ВыборкаДеталей.R3;
			КонецЕсли;

		КонецЕсли;
		
		Если ВыборкаДеталей.R4 > 0 Тогда
			 ОбластьМакет.Параметры.R4 = "R4: "+ВыборкаДеталей.R4;	
		КонецЕсли;
