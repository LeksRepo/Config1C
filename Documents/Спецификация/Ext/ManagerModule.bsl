...
Функция ПечатьСчетНаОплатуПокупателю(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = 	
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПРЕДСТАВЛЕНИЕ(ВТ_СписокДокуметов.Ссылка) КАК СсылкаПредставление
		|ИЗ
		|	ВТ_СписокДокуметов КАК ВТ_СписокДокуметов
		|ГДЕ
	...
	Если Не СписокОтвергнутых.Пустой() Тогда
			...
			Пока ВыборкаИсключенных.Следующий() Цикл
				ТекстОшибки = "Счет на оплату выписывается только юридическим лицам! Не печается: " + ВыборкаИсключенных.СсылкаПредставление;
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Спецификация.Ссылка,
		|	ПРЕДСТАВЛЕНИЕ(Спецификация.Ссылка) КАК СсылкаПредставление,
		|	Спецификация.Контрагент КАК Контрагент
		|ПОМЕСТИТЬ ИсключаемыеДокументы
		|ИЗ
		|	Документ.Спецификация КАК Спецификация
		|ГДЕ
		|	Спецификация.Ссылка В
		|			(ВЫБРАТЬ
		|				ВТ.Ссылка
		|			ИЗ
		|				СписокДокументов КАК ВТ)
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПРЕДСТАВЛЕНИЕ(ИсключаемыеДокументы.Контрагент) КАК КонтрагентПредставление
		|ИЗ
		|	ИсключаемыеДокументы КАК ИсключаемыеДокументы
		|
		|СГРУППИРОВАТЬ ПО
		|	ИсключаемыеДокументы.Контрагент";
			
	
	РезультатПроверкиКонтрагентов = Запрос.Выполнить();
	
	Если НЕ РезультатПроверкиКонтрагентов.Пустой() Тогда
		
	        ВыборкаКонтрагентов = РезультатПроверкиКонтрагентов.Выбрать();
			Пока ВыборкаКонтрагентов.Следующий() Цикл
				ТекстОшибки = "Проверьте реквизиты контрагента: " + ВыборкаКонтрагентов.КонтрагентПредставление ;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
			КонецЦикла;
				
			Запрос.Текст = "Выбрать ПРЕДСТАВЛЕНИЕ(Док.Ссылка) КАК СсылкаПредставление  ИЗ ИсключаемыеДокументы КАК Док" ;
			ВыборкаИсключенныхдокументов = Запрос.Выполнить().Выбрать();
			Пока ВыборкаИсключенныхдокументов.Следующий() Цикл
				ТекстОшибки = "Не печается: " + ВыборкаИсключенныхдокументов.СсылкаПредставление ;
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
			КонецЦикла;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	НЕ СписокДокументов.Ссылка В
		|				(ВЫБРАТЬ
		|					ИсключаемыеДокументы.Ссылка
		|				ИЗ
		|					ИсключаемыеДокументы КАК ИсключаемыеДокументы)";
	
		ОсталосьДокументов = Запрос.Выполнить();
		
		Если  ОсталосьДокументов.Пустой() Тогда
			
			Отказ = Истина;
			ТекстОшибки = "Нет документов для печати!";
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстОшибки);
			Возврат Неопределено;
		...
		РезультатТовары =  РезультатЗапроса[1].Выгрузить();
		...
		Пока ВыборкаДокументов.Следующий() Цикл
			...
			ОбластьПодвалСчета.Параметры.ФИОРуководителя = ФизическиеЛица.ФамилияИнициалыФизЛица(ВыборкаДокументов.ФИОРуководителя);
