...
Функция ПолучитьТаблицуМатериаловЯщики(ТаблицаДеталей, Документ) Экспорт
	...
	Для Каждого Деталь Из ТаблицаДеталей Цикл
		...
		//Длина кромки
		ДлинаКромки = ?(ЗначениеЗаполнено(Деталь.ВыборМебельнойКромкиСверху),Деталь.ШиринаДетали,0) +
		?(ЗначениеЗаполнено(Деталь.ВыборМебельнойКромкиСнизу),Деталь.ШиринаДетали,0) +
		?(ЗначениеЗаполнено(Деталь.ВыборМебельнойКромкиСлева),Деталь.ВысотаДетали,0) +
		?(ЗначениеЗаполнено(Деталь.ВыборМебельнойКромкиСправа),Деталь.ВысотаДетали,0);
...
Функция ПодставитьЗначенияФормул(Формула, СписокФормул)
	
	РазложенаяФормула = Формула;
	
	Для Каждого Фор Из СписокФормул Цикл
		
		РазложенаяФормула = СтрЗаменить(РазложенаяФормула,"Фор."+Фор[0],Фор[1]); 
...
Функция РазложитьЯщикиНаДетали(Документ) Экспорт
	...
	Для Каждого Ящик Из Документ.СписокЯщики Цикл
		...
		Для Каждого ДетальСхема Из Схема.Детали Цикл
			...
			//Выполнить("НоваяДеталь.ВысотаДетали = " + ЛексКлиентСервер.ПодставитьОбъектВФормулуЯщика("Ящик", ДетальСхема.Высота, Формулы)); 
			//Выполнить("НоваяДеталь.ШиринаДетали = " + ЛексКлиентСервер.ПодставитьОбъектВФормулуЯщика("Ящик", ДетальСхема.Ширина, Формулы));
			//Выполнить("НоваяДеталь.Количество = " + ЛексКлиентСервер.ПодставитьОбъектВФормулуЯщика("Ящик", ДетальСхема.Количество, Формулы));
...
Функция ПолучитьПараметрыЯщика(СтрокаЯщика, ЕстьРебро) Экспорт
	...
	Если ВидЯщика = Перечисления.ВидыЯщика.Обычный И Проем >= 150 Тогда
		...
		СтрокаЯщика.ШиринаБоковойСтороны = Проем - ТолщинаНаправляющей - 32; // 32 это две толщины стенок материала. Пока только ЛДСП 16 мм.
		
		//Дно ящика меньше с каждой стороны на  1 мм.
		// Технический зазор.
		СтрокаЯщика.ДлинаДно = ГлубинаЯщика - 2;
		СтрокаЯщика.ШиринаДно = Проем - ТолщинаНаправляющей - 2;
...
Функция МатериалПодЗаказПроверен(СпецификацияСсылка, Ошибки) Экспорт
	...
	Результат = Ложь;
	Свойства = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СпецификацияСсылка, "ЕстьМатериалПодЗаказ, Проведен");
	
	Если Свойства.ЕстьМатериалПодЗаказ Тогда
...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		ОбластьМатериалЗаказчикаШапка = Макет.ПолучитьОбласть("МатериалЗаказчика");
		ОбластьМатериалЗаказчикаСтрока = Макет.ПолучитьОбласть("МатериалЗаказчикаСтрока");
		
		Если Спецификация.Срочный Тогда
			СтрокаСрочный = "СРОЧНЫЙ";
		Иначе
			СтрокаСрочный = "";
		КонецЕсли;
		...
		Пока Выборка.Следующий() Цикл
			...
			МатериалЛДСП = (Материал = "10 ЛДСП" ИЛИ Материал = "16 ЛДСП" ИЛИ Материал = "10 ЛДСП+10 ЛДСП" ИЛИ Материал = "16 ЛДСП+10 ЛДСП" ИЛИ Материал = "16 ЛДСП+16 ЛДСП" ИЛИ Материал = "22 ЛДСП" ИЛИ Материал = "26 ЛДСП");
			...
			ИначеЕсли МатериалЛДСП ИЛИ Материал = "ДВП" ИЛИ Материал = "МДФ" ИЛИ Материал = "АГТПанель" ИЛИ Материал = "Столешница" ИЛИ Материал = "Пристенок" Тогда
				
				ОбластьШапка = Макет.ПолучитьОбласть("ШапкаЛДСП");
				ОбластьШапка.Параметры.ВидДетали = Материал;
		...
		ТабДок.ВерхнийКолонтитул.ТекстВЦентре = "Комплект ""Спецификация"". Страница [&НомерСтраницы] из [&СтраницВсего]. " + "Спецификация № " + Спецификация.Номер;
			...
			Если СписокМатериалыКороба.Количество() > 0 Тогда
				...
				Для Каждого ЭлементКороба Из СписокМатериалыКороба Цикл
					...
					Если МассивДеталиКороба.Количество() = 1 Тогда
						...
						СтруктураНадписи.Вставить("ЭтоПуфик", ТекущаяДеталь.Материал="Пуфик" ИЛИ ТекущаяДеталь.Материал="Пуф");
						...
						// Так это не работает
						// ЗначениеЗаполнено возвращает булево
