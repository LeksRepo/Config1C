		   ...
		   Для Каждого ДетальСхема Из Схема.Детали Цикл
			   ...
			   Если ДетальСхема.Тип = Перечисления.ВидыДеталейЯщиков.Корпус Тогда
				...
				Если ЗначениеЗаполнено(Кромка) Тогда					
					...
					Если ДетальСхема.КромкаВерх Тогда НоваяДеталь.ВыборМебельнойКромкиСверху = Кромка; КонецЕсли;
					Если ДетальСхема.КромкаНиз Тогда НоваяДеталь.ВыборМебельнойКромкиСнизу = Кромка; КонецЕсли;
					Если ДетальСхема.КромкаЛево Тогда НоваяДеталь.ВыборМебельнойКромкиСлева = Кромка; КонецЕсли;
					Если ДетальСхема.КромкаПраво Тогда НоваяДеталь.ВыборМебельнойКромкиСправа = Кромка; КонецЕсли;
...
Функция ПечатьШкафПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	Справочник.ВидыИзделийПоКаталогу КАК ВидыИзделийПоКаталогу
	|ГДЕ
