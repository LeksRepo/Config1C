...
Процедура ПечатьСпецификации_ВывестиШапку(Спецификация, ТабДок, НазваниеЗоны)
	...
	НомерВНаряде = "";
	Если РезультатНаряд.Следующий() Тогда
...
Процедура ПечатьСпецификации_СтеклянныеФасады(Спецификация, ТабДок)
	...
	ОбластьСтрокаЯчейка = Макет.ПолучитьОбласть("СтрокаФасадСтекляный");
...
Процедура ПечатьСпецификации_Двери(Спецификация, ТабДок)
	...
	Если Выборка.Количество() > 0 Тогда
		...
		Макет = Документы.Спецификация.ПолучитьМакет("Спецификация");
		ОбластьПустаяСтрока = Макет.ПолучитьОбласть("ПустаяСтрока");
		ОбластьЗаголовокДвери = Макет.ПолучитьОбласть("ДвериШапка");
		
		ОбластьШапкаТаблицы = Макет.ПолучитьОбласть("ДвериШапкаТаблицы");
		ОбластьСтрока = Макет.ПолучитьОбласть("СтрокаДверь");
		ОбластьСтрокаЖирный = Макет.ПолучитьОбласть("СтрокаДверьЖирный");
		
		ТабДок.Вывести(ОбластьПустаяСтрока);
		ТабДок.Вывести(ОбластьЗаголовокДвери);
							
		Пока Выборка.Следующий() Цикл
			
			НомерДвери = "№" + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Выборка.Двери.Код);
			
			ТабДок.Вывести(ОбластьПустаяСтрока);
			
			ОбластьШапкаТаблицы.Параметры.Заполнить(Выборка);
			ОбластьШапкаТаблицы.Параметры.НомерДвери = НомерДвери;
			...
			Если ЗначениеЗаполнено(Выборка.Двери.СтрокаДляФлэш) Тогда 
				...
				ОбластьШапкаТаблицы.Параметры.Профиль1 = "Вертикальный профиль";
				ОбластьШапкаТаблицы.Параметры.Ширина1 = ДанныеПрофиля[0];
				ОбластьШапкаТаблицы.Параметры.Паз1 = ДанныеПрофиля[1];
				
				ОбластьШапкаТаблицы.Параметры.Профиль2 = "Рамка верхняя";
				ОбластьШапкаТаблицы.Параметры.Ширина2 = ДанныеПрофиля[2];
				ОбластьШапкаТаблицы.Параметры.Паз2 = ДанныеПрофиля[3];

				ОбластьШапкаТаблицы.Параметры.Профиль3 = "Рамка нижняя";
				ОбластьШапкаТаблицы.Параметры.Ширина3 = ДанныеПрофиля[4];
				ОбластьШапкаТаблицы.Параметры.Паз3 = ДанныеПрофиля[5];
				
				ОбластьШапкаТаблицы.Параметры.Профиль4 = "Рамка средняя";
				ОбластьШапкаТаблицы.Параметры.Ширина4 = ДанныеПрофиля[6];
				ОбластьШапкаТаблицы.Параметры.Паз4 = ДанныеПрофиля[7];
...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати, НазваниеЗоны) Экспорт
	...
	ИначеЕсли НазваниеЗоны = "Стекольная зона" Тогда
		...
		ПечатьСпецификации_Материалы(Спецификация, ТабДок, НазваниеЗоны);
		ПечатьСпецификации_СтеклянныеФасады(Спецификация, ТабДок);
		
		//RonEXI: Перенесено во флэшку
