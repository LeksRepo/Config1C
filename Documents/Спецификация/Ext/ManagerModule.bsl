...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Для каждого Спецификация Из МассивДокументов Цикл
		...
		|	СпецификацияСписокМатериалы.ПоложениеРучки КАК ПоложениеРучки,
		|	СпецификацияСписокМатериалы.РасположениеПазов КАК РасложениеПазов,
		|	ВЫБОР
			...
			Пока ВыборкаПоМатериалам.Следующий() Цикл
				...
				Иначе // тут как было раньше
					...
					Если НЕ (МатериалЛДСП И ЗначениеЗаполнено(ВыборкаПоМатериалам.НомерИзделия) И ВыборкаПоМатериалам.ВидИзделия <> Перечисления.ВидыИзделий.ШкафКупе) Тогда
						...
						//Как только удалятся реквизиты "положение руяку" и "расположение петель" переделать
						Если ЗначениеЗаполнено(ВыборкаПоМатериалам.ПоложениеРучки) Тогда
							
							ОбластьСтрока.Параметры.ПоложениеРучки = ВыборкаПоМатериалам.ПоложениеРучки;
							
						Иначе
							
							ОбластьСтрока.Параметры.ПоложениеРучки = ВыборкаПоМатериалам.РасположениеРучки;
							
						КонецЕсли;
						
						Если ЗначениеЗаполнено(ВыборкаПоМатериалам.РасложениеПазов) Тогда
							
							ОбластьСтрока.Параметры.ПоложениеРучки = ВыборкаПоМатериалам.РасложениеПазов;
							
						Иначе
							
							ОбластьСтрока.Параметры.ПоложениеРучки = ВыборкаПоМатериалам.ПоложениеПазов;
...
Функция ПечатьСтеклянныеФасады(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	СпецификацияСписокМатериалы.Петли,
	|	СпецификацияСписокМатериалы.ПоложениеРучки КАК ПоложениеРучки,
	|	СпецификацияСписокМатериалы.РасположениеПазов КАК РасположениеПазов,
	...
	Пока Выборка.Следующий() Цикл
		...
		//Поменять, как уберуться реквизиты "ПоложениеРучки" и "РасположениеПазов"
		
		ПоложениеРучки = ВРег(Выборка.ПоложениеРучки);
		
		Если ЗначениеЗаполнено(ПоложениеРучки) Тогда 
		...
		Иначе
			...
			ПоложениеРучки = Выборка.РасположениеРучки;
