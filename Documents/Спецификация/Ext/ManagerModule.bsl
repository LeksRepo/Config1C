...
Функция ПолучитьКоличествоДнейНаИзготовление(Спецификация) Экспорт
	...
	Если Таблица.Количество()>0 Тогда
		...
		|	ЕСТЬNULL(Настройки.ДнейНаИзготовление,0) КАК ДнейНаИзготовление
		|ИЗ
		|	втТаб КАК Список
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураПодразделений.СрезПоследних(, Подразделение = &Подразделение) КАК Настройки
		|		ПО (Настройки.Номенклатура = Список.Номенклатура)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	Для Каждого Ящик Из Документ.СписокЯщики Цикл
		...
		Если ЗначениеЗаполнено(Ящик.РучкаНоменклатура) И (Ящик.КоличествоРучек > 0) Тогда
			...
			Если ЗначениеЗаполнено(Ящик.НомерИзделия) Тогда
				...
				Если ИзделиеШкаф ИЛИ (ИзделиеКорпуснаяМебель И Короб.Сборка) Тогда
					Запись.МестоОбработки = "Цех";
				Иначе
					Запись.МестоОбработки = "Склад";
				КонецЕсли;
		...
		Если Ящик.СхемаЯщика.Комплектация.Количество() > 0 Тогда
			...
			Для Каждого Стр Из ТабКомп Цикл
				...
				НомГруппа = ЛексСервер.ЗначениеРеквизитаОбъекта(Стр.Номенклатура, "НоменклатурнаяГруппа");
				Запись = ТЗ.Добавить();
				Запись.НомерИзделия = Ящик.НомерИзделия;
...
Функция СформироватьСтрокуМонтажСумма(СпецификацияСсылка)
	...
	СписокУслуг.Добавить(Справочники.Номенклатура.ТарифЗаСборку);
	СписокУслуг.Добавить(Справочники.Номенклатура.ТарифЗаСборкуКухни);
...
Процедура ПечатьСпецификации_Материалы(Спецификация, ТабДок, НазваниеЗоны)
	...
	|	ВЫБОР
	|		КОГДА СписокДеталей.НоменклатураДляСклеивания.КраткоеНаименование > """"
	|			ТОГДА СписокДеталей.НоменклатураДляСклеивания.КраткоеНаименование
	|		ИНАЧЕ СписокДеталей.НоменклатураДляСклеивания
	|	КОНЕЦ КАК Ручка,
	...
	|	И ВЫБОР
	|			КОГДА СписокПоКаталогу.Ссылка <> ЗНАЧЕНИЕ(Документ.Спецификация.ПустаяСсылка)
	|				ТОГДА СписокПоКаталогу.Ссылка = &Спецификация
	|			ИНАЧЕ ИСТИНА
	|		КОНЕЦ
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаПоМатериалам.Следующий() Цикл
			...
			Иначе
				...
				Если НЕ (МатериалЛДСП И ЗначениеЗаполнено(ВыборкаПоМатериалам.НомерИзделия) И ВыборкаПоМатериалам.Сборка И ВыборкаПоМатериалам.ВидИзделия <> Перечисления.ВидыИзделий.ШкафКупе) Тогда
...
Процедура ПечатьСпецификации_Фасады(Спецификация, ТабДок, НазваниеЗоны)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СборныйРамочныйФасад Тогда
			...
			ОбластьСтрокаЯчейка.Параметры.ВысотаДетали = Выборка.ВысотаДетали + Выборка.ШиринаСборнойРамки * 2 - 24;
			ОбластьСтрокаЯчейка.Параметры.ШиринаДетали = Выборка.ШиринаДетали + Выборка.ШиринаСборнойРамки * 2 - 24;
...
Функция ПечатьСпецификации(МассивДокументов, ОбъектыПечати, НазваниеЗоны) Экспорт
	...
	ИначеЕсли НазваниеЗоны = "Зона ОТК" Тогда 
		//RonEXI: Печать отключена в команде комплект документов
		ПечатьСпецификации_Материалы(Спецификация, ТабДок, НазваниеЗоны);
		ПечатьСпецификации_Фасады(Спецификация, ТабДок, НазваниеЗоны);
...
Функция ПечатьАктГотовности(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	И ЕСТЬNULL(НоменклатураПодразделений.ОкруглятьДоЛистов, ИСТИНА)
	|	И &Дилерский
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Функция ПечатьИзделиеПоКаталогу(МассивДокументов, ОбъектыПечати) Экспорт
	...
	ОбластьШапкаЯщики = Макет.ПолучитьОбласть("ШапкаЯщики");
	ОбластьСтрокаЯщики = Макет.ПолучитьОбласть("СтрокаЯщики");
	...
	Пока Выборка.Следующий() Цикл
		...
		НомерСпец = СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(Выборка.Ссылка.Номер, "0", "Слева");
		...
		Пока ВыборкаПоИзделиям.Следующий() Цикл
			...
			//	
			//	СписокДеталей = ВыборкаПоИзделиям.Изделие.СписокДеталей;
			...
			Если СписокЯщиков.Количество()>0 Тогда
				...
				Если МассивЯщиков.Количество()>0 Тогда
					...
					ТабДокЯщики = Новый ТабличныйДокумент;
					ТабДокЯщики.Вывести(ОбластьШапкаЯщики);
				
					Для Каждого Ящ Из МассивЯщиков Цикл
						
						ОбластьСтрокаЯщики.Параметры.НомерЯщика = Ящ.НомерЯщикаИзделия;
						ОбластьСтрокаЯщики.Параметры.Направляющие = Ящ.НаправляющиеНоменклатура;

						ТабДокЯщики.Вывести(ОбластьСтрокаЯщики);
						
					КонецЦикла;
					
					ТабДок.Присоединить(ТабДокЯщики);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КриволинейныеДеталиФРС") Тогда
		...
		ПодготовитьПечатнуюФорму("КриволинейныеДеталиФРС", "                       Криволинейные детали", "Документ.Спецификация.Раскрой",   
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
		
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КриволинейныеДеталиСтеколка") Тогда
		
		ПодготовитьПечатнуюФорму("КриволинейныеДеталиСтеколка", "                       Криволинейные детали", "Документ.Спецификация.Раскрой",   
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетНаОплатуПокупателю") Тогда
		...
		ПодготовитьПечатнуюФорму("СчетНаОплатуПокупателю", "СчетНаОплатуПокупателю", "Документ.Спецификация.СчетНаОплатуПокупателю",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
...
Функция ПечатьКомплектации(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	Комплектация.Спецификация.ПлощадьСборкиИзделия КАК ПлощадьСборкиИзделия
	|ИЗ
	|	Документ.Комплектация КАК Комплектация
	|ГДЕ
	|	НЕ Комплектация.ПометкаУдаления
	|	И Комплектация.Спецификация В(&МассивДокументов)";
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Спецификация.СписокМатериаловПодЗаказ.Количество() > 0 Тогда
			...
			ОбластьПодЗаказШапка = Макет.ПолучитьОбласть("ПодЗаказШапка");
			ТабДок.Вывести(ОбластьПодЗаказШапка);
			
			Для каждого СтрокаПодЗаказ Из Спецификация.СписокМатериаловПодЗаказ Цикл
				ОбластьПодЗаказСтрока = Макет.ПолучитьОбласть("ПодЗаказСтрока");
				ОбластьПодЗаказСтрока.Параметры.Заполнить(СтрокаПодЗаказ);
				ОбластьПодЗаказСтрока.Параметры.ЕдиницаИзмерения = СтрокаПодЗаказ.Номенклатура.ЕдиницаИзмерения; 
				ТабДок.Вывести(ОбластьПодЗаказСтрока);
			КонецЦикла;				
...
Функция ПечатьСчетНаОплатуПокупателю(МассивДокументов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	Спецификация.Ссылка,
		|	Спецификация.Контрагент.ЮридическоеЛицо КАК НаЮрЛицо
		|ПОМЕСТИТЬ ВТ_СписокДокуметов
		|ИЗ
		|	Документ.Спецификация КАК Спецификация
		|ГДЕ
		|	Спецификация.Ссылка В (&МассивДокументов)
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВТ_СписокДокуметов.Ссылка
		|ПОМЕСТИТЬ СписокДокументов
		|ИЗ
		|	ВТ_СписокДокуметов КАК ВТ_СписокДокуметов
		|ГДЕ
		|	ВТ_СписокДокуметов.НаЮрЛицо
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВТ_СписокДокуметов КАК ВТ_СписокДокуметов
		|ГДЕ
		|	НЕ ВТ_СписокДокуметов.НаЮрЛицо";
	
	Запрос.УстановитьПараметр("МассивДокументов", МассивДокументов);
	
	СписокОтвергнутых = Запрос.Выполнить();
	Если Не СписокОтвергнутых.Пустой() Тогда
		
			ВыборкаИсключенных = СписокОтвергнутых.Выбрать();
			Ошибки = Неопределено;
			ПолеОшибки = "[%1]";
			ТекстГруппыОшибок = "Документ %1 выписан не на юридичесое лицо.";
			ТекстОшибки = "Счет на оплату выписывается только юридическим лицам!";
			
			Пока ВыборкаИсключенных.Следующий() Цикл
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, ПолеОшибки, ТекстОшибки, 1,ВыборкаИсключенных.НомерСтроки - 1, ТекстГруппыОшибок);
			КонецЦикла;
	...
	Запрос.Текст = 
		...
		|	И ((ВЫРАЗИТЬ(Спецификация.Контрагент.ЮридическийАдрес КАК СТРОКА(100))) = &ПустаяСтрока
		|			ИЛИ Спецификация.Контрагент.ИНН = &ПустаяСтрока
		|			ИЛИ Спецификация.Контрагент.КПП = &ПустаяСтрока)";
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Спецификация.Представление КАК ПредставлениеДокумента,
	|	Спецификация.Организация.ПолноеНаименование КАК ПредставлениеПолучателяПлатежа,
	|	Спецификация.Организация.ИНН КАК ИННПолучателяПлатежа,
	|	Спецификация.Организация.КПП КАК КПППолучателяПлатежа,
	|	Спецификация.Организация.Банк КАК БанкПолучателяПлатежа,
	|	Спецификация.Организация.БИК КАК БИКБанкаПолучателяПлатежа,
	|	Спецификация.Организация.КорреспондирующийСчет КАК СчетБанкаПолучателяПлатежа,
	|	Спецификация.Организация.РасчетныйСчет КАК СчетПолучателяПлатежа,
	|	Спецификация.Дата,
	|	Спецификация.СуммаДокумента,
	|	Спецификация.Номер,
	|	Спецификация.Организация.ПолноеНаименование КАК ПоставщикПолноеНаименование,
	|	Спецификация.Организация.ИНН КАК ИННПоставщика,
	|	Спецификация.Организация.КПП КАК КПППоставщика,
	|	Спецификация.Организация.ЮридическийАдрес КАК ЮридическийАдресПоставщика,
	|	Спецификация.Контрагент.ПолноеНаименование КАК КонтрагентПолноеНаименование,
	|	Спецификация.Контрагент.ИНН КАК ИННКонтрагента,
	|	Спецификация.Контрагент.КПП КАК КППКонтрагента,
	|	Спецификация.Контрагент.ЮридическийАдрес КАК ЮридическийАдресКонтрагента,
	|	&Валюта КАК ВалютаДокумента,
	|	Спецификация.Организация.ФИОДолжностногоЛица КАК ФИОРуководителя
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПроверяемыеРеквизиты.ПредставлениеПолучателяПлатежа КАК Организация
	|ИЗ
	|	ВТ_ДанныеДокумента КАК ПроверяемыеРеквизиты
	|ГДЕ
	|	(ПроверяемыеРеквизиты.ИННПолучателяПлатежа ЕСТЬ NULL 
	|			ИЛИ ПроверяемыеРеквизиты.КПППолучателяПлатежа ЕСТЬ NULL 
	|			ИЛИ ПроверяемыеРеквизиты.БанкПолучателяПлатежа ЕСТЬ NULL 
	|			ИЛИ ПроверяемыеРеквизиты.БИКБанкаПолучателяПлатежа ЕСТЬ NULL 
	|			ИЛИ ПроверяемыеРеквизиты.СчетБанкаПолучателяПлатежа ЕСТЬ NULL 
	|			ИЛИ ПроверяемыеРеквизиты.СчетПолучателяПлатежа ЕСТЬ NULL )
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	Документ.СуммаДокумента КАК Цена,
	|	1 КАК Количество,
	|	""шт"" КАК ЕдиницаИзмерения
	|ИЗ
	|	ВТ_ДанныеДокумента КАК Документ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	Документ.ПредставлениеДокумента,
	|	Документ.ПредставлениеПолучателяПлатежа,
	|	Документ.ИННПолучателяПлатежа,
	|	Документ.КПППолучателяПлатежа,
	|	Документ.БанкПолучателяПлатежа,
	|	Документ.БИКБанкаПолучателяПлатежа,
	|	Документ.СчетБанкаПолучателяПлатежа,
	|	Документ.СчетПолучателяПлатежа,
	|	Документ.Дата,
	|	Документ.СуммаДокумента,
	|	Документ.Номер,
	|	Документ.ПоставщикПолноеНаименование,
	|	Документ.ИННПоставщика,
	|	Документ.КПППоставщика,
	|	Документ.ЮридическийАдресПоставщика,
	|	Документ.КонтрагентПолноеНаименование,
	|	Документ.ИННКонтрагента,
	|	Документ.КППКонтрагента,
	|	Документ.ЮридическийАдресКонтрагента,
	|	Документ.ВалютаДокумента,
	|	Документ.ФИОРуководителя
	|ИЗ
	|	ВТ_ДанныеДокумента КАК Документ";
	
	Запрос.УстановитьПараметр("Ссылка", Спецификация);
	Запрос.УстановитьПараметр("Валюта", Справочники.Валюты.НайтиПоКоду("810").Ссылка);
	
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	Если НЕ РезультатЗапроса[1].Пустой() Тогда
		
		ВыборкаПроверяемыеРеквизиты =  РезультатЗапроса[1].Выбрать();
		ВыборкаПроверяемыеРеквизиты.Следующий();
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = "Не все платежные реквизиты заполнены. Организация - " + ВыборкаПроверяемыеРеквизиты.Организация;
		Сообщение.Сообщить();
	...
	ВыборкаОсновная = РезультатЗапроса[3].Выбрать();
	...
	ОбластьЗаголовокСчета.Параметры.Заполнить(ВыборкаОсновная);
	ОбластьЗаголовокСчета.Параметры.БанкПолучателяПлатежаПредставление = ВыборкаОсновная.БанкПолучателяПлатежа;
	
	ТабДок.Вывести(ОбластьЗаголовокСчета);
	
	ОбластьЗаголовок = Макет.ПолучитьОбласть("Заголовок");
	НомерДокумента = ВыборкаОсновная.Номер;
	ДатаДокумента =  Формат(ВыборкаОсновная.Дата, "ДФ='дд ММММ гггг'") + " г.";
	ОбластьЗаголовок.Параметры.ТекстЗаголовка =  "Счет на оплату №" + НомерДокумента + "от " + ДатаДокумента;
	ТабДок.Вывести(ОбластьЗаголовок);
	
	ОбластьПоставщик = Макет.ПолучитьОбласть("Поставщик");
	ЮридическийАдресПоставщика = 	ВыборкаОсновная.ЮридическийАдресПоставщика;																						 
	ПредставлениеПоставщика =  ВыборкаОсновная.ПоставщикПолноеНаименование + 
															", ИНН " + ВыборкаОсновная.ИННПоставщика   +
															", КПП " + ВыборкаОсновная.КПППоставщика   +
															?(ЗначениеЗаполнено(ЮридическийАдресПоставщика),"," + ЮридическийАдресПоставщика ,"");
															
															
	ОбластьПокупатель = Макет.ПолучитьОбласть("Покупатель");
	ЮридическийАдресПокупателя = 	ВыборкаОсновная.ЮридическийАдресКонтрагента;	
	ИННПокупателя =  ВыборкаОсновная.ИННКонтрагента;
	КПППокупателя =  ВыборкаОсновная.КППКонтрагента;
	ПредставлениеПокупателя =  ВыборкаОсновная.КонтрагентПолноеНаименование + 
	...
	СуммаДокумента = ВыборкаОсновная.СуммаДокумента;
	...
	ВалютаДокумента = ВыборкаОсновная.ВалютаДокумента;
	СуммаКПрописи = СуммаДокумента;
	//ОбластьСуммаПрописью.Параметры.СуммаПрописью = РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаКПрописи,ВалютаДокумента.Ссылка);
	...
	ОбластьСуммаПрописью.Параметры.ИтоговаяСтрока = "Всего наименований "+  Товары.Количество() + ", на сумму " + СокрЛП(Формат(СуммаКПрописи,ФорматнаяСтрока)) + " руб.";
	ТабДок.Вывести(ОбластьСуммаПрописью); 
	
	//подвал
	ОбластьПодвалСчета = Макет.ПолучитьОбласть("ПодвалСчета");
	ОбластьПодвалСчета.Параметры.ФИОРуководителя = ФизическиеЛица.ФамилияИнициалыФизЛица(ВыборкаОсновная.ФИОРуководителя);
