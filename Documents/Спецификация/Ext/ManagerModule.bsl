...
Функция ПечатьСчетНаОплатуПокупателю(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	1 КАК НомерСтроки,
	|	Спецификация.СуммаДокумента КАК Сумма,
	...
	ОбластьИтого.Параметры.Всего = СокрЛП(Формат(Всего, ФорматнаяСтрока));
	ТабДок.Вывести(ОбластьИтого);
	
	ТабДок.Вывести(ОбластьИтогоНДС);
	
	ОбластьИтогоКОплате.Параметры.ИтогоКОплате = СокрЛП(Формат(Всего, ФорматнаяСтрока));
