...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Ссылка.Проведен И ЗначениеЗаполнено(ДатаИзготовления) И Изделие <> Справочники.Изделия.Переделка Тогда
		...
		Если ЭтоКухня Тогда
			...
			Запрос.Текст =
			"ВЫБРАТЬ
			|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Спецификация.Ссылка) КАК Ссылка
			|ИЗ
			|	Документ.Спецификация КАК Спецификация
			|ГДЕ
			|	Спецификация.ЭтоКухня
			|	И Спецификация.ДатаИзготовления = &ДатаИзготовления
			|	И Спецификация.Ссылка <> &Ссылка
			|	И Спецификация.Проведен
			|	И Спецификация.Производство = &Производство";
			...
			Если НЕ РезультатЗапроса.Пустой() Тогда
				Выборка = РезультатЗапроса.Выбрать();
				Выборка.Следующий();
				КоличествоКухонь = Выборка.Ссылка;
			КонецЕсли;
			
			Если Число(КоличествоКухонь) > 1 Тогда
				Отказ = Истина;
				ТекстСообщения = "На %1 превышено количество кухонь. Выберите другой день отгрузки.";
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстСообщения, ДатаИзготовления);
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, Ссылка);
			КонецЕсли;
