...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Ссылка.Проведен И ЗначениеЗаполнено(ДатаИзготовления) И Изделие <> Справочники.Изделия.Переделка Тогда
		...
		Если ЭтоКухня Тогда
			...
			Если Число(КоличествоКухонь) > 20 Тогда
				Отказ = Истина;
	...
	ДанныеДляНовогоРаскроя = ДанныеДляРаскроя + РегистрыСведений.РаскройДеталей.ПолучитьСтрокуРаскроя(ЭтотОбъект);
	
	# Область РаскройДеталей
	...
	Если ДанныеДляРаскроя <> "☻☺☺☺☺☺☺☺☺☺☺☺☺☺☻☺☺☺☺☺☻" Тогда
		...
		ЭтотОбъект.СтрокаНовогоРаскрояЛДСП = ЛексКлиентСервер.ПеревестиСтрокуВКодыСимволов(ДанныеДляНовогоРаскроя);
