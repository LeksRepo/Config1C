...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ЗначениеЗаполнено(Монтажник) И ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж Тогда
		...
		Отказ = ЗанятьМонтажника(ДатаМонтажа);
		
		Если ЭтоКухня И Дата > '2014.09.16' Тогда
			
			Если ДеньНедели(ДатаМонтажа + 60 * 60 * 24) = 7 Тогда
				Отказ = ЗанятьМонтажника(ДатаМонтажа + 60 * 60 * 24 * 2);
			Иначе
				Отказ = ЗанятьМонтажника(ДатаМонтажа + 60 * 60 * 24);
			КонецЕсли;
