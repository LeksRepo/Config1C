...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если (Изделие = Справочники.Изделия.Переделка) И (НЕ (Виновные.Выгрузить().Итог("Сумма") = СуммаДокумента)) Тогда
		ТекстСообщения = "Сумма удержания не совпадает с суммой документа";
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Объект.Виновные", ТекстСообщения);
	КонецЕсли;
	...
	Если НЕ Отказ Тогда
		...
		Если Изделие = Справочники.Изделия.Переделка Тогда
			...
			Для Каждого Строка Из Виновные Цикл
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе // не дилерский
		...
		Если Изделие = Справочники.Изделия.Переделка Тогда
			...
			ПроверяемыеРеквизиты.Добавить("Виновные");
