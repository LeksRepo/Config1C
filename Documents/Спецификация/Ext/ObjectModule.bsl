...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого Строка Из ЭтотОбъект.СписокМатериалы Цикл
		...
		Если ОбычныйМатериал Тогда
			...
			//Если Ссылка.АГТ[i].КоличествоПетель > 0 Тогда
			//		Если Ссылка.АГТ[i].КоличествоПетель = 1 Тогда КП = 2; 
			//		Иначе КП = Ссылка.АГТ[i].КоличествоПетель; КонецЕсли;
			//		Если Ссылка.АГТ[i].ПазыДляПетель = "Слева" Тогда
			//			Сторона = "3";
			//			Отступ = ЛексКлиентСервер.ВернутьОтступ(Ссылка.АГТ[i].Y1);
			//			Шаг = Окр((Ссылка.АГТ[i].Y1-2*Отступ)/(КП-1),0);
			//		ИначеЕсли Ссылка.АГТ[i].ПазыДляПетель = "Справа" Тогда
			//			Сторона = "4";
			//			Отступ = ЛексКлиентСервер.ВернутьОтступ(Ссылка.АГТ[i].Y1);
			//			Шаг = Окр((Ссылка.АГТ[i].Y1-2*Отступ)/(КП-1),0);
			//		ИначеЕсли Ссылка.АГТ[i].ПазыДляПетель = "Сверху" Тогда
			//			Сторона = "1";
			//			Отступ = ЛексКлиентСервер.ВернутьОтступ(Ссылка.АГТ[i].X1);
			//			Шаг = Окр((Ссылка.АГТ[i].X1-2*Отступ)/(КП-1),0);
			//		ИначеЕсли Ссылка.АГТ[i].ПазыДляПетель = "Снизу" Тогда
			//			Сторона = "2";
			//			Отступ = ЛексКлиентСервер.ВернутьОтступ(Ссылка.АГТ[i].X1);
			//			Шаг = Окр((Ссылка.АГТ[i].X1-2*Отступ)/(КП-1),0);
			//		КонецЕсли;
			//		стр.СтруктураДыр = "3_"+Сторона+"_"+Ссылка.АГТ[i].КоличествоПетель+"_"+Строка(Отступ)+"_"+Строка(Шаг)+"_0#"
			//	КонецЕсли;
			
			//			Функция ВернутьОтступ(Размер) Экспорт
			//	
