...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ТребуетсяПроверка = ОпределитьТребуетсяПроверка();	
...
Функция ОпределитьТребуетсяПроверка()
	
	Договор = Документы.Спецификация.ПолучитьДоговор(Ссылка);
	
	ДоговорБезМонтажа = (ТипЗнч(Договор) = Тип("ДокументСсылка.Договор")) И (Договор <> Документы.Договор.ПустаяСсылка()) И (ПакетУслуг <> Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж); 
	
	ЕстьРасчет = Ложь;
	
	УслугиРасчета = Новый Массив();
	УслугиРасчета.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.РасчетКухни, Подразделение));
	УслугиРасчета.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.РасчетШкафаКупе, Подразделение));
	УслугиРасчета.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.РасчетКорпуснойМебели, Подразделение));
	
	Для Каждого Стр ИЗ СписокНоменклатуры Цикл
		
		Для Каждого У ИЗ УслугиРасчета Цикл
			
			Если Стр.Номенклатура = У Тогда
		...
		Если ЕстьРасчет = Истина Тогда
			Прервать;	
		КонецЕсли;
		
	КонецЦикла;
	
	ДилерскийСРасчетом = Дилерский И ЕстьРасчет;
	
	Возврат (ДоговорБезМонтажа ИЛИ ДилерскийСРасчетом);  
...
Функция МонтажникСвободен(Дата1, Дата2, Ошибки);
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, "Объект.Монтажник", ТекстСообщения);
		Результат = Ложь;
