...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ДанныеДляРаскроя = ДанныеДляРаскроя + ЭтотОбъект.Ссылка +"☺" + ЭтотОбъект.Комментарий + "☻";
	...
	Для Каждого Строка Из ЭтотОбъект.СписокМатериалы Цикл
		...
		Если ОбычныйМатериал Тогда
			...
			Для каждого Элмент Из ОсновныеРеквизиты Цикл
		...
		ИначеЕсли ОбычныйФасад Тогда
			...
			//Для Каждого Строка Из ЭтотОбъект.СписокФасады Цикл
			...
			//	
			//	РеквизитыФасадов.Добавить(Строка.ШиринаДетали - 2 * Строка.УниверсальнаяКромка.ШиринаДетали + 2 * Строка.УниверсальнаяКромка.ШиринаПаза);
			//	РеквизитыФасадов.Добавить(Строка.ВысотаДетали - 2 * Строка.УниверсальнаяКромка.ШиринаДетали + 2 * Строка.УниверсальнаяКромка.ШиринаПаза);
			//	
			//Иначе
			...
			РеквизитыФасадов.Добавить(СтрокаИдентификатор);
			...
			Для каждого Элмент Из РеквизитыФасадов Цикл
	...
	/////////////////////////////////////
	///НЕ ВЫВОДИТСЯ КОЛИЧЕСТВО КРИВЫХ ДЕТАЛЕЙ, НУЖНО ДОБАВИТЬ
	/////////////////////////////////////
	...
	Для Каждого Элемент Из ЭтотОбъект.СписокМатериалы Цикл
		...
		Если ЗначениеЗаполнено(Элемент.СтрокаДляФлэш) Тогда
			...
			РеквизитыКривогоПила.Добавить(""); // Заказчик,
	...
	Если СтрокаДляПередачиКривойПил <> "☻" Тогда
	...
	Если НЕ Ссылка.Проведен И ЗначениеЗаполнено(ДатаИзготовления) И Изделие = Справочники.Изделия.Детали Тогда
	...
	//	ЗарплатаЦеха = СписокНоменклатуры.Итог("ЗарплатаЦеха");
	//	
	//	СуммаСрочности = 0;
	//	
	//	Если ЗначениеЗаполнено(СтрокаСрочности) Тогда
	//		СуммаСрочности = СтрокаСрочности.ЗарплатаЦеха;
	//	КонецЕсли;
	//	
	//	СтруктураНорматива = Документы.Спецификация.ПолучитьСтруктуруНорматива(Ссылка, Производство, Дата, ДатаИзготовления, ЗарплатаЦеха, СуммаСрочности, РежимЗаписи);
	//	
	//	Если СтруктураНорматива.Срочность = Неопределено Тогда
	//		Отказ = Истина;
	//		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Превышен максимум нарядов на день изготовления");
	//		Возврат;
	//	Иначе
	//		Срочный = СтруктураНорматива.Срочность;
	//	КонецЕсли;
	//	
	//	Если СтруктураНорматива.Срочность И СуммаСрочности = 0 Тогда
	//		
	//		СтоимостьУслуг = Документы.Спецификация.ПолучитьСтоимостьУслуг(СписокНоменклатуры.Выгрузить());
	//		
	//		НоваяСтрока = СписокНоменклатуры.Добавить();
	//		НоваяСтрока.Номенклатура = Справочники.Номенклатура.СрочностьЗаказа;
	//		НоваяСтрока.ЕдиницаИзмерения = НоваяСтрока.Номенклатура.ЕдиницаИзмерения;
	//		НоваяСтрока.Количество = 1;
	//		НоваяСтрока.НомерИзделия = 0;
	//		НоваяСтрока.ЗарплатаЦеха = СтоимостьУслуг.ЗарплатаЦеха * СтруктураНорматива.Коэффициент;
	//		НоваяСтрока.РозничнаяСтоимость = СтоимостьУслуг.РозничнаяСтоимость * СтруктураНорматива.Коэффициент;
	//		
	//	ИначеЕсли НЕ СтруктураНорматива.Срочность И СуммаСрочности > 0 Тогда
	//		СписокНоменклатуры.Удалить(СтрокаСрочности);
	//	КонецЕсли;
	//	
	//КонецЕсли;
	...
	СуммаНарядаСпецификации = СписокНоменклатуры.Итог("ЗарплатаЦеха");
...
Функция ВзаиморасчетыПроводки()
	...
	СтруктураОтбора.Вставить("Счет", ПланыСчетов.Управленческий.ВзаиморасчетыСПокупателями);
	СтруктураОтбора.Вставить("Субконто1", Контрагент);
	Если Дилерский Тогда
		СтруктураОтбора.Вставить("Субконто2", Неопределено);
	Иначе
		СтруктураОтбора.Вставить("Субконто2", Ссылка);
	КонецЕсли;
	...
	ОстатокПоСчету = РегистрыБухгалтерии.Управленческий.Остатки(,,СтруктураОтбора);	
	Если ОстатокПоСчету.Количество() > 0 Тогда
		Если ОстатокПоСчету[0].СуммаОстатокКт >= СуммаДокумента Тогда
			...
			Движение.Содержание = "Дилер разместил заказ на Спецификацию №" + Строка(Номер);
		...
		Иначе
			Отказ = Истина;
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Суммы аванса %1 у контрагента %2 недостаточно для проведения спецификации, необходимо %3", ОстатокПоСчету[0].СуммаОстатокКт, Контрагент, СуммаДокумента);
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект);
		КонецЕсли;
	Иначе
		Отказ = Истина;
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Отсутствует сумма аванса у контрагента %1, для проведения спецификации необходимо %2", Контрагент, СуммаДокумента);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ЭтотОбъект);
	КонецЕсли;
