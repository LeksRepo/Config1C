...
Функция ЗаполнитьОсновныеЦвета()
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ВертикальныйПрофиль Тогда
				...
				ЦветВертикальногоПрофиля = Выборка.Номенклатура;
			...
			ИначеЕсли Выборка.НоменклатурнаяГруппа = Справочники.НоменклатурныеГруппы.ЛДСП16 Тогда
				...
				Если НЕ ЗначениеЗаполнено(ЦветЛДСПОсновной) Тогда
					...
					ЦветЛДСПОсновной = Выборка.Номенклатура;
					
				ИначеЕсли НЕ ЗначениеЗаполнено(ЦветЛДСПДополнительный) Тогда
					
					ЦветЛДСПДополнительный = Выборка.Номенклатура;
...
Функция ЗаполнитьКоличествоКоробов()
	...
	Запрос.УстановитьПараметр("СписокИзделийПоКаталогу", СписокИзделийПоКаталогу);
	...
	Выборка.Следующий();
	КоличествоКоробов = Выборка.Количество;
...
Функция ПревышенЛимитНарядов()
	...
	Если ЭтоКухня Тогда
		...
		КоличествоКоробов = ФактКоробов + СписокИзделийПоКаталогу.Количество();
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ЗаполнитьОсновныеЦвета();
	ЗаполнитьКоличествоКоробов();
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если Изделие = Справочники.Изделия.Детали Тогда
		...
		Запись.КоличествоКоробовФакт = СписокИзделийПоКаталогу.Количество();
