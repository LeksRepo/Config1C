...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ Проведен И (ДатаИзготовления < НачалоДня(ТекущаяДата())) Тогда
		
		Отказ = Истина;
		ТекстСообщения = "Не корректная дата изготовления.";
	...
	Если Дата < '2016.11.19' Тогда
		// Чтобы при проведении старых документов суммы не изменялись.
	Иначе
...
Функция ДвиженияПоказателиСотрудника()
	
	Перем Сотрудник, Показатель;
	...
	Если Дилерский Тогда
		...
		Показатель = Перечисления.ВидыПоказателейСотрудников.СтоимостьРазмещенныхСпецификацийДилеров;
	Иначе
		Сотрудник = Автор.ФизическоеЛицо;
		Показатель = Перечисления.ВидыПоказателейСотрудников.СтоимостьРазмещенныхСпецификаций;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Сотрудник)
		И ЗначениеЗаполнено(Показатель) Тогда
...
Функция ДвиженияПродажиФакт()
	...
	ИначеЕсли Изделие.Серийное Тогда
		Сотрудник = Автор.ФизическоеЛицо;
		ВидПродажи = Перечисления.ВидыПродаж.СерийнаяМебель;
	Иначе
		Сотрудник = Автор.ФизическоеЛицо;
