...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого Строка Из Комплектация Цикл
		...
		Если Ном.НоменклатурнаяГруппа.ВидМатериала = Перечисления.ВидыМатериалов.Листовой ИЛИ НЕ Ном.Базовый Тогда
			
			Отказ = Истина;
			ТекстСообщения = "В комплектации запрещен листовой или не базовый материал: "+ Ном;
	...
	Если НЕ Проведен И ЗначениеЗаполнено(ДатаИзготовления) И (Изделие = Справочники.Изделия.Детали ИЛИ Дилерский) Тогда
		...
		//Вернуть проверку после, запуска обработок.
		ДокументПлановыйЛимит = Неопределено; //Документы.ПлановыйЛимит.ПолучитьДокументЗаПериод(Подразделение, ДатаИзготовления, Истина);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	//		
	//		ДобавитьОшибкуСпецификацияДилеру = НЕ (ТипЗнч(Автор) = Тип("СправочникСсылка.ВнешниеПользователи") 
	//		И Автор.ОбъектАвторизации = Контрагент);
	//		
	//	Иначе
	//		
	//		ДобавитьОшибкуСпецификацияДилеру = НЕ (ПользователиКлиентСервер.ЭтоСеансВнешнегоПользователя()
	//		И ПользователиКлиентСервер.ТекущийВнешнийПользователь().ОбъектАвторизации = Контрагент);
	//		
	//	КонецЕсли;
	//	
	//	Если ДобавитьОшибкуСпецификацияДилеру Тогда
	//		
