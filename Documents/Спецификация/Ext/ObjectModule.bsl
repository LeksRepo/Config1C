...
Функция ПревышенЛимитНарядов()
	...
	//	СтрокаСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("%1 завод не работает, выберите другой день", Формат(ДатаИзготовления, "ДЛФ=DD"));
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого Элемент Из ЭтотОбъект.СписокМатериалы Цикл
		...
		Если ЗначениеЗаполнено(Элемент.СтрокаДляФлэш) Тогда
			...
			// ну ахуеть, а с чего ты решил, что можно просто ещё один реквизит в строку флэшки добавить?
			// РеквизитыКривогоПила.Добавить("Комментарий: " + Элемент.Комментарий);
