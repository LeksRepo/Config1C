...
Процедура ЗаполнитьНаСервереИнженерТехнолог()
	...
	Запрос.Текст = 
		...
		|			&ОкончаниеПериода,
		|			Должность = ЗНАЧЕНИЕ(Справочник.Должности.ИнженерТехнолог)
	...
	Пока Выборка.Следующий() Цикл
		...
		НоваяСтрока = Объект.СписокФизлиц.Добавить();
		НоваяСтрока.ФизЛицо = СтруктураНарушений.Сотрудник;
		НоваяСтрока.Статья = Справочники.СтатьиДоходовРасходов.РасходыЗарплатаТехПерсонал;
...
Процедура ЗаполнитьНаСервереТехнолог()
	...
	Запрос.Текст = 
		...
		|	РегистрСведений.ПараметрыНачислений.СрезПоследних(
		|			&ОкончаниеПериода,
		|			Должность = ЗНАЧЕНИЕ(Справочник.Должности.Технолог)
		|				И Параметр В (&МассивПараметров)) КАК ПараметрыНачисленийСрезПоследних";
...
Процедура ЗаполнитьНаСервереДизайнеры()
	...
	Запрос.Текст = 
		...
		Пока ВыборкаПоДизайнерам.Следующий() Цикл
			...
			СтруктураПоказателейДизайнера = Документы.НачислениеЗаработнойПлаты.ПолучитьПоказателиДизайнера(Объект.НачалоПериода, Объект.ОкончаниеПериода, ВыборкаПоДизайнерам.Дизайнер, Объект.Подразделение);
