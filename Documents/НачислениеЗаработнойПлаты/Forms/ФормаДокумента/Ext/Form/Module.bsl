...
Процедура ЗаполнитьНаСервереИнженерТехнолог()
	...
	ТаблицаПараметров = ПолучитьЗначениеПараметров(МассивПараметров, Справочники.Должности.Технолог);
...
Процедура ЗаполнитьНаСервереДизайнеры()
	...
	Запрос.Текст = 
		...
		ТаблицаПараметров = ПолучитьЗначениеПараметров(МассивПараметров, Справочники.Должности.ДизайнерКонсультант);
		...
		Пока ВыборкаПоДизайнерам.Следующий() Цикл
			...
			ОплачиваемыеЗамеры = Окр(ФактПоЗамерам - ФактЗаключенныхДоговоров, 0, РежимОкругления.Окр15как10);
			...
			Комментарий = "Оклад=" + Оклад + ", замеров сверх плана=" + ОплачиваемыеЗамеры + ", кухонь=" + КоличествоКухонь + ", шкафов=" + 
				КоличествоШкафов + ", план по выручке перевыполнен на " + (ФактПоВыручке - ПланПоВыручке);
...
&НаСервере
Функция ПолучитьЗначениеПараметров(МассивПараметров, Должность)
