...
Процедура ЗаполнитьНаСервереИнженерТехнолог()
	...
	МассивПараметров.Добавить(ВидыПараметровНачислений.КоличествоОшибокТехнологаСредняяГраница);
	...
	МассивПараметров.Добавить(ВидыПараметровНачислений.НачислениеИнженеруТехнологуПоОшибкамСредняяГраница);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СуммаБалов > НижняяГраницаБалов Тогда
			...
			Если СуммаБалов < ВерхняяГраницаБалов Тогда
				
				Надбавка = НижнееНачислениеПоБалам;
				
			ИначеЕсли СуммаБалов > ВерхняяГраницаБалов Тогда
				
				Надбавка = ВерхнееНачислениеПоБалам;
		...
		Если КоличествоОшибок <= ВерхняяГраницаОшибок Тогда
			
			НоваяСтрока = Объект.СписокФизлиц.Добавить();
			НоваяСтрока.ФизЛицо = Выборка.Сотрудник;
			НоваяСтрока.Статья = Справочники.СтатьиДоходовРасходов.РасходыЗарплатаТехПерсонал;
			
			Если КоличествоОшибок > СредняяГраницаОшибок Тогда
				
				Надбавка = НижнееНачислениеПоОшибкам;
				
			ИначеЕсли КоличествоОшибок > НижняяГраницаОшибок Тогда
				
				Надбавка = СреднееНачислениеПоОшибкам;
				
			Иначе
				
				Надбавка = ВерхнееНачислениеПоОшибкам;
