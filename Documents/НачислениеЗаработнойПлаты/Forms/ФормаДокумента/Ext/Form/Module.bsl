...
Процедура ЗаполнитьНаСервереИнженерТехнолог()
	...
	|			&КонецПериода,
	|			Счет = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконто.ВидыПоказателейСотрудников),
	|			,
	|			Подразделение = &Подразделение
	|				И Субконто2 = &ФизЛицо
	|				И Субконто1 = ЗНАЧЕНИЕ(Перечисление.ВидыПоказателейСотрудников.БаллыТехнологов)) КАК УправленческийОстатки
	|ГДЕ
