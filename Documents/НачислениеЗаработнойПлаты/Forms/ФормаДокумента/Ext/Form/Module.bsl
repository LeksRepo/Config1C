...
Процедура ЗаполнитьНаСервереМонтажники()
	...
	СтавкиМонтажников = ПолучитьСтавкиМонтажников(Объект.Подразделение, Объект.Дата);
	...
	МассивПоказателей.Добавить(ВидыПоказателейСотрудников.КоличествоУстановленныхИзделийНеуд);
	МассивПоказателей.Добавить(ВидыПоказателейСотрудников.КоличествоМетровУстановленныхИзделийНеуд);
	МассивПоказателей.Добавить(ВидыПоказателейСотрудников.КоличествоУстановленныхКухоньНеуд);
	МассивПоказателей.Добавить(ВидыПоказателейСотрудников.КилометровУдаленныхМонтажейНеуд);
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КоличествоУстановленныхИзделий Тогда
			...
			Сумма = Выборка.Количество * СтавкиМонтажников.ТарифЗаСборку;
			Комментарий = "За сборку - " + Выборка.Количество + " шт.";
			
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КоличествоУстановленныхКухонь Тогда
			
			Сумма = Выборка.Количество * СтавкиМонтажников.ТарифЗаСборкуКухни;
			Комментарий = "За сборку кухни - " + Выборка.Количество + " шт.";
			
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КилометровУдаленныхМонтажейНеуд Тогда
			
			Сумма = Выборка.Количество * СтавкиМонтажников.ПроездМонтажникаЗаГородом * ПонижающийКоэффициент;
			Комментарий = "За удаленные монтажи неуд. - " + Выборка.Количество + " км.";
			
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КоличествоМетровУстановленныхИзделийНеуд Тогда
			
			Сумма = Выборка.Количество * СтавкиМонтажников.СборкаИзделия * ПонижающийКоэффициент;
			Комментарий = "За объем выполненных работ неуд. - " + Выборка.Количество + " кв.м.";
			
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КоличествоУстановленныхИзделийНеуд Тогда
			
			Сумма = Выборка.Количество * СтавкиМонтажников.ТарифЗаСборку * ПонижающийКоэффициент;
			Комментарий = "За сборку неуд. - " + Выборка.Количество + " шт.";
			
		ИначеЕсли Выборка.Показатель = ВидыПоказателейСотрудников.КоличествоУстановленныхКухоньНеуд Тогда
			
			Сумма = Выборка.Количество * СтавкиМонтажников.ТарифЗаСборкуКухни * ПонижающийКоэффициент;
			Комментарий = "За сборку кухни неуд. - " + Выборка.Количество + " шт.";	
		КонецЕсли;
...
&НаСервере
Функция ПолучитьСтавкиМонтажников(Подразделение, Период)
	...
	МассивНоменклатуры.Добавить(ОбщаяНоменклатура.СборкаИзделия);
	МассивНоменклатуры.Добавить(ОбщаяНоменклатура.ПроездМонтажникаЗаГородом);
	МассивНоменклатуры.Добавить(ОбщаяНоменклатура.ТарифЗаСборку);
	МассивНоменклатуры.Добавить(ОбщаяНоменклатура.ТарифЗаСборкуКухни);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Ссылка = ОбщаяНоменклатура.СборкаИзделия Тогда
			Имя = "СборкаИзделия"; 
		ИначеЕсли Выборка.Ссылка = ОбщаяНоменклатура.ПроездМонтажникаЗаГородом Тогда
			Имя = "ПроездМонтажникаЗаГородом";
		ИначеЕсли Выборка.Ссылка = ОбщаяНоменклатура.ТарифЗаСборку Тогда
			Имя = "ТарифЗаСборку";
		ИначеЕсли Выборка.Ссылка = ОбщаяНоменклатура.ТарифЗаСборкуКухни Тогда
			Имя = "ТарифЗаСборкуКухни";	
		КонецЕсли;	
