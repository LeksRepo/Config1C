...
&НаКлиенте
Процедура ПеределкиНажатие(Элемент, СтандартнаяОбработка)

	Если ТипЗнч(Переделки) = Тип("Строка") Тогда
		
		СтандартнаяОбработка = Ложь;
		
		Если Найти(Переделки, "Ошибка") <> 0 Тогда
		...
		ПараметрыПеределки = Новый Структура();
		ПараметрыПеределки.Вставить("Основание", Объект.Ссылка);
		ОткрытьФорму("Документ.Переделка.ФормаОбъекта", ПараметрыПеределки, ЭтаФорма);
