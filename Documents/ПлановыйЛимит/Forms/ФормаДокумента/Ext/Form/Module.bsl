...
Функция ПередИзменениемДанных()
	...
	Если ЗначениеЗаполнено(Объект.Подразделение) И ЗначениеЗаполнено(Объект.Дата) Тогда
		...
		Для Каждого Строка Из Объект.ТаблицаДней Цикл
			Если Строка.Норматив > 0 ИЛИ Строка.КоличествоКоробов > 0 ИЛИ Строка.КоличествоДеталей > 0 ИЛИ Строка.КоличествоДоставок > 0 Тогда
...
&НаКлиенте
Процедура ЗаполнитьДанными(КоличествоКоробов, Норматив, КоличествоДеталей, КоличествоДоставок) Экспорт
	...
	Для Каждого Строка Из Объект.ТаблицаДней Цикл
		...
		Если НЕ (ДеньНедели(Строка.Дата) = 7) Тогда
			...
			Если ДеньНедели(Строка.Дата) = 6 Тогда
				...
				Строка.КоличествоДеталей = Окр(КоличествоДеталей/2);
				Строка.КоличествоДоставок = Окр(КоличествоДоставок/2);
				
			Иначе
				
				Строка.Норматив = Норматив;
				Строка.КоличествоКоробов = КоличествоКоробов;
				Строка.КоличествоДеталей = КоличествоДеталей;
				Строка.КоличествоДоставок = КоличествоДоставок;
