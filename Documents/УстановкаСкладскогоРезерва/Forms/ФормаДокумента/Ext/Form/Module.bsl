...
Процедура СписокНоменклатурыНоменклатураПриИзменении(Элемент)
	...
	Строка = Элементы.СписокНоменклатуры.ТекущиеДанные;
	Резерв = ПолучитьРезервПоНоменклатуре(Строка.Номенклатура);
	Строка.Количество = Резерв;
	
КонецПроцедуры

&НаСервере
Функция ПолучитьРезервПоНоменклатуре(Ссылка)
	...
	|			Подразделение = &Подразделение
	|				И Склад = &Склад
	|				И Номенклатура = &Номенклатура) КАК Резерв";
	
	Результат = Запрос.Выполнить().Выбрать();
	
	Если Результат.Количество() > 0 Тогда
		 Результат.Следующий();	
		 Резерв = Результат.Количество;
	КонецЕсли;
