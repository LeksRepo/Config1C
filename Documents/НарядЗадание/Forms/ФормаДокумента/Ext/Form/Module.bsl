...
Функция ОпределитьКоличествоХлыстовогоМатериала(Номенклатура)
	...
	Пока МассивДеталей.Количество() > 0 Цикл
		...
		Если КоличествоОстатков > 0
			...
			РазмерХлыста = МассивОстатков[НомерОстатка].Размер;
			...
			Если ОтрезкиДляРазмещения.Количество() = 0 Тогда
				НомерОстатка = НомерОстатка - 1;
				Объект.ОстаткиХлыстов.Удалить(МассивОстатков[НомерОстатка].НомерСтроки-1);
				МассивОстатков.Удалить(НомерОстатка);
		...
		Для каждого РазмещеннаяДеталь Из Структура.Массив Цикл
			...
			НоваяСтрока.Размер = РазмещеннаяДеталь;
		...
		Если Структура.МинимальныйОстаток > 0 Тогда
			...
			НоваяСтрока.Размер = Структура.МинимальныйОстаток;
			НоваяСтрока.Номенклатура = Номенклатура;
			НоваяСтрока.НомерХлыста = НомерХлыста;
			НоваяСтрока.РазмерХлыста = РазмерХлыста;
...
&НаСервере
Процедура УдалитьНеИспользованныеОстаткиХлыстов()
	
	ТаблицаОстатков = Объект.ОстаткиХлыстов.Выгрузить();

	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("ТаблицаРаскрой", Объект.РаскройХлыстов.Выгрузить()); 
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Раскрой.Номенклатура,
	|	Раскрой.НомерХлыста,
	|	Раскрой.РазмерХлыста
	|ПОМЕСТИТЬ втХлысты
	|ИЗ
	|	&ТаблицаРаскрой КАК Раскрой
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	ТаблицаХлыстов = Запрос.Выполнить().Выгрузить();
	
	Для Каждого Строка Из ТаблицаХлыстов Цикл
		
		ПараметрыОтбора = Новый Структура();
		ПараметрыОтбора.Вставить("Номенклатура", Строка.Номенклатура);
		ПараметрыОтбора.Вставить("Размер", Строка.РазмерХлыста);
		
		Строки = ТаблицаОстатков.НайтиСтроки(ПараметрыОтбора);
		
		Если Строки.Количество() > 0 Тогда
			ТаблицаОстатков.Удалить(Строки[0]);		
		КонецЕсли;
	...
	Для Каждого Строка Из ТаблицаОстатков Цикл
		...
		ПараметрыОтбора = Новый Структура();
		ПараметрыОтбора.Вставить("Номенклатура", Строка.Номенклатура);
		ПараметрыОтбора.Вставить("Размер", Строка.Размер);
		
		Строки = Объект.ОстаткиХлыстов.НайтиСтроки(ПараметрыОтбора);
		
		Если Строки.Количество() > 0 Тогда		
			Объект.ОстаткиХлыстов.Удалить(Строки[0]);			
		КонецЕсли;
...
Функция ПерекроитьВесьХлыстовойМатериал(Объект)
	...
	ЗаполнитьОстаткиХлыстов();
	
	Для каждого Строка Из Объект.СписокХлыстовыхМатериалов Цикл
		
		ОпределитьКоличествоХлыстовогоМатериала(Строка.Номенклатура);
		
	КонецЦикла;
	
	УдалитьНеИспользованныеОстаткиХлыстов();
	
КонецФункции

&НаСервере
Процедура ЗаполнитьОстаткиХлыстов()
	
	Объект.ОстаткиХлыстов.Очистить();
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("СписокМатериалов", Объект.СписокХлыстовыхМатериалов.Выгрузить().ВыгрузитьКолонку("Номенклатура"));
	Запрос.УстановитьПараметр("Период", Объект.Дата - 1);
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	Обрезки.Номенклатура,
	|	Обрезки.Размер КАК Размер,
	|	Обрезки.КоличествоОстаток КАК Количество
	|ИЗ
	|	РегистрНакопления.ОбрезкиХлыстовойМатериал.Остатки(&Период, ) КАК Обрезки
	|ГДЕ
	|	Обрезки.Номенклатура В(&СписокМатериалов)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Размер";
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Счетчик = 1 По Выборка.Количество Цикл
			...
			Стр = Объект.ОстаткиХлыстов.Добавить();
			Стр.Номенклатура = Выборка.Номенклатура;
			Стр.Размер = Выборка.Размер;
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ЗапретРедактирования Тогда
...
Процедура ПодобратьИсключаемыеОстатки(Команда)
	...
	ОткрытьФорму("РегистрНакопления.ОбрезкиЛистовойМатериал.Форма.ФормаПодбора", ПараметрыФомы, Элементы.СписокИсключаемыхОстатков);
...
&НаКлиенте
Процедура ОстаткиХлыстовПередУдалением(Элемент, Отказ)
	Отказ = Истина;
КонецПроцедуры

&НаКлиенте
Процедура ОстаткиХлыстовПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	Отказ = Истина;
КонецПроцедуры
