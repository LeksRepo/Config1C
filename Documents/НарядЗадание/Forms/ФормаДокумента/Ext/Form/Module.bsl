...
&НаСервере
Функция ПолучитьТаблицуЗеркал(СписокСпецификаций) Экспорт
	...
	|	спрНоменклатура.Ссылка КАК НоменклатураОсновнаяПоСкладу,
	...
	|	И СпецификацияСписокМатериалы.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.Зеркало)
	...
	|	И ДвериСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.Зеркало)
	|	И спрНоменклатура.ОсновнаяПоСкладу";
...
Процедура ЗаполнитьНоменклатуройСервер() 
	...
	ТаблицаЗеркал = ПолучитьТаблицуЗеркал(СписокСпецификаций);
	Объект.СписокЗеркал.Загрузить(ТаблицаЗеркал);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	|	Справочник.Номенклатура КАК Номенклатура
	|ГДЕ
	|	Номенклатура.БазоваяНоменклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.Зеркало)
	|	И НЕ Номенклатура.ПометкаУдаления";
	
	Элементы.СписокЗеркалНоменклатураОсновнаяПоСкладу.СписокВыбора.ЗагрузитьЗначения(Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка"));
