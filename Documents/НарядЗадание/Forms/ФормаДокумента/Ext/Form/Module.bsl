...
Процедура ОткрытьКомплектацию(Команда)
	...
	Для каждого ИндексСтроки Из Элементы.СписокСпецификаций.ВыделенныеСтроки Цикл
		
		Строка = Объект.СписокСпецификаций.Получить(ИндексСтроки);
	...
	Если МассивКомплектаций.Количество() <> МассивСпецификаций.Количество() Тогда
		...
		ОткрытьФорму("Документ.Комплектация.ФормаОбъекта", ПараметрыФормы);
