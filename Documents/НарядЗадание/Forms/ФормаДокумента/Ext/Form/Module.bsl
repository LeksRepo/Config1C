...
Функция ПолучитьКомплектации(МассивСпецификаций)
	...
	Для каждого Спецификация Из МассивСпецификаций Цикл
		...
		Если НЕ ЗначениеЗаполнено(Комплектация) Тогда
			Комплектация = Документы.Комплектация.ПустаяСсылка();
		Конецесли;
		
		Результат.Вставить(Спецификация, Комплектация);
...
Процедура ОткрытьКомплектацию(Команда)
	...
	Для каждого КомплектацияКлюч Из МассивКомплектаций Цикл
		
		Комплектация = МассивКомплектаций.Получить(КомплектацияКлюч.Ключ);
		Уникальность =  Ложь;
		ПараметрыФормы = Новый Структура ("Ключ",Комплектация);
		
		Если НЕ ЗначениеЗаполнено(Комплектация) Тогда
			ПараметрыФормы.Вставить("Основание", КомплектацияКлюч.Ключ);
			Уникальность =  Истина;
		КонецЕсли;
