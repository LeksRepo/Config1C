...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если НЕ Отказ Тогда
		...
		Если Дата > Дата("20160601000000") Тогда
			...
			УвеличитьТребованияНакладные(Ссылка);
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ЗначениеЗаполнено(ЭтотОбъект.Ссылка) Тогда
		...
		ОчиститьСтрокуРаскроя(ЭтотОбъект.Ссылка);
...
Функция ОчиститьСтрокуРаскроя(Наряд)
	...
	//Запись.ТаблицаДеталей = Неопределено;
	//Запись.СтрокаКривогоПила = "";
	//Запись.Объект = Документы.НарядЗадание.ПустаяСсылка();
	//Запись.АлгоритмРаскроя = "";
	//Запись.ВремяФормирования = 0;
	//
	//НаборЗаписей.Записать();
...
Функция УвеличитьТребованияНакладные(Наряд)
	...
	Тз = Документы.ТребованиеНакладная.СформироватьТаблицуМатериалов(Наряд);
	
	Для Каждого Стр ИЗ Тз Цикл
		
		Если Стр.Затребовано > 0 И Стр.Номенклатура.МестоОбработки = Перечисления.МестоОбработки.Цех Тогда
			...
			Проводка.СубконтоДт[ПВХНарядЗадания] = Наряд;
			Проводка.СубконтоДт[ПВХНоменклатура] = Стр.Номенклатура;
			Проводка.КоличествоДт = Стр.Затребовано;
