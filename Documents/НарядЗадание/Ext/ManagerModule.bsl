...
Функция ПечатьМатериалПоЦеховымЗонам(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|ГДЕ
	|	СписокСпец.Ссылка В(&МассивНарядов)
	|;
	|//////////////////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	|ГДЕ
	|	НЕ (СписокСпецификаций.Спецификация ЕСТЬ NULL)
	|
	|;
	|///////////////////////////////////////////////////////////////////////////////
	|
	...
	|	СписокНомСпец.Спецификация КАК Спецификация
	|ИЗ
	|	Документ.НарядЗадание.СписокНоменклатуры КАК НарядЗаданиеСписокНоменклатуры
	|   	ЛЕВОЕ СОЕДИНЕНИЕ СписокНомСпец КАК СписокНомСпец
	|		ПО (НарядЗаданиеСписокНоменклатуры.Номенклатура = СписокНомСпец.Номенклатура
	|			И НарядЗаданиеСписокНоменклатуры.ПредоставитЗаказчик = СписокНомСпец.ПредоставитЗаказчик)
	|ГДЕ
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Пока ВыборкаЦеховаяЗона.Следующий() Цикл
			...
			// RonEXI: Сначала проходим наш материал, потом ещё раз заказчика. 
			// Так как "ПредоставитЗаказчик" находится внутри группы по спецификациям, по нему не упорядочить.
			
			ЭтоМатериалЗаказчика = 0;
			
			Пока ЭтоМатериалЗаказчика < 2 Цикл 
				...
				Пока ВыборкаНоменклатура.Следующий() Цикл
					...
					Пока ВыборкаСпец.Следующий() Цикл
						...
						Если ЗначениеЗаполнено(ВыборкаСпец.Спецификация) И (ВыборкаСпец.ПредоставитЗаказчик = ЭтоМатериалЗаказчика) Тогда

							Номер = ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ВыборкаСпец.Спецификация.Номер);
							СтрокаСпец = СтрокаСпец + Номер + Символы.ПС;
							
							Если НЕ ВыведенМатериалЗаказчикаЗаголовок 
								И ВыборкаСпец.ПредоставитЗаказчик Тогда
								ТабДок.Вывести(ОбластьМатериалЗаказчика);
								ВыведенМатериалЗаказчикаЗаголовок = Истина;
							КонецЕсли;
							
							ОбластьСписокНоменклатурыСтрока.Параметры.Заполнить(ВыборкаСпец);
							
						КонецЕсли;
							
					КонецЦикла;
					
					СтрокаСпец = Лев(СтрокаСпец,СтрДлина(СтрокаСпец)-1);
					
					Если ЗначениеЗаполнено(СтрокаСпец) Тогда
					
						ОбластьСписокНоменклатурыСтрока.Параметры.Спецификации = СтрокаСпец;
				...
				ЭтоМатериалЗаказчика = ЭтоМатериалЗаказчика + 1;
