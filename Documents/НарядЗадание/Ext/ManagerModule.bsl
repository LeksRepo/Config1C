...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КромочныйУчасток") Тогда
		ПодготовитьПечатнуюФорму("КромочныйУчасток", "Кромочный участок", "Документ.НарядЗадание.КромочныйУчасток",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СтекольныйУчасток") Тогда
		ПодготовитьПечатнуюФорму("СтекольныйУчасток", "Стекольный участок", "Документ.НарядЗадание.СтекольныйУчасток",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "КриволинейныйУчасток") Тогда
		ПодготовитьПечатнуюФорму("КриволинейныйУчасток", "Криволинейный участок", "Документ.НарядЗадание.КриволинейныйУчасток",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
...
Процедура ПодготовитьПечатнуюФорму(Знач ИмяМакета, ПредставлениеМакета, ПолныйПутьКМакету = "", МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ИмяМакета) Тогда
		...
		ИначеЕсли ИмяМакета = "КромочныйУчасток" Тогда
			ТабДок = КромочныйУчасток(МассивОбъектов, ОбъектыПечати);	
		ИначеЕсли ИмяМакета = "СтекольныйУчасток" Тогда
			ТабДок = СтекольныйУчасток(МассивОбъектов, ОбъектыПечати);	
		ИначеЕсли ИмяМакета = "КриволинейныйУчасток" Тогда
			ТабДок = КриволинейныйУчасток(МассивОбъектов, ОбъектыПечати);		
		КонецЕсли;
...
Функция ПечатьПоследовательностьВыпуска(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ОбластьШапкаДоЛДСП = Макет.ПолучитьОбласть("ШапкаДоЛДСП");
	ОбластьСтрокаДоЛДСП = Макет.ПолучитьОбласть("СтрокаДоЛДСП");
	
	ВставлятьРазделительСтраниц = Ложь;
	
	Для Каждого Наряд Из МассивОбъектов Цикл
		
		МассивУслугПрисадок = Новый Массив();
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.УслугаПрисадка, Наряд.Подразделение));
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.ОбратнаяПрисадка, Наряд.Подразделение));
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.ПрисадкаИндивидуальная, Наряд.Подразделение));
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.ПрисадкаЛДСП32ПодАнкер, Наряд.Подразделение));
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.ПрисадкаПодПетли, Наряд.Подразделение));
		МассивУслугПрисадок.Добавить(ЛексСерверПовтИсп.ПолучитьОбщуюНоменклатуру(Справочники.ОбщаяНоменклатура.ПрисадкаПодСтяжки, Наряд.Подразделение));
				
		Если ВставлятьРазделительСтраниц Тогда
			ТабДок.ВывестиГоризонтальныйРазделительСтраниц();
		КонецЕсли;
		
		КоличествоДеталейВНаряде = 0;
		
		Для Каждого Стр ИЗ Наряд.СписокСпецификаций Цикл
			
			Если Стр.Спецификация.КоличествоДеталей > 0 Тогда
				 КоличествоДеталейВНаряде = КоличествоДеталейВНаряде + Стр.Спецификация.КоличествоДеталей; 
			КонецЕсли;
			
		КонецЦикла;
		
		тзНормативы = ПолучитьНормативыПоЗонам(Наряд);
		
		НомерСтрокиНачало = ТабДок.ВысотаТаблицы + 1;
		
		ОбластьШапка = Макет.ПолучитьОбласть("Шапка"); //RonEXI: Чтобы очищались всё ячейки норматива
		ОбластьШапка.Параметры.Наряд = Наряд;
		ОбластьШапка.Параметры.ДатаИзготовления = Формат(Наряд.ДатаИзготовления, "ДФ=dd.MM.yy");
		ОбластьШапка.Параметры.КоличествоДеталей = КоличествоДеталейВНаряде;
		
		Инд = 1;
		
		Для Каждого Стр ИЗ тзНормативы Цикл
			
			ОбластьШапка.Параметры["Зона"+Инд] = Стр.ЦеховаяЗона;
			ОбластьШапка.Параметры["Норматив"+Инд] = Окр(Стр.Норматив);
			ОбластьШапка.Параметры["Часы"+Инд] = Формат(Стр.Норматив / 60,"ЧДЦ=1");
		...
		//RonEXI: Выводим материал листовой не ЛДСП, с порядком меньше номенклатурной группы ЛДСП16
		
		Запрос = Новый Запрос;
		Запрос.Параметры.Вставить("Наряд", Наряд);
		Запрос.Параметры.Вставить("ПорядокЛДСП16", Справочники.НоменклатурныеГруппы.ЛДСП16.ПорядокПечатиВРаскрое);
		Запрос.Текст =
		"ВЫБРАТЬ
		...
		|	И Спец.Ссылка = &Наряд
		|	И Порядок.Номенклатура.НоменклатурнаяГруппа.ПорядокПечатиВРаскрое < &ПорядокЛДСП16
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок.Номенклатура.НоменклатурнаяГруппа.ПорядокПечатиВРаскрое,
		|	Буква
		|ИТОГИ
		|	МАКСИМУМ(Порядок)
		|ПО
		...
		Если НЕ Результат.Пустой() Тогда
			...
			ТабДок.Вывести(ОбластьШапкаДоЛДСП);
			...
			Пока ВыборкаНом.Следующий() Цикл
				...
				ВыборкаСпец = ВыборкаНом.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
				
				Пока ВыборкаСпец.Следующий() Цикл
					
					Буква = Символ(64+ВыборкаСпец.Буква); 
					НомерСпец = ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ВыборкаСпец.Номер);
					
					СтрокаСпец = СтрокаСпец + "("+ Буква + ")" + НомерСпец + " ";
					
				КонецЦикла;
				
				ОбластьСтрокаДоЛДСП.Параметры.Порядок = Порядок;
				ОбластьСтрокаДоЛДСП.Параметры.Номенклатура = ВыборкаНом.Номенклатура;
				ОбластьСтрокаДоЛДСП.Параметры.Спецификации = СтрокаСпец;
				
				ТабДок.Вывести(ОбластьСтрокаДоЛДСП);
		...
		//RonEXI: Получаем таблицу кромок по спецухам.
		...
		Для Каждого Спец ИЗ Наряд.СписокСпецификаций Цикл
			...
			Для Каждого СтрокаПрисадка ИЗ Спец.СписокНоменклатуры Цикл
				
				Для Каждого У ИЗ МассивУслугПрисадок Цикл 
					Если СтрокаПрисадка.Номенклатура = У Тогда
		...
		Если НЕ Результат.Пустой() Тогда
			...
			МассивУжеПоказаныхКромок = Новый Массив();
			...
			Пока ВыборкаЦвет.Следующий() Цикл
				...
				ОбластьСтрокаТаблицаНоменклатура.Параметры.Порядок = Порядок;
				...
				Пока ВыборкаСпец.Следующий() Цикл
					...
					ВыводитьКромки = Истина;
					
					Для Каждого Н ИЗ МассивУжеПоказаныхКромок Цикл
						
						Если Н = ВыборкаСпец.Номер Тогда
							 ВыводитьКромки = Ложь;
						КонецЕсли;
					...
					Если (НЕ ВыводитьКромки) И (КромкиСпец.Количество() > 0) Тогда
						ОбластьСтрокаТаблица.Параметры.Выше = "КРОМКА ВЫШЕ!!!  БУКВА:  "+Символ(64+ВыборкаСпец.Буква);
					ИНАЧЕ
						ОбластьСтрокаТаблица.Параметры.Выше = "";
					КонецЕсли;
									
					ТабДок.Вывести(ОбластьСтрокаТаблица);
			
					Если ВыводитьКромки Тогда
						
						МассивУжеПоказаныхКромок.Добавить(ВыборкаСпец.Номер);
		...
		//RonEXI: Выводим материал листовой не ЛДСП, с порядком БОЛЬШЕ номенклатурной группы ЛДСП16
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				...
				Если Выборка.ВидПредоставления = 0 Тогда			
					ОбластьНеноменклатурныйСтрока.Рисунки.Удалить(ОбластьНеноменклатурныйСтрока.Рисунки.КартинкаПодЗаказ);
					ОбластьНеноменклатурныйСтрока.Рисунки.Удалить(ОбластьНеноменклатурныйСтрока.Рисунки.КартинкаПредоставляемый);
				ИначеЕсли Выборка.ВидПредоставления = 1 Тогда
					ОбластьНеноменклатурныйСтрока.Рисунки.Удалить(ОбластьНеноменклатурныйСтрока.Рисунки.КартинкаПредоставляемый);
				Иначе
					ОбластьНеноменклатурныйСтрока.Рисунки.Удалить(ОбластьНеноменклатурныйСтрока.Рисунки.КартинкаПодЗаказ);
				КонецЕсли;
...
Функция ПечатьХлыстМатСклад(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для Каждого Наряд ИЗ МассивОбъектов Цикл
		...
		|		ПО Склад.Номенклатура = Настройки.Номенклатура
		|ГДЕ
		|	Склад.Ссылка В
		|			(ВЫБРАТЬ
...
Функция ПолучитьНормативыПоЗонам(Наряд)
	
	Запрос = Новый Запрос();
	Запрос.УстановитьПараметр("Наряд", Наряд);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НарядЗаданиеСписокСпецификаций.Спецификация КАК Спец
	|ПОМЕСТИТЬ втСпецификации
	|ИЗ
	|	Документ.НарядЗадание.СписокСпецификаций КАК НарядЗаданиеСписокСпецификаций
	|ГДЕ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(СписокНом.ЗарплатаЦеха) КАК Норматив,
	|	СписокНом.Номенклатура.ЦеховаяЗона КАК ЦеховаяЗона
	|ИЗ
	...
	|				С.Спец
	|			ИЗ
	|				втСпецификации КАК С)
	|	И (СписокНом.Номенклатура.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Кромочная)
	|			ИЛИ СписокНом.Номенклатура.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.ФРС)
	|			ИЛИ СписокНом.Номенклатура.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Стеколка)
	|			ИЛИ СписокНом.Номенклатура.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Кухонная)
	|			ИЛИ СписокНом.Номенклатура.ЦеховаяЗона = ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Ящики))
	|	И СписокНом.Номенклатура.ВидНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ВидыНоменклатуры.Услуга)
	|
	|СГРУППИРОВАТЬ ПО
	|	СписокНом.Номенклатура.ЦеховаяЗона
	|
	|УПОРЯДОЧИТЬ ПО
	|	ЦеховаяЗона";
...
Функция КромочныйУчасток(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПараметрыПечати_КромочныйУчасток";
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ОтображатьСетку = Ложь;
	ТабДок.Защита = Истина;
	ТабДок.ТолькоПросмотр = Истина;
	...
	Макет = Документы.НарядЗадание.ПолучитьМакет("КромочныйУчасток");
	ОбластьЗаголовок = Макет.ПолучитьОбласть("Заголовок");
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Наряды", МассивОбъектов);
	Запрос.УстановитьПараметр("ПоказыватьСобранные", Истина);
	...
	|ИЗ
	|	Документ.НарядЗадание.СписокСпецификаций КАК СписокСпец
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Спецификация.СписокЯщики КАК Ящики
	|		ПО СписокСпец.Спецификация = Ящики.Ссылка
	...
	Пока Выборка.Следующий() Цикл
		...
		ВыборкаСпец = Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		Пока ВыборкаСпец.Следующий() Цикл
			
			Спецификация = ВыборкаСпец.Спецификация;
			ТзУслуги = ПолучитьУслугиПоСпецификации(Спецификация, Перечисления.ЦеховыеЗоны.Кромочная); 
			
			Если ТзУслуги.Количество() > 0 Тогда
				
				ОбластьСпецификация.Параметры.Спецификация = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаСпец.Спецификация.Номер);
				ОбластьСпецификация.Параметры.Норматив = ОКР(ТзУслуги.Итог("Норматив"));  
				ТабДок.Вывести(ОбластьСпецификация);
				
				ТзУслуги.Сортировать("Норматив Убыв");
				
				ТабДок.Вывести(ОбластьШапкаУслуги);
				
				Для Каждого Стр Из ТзУслуги Цикл
					
					ОбластьСтрокаУслуги.Параметры.Заполнить(Стр);
			...
			Если ТаблицаДеталиЯщиков.Количество() > 0 Тогда
				
				ТабДок.Вывести(ОбластьПустаяСтрока);
				ТабДок.Вывести(ОбластьШапкаЯщиков);
	
				ВыборкаЯщиков = ВыборкаСпец.Выбрать();
...
Функция СтекольныйУчасток(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПараметрыПечати_СтекольныйУчасток";
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ОтображатьСетку = Ложь;
	ТабДок.Защита = Истина;
	ТабДок.ТолькоПросмотр = Истина;
	...
	Макет = Документы.НарядЗадание.ПолучитьМакет("СтекольныйУчасток");
	ОбластьЗаголовок = Макет.ПолучитьОбласть("Заголовок");
	ОбластьСпецификация = Макет.ПолучитьОбласть("Спецификация");
	ОбластьПустаяСтрока = Макет.ПолучитьОбласть("ПустаяСтрока");
	ОбластьШапкаУслуги = Макет.ПолучитьОбласть("ШапкаУслуги");
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВыборкаСпец.Количество() > 0 Тогда
		...
		Пока ВыборкаСпец.Следующий() Цикл
			...
			Спецификация = ВыборкаСпец.Спецификация;
			ТзУслуги = ПолучитьУслугиПоСпецификации(Спецификация, Перечисления.ЦеховыеЗоны.Стеколка); 
...
Функция КриволинейныйУчасток(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабДок = Новый ТабличныйДокумент;
	ТабДок.ИмяПараметровПечати = "ПараметрыПечати_КриволинейныйУчасток";
	ТабДок.АвтоМасштаб = Истина;
	ТабДок.ОтображатьСетку = Ложь;
	ТабДок.Защита = Истина;
	ТабДок.ТолькоПросмотр = Истина;
	...
	Макет = Документы.НарядЗадание.ПолучитьМакет("КриволинейныйУчасток");
	ОбластьЗаголовок = Макет.ПолучитьОбласть("Заголовок");
	ОбластьСпецификация = Макет.ПолучитьОбласть("Спецификация");
	ОбластьПустаяСтрока = Макет.ПолучитьОбласть("ПустаяСтрока");
	ОбластьШапкаУслуги = Макет.ПолучитьОбласть("ШапкаУслуги");
	...
	ОбластьСтрокаДеталиСклейка = Макет.ПолучитьОбласть("СтрокаДеталиСклейка");
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаСпец.Следующий() Цикл
			...
			Спецификация = ВыборкаСпец.Спецификация;
			ТзУслуги = ПолучитьУслугиПоСпецификации(Спецификация, Перечисления.ЦеховыеЗоны.Криволинейная); 
			...
			ТзДетали = ПолучитьДеталиПоСпецификации(Спецификация, Перечисления.ЦеховыеЗоны.Криволинейная);
			
			Если ТзДетали.Количество() > 0 Тогда
				 
				ТабДок.Вывести(ОбластьШапкаДетали);

				Для Каждого Стр Из ТзДетали Цикл
					
					Если ЗначениеЗаполнено(Стр.НоменклатураДляСклеивания) Тогда
						ОбластьСтрокаДеталиСклейка.Параметры.Заполнить(Стр);
						ТабДок.Вывести(ОбластьСтрокаДеталиСклейка);
					Иначе
						ОбластьСтрокаДетали.Параметры.Заполнить(Стр);
...
Функция ПолучитьУслугиПоСпецификации(Спецификация, Зона)
	...
	|	СУММА(СпецификацияСписокНоменклатуры.ЗарплатаЦеха) КАК Норматив
	|ИЗ
	|	Документ.Спецификация.СписокНоменклатуры КАК СпецификацияСписокНоменклатуры
	|ГДЕ
	...
	|	И СпецификацияСписокНоменклатуры.Номенклатура.ЦеховаяЗона = &Зона
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПолучитьДеталиПоСпецификации(Спецификация, Зона)
	...
	"ВЫБРАТЬ
	|	Детали.Номенклатура,
	|	Детали.НоменклатураДляСклеивания КАК НоменклатураДляСклеивания,
	|	Детали.ВысотаДетали КАК Высота,
	|	Детали.ШиринаДетали КАК Ширина,
	|	Детали.Количество,
	|	Детали.НомерСтроки
	|ИЗ
	|	Документ.Спецификация.СписокДеталей КАК Детали
	|ГДЕ
	|	Детали.Ссылка = &Спецификация
	|	И Детали.МестоОбработки = &Зона
	|
	|УПОРЯДОЧИТЬ ПО
	|	Детали.НомерСтроки";
