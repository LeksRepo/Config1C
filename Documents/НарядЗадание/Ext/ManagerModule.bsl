...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПечатьРулонныйМатериал") Тогда
		ПодготовитьПечатнуюФорму("ПечатьРулонныйМатериал", "Рулонный материал", "Документ.НарядЗадание.РулонныйМатериал",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
...
Процедура ПодготовитьПечатнуюФорму(Знач ИмяМакета, ПредставлениеМакета, ПолныйПутьКМакету = "", МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода)
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, ИмяМакета) Тогда
		...
		ИначеЕсли ИмяМакета = "ПечатьРулонныйМатериал" Тогда
...
Функция ПечатьРулонныйМатериал(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = Документы.НарядЗадание.ПолучитьМакет("ПечатьРулонныйМатериал");
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	НарядЗаданиеСписокНоменклатурыПоСпецификациям.Спецификация.Номер,
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		Пока ВыборкаСпецификация.Следующий() Цикл
			...
			Пока ВыборкаНоменклатура.Следующий() Цикл
				...
				Если ВыборкаНоменклатура.КоличествоКривКромки > 0 Тогда
					ОбластьНоменклатураСтрока.Параметры.КоличествоКромки = Строка(ВыборкаНоменклатура.КоличествоВСпецификации - ВыборкаНоменклатура.КоличествоКривКромки) + " ( " + ВыборкаНоменклатура.КоличествоКривКромки + " ) "; 
				Иначе
					ОбластьНоменклатураСтрока.Параметры.КоличествоКромки = Строка(ВыборкаНоменклатура.КоличествоВСпецификации) + " ";
				КонецЕсли;
			...
			Пока ВыборкаПарковка.Следующий() Цикл
				...
				Пока ВыборкаНоменклатура.Следующий() Цикл
					...
					Пока ВыборкаСпецификация.Следующий() Цикл
						...
						Если ВыборкаСпецификация.НоменклатурнаяГруппа.ПринадлежитЭлементу(Справочники.НоменклатурныеГруппы.Кант)
							ИЛИ ВыборкаСпецификация.НоменклатурнаяГруппа.ПринадлежитЭлементу(Справочники.НоменклатурныеГруппы.Кромка) Тогда
							
							ОбластьСпецификацияСтрока.Параметры.КраткоеНаименованиеТорцевойЛенты = ВыборкаСпецификация.КраткоеНаименование;
						Иначе
							ОбластьСпецификацияСтрока.Параметры.КраткоеНаименованиеТорцевойЛенты = "";
						КонецЕсли;
						
						Если ВыборкаСпецификация.КоличествоКривКромки > 0 Тогда
							ОбластьСпецификацияСтрока.Параметры.КоличествоКромки = Строка(ВыборкаСпецификация.КоличествоВСпецификации - ВыборкаСпецификация.КоличествоКривКромки) + " ( " + ВыборкаСпецификация.КоличествоКривКромки + " ) "; 
						Иначе
...
Функция ПечатьПоследовательностьВыпуска(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для Каждого Наряд Из МассивОбъектов Цикл
		...
		Для Каждого Спец ИЗ Наряд.СписокСпецификаций Цикл
			...
			Для Каждого Изделие ИЗ Спец.СписокИзделийПоКаталогу Цикл
				...
				Если Присадка = "" Тогда
					...
					Если Изделие.Изделие.СписокПрисадок.Количество() > 0 Тогда
						Присадка = "Присад";	
					КонецЕсли;
			...
			Стр.СборкаПрисадка = Сборка; 
			Стр.СборкаПрисадка = ?(Стр.СборкаПрисадка <> "", Стр.СборкаПрисадка + ?(Присадка <> "", " / " + Присадка, ""), Присадка);   
			...
			Стр.КриволинейкаСклейка = Криволинейка; 
			Стр.КриволинейкаСклейка = ?(Стр.КриволинейкаСклейка <> "", Стр.КриволинейкаСклейка + ?(Склейка <> "", " / " + Склейка, ""), Склейка);
		...
		Запрос.Текст =
		"ВЫБРАТЬ
		|	Листы.Номенклатура КАК Номенклатура,
		|	Листы.Номенклатура.Наименование КАК Наименование,
		|	Листы.Спецификация КАК Спецификация,
		|	Листы.Спецификация.Номер КАК Номер,
		|	Спец.НомерСтроки КАК Буква
		|ИЗ
		|	Документ.НарядЗадание.СписокНоменклатурыПоСпецификациям КАК Листы
		|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.НарядЗадание.СписокСпецификаций КАК Спец
		|		ПО Листы.Спецификация = Спец.Спецификация
		|ГДЕ
		|	Листы.Ссылка = &Наряд
		|	И Листы.Номенклатура.НоменклатурнаяГруппа = ЗНАЧЕНИЕ(Справочник.НоменклатурныеГруппы.ЛДСП16)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Наименование,
		|	Буква
