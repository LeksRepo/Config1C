...
Функция ПечатьМатериалПоПарковкам(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	//RonEXI: Сначала выбираем криволинейные кромки по спецухам
	
	тзКромки = Новый ТаблицаЗначений;
	тзКромки.Колонки.Добавить("Наряд", Новый ОписаниеТипов("ДокументСсылка.НарядЗадание"));
	тзКромки.Колонки.Добавить("Спецификация", Новый ОписаниеТипов("ДокументСсылка.Спецификация"));
	тзКромки.Колонки.Добавить("Кромка", Новый ОписаниеТипов("СправочникСсылка.Номенклатура"));
	тзКромки.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число"));
	...
	|	Детали.Ссылка КАК Спецификация,
	|	Детали.ВысотаДетали,
	|	Детали.ВыборМебельнойКромкиСверху,
	|	Детали.ВыборМебельнойКромкиСлева,
	|	Детали.ВыборМебельнойКромкиСнизу,
	|	Детали.ВыборМебельнойКромкиСправа,
	|	Детали.СтрокаДляФлэш,
	|	Детали.ДетальРедактированная,
	|	Детали.ШиринаДетали,
	|	Детали.Количество,
	|	Детали.РадиусЛевоВерх,
	|	Детали.РадиусЛевоНиз,
	|	Детали.РадиусПравоВерх,
	|	Детали.РадиусПравоНиз,
	|	Детали.Срез,
	|	Детали.КантыИзРедактора
	|ИЗ
	|	Документ.Спецификация.СписокДеталей КАК Детали
	|		ЛЕВОЕ СОЕДИНЕНИЕ втСпецификации КАК Спец
	|		ПО Детали.Ссылка = Спец.Спецификация
	|ГДЕ
	|	Детали.Ссылка В
	|			(ВЫБРАТЬ
	|				Спец.Спецификация
	...
	|	И (Детали.ДетальРедактированная
	|			ИЛИ (Детали.ВыборМебельнойКромкиСверху <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|				ИЛИ Детали.ВыборМебельнойКромкиСлева <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|				ИЛИ Детали.ВыборМебельнойКромкиСнизу <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
	|				ИЛИ Детали.ВыборМебельнойКромкиСправа <> ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка))
	|				И (Детали.РадиусЛевоВерх > 0
	|					ИЛИ Детали.РадиусЛевоНиз > 0
	|					ИЛИ Детали.РадиусПравоВерх > 0
	|					ИЛИ Детали.РадиусПравоНиз > 0))";
	
	тзДетали = Запрос.Выполнить().Выгрузить();
		
	ПИ = 3.1415;
	
	Для Каждого Деталь ИЗ тзДетали Цикл
		
		Если Деталь.ДетальРедактированная И ЗначениеЗаполнено(Деталь.КантыИзРедактора) Тогда
			...
			Для Каждого СтрокаКант Из МассивКантов Цикл
				...
				Если ЗначениеЗаполнено(НоменклатураКант) И ТипКромления <> 0 Тогда

					Стр = тзКромки.Добавить();
					Стр.Наряд = Деталь.Наряд;
					Стр.Спецификация = Деталь.Спецификация;
					Стр.Кромка = НоменклатураКант;
					Стр.Количество = (ДлинаКанта * 2) * Деталь.Количество;     
		...
		Иначе
			...
			Мас = Новый Массив();
			Мас.Добавить(Новый Структура("Радиус, Кромка, Срез", Деталь.РадиусЛевоВерх, Деталь.ВыборМебельнойКромкиСверху, Ложь));
			Мас.Добавить(Новый Структура("Радиус, Кромка, Срез", Деталь.РадиусЛевоНиз, Деталь.ВыборМебельнойКромкиСнизу, Ложь));
			Мас.Добавить(Новый Структура("Радиус, Кромка, Срез", Деталь.РадиусПравоВерх, Деталь.ВыборМебельнойКромкиСверху, Ложь));
			Мас.Добавить(Новый Структура("Радиус, Кромка, Срез", Деталь.РадиусПравоНиз, Деталь.ВыборМебельнойКромкиСнизу, Деталь.Срез));
			
			Для Каждого Сторона ИЗ Мас Цикл
				
				Если НЕ Сторона.Срез И Сторона.Радиус > 0 И ЗначениеЗаполнено(Сторона.Кромка) Тогда
					
					Стр = тзКромки.Добавить();
					Стр.Наряд = Деталь.Наряд;
					Стр.Спецификация = Деталь.Спецификация;
					Стр.Кромка = Сторона.Кромка;
					Стр.Количество = (Сторона.Радиус * ПИ / 2) + (Деталь.ШиринаДетали - Сторона.Радиус) + (Деталь.ВысотаДетали - Сторона.Радиус);     
					Стр.Количество = Стр.Количество * Деталь.Количество;
	...
	тзКромки.Свернуть("Наряд, Спецификация, Кромка", "Количество");
	...
	Запрос.Параметры.Вставить("КривКромки", тзКромки);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	кКромки.Наряд,
	|	кКромки.Спецификация,
	|	кКромки.Кромка,
	|	ВЫРАЗИТЬ(кКромки.Количество / 1000 + 0.4999 КАК ЧИСЛО(15, 0)) КАК Количество
	|ПОМЕСТИТЬ втКривКромки
	|ИЗ
	|	&КривКромки КАК кКромки
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
		...
		Пока ВыборкаВидМатериала.Следующий() Цикл
			...
			Пока ВыборкаПарковка.Следующий() Цикл
				...
				Пока ВыборкаНоменклатура.Следующий() Цикл
					...
					АдресХранения = ВыборкаНоменклатура.АдресХранения;
					...
					Пока ВыборкаСпецификация.Следующий() Цикл
						...
						Если ВыборкаСпецификация.КоличествоКривКромки > 0 Тогда
							ОбластьСпецификацияСтрока.Параметры.КоличествоКромки = Строка(ВыборкаСпецификация.КоличествоВСпецификации) + " ( " + ВыборкаСпецификация.КоличествоКривКромки + " ) "; 
						Иначе
							ОбластьСпецификацияСтрока.Параметры.КоличествоКромки = Строка(ВыборкаСпецификация.КоличествоВСпецификации) + " ";
						КонецЕсли;
						...
						ОбластьСпецификацияСтрока.Параметры.АдресХранения = АдресХранения;
