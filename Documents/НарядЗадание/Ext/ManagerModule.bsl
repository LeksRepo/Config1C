...
Функция ПечатьХлыстовойМатериал(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	//ТаблицаРезультатов.Свернуть("Номенклатура", "РазмерОтрезка");
	...
	Пока ВыборкаДокументы.Следующий() Цикл
		...
		ВыборкаДляОбщегоКоличества = ВыборкаДокументы.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура ВыгрузитьСтрокиDXF(Документ) Экспорт
	...
	Если ЗначениеЗаполнено(Документ) 
	   ...
	   И ЗначениеЗаполнено(Документ.Подразделение.АдресКомпьютераЧПУ) Тогда
		...
		Каталог = Документ.Подразделение.АдресКомпьютераЧПУ;
		
		НомерНаряда = СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(Документ.Номер, "0", "Слева");
		ДатаИзготовления = Формат(Документ.ДатаИзготовления, "ДЛФ=D");
		...
		Если НЕ Результат.Пустой() Тогда
			...
			Исключение
				...
				Сообщить("Не удалось выгрузить чертежи для ЧПУ");
			...
			Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(Выборка.СтрокаDXF) Тогда
					 ...
					 СтрокаDXF = Выборка.СтрокаDXF;
					 НомерСпец = СтроковыеФункцииКлиентСервер.УдалитьПовторяющиесяСимволы(Выборка.Номер, "0", "Слева");
