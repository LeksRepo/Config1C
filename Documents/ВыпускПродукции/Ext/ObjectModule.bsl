...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Пока ВыборкаСпецификация.Следующий() Цикл
		...
		Если НЕ ВыборкаСпецификация.ПроведенаКомплектация Тогда
	...
	Для Каждого Строка Из СписокСпецификаций Цикл
		...
		Проводка.СчетКт = ПланыСчетов.Управленческий.ИзделияВЦеху;
	...
	СписатьДополнительныйЛимитЦеха();
...
Функция СписатьДополнительныйЛимитЦеха()
	...
	|	ВыпускПродукцииСписокСпецификаций.Ссылка.Подразделение
	...
	|			,
	|			(Подразделение, Субконто2) В
	|				(ВЫБРАТЬ
	|					т.Подразделение,
	|					т.Спецификация
	|				ИЗ
	|					втСпецификации КАК т)) КАК УправленческийОстатки
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Проводка.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.Спецификации] = ВыборкаДетальныеЗаписи.Спецификация;
...
Процедура НачислитьЗарплатуЦеху()
	...
	|	И Список.Номенклатура.ЦеховаяЗона <> ЗНАЧЕНИЕ(Перечисление.ЦеховыеЗоны.Офис)
