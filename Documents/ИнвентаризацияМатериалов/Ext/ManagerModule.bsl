...
Процедура ПечатьИнвентаризационнаяОпись(ТабДокумент, МассивОбъектов) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ВыборкаСтрокМатериалы = Выборка.Материалы.Выбрать();
...
//		ОбластьМакета.Параметры.ДатаОкончанияИнвентаризацииЛокальныйФормат  = Выборка.ДатаОкончанияИнвентаризации; 
		...
		Пока ВыборкаСтрокМатериалы.Следующий() Цикл
			...
			Если НЕ ПроверитьВыводТабличногоДокумента(ТабДокумент, СтрокаСПодвалом) Тогда
				...
				ПараметрыОбласти.СуммаФактическиНаСтраницеПрописью 					= ЧислоПрописью(ИтогФактСумма, 
			...
			ИтогФактСуммаВсего 		= ИтогФактСуммаВсего + ВыборкаСтрокМатериалы.Сумма;
			ИтогоКолво 		   		= ИтогоКолво 		 + ВыборкаСтрокМатериалы.КоличествоФакт; 
			ИтогФактСумма	   		= ИтогФактСумма 	 + ВыборкаСтрокМатериалы.Сумма;
		...
		ОбластьМакета.Параметры.СуммаФактическиНаСтраницеПрописью = ЧислоПрописью(ИтогФактСуммаВсего, 
		"Л = ru_RU; ДП = Истина","рубль, рубля, рублей, м, копейка, копейки, копеек, ж, 2");
