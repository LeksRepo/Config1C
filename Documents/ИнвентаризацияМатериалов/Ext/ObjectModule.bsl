...
//	
//	// dmn 15.08.2011
//	// На знаю как задумывалось, но предположу что этим же документом Инвентаризация оприходуем излишки и списываем недостачи
//	
//	Запрос = Новый Запрос;
//	Запрос.Текст = 
//	"ВЫБРАТЬ
//	|	ИнвентаризацияМатериаловМатериалы.Номенклатура,
//	|	ИнвентаризацияМатериаловМатериалы.Отклонение,
//	|	ИнвентаризацияМатериаловМатериалы.Цена,
//	|	ИнвентаризацияМатериаловМатериалы.ЕдиницаИзмерения
//	|ИЗ
...
//		
//		Проводки = Движения.МатериалыНаСкладах.Добавить();
//		Проводки.Период = Дата;
//		Проводки.Склад = Склад;
//		Проводки.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения;
//		Проводки.Номенклатура = Выборка.Номенклатура;


//		// оприходуем излишки
//		Если Выборка.Отклонение > 0 Тогда 
//			Проводки.ВидДвижения = ВидДвиженияНакопления.Приход;
//			Проводки.Количество = Выборка.Отклонение;
//			Проводки.Стоимость = Выборка.Отклонение * Выборка.Цена;
//		ИначеЕсли Выборка.Отклонение < 0 Тогда
//		// спишем недостачу
//			Проводки.ВидДвижения = ВидДвиженияНакопления.Расход;
//			Проводки.Количество = -Выборка.Отклонение;
//			Проводки.Стоимость = -Выборка.Отклонение * Выборка.Цена;
//		КонецЕсли;
//	КонецЦикла;
