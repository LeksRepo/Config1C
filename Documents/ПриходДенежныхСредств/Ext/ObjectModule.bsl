...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	// В рабочей базе проставить флаг НачислятьБонусЗаОплатуНаличными
	// для документов где есть сумма бонуса, после этого можно добавить в условие.
	
	Если БонусЗаОплатуНаличными > 0 Тогда
		
		СформироватьПроводкуБонусЗаОплатуНаличными();
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если СуммаДокумента < 0 Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Сумма документа должна быть больше 0.");
		Отказ = Истина;
	КонецЕсли;
	
	Если СчетДт = ПланыСчетов.Управленческий.ОперационнаяКасса
		И (Дата < '2019.04.10' ИЛИ НачислятьБонусЗаОплатуНаличными) Тогда // Костыль
		БонусЗаОплатуНаличными = ЛексСервер.РассчитатьБонусЗаОплатуНаличными(Подразделение, Дата, СуммаДокумента);
	КонецЕсли;
...
Функция СформироватьПроводкуБонусЗаОплатуНаличными()
	
	Проводка = Движения.Управленческий.Добавить();
	Проводка.Подразделение = Подразделение;
	Проводка.Период = Дата;
	Проводка.Сумма = БонусЗаОплатуНаличными;
	Проводка.Содержание = "Бонус клиенту за оплату наличными";
	
	Проводка.СчетДт = ПланыСчетов.Управленческий.Расходы;
	Проводка.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.СтатьиДР] = Справочники.СтатьиДоходовРасходов.ПрочиеРасходы;
	
	Проводка.СчетКт = СчетКт;
	
	Для Каждого Элемент Из СчетКт.ВидыСубконто Цикл
		
		СубконтоКт = ЭтотОбъект["Субконто" +Элемент.НомерСтроки+ "Кт"];
		
		Если СубконтоКт = Документы.Спецификация.ПустаяСсылка() Тогда
			СубконтоКт = Неопределено;
		КонецЕсли;
		
		Проводка.СубконтоКт[Элемент.ВидСубконто] = СубконтоКт;
