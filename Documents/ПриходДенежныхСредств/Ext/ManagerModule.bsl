...
Функция ПечатьТоварныйЧек(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|ИЗ
	|	Документ.ПриходДенежныхСредств КАК ПриходДенежныхСредств
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДолжностиСотрудников.СрезПоследних(&дата, ) КАК ДолжностиСотрудниковСрезПоследних
	|		ПО ПриходДенежныхСредств.Автор.ФизическоеЛицо = ДолжностиСотрудниковСрезПоследних.ФизЛицо
	|ГДЕ
	|	ПриходДенежныхСредств.Ссылка В(&Ссылка)
	|	И ПриходДенежныхСредств.Проведен";
