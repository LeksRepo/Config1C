...
Процедура Субконто2ДтНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТипЗнч(Объект.Субконто2Дт) = Тип("СправочникСсылка.Счета") Тогда
		СтандартнаяОбработка = Ложь;		
		ПараметрыФормы = Новый Структура;
		ПараметрыФормы.Вставить("ПринадлежитПодразделению", Объект.Подразделение);
		
		ВыбранныйСчет = ОткрытьФормуМодально("Справочник.Счета.ФормаВыбора", Новый Структура("Отбор", ПараметрыФормы));
		Если ВыбранныйСчет <> Неопределено Тогда
			Объект.Субконто2Дт = ВыбранныйСчет;
		КонецЕсли;
	КонецЕсли;
