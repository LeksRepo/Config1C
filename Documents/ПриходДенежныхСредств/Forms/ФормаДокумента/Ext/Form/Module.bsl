...
Процедура ВидОперацииПриИзменении(Элемент = Неопределено)
	...
	Подотчет = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОпераций.ВозвратОтПодотчетногоЛица");
	
	Элементы.Субконто1Дт.Доступность = Не Подотчет;
	Элементы.Субконто1Кт.Доступность = Не Подотчет;
	
	Если Подотчет Тогда
		
		Объект.Субконто1Дт = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенежныхСредств.ПоступлениеПодотчета");
		Объект.Субконто1Кт = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенежныхСредств.ВыплатаПодотчета");
