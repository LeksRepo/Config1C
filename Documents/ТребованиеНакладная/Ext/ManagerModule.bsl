...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПечатьХлыстовойМатериал") Тогда
		ПодготовитьПечатнуюФорму("ПечатьХлыстовойМатериал", "Хлыстовой материал", "Документ.ТребованиеНакладная.ПечатьХлыстовойМатериал",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода);
	КонецЕсли;
...
Функция СформироватьТаблицуМатериалов(НарядЗадание) Экспорт
	...
	ТЗ.Колонки.Добавить("НомерХлыста", Новый ОписаниеТипов("Число"));
	...
	Пока ВыборкаНоменклатура.Следующий() Цикл
		...
		Пока ВыборкаХлысты.Следующий() Цикл
			...
			Если ВыборкаХлысты.РазмерОстатка > 0 ИЛИ ИзОстатка Тогда
				...
				НоваяСтрока.НомерХлыста = ВыборкаХлысты.НомерХлыста; 
...
Функция ПечатьХлыстовойМатериал(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("МассивТребований", МассивОбъектов);
	Запрос.Параметры.Вставить("Подразделение", МассивОбъектов[0].Подразделение);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Требование.НарядЗадание КАК НарядЗадание
	|ПОМЕСТИТЬ втМассивНарядов
	|ИЗ
	|	Документ.ТребованиеНакладная КАК Требование
	|ГДЕ
	|	Требование.Ссылка В(&МассивТребований)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|			КОНЕЦ
	|	КОНЕЦ КАК ВидПредоставления,
	|	ТребованиеСписок.Отпущено КАК Отпущено,
	|	ТребованиеСписок.Номенклатура.ЕдиницаИзмерения КАК ТребованиеЕдиницаИзмерения,
	|	ТребованиеДокумент.Ссылка КАК ТребованиеСсылка
	|ИЗ
	|	Документ.НарядЗадание.РаскройХлыстов КАК НарядЗаданиеРаскройХлыстов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиНоменклатуры.СрезПоследних(, Подразделение = &Подразделение) КАК НастройкиНоменклатуры
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТребованиеНакладная.СписокНоменклатуры КАК ТребованиеСписок
	|		ПО (ТребованиеСписок.Ссылка.НарядЗадание = НарядЗаданиеРаскройХлыстов.Ссылка)
	|			И (ТребованиеСписок.НомерХлыста = НарядЗаданиеРаскройХлыстов.НомерХлыста)
	|			И (ТребованиеСписок.Номенклатура = НарядЗаданиеРаскройХлыстов.Номенклатура
	|				ИЛИ ТребованиеСписок.Номенклатура.БазоваяНоменклатура = НарядЗаданиеРаскройХлыстов.Номенклатура)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ТребованиеНакладная КАК ТребованиеДокумент
	|		ПО (ТребованиеДокумент.НарядЗадание = НарядЗаданиеРаскройХлыстов.Ссылка)
	|ГДЕ
	|	НарядЗаданиеРаскройХлыстов.Ссылка В
	|			(ВЫБРАТЬ
	|				Наряды.НарядЗадание
	|			ИЗ
	|				втМассивНарядов КАК Наряды)
	|
	|УПОРЯДОЧИТЬ ПО
	...
	|	МАКСИМУМ(АдресХранения),
	|	МАКСИМУМ(Отпущено),
	|	МАКСИМУМ(ТребованиеЕдиницаИзмерения),
	|	МАКСИМУМ(ТребованиеСсылка)
	|ПО
	|	Ссылка,
