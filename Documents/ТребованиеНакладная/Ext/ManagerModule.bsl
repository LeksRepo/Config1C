...
Функция ВывестиКонтрольРасходаМатериалов(ТабДок, Период, Подразделение)
	...
	Настройки.ПараметрыДанных.Элементы[2].Значение = Новый СтандартныйПериод(НачалоДня(Период), КонецДня(Период));
