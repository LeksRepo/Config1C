...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если ПроверитьТребованияНакладные(Ошибки) Тогда
	...
	Если НЕ Отказ Тогда
		...
		//ОприходоватьОбрезкиХлыстовогоМатериала();
...
Функция ОприходоватьОбрезкиХлыстовогоМатериала()
	...
	Запрос = Новый Запрос;
	Запрос.Параметры.Вставить("НарядЗадание", НарядЗадание);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	НарядЗаданиеРаскройХлыстов.НомерХлыста,
	|	СУММА(НарядЗаданиеРаскройХлыстов.Размер) КАК РазмерОтрезка,
	...
	|	НарядЗаданиеРаскройХлыстов.Номенклатура.Кратность";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		Остаток = Выборка.РазмерХлыста - Выборка.РазмерОтрезка;
		Если Остаток > Выборка.Кратность * 1000 Тогда
			
			Проводка = Движения.ОбрезкиХлыстовойМатериал.ДобавитьПриход();
			Проводка.Период = Дата;
			Проводка.Подразделение = Подразделение;
			Проводка.Номенклатура = Выборка.Номенклатура;
			Проводка.Размер = Остаток;
...
Функция ПроверитьТребованияНакладные(Ошибки);
	...
	ТабНоменклатура = Документы.ТребованиеНакладная.СформироватьТаблицуМатериалов(НарядЗадание);
	...
	|				ИЛИ СписокНом.Номенклатура = СписокНомНаряд.Номенклатура.БазоваяНоменклатура)
	|ГДЕ
