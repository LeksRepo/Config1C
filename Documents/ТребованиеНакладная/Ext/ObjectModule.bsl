...
Процедура ПроверитьСчетТребованияНакладные(Документ, Ошибки, Отказ);
	...
	Если ЗначениеЗаполнено(Документ.НарядЗадание) Тогда
		...
		|	Остатки.Субконто2 КАК ОстаткиНоменклатура
		|ИЗ
		|	Документ.ТребованиеНакладная.СписокНоменклатуры КАК СписокНом
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Управленческий.Остатки(, Счет = ЗНАЧЕНИЕ(ПланСчетов.Управленческий.ТребованияНакладные), , Субконто1 = &Наряд) КАК Остатки
		|		ПО (СписокНом.Номенклатура = Остатки.Субконто2
		|				ИЛИ СписокНом.Номенклатура.БазоваяНоменклатура = Остатки.Субконто2)
		|ГДЕ
		|	СписокНом.Ссылка = &Ссылка
		|	И Остатки.Субконто2 ЕСТЬ NULL ";
