...
Функция ПолучитьВопросыПоТеме(ПравоваяБаза)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю("По теме "+ ПравоваяБаза +" нет вопросов");
	КонецЕсли;
...
Процедура ОбновитьФорму()
	Элементы.ТекущийВопрос.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Вопрос %1 из %2", ТекущийВопрос+1, КолвоВопросов);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("ПравоваяБаза") Тогда
		...
		КолвоВопросов = Параметры.КоличествоВопросов;
		Элементы.Тема.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("Тема теста: %1", ПравоваяБаза);
