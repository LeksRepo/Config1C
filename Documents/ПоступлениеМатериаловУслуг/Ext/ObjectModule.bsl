...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура")
		...
		|ГДЕ
		|	Неном.Ссылка = &Ссылка
		|	И Неном.КоличествоПоставщик > 0
		|	И Неном.Поставщик = &Контрагент
		|
		|УПОРЯДОЧИТЬ ПО
		|	Порядок";
		...
		Для Каждого Стр ИЗ ТЗ Цикл
			 ...
			 НоваяСтрока.Содержание = Стр.Комментарий;
			 ...
			 Если ЗначениеЗаполнено(Стр.СпецНомер) Тогда
				НомерСпец = ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(Стр.СпецНомер);
			 	НоваяСтрока.Содержание = НоваяСтрока.Содержание + " Спец: " + НомерСпец + Стр.Комментарий;
