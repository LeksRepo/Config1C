...
Функция СформироватьДвиженияУслуги()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.Офис) Тогда
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ИнвентаризацияМатериалов") Тогда
		...
		ВидОприходования = Перечисления.ХозяйственнаяОперация.ОприходованиеМатериалов;
		Подразделение = Склад.Владелец;
		Организация = Подразделение.Владелец;
		
		Запрос = Новый Запрос;
		Запрос.Текст =
		"ВЫБРАТЬ
		|	ИнвентаризацияМатериаловМатериалы.Номенклатура,
		|	ИнвентаризацияМатериаловМатериалы.Отклонение КАК Отклонение,
		|	ИнвентаризацияМатериаловМатериалы.ЕдиницаХранения,
		|	ИнвентаризацияМатериаловМатериалы.Цена,
		|	ИнвентаризацияМатериаловМатериалы.Цена * ИнвентаризацияМатериаловМатериалы.Отклонение КАК Стоимость,
		|	ИнвентаризацияМатериаловМатериалы.ЕдиницаХранения.Коэффициент КАК Коэффициент
		|ИЗ
		|	Документ.ИнвентаризацияМатериалов.Материалы КАК ИнвентаризацияМатериаловМатериалы
		|ГДЕ
		|	ИнвентаризацияМатериаловМатериалы.Отклонение > 0
		|	И ИнвентаризацияМатериаловМатериалы.Ссылка = &Ссылка";
		...
		Пока Выборка.Следующий() Цикл
			...
			НоваяСтрока.Количество = Выборка.Отклонение;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	ИначеЕсли ВидОперации = Перечисления.ХозяйственнаяОперация.ЗакупкаУПоставщика Тогда
