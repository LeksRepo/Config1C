...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") 
		...
		|	ПодЗаказ.КоличествоПоставщик КАК Количество
		|ИЗ
		|	Документ.ОперативныйЗакуп.НоменклатураПодЗаказ КАК ПодЗаказ
		|ГДЕ
		|	ПодЗаказ.Ссылка = &Ссылка
		|	И ПодЗаказ.КоличествоПоставщик > 0
		|	И ПодЗаказ.Поставщик = &Контрагент";
