...
Функция СформироватьДвиженияМатериалы(фСчетКт)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Номенклатура.Базовый Тогда
			КоличествоБазовых = Выборка.Количество;
			Номенклатура = Выборка.Номенклатура;
		Иначе
			Номенклатура = Выборка.Номенклатура.БазоваяНоменклатура;
			КоличествоБазовых = Выборка.Количество * Выборка.Номенклатура.КоэффициентБазовых;
		КонецЕсли;
		...
		Если КоличествоБазовых <> 0 Тогда
			...
			Проводка.Период = Дата;
			Проводка.Подразделение = Подразделение;
			Проводка.Сумма = Выборка.Сумма / КоличествоБазовых;
			Проводка.Номенклатура = Номенклатура;
