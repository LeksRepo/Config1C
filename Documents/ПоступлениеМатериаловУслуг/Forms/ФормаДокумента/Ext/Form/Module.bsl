...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Объект.ИнтервалМеждуЗаказами = 20;	
	КонецЕсли;
...
Функция ПолучитьПлановуюЦену(Номенклатура, Подразделение)
	...
	Цена = РегистрыСведений.НастройкиНоменклатуры.ПолучитьПоследнее(ТекущаяДата(), Отбор).ПлановаяЗакупочная;
...
Процедура ЗаполнитьНаСервере()
	...
	|ИЗ
	|	втМатериалы КАК Таб
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиНоменклатуры.СрезПоследних(&Период, Подразделение = &Подразделение) КАК Цены
	|		ПО (ВЫБОР
	|				КОГДА Таб.Номенклатура.Базовый
