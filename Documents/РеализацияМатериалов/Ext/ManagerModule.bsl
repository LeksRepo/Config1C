...
Функция ПечатьТоварныйЧек(МассивДокументов, ОбъектыПечати) Экспорт
	...
	|	НомПодразделений.АдресХранения КАК АдресХранения,
	|	РеализацияМатериалов.Ссылка.АдресДоставки
	|ИЗ
	|	Документ.РеализацияМатериалов.СписокНоменклатуры КАК РеализацияМатериалов
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НоменклатураПодразделений.СрезПоследних(, ) КАК НомПодразделений
	|		ПО РеализацияМатериалов.Номенклатура = НомПодразделений.Номенклатура
	|			И РеализацияМатериалов.Ссылка.Подразделение = НомПодразделений.Подразделение
	|ГДЕ
