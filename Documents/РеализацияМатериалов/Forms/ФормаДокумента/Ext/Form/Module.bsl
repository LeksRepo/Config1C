...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если Объект.ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиента Тогда
		
		Если  Объект.АдресДоставки = "Введите адрес" Тогда
			ТекстСообщения = "Не введен адрес доставки";
		...
		Если НЕ ЗначениеЗаполнено(Объект.ДатаОтгрузки) Тогда
			ТекущийОбъект.ДатаОтгрузки = ТекущаяДата();
		КонецЕсли;
		СтруктураНорматива = Документы.Спецификация.ПолучитьСтруктуруНорматива(ТекущийОбъект.ДатаОтгрузки, ТекущийОбъект.Подразделение);
		Норматив = СтруктураНорматива.Норматив;
		Если Норматив = 0 Тогда
			ТекстСообщения = "" +Формат(ТекущийОбъект.ДатаОтгрузки, "ДФ=dd.MM.yyyy") + " - выходной. Выберите другой день отгрузки.";
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, , ТекстСообщения);
		КонецЕсли;
