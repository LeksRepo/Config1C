...
Процедура ЗаполнитьЦенуИКрастность(ТекущиеДанные)
	...
	Парам.Вставить("Офис", Объект.Офис);
...
Функция ПолучитьДанныеНоменклатуры(Парам)
	...
	Если Парам.Офис <> ПредопределенноеЗначение("Справочник.Офисы.ПустаяСсылка") Тогда
		Ответ.Вставить("Цена",Цена*Парам.Офис.КоэффициентМатериалы);
	Иначе
