...
Функция ПроверитьОстатокНаСкладе()
	...
	|	ВЫБОР
	|		КОГДА СпрНом.Ссылка.Базовый
	|			ТОГДА СпрНом.Ссылка
	|		ИНАЧЕ СпрНом.Ссылка.БазоваяНоменклатура
	|	КОНЕЦ КАК Базовая,
	...
	|		ПО СписокНом3.Базовая = ОстаткиНаСкладе.Субконто2
	...
	Для Каждого Строка Из ТЗ Цикл
		...
		Если Строка.Перерасход > 0 Тогда
			...
			СтрокаОтвет = СтрокаОтвет + Строка.Номенклатура + "   :   превышение: " + Строка.Перерасход + " " + Строка.Номенклатура.ЕдиницаИзмерения + Символы.ПС; 
