...
Функция ПолучитьАдресТаблицы()
	...
	Таблица.Колонки.РазмерЗаготовки.Имя = "Длина"; // Костыль
...
Функция ЗагрузитьОбрезкиХлыстовогоМатериала(фнАдресТаблицы)
	...
	Для Каждого СтрокаПодбора Из Таблица Цикл
		...
		НоваяСтрока.РазмерЗаготовки = СтрокаПодбора.Размер;
		НоваяСтрока.РазмерПродаваемый = НоваяСтрока.РазмерЗаготовки;
		НоваяСтрока.РазмерКлиенту = НоваяСтрока.РазмерЗаготовки;
...
Процедура ОбрезкиХлыстовогоМатериалаРазмерКлиентуПриИзменении(Элемент)
	...
	ОстатокОтЗаготовки = ТекущиеДанные.РазмерЗаготовки - ТекущиеДанные.РазмерКлиенту;
	
	Если ОстатокОтЗаготовки < 0 Тогда
		ТекущиеДанные.РазмерКлиенту = ТекущиеДанные.РазмерЗаготовки;
		ОстатокОтЗаготовки = 0;
	КонецЕсли;
	
	Если ОстатокОтЗаготовки < ТекущиеДанные.Кратность * 1000 Тогда
		ТекущиеДанные.РазмерПродаваемый = ТекущиеДанные.РазмерЗаготовки;
	Иначе
		ТекущиеДанные.РазмерПродаваемый = ТекущиеДанные.РазмерКлиенту;
	КонецЕсли;
