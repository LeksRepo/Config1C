...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтаФорма.Элементы.УвеличитьЦеныНаПроцент.Доступность = РазрешеноИзменятьЦену;
	ЭтаФорма.Элементы.УменшитьЦенуНаПроцент.Доступность = РазрешеноИзменятьЦену;
...
&НаКлиенте
Процедура УвеличитьЦеныНаПроцент(Команда)
	
	Для каждого Строка Из Объект.СписокНоменклатуры Цикл
	
		Строка.Цена = Строка.Цена + (Строка.Цена / 100 * Процент);
...
&НаКлиенте
Процедура УменшитьЦенуНаПроцент(Команда)
	
	Для каждого Строка Из Объект.СписокНоменклатуры Цикл
	
		Строка.Цена = ?(Процент > 0, Строка.Цена - (Строка.Цена / 100 * Процент), 0);
		Строка.Сумма = Строка.Количество * Строка.Цена;
