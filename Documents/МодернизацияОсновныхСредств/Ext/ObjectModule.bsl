...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	|	СостояниеОССрезПоследних.СрокАмортизации КАК СтарыйСрокАмортизации,
	|	СостояниеОССрезПоследних.ПервоначальнаяСтоимость КАК ПервоначальнаяСтоимость,
	|	тчДок.НомерСтроки,
	...
	|	СостояниеОССрезПоследних.НачислятьАмортизацию КАК СтараяНачислятьАмортизацию,
	|	УправленческийОстатки.СуммаОстатокДт КАК СтараяСтоимость
	|ИЗ
	|	тчДок КАК тчДок
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ПроверкаПодразделения) Тогда
			...
			Если НЕ Выборка.ПринятоКУчету ИЛИ Выборка.СтараяСтоимость = 0 Тогда
			...
			СтрокаТаблицы = ОсновныеСредства.Найти(Выборка.ОсновноеСредство);
			СтрокаТаблицы.СтараяНачислятьАмортизацию = Выборка.СтараяНачислятьАмортизацию;
			СтрокаТаблицы.СтараяСтоимость = Выборка.СтараяСтоимость;
			СтрокаТаблицы.СтарыйСрокАмортизации = Выборка.СтарыйСрокАмортизации;
			...
			Проводки.СрокАмортизации = Выборка.НовыйСрокАмортизации + Выборка.СтарыйСрокАмортизации;
