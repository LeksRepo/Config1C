...
Процедура ДвиженияПоказатели()
	...
	Если ТипСубконто2 = Тип("ДокументСсылка.Договор") Тогда
		ФизЛицо = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(Субконто2Дт.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
	...
	ИначеЕсли ТипСубконто2 = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
		ФизЛицо = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(Субконто2Дт.Договор.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
		ДобавитьПоказательСотруднику(ФизЛицо, Перечисления.ВидыПоказателейСотрудников.Выручка, Сумма, Субконто2Дт);
