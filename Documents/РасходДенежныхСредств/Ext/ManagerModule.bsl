	...
	Для каждого Объект Из МассивОбъектов Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Если НужноПечататьДопТаблицу Тогда
				...
				ОбластьОтчетПоСотруднику.Параметры.ОбщаяЗаработнаяПлата = 0;
				ОбластьОтчетПоСотруднику.Параметры.ОбщаяОперацияоннаяКасса = 0;
				ОбластьОтчетПоСотруднику.Параметры.ОбщийПодотчет = 0;
				ОбластьОтчетПоСотруднику.Параметры.ПредставлениеПодразделения = Выборка.ПредставлениеПодразделения;
				ОбластьОтчетПоСотруднику.Параметры.ЗаработнаяПлата = 0;
				ОбластьОтчетПоСотруднику.Параметры.ОперацияоннаяКасса = 0;
				ОбластьОтчетПоСотруднику.Параметры.Подотчет = 0;
				...
				|ИЗ
				|	РегистрБухгалтерии.Управленческий.Остатки(, Счет В (&СписокСчетов), &ВидСубконтоСотрудник, (ВЫРАЗИТЬ(Субконто1 КАК Справочник.ФизическиеЛица)) = &Сотрудник) КАК УправленческийОстатки
				|ИТОГИ
				|	СУММА(СуммаОстаток)
				|ПО
				|	Счет";
				...
				Пока ВыборкаПоГруппам.Следующий() Цикл
					...
					Если ВыборкаПоГруппам.Счет = ПланыСчетов.Управленческий.ВзаиморасчетыССотрудниками Тогда
						
						ОбластьОтчетПоСотруднику.Параметры.ОбщаяЗаработнаяПлата = ВыборкаПоГруппам.СуммаОстаток;
						
					ИначеЕсли ВыборкаПоГруппам.Счет = ПланыСчетов.Управленческий.ОперационнаяКасса Тогда
						
						ОбластьОтчетПоСотруднику.Параметры.ОбщаяОперацияоннаяКасса = ВыборкаПоГруппам.СуммаОстаток;
						
					ИначеЕсли ВыборкаПоГруппам.Счет = ПланыСчетов.Управленческий.Подотчет Тогда
						
						ОбластьОтчетПоСотруднику.Параметры.ОбщийПодотчет = ВыборкаПоГруппам.СуммаОстаток;
					...
					Пока ДетальнаяВыборка.Следующий() Цикл
						...
						Если ДетальнаяВыборка.Подразделение = Выборка.ПредставлениеПодразделения Тогда
							
							Если ДетальнаяВыборка.Счет = ПланыСчетов.Управленческий.ВзаиморасчетыССотрудниками Тогда
								
								ОбластьОтчетПоСотруднику.Параметры.ЗаработнаяПлата = ДетальнаяВыборка.СуммаОстаток;
								
							ИначеЕсли ДетальнаяВыборка.Счет = ПланыСчетов.Управленческий.ОперационнаяКасса Тогда
								
								ОбластьОтчетПоСотруднику.Параметры.ОперацияоннаяКасса = ДетальнаяВыборка.СуммаОстаток;
								
							ИначеЕсли ДетальнаяВыборка.Счет = ПланыСчетов.Управленческий.Подотчет Тогда
								
								ОбластьОтчетПоСотруднику.Параметры.Подотчет = ДетальнаяВыборка.СуммаОстаток;
