...
Функция ПечатьКассовыйОрдер(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для каждого Объект Из МассивОбъектов Цикл
		...
		|	РасходДенежныхСредств.СчетДт.ФискальныйКод КАК СубСчет,
		|	РасходДенежныхСредств.СчетКт.ФискальныйКод КАК КредитСубСчет,
		...
		|	РасходДенежныхСредств.Автор.ФизическоеЛицо.Наименование КАК ФИОКассира
		|ИЗ
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(Выборка.Субконто1Дт) = Тип("СправочникСсылка.Контрагенты")
				ИЛИ ТипЗнч(Выборка.Субконто1Дт) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
			...
			Если НужноПечататьДопТаблицу Тогда
				...
				Пока ВыборкаПоГруппам.Следующий() Цикл
					...
					ДетальнаяВыборка = ВыборкаПоГруппам.Выбрать();
					...
					ОбластьОтчетПоСотруднику.Параметры.ЗначениеПодразделения = Неопределено;
					Пока ДетальнаяВыборка.Следующий() Цикл
						Если ДетальнаяВыборка.Подразделение = Выборка.Подразделение Тогда
...
Функция ПолучитьПредставлениеОрганизации(Ссылка)
	
	Если ТипЗнч(Ссылка.Субконто2Дт) = Тип("ДокументСсылка.ДополнительноеСоглашение") Тогда
		Результат = Ссылка.Субконто2Дт.Договор.Организация;
	Иначе
		Результат = Ссылка.Подразделение.Организация;
	КонецЕсли;
