...
Процедура ВидОперацииНаСервере()
	...
	Подотчет = Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОпераций.ВыдачаПодотчетномуЛицу");
	...
	Если Подотчет Тогда
		...
		Объект.Субконто1Дт = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенежныхСредств.ВыплатаПодотчета");
		Объект.Субконто1Кт = ПредопределенноеЗначение("Справочник.СтатьиДвиженияДенежныхСредств.ПоступлениеПодотчета");
