...
//Функция ПолучитьСкидкуПостоянногоКлиента(Дата, Контрагент, Проведен = ИСТИНА)
...
Процедура ПересчитатьСкидку()
	...
	Объект.ПроцентСкидки = ЛексСервер.ПолучитьСкидкуДоговора(Объект.Подразделение, Объект.Дата, Объект.СуммаДокументаБезСкидки, Объект.ВидОплатыДоговора, Объект.Офис);
	//Объект.ПроцентСкидки = Объект.ПроцентСкидки + ПолучитьСкидкуПостоянногоКлиента(Объект.Дата, Объект.Контрагент);
