...
Процедура ПересчитатьСкидку()
	...
	Если СкидкаПостоянногоКлиента > 0 И ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ЭтаФорма.Элементы.СкидкаПостоянномуКлиенту.Видимость = Истина;  
	Иначе
		ЭтаФорма.Элементы.СкидкаПостоянномуКлиенту.Видимость = Ложь; 
	КонецЕсли
...
//	СтруктураСкидка = ЛексСервер.ПолучитьСкидкуДоговора(Объект.Подразделение, Объект.Дата, Объект.СуммаДокументаБезСкидки, Объект.ВидОплатыДоговора, Объект.Офис, Объект.Контрагент);
