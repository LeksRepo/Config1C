...
Процедура ПересчитатьСкидку()
	...
	Если Объект.ПостоянныйКлиент Тогда		
		СтруктураСкидка = ЛексСервер.ПолучитьСкидкуДоговора(Объект.Подразделение, Объект.Дата, Объект.СуммаДокументаБезСкидки, ВидОплаты, Объект.Офис, Контрагент, Объект.Ссылка, Ложь, Истина);
	Иначе
		СтруктураСкидка = ЛексСервер.ПолучитьСкидкуДоговора(Объект.Подразделение, Объект.Дата, Объект.СуммаДокументаБезСкидки, ВидОплаты, Объект.Офис, Контрагент, Объект.Ссылка);	
	КонецЕсли;
