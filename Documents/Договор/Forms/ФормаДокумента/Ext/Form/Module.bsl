...
Процедура ПересчитатьСкидку()
	...
	Если СкидкаПостоянногоКлиента > 0 И ЗначениеЗаполнено(Объект.Контрагент) Тогда
		ЭтаФорма.Элементы.ПроцентСкидки.Заголовок = ЭтаФорма.Элементы.ПроцентСкидки.Заголовок + "+ 2%(постоянный клиент)";  
	Иначе
		ЭтаФорма.Элементы.ПроцентСкидки.Заголовок="Процент скидки"; 
	КонецЕсли
