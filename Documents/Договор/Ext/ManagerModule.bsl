...
Функция РасчетПени(Договор, Дата) Экспорт
	...
	|	И ПриходДенежныхСредств.Дата < &ТекущаяДата";
		...
		//|	И ПриходДенежныхСредств.Дата МЕЖДУ КОНЕЦПЕРИОДА(ПриходДенежныхСредств.Дата, ДЕНЬ) И &ТекущаяДата";
	...
	Для каждого Стр Из ТЗ Цикл
		...
		Если НачислятьПеню Тогда
			...
			Если ЗначениеЗаполнено(Стр.Платеж) И НЕ ПервыйПлатеж Тогда
