...
Функция ПечатьАктПередачи(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|					ИЛИ СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ВыездМастераНаСборкуКухни)
	...
	Пока Выборка.Следующий() Цикл
		...
		ОбластьТело.Параметры.СуммаНаряда = Выборка.ВыездМастера + Выборка.СборкаИзделия + Выборка.ПроездМонтажникаЗаГородом;
...
Функция СпецификацияПроверена(СпецификацияСсылка) Экспорт
	...
	Если Найти(СтрокаСоединения, "TS305") Тогда
	...
	Если Час(ТекущаяДата()) < ВремяБезПроверки Тогда //часовой пояс в базе - Москва Если Час(ТекущаяДата()) < 18 Тогда
		...
		ЗапрещенныеСтатусы.Добавить(Перечисления.СтатусыСпецификации.ПроверяетсяМатериал);
		ЗапрещенныеСтатусы.Добавить(Перечисления.СтатусыСпецификации.МатериалПроверен);
