...
Функция ПечатьИзделийПоКаталогу(Выборка, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		|	ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВидФайла КАК РасположениеКартинки
		...
		|			И (ВЫБОР
		|				КОГДА (ФайлыКаталога.РасположениеКартинки = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаПравая)
		|						ИЛИ ФайлыКаталога.РасположениеКартинки = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаЛевая)
		|						ИЛИ ФайлыКаталога.РасположениеКартинки = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ОсновнаяКартинка))
		|				ТОГДА СпецификацияСписокИзделийПоКаталогу.РасположениеИзделия = ФайлыКаталога.РасположениеКартинки
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		|ГДЕ
...
Функция ПечатьИзделийШкафПоКаталогу(Выборка, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		|					ТОГДА ФайлыКаталога.РасположениеКартинки = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаЛевая)
		|				КОГДА СпецификацияСписокИзделийПоКаталогу.ВидИзделия = ЗНАЧЕНИЕ(Справочник.ВидыИзделийПоКаталогу.ПравыйБоковойЭлемент)
		|					ТОГДА ФайлыКаталога.РасположениеКартинки = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаПравая)
		|				ИНАЧЕ ИСТИНА
		|			КОНЕЦ)
		|ГДЕ
		|	СпецификацияСписокИзделийПоКаталогу.Ссылка В(&Ссылка)";
