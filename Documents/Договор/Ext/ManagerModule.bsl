...
Функция ПечатьДоговора(Выборка, ОбъектыПечати) // МассивОбъектов, ОбъектыПечати, ИмяМакета, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж Тогда
			...
			ПунктСборка = "1.12. %3, без взимания дополнительной платы, приступает к сборке Мебельного комплекта %1. Сборка осуществляется согласно ТПМК и Инструкции по сборке мебельного комплекта. %3 передает Мебельный комплект в собранном виде и комплектности согласно ТПМК";
			ПунктСборка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПунктСборка, ПриступаетКСборке, УстановитьНеПозднее, СтороныДоговора.ПродавецИсполнитель);
...
Функция РасчетПени(МассивДоговоров, Дата) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	КОНЕЦПЕРИОДА(ДоговорРассрочка.Дата, МЕСЯЦ) КАК Дата,
	|	ДоговорРассрочка.Сумма КАК Рассрочка,
	|	0 КАК Платеж,
	|	ДоговорРассрочка.Дата КАК СрокОплаты,
	|	NULL КАК ДатаОплаты
	...
	|
	|ВЫБРАТЬ
	|	КОНЕЦПЕРИОДА(УправленческийДвиженияССубконто.Регистратор.Дата, МЕСЯЦ),
	|	0,
	...
	|	МАКСИМУМ(ЕСТЬNULL(ТаблицаСроковИПлатежей.СрокОплаты, ТаблицаСроковИПлатежей.ДатаОплаты)) КАК СрокОплаты,
	...
	//ТЗ.Колонки.Добавить("Рассрочка");
	//ТЗ.Колонки.Добавить("Платеж");
	//ТЗ.Колонки.Добавить("СрокОплаты");
	//ТЗ.Колонки.Добавить("ДатаОплаты");
	СтрокаСегодня 						= ТЗ.Добавить();
	СтрокаСегодня.Дата 				= Дата;
	СтрокаСегодня.СрокОплаты 	= Дата;
	СтрокаСегодня.Рассрочка 		= 0;
	СтрокаСегодня.Платеж 			= 0;
	...
	Для каждого Стр Из Выборка Цикл
		...
		Если СчетчикЦиклов = 0 Тогда
			
			ПрошлаяДата = Стр.СрокОплаты;
			
		КонецЕсли;
		
		СчетчикЦиклов = СчетчикЦиклов + 1;
		
		Если КонецМесяца(Дата) - КонецМесяца(Стр.СрокОплаты) <> 0 Тогда
			
			ОбрабатываемаяДата = НачалоДня(Стр.СрокОплаты);
			
		Иначе
			
			ОбрабатываемаяДата = Дата;
		...
		Если НЕ ЗначениеЗаполнено(Стр.Платеж) И НЕ ПервыйПлатеж Тогда
			...
			ДатаПервыйПлатеж 	= Формат(ОбрабатываемаяДата, "ДФ=dd.MM.yyyy");
			ПервыйПлатеж 			= Истина;
	...
	Структура.Вставить("ПервыйПросроченныйПлатеж", ДатаПервыйПлатеж);
...
Функция СпецификацияПроверена(СпецификацияСсылка) Экспорт
	...
	Если Час(ТекущаяДата()) < 18 Тогда
		...
		ЗапрещенныеСтатусы.Добавить(Перечисления.СтатусыСпецификации.ПроверяетсяИнженером);
		...
		Для каждого Статус Из ЗапрещенныеСтатусы Цикл
			...
			Если Статус = ТекущийСтатус Тогда
				ТекстСообщения = "Для заключения договора, %1 должна быть проверена технологом расчетного центра";
