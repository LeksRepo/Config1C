...
Функция ПолучитьОсновнойМатериал(Спецификация, НомГруппа)
	...
	Запрос.Текст =
		"ВЫБРАТЬ ПЕРВЫЕ 2
		|	СпМат.Номенклатура КАК Номенклатура,
		|	СУММА(СпМат.ВысотаДетали * СпМат.ШиринаДетали * СпМат.Количество) КАК Сум
		|ИЗ
		|	Документ.Спецификация.СписокМатериалы КАК СпМат
		|ГДЕ
		|	СпМат.Ссылка = &Спецификация
		|	И (СпМат.Номенклатура.НоменклатурнаяГруппа.Родитель = &НомГруппа ИЛИ СпМат.Номенклатура.НоменклатурнаяГруппа = &НомГруппа) 
		|
		|СГРУППИРОВАТЬ ПО
		|	СпМат.Номенклатура
		|
	...
	Пока ВыборкаМат.Следующий() Цикл
		...
		Мас.Добавить(ВыборкаМат.Номенклатура);
...
Функция ПолучитьНоменклатуруИзКомплектации(Спецификация, НомГруппа)
	...
	Запрос.Текст =
		...
		|	СУММА(СпМат.ВысотаДетали * СпМат.ШиринаДетали * СпМат.Количество) КАК Сум
		|ИЗ
		|	Документ.Спецификация.Комплектация КАК Комплектация
		|ГДЕ
		|	Комплектация.Ссылка = &Спецификация
		|	И (Комплектация.Номенклатура.НоменклатурнаяГруппа.Родитель = &НомГруппа ИЛИ Комплектация.Номенклатура.НоменклатурнаяГруппа = &НомГруппа) 
		|
		|СГРУППИРОВАТЬ ПО
...
Функция ПечатьЭскизнаяЗаявка(Выборка, ОбъектыПечати)
	...
	ПараметрыМакета.Вставить("Ручки","----");
	...
	ПараметрыМакета.Вставить("Примечание","----");
	
	МасЛДСП=ПолучитьОсновнойМатериал(Спецификация, Справочники.НоменклатурныеГруппы.ЛДСП);
	Если МасЛДСП.Количество()=2 Тогда
		ПараметрыМакета.ОсновнойЛДСП=МасЛДСП[0];
		ПараметрыМакета.ДополнительныйЛДСП=МасЛДСП[1];
	КонецЕсли;
	Если МасЛДСП.Количество()=1 Тогда
		ПараметрыМакета.ОсновнойЛДСП=МасЛДСП[0];	
	КонецЕсли;
	
	МасДВП=ПолучитьОсновнойМатериал(Спецификация, Справочники.НоменклатурныеГруппы.ДВП);
	Если МасДВП.Количество()=1 Тогда
		ПараметрыМакета.ОсновнойДВП=МасДВП[0];
	КонецЕсли;
	
	МасМДФ=ПолучитьОсновнойМатериал(Спецификация, Справочники.НоменклатурныеГруппы.МДФ);
	Если МасМДФ.Количество()=1 Тогда
		ПараметрыМакета.ОсновнойМДФ=МасМДФ[0];
	КонецЕсли;
	...
	Для каждого Файл Из МассивФайлов Цикл
		...
		Если (НЕ Файл.ПометкаУдаления) И (Файл.ВидФайла=Перечисления.ВидыПрисоединенныхФайлов.Эскиз) Тогда
