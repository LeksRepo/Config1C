...
Функция ПечатьДоговора(Выборка, ОбъектыПечати) // МассивОбъектов, ОбъектыПечати, ИмяМакета, 
	...
	Пока Выборка.Следующий() Цикл
		...
		ПунктДоставка = "1.11. Продавец осуществляет доставку деталей Мебельного комплекта %1. (время может быть изменено) по адресу: %2";
		ПунктДоставка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПунктДоставка, ДатаОтгрузки, Выборка.Адрес);
		...
		ОбластьТело.Параметры.Заполнить(РеквизитыОрганизации);
		...
		ТабДок.Вывести(СформироватьОбластьШапкаДоговора(Выборка.Ссылка));
...
Функция СформироватьОбластьШапкаДоговора(ДоговорСсылка)
