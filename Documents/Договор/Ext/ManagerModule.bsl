...
Функция ВывестиОбластьШапкаДоговора(ТабДок, ДоговорСсылка) Экспорт
	...
	Область = Макет.ПолучитьОбласть("ШапкаДоговора");
	 ...
	 ФизическоеЛицо = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(ДоговорСсылка.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
	Доверенность = ЛексСервер.ПолучитьДоверенностьСотрудника(ФизическоеЛицо, ДоговорСсылка.Организация);
	...
	Область.Параметры.АвторРодительныйПадеж = ФизическоеЛицо.ВЛице;
...
Функция ВывестиОбластьРеквизитыДоговора(ТабДок, ДоговорСсылка) Экспорт
	...
	Дизайнер = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(ДоговорСсылка.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
...
Функция ПечатьАктПередачи(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	МАКСИМУМ(докДоговор.Спецификация.Подразделение.Организация.ПолноеНаименование) КАК ОрганизацияИзготовитель,
	...
	|	МАКСИМУМ(докДоговор.Спецификация.Подразделение.Организация) КАК Изготовилеть
	|ИЗ
	|	Документ.Договор КАК докДоговор
...
Функция РасчетПени(МассивДоговоров, Дата) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СуммаПени > 0 Тогда
			...
			ФизическоеЛицо = ЛексСервер.ПолучитьЗначениеНастройкиПользователя(СтруктураРеквизитовДоговора.Автор, ПланыВидовХарактеристик.НастройкиПользователей.ФизЛицо);
			НоваяСтрока.Автор = ФизическоеЛицо;
