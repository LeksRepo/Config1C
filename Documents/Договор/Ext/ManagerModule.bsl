...
Функция ПечатьАктПередачи(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	СУММА(Договор.СуммаДокумента) КАК СуммаДокумента,
	...
	|	МИНИМУМ(Договор.Спецификация.КоличествоМетровЛДСП) КАК КоличествоМетровЛДСП,
	...
	|	МАКСИМУМ(Договор.Ссылка) КАК Ссылка,
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.СборкаИзделия)
