...
Функция ПечатьИзделийПоКаталогу(Выборка, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		|		И СпецификацияСписокИзделийПоКаталогу.Расположение = ФайлыКаталога.Расположение
		|ГДЕ
...
Функция ПечатьИзделийШкафПоКаталогу(Выборка, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		|				ИНАЧЕ ФайлыКаталога.Расположение = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ОсновнаяКартинка)
		|			КОНЕЦ)
		|ГДЕ
		|	СпецификацияСписокИзделийПоКаталогу.Ссылка В(&Ссылка)";
