...
Функция ПечатьДоговора(Выборка, ОбъектыПечати) // МассивОбъектов, ОбъектыПечати, ИмяМакета, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ПакетУслуг = Перечисления.ПакетыУслуг.ДоставкаДоКлиентаИМонтаж Тогда
			...
			ПунктСборка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПунктСборка, ПриступаетКСборке, УстановитьНеПозднее, СтороныДоговора.ПродавецИсполнитель, Формат(ДатаУстановитьНеПозднее, "ДЛФ=DD"));
