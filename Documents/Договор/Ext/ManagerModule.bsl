...
Функция ПечатьДоговора(Выборка, ОбъектыПечати) // МассивОбъектов, ОбъектыПечати, ИмяМакета, 
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			ВЛицеВтораяСторона 					= СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(", в лице %1, действущего на основании %2, ", Выборка.РуководительОрганизации, Выборка.ДействуетНаОсновании);
		...
		РеквизитыОрганизации 	= ЛексСервер.ПолучитьРеквизитыОрганизации(ОрганизацияДляДоговора);
...
Функция Запрос(МассивОбъектов)
	...
	|	Договор.Спецификация.УслугаМонтаж КАК Монтаж,
	|	Договор.Спецификация.Доставка КАК Доставка,
