...
Функция ПечатьАктПередачи(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	МАКСИМУМ(Спецификация.ДовозОсуществил) КАК Экспедитор,
	|	МАКСИМУМ(Спецификация.Подразделение.Организация.ПолноеНаименование) КАК ОрганизацияИзготовилеть,
	|	МАКСИМУМ(Спецификация.Монтажник) КАК Монтажник,
	...
	|	МАКСИМУМ(Спецификация.Изделие) КАК Изделие,
	|	МАКСИМУМ(Спецификация.АдресМонтажа) КАК АдресМонтажа,
	...
	|	МАКСИМУМ(Спецификация.КоличествоМетровЛДСП) КАК КоличествоМетровЛДСП,
	|	МАКСИМУМ(Договор.Организация.ПолноеНаименование) КАК ОрганизацияПродавец,
	|	МАКСИМУМ(Договор.Офис.Телефон) КАК ТелефонОфиса,
	|	МАКСИМУМ(ВЫРАЗИТЬ(Спецификация.ДокументОснование КАК Документ.Замер).Замерщик) КАК Замерщик,
	|	СУММА(ВЫБОР
	...
	|	МАКСИМУМ(Спецификация.Подразделение.Организация) КАК Изготовилеть
	|ИЗ
	|	Документ.Договор КАК Договор
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Спецификация КАК Спецификация
	|		ПО (Спецификация.Ссылка = Договор.Спецификация)
