...
Функция ПечатьУведомлениеОЗадолженности(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗапросОплаты.Текст =
		"ВЫБРАТЬ
		|	ПриходДенежныхСредств.Сумма КАК СуммаПлатежа,
...
Функция ПечатьИзделийПоКаталогу(Выборка, ОбъектыПечати)
	...
	Пока Выборка.Следующий() Цикл
		...
		|	ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВидФайла КАК Расположение
		...
		|	И (ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаПравая)
		|		ИЛИ ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.КартинкаЛевая)
		|		ИЛИ ХранимыеФайлыВерсий.ВерсияФайла.Владелец.ВидФайла = ЗНАЧЕНИЕ(Перечисление.ВидыПрисоединенныхФайлов.ОсновнаяКартинка))		
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗРЕШЕННЫЕ
...
Функция ПечатьАктПередачи(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	СУММА(ВЫБОР
	|			КОГДА СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ТарифЗаСборку)
	|					ИЛИ СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ТарифЗаСборкуКухни)
	...
	|			И (СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ТарифЗаСборку)
	|				ИЛИ СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ТарифЗаСборкуКухни)
	|				ИЛИ СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.СборкаИзделия)
	|				ИЛИ СпецификацияСписокНоменклатуры.Номенклатура = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПроездМонтажникаЗаГородом))
	|ГДЕ
	|	докДоговор.Ссылка В(&Ссылка)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ПечатьЧертежДвери(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Для каждого Дверь Из МассивОбъектов.Двери Цикл
