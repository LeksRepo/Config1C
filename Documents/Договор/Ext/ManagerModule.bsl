	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаПоДоговорам.Следующий() Цикл
			...
			Если СчетчикЦиклов = 0 Тогда
				...
				ПрошлаяДата = Выборка.СрокОплаты;
				
			КонецЕсли;
			
			СчетчикЦиклов = СчетчикЦиклов + 1;
			
			Если КонецМесяца(Дата) - КонецМесяца(ВыборкаПоДоговорам.СрокОплаты) <> 0 Тогда
				
				ОбрабатываемаяДата = НачалоДня(ВыборкаПоДоговорам.СрокОплаты);
		...
		Если СуммаПени > 0 Тогда
			...
			НоваяСтрока.Автор = СтруктураРеквизитовДоговора.Автор;
			...
			НоваяСтрока.ДнейПросрочки = ДнейПросрочки;
