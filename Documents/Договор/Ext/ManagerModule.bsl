...
Функция ВывестиОбластьШапкаДоговора(ТабДок, ДоговорСсылка) Экспорт
	...
	Если ДоговорСсылка.Спецификация.Контрагент.ЮридическоеЛицо Тогда
		...
		КонецЕсли;
		ВЛицеВтораяСторона = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(", в лице %1, действущего на основании %2, ", РуководительРодительныйПадеж, ДоговорСсылка.Спецификация.Контрагент.ДействуетНаОсновании);
	Иначе
		ВЛицеВтораяСторона = "";
	КонецЕсли;
...
Функция ПечатьАктПередачиДилера(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Макет = Документы.Договор.ПолучитьМакет("АктПередачиДилера");
	ОбластьТело = Макет.ПолучитьОбласть("Тело");
	...
	|	ДоговорДилера.Спецификация.ПлощадьСборкиИзделия КАК ПлощадьСборкиИзделия
	|ИЗ
	|	Документ.ДоговорДилера КАК ДоговорДилера
	|ГДЕ
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "АктПередачиДилера") Тогда
		...
		ПодготовитьПечатнуюФорму("АктПередачиДилера", "Акт передачи мебельного комплекта", "",
		МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода, Выборка);
...
Процедура ПодготовитьПечатнуюФорму(Знач ИмяМакета, ПредставлениеМакета, ПолныйПутьКМакету = "", МассивОбъектов, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода, Выборка) Экспорт
	...
	ИначеЕсли ИмяМакета = "АктПередачиДилера" Тогда
		
		ТабДок = ПечатьАктПередачиДилера(МассивОбъектов, ОбъектыПечати);
