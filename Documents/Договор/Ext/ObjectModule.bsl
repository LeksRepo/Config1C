...
Функция ПроверитьШкафКупе()
	...
	//RonEXI: Теперь все изделия отправляются на рассчет если есть договор, даже каталожные.
	Возврат Перечисления.СтатусыСпецификации.Рассчитывается;
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если НЕ Спецификация.Проведен Тогда
		...
		Попытка
			...
			Если ЗначениеЗаполнено(ОбъектСпецификация.ДатаМонтажа) Тогда
				 ОбъектСпецификация.ПервичнаяДатаМонтажа = ОбъектСпецификация.ДатаМонтажа; 
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если НЕ ЗначениеЗаполнено(Спецификация.ПакетУслугИзделия) Тогда
		Отказ= Истина;
		ТекстСообщения = "Укажите пакет услуг изделия в " + Спецификация;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ДанныеЗаполнения)
		...
		ПакетУслугИзделия = ДанныеЗаполнения.ПакетУслугИзделия;
		
		Если НЕ ЗначениеЗаполнено(ПакетУслугИзделия) Тогда
			
			СтандартнаяОбработка = Ложь;
			Отказ = Истина;
			ТекстСообщения = "Укажите пакет услуг изделия";
